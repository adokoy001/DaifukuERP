...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'daifuku-deploy-test-')), release = join(root, 'release'), state = join(root, 'state');
  await mkdir(release); await mkdir(state, { mode: 0o700 }); await mkdir(join(release, 'web'));
  await writeFile(join(release, 'web/index.html'), 'synthetic SPA');
  const manifest = { format: 1, node: process.version, commit: 'a'.repeat(40), candidate: true, platform: 'linux', arch: process.arch, apiBase: '/api', files: await inventory(release) };
  const body = JSON.stringify(manifest); await writeFile(join(release, 'manifest.json'), body);
  ...
  return { root, release, state, config, manifestHash: digest(body), output: join(root, 'profile'), node: process.execPath, user: userInfo().username, hostname: 'erp.example.test', profile: 'cloud', allowCandidate: true };
}
test('verifies exact files, modes, internal links and approved manifest, rejecting tampering', async () => {
  ...
  try {
    ...
    await chmod(join(f.release, 'web/index.html'), 0o600); await assert.rejects(verifyRelease(f.release, f.manifestHash));
    await symlink(f.config, join(f.release, 'escape')); await assert.rejects(inventory(f.release));
    for (const path of ['.env', 'apps/api/.env.local', '.data/files', 'backups/db.dump', 'secret.key', '.npmrc']) assert.equal(restrictedSource(path), true);
...
test('plan makes no files; explicit rendering creates only its new profile and private evidence, never services or secrets', async () => {
  ...
  try {
    ...
    const plan = await renderProfile(f); assert.equal(plan.servicesStarted, false);
    await assert.rejects(lstat(f.output)); await assert.rejects(lstat(join(f.state, 'evidence')));
    await renderProfile({ ...f, execute: true });
    const unit = await readFile(join(f.output, 'daifuku-api.service'), 'utf8');
    assert.match(unit, /--env-file=/); assert.doesNotMatch(unit, /EnvironmentFile|synthetic-never/);
...
test('both profiles share byte-identical release; candidate, secret permissions, path injection and crossing operations are refused', async () => {
  ...
  try {
    ...
    const local = await renderProfile({ ...f, profile: 'onprem', cert: '/etc/ssl/erp.crt', key: '/etc/ssl/erp.key', hostname: 'erp.office.example' });
    ...
    for (const hostname of ['example.test { malicious }', '192.0.2.1', 'erp.local', 'erp.internal', 'erp.localhost', 'erp.-invalid.test', '0x7f.0.0.1']) await assert.rejects(renderProfile({ ...f, hostname }));
    await assert.rejects(renderProfile({ ...f, output: join(f.state, 'overwrite') }));
    await chmod(f.config, 0o644); await assert.rejects(renderProfile(f));
  } finally { await rm(f.root, { recursive: true, force: true }); }
});
...
test('runtime copies the installed production graph exactly, preserves portable links and omits unrelated dev packages', async () => {
  const root = await mkdtemp(join(tmpdir(), 'daifuku-runtime-test-')), stage = join(root, 'stage'), output = join(root, 'runtime');
  try {
    const api = join(stage, 'apps/api'), dep = join(stage, 'node_modules/.pnpm/dep@1.2.3/node_modules/dep');
    await mkdir(join(api, 'node_modules'), { recursive: true }); await mkdir(dep, { recursive: true });
    for (const name of ['package.json', 'pnpm-lock.yaml', 'pnpm-workspace.yaml', 'tsconfig.json']) await writeFile(join(stage, name), '{}');
    await writeFile(join(api, 'package.json'), JSON.stringify({ name: 'api', version: '1', dependencies: { dep: '^1.0.0' }, devDependencies: { unused: '*' } }));
    await writeFile(join(dep, 'package.json'), JSON.stringify({ name: 'dep', version: '1.2.3', peerDependencies: { optionalPeer: '*' } }));
    ...
    assert.deepEqual(packages.map((p) => [p.name, p.version]), [['api', '1'], ['dep', '1.2.3']]);
    assert.equal(JSON.parse(await readFile(join(output, 'apps/api/node_modules/dep/package.json'))).version, '1.2.3');
    await inventory(output);
    assert.equal(JSON.parse(await readFile(join(output, 'node_modules/.pnpm/node_modules/dep/package.json'))).version, '1.2.3');
...
test('real main/worker Web build includes React/TanStack full licenses and fixed versions, without backend dependencies', async () => {
  const stage = fileURLToPath(new URL('../../', import.meta.url)), root = await mkdtemp(join(tmpdir(), 'daifuku-web-notices-')), output = join(root, 'web');
  try {
    ...
    execFileSync('pnpm', ['--filter', '@daifuku/web', 'exec', 'vite', 'build', '--outDir', output], { cwd: stage, env, stdio: 'pipe', maxBuffer: 8 * 1024 * 1024 });
    ...
    for (const name of ['react', 'react-dom', '@tanstack/react-query', '@tanstack/react-router', '@tanstack/react-table', 'tailwindcss']) {
    ...
    for (const name of ['LICENSE', 'THIRD_PARTY_NOTICES.txt', 'web-packages.json']) assert.ok(files.some((file) => file.path === name && file.bytes > 0 && file.sha256));
  } finally { await rm(root, { recursive: true, force: true }); }
});

test('Web notices retain all contributing package NOTICEs and reject missing/empty licenses before emission', async () => {
  const root = await mkdtemp(join(tmpdir(), 'daifuku-web-license-test-')), stage = join(root, 'stage'), output = join(root, 'output');
  try {
    const web = join(stage, 'apps/web'), dep = join(web, 'node_modules/frozen'), child = join(dep, 'node_modules/child');
    await mkdir(child, { recursive: true }); await mkdir(join(output, '.license-inputs'), { recursive: true });
    ...
    await writeFile(join(css, 'package.json'), JSON.stringify({ name: 'tailwindcss', version: '4.0.0', license: 'MIT' }));
    ...
    await writeFile(join(output, '.license-inputs', 'a'.repeat(64) + '.json'), JSON.stringify(['apps/web/node_modules/frozen/index.js', 'apps/web/node_modules/frozen/node_modules/child/index.js']));
    ...
    const text = await readFile(join(output, 'THIRD_PARTY_NOTICES.txt'), 'utf8');
    for (const required of ['synthetic attribution must survive minification', 'synthetic license A', 'synthetic license B']) assert.ok(text.includes(required));
  } finally { await rm(root, { recursive: true, force: true }); }
});

test('Web release refuses absent build evidence or an input outside its frozen source', async () => {
  const root = await mkdtemp(join(tmpdir(), 'daifuku-web-evidence-test-')), stage = join(root, 'stage'), output = join(root, 'web');
  try {
