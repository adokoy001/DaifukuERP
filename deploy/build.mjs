...
async function prepareSource(source, temporary, env) {
  ...
  if (
    ...
    git('show', 'HEAD:.npmrc').trim() !== 'auto-install-peers=true\nstrict-peer-dependencies=false'
  )
...
export async function buildRelease({ source, output, candidate = false, progress = () => undefined }) {
  ...
  output = safePath(output);
  if (!process.version.startsWith('v22.') || process.platform !== 'linux' || !['x64', 'arm64'].includes(process.arch))
    throw new Error('Build on the Linux target architecture.');
  ...
  try {
    ...
    if (run('pnpm', ['--version'], stage, env).trim() !== packageInfo.packageManager?.replace(/^pnpm@/, ''))
      throw new Error('Use the release-pinned pnpm version.');
    const bundle = join(temporary, 'bundle');
    await mkdir(bundle);
    await cp(stage, join(bundle, 'source'), { recursive: true, verbatimSymlinks: true });
    progress('offline-dependencies');
    run('pnpm', ['install', '--offline', '--frozen-lockfile', '--ignore-scripts'], stage, env);
    progress('web');
    run('pnpm', ['--filter', '@daifuku/web', 'build'], stage, env);
    ...
    for (const file of ['edge.mjs', 'LICENSE', 'THIRD_PARTY_NOTICES.txt'])
      await cp(join(stage, 'apps/edge/dist', file), join(bundle, 'edge', file));
    const manifest = {
      format: 1,
      commit,
      candidate,
      ...
      apiBase: '/api',
      lockSha256: digest(await readFile(join(stage, 'pnpm-lock.yaml'))),
    ...
    const archive = join(
      output,
      `DaifukuERP-${commit.slice(0, 12)}-${process.arch}${candidate ? '-candidate' : ''}.tar.gz`,
    );
    await cp(packed, join(output, '.release.tmp'), { errorOnExist: true, force: false });
    await link(join(output, '.release.tmp'), archive);
    await rm(join(output, '.release.tmp'));
