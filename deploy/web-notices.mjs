...
async function packageForInput(stage, input) {
  ...
  if (!within(stage, actual) || !relative(stage, actual).split('/').includes('node_modules'))
    throw new Error('Web license input escapes installed dependencies.');
...
async function packageNotices(directory, metadata) {
  ...
  for (const file of files) {
    ...
    if (!within(directory, actual) || !(await lstat(actual)).isFile())
      throw new Error('Invalid Web dependency notice: ' + label);
...
export async function writeWebNotices(stage, destination) {
  ...
  for (const file of files) {
    ...
    for (const input of inputs) {
      ...
      packages.set(directory, {
        ...
        license: metadata.license ?? 'see included license',
  ...
  packages.set(css.directory, {
    name: css.metadata.name,
    version: css.metadata.version,
    license: css.metadata.license,
    path: relative(stage, css.directory),
    notices: cssNotices,
  });
  ...
  const body = [
    'Daifuku Web - third-party notices',
    'Keep this file and LICENSE with the Web distribution. These dependencies contribute to the main/worker bundles, Tailwind CSS pipeline or embedded Feather icons.',
  ];
  ...
  const inventory = sorted.map(({ notices, ...pkg }) => ({
    ...pkg,
