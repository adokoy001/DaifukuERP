...
`runMigrations` を直接使う検証や管理ツールも、書込み元を停止したうえで専用owner接続を用意し、呼出元が接続を閉じます。列の型変更後に同じ接続の古い準備済み照会を再利用せず、データ照合や業務処理は再接続します。前後の値比較は保持し、実際の取得結果を旧型へ戻して期待値に合わせないでください。
