...
新しい勤務制度・給与計算・組織階層を追加するときは、通常制の既存値を都合よく変更するのでなく、適用期間と受入例を持つ契約を追加します。法制度の根拠は [domain文書](../domain/japan-workforce.md)、実装例は [仕様](../specs/workforce-backend-contract.md) と対応試験を確認します。税・社会保険の外部確認額を入力できることと、その額を本機能で自動算定できることを区別します。
