...
- 招待メール・本人のメール再設定、OIDC SSO、TOTP MFA・単回回復コードは [認証とメールの導入・運用](enterprise-identity.md) を参照してください。再設定しても MFA は解除されません。メール配送が未設定なら配信済みにはならず、設定後に招待の再発行や再設定の再申請が必要です。
- MFA の回復コードも認証器も失った場合の管理者向け復旧、必須 MFA ポリシー、端末単位の失効は未対応です。秘密や回復コードを公開 Issue へ書かないでください。
