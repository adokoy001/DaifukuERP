...
- 通常の変数宣言は `const a = ...; const b = ...;` のように個別の文にする。ESLintの `one-var: ['error', 'never']` で検査し、初期化順とスコープを保って自動修正する。分割代入は1つの宣言として使える。ルールが扱わない `for` 初期化部を、形式だけのためにループの外へ移さない。
...
| one-var: never | 宣言ごとに型・初期化・差分を追いやすくする | 初期化順とスコープを保って個別宣言へ |
...
追加の宣言・型・規約調整は [一貫性仕様](../specs/source-consistency.md) を参照する。ルールの根拠（2026-09-14確認）: [ESLint one-var](https://eslint.org/docs/latest/rules/one-var)。
