...
- 状態: approved ／ パッケージ: l10n/jp（`@daifuku/l10n-jp`）／ 依存: kernel, accounting, tax, tax_filing, workforce（sales/purchase には依存せず、**override 名で**接続する: ADR-0008）
...
- AC-6 The package registers a module `l10n_jp` (`defineModule({ name: 'l10n_jp', depends: ['accounting', 'tax', 'tax_filing', 'workforce'], entities: [], hooks, seed })`) with no entities of its own. Country filing profiles and payroll providers register through the public ports of their owning modules; those modules do not import l10n.
...
## 関係するファイル
申告準備と給与制度の追加入口は [給与制度仕様](payroll-rule-versions.md)、[給与構造](../architecture/payroll-automation.md)、[申告準備仕様](tax-filing-preparation.md) を参照。以下は当初の会計・税レイアウトのファイル地図。
