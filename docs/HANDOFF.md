...
5. 変更後は `pnpm gate`、Web 変更時は `pnpm --filter @daifuku/web typecheck` と `build`、該当する画面/E2E も確認する。ブラウザ試験は CONTRIBUTING の専用空 DB 手順に従う。
...
多通貨、国別制度パック、受発注・分納と請求の分離、在庫評価の遡及再計算、外部配信 worker、SSO/MFA・本人パスワード再設定、実製品比較は次の独立した仕様として扱う。円・国内向け試験業務で得た不変条件を、15業界と従業員業務の台本を足場に広げる。
