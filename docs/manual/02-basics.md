...
2026-09-12のUIに合わせて操作説明を更新しました。画像は旧UIです。変更点の要約は「[付録 C](appendix-c-ui-refresh.md)」を参照してください。
...
左側の藍色の帯が **サイドバー**（メニュー）です。「ワークスペース」と業務ごとのメニューに続き、折りたたみの「レポート」「すべてのエンティティ」、「設定」が並びます。「すべてのエンティティ」は表示できる台帳を検索でき、伝票の明細台帳は親の伝票から操作します。左上のロゴまたは「ワークスペース」でホームに戻ります。言語切替「JA / EN」、利用者名とログアウトは左下にあります。スマートフォンでは上部のメニューボタンでサイドバーを開きます。

**ホーム** の上部には作業中・確定済みの伝票件数、利用できるレポート数、新規作成への入口があります。その下の業務カードで伝票ごとの「下書き・確定・取消」件数、台帳、レポートを開けます。件数は実際の記録から取得します。
...
サイドバー下部の「JA」「EN」で、メニュー・項目名・ボタンの言語を切り替えます。入力したデータ（取引先名など）は切り替わりません。一部の項目名（レポートの条件欄の「From」「As Of」、設定画面の細目など）は日本語表示でも英語のままです。
