...
店舗のLANにWindows/Linux/macOSの中継を置き、本部のERPへ外向きに接続します。[OS別セットアップ](edge-service-setup.md)で常駐サービスを導入できます。クラウドとオンプレで同じ中継・管理画面を使います。店舗ルーターの受信ポート開放は不要です。オンプレERPが別拠点から見えない場合は、VPN等でERPのHTTPS接続先へ到達できる構成を用意します。
...
| 店舗設置担当 | 対象OSと耐久ディスクを準備し、[サービスのセットアップ](edge-service-setup.md)で同梱Nodeと常駐サービスを導入 |
