...
「取引先」→「取引先」→「+ 新規」で登録します。顧客も仕入先も同じ台帳で、「顧客」「仕入先」のチェックで区別します（両方に入れてもかまいません）。
...
| 単位 | 個・式・時間・kg・箱 など（「品目」→「単位」で確認・追加） |
...
「在庫」→「倉庫」に、初期状態で「本店（主倉庫）」（コード `MAIN`、既定の倉庫）が入っています。仕入・売上の請求書からの自動入出庫は、設定「既定の倉庫コード」の倉庫で行われます。倉庫を増やすときは「+ 新規」で、倉庫コード・倉庫名を入れます。「既定の倉庫」のチェックは 1 か所だけにしてください。
