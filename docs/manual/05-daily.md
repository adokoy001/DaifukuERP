...
請求書や入出金から自動で作られない取引（元入金、振替など）は「会計・資金」→「仕訳」で入力します。
...
物品の在庫は、仕入請求書・売上請求書の確定で自動で動きます。それ以外の動き（請求書のない入荷、社内使用、倉庫間の移動）は「在庫・商品」→「入出庫」で入力します。
...
**棚卸** は「在庫・商品」→「棚卸」で行います。「倉庫」「棚卸日」を入れ、明細に「品目」と「実棚数量」を入れて保存すると、「帳簿数量」と「差異数量」が表示されます。「確定」すると、差異の分だけ調整の入出庫伝票が自動で作られ、在庫が実地の数量に合わせられます。

![棚卸（帳簿 10 冊、実棚 9 冊、差異 -1）](img/05-stock-count.png)

現在の在庫は「分析・レポート」→「在庫照会」で、品目・倉庫ごとの数量・移動平均単価・在庫金額を確認できます。
