...
この付録は1法人内の店舗運営と会社・店舗への所属管理を対象にしています。別の共通機能として、SSO/MFA・確認メール、Squareの決済仮勘定連携、管理用連結精算表、FC精算を追加しました（[付録H](appendix-h-enterprise-operations.md)）。自由SQLの外部BIと定期配信は含みません。接続・サービス公開・災害復旧・負荷容量の運用判断は、導入先ごとに別途確認します。
