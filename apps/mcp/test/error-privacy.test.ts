...
describe('quality-foundation AC-1: MCP protocol errors', () => {
  it('sanitizes non-tool protocol errors and diagnostic logs', async () => {
    ...
    const log: Logger = {
      info: () => undefined,
      warn: () => undefined,
      error: (message, data) => void logs.push({ message, data }),
    };
    const error = await withProtocolErrors(log, 'resources/read', () =>
      Promise.reject(new Error(`SQL params: ${SECRET}`, { cause: { code: '23503', detail: SECRET } })),
    ).catch((e: unknown) => e);
    ...
    expect(logs).toEqual([
      expect.objectContaining({
        data: expect.objectContaining({ method: 'resources/read', category: 'database', sqlState: '23503' }),
      }),
    ]);
  });
  ...
  it('keeps intentional business errors useful to callers', async () => {
    ...
    await expect(
      withProtocolErrors(log, 'tools/list', () =>
        Promise.reject(new Conflict('Version changed', 'Reload.', { version: 2 })),
      ),
    ).rejects.toMatchObject({ data: { code: 'CONFLICT', hint: 'Reload.', details: { version: 2 } } });
  });
});
