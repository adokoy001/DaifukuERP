...
import { auditTrail, companyMemberships, hashPassword, newId, registerCrudActions, registry, todayLocal, users, type ContextParams, type Logger } from '@daifuku/kernel';
...
async function call(c: Client, name: string, args: Record<string, unknown> = {}): Promise<{ isError: boolean; body: Record<string, unknown> }> {
...
describe('AC-1 startup authentication -> agent context', () => {
  ...
  it('AC-1 DAIFUKU_COMPANY_ID overrides the default company; bad credentials are rejected', async () => {
    ...
    await expect(openAgentSession(db.owner, { ...LOGIN, companyId: newId() }, silent)).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
...
describe('AC-2 tools/list mirrors the action registry', () => {
  it('AC-2 lists one tool per action with derived names, bilingual description, mutates flag and readOnlyHint', async () => {
    ...
    expect(names).toEqual(expect.arrayContaining(['mcp_test_item_list', 'mcp_test_item_create', 'mcp_test_item_update', 'mcp_test_order_submit', 'mcp_test_echo']));

    const list = tools.find((t) => t.name === 'mcp_test_item_list');
    const create = tools.find((t) => t.name === 'mcp_test_item_create');
    expect(list?.description).toBe('List MCP test item records with optional where/search/orderBy/limit/offset.\nMCPテスト品目を検索・一覧します。where/search/orderBy/limit/offset を指定できます。\nmutates: no');
...
describe('AC-3 tools/call runs the action in a context; errors are self-describing', () => {
  ...
  it('AC-3/AC-5 create then list round-trips through the tools', async () => {
    ...
    expect(created.body).toMatchObject({ name: 'Widget', code: 'W-1', price: '12.5', kind: 'goods', isActive: true, version: 1, companyId: db.companyId });
  ...
  it('AC-5 audit rows written through the MCP server carry actorType agent and onBehalfOf the user', async () => {
    ...
    expect(trail[0]).toMatchObject({ op: 'create', actorType: 'agent', actorId: 'test-agent', onBehalfOf: db.adminUserId });
  ...
  it('AC-3 permission denied for a role without the op (default deny, ADR-0007)', async () => {
    ...
    await db.owner.drizzle.insert(users).values({ id: viewerId, tenantId: db.tenantId, email: 'viewer@example.com', name: 'Viewer', passwordHash: hashPassword('viewer-password'), roles: ['viewer'], defaultCompanyId: db.companyId });
    await db.owner.drizzle.insert(companyMemberships).values({ tenantId: db.tenantId, userId: viewerId, companyId: db.companyId, roles: ['viewer'] });
    const viewer = await connectClient({ ...params, actor: { type: 'agent', id: 'viewer-agent', onBehalfOf: viewerId }, roles: ['viewer'] });
    try {
      const denied = await call(viewer.client, 'mcp_test_item_create', { name: 'nope' });
      expect(denied.isError).toBe(true);
      expect(denied.body).toMatchObject({ code: 'PERMISSION_DENIED', details: { entity: 'mcp_test_item', op: 'create', roles: ['viewer'] } });
  ...
  it('AC-3 document lifecycle works through generic tools (create -> submit assigns a number)', async () => {
    const order = await call(client, 'mcp_test_order_create', { itemId: createdId, date: todayLocal(new Date()), amount: '100' });
...
describe('AC-4 resources expose metadata', () => {
  it('AC-4 resources/list has daifuku://meta and one entry per entity; a template covers daifuku://entities/{name}', async () => {
    ...
    expect(resources.map((r) => r.uri)).toEqual(expect.arrayContaining(['daifuku://meta', 'daifuku://entities/mcp_test_item', 'daifuku://entities/mcp_test_order']));
  ...
  it('AC-4 daifuku://meta returns appMeta(ctx) as JSON', async () => {
    ...
    const meta = JSON.parse(textOf(first)) as { entities: { name: string }[]; actions: { name: string }[]; modules: { name: string }[]; roles: string[] };
  ...
  it('AC-4 daifuku://entities/{name} returns entityMeta; unknown entity is a protocol error with a hint', async () => {
    ...
    const meta = JSON.parse(textOf(res.contents[0])) as { name: string; fields: { name: string; kind: string }[]; ops: string[] };
    ...
    await expect(client.readResource({ uri: 'daifuku://entities/nope' })).rejects.toThrow(/entity "nope" does not exist/);
