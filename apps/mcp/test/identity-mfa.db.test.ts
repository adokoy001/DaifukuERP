...
const key = Buffer.alloc(32, 7).toString('base64'), now = new Date(), log: Logger = { info() {}, warn() {}, error() {} };
beforeAll(async () => {
  db = await freshDb();
  ...
  const confirmed = await confirmMfa(db.owner, { userId: db.adminUserId, tenantId: db.tenantId, sessionVersion: 1 }, setup.setupToken, totp(setup.secret, Math.floor(now.getTime() / 30000)), key, now);
...
describe('MCP honors MFA before issuing an agent context', () => {
  it('denies password-only login, verifies one-time recovery and rejects stale or unverified contexts', async () => {
    ...
    expect(session?.params.mfaVerified).toBe(true); if (!session) throw new Error('Expected a verified session');
    expect(await openAgentSession(db.owner, { ...login, mfaCode: codes[0], identityEncryptionKey: key }, log)).toBeNull();
    await expect(refreshAgentContext(db.owner, { ...session.params, mfaVerified: false })).rejects.toMatchObject({ httpStatus: 401 });
  ...
  it('does not exhaust login capacity when a client successfully reconnects repeatedly', async () => {
    for (let attempt = 0; attempt < 15; attempt++) {
      const session = await openAgentSession(db.owner, { email: 'admin@example.com', password: 'password', agentId: 'reconnect-agent', companyId: db.companyId }, log);
