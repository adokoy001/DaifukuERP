...
const Site = defineEntity({ name: 'mcp_scope_site', label: label('拠点', 'Site'), fields: { name: f.text({ required: true }) }, siteAccess: { kind: 'store', field: 'id' }, permissions: { roles: { workforce_employee: ['read'] } } });
const SiteProbe = defineEntity({ name: 'mcp_site_probe', label: label('拠点試験', 'Site probe'), fields: { siteId: f.ref('mcp_scope_site', { required: true }), userId: f.uuid({ required: true }), name: f.text({ required: true }), storageKey: f.text({ hidden: true, outputHidden: true }) }, siteAccess: { kind: 'store', field: 'siteId' }, permissions: { roles: { workforce_employee: ['read'] }, rowRules: [{ roles: ['workforce_employee'], where: { userId: '$ctx.userId' } }] } });
registerCrudActions();
...
describe('foundation-refresh MCP identity and current authorization', () => {
  ...
  it('propagates siteIds and onBehalfOf, then revokes scope on an already-connected MCP client', async () => {
    ...
    await db.owner.drizzle.insert(users).values({ id: userId, tenantId: db.tenantId, name: 'Site scoped employee', email: 'site-scope@example.com', passwordHash: hashPassword('password'), roles: ['workforce_employee'], defaultCompanyId: db.companyId });
    await db.owner.drizzle.insert(companyMemberships).values({ tenantId: db.tenantId, userId, companyId: db.companyId, roles: ['workforce_employee'], accessScope: 'sites', siteIds: [siteId] });
    const allowed = await db.run({}, (ctx) => repo(ctx, SiteProbe).create({ siteId, userId, name: 'Own assigned site', storageKey: 'internal-storage-location' }));
    await db.run({}, (ctx) => repo(ctx, SiteProbe).create({ siteId: outside, userId, name: 'Own outside site' }));
    await db.run({}, (ctx) => repo(ctx, SiteProbe).create({ siteId, userId: newId(), name: 'Other employee same site' }));
    const session = await openAgentSession(db.owner, { email: 'site-scope@example.com', password: 'password', agentId: 'workforce-agent', companyId: undefined }, log);
    if (!session) throw new Error('test login failed');
    expect(session.params).toMatchObject({ accessScope: 'sites', siteIds: [siteId], actor: { type: 'agent', onBehalfOf: userId } });
    ...
    try {
      ...
      const own = await client.callTool({ name: 'mcp_site_probe_get', arguments: { id: allowed.id } }, CallToolResultSchema);
      ...
      const revoked = await client.callTool({ name: 'mcp_site_probe_get', arguments: { id: allowed.id } }, CallToolResultSchema);
  ...
  it('AC-1 revokes writes and resources on an already-connected client', async () => {
    const id = newId();
    await db.owner.drizzle.insert(users).values({ id, tenantId: db.tenantId, name: 'Revocation test', email: 'revoke@example.com', passwordHash: hashPassword('password'), roles: ['admin'], defaultCompanyId: db.companyId });
    await db.owner.drizzle.insert(companyMemberships).values({ tenantId: db.tenantId, userId: id, companyId: db.companyId, roles: ['admin'] });
    const session = await openAgentSession(db.owner, { email: 'revoke@example.com', password: 'password', agentId: 'revocation-agent', companyId: undefined }, log);
    if (!session) throw new Error('test login failed');
    const server = buildMcpServer({ app: db.app, owner: db.owner, params: session.params, log });
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
    await server.connect(serverTransport);
    ...
    await client.connect(clientTransport);
    try {
      const first = await client.callTool({ name: 'mcp_test_item_create', arguments: { name: 'Before revocation' } }, CallToolResultSchema);
      expect(first.isError).not.toBe(true);
      await db.owner.sql`update user_company_memberships set roles = '["viewer"]'::jsonb where user_id = ${id}`;
      const denied = await client.callTool({ name: 'mcp_test_item_create', arguments: { name: 'After role revocation' } }, CallToolResultSchema);
      ...
      const disabled = await client.callTool({ name: 'mcp_test_item_create', arguments: { name: 'After disabled' } }, CallToolResultSchema);
