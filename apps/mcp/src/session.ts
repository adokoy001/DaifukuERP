...
function inactiveSession(): DaifukuError {
  return new DaifukuError(
    'PERMISSION_DENIED',
    'The user or company is inactive, unknown, or outside this session.',
    'Reconnect with an active user and a company in the same tenant.',
    undefined,
    401,
  );
}
...
export async function refreshAgentContext(owner: Database, params: ContextParams): Promise<ContextParams> {
  ...
  if (
    !principal ||
    principal.tenantId !== params.tenantId ||
    params.sessionVersion !== principal.sessionVersion ||
    (principal.mfaEnabled && params.mfaVerified !== true)
  )
...
export async function openAgentSession(owner: Database, login: AgentLogin, log: Logger): Promise<AgentSession | null> {
  ...
  try {
    principal = await withIdentityAttempt(
      owner,
      [{ key: `verify-identity:login:${login.tenantId ?? ''}:${login.email.toLowerCase()}`, limit: 10 }],
      async () => {
    ...
    if (principal.mfaEnabled)
      await withIdentityAttempt(
        owner,
        [{ key: `factor:${principal.tenantId}:${principal.userId}`, limit: 10 }],
        async () => {
          if (!login.mfaCode || !login.identityEncryptionKey) throw identityDenied();
          await withContext(
            owner,
            { tenantId: principal.tenantId, companyId: null, actor: { type: 'user', id: principal.userId }, roles: [] },
            async (ctx) => {
              const user = await lockIdentity(ctx, principal.sessionVersion);
              await verifyFactor(ctx, user, login.mfaCode ?? '', login.identityEncryptionKey ?? '');
            },
          );
        },
      );
  ...
  if (companyId === null)
    log.warn('user has no default company; running at tenant level (companyId=null)', { user: principal.userId });
  return {
    params: {
