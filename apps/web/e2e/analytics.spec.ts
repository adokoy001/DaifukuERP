...
test.beforeAll(async ({ request }) => {
  const session = await api<{ token: string; user: { defaultCompanyId: string } }>(request, {}, '/auth/login', { email: process.env.E2E_EMAIL ?? 'admin@example.com', password: process.env.E2E_PASSWORD ?? 'password', ...(process.env.E2E_TENANT_ID ? { tenantId: process.env.E2E_TENANT_ID } : {}) });
  ...
  for (const year of new Set([FROM.slice(0, 4), BUSINESS_DATE.slice(0, 4)])) if (!years.items.some(row => row.startDate === `${year}-01-01`)) await api(request, headers, '/actions/accounting.open_fiscal_year', { startDate: `${year}-01-01` });
  ...
  for (let partnerIndex = 0; partnerIndex < 2; partnerIndex++) {
    ...
    const partner = await api(request, headers, '/api/partner', { code: `AN-${run}-${partnerIndex}`, name, isCustomer: true });
    for (const [index, date] of [FROM, CURRENT].entries()) {
      const invoice = await api(request, headers, '/api/sales_invoice', { partnerId: partner.id, date, priceIncludesTax: false, lines: { sales_invoice_line: [{ description: '分析用の合成サービス', quantity: '1', unitPrice: String(100 + partnerIndex * 400 + index * 200), taxCategory: 'exempt' }] } });
      await api(request, headers, '/actions/sales_invoice.submit', { id: invoice.id, expectedVersion: invoice.version });
    }
  }
});

async function snapshotAfter(page: Page, action: () => Promise<unknown>): Promise<AnalyticsSnapshot> {
  const pending = page.waitForResponse(response => response.url().endsWith('/analytics/snapshot') && response.request().method() === 'POST');
  await action();
...
async function ownPeriod(page: Page) {
  ...
  return snapshotAfter(page, () => page.getByRole('button', { name: '条件を適用・最新データで集計', exact: true }).click());
}
...
test('実データとWorkerで初期表示し、対象・初期状態と日付条件の変更を反映する', async ({ page }) => {
  ...
  const changed = await snapshotAfter(page, () => page.getByRole('combobox', { name: '集計対象', exact: true }).selectOption('workforce_expense'));
  expect(changed.dataset).toBe('workforce_expense'); expect(changed.meta.state).toBe('approved');
...
test('行列の階層・軸反転・平均指標・グラフを操作しても実レコードの総計が整合する', async ({ page }, testInfo) => {
  await openAnalytics(page); await ownPeriod(page);
  const total = await grandAmount(page).textContent();
  await page.getByRole('button', { name: '階層を展開', exact: true }).click();
  await expect(page.getByTestId('pivot-result').locator('tbody tr')).toHaveCount(FROM.slice(0, 4) === BUSINESS_DATE.slice(0, 4) ? 3 : 4);
  ...
  await expect(grandAmount(page)).toHaveText(total ?? '');
  await page.getByRole('button', { name: '階層を展開', exact: true }).click();
  await expect(page.getByTestId('pivot-result').locator('thead tr').first().locator('th')).toHaveCount(FROM.slice(0, 4) === BUSINESS_DATE.slice(0, 4) ? 5 : 6);
  await expect(grandAmount(page)).toHaveText(total ?? '');
  await page.getByRole('button', { name: '行と列を入れ替え', exact: true }).click();
  ...
  await page.evaluate(() => window.scrollTo(0, 0)); await page.locator('.analytics-hero').scrollIntoViewIfNeeded();
...
test('分析設定のみを保存・呼出し・別名保存・削除し、相対期間を保持する', async ({ page }) => {
  ...
  await snapshotAfter(page, () => page.getByRole('button', { name: '条件を適用・最新データで集計', exact: true }).click());
  const writes: string[] = [];
  page.on('request', request => { if (['PUT', 'PATCH', 'DELETE'].includes(request.method()) || request.method() === 'POST' && !request.url().endsWith('/analytics/snapshot')) writes.push(request.url()); });
  ...
  const raw = await page.evaluate(() => Object.entries(localStorage).filter(([key]) => key.startsWith('daifuku.analytics.v1.')).map(([, value]) => JSON.parse(value) as { version: number; items: { settings: Record<string, unknown> }[] }));
  expect(raw).toHaveLength(1); expect(raw[0]?.items).toHaveLength(1);
  expect(raw[0]?.items[0]?.settings.period).toBe('previous-month');
  expect(Object.keys(raw[0]?.items[0]?.settings ?? {}).sort()).toEqual(['dataset', 'period', 'from', 'to', 'state', 'pivot', 'chart', 'chartMeasure', 'subtotals', 'expandedRows', 'expandedColumns'].sort());
  await snapshotAfter(page, () => page.reload());
  await snapshotAfter(page, () => page.getByRole('combobox', { name: '保存した分析', exact: true }).selectOption(savedId));
...
test('同じ利用者の2タブで新規設定を保持し、古い設定の上書き・削除を防いで別名保存できる', async ({ page, context }) => {
  ...
  try {
    ...
    await openAnalytics(page); await openAnalytics(second);
    ...
    await snapshotAfter(page, () => page.getByRole('combobox', { name: '保存した分析', exact: true }).selectOption(idA));
    await snapshotAfter(second, () => second.getByRole('combobox', { name: '保存した分析', exact: true }).selectOption(idA));
    ...
    await expect(second.getByRole('combobox', { name: '保存した分析', exact: true }).locator(`option[value="${idA}"]`)).toHaveText('E2E タブAの最新分析');
    ...
    await expect(second.getByRole('alert').filter({ hasText: 'この分析は別のタブで変更または削除されています。' })).toBeVisible();
    ...
    await expect(second.getByRole('alert').filter({ hasText: 'この分析は別のタブで変更または削除されています。' })).toBeVisible();
    const savedNames = () => page.evaluate(() => Object.entries(localStorage).filter(([key]) => key.startsWith('daifuku.analytics.v1.')).flatMap(([, value]) => (JSON.parse(value) as { items: { name: string }[] }).items.map(item => item.name)).sort());
...
test('390px幅で操作でき、カタログ権限失効の障害注入で保持中の分析結果を消す', async ({ page }, testInfo) => {
  ...
  await page.route('**/analytics/catalog', route => route.fulfill({ status: 403, contentType: 'application/json', body: JSON.stringify({ error: { code: 'PERMISSION_DENIED', message: 'E2E simulated catalog revocation', hint: 'Access revoked for this test.' } }) }));
...
test('実際の試算表と期間初期値を確認し、合成レスポンスで帳票の検索・並替え・ページングを検証する', async ({ page }) => {
  await login(page); await page.goto('/r/accounting.trial_balance');
  ...
  const live = page.waitForResponse(response => response.url().endsWith('/actions/accounting.trial_balance') && response.request().method() === 'POST');
  await form.getByRole('button', { name: '実行', exact: true }).click();
  const liveResponse = await live; expect(liveResponse.ok(), await liveResponse.text()).toBe(true);
  ...
  await page.route('**/actions/accounting.trial_balance', route => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ title: { ja: '合成帳票・ページング検証', en: 'Synthetic report paging fixture' }, columns: [{ key: 'name', label: { ja: '名称', en: 'Name' }, kind: 'text' }, { key: 'amount', label: { ja: '金額', en: 'Amount' }, kind: 'decimal' }], rows: Array.from({ length: 120 }, (_, i) => ({ name: `合成行${String(i + 1).padStart(3, '0')}`, amount: String(i + 1) })), totals: { amount: '7260' } }) }));
  await form.getByRole('button', { name: '実行', exact: true }).click();
  ...
  await page.getByRole('navigation', { name: '帳票結果のページ移動' }).getByRole('button', { name: '次へ', exact: true }).click();
  ...
  await page.getByRole('button', { name: /^金額/ }).click(); await page.getByRole('button', { name: /^金額/ }).click();
