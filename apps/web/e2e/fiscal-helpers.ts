...
export async function fiscalFixture(request: APIRequestContext) {
  ...
  const company = (JSON.parse(text) as { id: string }[])[0];
  if (!company) throw new Error('Missing synthetic company');
  ...
  const member = async (role: string, name: string) => {
    ...
    await api(
      request,
      headers,
      `/admin/users/${row.id}/companies/${company.id}`,
      {
        expectedVersion: 0,
        roles: [role],
        accessScope: role === 'workforce_payroll' ? 'all' : 'sites',
        storeIds: [],
        siteIds: role === 'workforce_payroll' ? [] : [site.id],
      },
      'PUT',
    );
    return { ...row, email };
  ...
  const policy = await api(request, headers, '/api/workforce_pay_policy', {
    code: 'FISCAL-POLICY',
    name: '合成通常規程',
    ...
    basis: 'Synthetic E2E company agreement',
  });
  await api(request, headers, '/api/workforce_pay_terms', {
    ...
    confirmed: true,
    basis: 'Synthetic full-month fixed salary with attendance completion confirmed separately',
  });
