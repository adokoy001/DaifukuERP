...
test('BI: exact sources, fresh CSV authorization, changed-filter guard and mobile layout', async ({
  page,
  request,
}) => {
  ...
  const data = await api<{ overview: { grossSales: string }; sourceTable: { rows: Row[] } }>(
    request,
    headers,
    '/actions/restaurant_chain.operations_snapshot',
    input,
  );
  expect(data.sourceTable.rows.length).toBeGreaterThan(0);
  await expect(page.locator('.operations-metric').first()).toContainText(
    Number(data.overview.grossSales).toLocaleString('ja-JP'),
  );
  const sources = page
    .locator('.control-panel')
    .filter({ has: page.getByRole('heading', { name: '集計の根拠となる日次締め' }) });
  const fresh = page.waitForResponse(
    (r) => r.url().endsWith('/actions/restaurant_chain.operations_sources/export') && r.request().method() === 'POST',
  );
  const downloading = page.waitForEvent('download');
  await sources.getByRole('button', { name: /CSV/ }).click();
  const exportResponse = await fresh;
  expect(exportResponse.ok()).toBe(true);
  expect(exportResponse.request().postDataJSON()).toEqual(input);
  const download = await downloading;
  const path = await download.path();
  expect(path).not.toBeNull();
  const csv = await readFile(path ?? '', 'utf8');
  expect(csv).toContain('元の締め,締め番号,店舗');
...
test('stores: plan -> staff no-sales submission -> manager return/approve -> headquarters posting', async ({
  page,
  request,
  browser,
}) => {
  ...
  await dialog.getByLabel('対象店舗').selectOption(store.id);
  await dialog.getByLabel('1営業日の税込売上目標（円）').fill('10000');
  await dialog.getByRole('button', { name: '営業計画を作成', exact: true }).click();
  await expect(dialog).toHaveCount(0);
  await expect(page.getByTestId('operations-board')).toContainText('未提出');
  ...
  try {
    const clerkPage = await staffContext.newPage();
    await login(clerkPage, staff.email, PASSWORD);
    ...
    dialog = clerkPage.getByRole('dialog');
    await dialog.getByLabel('理由').fill('開店したが来店がなかったため。');
    ...
    await expect(
      clerkPage.getByTestId('operations-board').getByRole('button', { name: '承認', exact: true }),
    ).toHaveCount(0);
    const managerPage = await managerContext.newPage();
    await login(managerPage, manager.email, PASSWORD);
    await openOperations(managerPage);
    await boardAction(managerPage, '差戻し', '報告を差戻し', '店頭確認の記載をお願いします');
    await openOperations(clerkPage);
    await expect(clerkPage.getByTestId('operations-board')).toContainText('店頭確認の記載');
    await boardAction(clerkPage, '提出', '店長へ提出');
    await openOperations(managerPage);
    await boardAction(managerPage, '承認', '報告を承認');
    ...
    await openOperations(page, store.id);
    await boardAction(page, '本部確定', '本部で確定');
    ...
    const staffSession = await api<{ token: string }>(request, {}, '/auth/login', {
      email: staff.email,
      password: PASSWORD,
    });
    const financial = await request.get(API + '/api/sales_invoice', {
      headers: { authorization: 'Bearer ' + staffSession.token, 'x-company-id': companyId },
    });
    ...
    await api(
      request,
      headers,
      '/admin/users/' + staff.id + '/companies/' + companyId,
      { expectedVersion: 1 },
      'DELETE',
    );
    const deniedReport = clerkPage.waitForResponse(
      (r) => r.url().endsWith('/actions/restaurant_chain.operations_snapshot') && r.request().method() === 'POST',
    );
    ...
    const deniedCsv = await request.post(API + '/actions/restaurant_chain.operations_sources/export', {
      headers: { authorization: 'Bearer ' + staffSession.token, 'x-company-id': companyId },
      data: { from: DAY, to: DAY, asOf: DAY },
    });
