...
export { PASSWORD };
export async function signInQuality(page: Page, email: string) {
  await signInBase(page, email, PASSWORD);
}
...
export async function shiftFixture(request: APIRequestContext) {
  ...
  const profile = {
    ...
    targetMinutes: 960,
    maxWeeklyMinutes: 1920,
    maxDailyMinutes: 480,
    maxDays: 5,
    maxConsecutiveDays: 5,
    minRestMinutes: 660,
  };
  for (const person of [bob, missing])
    await api(request, headers, '/actions/workforce.save_shift_profile', {
      employeeId: person.employee.id,
      expectedVersion: 0,
      profile,
    });
  await api(request, bob.headers, '/actions/workforce.save_shift_availability', {
    weekStart: SHIFT_WEEK,
    expectedVersion: 0,
    days: shiftDays('available'),
  });
...
export async function shiftAction(page: Page, name: string, click: () => Promise<void>) {
  const waiting = page.waitForResponse(
    (response) => response.url().endsWith('/actions/workforce.' + name) && response.request().method() === 'POST',
  );
  await click();
...
export async function openPlanner(page: Page, f: ShiftFixture) {
  ...
  await expect(page.getByTestId('shift-page')).toBeVisible();
  await page.getByLabel('計画する週（月曜）', { exact: true }).fill(SHIFT_WEEK);
  await expect(page.getByTestId('shift-planner')).toBeVisible();
  await expect(page.getByRole('combobox', { name: '計画する拠点', exact: true })).toHaveValue(f.site.id);
}
export async function openMyShifts(page: Page, email: string) {
  await signInQuality(page, email);
  await page.goto('/me');
  await page
