...
async function conflictAndRefresh(page: Page, request: APIRequestContext, headers: Headers, user: Row) {
  ...
  const current = catalog.users.find((u) => u.id === user.id);
  ...
  const elsewhere = await api(
    ...
    headers,
    '/admin/users/' + user.id,
    { expectedVersion: current?.version, name: '別の管理者の更新 ' + user.id },
    'PATCH',
  );
  const conflict = await confirmSave(
    page,
    profile,
    '利用者情報を保存',
    '利用者情報を更新',
  ...
  const latest = await api(
    ...
    headers,
    '/admin/users/' + user.id,
    { expectedVersion: elsewhere.version, name: '背景更新の最新値 ' + user.id },
    'PATCH',
  );
...
async function cancelSwitches(page: Page, profile: Locator, pendingName: string, companyId: string, email: string) {
  ...
  const other = await company
    .locator('option')
    .evaluateAll(
      (options, current) => options.map((o) => (o as HTMLOptionElement).value).find((v) => v !== current),
      companyId,
    );
  expect(other).toBeDefined();
  page.once('dialog', (dialog) => dialog.dismiss());
  await company.selectOption(other ?? '');
...
test('access: store membership, preserved drafts, conflict reload, cancelled navigation and audited changes', async ({
  page,
  request,
}) => {
  ...
  const email = 'access-ui.' + suffix + '@example.com';
  ...
  await confirmSave(
    ...
    '会社へのアクセスを保存',
    'アクセスを更新',
    '/admin/users/' + user.id + '/companies/' + companyId,
    'PUT',
  );
  const history = page.locator('.access-audit');
  const profileEntry = history.locator('details').filter({ hasText: finalName });
  await expect(profileEntry).toHaveCount(1);
  await profileEntry.locator('summary').click();
  const nameRow = profileEntry
    .getByRole('row')
    .filter({ has: page.getByRole('rowheader', { name: '表示名', exact: true }) });
  ...
  await expect(roleEntry).toHaveCount(1);
  await roleEntry.locator('summary').click();
  const roleRow = roleEntry
    .getByRole('row')
    .filter({ has: page.getByRole('rowheader', { name: '業務ロール', exact: true }) });
