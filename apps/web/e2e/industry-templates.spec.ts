...
const companies = { appliance_store: '電器店サンプル｜あかり電器', farm: '農家サンプル｜ひなた農園', restaurant_chain: '飲食チェーンサンプル｜こもれび食堂' };
...
async function login(page: Page) {
  ...
  if (TENANT) { await page.getByText('組織を指定してログイン', { exact: true }).click(); await page.getByLabel('組織ID（任意）').fill(TENANT); }
...
async function applyTemplate(page: Page, pack: Pack) {
  ...
  if (await card.getByLabel('サンプルデータも追加する').count()) {
    ...
    const response = page.waitForResponse((r) => r.request().method() === 'POST' && r.url().endsWith('/actions/pack.apply'));
...
async function api(request: APIRequestContext, headers: Headers, path: string, data?: unknown): Promise<Row> {
  const response = data === undefined ? await request.get(`${API}${path}`, { headers }) : await request.post(`${API}${path}`, { headers, data });
  expect(response.ok(), `${path}: ${await response.text()}`).toBe(true);
  return response.json() as Promise<Row>;
}
async function headersFor(request: APIRequestContext, companyId: string): Promise<Headers> {
  const session = await api(request, {}, '/auth/login', { email: EMAIL, password: PASSWORD, ...(TENANT ? { tenantId: TENANT } : {}) });
...
async function number(cell: Locator, value: string) {
  ...
  await input.click(); await input.press('ControlOrMeta+a'); await input.pressSequentially(value); await input.press('Tab');
...
async function submit(page: Page, entity: string): Promise<Row> {
  ...
  const response = page.waitForResponse((r) => r.request().method() === 'POST' && r.url().includes(`/api/${entity}/`) && r.url().endsWith('/submit'));
...
async function action(page: Page, name: string, title: string, extra: Record<string, string> = {}): Promise<Row> {
  ...
  for (const [key, value] of Object.entries(extra)) await form.locator(`[data-field="${key}"]`).locator('input,textarea').fill(value);
...
test('電器店: 画面導入・受付作成・開始・作業完了・請求を汎用業務フォームで通す', async ({ page, request }) => {
  ...
  await applyTemplate(page, 'appliance_store'); await isolatedMenu(page, 'appliance_store');
  ...
  expect(await action(page, 'appliance_store.start_service', title)).toMatchObject({ id: service.id, status: 'in_progress' });
  expect(await action(page, 'appliance_store.complete_service', title, { completedDate: DATE, workReport: '点検、調整、試運転により正常動作を確認。' })).toMatchObject({ id: service.id, docstatus: 1, status: 'completed' });
  expect(await action(page, 'appliance_store.invoice_service', title)).toMatchObject({ docstatus: 1, subtotal: '8000', taxTotal: '800', total: '8800' });
});
...
test('農家: 画面導入・作期作成確定・収穫入力確定と在庫評価を通す', async ({ page, request }) => {
  ...
  await applyTemplate(page, 'farm'); await isolatedMenu(page, 'farm');
  const headers = await headersFor(request, companyId);
  ...
  await page.goto('/e/farm_harvest/new'); form = page.getByTestId('record-form');
  ...
  expect(await api(request, headers, `/api/stock_entry/${String(harvest.stockEntryId)}`)).toMatchObject({ docstatus: 1 });
});
...
test('飲食店: 画面導入・店内持帰り日次締め・集計・会社切替後のメニュー分離', async ({ page, request }) => {
  ...
  await applyTemplate(page, 'restaurant_chain'); await isolatedMenu(page, 'restaurant_chain');
  const headers = await headersFor(request, companyId);
  ...
  const store = await api(request, headers, '/api/restaurant_chain_store', { code: `RU${marker}`, name: storeName, warehouseId: warehouse.id, cashAccountId: source?.cashAccountId, partnerId: source?.partnerId });
  const products = await list(request, headers, 'product');
  const rice = products.find((row) => row.code === 'RC-RICE'), chicken = products.find((row) => row.code === 'RC-CHICKEN');
  const receipt = await api(request, headers, '/api/stock_entry', { type: 'receipt', date: DATE, warehouseId: warehouse.id, lines: { stock_entry_line: [{ productId: rice?.id, quantity: '10', unitCost: '500' }, { productId: chicken?.id, quantity: '5', unitCost: '1000' }] } });
  ...
  await form.locator('[data-field="date"] input').fill(DATE);
  await number(form.locator('[data-field="cashAmount"]'), '10000'); await number(form.locator('[data-field="cardAmount"]'), '5000'); await number(form.locator('[data-field="qrAmount"]'), '1400');
  const grid = page.getByTestId('lines-restaurant_chain_closing_line');
  for (const [index, mode, quantity, price] of [[0, 'dine_in', '10', '1100'], [1, 'takeaway', '5', '1080']] as const) {
    await grid.getByRole('button', { name: '行を追加' }).click();
    ...
    await number(row.locator('[data-field="quantity"]'), quantity); await number(row.locator('[data-field="unitPrice"]'), price);
  }
  ...
  expect(closed).toMatchObject({ storeId: store.id, total: '16400', taxTotal: '1400', consumptionCost: '3000', wasteCost: '50' });
  await page.goto('/r/restaurant_chain.daily_summary');
  const report = page.getByTestId('report-form');
  await report.locator('[data-field="from"] input').fill(DATE); await report.locator('[data-field="to"] input').fill(DATE);
  await choose(report.locator('[data-field="storeId"]'), storeName);
  const response = page.waitForResponse((r) => r.request().method() === 'POST' && r.url().endsWith('/actions/restaurant_chain.daily_summary'));
  await report.getByRole('button', { name: '実行', exact: true }).click();
  const result = await response;
  expect(result.ok(), await result.text()).toBe(true);
  expect(await result.json()).toMatchObject({ totals: { total: '16400', taxTotal: '1400', consumptionCost: '3000', wasteCost: '50' } });
  await expect(page.getByTestId('report-result')).toContainText(storeName);
  await selectCompany(page, 'appliance_store'); await isolatedMenu(page, 'appliance_store');
  await selectCompany(page, 'farm'); await isolatedMenu(page, 'farm');
  await selectCompany(page, 'restaurant_chain'); await isolatedMenu(page, 'restaurant_chain');
});
