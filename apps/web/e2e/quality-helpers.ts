...
export async function signInQuality(
  page: Page,
  email = process.env.E2E_EMAIL ?? 'admin@example.com',
  password = process.env.E2E_PASSWORD ?? 'password',
) {
...
export async function qualityPartner(request: APIRequestContext, headers: Headers, name: string) {
  return api(request, headers, '/api/partner', {
    code: 'Q' + crypto.randomUUID().replaceAll('-', '').slice(0, 15),
    name,
    isCustomer: true,
  });
}
export const failedRead = (status = 503) => ({
  status,
  contentType: 'application/json',
  body: JSON.stringify({
    error: {
      code: status === 403 ? 'PERMISSION_DENIED' : status === 404 ? 'NOT_FOUND' : 'INTERNAL',
