...
import {
  ...
  qualitySession,
  signInQuality,
} from './quality-helpers.ts';

async function openExpense(page: Page, email: string) {
  await page.setViewportSize({ width: 375, height: 812 });
  await signInQuality(page, email, PASSWORD);
...
test('read recovery: mobile expense draft survives polling failure, retry and successful save', async ({
  page,
  request,
}) => {
  ...
  await expect(dialog.getByLabel('利用目的・内容', { exact: true })).toHaveValue(description);
  await expect(dialog.getByLabel('精算する金額（円）', { exact: true })).toHaveValue('1234');
  ...
  await notice.getByRole('button', { name: '再取得', exact: true }).click();
  await restored;
  ...
  expect(command).toMatchObject({ status: 'draft' });
  expect(await api(request, person.headers, '/api/workforce_expense/' + command.id)).toMatchObject({
    description,
    amount: '1234',
    status: 'draft',
  });
  await expect(dialog).toHaveCount(0);
  await expect(page.locator('article.workforce-record').filter({ hasText: description })).toBeVisible();
});

for (const status of [401, 403, 404])
  test(`read recovery: ${status} removes the open draft and cached employee view`, async ({ page, request }) => {
    const person = await qualityEmployee(request);
    await page.clock.install();
    const dialog = await openExpense(page, person.email);
...
test('read recovery: generic record reconnect preserves dirty values, then 404 hides the form', async ({
  page,
  request,
}) => {
  ...
  await expect(page.getByTestId('read-refresh-notice')).toBeVisible();
  await expect(input).toHaveValue(changed);
  await page.unroute(path);
  await page.getByTestId('read-refresh-notice').getByRole('button').click();
  await expect(page.getByTestId('read-refresh-notice')).toHaveCount(0);
  await expect(input).toHaveValue(changed);
