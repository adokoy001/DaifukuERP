...
async function shot(page: Page, name: string) {
  if ((page.viewportSize()?.width ?? 1440) < 760) {
    ...
    if (!opened) {
      ...
      await expect
        .poll(() => page.locator('#app-navigation').evaluate((element) => element.getBoundingClientRect().right))
        .toBeLessThanOrEqual(0);
    } else
      await expect
        .poll(() => page.locator('#app-navigation').evaluate((element) => element.getBoundingClientRect().left))
        .toBe(0);
  }
  await page.screenshot({
    path: process.env.E2E_EVIDENCE_DIR ? join(process.env.E2E_EVIDENCE_DIR, name) : test.info().outputPath(name),
    fullPage: true,
  });
}
...
async function catalogHrefs(page: Page): Promise<string[]> {
  ...
  for (let index = 0; index < 50; index++) {
    ...
    expect(links.length).toBeLessThanOrEqual(24);
    found.push(...links);
    if (!(await pager.count()) || (await pager.getByRole('button', { name: '次へ', exact: true }).isDisabled()))
      return found;
...
test('navigation AC-1/2/4/5/11: compact areas, complete paged directory and bilingual local search', async ({
  page,
}) => {
  const meta = await loginWithMeta(page);
  const navHrefs = await menu(page)
    .getByRole('link')
    .evaluateAll((items) => items.map((item) => item.getAttribute('href')));
  expect(navHrefs.length).toBeLessThanOrEqual(11);
  expect(
    navHrefs.every(
      (href) => href === '/' || href === '/me' || href === '/workspaces' || /^\/workspaces\/[a-z]+$/.test(href ?? ''),
    ),
  ).toBe(true);
  ...
  for (const entity of meta.entities.filter((entity) => entity.ops.includes('read') && !children.has(entity.name)))
    expect(all).toContain(`/e/${entity.name}`);
  for (const action of meta.actions.filter((action) => action.resultKind === 'table'))
    expect(all).toContain(`/r/${action.name}`);
  for (const child of children) expect(all).not.toContain(`/e/${child}`);
  await page
    .getByRole('navigation', { name: '画面一覧のページ' })
    .getByRole('button', { name: '先頭', exact: true })
    .click();
  const recordRequests: string[] = [];
  page.on('request', (request) => {
    if (originalPath(request).startsWith('/api/')) recordRequests.push(request.url());
  });
  ...
  expect(reports.every((href) => href.startsWith('/r/'))).toBe(true);
...
test('navigation AC-6/7: area and original list breadcrumbs preserve a cancelled draft', async ({ page }) => {
  ...
  await cancelledLeave(
    page,
    () => menu(page).getByRole('link', { name: '分析・レポート', exact: true }).click(),
    value,
  );
  const dialog = page.waitForEvent('dialog');
...
test('navigation AC-8: 390px modal drawer traps focus, closes with Escape and returns to its trigger', async ({
  page,
}) => {
  await login(page);
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  await expect(menu(page)).toBeHidden();
  await page.keyboard.press('Tab');
  await expect(menu(page).locator(':focus')).toHaveCount(0);
  await opener.click();
  await menu(page).getByRole('link', { name: '販売・仕入', exact: true }).click();
  await expect(drawer).toHaveCount(0);
  await expect(page).toHaveURL(/\/workspaces\/sales$/);
...
test('navigation AC-3/10: a store employee finds permitted operations but no privileged destination', async ({
  page,
  request,
  browser,
}) => {
  ...
  try {
    ...
    await login(employee, staff.email, PASSWORD);
    ...
    const session = await api<{ token: string }>(request, {}, '/auth/login', {
      email: staff.email,
      password: PASSWORD,
    });
    const denied = await request.get(API + '/api/sales_invoice', {
...
test('navigation AC-9: home fetches no document totals until opened, then only the selected module', async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  ...
  page.on('response', (response) => {
    ...
    pending.push(
      (async () => {
        ...
        counts.set(`${entity}:${status}`, data.total);
      })(),
    );
  });
  const meta = await loginWithMeta(page);
  ...
  for (const module of ['sales', 'purchase']) {
    if (module !== 'sales') {
      ...
      await selector.selectOption(module);
    }
    const documents = meta.entities.filter(
      (entity) => entity.kind === 'document' && entity.module === module && !children.has(entity.name),
    );
    ...
    await expect.poll(() => countRequests.length).toBe(documents.length * 3);
    await Promise.all(pending);
    expect(new Set(countRequests.map((request) => request.entity))).toEqual(
      new Set(documents.map((entity) => entity.name)),
    );
    for (const entity of documents)
      for (const status of [0, 1, 2]) {
        const count = counts.get(`${entity.name}:${status}`);
        expect(count).toBeDefined();
        await expect(
          page.locator(`[data-entity="${entity.name}"] [data-docstatus="${status}"] [data-testid="doc-count"]`),
        ).toHaveText(Number(count).toLocaleString('ja-JP'));
      }
  }
});
