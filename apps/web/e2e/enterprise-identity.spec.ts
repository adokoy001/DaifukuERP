...
function authenticator(secret: string) {
  ...
  for (const character of secret) { value = (value << 5) | 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567'.indexOf(character); bits += 5; if (bits >= 8) { bytes.push((value >>> (bits - 8)) & 255); bits -= 8; } }
  const counter = Buffer.alloc(8); counter.writeBigUInt64BE(BigInt(Math.floor(Date.now() / 30000)));
  const digest = createHmac('sha1', Buffer.from(bytes)).update(counter).digest(), offset = (digest[19] ?? 0) & 15;
...
async function passwordLogin(page: Page, email = 'admin@example.com', password = initialPassword) {
  ...
  await page.getByRole('button', { name: 'ログイン', exact: true }).click(); await expect(page.locator('#password')).not.toBeVisible();
}
...
async function latestMail(request: APIRequestContext, recipient: string, purpose: string) {
  ...
  expect(link).not.toBeNull(); return link?.[0] ?? '';
}
async function setMailPassword(page: Page, link: string, password: string, invitation: boolean) {
  await page.goto(link); await page.locator('#mail-password').fill(password); await page.locator('#mail-confirm').fill(password);
  expect(new URL(page.url()).hash).toBe('');
  await page.getByRole('button', { name: invitation ? '登録して利用を始める' : 'パスワードを再設定', exact: true }).click();
  await expect(page.getByRole('status')).toContainText(invitation ? 'アカウントを有効にしました' : 'パスワードを再設定しました');
}
test.describe.serial('enterprise identity with synthetic OIDC and verified TLS SMTP', () => {
  test('explicit browser SSO linking, TOTP setup and single-use recovery login', async ({ page }) => {
    await passwordLogin(page); await page.goto('/account');
    await page.getByRole('button', { name: '組織アカウントを紐付け', exact: true }).click(); await verify(page);
    await expect(page.getByRole('status')).toContainText('組織アカウントを紐付けました'); expect(new URL(page.url()).search).toBe('');
    await page.getByRole('link', { name: 'ログインへ', exact: true }).click(); await page.getByRole('button', { name: '合成組織SSO' }).click();
    await expect(page).not.toHaveURL(/auth\/oidc|login/); await page.goto('/account');
    await page.getByRole('button', { name: '認証アプリを登録' }).click(); await verify(page);
    ...
    await expect(page).toHaveURL(/auth\/recovery-codes/); recovery = await page.locator('.identity-recovery li').allTextContents(); expect(recovery).toHaveLength(10);
    await page.getByLabel('安全な場所に保存しました').check(); await page.getByRole('link', { name: '保存してログインへ' }).click();
    await passwordLogin(page); await loginMfa(page, recovery[0] ?? ''); await expect(page).not.toHaveURL(/login/);
    await page.goto('/account'); await expect(page.getByText('回復コード 残り9件')).toBeVisible();
    await page.context().clearCookies(); await page.evaluate(() => { localStorage.clear(); sessionStorage.clear(); });
    await page.goto('/login'); await page.getByRole('button', { name: '合成組織SSO' }).click();
    await expect(page.getByRole('heading', { name: 'もう一度、本人確認' })).toBeVisible();
    await loginMfa(page, recovery[0] ?? ''); await expect(page.getByRole('alert')).toContainText('コードを確認');
    await loginMfa(page, recovery[1] ?? ''); await expect(page).not.toHaveURL(/login/);
  });
  test('admin invitation and email password reset preserve no-company account access on mobile', async ({ page, request }, testInfo) => {
    await passwordLogin(page); await loginMfa(page, recovery[2] ?? ''); await expect(page).not.toHaveURL(/login/); await page.goto('/admin/users');
    await page.getByRole('button', { name: /メールで招待/ }).click();
    await page.getByLabel('招待する方の氏名').fill('合成 招待利用者'); await page.getByLabel('招待先メールアドレス').fill('invited@example.test');
    await verify(page, initialPassword, recovery[3]); await expect(page.getByRole('status')).toContainText('配送待ち');
    const inviteLink = await latestMail(request, 'invited@example.test', 'accept-invitation');
    await page.setViewportSize({ width: 390, height: 844 }); await setMailPassword(page, inviteLink, 'invited-test-password', true);
    await passwordLogin(page, 'invited@example.test', 'invited-test-password'); await page.goto('/account');
    await expect(page.getByRole('button', { name: '認証アプリを登録' })).toBeVisible();
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true); await page.screenshot({ path: testInfo.outputPath('identity-account-mobile.png'), fullPage: true });
    await page.goto('/forgot-password'); await page.locator('#reset-email').fill('invited@example.test'); await page.getByRole('button', { name: '再設定メールを依頼' }).click();
    await expect(page.getByRole('status')).toContainText('入力された情報で再設定できる場合');
    const resetLink = await latestMail(request, 'invited@example.test', 'reset-password'); await setMailPassword(page, resetLink, 'changed-invited-password', false);
    await passwordLogin(page, 'invited@example.test', 'changed-invited-password'); await page.goto('/account'); await expect(page.getByRole('button', { name: '認証アプリを登録' })).toBeVisible();
