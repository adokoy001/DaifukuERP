...
export async function newStore(request: APIRequestContext, headers: Headers, name: string) {
  ...
  return api(request, headers, '/api/restaurant_chain_store', {
    code: 'OPS' + Date.now(),
    name,
...
export async function member(
  ...
  await api(
    ...
    { expectedVersion: 0, roles: [role], accessScope: 'stores', storeIds: [storeId] },
    'PUT',
  );
  return { ...user, email };
}
...
export async function boardAction(page: Page, label: string, confirm: string, note?: string) {
  ...
  const response = page.waitForResponse(
    (r) =>
      ...
      /actions\/restaurant_chain\.(review|submit_for_review|finalize)$/.test(r.url()),
  );
