...
test('reference search: keyboard cannot select old candidates during debounce or a delayed response', async ({
  page,
  request,
}) => {
  ...
  await expect(page.getByRole('option', { name: oldName, exact: false })).toBeVisible();
  ...
  await expect(input).toHaveValue(newName);
  release();
  await expect(page.getByRole('option', { name: newName, exact: false })).toBeVisible();
...
test('reference search: failure preserves text, retry works and keyboard keeps the active option visible', async ({
  page,
  request,
}) => {
  const { headers } = await qualitySession(request),
  ...
  await expect(page.getByRole('alert')).toContainText('候補を取得できませんでした');
  await input.press('Enter');
  await expect(input).toHaveValue(key);
  await expect(page.getByRole('listbox').getByRole('option')).toHaveCount(0);
  await assertNoOverflow(page);
  await page.screenshot({ path: test.info().outputPath('reference-failure-mobile-375.png'), fullPage: true });
  fail = false;
  await page.getByRole('alert').getByRole('button').tap();
  ...
  await input.press('Enter');
  await expect(input).toHaveValue(new RegExp('^' + key));
  await input.focus();
  ...
  await expect(input).toHaveAttribute('aria-expanded', 'false');
  await assertNoOverflow(page);
  await page.screenshot({ path: test.info().outputPath('reference-mobile-375.png'), fullPage: true });
});
