...
async function stockQuantity(request: APIRequestContext, fixture: FinanceFixture) {
  const query = new URLSearchParams({
    where: JSON.stringify({ productId: fixture.product.id, warehouseId: fixture.warehouse.id }),
  });
  const balances = await api<{ items: (Row & { qty: string })[] }>(
    request,
    fixture.headers,
    '/api/stock_balance?' + query,
  );
...
async function submitOrderFromRecord(page: Page, id: string) {
  ...
  const pending = page.waitForResponse(
    (response) => response.url().endsWith(`/api/trade_order/${id}/submit`) && response.request().method() === 'POST',
  );
...
async function invoiceQuantity(page: Page, quantity: string) {
  ...
  const made = await financeAction<TradeCommand>(page, 'trade.bill_fulfillment', () =>
    dialog.getByRole('button', { name: '数量を確認して請求を確定', exact: true }).click(),
  );
  await expect(dialog).toHaveCount(0);
  return made;
}

test('quotation conversion → generic order submission → partial shipment and invoice keep stock singular in Japanese and English', async ({
  page,
  request,
}) => {
  ...
  const dialog = page.getByRole('dialog');
  await dialog.getByLabel('受注日', { exact: true }).fill(BUSINESS_DATE);
  const converted = await financeAction<TradeCommand>(page, 'trade.convert_quotation', () =>
    dialog.getByRole('button', { name: '見積内容を引き継いで受注作成', exact: true }).click(),
  );
  await expect(dialog).toHaveCount(0);
  const submitted = await submitOrderFromRecord(page, converted.id);
  await page.goto('/commerce/trade');
  await page.getByRole('button', { name: String(submitted.number), exact: true }).click();
  await page.getByRole('button', { name: '出荷数量を入力', exact: true }).click();
  await dialog.getByLabel('入出庫日', { exact: true }).fill(BUSINESS_DATE);
  await selectWarehouse(page);
  const quantity = dialog.getByRole('textbox', { name: '商流検証部品 今回の数量', exact: true });
  ...
  const fulfillment = await financeAction<TradeCommand>(page, 'trade.fulfill_order', () =>
    dialog.getByRole('button', { name: '数量を確認して入出庫', exact: true }).click(),
  );
  await expect(dialog).toHaveCount(0);
  ...
  expect(detail.fulfillments[0]?.id).toBe(fulfillment.id);
  expect(detail.fulfillments[0]?.billings[0]?.total).toBe('4400');
  expect(await stockQuantity(request, fixture)).toBe('94');
  await expect(
    page.getByRole('link', { name: String(detail.fulfillments[0]?.billings[0]?.invoiceNumber), exact: true }),
  ).toBeVisible();
  await assertNoOverflow(page);
  await page.screenshot({ path: test.info().outputPath('trade-sales-desktop-ja.png'), fullPage: true });
  ...
  await assertNoOverflow(page);
  await page.screenshot({ path: test.info().outputPath('trade-sales-desktop-en.png'), fullPage: true });
});

test('purchasing partial receipt and invoice work at 390px and closing retains visible unbilled goods', async ({
  page,
  request,
}) => {
  const fixture = await financeFixture(request),
  ...
  await signInQuality(page, fixture.email, PASSWORD);
  await page.goto('/commerce/trade');
  await page.getByRole('button', { name: '仕入', exact: true }).click();
  await page.getByRole('button', { name: String(order.number), exact: true }).click();
  ...
  await assertNoOverflow(page);
  await page.screenshot({ path: test.info().outputPath('trade-receipt-dialog-mobile-390.png'), fullPage: true });
  await financeAction(page, 'trade.fulfill_order', () =>
    dialog.getByRole('button', { name: '数量を確認して入出庫', exact: true }).click(),
  );
  await expect(dialog).toHaveCount(0);
  expect(await stockQuantity(request, fixture)).toBe('106');
  await invoiceQuantity(page, '4');
  let detail = await orderDetail(request, fixture, order.id);
  expect(detail.lines[0]).toMatchObject({ remainingQuantity: '4', unbilledQuantity: '2' });
  expect(detail.fulfillments[0]?.billings[0]?.total).toBe('2640');
  expect(await stockQuantity(request, fixture)).toBe('106');
  await page.getByRole('button', { name: '残数を終了', exact: true }).click();
  await dialog.getByLabel('理由', { exact: true }).fill('残り4個の発注を打切り、既入荷2個は別便で請求');
  await financeAction(page, 'trade.close_order', () =>
    dialog.getByRole('button', { name: '理由を記録して実行', exact: true }).click(),
  );
  await expect(dialog).toHaveCount(0);
  ...
  await assertNoOverflow(page);
  await page.screenshot({ path: test.info().outputPath('trade-purchase-closed-mobile-390.png'), fullPage: true });
  await invoiceQuantity(page, '2');
  detail = await orderDetail(request, fixture, order.id);
  expect(detail.order.status).toBe('closed');
  expect(detail.lines[0]).toMatchObject({ remainingQuantity: '4', unbilledQuantity: '0', billedQuantity: '6' });
  expect(detail.fulfillments[0]?.billings.map((billing) => billing.total)).toEqual(['2640', '1320']);
  expect(await stockQuantity(request, fixture)).toBe('106');
});
