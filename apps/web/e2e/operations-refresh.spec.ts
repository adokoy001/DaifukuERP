...
import { DAY, adminHeaders, api, login, newStore, openOperations, restaurant, type Headers, type Row } from './operations-helpers.ts';
...
async function prepareReceivable(request: APIRequestContext, headers: Headers) {
  ...
  const rice = products.items.find((row) => row.code === 'RC-RICE'), chicken = products.items.find((row) => row.code === 'RC-CHICKEN');
  ...
  expect(rice).toBeDefined(); expect(chicken).toBeDefined(); expect(recipe).toBeDefined();
  const stock = await api(request, headers, '/api/stock_entry', { type: 'receipt', date: DAY, warehouseId: store.warehouseId, lines: { stock_entry_line: [{ productId: rice?.id, quantity: '1', unitCost: '500' }, { productId: chicken?.id, quantity: '1', unitCost: '1000' }] } });
  await api(request, headers, `/api/stock_entry/${stock.id}/submit`, {});
  const draft = await api(request, headers, '/api/restaurant_chain_closing', { storeId: store.id, date: DAY, cashAmount: '0', cardAmount: '1100', qrAmount: '0', cashSalesCounted: '0', lines: { restaurant_chain_closing_line: [{ recipeId: recipe?.id, serviceMode: 'dine_in', quantity: '1', unitPrice: '1100' }] } });
  const closing = await api(request, headers, `/api/restaurant_chain_closing/${draft.id}/submit`, { expectedVersion: draft.version });
...
async function refreshBoth(page: Page, input: Record<string, string>): Promise<Evidence> {
  const snapshot = page.waitForResponse((res) => res.request().method() === 'POST' && res.url().endsWith('/actions/restaurant_chain.operations_snapshot'));
  const settlements = page.waitForResponse((res) => res.request().method() === 'POST' && res.url().endsWith('/actions/restaurant_chain.settlement_evidence'));
...
test('同条件の再集計で、開いたままの入金残高を外部入金・取消後の実値へ更新する', async ({ page, request }) => {
  ...
  await login(page);
  const companyId = await restaurant(page), headers = await adminHeaders(request, companyId);
  ...
  const bank = accounts.items.find((row) => row.code === '1100'); expect(bank).toBeDefined();
  const input = { from: DAY, to: DAY, asOf: DAY, storeId: store.id };
  await openOperations(page, store.id);
  const evidence = page.locator('.control-panel').filter({ has: page.getByRole('heading', { name: '売上伝票と消込の確認', exact: true }) });
  ...
  const receipt = await api(request, headers, '/api/payment', { direction: 'receive', partnerId: invoice.partnerId, date: DAY, method: 'bank_transfer', amount: '400', accountId: bank?.id, note: 'operations-refresh E2E isolated receipt', lines: { payment_allocation: [{ invoiceEntity: 'sales_invoice', invoiceId: invoice.id, amount: '400' }] } });
  const paid = await api(request, headers, `/api/payment/${receipt.id}/submit`, { expectedVersion: receipt.version });
  expect(await api(request, headers, '/api/sales_invoice/' + invoice.id)).toMatchObject({ paidAmount: '400', balance: '700' });
  ...
  expect(await api(request, headers, '/api/sales_invoice/' + invoice.id)).toMatchObject({ paidAmount: '0', balance: '1100' });
