...
export function SsoProviders() {
  ...
  return (
    <section className="identity-sso">
      <p>{t({ ja: '組織のアカウントでログイン', en: 'Sign in with your organization' })}</p>
      {providers.data?.items.map((provider) => (
        <button type="button" className="btn" disabled={busy} key={provider.id} onClick={() => void start(provider.id)}>
          {provider.label}
        </button>
      ))}
      {providers.isError || error ? (
        <p className="account-error" role="alert">
          {t({
            ja: 'SSO情報を取得できません。再読込するか、パスワードでログインしてください。',
            en: 'SSO is unavailable. Reload or sign in with your password.',
          })}
        </p>
      ) : null}
    </section>
