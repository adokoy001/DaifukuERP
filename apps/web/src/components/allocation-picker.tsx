...
export function useAllocationConvention(entity: EntityMeta): AllocationConvention | undefined {
  const meta = useMeta();
  return useMemo(() => (meta.data ? allocationConventionOf(entity, meta.data.entities, meta.data.actions) : undefined), [entity, meta.data]);
}
...
export function useSubmitBlock(entity: EntityMeta, record: RecordJson | undefined): string | undefined {
  ...
  return conv && record && (record.docstatus ?? 0) === 0 && savedOverAllocation(conv, record) ? t(S.submitBlockedOverAllocated) : undefined;
}

const PROBLEM_TEXT: Record<PickProblem, Label> = { invalid: S.pickInvalid, notPositive: S.pickNotPositive, overBalance: S.pickOverBalance };

function SummaryBar({ values, rows, hasAmount, minScale }: { values: FormValues; rows: GridRow[]; hasAmount: boolean; minScale: number }) {
  const { t } = useLocale();
...
function usePicks(conv: AllocationConvention, values: FormValues, rows: GridRow[]) {
  ...
  const toggle = (inv: OutstandingInvoice) =>
    setPicks((p) => {
      if (p[inv.invoiceId] !== undefined) return Object.fromEntries(Object.entries(p).filter(([id]) => id !== inv.invoiceId));
      const remaining = conv.hasAmount ? remainingFor(values[ALLOCATION.amountField], rows, Object.values(p)) : undefined;
...
function PickRow({ inv, amount, added, amountField, minScale, onToggle, onAmount }: { inv: OutstandingInvoice; amount: string | undefined; added: boolean; amountField: FieldMeta; minScale: number; onToggle: () => void; onAmount: (v: string) => void }) {
  const { t } = useLocale();
  const problem = amount === undefined ? undefined : pickProblem(amount, inv.balance);
  const name = inv.number || inv.invoiceId;
  return (
    <tr data-testid="outstanding-row" data-number={inv.number} data-invoice-id={inv.invoiceId} className="border-t border-neutral-100">
      <td className="px-2 py-1">
        <input type="checkbox" className="h-4 w-4 accent-sky-700" aria-label={`${t(S.select)} ${name}`} checked={amount !== undefined} disabled={added} onChange={onToggle} />
      </td>
      ...
      <td className="w-40 px-2 py-1">
        {amount !== undefined ? <NumberField id={`pick-${inv.invoiceId}`} field={amountField} value={amount} onChange={(v) => onAmount(typeof v === 'string' ? v : '')} disabled={false} invalid={problem !== undefined} ariaLabel={`${t(S.allocationAmount)} ${name}`} /> : null}
        {problem ? (
...
function OutstandingPanel({ conv, input, amountField, values, rows, columns, onAppend, onClose }: PanelProps) {
  ...
  const chosen = invoices.flatMap((invoice) => (picks[invoice.invoiceId] !== undefined ? [{ invoice, amount: picks[invoice.invoiceId] ?? '' }] : []));
  ...
  return (
    <div role="region" aria-label={t(S.outstandingInvoices)} data-testid="outstanding-panel" onKeyDown={onKeyDown} className="mt-2 rounded border border-sky-200 bg-sky-50/40">
      ...
      {query.data && invoices.length === 0 ? <div className="px-2 py-2 text-xs text-neutral-500">{t(S.outstandingEmpty)}</div> : null}
      {invoices.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {invoices.map((inv) => (
                <PickRow key={inv.invoiceId} inv={inv} amount={picks[inv.invoiceId]} added={added.has(inv.invoiceId)} amountField={amountField} minScale={minScale} onToggle={() => toggle(inv)} onAmount={(v) => setAmount(inv.invoiceId, v)} />
              ))}
            </tbody>
          </table>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {!readOnly ? (
          <span className="ml-auto flex items-center gap-2">
            ...
            <button type="button" className="btn px-2 py-0.5" disabled={!input || !amountField} aria-expanded={open} onClick={() => setOpen((o) => !o)}>
              {t(S.pickOutstanding)}
            </button>
          </span>
        ) : null}
      </div>
      {open && input && amountField && !readOnly ? <OutstandingPanel conv={conv} input={input} amountField={amountField} values={values} rows={rows} columns={columns} onAppend={onAppend} onClose={() => setOpen(false)} /> : null}
    </div>
  );
}
