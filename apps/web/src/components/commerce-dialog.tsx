...
export function CommerceDialog({ title, description, submitLabel, children, onSubmit, onClose, stale = false, confirmOnly = false, readOnly = false, submitDisabled = false }: {
  ...
  const id = useId();
  const [dirty, setDirty] = useState(false), [busy, setBusy] = useState(false), [discarding, setDiscarding] = useState(false), [error, setError] = useState<unknown>();
  const blocker = useBlocker({ shouldBlockFn: () => Boolean(getToken()) && !exitAllowed.current && (locked.current || dirty), enableBeforeUnload: dirty || busy, withResolver: true });
  useEffect(() => { dialog.current?.showModal(); }, []);
  const close = () => { if (locked.current) return; if (dirty) setDiscarding(true); else onClose(); };
  const cancelDiscard = () => { setDiscarding(false); if (blocker.status === 'blocked') blocker.reset(); };
  ...
  return <dialog ref={dialog} className="action-dialog workforce-dialog" aria-labelledby={id} onCancel={(e) => { e.preventDefault(); close(); }}>
    <header className="workforce-dialog-heading"><div><span className="eyebrow">DAIFUKU COMMERCE</span><h2 id={id}>{title}</h2></div><button type="button" className="btn workforce-icon-button" disabled={busy} onClick={close} aria-label={t({ ja: '閉じる', en: 'Close' })}><Icon name="close" /></button></header>
    <ReadRefreshNotice />
