  ...
  return (
    ...
    >
      <header className="workforce-dialog-heading">
        <div>
          <span className="eyebrow">DAIFUKU COMMERCE</span>
          <h2 id={id}>{title}</h2>
        </div>
        <button
          type="button"
      ...
      <form
        ...
        onSubmit={async (event) => {
          ...
          if (locked.current || stale || readOnly || submitDisabled) return;
        ...
        <footer className="workforce-dialog-actions">
          ...
          {readOnly ? null : (
            <button
              ...
              disabled={busy || stale || submitDisabled || (!dirty && !confirmOnly)}
            >
