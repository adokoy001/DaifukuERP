...
export function WorkforceCalculation({ row }: { row: PayrollSummary }) {
  ...
  return <details className="workforce-calculation"><summary>{t({ ja: '賃金条件・日別の計算根拠', en: 'Pay terms and daily calculation evidence' })}</summary>
    <p className="workforce-notice">{t({ ja: '通常の労働時間制・日本円の計算です。実時間で集計し、通常賃金と割増賃金の合計をそれぞれ1円単位で切り上げます。月給の期間按分は暦日数です。欠勤の調整は根拠を付けた明示的な控除で扱います。', en: 'Ordinary working time, JPY. Exact time is aggregated before base and premium totals are each rounded up to whole yen. Monthly salary uses calendar-day proration. Absence adjustments need an explicit deduction and evidence.' })}</p>
    <div className="workforce-record-list">{terms.map((term, i) => <section className="workforce-record" key={i}><h3>{text(term.validFrom)} — {text(term.validTo)}</h3><p>{t(term.payType === 'monthly' ? { ja: '月給', en: 'Monthly salary' } : { ja: '時給', en: 'Hourly rate' })}: <WorkforceMoney value={amount(term.payType === 'monthly' ? term.monthlySalary : term.hourlyRate)} /></p><p>{text(term.basis)}</p></section>)}</div>
    ...
    <div className="workforce-record-list">{days.map((day, i) => <section className="workforce-record" key={i}><h3>{text(day.date)}</h3><dl className="workforce-definition"><div><dt>{t({ ja: '労働時間', en: 'Worked time' })}</dt><dd>{duration(day.workedMs)}</dd></div><div><dt>{t({ ja: '時間外 / うち高率', en: 'Overtime / high rate' })}</dt><dd>{duration(day.overtimeMs)} / {duration(day.highOvertimeMs)}</dd></div><div><dt>{t({ ja: '法定休日 / 深夜', en: 'Statutory holiday / night' })}</dt><dd>{duration(day.holidayMs)} / {duration(day.nightMs)}</dd></div><div><dt>{t({ ja: '割増の時給基礎', en: 'Hourly premium basis' })}</dt><dd><WorkforceMoney value={amount(day.hourlyBasis)} /></dd></div><div><dt>{t({ ja: '通常賃金（集計前）', en: 'Base before aggregation' })}</dt><dd><WorkforceMoney value={amount(day.baseAmount)} /></dd></div><div><dt>{t({ ja: '割増賃金（集計前）', en: 'Premium before aggregation' })}</dt><dd><WorkforceMoney value={amount(day.premiumAmount)} /></dd></div></dl></section>)}</div>
  </details>;
}
