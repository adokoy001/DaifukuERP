...
export function DocActions({ entity, record, onAfter, submitBlocked, blocked, onBusy }: DocActionsProps) {
  ...
  const execute = (correctionDate?: string) => {
    ...
    if (choice.op === 'delete') {
      ...
      const op = choice.op;
      act.mutate(
        { id: record.id, op, expectedVersion: record.version, ...(correctionDate ? { correctionDate } : {}) },
        {
          onSuccess: (rec) => {
            toast.success(choice.done);
            onAfter(op, rec);
  ...
  const actions = [
    {
      op: 'submit' as const,
      visible: isDoc && ds === 0 && entity.ops.includes('submit'),
      title: t(S.submit),
      message: t(S.confirmSubmit),
      done: t(S.submitted),
    },
    {
      op: 'cancel' as const,
      visible: isDoc && ds === 1 && entity.ops.includes('cancel'),
      title: t(S.cancelDoc),
      message: t(S.confirmCancel),
      done: t(S.cancelled),
    },
    {
      op: 'amend' as const,
      visible: isDoc && ds === 2 && entity.ops.includes('amend'),
      title: t(S.amend),
      message: t(S.amend),
      done: t(S.amended),
    },
    {
      op: 'delete' as const,
      visible: entity.ops.includes('delete') && (!isDoc || ds === 0),
      title: t(S.delete),
      message: t(S.confirmDelete),
      done: t(S.deleted),
    },
  ].filter((a) => a.visible);
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {blocked ? (
        <span role="status" className="dirty-label">
          {blocked}
        </span>
      ) : null}
      {submitBlocked && ds === 0 ? (
        <span role="alert" data-testid="submit-blocked" className="text-xs text-red-700">
          {submitBlocked}
        </span>
      ) : null}
      {actions.map((action) => (
        <button
          key={action.op}
          type="button"
          className={`btn ${action.op === 'submit' ? 'btn-primary' : action.op === 'cancel' ? 'btn-danger' : ''}`}
          disabled={busy || (action.op === 'submit' && submitBlocked !== undefined)}
          title={blocked ?? (action.op === 'submit' ? submitBlocked : undefined)}
      ...
      {choice ? (
        <ActionConfirm
          ...
          cancelDocument={choice.op === 'cancel'}
          destructive={choice.op === 'cancel' || choice.op === 'delete'}
          onConfirm={execute}
