...
export function CompanyPicker({ disabled }: { disabled: boolean }) {
  ...
  const change = (id: string) => {
    ...
    } catch (err) { setSwitching(false); toast.error(err); }
  };
  return <div className="company-picker"><Icon name="building" /><div><label htmlFor="template-company">{t({ ja: '対象の会社', en: 'Selected company' })}</label>
    {companies.isError ? <span role="alert">{t({ ja: '会社一覧を取得できませんでした', en: 'Unable to load companies' })}</span> : <select id="template-company" value={selected ?? ''} disabled={disabled || switching || !companies.data} onChange={(e) => change(e.target.value)}>
