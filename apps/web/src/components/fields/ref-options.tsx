...
export function RefOptions({
  ...
  const { t } = useLocale();
  return (
    <div className="absolute z-20 mt-0.5 w-full rounded border border-neutral-300 bg-white shadow-lg">
      {pending ? (
        <p role="status" className="px-3 py-2 text-xs text-neutral-600">
          {t({ ja: '候補を検索中…', en: 'Searching for matches…' })}
        </p>
      ) : null}
      {failed ? (
        <div role="alert" className="px-3 py-2 text-xs text-red-800">
          <p>
            {t({
              ja: '候補を取得できませんでした。検索条件を保持しています。',
              en: 'Could not load matches. Your search is preserved.',
            })}
          </p>
      ...
      >
        {items.map((row, index) => (
          ...
          >
            {refLabel(row, field.refDisplayField)}
            {typeof row.code === 'string' && row.code ? (
              <span className="ml-2 font-mono text-xs text-neutral-500">{row.code}</span>
            ) : null}
          </li>
        ))}
      </ul>
