  ...
  if (col.kind === 'ref' && col.ref && typeof value === 'string' && value) {
    return (
      <td className={cls}>
        <Link
          to="/e/$entity/$id"
          params={{ entity: col.ref, id: value }}
    ...
    <tfoot className="border-t-2 border-neutral-300 bg-neutral-50 font-medium">
      <tr data-testid="report-totals">
        {result.columns.map((col, i) => {
          ...
          const text =
            v !== undefined ? formatValue(asField(col), v, locale, { currencyScale }).text : i === 0 ? t(S.sums) : '';
          return (
            <td
              key={col.key}
              data-total={col.key}
              className={`px-2 py-1 whitespace-nowrap ${alignOf(col) === 'right' ? 'num' : ''}`}
            >
              {text}
            </td>
          );
...
function ExtraTotals({ result, currencyScale }: { result: TableResult; currencyScale: number }) {
  ...
  return (
    <section
      aria-label={t(S.sums)}
      data-testid="report-extra-totals"
    ...
    <div className="flex flex-col gap-2 report-browse">
      <div className="report-browse-toolbar">
        <label htmlFor={searchId}>
          {t({ ja: '結果内を検索', en: 'Search returned rows' })}
          <input
            id={searchId}
            className="input"
            type="search"
            ...
            placeholder={t({ ja: '名称・日付・金額など', en: 'Name, date, amount…' })}
          />
        </label>
        <label>
          {t({ ja: '1ページの行数', en: 'Rows per page' })}
          <select
            className="input"
            value={pageSize}
            onChange={(event) => {
        ...
        {sort ? (
          ...
          >
            {t({ ja: '帳票の順序に戻す', en: 'Restore report order' })}
          </button>
        ) : null}
      </div>
      <p className="report-browse-note" role="status" data-testid="report-visible-count">
        {t({
          ja: `受信 ${view.total.toLocaleString()} 行 · 一致 ${view.matched.toLocaleString()} 行 · ${view.from.toLocaleString()}–${view.to.toLocaleString()} 行を表示`,
          en: `${view.total.toLocaleString()} returned · ${view.matched.toLocaleString()} matching · showing ${view.from.toLocaleString()}–${view.to.toLocaleString()}`,
        })}
      </p>
      {totalsPresent ? (
        <p className="report-browse-note">
          {t({
            ja: '合計欄は受信した帳票全体の集計値です。検索やページ移動では変わりません。CSV出力にも帳票全体が含まれます。',
            en: 'Totals describe the returned report and do not change with search or pagination. CSV export also includes the full report.',
          })}
        </p>
      ) : null}
      ...
      <nav aria-label={t({ ja: '帳票結果のページ移動', en: 'Report pagination' })} className="report-browse-pages">
        <button type="button" className="btn" disabled={view.page === 0} onClick={() => setPage(0)}>
          {t({ ja: '最初', en: 'First' })}
        </button>
        <button type="button" className="btn" disabled={view.page === 0} onClick={() => setPage(view.page - 1)}>
          {t({ ja: '前へ', en: 'Previous' })}
        </button>
        <label htmlFor={pageId}>
          ...
          <input
            id={pageId}
            className="input report-page-number"
            type="number"
            min={1}
            max={view.pages}
            value={view.page + 1}
            onChange={(event) => {
              ...
              if (Number.isInteger(value) && value >= 1) setPage(Math.min(view.pages, value) - 1);
            }}
          />{' '}
          ...
          <tr>
            {result.columns.map((col) => (
              <th
                ...
                className={`border-b border-neutral-200 px-2 py-1.5 font-medium whitespace-nowrap ${alignOf(col) === 'right' ? 'text-right' : ''}`}
              >
                <button
                  type="button"
                  className="report-sort-button"
                  onClick={() => sortBy(col.key)}
                  title={t({ ja: '昇順 → 降順 → 帳票の順序', en: 'Ascending → descending → report order' })}
                >
                  {t(col.label)}{' '}
                  <span aria-hidden="true">{sort?.key === col.key ? (sort.direction === 'asc' ? '↑' : '↓') : '↕'}</span>
                </button>
              </th>
            ))}
          </tr>
            ...
            <tr>
              <td colSpan={Math.max(1, result.columns.length)} className="px-2 py-6 text-center text-neutral-500">
                {result.rows.length === 0
                  ? t(S.reportEmpty)
                  : t({ ja: '検索条件に一致する行がありません。', en: 'No rows match your search.' })}
              </td>
            </tr>
          ) : (
