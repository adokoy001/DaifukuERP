...
export function lineLockReason(state: {
  canWrite: boolean;
  saving: boolean;
  actionBusy: boolean;
  conflict: boolean;
  ...
  if (state.conflict)
    return {
      ja: '競合があります。最新の記録を読み込んでください',
      en: 'A conflict requires reloading the latest record.',
    };
  if (!state.canWrite)
    return { ja: '閲覧モードのため明細は変更できません', en: 'Lines cannot be changed in view mode.' };
