...
import {
  ...
  mutateAnalyses,
  sameSavedAnalysis,
  type AnalysisLockManager,
  type AnalysisStorage,
  type SavedAnalysis,
} from './analytics-storage.ts';
...
function entry(id = 'analysis-1'): SavedAnalysis {
  return { id, name: '月次の売上', updatedAt: '2026-09-13T04:00:00.000Z', settings: settings() };
}
...
describe('cross-tab analysis mutations', () => {
  ...
  it('checks current content as well as timestamps and rejects stale save or delete without losing other analyses', async () => {
    ...
    await expect(
      mutateAnalyses(storage, locks, key, { kind: 'save', entry: { ...baseline, name: '古いタブで変更' }, baseline }),
    ).rejects.toMatchObject({ code: 'conflict' });
    await expect(
      mutateAnalyses(storage, locks, key, { kind: 'delete', id: baseline.id, baseline }),
    ).rejects.toMatchObject({ code: 'conflict' });
  ...
  it('only deletes the matching loaded version and refuses to resurrect an analysis another tab deleted', async () => {
    ...
    await expect(
      mutateAnalyses(storage, locks, key, { kind: 'save', entry: baseline, baseline }),
    ).rejects.toMatchObject({ code: 'conflict' });
    await expect(mutateAnalyses(storage, locks, key, { kind: 'delete', id: 'other', baseline })).rejects.toMatchObject({
      code: 'conflict',
    });
  });
  it('does not overwrite an existing id as a new analysis or bypass limits using a stale list', async () => {
    ...
    await expect(mutateAnalyses(storage, locks, key, { kind: 'save', entry: entry('new-id') })).rejects.toThrow(
      'Up to 30',
    );
  ...
  it('never falls back to an unlocked write when Web Locks are unavailable or rejected', async () => {
    ...
    await expect(mutateAnalyses(storage, undefined, key, { kind: 'save', entry: entry() })).rejects.toMatchObject({
      code: 'unsupported',
    });
    const locks: AnalysisLockManager = { request: () => Promise.reject(new Error('Lock access denied')) };
    await expect(mutateAnalyses(storage, locks, key, { kind: 'save', entry: entry() })).rejects.toThrow(
      'Lock access denied',
    );
...
function stored(value: unknown): AnalysisStorage {
  return { getItem: () => JSON.stringify(value), setItem: vi.fn() };
}
describe('scoped analysis persistence', () => {
  ...
  it('isolates every tenant, user and company dimension', () => {
    const storage = memory();
    writeAnalyses(storage, key, [entry()]);
    for (const scope of [
  ...
  it('cannot collide when scope values contain separators or percent escapes', () => {
    const scopes = [
      ...
      { tenantId: 'a', userId: 'b.c', companyId: 'd' },
      { tenantId: 'a', userId: 'b', companyId: 'c.d' },
      { tenantId: 'a%2Eb', userId: 'c', companyId: 'd' },
      { tenantId: 'a/b', userId: 'c', companyId: 'd' },
    ];
  ...
  it('allows updating and deleting a saved analysis without retaining old content', () => {
    ...
    const updated = {
      ...entry(),
      name: '改訂した月次売上',
      settings: { ...settings(), period: 'custom' as const, from: '2024-04-01', to: '2025-03-31' },
    };
    writeAnalyses(storage, key, [updated]);
    expect(readAnalyses(storage, key)).toEqual([updated]);
    writeAnalyses(storage, key, []);
    expect(readAnalyses(storage, key)).toEqual([]);
  });
});
describe('stored configuration validation', () => {
  ...
  it.each(['snapshot', 'records', 'accessToken', 'password'])(
    'rejects unexpected %s instead of persisting raw data or credentials',
    (field) => {
  ...
  it('rejects extra properties nested in axes, measures or saved entries', () => {
    ...
    expect(
      parseAnalyticsSettings({
        ...base,
        pivot: { ...base.pivot, rows: [{ field: 'date', grain: 'year', rows: [{ amount: 'private' }] }] },
      }),
    ).toBeUndefined();
    expect(
      parseAnalyticsSettings({
        ...base,
        pivot: { ...base.pivot, measures: [{ field: 'amount', op: 'sum', values: ['private'] }] },
      }),
    ).toBeUndefined();
    expect(() =>
      readAnalyses(stored({ version: 1, items: [{ ...entry(), rows: [{ amount: 'private' }] }] }), key),
    ).toThrow();
  });
  it('rejects unsupported envelope data instead of accepting a hidden snapshot', () => {
    expect(() =>
      readAnalyses(stored({ version: 1, items: [entry()], snapshot: [{ amount: 'private' }] }), key),
    ).toThrow();
  });
  ...
  it('rejects impossible/reversed dates and unbounded expansion keys', () => {
    for (const value of [
      { ...settings(), from: '2025-02-29' },
      { ...settings(), from: '2027-01-01' },
      { ...settings(), to: '' },
      { ...settings(), expandedRows: new Array<string>(1001).fill('[]') },
  ...
  it('bounds count and payload size before invoking the storage write', () => {
    ...
    expect(() =>
      writeAnalyses(
        storage,
        key,
        Array.from({ length: 31 }, (_, i) => entry(`id-${i}`)),
      ),
    ).toThrow();
    expect(storage.setItem).not.toHaveBeenCalled();
  ...
  it('propagates storage access/quota failures for a visible UI recovery message', () => {
    const forbidden: AnalysisStorage = {
      getItem: () => {
