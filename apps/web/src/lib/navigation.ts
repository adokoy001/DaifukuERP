...
const RECORD_PATH =
  /^\/e\/([a-z][a-z0-9_]*)(?:\/(?:new|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}))?$/i;
...
function add(entries: Map<string, NavigationEntry>, entry: NavigationEntry): void {
  ...
  if (!previous) {
    entries.set(entry.href, { ...entry, aliases: uniqueLabels(entry.aliases) });
    return;
  }
...
function allowedDedicated(definition: DedicatedNavigation, meta: AppMeta, tenantAdmin: boolean): boolean {
  ...
  if (definition.condition === 'reports')
    return meta.actions.some((action) => ACTION_NAME.test(action.name) && action.resultKind === 'table');
...
function dedicatedEntry(definition: DedicatedNavigation, meta: AppMeta): NavigationEntry {
  ...
  return {
    ...
    kind: definition.kind ?? 'workspace',
    featured: true,
    aliases: moduleLabel ? [moduleLabel] : [],
...
function recordEntry(entity: EntityMeta, meta: AppMeta, label = entity.label): NavigationEntry {
  ...
  return {
    ...
    href,
    label,
    description: { ja: `${entity.label.ja}を一覧・検索します。`, en: `Browse and search ${entity.label.en}.` },
    workspace: workspaceForModule(entity.module),
    kind: 'record',
    featured: false,
    aliases: [entity.label, ...(moduleLabel ? [moduleLabel] : [])],
...
function actionEntry(action: ActionMeta, meta: AppMeta, label = reportTitle(action)): NavigationEntry {
  ...
  return {
    ...
    workspace: report ? 'reports' : workspaceForModule(action.module),
    kind: report ? 'report' : 'action',
    module: action.module,
    featured: false,
    aliases: [reportTitle(action), ...(moduleLabel ? [moduleLabel] : [])],
  };
}
function menuEntry(
  item: MenuItem,
  ...
  dedicated: Map<string, NavigationEntry>,
): NavigationEntry | undefined {
  if (item.entity && item.route) return undefined;
  if (item.entity) {
    const entity = entities.get(item.entity);
    return entity ? recordEntry(entity, meta, item.label) : undefined;
  }
  ...
  return !action.generic && action.resultKind !== 'table' && action.module !== 'pack'
    ? actionEntry(action, meta, item.label)
    : undefined;
}
...
export function buildNavigation(meta: AppMeta | undefined, tenantAdmin: boolean): NavigationCatalog {
  ...
  const entities = new Map(
    meta.entities
      .filter((entity) => ENTITY_NAME.test(entity.name) && entity.ops.includes('read') && !lineNames.has(entity.name))
      .map((entity) => [entity.name, entity]),
  );
  const actions = new Map(
    meta.actions.filter((action) => ACTION_NAME.test(action.name)).map((action) => [action.name, action]),
  );
  const entries = new Map<string, NavigationEntry>();
  for (const definition of DEDICATED_NAVIGATION)
    if (allowedDedicated(definition, meta, tenantAdmin)) add(entries, dedicatedEntry(definition, meta));
  const dedicated = new Map(entries);
  for (const module of meta.modules)
    for (const item of [...module.menus].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))) {
  ...
  return {
    ...
    workspaces: WORKSPACES.map((workspace) => ({
      ...workspace,
      entries: all.filter((entry) => entry.workspace === workspace.id),
...
function searchRank(entry: NavigationEntry, terms: string[], locale: Locale): number | undefined {
  ...
  const content = normalize(
    [
      ...
      ...entry.aliases.flatMap((alias) => [alias.ja, alias.en]),
      entry.module ?? '',
      entry.href,
    ].join(' '),
    locale,
  ...
  return terms.reduce(
    (score, term) => score + (label === term ? 8 : label.startsWith(term) ? 4 : label.includes(term) ? 2 : 0),
    0,
  );
}
...
export function searchNavigation(catalog: NavigationCatalog, query: string, locale: Locale): NavigationEntry[] {
  ...
  return catalog.entries
    .flatMap((entry) => {
      const rank = searchRank(entry, terms, locale);
    ...
    .sort(
      (a, b) =>
        ...
        collator.compare(a.entry.label[locale], b.entry.label[locale]) ||
        (a.entry.href < b.entry.href ? -1 : a.entry.href > b.entry.href ? 1 : 0),
    )
    .map((item) => item.entry);
}
