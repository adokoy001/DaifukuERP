...
const field = (name: string, kind: string, extra: Partial<FieldMeta> = {}): FieldMeta => ({
  name,
  kind,
...
describe('UI refresh input contract', () => {
  it('shows literal defaults without replacing existing false/null values', () => {
    const fields = [
      field('active', 'bool', { hasDefault: true, defaultValue: true }),
      field('taxCategory', 'enum', { defaultValue: 'standard' }),
      field('taxes', 'json', { defaultValue: [] }),
    ];
    expect(initialValues(fields)).toEqual({ active: true, taxCategory: 'standard', taxes: '[]' });
    expect(initialValues(fields, { active: false, taxCategory: null, taxes: null })).toEqual({
      active: false,
      taxCategory: '',
      taxes: '',
    });
  ...
  it('never requires or sends computed fields, even when a client supplied a value', () => {
    const fields = [
      ...
      field('taxes', 'json', { required: true, serverOwned: true }),
      field('balance', 'decimal', { required: true, readOnly: true }),
    ];
    ...
    const plan = planSave({
      ...
      mode: 'create',
      record: undefined,
      values: { name: 'Invoice', taxes: 'broken JSON', balance: '0' },
      specs: [],
      lines: {},
      originalLines: {},
    ...
    for (const f of fields.slice(1))
      expect(isFieldEditable(f, { mode: 'update', docstatus: 1, allowOnSubmit: [f.name] })).toBe(false);
  });
  it('compares unsaved rows by values/order, not regenerated UI row keys', () => {
    ...
    expect(formFingerprint({}, { lines: before })).not.toBe(
      formFingerprint({}, { lines: [{ ...original, values: { quantity: '3' } }] }),
    );
...
describe('product selection', () => {
  const product: RecordJson = {
    id: 'p1',
    name: '商品',
    salePrice: '100.123456',
    purchasePrice: '80',
  ...
  const columns = [
    ...
    field('taxCategory', 'enum'),
    field('uomId', 'ref'),
  ];
  it('fills the displayed invoice cells using exact Decimal strings', () => {
    expect(productDefaults(product, columns, false)).toEqual({
      description: '商品',
  ...
  it('does not invent prices or overwrite server-owned cells', () => {
    expect(
      productDefaults(
        ...
        [...columns, field('amount', 'decimal', { serverOwned: true })],
        false,
      ),
    ).not.toHaveProperty('unitPrice');
...
describe('schema mode switching', () => {
  ...
  it('round-trips unsaved changes in both directions and retains unknown JSON keys', () => {
    const json = switchSchemaMode(
      { values: { title: 'new value' }, errors: {}, json: '{"title":"old","extra":true}', jsonMode: false },
      fields,
    );
...
describe('CSV formula safety', () => {
  it('escapes dangerous textual prefixes, preserves genuine negative decimal amounts', () => {
    const result: TableResult = {
      title: { ja: '', en: '' },
      columns: [
        { key: 'name', kind: 'text', label: { ja: '=HEADER', en: 'Name' } },
        { key: 'amount', kind: 'decimal', label: { ja: '金額', en: 'Amount' } },
      ],
      rows: [
        { name: '=1+1', amount: '-123.45' },
        { name: '\t@SUM(A1)', amount: '=1+1' },
        { name: '-text', amount: '+100' },
      ],
    };
    ...
    const rows = [
      {
        ...
        label: '軽減8%',
        rate: '0.08',
        taxable: '12345678901234.5',
        tax: '987654312098',
        gross: '13333333213332.5',
      },
    ];
