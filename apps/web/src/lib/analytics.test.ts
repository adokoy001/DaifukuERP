...
const dataset: AnalyticsDataset = {
  id: 'sales',
  title: { ja: '売上', en: 'Sales' },
  description: { ja: '確定した請求', en: 'Submitted invoices' },
  grain: { ja: '請求1件', en: 'One invoice' },
  dateField: 'date',
  dimensions: [
    { key: 'date', label: { ja: '請求日', en: 'Invoice date' }, kind: 'date' },
    { key: 'partner', label: { ja: '取引先', en: 'Customer' }, kind: 'text' },
  ],
  measures: [{ key: 'amount', label: { ja: '金額', en: 'Amount' }, kind: 'decimal' }],
  defaultRows: ['partner'],
  defaultColumns: ['date'],
  defaultMeasure: 'amount',
  defaultState: 'submitted',
  states: [
    { value: 'submitted', label: { ja: '確定', en: 'Submitted' } },
...
describe('analysis business dates and useful defaults', () => {
  it('changes month at midnight in Japan, independent of the browser time zone', () => {
    expect(periodDates('month', new Date('2026-09-30T14:59:59.999Z'))).toEqual({
      from: '2026-09-01',
      to: '2026-09-30',
    });
    expect(periodDates('month', new Date('2026-09-30T15:00:00.000Z'))).toEqual({
      from: '2026-10-01',
      to: '2026-10-01',
    });
  });
  ...
  it('resolves previous month across leap years and January boundaries', () => {
    expect(periodDates('previous-month', new Date('2024-03-01T00:00:00Z'))).toEqual({
      from: '2024-02-01',
      to: '2024-02-29',
    });
    expect(periodDates('previous-month', new Date('2025-03-01T00:00:00Z'))).toEqual({
      from: '2025-02-01',
      to: '2025-02-28',
    });
    expect(periodDates('previous-month', new Date('2026-01-01T00:00:00Z'))).toEqual({
      from: '2025-12-01',
      to: '2025-12-31',
    });
  });
  it('re-resolves saved relative dates at recall while preserving custom dates', () => {
    ...
    expect(
      settingsDates(
        { ...settings, period: 'custom', from: '2024-04-01', to: '2025-03-31' },
  ...
  it.each([
    ...
    '',
  ])('rejects impossible or noncanonical date %s', (value) => {
  ...
  it('uses approved source defaults and derives date grouping from metadata', () => {
    ...
    expect(defaults.pivot).toEqual({
      rows: [{ field: 'partner', grain: 'value' }],
      columns: [{ field: 'date', grain: 'month' }],
...
describe('templates and source compatibility', () => {
  it('only offers patterns for authorized catalog entries, with usable dimensions', () => {
    ...
    expect(templates.find((template) => template.id === 'sales-monthly')?.settings).toMatchObject({
      chart: 'line',
      pivot: {
        rows: [
  ...
  it('rejects a removed source/state/field or a date grain on a text field', () => {
    ...
    const cases: AnalyticsSettings[] = [
      { ...defaults, dataset: 'private-payroll' },
      { ...defaults, state: 'removed-state' },
