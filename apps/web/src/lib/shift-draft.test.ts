...
const published: ShiftPlanSummary = {
  id: 'published',
  siteId: 'site',
  ...
  slots: [],
  assignments: [{ slotId: 'slot', employeeId: 'employee', locked: true }],
  seed: 13,
  acknowledgeShortage: false,
  reason: 'approved',
  publishedAt: '2026-09-12T09:00:00+09:00',
};
const board: ShiftBoard = {
  site: { id: 'site', code: 'S', name: 'Main' },
  weekStart: '2026-09-14',
  sourceRevision: 'latest',
  problem: { weekStart: '2026-09-14', employees: [], slots: [], availability: [], leave: [], existing: [], rules: [] },
  employeeVersions: [],
  profiles: [],
  availability: [],
  draft: null,
...
describe('shift draft concurrency and publication boundaries', () => {
  it('does not silently accept a saved draft after profiles or availability changed', () => {
    const current = {
      ...board,
      draft: { ...published, id: 'draft', status: 'draft' as const, sourceRevision: 'older-sources' },
    };
  ...
  it('makes a revision new while preserving the published allocation and locks', () => {
    ...
    expect(revision).not.toHaveProperty('planId');
    expect(revision.expectedVersion).toBe(0);
    expect(revision.assignments).toEqual(published.assignments);
    expect(revision.sourceRevision).toBe('latest');
  ...
  it('detects concurrent cancellation, replacement and update without mistaking viewing a publication for a draft conflict', () => {
    ...
    expect(
      draftWasReplaced(draft, { ...board, draft: { ...published, id: 'draft', status: 'draft', version: 3 } }, true),
    ).toBe(true);
    expect(draftWasReplaced(draft, { ...board, draft: { ...published, id: 'other', status: 'draft' } }, true)).toBe(
      true,
    );
    expect(draftWasReplaced(draft, { ...board, draft: { ...published, id: 'draft', status: 'draft' } }, true)).toBe(
      false,
    );
    ...
    expect(
      draftWasReplaced(unsaved, { ...board, draft: { ...published, id: 'concurrent', status: 'draft' } }, true),
    ).toBe(true);
  });
});
