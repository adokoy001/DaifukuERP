...
export function parseOperationsSearch(raw: Record<string, unknown>, now = new Date()): OperationsFilters {
  ...
  return {
    from: validDate(raw.from) ? raw.from : today.slice(0, 8) + '01',
    to: validDate(raw.to) ? raw.to : today,
    asOf: validDate(raw.asOf) ? raw.asOf : today,
    ...(typeof raw.storeId === 'string' && /^[0-9a-f-]{36}$/i.test(raw.storeId) ? { storeId: raw.storeId } : {}),
  };
}
export function maximumMagnitude(values: string[]): string {
  return values.reduce((max, value) => {
    const abs = value.replace(/^[+-]/, '');
    return compareDecimalStrings(abs, max) === 1 ? abs : max;
  }, '0');
}
...
export const reviewLabels: Record<string, { ja: string; en: string }> = {
  review_pending: { ja: '店長確認待ち', en: 'Awaiting review' },
  finalize_pending: { ja: '本部確定待ち', en: 'Awaiting posting' },
  scheduled_closed: { ja: '休業予定', en: 'Planned closure' },
  draft: { ja: '下書き', en: 'Draft' },
  submitted: { ja: '店長確認待ち', en: 'Awaiting review' },
  approved: { ja: '本部確定待ち', en: 'Awaiting posting' },
  returned: { ja: '差戻し', en: 'Returned' },
  finalized: { ja: '確定済み', en: 'Posted' },
  missing: { ja: '未提出', en: 'Missing' },
  no_sales: { ja: '売上ゼロ', en: 'No sales' },
  closed: { ja: '休業', en: 'Closed' },
  cancelled: { ja: '取消', en: 'Cancelled' },
  unplanned: { ja: '計画なし', en: 'Unplanned' },
};
