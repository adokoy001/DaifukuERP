...
export interface ShiftDraft { slots: ShiftSlot[]; assignments: ShiftAssignment[]; seed: number; sourceRevision: string; planId?: string; expectedVersion: number }
...
export function draftFromBoard(board: ShiftBoard): ShiftDraft {
  ...
  return { slots: plan?.slots ?? [], assignments: plan?.assignments ?? [], seed: plan?.seed ?? 1,
    sourceRevision: board.draft?.sourceRevision ?? board.sourceRevision,
    ...(plan ? { planId: plan.id } : {}), expectedVersion: plan?.version ?? 0 };
}
...
export function draftWasReplaced(draft: ShiftDraft, board: ShiftBoard, editing: boolean): boolean {
  return editing && (draft.planId ? board.draft?.id !== draft.planId || board.draft?.version !== draft.expectedVersion : Boolean(board.draft));
}
