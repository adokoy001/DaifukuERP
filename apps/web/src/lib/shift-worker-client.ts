...
interface Callbacks {
  result(value: ShiftRecommendation): void;
  failed(code: ShiftWorkerFailure): void;
...
export function createShiftRunner(factory: () => ShiftWorkerPort | null, callbacks: Callbacks, timeoutMs = 20000) {
  ...
  const finish = (worker: ShiftWorkerPort, outcome: () => void) => {
