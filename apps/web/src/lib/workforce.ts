...
export const workforceStatuses: Record<string, Label> = {
  working: { ja: '勤務中', en: 'Working' },
  break: { ja: '休憩中', en: 'On break' },
  closed: { ja: '退勤済み', en: 'Clocked out' },
  submitted: { ja: '確認待ち', en: 'Submitted' },
...
export function minutesLabel(minutes: number, locale: Locale): string {
  ...
  return locale === 'ja'
    ? `${Math.floor(whole / 60)}時間 ${whole % 60}分`
...
export const leavePortions: Record<string, Label> = {
  full: { ja: '1日', en: 'Full day' },
  morning: { ja: '午前半日', en: 'Morning' },
  afternoon: { ja: '午後半日', en: 'Afternoon' },
};
export const deductionLabels = {
  income_tax: { ja: '所得税', en: 'Income tax' },
  resident_tax: { ja: '住民税', en: 'Resident tax' },
  health_insurance: { ja: '健康保険', en: 'Health insurance' },
  nursing_insurance: { ja: '介護保険', en: 'Nursing care insurance' },
  pension: { ja: '厚生年金', en: 'Employees pension' },
  employment_insurance: { ja: '雇用保険', en: 'Employment insurance' },
  child_support: { ja: '子ども・子育て支援金', en: 'Child support contribution' },
  other: { ja: 'その他控除', en: 'Other deductions' },
