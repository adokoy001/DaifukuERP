...
describe('annual declaration wire input', () => {
  it('does not silently assert legal eligibility when confirmations are unchecked', () => {
    ...
    expect(result.housingEligibilityConfirmed).toBe(false);
    expect(result.distinctEarthquakeContractsConfirmed).toBe(false);
    expect(result.spouse).toBeNull();
    expect(result.relatives).toEqual([]);
    expect(result.otherIncome).toBe('');
  });
  it('preserves yen strings and individual family confirmations without substituting facts', () => {
    const data = new FormData();
    for (const [key, value] of Object.entries({
      otherIncome: '999999999999',
      resident: 'on',
      factsConfirmed: 'on',
      'relative0.code': 'Child1',
      'relative0.income': '850000',
      'relative0.resident': 'on',
      'relative0.sharedLivelihoodConfirmed': 'on',
      'previous0.taxablePay': '1234567',
      'unpaid0.period': '2026-01',
...
describe('working-time calendar input', () => {
  it('covers complete calendar months across leap years and year boundaries with a strict size limit', () => {
    ...
    const days = monthDays('2026-12', '2027-02');
    ...
    for (const [from, to] of [
      ...
      expect(monthDays(from as string, to as string)).toEqual([]);
  });
  ...
  it('does not turn empty or malformed time controls into valid midnight', () => {
    expect(timeMinute('09:30')).toBe(570);
    expect(timeInput(570)).toBe('09:30');
