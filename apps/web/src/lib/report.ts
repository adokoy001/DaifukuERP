...
const INPUT_LABELS: Record<string, Label> = {
  from: { ja: '開始日', en: 'From' }, to: { ja: '終了日', en: 'To' },
  asOf: { ja: '基準日', en: 'As of' }, date: { ja: '対象日', en: 'Date' },
  period: { ja: '対象月（YYYY-MM）', en: 'Period (YYYY-MM)' }, month: { ja: '対象月（YYYY-MM）', en: 'Month (YYYY-MM)' },
  year: { ja: '対象年（西暦）', en: 'Year' },
  accountId: { ja: '勘定科目', en: 'Account' }, productId: { ja: '商品・品目', en: 'Product' },
  warehouseId: { ja: '倉庫', en: 'Warehouse' }, partnerId: { ja: '取引先', en: 'Partner' },
};
...
export function reportInputFields(fields: readonly SchemaField[]): SchemaField[] {
  return fields.map((field) => {
    const generated = field.name.replace(/([A-Z])/g, ' $1').replace(/_/g, ' ').replace(/^./, (c) => c.toUpperCase());
...
export function reportPeriodOptions(fields: readonly SchemaField[]): ReportPeriod[] {
  ...
  const hasRange = ['from', 'to'].every((name) => fields.some((field) => field.name === name && periodField(field) === 'date'));
...
export function reportPeriodLabel(fields: readonly SchemaField[], period: ReportPeriod): Label {
  ...
  if (kinds.every((kind) => kind === 'year')) return period === 'current' ? { ja: '今年', en: 'This year' } : { ja: '前年', en: 'Previous year' };
  if (kinds.every((kind) => kind === 'date') && !fields.some((field) => field.name === 'from')) return period === 'current' ? { ja: '今日', en: 'Today' } : { ja: '先月末', en: 'Previous month end' };
...
export function reportPeriodValues(fields: readonly SchemaField[], period: ReportPeriod, now = new Date()): Record<string, string | number> {
  ...
  const onlyYear = fields.filter((field) => periodField(field) !== undefined).every((field) => periodField(field) === 'year');
  for (const field of fields) {
    ...
    if (kind === 'year') values[field.name] = onlyYear ? year - (period === 'previous' ? 1 : 0) : Number(to.slice(0, 4));
  }
  return values;
}
...
export function reportDateErrors(input: Record<string, unknown>, fields: readonly SchemaField[], locale: Locale): Record<string, string> {
  const errors: Record<string, string> = {};
  for (const field of fields) {
    const value = input[field.name];
    if (periodField(field) === 'date' && value !== undefined && value !== null && value !== '' && !validDate(value)) {
      errors[field.name] = locale === 'ja' ? '実在する日付を YYYY-MM-DD で入力してください。' : 'Enter a valid date as YYYY-MM-DD.';
    }
  }
  const range = ['from', 'to'].every((name) => fields.some((field) => field.name === name && periodField(field) === 'date'));
  if (range && validDate(input.from) && validDate(input.to) && input.from > input.to) {
    errors.to = locale === 'ja' ? '終了日は開始日以降にしてください。' : 'The end date must be on or after the start date.';
  }
  return errors;
}

export interface ReportSort { key: string; direction: 'asc' | 'desc' }
export interface ReportRow { row: Record<string, unknown>; index: number }
export interface ReportRowsPage { rows: ReportRow[]; matched: number; total: number; page: number; pages: number; from: number; to: number }
...
export function reportRowsPage(result: TableResult, opts: {
  query?: string; sort?: ReportSort; page?: number; pageSize?: number; locale?: Locale;
  cellText?: (column: TableColumn, value: unknown) => string;
} = {}): ReportRowsPage {
  const pageSize = opts.pageSize === 25 ? 25 : 50;
  const query = (opts.query ?? '').trim().normalize('NFKC').toLocaleLowerCase(opts.locale ?? 'ja');
  let rows = result.rows.map((row, index) => ({ row, index }));
  if (query) rows = rows.filter(({ row }) => result.columns.some((column) => {
    const value = row[column.key];
    const raw = missing(value) ? '' : String(value);
    const displayed = opts.cellText?.(column, value) ?? '';
    return `${raw}\n${displayed}`.normalize('NFKC').toLocaleLowerCase(opts.locale ?? 'ja').includes(query);
  }));
  ...
  if (column && opts.sort) {
    ...
    rows.sort((a, b) => {
      const av = a.row[column.key], bv = b.row[column.key];
  ...
  const matched = rows.length, pages = Math.max(1, Math.ceil(matched / pageSize));
  const requested = typeof opts.page === 'number' && Number.isFinite(opts.page) ? Math.floor(opts.page) : 0;
  const page = Math.max(0, Math.min(pages - 1, requested)), offset = page * pageSize;
  return { rows: rows.slice(offset, offset + pageSize), matched, total: result.rows.length, page, pages, from: matched ? offset + 1 : 0, to: Math.min(matched, offset + pageSize) };
}
