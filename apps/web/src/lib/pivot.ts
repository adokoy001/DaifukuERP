...
export class PivotError extends Error {
  constructor(readonly code: PivotErrorCode) {
    super(code);
...
function defined<T>(value: T | undefined): T {
  if (value === undefined) throw new PivotError('invalid_value');
  return value;
}
...
function decimal(input: unknown): Decimal | null {
  ...
  if (!parsed || parsed.int.length + parsed.frac.length > MAX_DECIMAL_DIGITS || parsed.frac.length > MAX_DECIMAL_SCALE)
    throw new PivotError('invalid_value');
...
function calendarValue(input: string, grain: Exclude<PivotGrain, 'value'>): string {
  ...
  if (
    ...
    day < 1 ||
    day > (days[month - 1] ?? 0) ||
    (input.length > 10 && (!Number.isFinite(Date.parse(input)) || Number(input.slice(11, 13)) > 23))
  )
    throw new PivotError('invalid_date');
...
export function validatePivotConfig(config: PivotConfig): void {
  if (
    ...
    !Array.isArray(config.measures) ||
    config.rows.length > 3 ||
    config.columns.length > 3 ||
    config.measures.length < 1 ||
    config.measures.length > 3
  )
    throw new PivotError('invalid_config');
  for (const axes of [config.rows, config.columns]) {
    const seen = new Set<string>();
    for (const axis of axes) {
      if (!axis || !validField(axis.field) || (axis.grain !== undefined && !grains.has(axis.grain)))
        throw new PivotError('invalid_config');
...
function root(): PivotNode {
  return { key: PIVOT_ROOT_KEY, path: [], depth: 0, parent: null, children: [], label: '総計' };
}
function ensurePath(nodes: Map<string, PivotNode>, path: (string | null)[]): string[] {
  ...
  for (let depth = 1; depth <= path.length; depth++) {
    ...
    nodes.set(key, {
      ...
      parent,
      children: [],
      label: value === null ? '(未設定)' : value === '' ? '(空文字)' : value,
    });
...
function sortedNodes(nodes: Map<string, PivotNode>): PivotNode[] {
  ...
  const visit = (key: string) => {
    const node = defined(nodes.get(key));
    ordered.push(node);
...
function accumulate(state: Aggregate, observation: Observation, op: PivotOperation): void {
  if (op === 'rows' || op === 'count') {
    if (observation.present) state.count++;
    return;
  }
  ...
  else if ((op === 'min' && compare(value, state.value) < 0) || (op === 'max' && compare(value, state.value) > 0))
    state.value = value;
}
...
export function pivot(rows: readonly Record<string, unknown>[], config: PivotConfig): PivotResult {
  ...
  for (const row of rows) {
    ...
    const rowKeys = ensurePath(
      rowNodes,
      config.rows.map((axis) => axisValue(row, axis)),
    );
    const columnKeys = ensurePath(
      columnNodes,
      config.columns.map((axis) => axisValue(row, axis)),
    );
    const observations = config.measures.map((measure) => observe(row, measure));
    for (const rowKey of rowKeys)
      for (const columnKey of columnKeys) {
        const aggregates = getCell(rowKey, columnKey);
        for (let i = 0; i < config.measures.length; i++)
          accumulate(defined(aggregates[i]), defined(observations[i]), defined(config.measures[i]).op);
      }
  }
  const cells: PivotResult['cells'] = {};
  for (const [key, aggregates] of states)
    cells[key] = aggregates.map((state, i) => resultValue(state, defined(config.measures[i]).op));
  return {
    rowNodes: sortedNodes(rowNodes),
    columnNodes: sortedNodes(columnNodes),
    cells,
    rowCount: rows.length,
    cellCount: states.size,
    measureCount: config.measures.length,
  };
}
