...
describe('AC-3 reportTitle', () => {
  it('uses the first clause of the description in each language and falls back to the action name', () => {
    const t = reportTitle({
      ...
      description: {
        ja: '試算表を返します。勘定科目ごとに、期首残高…',
        en: 'Trial balance: per account, opening balance before `from`.',
      },
    });
    expect(t).toEqual({ ja: '試算表', en: 'Trial balance' });
    expect(
      reportTitle({ name: 'tax.summary', description: { ja: '消費税集計表を返します。', en: 'Tax summary.' } }),
    ).toEqual({ ja: '消費税集計表', en: 'Tax summary' });
    ...
    expect(reportTitle({ name: 'x.y', description: { ja: 'あ'.repeat(40), en: 'a'.repeat(40) } }).en).toBe(
      `${'a'.repeat(32)}…`,
    );
  });
});
...
describe('report period controls', () => {
  ...
  it('initializes actual date/month/year inputs but never fabricates references', () => {
    expect(reportInitialValues(fields, now)).toEqual({
      from: '2026-09-01',
      to: '2026-09-13',
      asOf: '2026-09-13',
      period: '2026-09',
  ...
  it('resolves inclusive calendar periods across year boundaries, leap years and the Japanese midnight', () => {
    expect(reportPeriodValues(fields, 'previous', new Date('2025-01-10T00:00:00Z'))).toMatchObject({
      from: '2024-12-01',
      to: '2024-12-31',
      period: '2024-12',
      year: 2024,
    });
    expect(reportPeriodValues(fields, 'previous', new Date('2024-03-01T00:00:00Z'))).toMatchObject({
      from: '2024-02-01',
      to: '2024-02-29',
    });
    expect(reportPeriodValues(fields, 'current', new Date('2026-09-30T15:00:00Z'))).toMatchObject({
      from: '2026-10-01',
      to: '2026-10-01',
    });
  ...
  it('preserves explicit schema defaults including null/false, and leaves unrelated fields empty', () => {
    const input = schemaFields(
      {
        type: 'object',
        properties: {
          from: { ...date, default: '2026-01-01' },
          to: date,
    ...
    expect(schemaInitialValues(input, defaults)).toEqual({
      from: '2026-01-01',
      to: '2026-09-13',
      asOf: '',
      includeDraft: false,
    ...
    expect(schemaToPayload(schemaInitialValues(fields, reportInitialValues(fields, now)), fields).payload.year).toBe(
      2026,
    );
  });
  it('preserves specific schema labels and only translates generated labels', () => {
    const input = schemaFields(
      {
        type: 'object',
        properties: { from: { ...date, title: '完了期間の開始日' }, to: date, period: { type: 'string' } },
      },
      () => undefined,
    );
    expect(reportInputFields(input).map((field) => field.label.ja)).toEqual([
      '完了期間の開始日',
      '終了日',
      '対象月（YYYY-MM）',
    ]);
  ...
  it('labels and resolves a year-only period differently from a monthly report or an as-of date', () => {
    ...
    expect(
      reportPeriodLabel(
        fields.filter((field) => field.name === 'asOf'),
        'current',
      ).ja,
    ).toBe('今日');
    expect(
...
describe('returned report rows search, sort and pagination', () => {
  ...
  it('bounds the DOM page and clamps empty, negative, oversized and non-finite page requests', () => {
    ...
    expect(reportRowsPage(result, { query: 'not-found' })).toMatchObject({
      matched: 0,
      page: 0,
      pages: 1,
      from: 0,
  ...
  it('sorts huge and negative decimal values exactly, preserves ties and puts missing cells last in either direction', () => {
    const exact = {
      ...result,
      rows: ['9007199254740993.000001', '9007199254740993.000002', '-0.000002', '-0.000001', '1', '1.000000', null].map(
        (amount, i) => ({ amount, name: String(i) }),
      ),
    };
    expect(reportRowsPage(exact, { sort: { key: 'amount', direction: 'asc' } }).rows.map(({ index }) => index)).toEqual(
      [2, 3, 4, 5, 0, 1, 6],
    );
    expect(
      reportRowsPage(exact, { sort: { key: 'amount', direction: 'desc' } }).rows.map(({ index }) => index),
    ).toEqual([1, 0, 4, 5, 3, 2, 6]);
  });
  it('searches normalized raw and displayed values while preserving server totals, references and source row order', () => {
    ...
    expect(
      reportRowsPage(result, { query: 'JAPAN', cellText: (_column, value) => (value === '拠点0' ? 'Japan' : '') })
        .matched,
    ).toBe(1);
  ...
  it('orders text naturally and handles invalid numeric strings without breaking the table', () => {
    const source = {
      ...result,
      rows: [
        { name: 'item10', amount: 'N/A' },
        { name: 'item2', amount: '2' },
        { name: 'item1', amount: '' },
      ],
    };
    expect(
      reportRowsPage(source, { sort: { key: 'name', direction: 'asc' }, locale: 'en' }).rows.map(({ row }) => row.name),
    ).toEqual(['item1', 'item2', 'item10']);
    expect(reportRowsPage(source, { sort: { key: 'amount', direction: 'desc' } }).rows).toHaveLength(3);
    expect(
      reportRowsPage(source, { sort: { key: 'unknown', direction: 'asc' } }).rows.map(({ index }) => index),
    ).toEqual([0, 1, 2]);
  });
});
