...
const base = { module: 'payment', scope: 'company' as const, displayField: undefined, hasExt: true, ops: ['read' as const, 'create' as const] };
...
const payment: EntityMeta = {
  ...
  kind: 'document',
  label: L('入出金', 'Payment'),
  fields: [field('direction', 'enum', { required: true, values: ['receive', 'pay'] }), field('partnerId', 'ref', { required: true, ref: 'partner' }), field('amount', 'decimal', { required: true, money: true, scale: 0 })],
...
const outstanding: ActionMeta = {
  ...
  mutates: false,
  resultKind: 'table',
  inputSchema: { type: 'object', properties: { direction: { type: 'string' }, partnerId: { type: 'string' } }, required: ['direction'] },
};
...
describe('AC-1 allocation convention (docs/conventions/ui.md)', () => {
  it('payment + payment_allocation + payment.outstanding match', () => {
    expect(allocationConventionOf(payment, entities, [outstanding])).toEqual({ lineEntity: 'payment_allocation', action: 'payment.outstanding', invoiceEntities: ['sales_invoice', 'purchase_invoice'], hasAmount: true });
  });
  ...
  it('no match without the action (or when it is not a table), the header fields, or a *_allocation line with the line fields', () => {
    ...
    expect(allocationConventionOf(payment, entities, [{ ...outstanding, inputSchema: { type: 'object', properties: { foo: {} } } }])).toBeUndefined();
    expect(allocationConventionOf({ ...payment, fields: payment.fields.filter((f) => f.name !== 'direction') }, entities, [outstanding])).toBeUndefined();
    expect(allocationConventionOf({ ...payment, lines: [{ entity: 'payment_line', parentField: 'paymentId' }] }, [payment, { ...allocation, name: 'payment_line' }], [outstanding])).toBeUndefined();
    expect(allocationConventionOf(payment, [payment, { ...allocation, fields: allocation.fields.filter((f) => f.name !== 'invoiceEntity') }], [outstanding])).toBeUndefined();
  ...
  it('the action input comes from the current form values, only once partner and direction are set', () => {
    expect(outstandingInput({ direction: 'receive', partnerId: 'p1', amount: '1' })).toEqual({ partnerId: 'p1', direction: 'receive' });
...
describe('AC-1 outstanding TableResult -> pickable invoices', () => {
  const result: TableResult = {
    ...
    rows: [
      { invoiceId: 'i1', number: 'INV-2026-000001', date: '2026-09-01', dueDate: '2026-10-31', total: '1100', paidAmount: '0', balance: '1100' },
  ...
  it('maps number/date/dueDate/balance and the invoice entity from meta; skips rows without id or balance', () => {
    expect(outstandingInvoices(result)).toEqual([
      { invoiceId: 'i1', invoiceEntity: 'sales_invoice', number: 'INV-2026-000001', date: '2026-09-01', dueDate: '2026-10-31', balance: '1100' },
      { invoiceId: 'i2', invoiceEntity: 'sales_invoice', number: 'INV-2026-000002', date: '2026-09-02', dueDate: '', balance: '550.5' },
    ]);
  });
...
describe('AC-1/AC-2 allocation math (decimal strings, lib/decimal.ts)', () => {
  it('配分合計 / 未配分 and over-allocation', () => {
    expect(allocationSummary('1100', [row('600'), row('500')])).toEqual({ allocated: '1100', unallocated: '0', over: false });
    expect(allocationSummary('1000', [row('600.5'), row('400')])).toEqual({ allocated: '1000.5', unallocated: '-0.5', over: true });
    expect(allocationSummary('0.3', [row('0.1'), row('0.2')])).toEqual({ allocated: '0.3', unallocated: '0', over: false });
    expect(allocationSummary('', [row('10'), row('abc')])).toEqual({ allocated: '10', unallocated: undefined, over: false });
  ...
  it('picks become grid rows (invoiceEntity, invoiceId, amount) that save like typed rows; ids already in the grid are known', () => {
    const invoice = { invoiceId: '01a09002-9cbd-752e-867f-6efc3653e03c', invoiceEntity: 'sales_invoice', number: 'INV-1', date: '', dueDate: '', balance: '1100' };
    ...
    expect(rowsToPayload(rows, columns).rows).toEqual([{ invoiceEntity: 'sales_invoice', invoiceId: invoice.invoiceId, amount: '1100' }]);
  ...
  it('a saved document is over-allocated when Σ lines > amount (the 確定 button)', () => {
    ...
    const rec = (amount: string, amounts: string[]): RecordJson => ({
      ...
      version: 1,
      amount,
      lines: { payment_allocation: amounts.map((a, i) => ({ id: `l${i}`, tenantId: 't', companyId: 'c', createdAt: '', updatedAt: '', createdBy: null, updatedBy: null, version: 1, amount: a })) },
    });
