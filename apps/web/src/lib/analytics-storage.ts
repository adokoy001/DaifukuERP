...
export type AnalysisMutation =
  | { kind: 'save'; entry: SavedAnalysis; baseline?: SavedAnalysis }
  | { kind: 'delete'; id: string; baseline: SavedAnalysis };
export class AnalysisStorageError extends Error {
  constructor(readonly code: 'unsupported' | 'conflict') {
    super(code);
...
function text(value: unknown, max = 150): value is string {
...
export function parseAnalyticsSettings(value: unknown): AnalyticsSettings | undefined {
  ...
  if (
    ...
    !text(value.state) ||
    !value.state ||
    !PERIODS.some((period) => period.value === value.period) ||
    !text(value.from) ||
    !text(value.to) ||
  ...
  if (
    ...
    !['bar', 'line', 'none'].includes(value.chart) ||
    typeof value.subtotals !== 'boolean' ||
    !Number.isInteger(value.chartMeasure) ||
    Number(value.chartMeasure) < 0 ||
    Number(value.chartMeasure) > 2
  )
    return undefined;
  ...
  for (const axes of [pivot.rows, pivot.columns]) {
    if (
      ...
      !axes.every(
        (item: unknown) =>
          object(item) &&
          exact(item, ['field', 'grain']) &&
          text(item.field) &&
          item.field.length > 0 &&
          (item.grain === undefined || (typeof item.grain === 'string' && Object.hasOwn(GRAIN_LABELS, item.grain))),
      )
    )
      return undefined;
  }
  if (
    !Array.isArray(pivot.measures) ||
    !pivot.measures.length ||
    pivot.measures.length > 3 ||
    !pivot.measures.every(
      (item: unknown) =>
        object(item) &&
        exact(item, ['field', 'op']) &&
        text(item.field) &&
        ...
        Object.hasOwn(OP_LABELS, item.op),
    )
  )
    return undefined;
  if (Number(value.chartMeasure) >= pivot.measures.length) return undefined;
  try {
    validatePivotConfig(pivot as unknown as AnalyticsSettings['pivot']);
  } catch {
    return undefined;
  }
  for (const expanded of [value.expandedRows, value.expandedColumns])
    if (!Array.isArray(expanded) || expanded.length > 1000 || !expanded.every((key: unknown) => text(key, 2000)))
      return undefined;
...
export function readAnalyses(storage: AnalysisStorage, key: string): SavedAnalysis[] {
  ...
  if (
    !object(parsed) ||
    !exact(parsed, ['version', 'items']) ||
    parsed.version !== 1 ||
    !Array.isArray(parsed.items) ||
    parsed.items.length > MAX_SAVED
  )
    throw new Error('Unsupported saved analysis settings.');
  const items: SavedAnalysis[] = [];
  for (const item of parsed.items) {
    if (
      !object(item) ||
      !exact(item, ['id', 'name', 'updatedAt', 'settings']) ||
      !text(item.id) ||
      !text(item.name, 80) ||
      !item.name.trim() ||
      !text(item.updatedAt)
    ...
    if (!settings || items.some((entry) => entry.id === item.id))
      throw new Error('Invalid saved analysis configuration.');
 ...
 */
export async function mutateAnalyses(
  storage: AnalysisStorage,
  locks: AnalysisLockManager | undefined,
  key: string,
  change: AnalysisMutation,
  ...
  return locks.request(key, { mode: 'exclusive' }, () => {
    ...
    if (baseline ? baseline.id !== id || !sameSavedAnalysis(current, baseline) : current !== undefined)
      throw new AnalysisStorageError('conflict');
