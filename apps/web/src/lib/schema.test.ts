...
const refOf = (name: string) =>
  name === 'partner' ? { displayField: 'name' } : name === 'fiscal_year' ? { displayField: undefined } : undefined;
...
describe('AC-3/AC-5 schemaFields', () => {
  ...
  it('maps types: date by name/pattern, ref-by-name for known entities, enum, int, number, bool, nullable unwrap, json fallback', () => {
    ...
    expect(byName.get('kind')).toMatchObject({
      kind: 'enum',
      values: ['a', 'b'],
      default: 'a',
      description: 'which',
  ...
  it('toFieldMeta adapts to the entity widgets', () => {
    expect(
      toFieldMeta(
        byName.get('partnerId') ?? fields[0] ?? { name: 'x', kind: 'text', label: { ja: '', en: '' }, required: false },
      ),
    ).toMatchObject({ kind: 'ref', ref: 'partner', refDisplayField: 'name', hasDefault: false });
    expect(
      toFieldMeta(
        byName.get('ratio') ?? fields[0] ?? { name: 'x', kind: 'text', label: { ja: '', en: '' }, required: false },
      ).kind,
    ).toBe('decimal');
  ...
  it('initial values come from defaults, overridden by a stored value; payload omits empties, converts numbers, flags required', () => {
    ...
    expect(schemaToPayload({ ...initial, from: 'x', to: 'y', limit: '1.5' }, fields).errors).toEqual({
...
describe('AC-5 setting issue paths / AC-3 ref resolver', () => {
  ...
  it('refResolverFrom resolves entities visible in /meta with their display field', () => {
    const refOf2 = refResolverFrom([
      { name: 'partner', displayField: 'name' } as never,
      { name: 'account', displayField: undefined } as never,
    ]);
