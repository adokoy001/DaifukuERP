...
export function switchSchemaMode(state: SchemaModeState, fields: readonly SchemaField[]): SchemaModeState {
  if (!state.jsonMode) {
    ...
    try {
      ...
      /* The valid form values replace an invalid old buffer. */
    }
  ...
  try {
    const value: unknown = JSON.parse(state.json);
    if (typeof value !== 'object' || value === null || Array.isArray(value))
      return { ...state, errors: { _json: 'オブジェクト形式の JSON を入力してください / Enter a JSON object.' } };
    return {
      ...state,
      values: schemaInitialValues(fields, value as Record<string, unknown>),
      jsonMode: false,
      errors: {},
    };
  } catch {
