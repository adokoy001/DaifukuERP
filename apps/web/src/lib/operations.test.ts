...
describe('operations dates and exact chart scaling', () => {
  it('uses Tokyo dates across the UTC month boundary', () => {
    ...
    expect(parseOperationsSearch({}, new Date('2026-09-30T15:00:00Z'))).toEqual({
      from: '2026-10-01',
      to: '2026-10-01',
      asOf: '2026-10-01',
    });
  ...
  it('preserves explicit criteria and discards malformed store selectors', () => {
    expect(parseOperationsSearch({ from: '2026-09-01', to: '2026-09-05', asOf: '2026-09-06', storeId: 'bad' })).toEqual(
      { from: '2026-09-01', to: '2026-09-05', asOf: '2026-09-06' },
    );
  });
  it('compares amounts beyond safe Number precision', () => {
    expect(maximumMagnitude(['-9007199254740993.000001', '9007199254740993', '0.0000001'])).toBe(
