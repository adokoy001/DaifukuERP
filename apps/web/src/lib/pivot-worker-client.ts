...
export interface PivotWorkerInput {
  rows: readonly Record<string, unknown>[];
  config: PivotConfig;
}
export type PivotWorkerReply =
  { kind: 'result'; result: PivotResult } | { kind: 'error'; code: PivotErrorCode | 'failed' };
...
interface Callbacks {
  result(value: PivotResult): void;
  failed(code: PivotWorkerFailure): void;
...
export function createPivotRunner(factory: () => PivotWorkerPort | null, callbacks: Callbacks, timeoutMs = 20_000) {
  ...
  const finish = (worker: PivotWorkerPort, outcome: () => void) => {
  ...
  return {
    run(rows: readonly Record<string, unknown>[], config: PivotConfig) {
      ...
      worker.onmessage = ({ data }) =>
        finish(worker, () => {
          if (data.kind === 'result') callbacks.result(data.result);
          else callbacks.failed(data.code);
        });
      ...
      try {
        ...
        finish(worker, () => callbacks.failed('failed'));
      }
    },
    cancel() {
      dispose();
