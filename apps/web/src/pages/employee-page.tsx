...
export function EmployeePage() {
  ...
  return <ReadRecoveryProvider sources={[meta, portal]}><div className="workspace-page workforce-page" data-testid="employee-portal">
    ...
    <WorkforceTabs tabs={tabs.filter((item) => item.id !== 'shifts' || actions.includes('workforce.my_shifts'))} selected={tab} onSelect={setTab} />
    {tab !== 'shifts' ? <div className="workforce-toolbar"><label>{t({ ja: '表示する月', en: 'Month' })}<input className="input" type="month" value={period} onChange={(e) => { if (/^\d{4}-(0[1-9]|1[0-2])$/.test(e.target.value)) setPeriod(e.target.value); }} /></label><button type="button" className="btn" disabled={portal.isFetching} onClick={() => void portal.refetch()}>{t({ ja: '最新の状態を確認', en: 'Refresh' })}</button></div> : null}
