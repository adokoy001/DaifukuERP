...
export function EmployeePage() {
    ...
    return (
      <div className="workspace-page workforce-page">
        <p className="workforce-notice">
          {t({
            ja: 'この会社で従業員ポータルを利用する権限がありません。管理者に会社所属と役割をご確認ください。',
            en: 'Employee portal access is unavailable in this company. Ask your administrator to review your membership and role.',
          })}
        </p>
      </div>
    );
  ...
  const pending =
    data.leaveRequests.filter((r) => r.status === 'pending').length +
    data.expenses.filter((r) => r.status === 'submitted').length +
  ...
  return (
    <ReadRecoveryProvider sources={[meta, portal]}>
      <div className="workspace-page workforce-page" data-testid="employee-portal">
        <ReadRefreshNotice />
        <WorkforceHero
          title={t({ ja: '今日も、おつかれさまです。', en: 'Your workday, made clearer.' })}
          description={t({
            ja: '出退勤も、申請も、明細の確認も。自分の仕事の記録をひとつに。',
            en: 'Clock in, send requests and check payslips. Your work records, together.',
          })}
          {...(data.employee ? { name: data.employee.name, site: data.employee.code } : {})}
          side={
            <>
              <strong>{data.today.replaceAll('-', '.')}</strong>
              <span>{t({ ja: 'あなたのワークスペース', en: 'Your personal workspace' })}</span>
            </>
          }
        />
        {!registered ? (
          <p className="workforce-notice" role="status">
            {t({
              ja: 'この会社の従業員登録が必要です。管理者へ、利用者アカウントと所属拠点の登録をご依頼ください。',
              en: 'An active employee record is required for this company. Ask your administrator to link your account and work site.',
            })}
          </p>
        ) : null}
        <WorkforceTabs
          tabs={tabs.filter(
            (item) =>
              (item.id !== 'shifts' || actions.includes('workforce.my_shifts')) &&
              (item.id !== 'fiscal' || actions.includes('workforce.my_fiscal_portal')),
          )}
          selected={tab}
          onSelect={setTab}
        />
        {!['shifts', 'fiscal'].includes(tab) ? (
        ...
        {tab === 'today' ? (
          <div className="workforce-today">
            <div className="workforce-metrics">
              <WorkforceMetric
                title={t({ ja: '有給の残り', en: 'Available leave' })}
                value={formatDecimal(data.leaveBalance) + t({ ja: ' 日', en: ' days' })}
                note={t({ ja: '有効な付与の残数', en: 'Remaining valid grants' })}
                icon="leaf"
                tone="cyan"
              />
              <WorkforceMetric
                title={t({ ja: '確認待ちの申請', en: 'Requests awaiting review' })}
                value={pending}
                note={t({ ja: '表示中の月の有給・訂正・経費', en: 'Leave, corrections and expenses this month' })}
                icon="clock"
                tone="coral"
              />
              <WorkforceMetric
                title={t({ ja: '確定した給与明細', en: 'Confirmed payslips' })}
                value={data.payrolls.filter((r) => r.status === 'confirmed').length}
                note={t({ ja: '給与タブで明細を確認できます', en: 'Open Payslips to view details' })}
                icon="document"
              />
            </div>
        ...
        <p className="workforce-footer-note">
          {t({
            ja: '給与や申請内容を端末のオフライン用データとして保存しません。打刻と申請には通信が必要です。',
            en: 'Payslips and requests are not stored as offline app data. Clocking and requests need a connection.',
          })}
          {actions.includes('workforce.management_portal') ? (
            <>
              {' '}
              <Link to="/workforce">{t({ ja: '従業員の管理画面へ', en: 'Open workforce management' })}</Link>
            </>
          ) : null}
        </p>
      </div>
