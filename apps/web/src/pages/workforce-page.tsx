...
export function WorkforcePage() {
  ...
  return <ReadRecoveryProvider sources={[meta, portal]}><div className="workspace-page workforce-page" data-testid="workforce-management">
    <ReadRefreshNotice />
    ...
    {actions.includes('workforce.shift_board') ? <Link className="btn btn-primary" to="/workforce/shifts">{t({ ja: 'シフト計画・推薦を開く', en: 'Plan and recommend shifts' })}</Link> : null}
    <div className="button-row">{actions.includes('workforce.fiscal_board') ? <Link className="btn" to="/workforce/payroll">{t({ ja: '給与・税保険・年末調整へ', en: 'Payroll, tax and year-end' })}</Link> : null}{actions.includes('workforce.work_system_board') ? <Link className="btn" to="/workforce/systems">{t({ ja: '勤務制度・所定カレンダーへ', en: 'Working-time systems and calendars' })}</Link> : null}</div>
