  ...
  return (
    <ReadRecoveryProvider sources={[query]}>
      ...
      <button className="btn shift-refresh" disabled={query.isFetching} onClick={() => void query.refetch()}>
        {t({ ja: '最新の資料を確認', en: 'Refresh planning sources' })}
      </button>
      <ShiftPlanner
        key={key}
        scopeKey={key}
...
export function ShiftPage() {
  ...
  const change = (work: () => void) => {
    if (
      ...
      globalThis.confirm(
        t({
          ja: '未保存のシフト案・推薦を破棄して切り替えますか？',
          en: 'Discard the unsaved plan or recommendation and switch?',
        }),
      )
    )
      work();
  };
  if ((meta.isError && !canRetainData(meta)) || (sites.isError && !canRetainData(sites)))
    return (
      <div className="workspace-page">
        <WorkforceError error={meta.error ?? sites.error} />
      </div>
    );
  ...
  if (!allowed)
    return (
      <div className="workspace-page">
        <p className="notice">
          {t({ ja: 'この会社でシフトを管理する権限がありません。', en: 'You cannot manage shifts in this company.' })}
        </p>
      </div>
    );
  const siteId = sites.data?.items.some((row) => row.id === site) ? site : (sites.data?.items[0]?.id ?? '');
  return (
    <ReadRecoveryProvider sources={[meta, sites]}>
      <div className="workspace-page workforce-page shift-page" data-testid="shift-page">
        <ReadRefreshNotice />
        <WorkforceHero
          management
          title={t({ ja: 'みんなの希望を、働きやすい一週間へ。', en: 'Turn team preferences into a workable week.' })}
          description={t({
            ja: '必要人数と勤務条件を確認し、推薦案を整えて社員へ公開します。',
            en: 'Check staffing needs and work profiles, refine a recommendation and publish it to employees.',
          })}
        />
        <Link className="btn" to="/workforce">
          {t({ ja: '社員・勤怠管理へ', en: 'Workforce management' })}
        </Link>
        <div className="workforce-toolbar">
          <label>
            {t({ ja: '計画する拠点', en: 'Planning site' })}
            <select
              className="input"
              disabled={busy}
              value={siteId}
              onChange={(event) => change(() => setSite(event.target.value))}
            >
              {sites.data?.items.map((row) => (
          ...
          <label>
            {t({ ja: '計画する週（月曜）', en: 'Planning week (Monday)' })}
            <input
              className="input"
              type="date"
              disabled={busy}
              value={week}
              onChange={(event) => {
                if (event.target.value) change(() => setWeek(shiftMonday(event.target.value)));
              }}
            />
          </label>
          <span className="shift-muted">Asia/Tokyo · 7{t({ ja: '日間', en: ' days' })}</span>
        </div>
        {siteId ? (
          <Board
            key={`${siteId}:${week}`}
          ...
          <p className="workforce-notice">
            {t({
              ja: '有効な拠点を人事担当者に登録してもらってください。',
              en: 'Ask HR to create an active work site.',
            })}
          </p>
        )}
      </div>
