...
export function useSettings(enabled = true): UseQueryResult<SettingMeta[]> {
  return useQuery({ queryKey: settingsKey, queryFn: () => request<SettingMeta[]>('/meta/settings'), enabled, retry: false });
}

export function useSaveSetting(): UseMutationResult<SettingMeta, Error, { key: string; value: unknown }> {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ key, value }: { key: string; value: unknown }) => request<SettingMeta>(`/meta/settings/${encodeURIComponent(key)}`, { method: 'PUT', body: { value } }),
    onSuccess: (saved) => qc.setQueryData<SettingMeta[]>(settingsKey, (prev) => (prev ?? []).map((s) => (s.key === saved.key ? saved : s))),
  });
}
