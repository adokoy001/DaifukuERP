...
export function useFinanceAccess() {
  return useQuery({
    queryKey: ['finance-access', financeIdentity()],
...
export function useFinanceRead<T>(
  action: string,
  ...
  return useQuery({
    queryKey: ['finance', financeIdentity(), action, input],
    queryFn: async ({ signal }) =>
      parse(await request<unknown>('/actions/' + action, { method: 'POST', body: input, signal, cache: 'no-store' })),
    enabled,
    retry: false,
    staleTime: 0,
    gcTime: 0,
...
export function useFinanceList(entity: string, where: Record<string, unknown>, enabled: boolean, offset = 0) {
  ...
  return useQuery({
    queryKey: ['finance', financeIdentity(), 'list', entity, where, offset],
    queryFn: ({ signal }) => request<ListResponse>(`/api/${entity}?${params}`, { signal, cache: 'no-store' }),
    enabled,
    retry: false,
    staleTime: 0,
    gcTime: 0,
...
export function useFinanceCommand<T = unknown>() {
  ...
  return useMutation({
    ...
    retry: false,
    networkMode: 'always',
    mutationFn: async ({ action, input }: { action: string; input: unknown }) => {
      if (financeIdentity() !== identity)
        throw new Error(
          '会社または利用者が変わりました。開き直してください。 / Reopen this workspace after switching company or user.',
        );
      const result = await request<T>('/actions/' + action, { method: 'POST', body: input, cache: 'no-store' });
      if (financeIdentity() !== identity)
        throw new Error(
          '会社または利用者が変わりました。結果は元の会社で確認してください。 / Check the result in the original company.',
        );
      return result;
    },
    onSuccess: async () => {
    ...
    onError: async (error) => {
      if (isApiError(error) && [401, 403, 404].includes(error.status))
        await Promise.all([
          qc.resetQueries({ queryKey: ['finance'] }),
          qc.resetQueries({ queryKey: ['finance-access'] }),
