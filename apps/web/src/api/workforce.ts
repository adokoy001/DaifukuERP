...
import type { AttendanceSummary, EmployeeSummary, ManagementPortal, MyPortal, PayrollSummary } from '@daifuku/mod-workforce/contract';
...
export function useMyPortal(period: string, enabled: boolean) { return useWorkforceRead<MyPortal>('workforce.my_portal', { period }, enabled); }
export function useManagementPortal(period: string, enabled: boolean) { return useWorkforceRead<ManagementPortal>('workforce.management_portal', { period }, enabled); }
...
export function tokyoTimestamp(value: string): string { return value.length === 16 ? value + ':00+09:00' : value + '+09:00'; }
