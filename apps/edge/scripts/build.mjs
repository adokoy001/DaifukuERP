...
const result = await build({
  entryPoints: { edge: 'src/main.ts', setup: 'setup/main.ts' },
  outdir: 'dist',
  outExtension: { '.js': '.mjs' },
  bundle: true,
  platform: 'node',
...
const notices = [
  'Daifuku LAN agent - bundled third-party notices',
  'Keep this file and LICENSE beside edge.mjs when redistributing the standalone bundle.',
];
for (const [name, { directory, manifest }] of [...packages.entries()].sort(([a], [b]) => a.localeCompare(b, 'en'))) {
  ...
  notices.push(
    '========================================',
    name + ' (' + String(manifest.license ?? 'see included license') + ')',
  );
