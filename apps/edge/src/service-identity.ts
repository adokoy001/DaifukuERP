...
export function unprivilegedServiceIdentity(value: unknown): value is ServiceIdentity {
  ...
  return uid >= 400 && uid < 500 && uid === euid && gid > 0 && gid === egid && groups.includes(gid) && groups.every((group) => !privilegedMacGroups.has(group));
}
export function matchesMacServiceIdentity(platform: string, runtime: { identity?: unknown; unprivilegedIdentityRequired?: unknown; unprivilegedIdentityVerified?: unknown }, account?: { uid?: number; gid?: number; groups?: number[] }): boolean {
  if (platform !== 'darwin') return true;
  return runtime.unprivilegedIdentityRequired === true && runtime.unprivilegedIdentityVerified === true && unprivilegedServiceIdentity(runtime.identity) && runtime.identity.uid === account?.uid && runtime.identity.gid === account?.gid && Array.isArray(account?.groups) && sameMacGroups(runtime.identity.groups, account.groups);
}
