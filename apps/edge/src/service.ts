...
class ServiceStatus {
  ...
  verifyIdentity(value: boolean): Promise<void> {
    this.identityVerified = value;
    return this.refresh();
  }
  set(phase: Phase): Promise<void> {
    this.phase = phase;
    return this.refresh();
  }
  refresh(): Promise<void> {
    this.queue = this.queue.then(() =>
      syncJson(this.path, {
        ...
        unprivilegedIdentityVerified: this.identityVerified,
        unprivilegedIdentityRequired: process.env.DAIFUKU_EDGE_REQUIRE_UNPRIVILEGED_IDENTITY === '1',
        identity: currentServiceIdentity(),
      }),
    );
    return this.queue;
  }
...
async function verifyUnprivilegedIdentity(): Promise<boolean> {
  ...
  if (
    !unprivilegedServiceIdentity(identity) ||
    !sameMacGroups(identity.groups, await macAccountGroups(nativeDirectoryRunner))
  )
    throw new EdgeError('service_identity_not_unprivileged');
  return true;
}
export async function runService(
  ...
  try {
    while (!local.signal.aborted) {
      try {
        await status.verifyIdentity(false);
        await status.verifyIdentity(await verifyUnprivilegedIdentity());
        const phase = await connectService(credentials, directory);
        await status.set(phase);
        log(phase);
        ...
        log(code);
        await status.set(code === 'credential_rejected' ? 'credential_rejected' : 'error');
      }
      if (!local.signal.aborted)
        await delay(SERVICE_RETRY_MS, undefined, { signal: local.signal }).catch(() => undefined);
    }
    if (statusFailed) throw new EdgeError('service_status_write_failed');
  } finally {
    clearInterval(timer);
