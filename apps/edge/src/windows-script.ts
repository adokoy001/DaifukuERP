...
function LocalPath([string]$path) {
  ...
  $part = $full
  while ($part) {
    if (Test-Path -LiteralPath $part) { $item = Get-Item -Force -LiteralPath $part; if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) { throw 'unsafe_state_path' }; if ($part -ne $full) { SafeParentAcl (Get-Acl -LiteralPath $part) } }
...
Reply $null
`,
  remove: String.raw`$full = PrivateFile $edgeInput.path; [IO.File]::Delete($full); Reply $null`,
  lock: String.raw`
$full = LocalPath $edgeInput.path
[void](PrivateDirectory ([IO.Path]::GetDirectoryName($full)))
if (Test-Path -LiteralPath $full) { [void](PrivateFile $full) }
...
` + actions[operation] + String.raw`
  ...
  if ($code -notmatch '^(ENOENT|private_file_permissions_required|local_state_path_required|unsafe_state_path|private_parent_required|unsafe_state_directory|invalid_state_file|journal_capacity_exceeded|private_atomic_replace_failed|unsafe_parent_permissions|private_file_exists)$') { $code = 'windows_private_io_failed' }
