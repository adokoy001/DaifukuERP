...
export function numericMacGroups(text: string): number[] {
  ...
  if (groups.some((group) => !Number.isSafeInteger(group) || group > 4294967295) || groups.length > 1024)
    throw new EdgeError('mac_account_group_list_invalid');
  return groups;
}
export function sameMacGroups(left: number[], right: number[]): boolean {
  return (
    JSON.stringify([...new Set(left)].sort((a, b) => a - b)) ===
...
export async function macAccountGroups(run: DirectoryRunner): Promise<number[]> {
  ...
  const uuid = ids[0]?.[1];
  if (ids.length !== 1 || !uuid || !/^[a-fA-F0-9]{8}(?:-[a-fA-F0-9]{4}){3}-[a-fA-F0-9]{12}$/.test(uuid))
    throw new EdgeError('mac_account_guid_invalid');
  for (const [attribute, value] of [
    ['GroupMembership', '_daifukuedge'],
    ['GroupMembers', uuid],
    ['NestedGroups', uuid],
  ]) {
    ...
    if ((await output(run, '/usr/bin/dscl', ['/Search', '-search', '/Groups', attribute, value])).trim())
      throw new EdgeError('mac_account_explicit_group_membership');
  }
  const groups = numericMacGroups(await output(run, '/usr/bin/id', ['-G', '_daifukuedge']));
  if (groups.some((group) => privilegedMacGroups.has(group)))
    throw new EdgeError('mac_account_privileged_group_membership');
  return groups;
}
export const nativeDirectoryRunner: DirectoryRunner = async (file, args) => {
  try {
    const result = await execute(file, args, { timeout: 10000, maxBuffer: 1024 * 1024 });
    return { code: 0, ...result };
  } catch {
    throw new EdgeError('mac_account_membership_lookup_failed');
  }
};
