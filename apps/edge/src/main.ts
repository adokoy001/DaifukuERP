...
async function main(): Promise<void> {
  ...
  if (args.values.help || !command) {
    process.stdout.write(
      'Daifuku edge 0.0.1 (Windows / Linux / macOS / Node 22+)\nCommands: pair, session, rotate, run, service, once, inspect\nOptions: --config <private JSON> --state <private directory> [--token-file <private pairing JSON>]\n',
    );
    return;
  }
  if (
    !['pair', 'session', 'rotate', 'run', 'service', 'once', 'inspect'].includes(command) ||
  ...
  try {
    ...
    if (command === 'pair') {
      ...
      const parsed = z
        .object({ pairingToken: edgeSecret })
        .strict()
        .safeParse(await readPrivateJson(resolve(args.values['token-file']), 2048));
      if (!parsed.success) throw new EdgeError('invalid_pairing_file');
      const session = await credentials.pair(parsed.data.pairingToken);
      process.stdout.write(JSON.stringify(session) + '\n');
    } else if (command === 'session' || command === 'rotate')
      process.stdout.write(
        JSON.stringify(await (command === 'rotate' ? credentials.rotate() : credentials.session())) + '\n',
      );
    else if (command === 'inspect')
      process.stdout.write(
        JSON.stringify(
          journal.records().map((row) => ({
            jobId: row.jobId,
            attempt: row.attempt,
            phase: row.phase,
            code: row.result?.code,
            deviceJobId: row.deviceJobId,
    ...
    else if (command === 'service') await runService(credentials, journal, state, abort.signal, log);
    else {
      const agent = new EdgeAgent(credentials, journal, log);
      if (command === 'once') {
        await credentials.session();
        await agent.tick(abort.signal);
      } else await agent.run(abort.signal);
    }
  } finally {
