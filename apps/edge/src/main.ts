...
async function main(): Promise<void> {
  ...
  if (args.values.help || !command) { process.stdout.write('Daifuku edge 0.0.1 (Windows / Linux / macOS / Node 22+)\nCommands: pair, session, rotate, run, service, once, inspect\nOptions: --config <private JSON> --state <private directory> [--token-file <private pairing JSON>]\n'); return; }
  if (!['pair', 'session', 'rotate', 'run', 'service', 'once', 'inspect'].includes(command) || args.positionals.length !== 1 || !args.values.config || !args.values.state) throw new EdgeError('invalid_cli_arguments');
