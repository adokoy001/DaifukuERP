...
export async function privateDirectory(path: string): Promise<void> {
  if (process.platform === 'win32') { await windowsIo('directory', { path }); return; }
  await mkdir(path, { recursive: true, mode: 0o700 });
  const stat = await lstat(path); if (!stat.isDirectory() || stat.isSymbolicLink()) throw new EdgeError('unsafe_state_directory'); privateMode(stat.mode, stat.uid); await assertNoMacAcl(path);
}
export async function readPrivateJson(path: string, maxBytes = 2000000): Promise<unknown> {
  if (process.platform === 'win32') { const text = await windowsIo('read', { path, maxBytes }); if (typeof text !== 'string') throw new EdgeError('invalid_state_file'); return JSON.parse(text) as unknown; }
  const handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW);
  try { const stat = await handle.stat(); privateMode(stat.mode, stat.uid); if (!stat.isFile() || stat.size > maxBytes) throw new EdgeError('invalid_state_file'); await assertNoMacAcl(path); return JSON.parse(await handle.readFile('utf8')) as unknown; }
...
export async function syncJson(path: string, value: unknown, maxBytes = 2000000): Promise<void> {
  ...
  if (process.platform === 'win32') { await windowsIo('write', { path, bytes: Buffer.from(data).toString('base64'), maxBytes }); return; }
...
async function checkPrivateFile(path: string): Promise<void> {
  await privateDirectory(dirname(path));
  const handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW);
  try { const stat = await handle.stat(); privateMode(stat.mode, stat.uid); if (!stat.isFile()) throw new EdgeError('invalid_state_file'); await assertNoMacAcl(path); }
  finally { await handle.close(); }
}
...
export async function movePrivateFile(source: string, destination: string): Promise<void> {
  ...
  if (process.platform === 'win32') { await windowsIo('move', { path: source, destination }); return; }
  await checkPrivateFile(source);
  try { await lstat(destination); throw new EdgeError('private_file_exists'); } catch (error) { if (!missingFile(error)) throw error; }
  await rename(source, destination); await syncDirectory(dirname(source));
}
export async function removePrivateFile(path: string): Promise<void> {
  if (process.platform === 'win32') { await windowsIo('remove', { path }); return; }
  await checkPrivateFile(path); await unlink(path); await syncDirectory(dirname(path));
}
