...
export const macosLockScript = 'zmodload zsh/system || exit 69; zsystem supports flock || exit 69; zsystem flock -t 0 -f edge_lock_fd "$1"; edge_lock_status=$?; if (( edge_lock_status == 2 )); then exit 73; elif (( edge_lock_status != 0 )); then exit 74; fi; print -r -- locked; IFS= read -r edge_input; exit 0';
