...
export async function assertNoMacAcl(path: string): Promise<void> {
  ...
  try {
    text = (
      await promisify(execFile)('/bin/ls', ['-lde', path], { encoding: 'utf8', timeout: 10000, maxBuffer: 65536 })
    ).stdout;
  } catch {
    throw new EdgeError('macos_acl_check_failed');
  }
 ...
 */
export const macosLockScript =
  'zmodload zsh/system || exit 69; zsystem supports flock || exit 69; zsystem flock -t 0.01 -f edge_lock_fd "$1"; edge_lock_status=$?; if (( edge_lock_status == 2 )); then exit 73; elif (( edge_lock_status != 0 )); then exit 74; fi; print -r -- locked; IFS= read -r edge_input; exit 0';
