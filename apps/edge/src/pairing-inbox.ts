...
async function exists(path: string): Promise<boolean> { try { await lstat(path); return true; } catch (error) { if (missingFile(error)) return false; throw error; } }
export async function connectService(credentials: Credentials, directory: string): Promise<'running' | 'pairing_required' | 'credential_rejected'> {
  const inbox = join(directory, 'pairing.json'), processing = join(directory, 'pairing.processing.json');
  ...
  if (await exists(inbox)) {
    if (await exists(processing)) {
      if ((await readdir(directory)).filter((name) => archivePattern.test(name)).length >= 10) throw new EdgeError('pairing_archive_full');
  ...
  let value: unknown; try { value = await readPrivateJson(processing, 2048); } catch (error) { if (error instanceof SyntaxError) throw new EdgeError('invalid_pairing_file'); throw error; }
  const input = pairing.safeParse(value); if (!input.success) throw new EdgeError('invalid_pairing_file');
  await credentials.pair(input.data.pairingToken); await credentials.session(); await removePrivateFile(processing); return 'running';
}
