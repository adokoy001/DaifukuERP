...
export interface IppAttribute {
  name: string;
  tag: number;
  values: readonly IppValue[];
}
export interface IppMessage {
  code: number;
  requestId: number;
  attributes: Map<string, IppValue[]>;
}
...
function encoded(value: IppValue): Buffer {
  ...
  if (typeof value === 'number') result.writeInt32BE(value);
  else {
    result.writeInt32BE(value[0]);
    result.writeInt32BE(value[1], 4);
  }
  return result;
}
export function ippRequest(
  operation: number,
  uri: string,
  ...
  group(1, [
    { name: 'attributes-charset', tag: 0x47, values: ['utf-8'] },
    { name: 'attributes-natural-language', tag: 0x48, values: ['en'] },
    { name: 'printer-uri', tag: 0x45, values: [uri] },
    ...attributes,
  ]);
  group(2, jobAttributes);
  chunks.push(Buffer.from([3]));
  if (document) chunks.push(document);
  return { body: Buffer.concat(chunks), requestId };
}
function value(tag: number, data: Buffer): IppValue {
  if ([0x21, 0x23].includes(tag)) {
    if (data.length !== 4) throw new EdgeError('invalid_ipp_integer');
    return data.readInt32BE();
  }
  if (tag === 0x22) {
    if (data.length !== 1 || (data[0] !== 0 && data[0] !== 1)) throw new EdgeError('invalid_ipp_boolean');
    return data[0] === 1;
  }
  if (tag === 0x33) {
    if (data.length !== 8) throw new EdgeError('invalid_ipp_range');
    return [data.readInt32BE(), data.readInt32BE(4)];
  }
...
export function parseIpp(body: Buffer, requestId?: number): IppMessage {
  if (body.length < 9 || body.length > 262144 || ![1, 2].includes(body[0] ?? 0))
    throw new EdgeError('invalid_ipp_response');
  ...
  while (position < body.length) {
    ...
    if (tag !== undefined && tag < 0x10) {
      if (![1, 2, 4, 5, 6, 7, 9].includes(tag)) throw new EdgeError('invalid_ipp_group');
      group = tag;
      name = '';
      continue;
    }
    if (!group || tag === undefined || position + 2 > body.length || ++count > 2048)
      throw new EdgeError('invalid_ipp_response');
    const length = body.readUInt16BE(position);
    position += 2;
    if (position + length + 2 > body.length) throw new EdgeError('truncated_ipp_response');
    if (length) {
      name = body.subarray(position, position + length).toString('ascii');
      if (!/^[a-z0-9-]{1,255}$/.test(name)) throw new EdgeError('invalid_ipp_attribute');
    } else if (!name) throw new EdgeError('invalid_ipp_continuation');
    position += length;
    const valueLength = body.readUInt16BE(position);
    position += 2;
    if (position + valueLength > body.length) throw new EdgeError('truncated_ipp_response');
    const decoded = value(tag, body.subarray(position, position + valueLength));
