...
export function localDevice(
  config: EdgeConfig,
  job: Pick<EdgeClaimedJob, 'deviceId' | 'localDeviceId' | 'driver'>,
): DeviceConfig {
  const device = config.devices.find(
    (row) => row.deviceId === job.deviceId && row.localDeviceId === job.localDeviceId && row.driver === job.driver,
  );
  if (!device) throw new EdgeError('device_not_allowlisted');
  return device;
}
export async function executeDevice(
  ...
  if (device.driver === 'simulator') {
    ...
    return {
      state: 'succeeded',
      code:
        job.request.kind === 'cash.dispense'
          ? 'simulated_cash_dispense'
          : job.request.kind === 'print.text'
            ? 'simulated_print'
            : 'simulator_online',
      summary: 'SIMULATION ONLY: no physical device operation.',
    };
  }
  ...
  if (job.request.kind === 'device.status') {
    try {
      return await printerStatus(transport);
    } catch {
      return { state: 'failed', code: 'ipp_status_unavailable' };
    }
  }
...
export async function recoverDevice(config: EdgeConfig, row: JournalRecord, signal: AbortSignal): Promise<EdgeResult> {
  if (row.localConfigHash !== deviceBindingHash(config, row.deviceId))
    return { state: 'uncertain', code: 'local_device_mapping_changed' };
  const device = localDevice(config, row);
  if (device.driver !== 'ipp_text' || !row.deviceJobId)
    return { state: 'uncertain', code: 'agent_restarted_after_start' };
