    ...
    it('systemd accepts the rendered unit for ' + name + ' without registering or starting a service', async () => {
      ...
      try {
        ...
        const context: ServiceContext = {
          ...
          caPath: join(statePath, 'ca-api.pem'),
        };
        ...
        await expect(
          execute('/usr/bin/systemd-analyze', ['verify', path], {
            timeout: 15000,
            env: { PATH: '/usr/bin:/bin', LANG: 'C' },
          }),
        ).rejects.toMatchObject({ code: 1, stderr: expect.stringContaining('WorkingDirectory= path is not absolute') });
      } finally {
