...
const context: ServiceContext = {
  platform: 'win32',
  installationId: 'ae79484c-7253-449b-970a-7715043529e8',
  installRoot: 'C:\\Program Files\\DaifukuEdge',
  statePath: 'C:\\ProgramData\\DaifukuEdge',
  ...
  configPath: 'C:\\ProgramData\\DaifukuEdge\\config.json',
  logPath: 'C:\\ProgramData\\DaifukuEdge\\logs',
...
describe('Windows service installer boundary', () => {
  it('runs a real SCM wrapper as LocalService with only fixed agent arguments', () => {
    ...
    expect(xml).toContain(
      '&quot;C:\\Program Files\\DaifukuEdge\\releases\\release-1\\app\\edge.mjs&quot; service --config',
    );
  ...
  it('escapes XML paths, preserving Unicode and literal shell metacharacters', () => {
    const value = {
      ...context,
      statePath: 'C:\\ProgramData\\お店 & $cash',
      configPath: 'C:\\ProgramData\\お店 & $cash\\config.json',
      logPath: 'C:\\ProgramData\\お店 & $cash\\logs',
      caPath: 'C:\\ProgramData\\お店 & $cash\\ca.pem',
    };
    expect(windowsServiceXml(value)).toContain('お店 &amp; $cash');
    expect(windowsServiceXml(value)).toContain(
      'name="NODE_EXTRA_CA_CERTS" value="C:\\ProgramData\\お店 &amp; $cash\\ca.pem"',
    );
  });
  it.each([
    '\\\\server\\share',
    ...
    'C:\\edge:file',
    'C:\\edge\\file?',
  ])('rejects unsafe Windows path %j before any process is launched', (path) => {
  ...
  it('rejects runtime/state overlap, release escape, and a private config outside state', () => {
    for (const changed of [
      { statePath: context.installRoot },
      { releaseDir: 'C:\\temp\\other' },
      { nodePath: 'C:\\Windows\\cmd.exe' },
      { configPath: 'C:\\Users\\Public\\config.json' },
      { caPath: 'C:\\Users\\Public\\ca.pem' },
    ]) {
  ...
  it('inspection delegates only a read-only command and preserves ownership conflicts', async () => {
    const calls: string[] = [];
    const run: WindowsRunner = async (operation) => {
      calls.push(operation);
      return {
        serviceExists: true,
        serviceRunning: true,
        serviceOwned: false,
        conflicts: ['windows_service_not_owned'],
      };
    };
  ...
  it('rejects malformed status and validates context before mutation', async () => {
    ...
    await expect(adapter.uninstall({ ...context, servicePath: 'C:\\another-service' })).rejects.toThrow(
      'windows_service_path_mismatch',
    );
  ...
  it('only exposes a valid observed agent PID, never a coerced service PID', async () => {
    ...
    for (const processId of ['314', 0, -1, 2.5])
      await expect(createWindowsAdapter(async () => ({ ...result, processId })).inspect(context)).rejects.toThrow(
        'windows_service_output_invalid',
      );
  });
  it('keeps every fixed helper below the Windows process command-line limit', () => {
    for (const operation of [
      ...
      expect(encodedWindowsSetup(operation).length).toBeLessThan(30000);
  });
});
