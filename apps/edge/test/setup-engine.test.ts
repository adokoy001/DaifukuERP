...
async function install(value: Awaited<ReturnType<typeof setupFixture>>, execute = true) {
  ...
  const request = { operation: 'install' as const, bundle, installRoot: value.fixture.installRoot, statePath: value.fixture.statePath, configSource: value.fixture.configSource, execute, resume: false };
  const result = await deploy(request, value.adapter); return { request, result };
}

it('a valid dry-run inspects protected target ancestors but makes no files, account or service changes', async () => {
  const f = await fixture(), bundle = await f.bundle(), before = await f.snapshot();
  const request = { operation: 'install' as const, bundle, installRoot: f.fixture.installRoot, statePath: f.fixture.statePath, configSource: f.fixture.configSource, execute: false, resume: false };
  expect(await deploy(request, f.adapter)).toMatchObject({ execute: false, retainsCredentialsAndJournal: true });
  expect(await f.snapshot()).toEqual(before); expect(f.fixture.events).toEqual([]);
...
it('real administrative path policy and real link checks are not disabled by production code', async () => {
  const f = await fixture(), actual = await vi.importActual<typeof SetupIo>('../setup/io.ts');
  if (process.platform !== 'win32') await expect(actual.pathChain(f.fixture.root, true)).rejects.toThrow(/administrator_owned/);
...
it('rejects manifest hash tampering, additional files and source hardlinks before a deployment is created', async () => {
  const f = await fixture(), bundle = await f.bundle();
  await expect(verifyBundle(bundle.directory, '0'.repeat(64))).rejects.toThrow(/manifest_hash_mismatch/);
  const extra = join(bundle.directory, 'unexpected'); await writeFile(extra, 'extra');
  await expect(verifyBundle(bundle.directory, bundle.manifestHash)).rejects.toThrow(/unexpected_bundle_file/); await unlink(extra);
  await link(join(bundle.directory, 'LICENSE'), join(f.fixture.root, 'hardlink'));
  await expect(verifyBundle(bundle.directory, bundle.manifestHash)).rejects.toThrow(/unsafe_path_link|unsafe_regular_file/);
...
it('rejects a source directory symlink even when the caller supplies the correct manifest digest', async () => {
  const f = await fixture(), bundle = await f.bundle(), alias = join(f.fixture.root, 'bundle-alias');
...
it('refuses mixed state/install paths and unrelated contents instead of adopting or clearing them', async () => {
  const f = await fixture(), bundle = await f.bundle();
  const base = { operation: 'install' as const, bundle, configSource: f.fixture.configSource, execute: true, resume: false };
  await expect(deploy({ ...base, installRoot: f.fixture.installRoot, statePath: join(f.fixture.installRoot, 'data') }, f.adapter)).rejects.toThrow(/must_be_separate/);
  await expect(deploy({ ...base, installRoot: f.fixture.source, statePath: f.fixture.statePath }, f.adapter)).rejects.toThrow(/unmanaged_directory_not_empty/);
  expect(f.fixture.events).toEqual([]);
...
describe.each(['prepare', 'protect', 'register', 'start'])('explicit resume after %s failure', (step) => {
  it('retains intent and completes only with the same operation, artifact and original configuration', async () => {
    const f = await fixture(), bundle = await f.bundle();
    const request = { operation: 'install' as const, bundle, installRoot: f.fixture.installRoot, statePath: f.fixture.statePath, configSource: f.fixture.configSource, execute: true, resume: false };
    f.fixture.fail = step;
    ...
    expect(pending.pending.operation).toBe('install'); expect(pending.pending.target.manifestHash).toBe(bundle.manifestHash);
    await expect(deploy(request, f.adapter)).rejects.toThrow(/matching_explicit_resume/);
    await expect(deploy({ ...request, operation: 'update', resume: true }, f.adapter)).rejects.toThrow(/matching_explicit_resume/);
    await writeFile(f.fixture.configSource, JSON.stringify({ ...f.fixture.config, apiBaseUrl: 'https://different.example.test/api' }));
    ...
    expect(installed.installationId).toBe(pending.installationId); expect(installed.pending).toBeUndefined(); expect(installed.status).toBe('installed');
  });
});
it('resumes a service registration that succeeded before its command reported failure', async () => {
  const f = await fixture(), bundle = await f.bundle();
  const request = { operation: 'install' as const, bundle, installRoot: f.fixture.installRoot, statePath: f.fixture.statePath, configSource: f.fixture.configSource, execute: true, resume: false };
  f.fixture.fail = 'register'; f.fixture.failAfter = true;
  await expect(deploy(request, f.adapter)).rejects.toThrow(/injected_register/);
  expect(f.fixture.registered).toBeDefined(); expect(f.fixture.running).toBe(false);
  expect(await deploy({ ...request, resume: true }, f.adapter)).toMatchObject({ status: 'installed' });
});
it('updates and uninstalls through the owned service while retaining old/new code, credentials, journal and configuration', async () => {
  ...
  const old = f.fixture.registered; if (!old) throw new Error('Expected installed fixture');
  const credentials = join(f.fixture.statePath, 'credentials.json'), journal = join(f.fixture.statePath, 'journal.json');
  await writeFile(credentials, 'synthetic private credentials'); await writeFile(journal, 'unresolved device work');
  const config = await readFile(old.configPath);
  const bundle = await f.bundle('v2');
  const update = { operation: 'update' as const, bundle, installRoot: f.fixture.installRoot, statePath: f.fixture.statePath, execute: true, resume: false };
  await expect(deploy({ ...update, configSource: f.fixture.configSource }, f.adapter)).rejects.toThrow(/preserves_existing_config/);
  const before = f.fixture.events.length;
  expect(await deploy(update, f.adapter)).toMatchObject({ status: 'installed' });
  const updated = f.fixture.registered; if (!updated) throw new Error('Expected updated fixture');
  const events = f.fixture.events.slice(before); expect(events.indexOf('stop')).toBeLessThan(events.indexOf('register'));
  expect(await readFile(updated.configPath)).toEqual(config);
  const existing = await f.snapshot(); const count = f.fixture.events.length;
  await operate({ operation: 'uninstall', installRoot: f.fixture.installRoot, execute: false }, f.adapter);
  expect(await f.snapshot()).toEqual(existing); expect(f.fixture.events.length).toBe(count);
  await operate({ operation: 'uninstall', installRoot: f.fixture.installRoot, execute: true }, f.adapter);
  expect(f.fixture.registered).toBeUndefined(); expect(f.fixture.running).toBe(false);
  expect(await readFile(credentials, 'utf8')).toBe('synthetic private credentials'); expect(await readFile(journal, 'utf8')).toBe('unresolved device work');
  expect(await readFile(updated.configPath)).toEqual(config); expect(await lstat(old.nodePath)).toBeDefined(); expect(await lstat(updated.nodePath)).toBeDefined();
});
it('a changed source after verification does not register or start partially copied code', async () => {
  ...
  await expect(deploy({ operation: 'install', bundle, installRoot: f.fixture.installRoot, statePath: f.fixture.statePath, configSource: f.fixture.configSource, execute: true, resume: false }, f.adapter)).rejects.toThrow(/changed_during_copy|unsafe_regular_file/);
  expect(f.fixture.events).not.toContain('register'); expect(f.fixture.events).not.toContain('start');
  const marker = JSON.parse(await readFile(join(f.fixture.installRoot, 'installation.json'), 'utf8'));
  expect(marker.pending).toBeDefined(); expect(marker.active).toBeUndefined();
});
