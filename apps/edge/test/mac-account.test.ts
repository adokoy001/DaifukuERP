...
import {
  ...
  inspectMacAccount,
  prepareMacAccount,
  validateMacAccountAttributes,
} from '../setup/posix/account-mac.ts';
...
describe('mac directory-service attribute validation', () => {
  it('parses native IsHidden and multiline standard names without treating other native values as ownership', () => {
    const parsed = dsRecord(
      'dsAttrTypeNative:IsHidden: 1\ndsAttrTypeNative:RealName: foreign\nRealName:\n Daifuku edge installer synthetic\nPassword: *\nAuthenticationAuthority: ;DisabledUser;\n',
    );
    expect(parsed.IsHidden).toBe('1');
    expect(parsed.RealName).toBe('Daifuku edge installer synthetic');
  ...
  it.each([
    ['UniqueID', '0', 'uid_invalid'],
    ['UniqueID', '501', 'uid_invalid'],
    ['PrimaryGroupID', '0', 'primary_group_changed'],
    ['UserShell', '/bin/zsh', 'shell_changed'],
    ['NFSHomeDirectory', '/Users/other', 'home_changed'],
    ['Password', 'synthetic-secret', 'password_changed'],
    ['Password', '********', 'password_changed'],
    ['IsHidden', '0', 'hidden_changed'],
    ['AuthenticationAuthority', ';ShadowHash;', 'authentication_changed'],
  ...
  it('creates and verifies its tagged account with the native hidden field serialization', async () => {
    ...
    f.groupId = 4294967294;
    expect(await prepareMacAccount(f, f.context)).toEqual({
      name: '_daifukuedge',
      group: 'nobody',
      uid: 401,
      gid: 4294967294,
      groups: [12, 61, 4294967294],
    });
    expect(await inspectMacAccount(f, f.context)).toMatchObject({ uid: 401 });
    const before = [...f.changes];
    const record = f.user;
    if (!record) throw new Error('Missing synthetic account');
    record.RealName = 'foreign';
    await expect(prepareMacAccount(f, f.context)).rejects.toThrow('not owned');
    expect(f.changes).toEqual(before);
    record.RealName = ownershipTag(f.context);
    record.UserShell = '/bin/zsh';
    await expect(prepareMacAccount(f, f.context)).rejects.toThrow('mac_account_shell_changed');
    expect(f.changes).toEqual(before);
  });
});
...
it('verifies the owned non-login account without mistaking directory nested groups for process credentials', async () => {
  ...
  const run = f.run;
  vi.spyOn(f, 'run').mockImplementation((executable, args) =>
    executable === '/usr/bin/id' && args[0] === '-G'
      ? Promise.resolve({ code: 0, stdout: '4294967294 12 61 701 100', stderr: '' })
      : run(executable, args),
  );
