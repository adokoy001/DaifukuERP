...
export async function linuxPathDiagnostics(source, readStat = lstat) {
  ...
  if (source)
    targets.push(
      { label: 'source-directory', path: source.directory },
      { label: 'source-node', path: source.node },
      { label: 'source-setup', path: source.setup },
    );
  return Promise.all(
    targets.map(async ({ label, path }) => {
      try {
        return { label, ...detail(await readStat(path)) };
      } catch (error) {
        return { label, code: error.code === 'ENOENT' ? 'missing' : 'stat_unavailable' };
      }
    }),
  );
}
export async function normalizeLinuxFixture(guard, io = { lstat, chmod }) {
  if (guard.platform !== 'linux' || guard.githubActions !== 'true' || guard.explicit !== true || guard.uid !== 0)
    throw new Error('disposable_linux_root_fixture_required');
  const before = await io.lstat('/opt');
  if (!before.isDirectory() || before.isSymbolicLink() || before.uid !== 0)
    throw new Error('unexpected_runner_opt_owner_or_type');
  ...
  if (
    ...
    throw new Error('runner_opt_normalization_not_confirmed');
