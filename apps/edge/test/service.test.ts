...
describe('service pairing inbox and durable recovery', () => {
  it('waits without credentials and exposes a private, secret-free service status', async () => {
    ...
    try { expect(await readPrivateJson(join(relay.directory, 'service-status.json'))).toMatchObject({ phase: 'stopped' }); }
  ...
  it('recovers a lost pairing reply after restart before removing the consumed code', async () => {
    ...
    try {
      const credentials = await Credentials.open(relay.directory, relay.config());
      await syncJson(join(relay.directory, 'pairing.json'), { pairingToken: relay.state.pairing }); relay.state.pairLost = true;
      await expect(connectService(credentials, relay.directory)).rejects.toThrow('transport_failed');
      expect(await readdir(relay.directory)).toContain('pairing.processing.json'); expect(relay.state.pairing).toBe('');
      const restarted = await Credentials.open(relay.directory, relay.config()), pair = vi.spyOn(restarted, 'pair');
      expect(await connectService(restarted, relay.directory)).toBe('running'); expect(pair).not.toHaveBeenCalled();
  ...
  it('preserves a rejected code and archives it only when a replacement arrives', async () => {
    const relay = await relayFixture();
    try {
      const credentials = await Credentials.open(relay.directory, relay.config()), invalid = { pairingToken: secret() };
      ...
      expect(names).toHaveLength(1); const name = names[0]; if (!name) throw new Error('Missing synthetic archive');
  ...
  it('retains both private inboxes when the archive limit is reached', async () => {
    ...
    try {
      const credentials = await Credentials.open(relay.directory, relay.config());
      const old = { pairingToken: secret() }, next = { pairingToken: relay.state.pairing };
      await syncJson(join(relay.directory, 'pairing.processing.json'), old); await syncJson(join(relay.directory, 'pairing.json'), next);
      for (let i = 0; i < 10; i++) await syncJson(join(relay.directory, 'pairing-rejected-' + randomUUID() + '.json'), old);
      ...
      expect(await readPrivateJson(join(relay.directory, 'pairing.json'))).toEqual(next); expect(relay.state.credential).toBe('');
    } finally { await relay.close(); }
  });
  it('stops on rejected credentials and re-pairs only through a new private code', async () => {
    const relay = await relayFixture();
    try {
      const credentials = await Credentials.open(relay.directory, relay.config()); await credentials.pair(relay.state.pairing);
      relay.state.credential = secret(); relay.state.pairing = secret();
      expect(await connectService(credentials, relay.directory)).toBe('credential_rejected'); expect(relay.state.claims).toBe(0);
      await syncJson(join(relay.directory, 'pairing.json'), { pairingToken: relay.state.pairing });
      expect(await connectService(credentials, relay.directory)).toBe('running'); expect((await credentials.session()).gatewayId).toBe(relay.session().gatewayId);
    } finally { await relay.close(); }
  });
  it('leaves a malformed processing file private and does not submit it', async () => {
    ...
    try {
      ...
      expect(await readPrivateJson(join(relay.directory, 'pairing.processing.json'))).toEqual({ pairingToken: 'invalid' }); expect(relay.state.credential).toBe('');
    } finally { await relay.close(); }
  });
});
