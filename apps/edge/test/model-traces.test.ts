...
function checkpoint(traceId: string, event: string): Step {
  ...
  assert(step, `Missing model checkpoint ${traceId}: ${event}`);
  return step;
}
...
async function prepare(relay: Awaited<ReturnType<typeof relayFixture>>, printerUri: string) {
  const value = job({ kind: 'print.text', payload: { title: 'Synthetic model', text: 'Model trace only', copies: 1 } });
  value.driver = 'ipp_text';
  const config = relay.config();
  config.devices = [{ deviceId: value.deviceId, localDeviceId: value.localDeviceId, driver: 'ipp_text', printerUri }];
...
describe('AC-8: model journal/device projections against real TLS, disk and IPP fixtures', () => {
  it('EDGE-TRACE-LOST-START: durable intent precedes HTTP; lost grant never reaches the printer', async () => {
    ...
    try {
      ...
      vi.spyOn(prepared.credentials.client, 'start').mockImplementation(async (lease) => {
        await matches(relay.directory, printer.state.printCalls, checkpoint(trace, 'PersistStarting:1'));
        checkedIntent = true;
  ...
  it('EDGE-TRACE-PHYSICAL-UNKNOWN: lost printer and completion replies survive restart without resend', async () => {
    ...
    try {
      ...
      await expect(new EdgeAgent(prepared.credentials, prepared.journal).tick(signal())).rejects.toThrow(
        'transport_failed',
      );
      expect(sendCheckpoint).toBeDefined();
      await sendCheckpoint;
      await matches(relay.directory, printer.state.printCalls, checkpoint(trace, 'DropComplete:1'));
      const first = relay.state.completeCalls[0];
      assert(first);
      ...
      await matches(relay.directory, printer.state.printCalls, checkpoint(trace, 'DeliverComplete:1'));
      expect(relay.state.completeCalls).toHaveLength(2);
      expect(relay.state.completeCalls[1]).toEqual(first);
      expect(relay.state.startCalls).toBe(1);
  ...
  it('EDGE-TRACE-OBSOLETE: old journal is acknowledged only by an explicit fencing disposition', async () => {
    ...
    try {
      ...
      await expect(
        new EdgeAgent(prepared.credentials, await Journal.open(relay.directory)).tick(signal()),
      ).rejects.toThrow('job_result_not_accepted');
      ...
      expect(relay.state.completeCalls).toHaveLength(2);
      expect(relay.state.completeCalls[1]).toEqual(relay.state.completeCalls[0]);
      expect(relay.state.eventCalls).toHaveLength(0);
    } finally {
      await printer.close();
      await relay.close();
    }
