...
async function main(): Promise<void> {
  ...
  const script =
    "$ErrorActionPreference='Stop';$ProgressPreference='SilentlyContinue';try{$null=Get-Acl -LiteralPath 'C:\\ProgramData';[Console]::Out.WriteLine('acl_available')}catch{[Console]::Out.WriteLine('acl_unavailable')}";
  const probe = async (environment: NodeJS.ProcessEnv) =>
    (
  ...
  assert.equal(
    await probe(legacy),
    'acl_unavailable',
    'Legacy duplicate-key spawn reproduces the incompatible module failure',
  );
  ...
  const faulted = windowsScript('directory')
    .replace('[Console]::In.ReadLine() | ConvertFrom-Json', '$null')
    .replace(
  ...
  try {
    await promisify(execFile)(
      ...
      [
        ...
        '-NonInteractive',
        '-EncodedCommand',
        Buffer.from(faulted, 'utf16le').toString('base64'),
      ],
      { env: windowsPowerShellEnvironment(source), windowsHide: true, timeout: 10000 },
    );
  } catch (error) {
  ...
  process.stdout.write(
    `Windows inherited module collision reproduced and fixed; private error redaction PASS (${process.version})\n`,
  );
}
