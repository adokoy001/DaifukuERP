...
describe('macOS unprivileged process identity', () => {
  it('accepts OS ordinary memberships but rejects administrative process capabilities', () => {
    expect(unprivilegedServiceIdentity(primary)).toBe(true); expect(unprivilegedServiceIdentity({ ...primary, groups: [] })).toBe(false);
    expect(unprivilegedServiceIdentity({ ...primary, groups: [4294967294, 12, 61, 100, 701] })).toBe(true);
    for (const extra of [0, 80, 98, 204]) expect(unprivilegedServiceIdentity({ ...primary, groups: [...primary.groups, extra] })).toBe(false);
    for (const changed of [{ uid: 0 }, { euid: 0 }, { gid: 0 }, { egid: 0 }, { uid: 501 }, { groups: 'unknown' }]) expect(unprivilegedServiceIdentity({ ...primary, ...changed })).toBe(false);
  ...
  it('requires measured identity, explicit enforcement and exact owned account for Mac readiness', () => {
    const runtime = { identity: primary, unprivilegedIdentityRequired: true, unprivilegedIdentityVerified: true }, account = { uid: 400, gid: 4294967294, groups: primary.groups };
    expect(matchesMacServiceIdentity('darwin', runtime, account)).toBe(true);
    for (const altered of [{}, { identity: primary }, { ...runtime, unprivilegedIdentityRequired: false }, { ...runtime, unprivilegedIdentityVerified: false }, { ...runtime, identity: { ...primary, groups: [0] } }]) expect(matchesMacServiceIdentity('darwin', altered, account)).toBe(false);
  ...
  it('parses only the explicit environment section and refuses duplicate keys or sections', () => {
    ...
    expect(daemonEnvironment('environment = {\n DAIFUKU_EDGE_REQUIRE_UNPRIVILEGED_IDENTITY = 1\n}')).toEqual({ DAIFUKU_EDGE_REQUIRE_UNPRIVILEGED_IDENTITY: '1' });
    for (const text of ['DAIFUKU_EDGE_REQUIRE_UNPRIVILEGED_IDENTITY => 1', 'environment = {\n A => 1\n A => 2\n}', 'environment = {\n A => 1\n}\nenvironment = {\n A => 1\n}']) expect(() => daemonEnvironment(text)).toThrow('environment');
  });
  it('requires standard InitGroups=true on disk and the fixed guard in the loaded daemon', async () => {
    ...
    await expect(adapter.start(f.context)).rejects.toThrow('unprivileged identity'); expect(f.changes.length).toBe(before);
    f.loaded = original;
  ...
  it('runs unpaired with ordinary OS groups after verifying no explicit additions', async () => {
    ...
    try {
      await until(() => codes.includes('pairing_required')); expect(verified).toHaveBeenCalled(); expect(relay.state.claims).toBe(0);
    } finally { abort.abort(); await run; measured.mockRestore(); verified.mockRestore(); vi.unstubAllEnvs(); await relay.close(); }
  });
  it('does not pair or execute work when runtime has a privileged membership', async () => {
    ...
    vi.stubEnv('DAIFUKU_EDGE_REQUIRE_UNPRIVILEGED_IDENTITY', '1');
    const measured = vi.spyOn(identity, 'currentServiceIdentity').mockReturnValue({ ...primary, groups: [4294967294, 80] });
    const run = runService(credentials, journal, relay.directory, abort.signal, (code) => codes.push(code));
    try {
      await until(() => codes.includes('service_identity_not_unprivileged'));
      await vi.waitFor(async () => expect(await readPrivateJson(join(relay.directory, 'service-status.json'))).toMatchObject({ phase: 'error', unprivilegedIdentityRequired: true, identity: { groups: [4294967294, 80] } }));
