...
function fixture(
  options: { groups?: string; explicit?: string; invalidGuid?: string; errorAt?: string; thrown?: boolean } = {},
) {
  return vi.fn<DirectoryRunner>(async (file, args) => {
    ...
    if (args[1] === '-read')
      return { code: 0, stdout: options.invalidGuid ?? 'GeneratedUID: ' + guid + '\n', stderr: '' };
    if (args[1] === '-search')
      return {
        code: 0,
        stdout: args[3] === options.explicit ? 'synthetic_group\t\t' + args[3] + ' = explicit value\n' : '',
        stderr: '',
      };
...
describe('macOS directory membership policy', () => {
  it('accepts ordinary computed groups with no direct name/UUID or nested user-UUID assignment', async () => {
    ...
    expect(run.mock.calls).toContainEqual([
      '/usr/bin/dscl',
      ['/Search', '-search', '/Groups', 'GroupMembership', '_daifukuedge'],
    ]);
  ...
  it.each(['GroupMembership', 'GroupMembers', 'NestedGroups'])(
    'rejects any explicit %s assignment, including an ordinary group',
    async (explicit) => {
  ...
  it.each([0, 80, 98, 204])('rejects effective privileged group %s even without a direct assignment', async (group) => {
    await expect(macAccountGroups(fixture({ groups: '-2 12 61 100 ' + group }))).rejects.toThrow(
      'mac_account_privileged_group_membership',
    );
  });
  it.each(['GeneratedUID', 'GroupMembership', 'GroupMembers', 'NestedGroups', '-G'])(
    'never treats a failed %s lookup as no membership',
    async (errorAt) => {
  ...
  it('fails closed on a thrown transport error or invalid/multiple UUIDs', async () => {
    ...
    for (const invalidGuid of ['', 'GeneratedUID: invalid', 'GeneratedUID: ' + guid + '\nGeneratedUID: ' + guid])
      await expect(macAccountGroups(fixture({ invalidGuid }))).rejects.toThrow('mac_account_guid_invalid');
  });
  it('normalizes signed nobody, ordering and duplicates but never accepts malformed group text', () => {
    ...
    expect(sameMacGroups([12, 61, 100], [100, 61, 12, 12])).toBe(true);
    expect(sameMacGroups([12, 61], [12, 61, 100])).toBe(false);
    for (const value of ['', '-1', 'nobody 12', '4294967296', '1.5'])
      expect(() => numericMacGroups(value)).toThrow('mac_account_group_list_invalid');
  });
});
