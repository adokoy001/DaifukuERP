...
async function main(): Promise<void> {
  ...
  try {
    ...
    const marker = join(root, 'installation.json'), lock = join(root, '.setup-lock');
    ...
    release = await acquireWriter(lock, () => { throw new Error('unexpected_lock_loss'); });
    await assert.rejects(acquireWriter(lock, () => undefined), /another_agent_is_running/);
    await release(); release = undefined;
    release = await acquireWriter(lock, () => { throw new Error('unexpected_lock_loss'); });
    const staged = join(root, 'pairing.incoming.json'), inbox = join(root, 'pairing.json');
    ...
    await assert.rejects(windowsDurablePublish(staged, inbox), (error: unknown) => error instanceof Error && 'code' in error && error.code === 'EEXIST');
    ...
    await assert.rejects(windowsDurableReplace(join(link, 'pairing.incoming.json'), join(link, 'pairing.json')), /windows_reparse_path_refused/);
    // A developer PC is not elevated; verify that service mutation remains unavailable.
    let administrator = true;
    try { await runWindowsSetup('administrator'); } catch (error) { assert.match(error instanceof Error ? error.message : '', /windows_administrator_required/); administrator = false; }
    process.stdout.write(`Windows durable setup I/O, inbox no-overwrite and OS writer lock PASS (${process.version}; administrator=${administrator}; no SCM operations)\n`);
  } finally {
    await release?.();
    await unlink(link).catch(() => undefined);
    const resolved = resolve(root);
    if (resolved.startsWith(parent + sep) && resolved.slice(parent.length + 1).startsWith('DaifukuEdgeSetupNative-') && !resolved.slice(parent.length + 1).includes(sep)) await rm(resolved, { recursive: true, force: true });
  }
}
