...
describe('protected installer inputs and truthful runtime freshness', () => {
  it('reads a private input owned by the caller and rejects an oversized input', async () => {
    ...
    expect(JSON.parse((await readPrivateSource(f.fixture.configSource, 65536)).toString('utf8'))).toEqual(
      f.fixture.config,
    );
  ...
  it('rejects a readable-by-others configuration before preparing any managed files', async () => {
    const f = await fixture();
    await chmod(f.fixture.configSource, 0o644);
    await expect(prepareConfig(f.fixture.configSource, f.fixture.statePath)).rejects.toThrow(
      'private_file_permissions_required',
    );
    await expect(lstat(f.fixture.statePath)).rejects.toMatchObject({ code: 'ENOENT' });
  ...
  it('rejects source aliases rather than copying an unsafe input', async () => {
    ...
    await expect(readPrivateSource(hard, 65536)).rejects.toThrow(/unsafe_path_link|invalid_state_file/);
  });
  it('does not queue a world-readable pairing token or mutate the service', async () => {
    const f = await fixture();
    await install(f);
    const source = join(f.fixture.root, 'pair.json');
    await writeFile(source, JSON.stringify({ pairingToken: 's'.repeat(43) }), { mode: 0o644 });
    const events = [...f.fixture.events];
    await expect(
      operate(
        { operation: 'pair', installRoot: f.fixture.installRoot, execute: true, pairingSource: source },
        f.adapter,
      ),
    ).rejects.toThrow('private_file_permissions_required');
    expect(f.fixture.events).toEqual(events);
    await expect(lstat(join(f.fixture.statePath, 'pairing.json'))).rejects.toMatchObject({ code: 'ENOENT' });
  });
  it.each(['unowned', 'private_file_conflict'] as const)(
    'marks a matching PID heartbeat unhealthy when inspection reports %s',
    async (failure) => {
      const f = await fixture();
      await install(f);
      const inspect = f.adapter.inspect;
      f.adapter.inspect = async (context) => ({
        ...(await inspect(context)),
        serviceOwned: failure !== 'unowned',
        conflicts: failure === 'private_file_conflict' ? ['Private file permissions changed.'] : [],
      });
      expect(
        await operate({ operation: 'status', installRoot: f.fixture.installRoot, execute: false }, f.adapter),
      ).toMatchObject({ serviceRunning: true, processId: process.pid, runtimeStatusFresh: false });
    },
  );
});
