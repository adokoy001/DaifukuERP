...
interface Entry extends FileInfo {
  text: string;
  acl?: boolean;
}
export class PosixFixture implements PosixHost {
  ...
  constructor(public platform: 'linux' | 'darwin') {
    ...
    this.context = {
      ...
      configPath: join(statePath, 'config.json'),
      logPath: join(statePath, 'logs'),
      servicePath: join(installRoot, 'service'),
    };
    for (const path of [installRoot, dirname(statePath), dirname(linuxUnitPath), dirname(macPlistPath)])
      this.directoryTree(path);
    const rootEntry = this.files.get(installRoot);
    if (rootEntry) rootEntry.mode = 0o700;
    this.put(join(installRoot, 'installation.json'), JSON.stringify(this.context), 0, 0o600);
  }
  uid = () => this.uidValue;
  directoryTree(path: string): void {
    if (path !== '/') this.directoryTree(dirname(path));
    if (!this.files.has(path))
      this.files.set(path, { kind: 'directory', uid: 0, gid: 0, mode: 0o755, links: 1, text: '' });
  }
  put(path: string, text: string, uid: number, mode = 0o600): void {
    this.files.set(path, { kind: 'file', uid, gid: uid === 0 ? 0 : this.groupId, mode, links: 1, text });
  }
  ...
  write = async (path: string, text: string, mode: number) => {
    if (this.files.has(path)) throw new Error('EEXIST');
    this.put(path, text, 0, mode);
    this.changes.push('write:' + path);
  };
  rename = async (from: string, to: string) => {
    const entry = this.files.get(from);
    if (!entry) throw new Error('Missing temporary fixture file');
    this.files.set(to, entry);
    this.files.delete(from);
    this.changes.push('rename:' + to);
  };
  ...
  mkdir = async (path: string, mode: number) => {
    if (this.files.has(path)) throw new Error('EEXIST');
    this.files.set(path, { kind: 'directory', uid: 0, gid: 0, mode, links: 1, text: '' });
    this.changes.push('mkdir:' + path);
  };
  ...
  run = async (executable: string, args: string[]): Promise<CommandResult> => {
    ...
    if (executable === '/bin/ls')
      return this.ok(`drwx------${this.files.get(args[1] ?? '')?.acl ? '+' : ''} 1 root wheel 0 path`);
    if (executable === '/bin/chmod') {
      const entry = this.files.get(args[1] ?? '');
      if (entry) entry.acl = false;
      this.changes.push('acl:' + args[1]);
      return this.ok();
    }
    ...
    if (executable === '/usr/bin/id')
      return this.user
        ? this.ok(
            args[0] === '-G'
              ? String(this.groupId) + (this.platform === 'darwin' ? ' 12 61' : '') + (this.extraGroup ? ' 0' : '')
              : (this.user.UniqueID ?? ''),
          )
        : { code: 1, stdout: '', stderr: 'no such user' };
    if (executable === '/usr/sbin/useradd') {
      this.user = { UniqueID: '401', RealName: ownershipTag(this.context), Password: '!' };
      this.changes.push('create-account');
      return this.ok();
    }
  ...
  private getent(args: string[]): CommandResult {
    ...
    return this.ok(
      `daifuku-edge:x:${this.user.UniqueID}:${this.groupId}:${this.user.RealName}:/nonexistent:/usr/sbin/nologin`,
    );
  }
  private dscl(args: string[]): CommandResult {
    if (args[0] === '/Search' && args[1] === '-search') return this.ok();
    if (args[0] === '/Search')
      return this.ok(`root 0\n_reserved 400\n${this.user?.UniqueID ? '_daifukuedge ' + this.user.UniqueID : ''}`);
    if (args[1] === '-list') return this.ok('root\n' + (this.user ? '_daifukuedge' : ''));
    if (args[2] === '/Groups/everyone')
      return this.ok('PrimaryGroupID: 12\nGeneratedUID: ABCDEFAB-CDEF-ABCD-EFAB-CDEF0000000C');
    if (args[2] === '/Groups/localaccounts')
      return this.ok('PrimaryGroupID: 61\nGeneratedUID: ABCDEFAB-CDEF-ABCD-EFAB-CDEF0000003D');
    if (args[2] === '/Groups/nobody') return this.ok('PrimaryGroupID: ' + this.groupId);
    if (args[1] === '-read')
      return this.ok(
        Object.entries(this.user ?? {})
          .map(([key, value]) => (key === 'IsHidden' ? 'dsAttrTypeNative:IsHidden' : key) + ': ' + value)
          .join('\n'),
      );
    if (args[1] === '-create') {
      this.user ??= {};
      this.user[args[3] ?? ''] = args[4] ?? '';
      this.changes.push('dscl:' + args[3]);
      return this.ok();
    }
  ...
  private launchctl(args: string[]): CommandResult {
    if (args[0] === 'print') {
      if (!this.loaded)
        return { code: 113, stdout: '', stderr: 'Could not find service "jp.daifuku.edge" in domain for system' };
      const values = (
        this.loaded
          .match(/<key>ProgramArguments<\/key><array>(.*?)<\/array>/)?.[1]
          ?.match(/<string>(.*?)<\/string>/g) ?? []
      ).map((value) => value.slice(8, -9));
      const ca = this.loaded.match(/<key>NODE_EXTRA_CA_CERTS<\/key><string>(.*?)<\/string>/)?.[1];
      return this.ok(
        `system/jp.daifuku.edge = {\n path = ${macPlistPath}\n program = ${values[0]}\n username = _daifukuedge\n group = nobody\n arguments = {\n${values.join('\n')}\n }\n${this.running ? ' pid = 12345\n' : ''} environment = {\n${this.loaded.includes('<key>DAIFUKU_EDGE_REQUIRE_UNPRIVILEGED_IDENTITY</key><string>1</string>') ? ' DAIFUKU_EDGE_REQUIRE_UNPRIVILEGED_IDENTITY => 1\n' : ''}${ca ? ' NODE_EXTRA_CA_CERTS => ' + ca + '\n' : ''} }\n}`,
      );
    }
    this.changes.push('launchctl:' + args[0]);
    if (args[0] === 'bootstrap') {
      this.loaded = this.files.get(macPlistPath)?.text ?? null;
      this.running = true;
    }
  ...
  addRelease(context = this.context): void {
    ...
    const parent = this.files.get(join(context.installRoot, 'releases'));
    if (isNew && parent) parent.mode = 0o700;
