...
describe('bounded IPP protocol and device outcomes', () => {
  it('roundtrips typed attributes and rejects truncated, oversized or mismatched replies', () => {
    const encoded = ippRequest(11, 'ipp://localhost/print', [
      { name: 'requested-attributes', tag: 0x44, values: ['job-id', 'job-state'] },
    ]);
    expect(parseIpp(encoded.body, encoded.requestId).attributes.get('requested-attributes')).toEqual([
      'job-id',
      'job-state',
    ]);
    for (const body of [encoded.body.subarray(0, 7), encoded.body.subarray(0, -1), Buffer.alloc(262145)])
      expect(() => parseIpp(body)).toThrow();
  ...
  it('does not call acceptance completion and observes job state until the printer reports complete', async () => {
    ...
    try {
      ...
      expect(result).toMatchObject({ state: 'succeeded', code: 'ipp_reported_completed', deviceJobId: '41' });
      expect(references).toEqual(['41']);
      expect(printer.state.printCalls).toBe(1);
      expect(printer.state.queryCalls).toBe(2);
    } finally {
      await printer.close();
    }
  });
  it('fails before printing when Japanese text lacks an explicitly supported UTF-8 media type', async () => {
    ...
    try {
      ...
      const result = await printText(
        printerTransport(device, config, signal),
        request,
        { signal, canSend: () => true, accepted: async () => undefined },
        1000,
      );
      expect(result.code).toBe('ipp_text_format_unsupported');
  ...
  it('does not send escape or device control sequences as plain text', async () => {
    ...
    const result = await printText(
      ...
      { ...request, payload: { ...request.payload, text: 'text' + String.fromCharCode(27) + '@' } },
      { signal, canSend: () => true, accepted: async () => undefined },
      1000,
    );
    expect(result.code).toBe('text_control_characters_rejected');
    expect(calls).toEqual([]);
  });
  it('treats forwarded completion and completion with errors as uncertain', () => {
    for (const reason of ['queued-in-device', 'job-completed-with-errors', 'job-completed-with-warnings'])
      expect(
        terminalJob(
          {
            code: 0,
    ...
    expect(terminalJob({ code: 0, requestId: 1, attributes: new Map([['job-state', [7]]]) }, '41')).toMatchObject({
      state: 'failed',
      code: 'ipp_cancelled',
    });
  });
