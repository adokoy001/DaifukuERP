...
export function safePath(value: string): string {
  if (
    ...
    throw new Error('unsafe_absolute_path');
  const path = resolve(value);
  if (path === parse(path).root) throw new Error('filesystem_root_forbidden');
  return path;
}
...
export async function pathChain(path: string, administrative = false): Promise<void> {
  ...
  for (const item of paths) {
    ...
    try {
      ...
      if (missingFile(error)) break;
      throw error;
    }
    if (stat.isSymbolicLink() || (!stat.isDirectory() && item !== resolve(path)) || (stat.isFile() && stat.nlink !== 1))
      throw new Error('unsafe_path_link');
    if (administrative && process.platform !== 'win32' && (stat.uid !== 0 || stat.mode & 0o022))
      throw new Error('administrator_owned_path_required');
  }
}
export async function readRegular(path: string, max = 2_000_000): Promise<Buffer> {
  ...
  try {
    ...
    if (!stat.isFile() || stat.nlink !== 1 || stat.size > max) throw new Error('unsafe_regular_file');
...
export async function jsonFile(path: string): Promise<unknown | undefined> {
  try {
    return JSON.parse((await readRegular(path)).toString('utf8')) as unknown;
  } catch (error) {
...
export async function durableJson(path: string, value: unknown): Promise<void> {
  ...
  if (process.platform === 'win32') await windowsDurableReplace(temporary, path);
  else await rename(temporary, path);
  await syncDirectory(dirname(path));
}
export async function absentOrEmpty(path: string, permitted: string[] = []): Promise<void> {
  try {
    if ((await readdir(path)).some((entry) => !permitted.includes(entry)))
      throw new Error('unmanaged_directory_not_empty');
  } catch (error) {
