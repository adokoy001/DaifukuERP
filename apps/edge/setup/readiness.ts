...
export async function awaitService(context: ServiceContext, adapter: ServiceAdapter, startedAt: number): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (inspection.serviceRunning && inspection.processId === state.pid && observed >= startedAt && age >= 0 && age < 90000 && ['pairing_required', 'connecting', 'running', 'credential_rejected'].includes(String(state.phase))) return;
    await delay(1000);
  }
