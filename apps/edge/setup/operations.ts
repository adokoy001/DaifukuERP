...
async function inspect(request: OperationRequest, adapter: ServiceAdapter) {
  ...
  const marker = await readInstallation(root); const context = marker?.pending?.target.context ?? marker?.active?.context;
  ...
  if (marker.pending && marker.active && service.conflicts.length) service = await adapter.inspect(marker.active.context);
  if (request.operation !== 'status' && (service.conflicts.length || (service.serviceExists && !service.serviceOwned))) throw new Error('service_registration_conflict');
...
export async function operate(request: OperationRequest, adapter: ServiceAdapter): Promise<unknown> {
  ...
  const output = { operation: request.operation, execute: request.execute, service: adapter.serviceId, ...initial.service, statePath: initial.context.statePath, dataRetained: true, pendingPhase: initial.marker.pending?.phase ?? null };
  ...
  try {
    ...
    if (request.operation === 'uninstall') { if (service.serviceExists) await adapter.uninstall(context); marker.status = 'uninstalled'; await saveInstallation(marker); }
      ...
      const target = join(context.statePath, 'pairing.json'), staging = join(context.statePath, 'pairing.incoming.json'), data = JSON.stringify(pairing);
      if (await jsonFile(target) !== undefined) throw new Error('pairing_already_queued');
      try { await exclusiveWrite(staging, data); }
      catch (error) { if (!(error instanceof Error && 'code' in error && error.code === 'EEXIST') || digest(await readRegular(staging, 8192)) !== digest(data)) throw error; }
      await adapter.protect(context);
      if (process.platform === 'win32') await windowsDurablePublish(staging, target); else await rename(staging, target);
