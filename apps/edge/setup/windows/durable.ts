...
export async function windowsDurableReplace(source: string, target: string): Promise<void> {
  const from = windowsPath(source);
  const to = windowsPath(target);
  if (from.toLowerCase() === to.toLowerCase() || win32.dirname(from).toLowerCase() !== win32.dirname(to).toLowerCase())
    throw new Error('windows_atomic_paths_invalid');
...
export async function windowsDurablePublish(source: string, target: string): Promise<void> {
  ...
  if (from.toLowerCase() === to.toLowerCase() || win32.dirname(from).toLowerCase() !== win32.dirname(to).toLowerCase())
    throw new Error('windows_atomic_paths_invalid');
  try {
    await runWindowsPayload('durablePublish', { source: from, target: to });
  } catch (error) {
    if (error instanceof Error && error.message === 'windows_atomic_target_exists')
