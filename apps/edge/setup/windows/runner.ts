...
export type WindowsRunner = (
  operation: WindowsSetupOperation,
...
export async function runWindowsPayload(
  operation: WindowsSetupOperation,
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill();
      reject(new Error('windows_service_operation_timeout'));
    }, 100000);
    child.on('error', () => {
      clearTimeout(timer);
      reject(new Error('windows_powershell_required'));
    });
    child.stdout.on('data', (data: Buffer) => {
      output += data.toString('utf8');
      if (output.length > 65536) {
        child.kill();
        reject(new Error('windows_service_output_invalid'));
      }
    });
    child.once('close', (code) => {
      ...
      try {
        ...
        if (typeof result.code === 'string' && /^windows_[a-z_]{1,64}$/.test(result.code)) {
          reject(new Error(result.code));
          return;
        }
      } catch {
        /* Child output is never included in errors or logs. */
      }
      reject(new Error('windows_service_operation_failed'));
    });
  });
}
export const runWindowsSetup: WindowsRunner = async (operation, context, xml) =>
  runWindowsPayload(operation, { context, xml });
