...
export async function replaceDefinition(host: PosixHost, path: string, body: string, context: ServiceContext): Promise<void> {
  await rootParents(host, path);
  const previous = await rootFile(host, path);
  if (previous !== null && !previous.includes(ownershipTag(context))) throw new Error('Existing service definition belongs to another installation.');
...
export async function saveDefinition(host: PosixHost, context: ServiceContext, name: string, destination: string, body: string): Promise<void> {
