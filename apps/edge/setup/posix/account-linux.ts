...
export async function linuxGroup(host: PosixHost): Promise<{ name: string; gid: number }> {
  for (const name of ['nogroup', 'nobody']) {
    ...
    if (fields[0] !== name || !/^\d+$/.test(fields[2] ?? '') || Number(fields[2]) === 0)
      throw new Error('Invalid unprivileged service group.');
...
export async function inspectLinuxAccount(host: PosixHost, context: ServiceContext): Promise<PosixAccount | null> {
  ...
  if (
    fields.length !== 7 ||
    fields[0] !== 'daifuku-edge' ||
    !/^\d+$/.test(fields[2] ?? '') ||
    Number(fields[2]) < 1 ||
    fields[3] !== String(group.gid) ||
    fields[4] !== ownershipTag(context) ||
  ...
  if (groups.length !== 1 || groups[0] !== String(group.gid))
    throw new Error('Service account has unexpected supplementary groups.');
  const shadow = await command(host, '/usr/bin/getent', ['shadow', 'daifuku-edge']);
  if (!['!', '!!', '*'].includes(shadow.trim().split(':')[1] ?? ''))
    throw new Error('Service account authentication is not locked.');
...
export async function prepareLinuxAccount(host: PosixHost, context: ServiceContext): Promise<PosixAccount> {
  ...
  const group = await linuxGroup(host);
  await command(host, '/usr/sbin/useradd', [
    '--system',
    '--gid',
    group.name,
    '--no-create-home',
