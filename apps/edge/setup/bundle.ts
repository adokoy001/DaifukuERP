...
export const manifestSchema = z
  .object({
    format: z.literal(1),
    kind: z.literal('daifuku-edge-bundle'),
    releaseId: z.string().regex(/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,99}$/),
...
async function inventory(directory: string, prefix = ''): Promise<string[]> {
  ...
  for (const entry of await readdir(join(directory, prefix), { withFileTypes: true })) {
    ...
    else throw new Error('bundle_links_forbidden');
  }
  return out.sort();
}
export async function verifyBundle(
  ...
  const raw = await readRegular(join(directory, 'manifest.json'));
  if (digest(raw) !== expectedHash) throw new Error('manifest_hash_mismatch');
  ...
  if (
    ...
    paths.some((path) => path.startsWith('/') || path.split('/').some((part) => !part || part === '.' || part === '..'))
  )
    throw new Error('unsafe_bundle_entry');
  const required = [
  ...
  if (JSON.stringify(await inventory(directory)) !== JSON.stringify([...paths, 'manifest.json'].sort()))
    throw new Error('unexpected_bundle_file');
  for (const file of manifest.files) {
    const bytes = await readRegular(join(directory, file.path), file.bytes);
    if (bytes.length !== file.bytes || digest(bytes) !== file.sha256) throw new Error('bundle_file_hash_mismatch');
  }
...
export async function copyBundle(bundle: VerifiedBundle, destination: string): Promise<void> {
  ...
  for (const file of bundle.manifest.files) {
    ...
    const data = await readRegular(join(bundle.directory, file.path), file.bytes);
    if (digest(data) !== file.sha256) throw new Error('bundle_changed_during_copy');
    try {
      await exclusiveWrite(target, data, file.executable ? 0o755 : 0o644);
    } catch (error) {
      if (
        !(error instanceof Error && 'code' in error && error.code === 'EEXIST') ||
        digest(await readRegular(target, file.bytes)) !== file.sha256
      )
        throw error;
    }
