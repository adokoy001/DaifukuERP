...
const fail = (message: string): never => { throw new Error(`E2E fixture: ${message}`); };
...
function fixtureUrls() {
  ...
  for (const url of [owner, app]) {
    if (!['postgres:', 'postgresql:'].includes(url.protocol) || !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname)
      ...
      || url.search || url.hash) fail('use a loopback dedicated E2E database with no connection overrides');
  }
  if (owner.hostname !== app.hostname || owner.port !== app.port || owner.pathname !== app.pathname
    || owner.username === app.username || app.username !== 'daifuku_app') fail('owner/app must be distinct roles on the same fixture database');
  return { owner: owner.href, app: app.href };
}

function businessDate() {
  const value = process.env.E2E_INVOICE_DATE ?? new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Tokyo' }).format(new Date());
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value) || !Number.isFinite(Date.parse(value + 'T00:00:00Z'))
    || new Date(value + 'T00:00:00Z').toISOString().slice(0, 10) !== value) fail('E2E_INVOICE_DATE must be a valid YYYY-MM-DD date');
  return value;
}
...
async function assertEmpty(owner: Database, app: Database) {
  ...
  if (!identity?.owns_db || !identity.bypass) fail('the owner connection must own the dedicated database and have migration BYPASSRLS');
  ...
  const objects = await owner.sql`select nspname from pg_namespace where nspname not in ('public', 'information_schema') and nspname not like 'pg_%'
  ...
  if (objects.length) fail('database is not empty; create a NEW dedicated database. No reset or overwrite is performed');
}
...
async function prepare(owner: Database, day: string) {
  ...
  process.stdout.write(`Prepared an empty E2E database: 4 synthetic companies, industry packs not yet applied; business date ${day}.\n`);
}

async function main() {
  // Explicit process environment only: never load a user's .env to select a mutation target.
  const urls = fixtureUrls(), day = businessDate();
  const owner = connect(urls.owner, { max: 1 }), app = connect(urls.app, { max: 1 });
  try {
