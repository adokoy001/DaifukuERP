...
export function parsePackArgs(argv: readonly string[]): PackCliArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--sample') opts.sample = true;
      ...
      const value = args[++i];
      if (companyId !== undefined || value === undefined || !isUuid(value))
        throw new Error(`--company requires one company id (uuid). ${USAGE}`);
      companyId = value;
...
export async function resolvePackCompany(
  owner: Database,
  companyId: string | undefined,
  ...
  if (!demo)
    throw new Error(
      'Demo company not found: pass --company <id> for an existing company, or initialize a separate demo database.',
    );
