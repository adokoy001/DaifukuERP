...
export function registerAnalyticsRoutes(app: FastifyInstance, opts: { db: Database }): void {
  app.get('/analytics/catalog', { schema: { tags: ['analytics'], summary: 'Authorized browser analytics sources, fields and defaults' } }, async (req, reply) => {
  ...
  app.post('/analytics/snapshot', { schema: { tags: ['analytics'], summary: 'Complete bounded read-only snapshot for browser pivoting', body: snapshotInput } }, async (req, reply) => {
    reply.header('cache-control', 'no-store');
    const input = parse(snapshotInput, req.body, 'body');
    return withContext(opts.db, req.contextParams(), (ctx) => analyticsSnapshot(ctx, input), { readOnlySnapshot: true });
  });
}
