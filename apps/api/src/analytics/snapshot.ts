...
export const snapshotInput = z
  ...
  .strict()
  .refine((input) => input.from <= input.to, { path: ['to'], message: '終了日は開始日以降にしてください。' });
  ...
  return createHash('sha256')
    .update(
      JSON.stringify({
        ...
        sites: sorted(ctx.siteIds),
        stores: sorted(ctx.storeIds),
        packs: sorted(ctx.appliedPacks),
...
export function analyticsMeasureValue(source: SourceDefinition, key: string, row: Row): string | null {
  ...
  if (derived?.signedBy) {
    ...
    if (direction !== 'pay' && direction !== 'receive')
      throw new StateError('入出金区分を確認できませんでした。', '銀行明細の区分を確認してください。');
...
export async function collectCompleteRows(
  readPage: (offset: number) => Promise<{ items: Row[]; total: number }>,
  ...
  for (;;) {
    ...
    if (page.total > limit)
      throw new ValidationError(
        `分析対象が上限 ${limit.toLocaleString('ja-JP')} 件を超えています。`,
        [{ path: 'from', message: '期間を短くして再取得してください。' }],
        '部分的な合計を表示しないため、取得を中止しました。',
      );
    if (expected === undefined) expected = page.total;
    if (
      page.total !== expected ||
      rows.length + page.items.length > expected ||
      (!page.items.length && rows.length < expected)
    )
      throw new StateError('分析データの完全性を確認できませんでした。', '期間を見直して再取得してください。');
...
async function referenceLabels(
  ...
  for (const col of dataset.dimensions) {
    ...
    try {
      assertReadableFields(ctx, target, [display]);
    } catch (error) {
      if (error instanceof PermissionDenied) continue;
      throw error;
    }
    ...
    for (let index = 0; index < ids.length; index += 500) {
      const page = await repo(ctx, target).list({
...
export async function analyticsSnapshot(ctx: Context, input: SnapshotInput): Promise<AnalyticsSnapshot> {
  ...
  if (!state)
    throw new ValidationError('この分析対象では利用できない状態です。', [
      { path: 'state', message: '対象の状態一覧から選択してください。' },
    ]);
  ...
  const needed = new Set([
    ...dataset.dimensions.map((col) => col.key),
    ...dataset.measures.flatMap((col) => {
  ...
  const raw = await collectCompleteRows(async (offset) => {
    ...
    return {
      total: page.total,
      items: page.items.map((row) => Object.fromEntries([...needed].map((key) => [key, row[key]]))),
    };
  });
  if (source.dimensions.includes('currency')) {
    ...
    if (new Set(raw.map((row) => row.currency)).size > 1)
      throw new StateError('複数の通貨が含まれるため合算できません。', '単一通貨の対象を選択してください。');
  }
  ...
  return {
    ...
    columns,
    rows,
    scopeKey: analyticsScopeKey(ctx, catalog),
    meta: {
      from: input.from,
      to: input.to,
      state: input.state,
