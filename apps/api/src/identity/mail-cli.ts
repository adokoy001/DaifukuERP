...
async function main(): Promise<void> {
  ...
  try {
    console.warn(JSON.stringify(await deliverIdentityMail(owner, identity.encryptionKey, smtpTransport(smtp))));
  } finally {
    await owner.close();
  }
}
main().catch((error: unknown) => {
  console.error(JSON.stringify({ message: 'Identity mail worker failed.', ...safeErrorDiagnostics(error) }));
  process.exitCode = 1;
});
