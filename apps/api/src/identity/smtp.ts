...
export function smtpTransport(config: SmtpOptions): IdentityMailTransport {
  const transport = nodemailer.createTransport({
    ...
    tls: { minVersion: 'TLSv1.2', rejectUnauthorized: true, ...(config.tlsCa ? { ca: config.tlsCa } : {}) },
  ...
  return {
    async send(message) {
      ...
      if (response.rejected.length || response.accepted.length !== 1)
        throw new Error('Mail recipient was not accepted');
    },
  };
}
