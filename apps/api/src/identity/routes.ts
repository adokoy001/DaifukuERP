import {
  ...
  changeMfa,
  PermissionDenied,
  completeIdentityToken,
  completeMfaLogin,
  confirmMfa,
  identityRateLimit,
  identitySecurity,
  inviteIdentity,
  requestIdentityReset,
  stepUpIdentity,
  withContext,
} from '@daifuku/kernel';
...
export function registerIdentityRoutes(app: FastifyInstance, options: IdentityRouteOptions): void {
  ...
  app.get('/auth/security', async (req) => ({
    ...(await withContext(options.db, req.contextParams(), (ctx) =>
      identitySecurity(ctx, currentIdentity(req).sessionVersion),
    )),
    configured: Boolean(options.identity),
  }));
  app.post('/auth/step-up', async (req) => {
    ...
    return verifyAttempt(
      ...
      'stepup',
      () =>
        withContext(options.db, req.contextParams(), (ctx) =>
          stepUpIdentity(ctx, current.sessionVersion, input.currentPassword, input.code, config.encryptionKey),
        ),
      current.userId,
    );
  });
    ...
    return verifyAttempt(
      ...
      () =>
        confirmMfa(
          ...
          input.setupToken,
          input.code,
          configured(options).encryptionKey,
        ),
      currentIdentity(req).userId,
    );
  });
  for (const [path, mode] of [
    ...
    app.post(`/auth/mfa/${path}`, async (req) => {
      const input = parse(step, req.body, 'body');
      return withContext(options.db, req.contextParams(), (ctx) =>
        changeMfa(ctx, currentIdentity(req).sessionVersion, input.stepUpToken, mode),
      );
    });
  app.post('/auth/mfa/verify', async (req) => {
    ...
    return verifyAttempt(options.owner, req, 'mfa-login', async () => {
      const identity = await completeMfaLogin(
        options.owner,
  ...
  app.post('/auth/password-reset/request', async (req) => {
    ...
    try {
      if (options.identity)
        await requestIdentityReset(options.owner, input.email, input.tenantId, {
          ...options.identity,
          mailConfigured: Boolean(options.identity.mailTransport),
        });
    } catch {
      /* Same response for an inactive/raced/unknown identity; do not disclose its state. */
    } finally {
  ...
  for (const [path, purpose] of [
    ['password-reset/complete', 'password_reset'],
    ['invitations/accept', 'invitation'],
  ] as const)
    app.post(`/auth/${path}`, async (req) => {
      const input = parse(z.object({ token, newPassword: password }).strict(), req.body, 'body');
      return verifyAttempt(options.owner, req, purpose, () =>
        completeIdentityToken(options.owner, input.token, purpose, input.newPassword),
      );
    });
  app.post('/auth/invitations', async (req) => {
    ...
    return withContext(options.db, req.contextParams(), (ctx) =>
      inviteIdentity(ctx, current.sessionVersion, input, { ...config, mailConfigured: Boolean(config.mailTransport) }),
    );
  });
}
