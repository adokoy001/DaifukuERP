...
import { DaifukuError, getCompany, isAdmin, PermissionDenied, registry, setSetting, type Context, type Database, type Label, type SettingDef } from '@daifuku/kernel';
...
function unknownSetting(key: string): DaifukuError {
  return new DaifukuError('NOT_FOUND', `setting "${key}" is not declared`, 'List declared settings with GET /meta/settings.', { key }, 404);
}

function assertSettingsRole(ctx: Context): void {
  if ((ctx.accessScope === 'stores' || ctx.accessScope === 'sites')) throw new PermissionDenied('company_settings', 'read', ctx.roles);
  if (!isAdmin(ctx) && !ctx.roles.includes('settings')) throw new PermissionDenied('company_settings', 'read', ctx.roles);
}
...
function toOut(def: SettingDef, stored: Record<string, unknown>, toJsonSchema: JsonSchemaFn): SettingOut {
  ...
  const out: SettingOut = { key: def.key, label: def.label, schema: toJsonSchema(def.schema, 'input') ?? { type: 'object' }, value: parsed?.success ? parsed.data : null };
  if (def.description) out.description = def.description;
  return out;
}

export function registerSettingsRoutes(app: FastifyInstance, opts: { db: Database; toJsonSchema: JsonSchemaFn }): void {
  app.get('/meta/settings', { schema: { tags: ['meta'], summary: 'Company settings declared by modules, with their JSON Schema and current value (admin/settings role)' } }, async (req) =>
  ...
  app.put('/meta/settings/:key', { schema: { tags: ['meta'], summary: 'Set one company setting: body { value } validated by the declaring module (admin/settings role)', params: keyParams, body: putBody } }, async (req) => {
