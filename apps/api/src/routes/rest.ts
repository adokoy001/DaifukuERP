...
const documentInput = z
  .object({ expectedVersion: z.number().int().min(1).optional(), correctionDate: z.iso.date().optional() })
  .strict();
  ...
  return s
    ...
    .map((part) => {
      ...
      if (!field || (dir !== undefined && dir !== 'asc' && dir !== 'desc')) {
        throw new ValidationError(`invalid orderBy "${part}"`, [
          { path: 'orderBy', message: 'expected field or field:asc|desc' },
        ]);
      }
...
export function updateInput(id: string, body: unknown): Record<string, unknown> {
  if (body !== null && typeof body === 'object' && !Array.isArray(body)) {
    ...
    const wrapped =
      ...
      b.patch !== null &&
      keys.every((k) => k === 'patch' || k === 'expectedVersion');
    if (wrapped)
      return { id, patch: b.patch, ...(b.expectedVersion !== undefined ? { expectedVersion: b.expectedVersion } : {}) };
  }
...
export function registerRestRoutes(app: FastifyInstance, opts: { db: Database }): void {
  ...
  app.get(
    ...
    {
      schema: {
        ...tag('List records (search, limit, offset, orderBy=field:asc, where=<json>)'),
        querystring: listQuery,
      },
    },
    async (req) => act(req, `${entityOf(req.params)}.list`, listInput(req.query)),
  );
  ...
  app.post('/api/:entity', { schema: tag('Create a record (body = insert input)'), ...NO_VALIDATION }, async (req) =>
    act(req, `${entityOf(req.params)}.create`, req.body),
  );
  app.patch(
    '/api/:entity/:id',
    { schema: tagId('Update a record (body = patch, or { patch, expectedVersion })'), ...NO_VALIDATION },
    async (req) => {
      const { id } = parse(idParams, req.params, 'params');
  ...
  app.post(
    '/api/:entity/:id/:op',
    { schema: { tags: ['rest'], summary: 'Document lifecycle: submit | cancel | amend', params: opParams } },
    async (req) => {
