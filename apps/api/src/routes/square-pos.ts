...
import {
  ...
  squareConnectionSchema,
  verifySquareSignature,
  type SquareConnection,
} from '../adapters/square-pos.ts';
export interface SquarePosOptions {
  app: Database;
  owner: Database;
  connections?: readonly SquareConnection[];
}
...
export async function registerSquarePosRoutes(server: FastifyInstance, options: SquarePosOptions) {
  ...
  await server.register(async (scope) => {
    ...
    scope.post<{ Params: { connectionKey: string }; Body: string }>(
      '/webhooks/square/:connectionKey',
      { schema: { hide: true }, bodyLimit: 1_048_576 },
      async (request, reply) => {
        const connection = connections.find((row) => row.key === request.params.connectionKey);
        if (!connection)
          throw new DaifukuError(
            'INVALID_STATE',
            'Square connection is not configured',
            'Configure a dedicated integration principal and verified merchant/location mapping.',
            undefined,
            503,
          );
        ...
        if (
          ...
          !verifySquareSignature(request.body, typeof signature === 'string' ? signature : undefined, connection)
        )
          throw new PermissionDenied('square_webhook', 'signature', []);
        const event = normalizeSquareEvent(request.body);
        if (
          event.merchantId !== connection.merchantId ||
          (event.locationId !== null && event.locationId !== connection.externalLocationId)
        )
          throw new PermissionDenied('square_webhook', 'merchant-location', []);
        const principal = await loadPrincipal(options.owner, connection.userId);
        if (!principal || principal.tenantId !== connection.tenantId)
          throw new PermissionDenied('square_webhook', 'integration-principal', []);
        ...
        const result = await withContext(
          ...
          (ctx) => receivePosEvent(ctx, connection.locationId, event),
        );
