...
export function registerAccessAdminRoutes(app: FastifyInstance, opts: { db: Database }): void {
  app.get(
    '/admin/access',
    { schema: { tags: ['access'], summary: 'Tenant user and company access directory' } },
    (req) => withRequestContext(opts.db, req, accessDirectory),
  );
  app.get(
    '/admin/access/audit',
    { schema: { tags: ['access'], querystring: z.object({ userId: z.uuid() }) } },
    (req) => {
  ...
  app.post(
    '/admin/users',
    { schema: { tags: ['access'], body: createUserSchema }, validatorCompiler: passthroughValidator },
    (req) => withRequestContext(opts.db, req, (ctx) => createManagedUser(ctx, req.body)),
  );
  app.patch(
    '/admin/users/:id',
    {
      schema: { tags: ['access'], params: userParams, body: updateUserSchema },
  ...
  app.put(
    '/admin/users/:id/companies/:companyId',
    {
      schema: { tags: ['access'], params: membershipParams, body: membershipSchema },
      validatorCompiler: passthroughValidator,
    },
    (req) => {
  ...
  app.delete(
    '/admin/users/:id/companies/:companyId',
    {
      schema: { tags: ['access'], params: membershipParams, body: removeMembershipSchema },
      validatorCompiler: passthroughValidator,
    },
    (req) => {
      const { id, companyId } = parse(membershipParams, req.params, 'params');
