import { bootstrapTenant, connect, runMigrations, systemParams, withContext, type Database, type ModuleDef } from '@daifuku/kernel';
...
export interface SetupHooks { progress?: (phase: string) => void; checkpoint?: (phase: string) => Promise<void> }
async function checkpoint(options: SetupOptions, state: SetupState, phase: string, hooks: SetupHooks): Promise<void> {
  state.phase = phase; await saveState(options.stateDir, state); hooks.progress?.(phase); await hooks.checkpoint?.(phase);
}
async function bootstrap(owner: Database, options: SetupOptions, state: SetupState, password: string, modules: readonly ModuleDef[], hooks: SetupHooks): Promise<void> {
  ...
  const company = await owner.sql`select code, name from companies where id=${boot.companyId} and tenant_id=${identity.tenantId}`;
  if (company[0]?.code !== identity.companyCode || company[0]?.name !== identity.companyName) throw new SetupError('BOOTSTRAP_IDENTITY', '既存管理者の会社が初回指定と一致しません。設定を更新せず停止しました。');
...
function requirements(options: SetupOptions, plan: SetupPlan): void {
  if (options.confirmTarget !== plan.target.id) throw new SetupError('CONFIRM_TARGET', '計画に表示された対象識別子をconfirm-targetで指定してください。');
  if (!options.maintenanceConfirmed) throw new SetupError('MAINTENANCE_REQUIRED', '全API/MCP/worker・外部書込元を停止し、maintenance-confirmedを指定してください。');
  if (plan.otherConnections) throw new SetupError('DATABASE_BUSY', '対象DBに他の接続があります。切断・強制終了は行わないので、利用元を停止して再計画してください。');
  if (!plan.noOp && (!plan.restoreTarget || !plan.restoreEmpty)) throw new SetupError('RESTORE_REQUIRED', '変更前に使用する明示の空RESTORE_CHECK_URLが必要です。復元済DBを消去して再利用することはありません。');
}
export async function runSetup(options: SetupOptions, hooks: SetupHooks = {}, dir = MIGRATIONS_DIR): Promise<SetupPlan> {
  ...
  if (existing && (existing.targetId !== plan.target.id || (existing.phase !== 'complete' && existing.operationMode !== options.mode))) throw new SetupError('STATE_CHANGED', '計画後にcheckpointの対象またはmodeが変化しました。再計画してください。');
  ...
  try {
    if ((await inspectTarget(owner, secrets.ownerUrl)).id !== plan.target.id) throw new SetupError('TARGET_CHANGED', '実行接続先が計画したDBと一致しません。再計画してください。');
    ...
    if (await otherConnections(owner)) throw new SetupError('DATABASE_BUSY', '計画後に他の接続が開始されました。利用元を停止して再実行してください。');
    await privateDirectory(options.stateDir);
    const state: SetupState = existing ?? { format: 1, operationMode: options.mode, targetId: plan.target.id, ...(identity ? { identity } : {}), phase: 'planned', seededModules: [], backups: [] };
    await runtimeConfig(options, secrets, state);
    if (plan.noOp) { hooks.progress?.('already-complete'); return plan; }
    ...
    if (await otherConnections(owner)) throw new SetupError('DATABASE_BUSY', 'バックアップ中に他の接続が開始されました。対象の移行は開始していません。');
    await owner.sql`set lock_timeout='5s'`;
    await runMigrations(owner, dir);
    await checkpoint(options, state, 'migrated', hooks);
    if (options.mode === 'install') { const runtime = await loadRuntime({ schema: true }); await bootstrap(owner, options, state, password, runtime.modules, hooks); }
