...
export async function inspectTarget(db: Database, url: string): Promise<Target> {
  ...
  const row = rows[0];
  if (!row || row.username !== row.owner || !row.bypass || row.superuser)
...
async function checkApp(secrets: SetupSecrets, target: Target): Promise<void> {
  ...
  try {
    const rows = await app.sql<
      { username: string; database: string; bad: boolean; owner_member: boolean; can_create: boolean }[]
    >`
    ...
    if (
      ...
      throw new SetupError(
        'APP_ROLE',
        'appはdaifuku_app専用roleで、所有者・管理権限・public CREATE・owner継承を持たない必要があります。',
      );
  } finally {
    await app.close();
  }
...
export async function relationNames(db: Database): Promise<string[]> {
  ...
  >`select n.nspname as schema, c.relname as name from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname not in ('pg_catalog','information_schema') and n.nspname not like 'pg_toast%' and c.relkind in ('r','p','v','m','S','f') order by n.nspname,c.relname`;
  return rows.map((row) => `${row.schema}.${row.name}`);
}
export async function otherConnections(db: Database): Promise<number> {
  const rows =
    await db.sql`select count(*)::int as count from pg_stat_activity where datname=current_database() and pid <> pg_backend_pid()`;
  return Number(rows[0]?.count ?? 0);
}
async function inspectRestore(
  secrets: SetupSecrets,
  target: Target,
): Promise<{ label: string | null; empty: boolean }> {
  ...
  try {
    ...
    if (rt.id === target.id || (rt.host === target.host && rt.port === target.port && rt.database === target.database))
      throw new SetupError('RESTORE_SAME_TARGET', '復元確認先を導入先と同じDBにはできません。');
    return {
      label: `${rt.host}:${rt.port}/${rt.database}`,
      empty: (await relationNames(restore)).length === 0 && (await otherConnections(restore)) === 0,
    };
  } finally {
    await restore.close();
  }
...
export async function buildPlan(
  ...
  try {
    ...
    if (state && state.targetId !== target.id)
      throw new SetupError('STATE_TARGET', 'checkpointは別のDBを指しています。流用・上書きできません。');
    if (state && state.phase !== 'complete' && state.operationMode !== options.mode)
      throw new SetupError(
        'STATE_MODE',
        '未完了の導入・更新は同じmodeで再開してください。管理者作成や移行を飛ばして完了扱いにはしません。',
      );
    ...
    if ((!empty && !applied.length) || (options.mode === 'install' && !empty && !state?.identity))
      throw new SetupError(
        'EXISTING_DATABASE',
        '新規導入先が空ではないか、既知の移行履歴がありません。resetや自動採用は行いません。',
      );
    if (options.mode === 'upgrade' && !applied.length)
      throw new SetupError('NOT_INSTALLED', '更新対象に大福の移行履歴がありません。');
    ...
    return {
      ...
      pending: catalog.slice(applied.length).map((file) => file.tag),
      migrationHash: digest,
      backupDirectory: join(options.stateDir, 'backups'),
      restoreTarget: restore.label,
      restoreEmpty: restore.empty,
      otherConnections: await otherConnections(owner),
      noOp: state?.phase === 'complete' && state.completedHash === digest && applied.length === catalog.length,
    };
  } finally {
    await owner.close();
  }
