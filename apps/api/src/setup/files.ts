...
export async function privateFile(path: string): Promise<string> {
  ...
  try {
    ...
    if (!stat.isFile() || (stat.mode & 0o077) !== 0 || (process.getuid && stat.uid !== process.getuid())) throw new SetupError('PRIVATE_PERMISSIONS', '秘密ファイルは自分が所有する通常ファイル、権限0600にしてください。');
...
export async function privateDirectory(path: string): Promise<void> {
  ...
  if (await realpath(ancestor) !== resolve(ancestor)) throw new SetupError('STATE_SYMLINK', '運用ディレクトリの経路にsymlinkを使用できません。');
  await mkdir(path, { recursive: true, mode: 0o700 });
  const stat = await lstat(path);
  if (!stat.isDirectory() || stat.isSymbolicLink() || (stat.mode & 0o077) !== 0 || (process.getuid && stat.uid !== process.getuid())) throw new SetupError('STATE_PERMISSIONS', '運用ディレクトリは自分の所有で権限0700にしてください。');
}
...
export async function readState(dir: string): Promise<SetupState | undefined> {
  ...
  if (state.format !== 1 || !['install', 'upgrade'].includes(state.operationMode) || typeof state.phase !== 'string' || typeof state.targetId !== 'string' || !Array.isArray(state.seededModules) || !Array.isArray(state.backups)) throw new SetupError('STATE_INVALID', 'checkpoint形式が不正です。新規導入として上書きしません。');
  return state;
}
