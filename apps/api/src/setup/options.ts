...
export function parseOptions(args: string[]): SetupOptions {
  const { values, positionals } = parseArgs({
    ...
    options: {
      ...
      'generate-admin-password': { type: 'boolean', default: false },
    },
  });
  const mode = positionals[0] ?? 'install';
  if (!['install', 'upgrade'].includes(mode) || positionals.length > 1 || !values.env || !values['state-dir'])
    throw new SetupError(
      'USAGE',
      'pnpm run setup install|upgrade --env /abs/private.env --state-dir /abs/operations（既定は計画のみ）',
    );
  if (!isAbsolute(values.env) || !isAbsolute(values['state-dir']))
    throw new SetupError('PATH', 'envとstate-dirは絶対パスで指定してください。');
  return {
    mode: mode as SetupOptions['mode'],
    envFile: values.env,
    ...
    adminPasswordFile: values['admin-password-file'],
    generateAdminPassword: values['generate-admin-password'],
  };
}
export function connection(url: string, allowRemote: boolean): URL {
  ...
  try {
    ...
    throw new SetupError('URL', 'PostgreSQL接続設定の形式を確認してください。');
  }
  if (
    ...
    throw new SetupError('URL', 'ホスト・DB名・role・passwordを持つPostgreSQL URLが必要です。');
  ...
  if (password.length < 16 || classes < 3 || /[\r\n\0]/.test(password))
    throw new SetupError(
      'DB_SECRET',
      'DBのpasswordは16文字以上・英大/小/数字/記号の3種類以上とし、改行・NULを含めないでください。',
    );
  if (!allowRemote && !['localhost', '127.0.0.1', '[::1]'].includes(parsed.hostname))
    throw new SetupError('REMOTE', '既定はlocalhostだけです。遠隔DBは接続先を確認してallow-remoteを明示してください。');
  if (
    ...
    throw new SetupError(
      'URL_OPTIONS',
      '接続URLの追加optionはsslmodeだけを許可します。host・role・DBの別指定は使用できません。',
    );
  const ssl = parsed.searchParams.get('sslmode');
  if (ssl && !['disable', 'require', 'verify-full'].includes(ssl))
    throw new SetupError('URL_SSL', 'sslmodeはdisable/require/verify-fullから指定してください。');
  if (!['localhost', '127.0.0.1', '[::1]'].includes(parsed.hostname) && ssl !== 'verify-full')
    throw new SetupError(
      'REMOTE_TLS',
      '遠隔DBにはsslmode=verify-fullと、Node/PostgreSQL双方が信頼する証明書が必要です。',
    );
  return parsed;
}
export function validateJwt(secret: string): void {
  if (
    ...
    /[\r\n\0]/.test(secret) ||
    /dev-secret-change-me|^password$|^secret$/i.test(secret)
  )
    throw new SetupError(
      'WEAK_SECRET',
      'JWT_SECRETは既知の開発値以外、改行を含まない32文字以上のランダムな値にしてください。',
    );
}
export async function readSecrets(options: SetupOptions): Promise<SetupSecrets> {
  ...
  if (
    ...
    throw new SetupError('TARGET_MISMATCH', 'owner/appは同じホスト・port・DBの別roleでなければなりません。');
  if (decodeURIComponent(owner.password) === decodeURIComponent(app.password))
    throw new SetupError(
      'SHARED_DB_SECRET',
      'owner/appは異なるpasswordを使用してください。appの秘密でownerへ接続できる設定は拒否します。',
    );
  ...
  if (options.mode === 'upgrade' && !env.JWT_SECRET)
    throw new SetupError(
      'EXISTING_SECRET_REQUIRED',
      '更新には既存JWT_SECRETを含む保護envを指定してください。新しい認証秘密へ置換しません。',
    );
  return {
    ownerUrl: env.DATABASE_URL_OWNER,
    appUrl: env.DATABASE_URL,
    restoreUrl: env.RESTORE_CHECK_URL,
