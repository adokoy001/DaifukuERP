  ...
  const origins = value.split(',').map((item) => {
    try {
      ...
      throw new ApiConfigError(
        'CORS_ORIGINS must be comma-separated http(s) origins without credentials, paths, queries, fragments, or wildcards',
      );
    }
  });
...
export function readApiConfig(env: Environment): ApiConfig {
  ...
  if (!['development', 'test', 'production'].includes(mode))
    throw new ApiConfigError('NODE_ENV must be development, test, or production');
  const host = env.HOST ?? '127.0.0.1';
  if (!host || /[\s\0/\\]/.test(host))
    throw new ApiConfigError('HOST must be a nonempty IP address or hostname without whitespace');
  const port = env.PORT ?? '3000';
  if (!/^\d+$/.test(port) || Number(port) < 1 || Number(port) > 65535)
    throw new ApiConfigError('PORT must be a decimal integer from 1 to 65535');
  ...
  if (
    ...
    (exposed &&
      (jwtSecret.length < 32 ||
        ...
        /dev-secret-change-me|^password$|^secret$/i.test(jwtSecret)))
  ) {
    throw new ApiConfigError(
      'JWT_SECRET must contain no newline or NUL; production or non-loopback HOST requires a random secret of at least 32 characters and 8 distinct characters, excluding known development values',
    );
  }
  let trustedProxies: readonly string[];
  try {
    trustedProxies = trustedProxyPeers(env.TRUSTED_PROXY_CIDRS);
  } catch {
    throw new ApiConfigError(
      'TRUSTED_PROXY_CIDRS must be a bounded list of explicit IP addresses or CIDRs; broad trust is forbidden.',
    );
  }
  return {
    trustedProxies,
...
export function startupErrorMessage(error: unknown): string {
  return error instanceof ApiConfigError
    ? error.message
    : 'API startup failed. Verify the database, storage, and listen settings; connection details are not logged.';
}
