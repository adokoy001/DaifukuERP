...
export function relaySecret(request: FastifyRequest): string {
  ...
  if (
    ...
    throw new DaifukuError(
      'PERMISSION_DENIED',
      'Invalid relay authentication transport',
      'Use only the native Authorization bearer header.',
      undefined,
      401,
    );
...
export function relayContext<T>(opts: EdgeOptions, principal: RelayPrincipal, fn: (ctx: Context) => Promise<T>) {
  return withContext(
    opts.app,
    {
      ...
      actor: { type: 'relay', id: principal.gatewayId },
      roles: ['relay'],
      accessScope: 'all',
      relay: {
...
export async function withRelayRequest<T>(opts: EdgeOptions, req: FastifyRequest, fn: (ctx: Context) => Promise<T>) {
  const principal = await withIdentityAttempt(opts.owner, [{ key: 'relay-auth:' + req.ip, limit: 100 }], () =>
    authenticateRelay(opts.owner, relaySecret(req)),
  );
...
export function sessionView(principal: RelayPrincipal, now = new Date()) {
  return {
    gatewayId: principal.gatewayId,
    companyId: principal.companyId,
    siteId: principal.siteId,
    credentialVersion: principal.credentialVersion,
    credentialExpiresAt: principal.expiresAt.toISOString(),
    serverTime: now.toISOString(),
    protocolVersion: EDGE_PROTOCOL_VERSION,
    pollAfterMs: EDGE_POLL_MS,
    heartbeatAfterMs: EDGE_HEARTBEAT_MS,
  };
}
export async function verifiedSocket(opts: EdgeOptions, request: FastifyRequest) {
  const principal = await withIdentityAttempt(opts.owner, [{ key: 'relay-auth:' + request.ip, limit: 100 }], () =>
    authenticateRelay(opts.owner, relaySecret(request)),
  );
  await identityRateLimit(opts.owner, 'relay-sockets:' + principal.tenantId + ':' + principal.gatewayId, 20, 60);
  await relayContext(opts, principal, (ctx) => relayWork(ctx, async () => true));
  return principal;
}
