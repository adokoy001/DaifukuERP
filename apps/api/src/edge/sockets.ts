  ...
  server.get(
    edgeRoutes.notifications,
    {
      ...
      preValidation: async (req) => {
        pending.set(req, await verifiedSocket(opts, req));
      },
    },
    (socket, req) => {
      const principal = pending.get(req);
      pending.delete(req);
      if (!principal) {
      ...
      const check = async () => {
        ...
        try {
          ...
          socket.close(1008, 'Gateway authorization expired');
        } finally {
          running = false;
        }
      };
      ...
      socket.on('close', () => {
        ...
        group.delete(socket);
        if (!group.size) groups.delete(key);
      });
    },
  );
