...
import { z } from 'zod';
export const squareConnectionSchema = z
  .object({
    ...
    externalLocationId: z.string().min(1).max(100),
    notificationUrl: z.url().refine((value) => {
      const u = new URL(value);
      return u.protocol === 'https:' && !u.username && !u.password && !u.hash && !u.search;
    }),
    signatureKey: z.string().min(16).max(500),
  })
  .strict();
...
export function parseSquareConnections(raw: string | undefined): SquareConnection[] {
  ...
  if (
    ...
    new Set(rows.map((row) => `${row.merchantId}:${row.externalLocationId}`)).size !== rows.length
  )
    throw new Error('Square connection keys and merchant/location mappings must be unique.');
  return rows;
}
export function verifySquareSignature(
  body: string,
  signature: string | undefined,
  connection: Pick<SquareConnection, 'notificationUrl' | 'signatureKey'>,
): boolean {
...
function money(value: unknown) {
  ...
  return {
    amount:
      typeof row.amount === 'number' && Number.isSafeInteger(row.amount) && row.amount >= 0 ? String(row.amount) : null,
...
export function normalizeSquareEvent(body: string): NormalizedPosEvent {
  ...
  if (!eventId || !merchantId || !eventAt || !z.iso.datetime({ offset: true }).safeParse(eventAt).success)
    throw new ValidationError(
      ...
      [],
      'Require event_id, merchant_id and created_at from the signed event.',
    );
  const kind = ['payment.created', 'payment.updated'].includes(type)
    ? 'payment'
  ...
  if (
    ...
    (!created ||
      !updated ||
      !z.iso.datetime({ offset: true }).safeParse(created).success ||
      !z.iso.datetime({ offset: true }).safeParse(updated).success ||
      !string(item.id))
  )
    unsupportedReason = 'Missing or invalid provider transaction identity/time';
  ...
  if (
    ...
    unsupportedReason = 'Unknown provider transaction status';
  if (state === 'COMPLETED' && (!amount.amount || !amount.currency))
    unsupportedReason = 'Completed money is missing or unsafe';
  return normalizedPosEvent.parse({
    provider: 'square',
    eventId,
    merchantId,
    locationId,
    kind: unsupportedReason ? 'unsupported' : kind,
    externalId,
    paymentId,
    state,
    occurredAt: created && z.iso.datetime({ offset: true }).safeParse(created).success ? created : eventAt,
    updatedAt: updated && z.iso.datetime({ offset: true }).safeParse(updated).success ? updated : eventAt,
    ...amount,
