...
beforeAll(async () => {
  ...
  await db.run({}, async (ctx) => {
    for (const module of modules) await module.seed?.(ctx);
  });
...
const act = (name: string, input: unknown) =>
  db.run({}, (ctx) => runAction(ctx, name, input)) as Promise<Record<string, unknown>>;
async function invoice() {
  const customer = await act('partner.create', { name: 'Document options customer', isCustomer: true });
  return act('sales_invoice.create', {
    partnerId: customer.id,
    date: '2026-09-12',
    lines: {
      sales_invoice_line: [{ description: 'Service', quantity: '1', unitPrice: '9000', taxCategory: 'standard' }],
    },
  });
}
const post = (url: string, payload: unknown, method: 'POST' | 'DELETE' = 'POST') =>
  app.inject({
    method,
    url,
...
describe('foundation-refresh lifecycle transport', () => {
  it('preserves the explicit correction date through REST into both invoice history and reversal posting', async () => {
    ...
    const cancelled = await post(`/api/sales_invoice/${String(row.id)}/cancel`, {
      expectedVersion: row.version,
      correctionDate: '2026-09-13',
    });
    expect(cancelled.statusCode, cancelled.body).toBe(200);
    expect(cancelled.json()).toMatchObject({ docstatus: 2, cancelledDate: '2026-09-13' });
    const reversed = await db.run({}, (ctx) =>
      repo(ctx, JournalEntry).list({ where: { reversalOf: String(row.journalEntryId) } }),
    );
  ...
  it('keeps a draft when DELETE carries a stale version', async () => {
    const draft = await invoice();
    await act('sales_invoice.update', {
      id: draft.id,
      patch: { note: 'Saved in another window' },
