...
describe('deployment proxy trust boundary', () => {
  it('rejects unrestricted or ambiguous peers and preserves explicit addresses', () => {
    ...
    for (const value of [
      ...
      expect(() => trustedProxyPeers(value)).toThrow();
    expect(
      readApiConfig({
        DATABASE_URL: 'synthetic',
        DATABASE_URL_OWNER: 'synthetic',
        JWT_SECRET: 'development',
        TRUSTED_PROXY_CIDRS: '127.0.0.1/32',
      }).trustedProxies,
    ).toEqual(['127.0.0.1/32']);
  });
  it('accepts the nearest forwarded address only from a configured proxy and ignores spoofed direct headers', async () => {
    ...
    try {
      ...
      expect((await app.inject({ url: '/', remoteAddress: '127.0.0.1', headers })).json()).toEqual({
        ip: '198.51.100.2',
      });
