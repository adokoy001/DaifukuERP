...
describe('migration prerequisite ordering', () => {
  it('creates added columns and tables before a unique key, then the foreign key referencing it', () => {
    ...
    const foreign = 'ALTER TABLE "memberships" ADD CONSTRAINT "fk" FOREIGN KEY ("tenant_id", "user_id") REFERENCES "users"("tenant_id", "id");';
    ...
    expect(orderMigrationConstraints([column, table, foreign, unique, policy])).toEqual([column, table, unique, foreign, policy]);
    expect(orderMigrationConstraints([column, table, unique, foreign, policy])).toEqual([column, table, unique, foreign, policy]);
  });
