...
defineAction({
  name: 'access_probe.report',
  description: label('権限試験', 'Access probe'),
  input: z.object({}),
  output: tableResult,
  permission: { entity: Partner.name, op: 'read' },
  exportEntities: [Partner.name],
  tx: 'none',
  handler: async (ctx) => ({
    title: label('取引先', 'Partners'),
    columns: [{ key: 'name', label: label('名称', 'Name'), kind: 'text' as const }],
    rows: (await repo(ctx, Partner).list()).items.map((p) => ({ name: p.name })),
  }),
});
const Store = defineEntity({
  name: 'access_probe_store',
  label: label('担当店', 'Assigned store'),
  fields: { name: f.text({ required: true }) },
  permissions: { roles: { chain_staff: ['read'], chain_manager: ['read', 'export'] } },
...
async function assign(userId: string, roles = ['admin'], expectedVersion = 0) {
  return call('PUT', `/admin/users/${userId}/companies/${db.companyId}`, {
    expectedVersion,
    roles,
    accessScope: 'all',
    storeIds: [],
...
beforeAll(async () => {
  db = await freshDb();
  app = await buildServer({ owner: db.owner, app: db.app, jwtSecret: 'access-test-secret' });
  await app.ready();
  adminToken = (await login('admin@example.com', 'password')).json<{ token: string }>().token;
});
afterAll(async () => {
  await app.close();
  await db.close();
...
describe('tenant administration and fresh company authorization', () => {
  it('creates users, assigns an initial company, and only exposes explicitly assigned companies', async () => {
    ...
    expect(
      (await call('PATCH', `/admin/users/${other.userId}`, { expectedVersion: 1, name: 'Wrong tenant' })).statusCode,
    ).toBe(404);
  ...
  it('company admin cannot manage users or self-elevate to tenant admin', async () => {
    ...
    expect(
      (await call('PATCH', `/admin/users/${user.id}`, { expectedVersion: 2, tenantAdmin: true }, token)).statusCode,
    ).toBe(403);
    expect((await call('GET', '/auth/me', undefined, token)).json()).toMatchObject({
      user: { roles: ['admin'], tenantAdmin: false },
    });
  });
  it('uses current memberships for report export and rejects stale membership versions', async () => {
    ...
    const meta = (await call('GET', '/meta', undefined, token)).json<{
      actions: { name: string; canExport: boolean }[];
    }>();
    expect(meta.actions.find((a) => a.name === 'access_probe.report')?.canExport).toBe(false);
    expect(
      (await call('DELETE', `/admin/users/${user.id}/companies/${db.companyId}`, { expectedVersion: 2 })).statusCode,
    ).toBe(200);
  ...
  it('disabling/reactivating or resetting a password cannot revive old tokens; audit has no secrets', async () => {
    const user = await create('session@example.com');
    ...
    expect(
      (await call('PATCH', `/admin/users/${user.id}`, { expectedVersion: version, active: false })).statusCode,
    ).toBe(200);
    expect((await call('GET', '/auth/me', undefined, token)).statusCode).toBe(401);
    version += 1;
    expect(
      (await call('PATCH', `/admin/users/${user.id}`, { expectedVersion: version, active: true })).statusCode,
    ).toBe(200);
    expect((await call('GET', '/auth/me', undefined, token)).statusCode).toBe(401);
    const currentToken = (await login('session@example.com')).json<{ token: string }>().token;
    version += 1;
    expect(
      (await call('PATCH', `/admin/users/${user.id}`, { expectedVersion: version, password: 'a-new-secret-password' }))
        .statusCode,
    ).toBe(200);
    ...
    expect(audit.body).not.toContain('scrypt');
    expect(audit.body).not.toContain('a-new-secret-password');
  ...
  it('validates store assignments and resolves the store boundary again on each HTTP call', async () => {
    ...
    expect(
      (await call('PUT', url, { expectedVersion: 0, roles: ['chain_staff'], accessScope: 'stores', storeIds: [] }))
        .statusCode,
    ).toBe(400);
    expect(
      (await call('PUT', url, { expectedVersion: 0, roles: ['admin'], accessScope: 'stores', storeIds: [storeA.id] }))
        .statusCode,
    ).toBe(400);
    expect(
      (
  ...
  it('does not honor legacy roles and prevents self access changes', async () => {
    ...
    expect((await call('GET', '/auth/me', undefined, token)).json()).toMatchObject({
      user: { roles: [], tenantAdmin: false },
    });
    const self = await current(db.adminUserId);
    expect(
      (await call('PATCH', `/admin/users/${self.id}`, { expectedVersion: self.version, tenantAdmin: false }))
        .statusCode,
    ).toBe(403);
    expect(
      (await call('PATCH', `/admin/users/${self.id}`, { expectedVersion: self.version, active: false })).statusCode,
    ).toBe(403);
