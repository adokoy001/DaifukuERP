...
const login = (password = 'password', email = 'admin@example.com', ip = '192.0.2.1') => app.inject({ method: 'POST', url: '/auth/login', payload: { email, password }, remoteAddress: ip });
beforeAll(async () => { db = await freshDb(); app = await buildServer({ owner: db.owner, app: db.app, jwtSecret: 'synthetic-rate-limit-jwt-secret', identity: { encryptionKey: Buffer.alloc(32, 9).toString('base64'), webUrl: 'https://erp.example.com', providers: [] } }); await app.ready(); });
...
describe('authentication budgets for shared networks', () => {
  it('keeps successful repeated login and authenticated/provider reads available behind one NAT', async () => {
    for (let index = 0; index < 15; index++) { const response = await login(); expect(response.statusCode, response.body).toBe(200); }
    const token = (await login()).json<{ token: string }>().token;
    for (let index = 0; index < 105; index++) {
      expect((await app.inject({ url: '/auth/me', remoteAddress: '192.0.2.1', headers: { authorization: `Bearer ${token}` } })).statusCode).toBe(200);
  ...
  it('retains failed login limits across IPs and does not charge another identity or read-only provider listing', async () => {
    for (let index = 0; index < 10; index++) expect((await login('wrong', 'absent@example.com', `192.0.2.${index + 10}`)).statusCode).toBe(401);
  ...
  it('limits password spraying per IP without disabling other networks or provider discovery', async () => {
    for (let index = 0; index < 100; index++) expect((await login('wrong', `spray-${index}@example.com`, '192.0.2.200')).statusCode).toBe(401);
  ...
  it('bounds administrator invitation issuance separately from successful authentication', async () => {
    ...
    for (let index = 0; index < 31; index++) {
      const step = await app.inject({ method: 'POST', url: '/auth/step-up', headers, payload: { currentPassword: 'password' } });
      expect(step.statusCode, step.body).toBe(200);
      const response = await app.inject({ method: 'POST', url: '/auth/invitations', headers, payload: { email: `invited-${index}@example.test`, name: 'Synthetic invitee', stepUpToken: step.json<{ stepUpToken: string }>().stepUpToken } });
  ...
  it('does not accumulate successful step-up proofs as failures', async () => {
    ...
    for (let index = 0; index < 12; index++) {
      const response = await app.inject({ method: 'POST', url: '/auth/step-up', headers: { authorization: `Bearer ${token}` }, payload: { currentPassword: 'password' } });
