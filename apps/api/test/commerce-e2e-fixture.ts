...
function fail(): never {
  throw new Error(
    'Commerce E2E requires an explicitly prepared loopback fixture database and matching synthetic administrator.',
  );
}
async function main() {
  const [tenantId, userId, runId] = process.argv.slice(2);
  if (
    process.env.NODE_ENV === 'production' ||
    process.env.E2E_PREPARE !== '1' ||
    !tenantId ||
    !userId ||
  ...
  for (const url of [ownerUrl, appUrl])
    if (
      ...
      fail();
  if (
    ownerUrl.hostname !== appUrl.hostname ||
    ownerUrl.port !== appUrl.port ||
    ownerUrl.pathname !== appUrl.pathname ||
    ownerUrl.username === appUrl.username ||
...
main().catch(() => {
  process.stderr.write(
    'Commerce fixture preparation refused or failed. No database reset was performed; do not publish connection settings.\n',
  );
  process.exitCode = 1;
});
