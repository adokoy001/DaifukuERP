...
const owner = connect(OWNER_URL, { max: 1 }), app = connect(APP_URL, { max: 1 }), previous = legacyMigrationFolder(11);
const tenant = newId(), company = newId(), user = newId(), site = newId(), employee = newId(), plan = newId();
const tables = ['identity_challenges', 'identity_factors', 'identity_links', 'identity_mail_outbox', 'workforce_payroll_rules', 'workforce_payroll_condition', 'workforce_payroll_tax_evidence', 'workforce_year_end_declaration', 'workforce_year_end_adjustment', 'workforce_work_system_period', 'pos_integration_location', 'pos_integration_inbox', 'pos_integration_transaction', 'group_accounting_run', 'franchise_agreement', 'franchise_settlement'];
type Sql = postgres.TransactionSql;
const scoped = <T>(db: Database, tenantId: string, fn: (tx: Sql) => Promise<T>) => db.sql.begin(async (tx) => { await tx`select set_config('app.tenant_id',${tenantId},true)`; return fn(tx); });
beforeAll(async () => {
...
const facts = () => scoped(owner, tenant, async (tx) => ({
  ...
  companies: await tx`select * from companies order by id`, employees: await tx`select * from workforce_employee order by id`,
  profiles: await tx`select * from workforce_shift_profile order by id`, plans: await tx`select * from workforce_shift_plan order by id`,
}));
it('preserves historical facts, initializes MFA off and adds only empty enterprise tables with RLS and scoped foreign keys', async () => {
  ...
  const [identity] = await scoped(app, tenant, (tx) => tx`select mfa_enabled,session_version from users where id=${user}`);
  ...
  for (const name of tables) {
    ...
    expect((await owner.sql`select relrowsecurity,relforcerowsecurity from pg_class where relname=${name}`)[0]).toMatchObject({ relrowsecurity: true, relforcerowsecurity: true });
  }
  await expect(app.sql`select * from identity_rate_limits`).rejects.toMatchObject({ code: '42501' });
  await scoped(app, tenant, (tx) => tx`insert into identity_factors(user_id,tenant_id,secret_cipher,recovery_hashes) values(${user},${tenant},'synthetic-encrypted-fixture','[]')`);
  expect((await scoped(app, newId(), (tx) => tx`select count(*)::int n from identity_factors`))[0]?.n).toBe(0);
  await expect(scoped(app, tenant, (tx) => tx`insert into identity_links(id,tenant_id,user_id,provider_id,issuer,subject) values(${newId()},${tenant},${newId()},'test','https://issuer.example.invalid','subject')`)).rejects.toMatchObject({ code: '23503' });
