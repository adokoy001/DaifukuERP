...
beforeEach(async () => { await dropAll(owner); });
afterAll(async () => { await app.close(); await owner.close(); rmSync(legacy, { recursive: true, force: true }); });
...
describe('foundation-refresh AC-3 populated schema upgrade', () => {
  it('preserves original data and only backfills facts supported by original documents', async () => {
    ...
    await runMigrations(owner, MIGRATIONS_DIR);
    const [invoice] = await owner.sql`select total, paid_amount, balance, control_account_id, issued_snapshot, settlement_history from sales_invoice where id=${ids.invoice}`;
    expect(invoice).toMatchObject({ total: '1000.000000', paid_amount: '100.000000', balance: '900.000000', control_account_id: ids.account, issued_snapshot: null, settlement_history: true });
    ...
    const settlements = await owner.sql`select date::text, amount from sales_settlement where invoice_id=${ids.invoice}`;
    expect(settlements).toEqual([{ date: '2026-04-10', amount: '100.000000' }]);
    const [unknown] = await owner.sql`select paid_amount, balance, settlement_history, issued_snapshot from sales_invoice where id=${ids.unknownInvoice}`;
    expect(unknown).toMatchObject({ paid_amount: '50.000000', balance: '950.000000', settlement_history: false, issued_snapshot: null });
    await withContext(app, systemParams(ids.tenant, ids.company), async (ctx) => {
      ...
      await expect(invoiceBalancesAsOf(ctx, [missingHistory], '2026-03-31' as LocalDate)).rejects.toMatchObject({ code: 'INVALID_STATE' });
    });
  ...
  it('does not relabel an existing non-JPY invoice or payment as yen', async () => {
    ...
    await expect(runMigrations(owner, MIGRATIONS_DIR)).rejects.toMatchObject({ cause: { message: expect.stringContaining('non-JPY financial records') } });
    const [count] = await owner.sql`select count(*)::int n from drizzle.__drizzle_migrations`;
    expect(count?.n).toBe(7);
    ...
    const [column] = await owner.sql`select count(*)::int n from information_schema.columns where table_schema='public' and table_name='sales_invoice' and column_name='currency'`;
  ...
  it('does not treat an unknown zero balance history as a verified empty history', async () => {
    ...
    await withContext(app, systemParams(ids.tenant, ids.company), async (ctx) => {
      ...
      await expect(invoiceBalancesAsOf(ctx, [row], '2026-04-30' as LocalDate)).rejects.toMatchObject({ code: 'INVALID_STATE' });
      await expect(applyPayment(ctx, { invoiceId: row.id, amount: '50', date: '2026-04-15' as LocalDate })).rejects.toMatchObject({ code: 'INVALID_STATE' });
    });
