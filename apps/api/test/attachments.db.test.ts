...
import { auditTrail, bootstrapTenant, companyMemberships, configureStorage, hashPassword, LocalStorage, newId, registerCrudActions, users } from '@daifuku/kernel';
...
async function upload(opts: UploadOpts = {}): Promise<JsonResponse> {
  ...
  const file: Part = { name: 'file', filename: opts.filename ?? `file-${seq}.pdf`, contentType: opts.contentType ?? 'application/pdf', data: typeof content === 'string' ? new TextEncoder().encode(content) : content };
  ...
  const t = opts.token === undefined ? token : opts.token;
  const res = await app.inject({ method: 'POST', url: '/api/attachments/upload', headers: { ...(t ? { authorization: `Bearer ${t}` } : {}), 'content-type': contentType, ...opts.headers }, payload });
  return { status: res.statusCode, body: res.json() as JsonResponse['body'] };
}

async function download(id: string, t: string | null = token): Promise<{ status: number; headers: Record<string, string | string[] | number | undefined>; body: Buffer; json: () => unknown }> {
  const res = await app.inject({ method: 'GET', url: `/api/attachments/${id}/download`, headers: t ? { authorization: `Bearer ${t}` } : {} });
...
async function addUser(email: string, password: string, roles: string[]): Promise<string> {
  const id = newId();
  await db.owner.drizzle.insert(users).values({ id, tenantId: db.tenantId, email, name: email, passwordHash: hashPassword(password), roles, defaultCompanyId: db.companyId });
  await db.owner.drizzle.insert(companyMemberships).values({ tenantId: db.tenantId, userId: id, companyId: db.companyId, roles });
...
describe('attachment entity through the API (AC-1)', () => {
  it('AC-1 the module is registered with its entity, actions and role table; no role has delete', async () => {
    ...
    const entity = (meta.body.entities as { name: string; ops: string[]; module: string }[]).find((e) => e.name === 'attachment');
    ...
    expect((viewerMeta.body.entities as { name: string; ops: string[] }[]).find((e) => e.name === 'attachment')?.ops).toEqual(['read']);
    const actions = (meta.body.actions as { name: string }[]).map((a) => a.name);
    expect(actions).toEqual(expect.arrayContaining(['attachment.search', 'attachment.supersede', 'attachment.link', 'attachment.for_record', 'attachment.list', 'attachment.get', 'attachment.create', 'attachment.update']));
    const fields = await call('GET', '/meta/entities/attachment');
    expect((fields.body.fields as { name: string }[]).map((f) => f.name)).toEqual(['storageKey', 'filename', 'contentType', 'size', 'sha256', 'kind', 'txnDate', 'amount', 'partnerId', 'linkedEntity', 'linkedId', 'note', 'supersededById']);
  });
...
describe('POST /api/attachments/upload (AC-2, AC-7)', () => {
  it('AC-2 stores the bytes, creates the row with the multipart fields and returns it (fields before or after the file)', async () => {
    const res = await upload({
      ...
      fields: { kind: 'receipt', txnDate: '2026-04-01', amount: '12345.50', partnerId, note: 'タクシー代', linkedEntity: 'partner', linkedId: partnerId },
      token: salesToken,
    });
    ...
    const first = await upload({ fields: { kind: 'contract', txnDate: '2026-05-01', amount: '100' }, fieldsFirst: true, contentType: 'text/csv; charset=utf-8', filename: 'ledger.csv' });
    expect(first.status).toBe(200);
    expect(first.body).toMatchObject({ kind: 'contract', txnDate: '2026-05-01', amount: '100', contentType: 'text/csv', filename: 'ledger.csv' });
  });
  ...
  it('AC-2 rejects disallowed types, empty/missing files, non-multipart bodies and bad fields with the standard error shape; stores nothing', async () => {
    ...
    expect(zip.body.error).toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'file.contentType' }] }, hint: expect.stringContaining('application/pdf') });
    ...
    const noFile = await app.inject({ method: 'POST', url: '/api/attachments/upload', headers: { authorization: `Bearer ${token}`, 'content-type': onlyFields.contentType }, payload: onlyFields.payload });
    expect(noFile.statusCode).toBe(400);
    expect(noFile.json()).toMatchObject({ error: { code: 'VALIDATION', details: { issues: [{ path: 'file' }] }, hint: expect.stringContaining('multipart') } });
  ...
  it('AC-2 a file over 20 MB is refused (413) by the multipart limit; exactly 20 MB is accepted', async () => {
    const tooBig = await upload({ content: new Uint8Array(20 * 1024 * 1024 + 1).fill(0x41), contentType: 'text/plain', filename: 'big.txt' });
    expect(tooBig.status).toBe(413);
    expect(tooBig.body.error).toMatchObject({ code: 'VALIDATION', message: expect.stringContaining('20 MB'), details: { maxBytes: 20 * 1024 * 1024 } });
    const limit = await upload({ content: new Uint8Array(20 * 1024 * 1024).fill(0x42), contentType: 'text/plain', filename: 'exact.txt' });
  ...
  it('AC-7 the same content twice in one company -> 409 CONFLICT with details.existingId; another company may store it', async () => {
    ...
    expect(first.status).toBe(200);
    const again = await upload({ content: 'duplicate evidence', filename: 'renamed.pdf', fields: { kind: 'receipt' }, token: salesToken });
    expect(again.status).toBe(409);
    expect(again.body.error).toMatchObject({ code: 'CONFLICT', details: { existingId: first.body.id, sha256: first.body.sha256 }, hint: expect.stringContaining('supersede') });
...
describe('GET /api/attachments/:id/download (AC-3)', () => {
  it('AC-3 streams the bytes with the original filename (RFC 5987) and content type; viewer may read', async () => {
    ...
    expect(res.headers['content-disposition']).toBe(`attachment; filename="____ (1).png"; filename*=UTF-8''%E3%82%B9%E3%82%AD%E3%83%A3%E3%83%B3%20%281%29.png`);
    ...
    expect(contentDisposition('quo"te\\.pdf')).toBe(`attachment; filename="quo_te_.pdf"; filename*=UTF-8''quo%22te%5C.pdf`);
  });
  ...
  it('AC-3 applies the attachment.get permission check: no read role -> 403, unknown/foreign ids -> 404, no token -> 401, bad id -> 400', async () => {
    ...
    await bootstrapTenant(db.owner, { tenantName: 'Other', companyCode: 'O1', companyName: 'Other Co', adminEmail: 'other@example.com', adminName: 'Other', adminPassword: 'other-pw' });
...
describe('actions through the API (AC-4, AC-5, AC-6, AC-8)', () => {
  it('AC-4 attachment.search combines date range, amount range, partner and kind; REST list/search still work', async () => {
    ...
    const a = await upload({ content: `${tag} a`, fields: { txnDate: '2026-06-10', amount: '5000', partnerId, kind: 'invoice_received', note: tag } });
    const b = await upload({ content: `${tag} b`, fields: { txnDate: '2026-06-20', amount: '7000', partnerId, kind: 'invoice_received', note: tag } });
    const c = await upload({ content: `${tag} c`, fields: { txnDate: '2026-07-01', amount: '7000', kind: 'receipt', note: tag } });
    for (const r of [a, b, c]) expect(r.status).toBe(200);
    const ids = (res: JsonResponse) => (res.body.items as { id: string }[]).map((i) => i.id);
    const june = await call('POST', '/actions/attachment.search', { body: { txnDateFrom: '2026-06-01', txnDateTo: '2026-06-30', partnerId }, token: viewerToken });
    ...
    const combo = await call('POST', '/actions/attachment.search', { body: { txnDateFrom: '2026-06-01', amountFrom: '6000', kind: 'invoice_received', partnerId } });
    expect(ids(combo)).toEqual([b.body.id]);
    expect((combo.body.items as { amount: string }[])[0]?.amount).toBe('7000');
    const invalid = await call('POST', '/actions/attachment.search', { body: { txnDateFrom: '2026-07-01', txnDateTo: '2026-06-01' } });
  ...
  it('AC-5/AC-8 supersede records the replacement and reason in the audit trail; the old file is still downloadable', async () => {
    const v1 = await upload({ content: 'invoice v1 (wrong amount)', fields: { kind: 'invoice_received', amount: '1000' } });
    const v2 = await upload({ content: 'invoice v2 (corrected)', fields: { kind: 'invoice_received', amount: '1100' } });
    const res = await call('POST', '/actions/attachment.supersede', { body: { id: v1.body.id, newAttachmentId: v2.body.id, reason: '金額誤り。訂正版を受領' }, token: salesToken });
    expect(res.status, JSON.stringify(res.body)).toBe(200);
    ...
    expect(entries.find((e) => e.op === 'supersede')).toMatchObject({ action: 'attachment.supersede', after: { supersededById: v2.body.id, reason: '金額誤り。訂正版を受領' } });
    ...
    const twice = await call('POST', '/actions/attachment.supersede', { body: { id: v1.body.id, newAttachmentId: v2.body.id, reason: 'again' } });
    expect(twice.status).toBe(409);
    expect(twice.body.error?.code).toBe('INVALID_STATE');
    const noReason = await call('POST', '/actions/attachment.supersede', { body: { id: v2.body.id, newAttachmentId: v1.body.id } });
    expect(noReason.status).toBe(400);
    expect(noReason.body.error?.details).toMatchObject({ issues: [{ path: 'reason' }] });
    const viewer = await call('POST', '/actions/attachment.supersede', { body: { id: v2.body.id, newAttachmentId: v1.body.id, reason: 'x' }, token: viewerToken });
    expect(viewer.status).toBe(403);
  });
  ...
  it('AC-6 link verifies the entity and record visibility; for_record lists a record’s attachments newest first', async () => {
    ...
    const linked = await call('POST', '/actions/attachment.link', { body: { id: a.body.id, entity: 'partner', recordId: partnerId }, token: salesToken });
    expect(linked.status).toBe(200);
    expect(linked.body).toMatchObject({ linkedEntity: 'partner', linkedId: partnerId });
    expect((await call('POST', '/actions/attachment.link', { body: { id: b.body.id, entity: 'partner', recordId: partnerId } })).status).toBe(200);
    const list = await call('POST', '/actions/attachment.for_record', { body: { entity: 'partner', recordId: partnerId }, token: viewerToken });
    ...
    const unknownEntity = await call('POST', '/actions/attachment.link', { body: { id: a.body.id, entity: 'nope', recordId: partnerId } });
    expect(unknownEntity.status).toBe(400);
    expect(unknownEntity.body.error?.details).toMatchObject({ issues: [{ path: 'entity' }] });
    const ghost = await call('POST', '/actions/attachment.link', { body: { id: a.body.id, entity: 'partner', recordId: newId() } });
    expect(ghost.status).toBe(404);
    const viewer = await call('POST', '/actions/attachment.link', { body: { id: a.body.id, entity: 'partner', recordId: partnerId }, token: viewerToken });
    expect(viewer.status).toBe(403);
    // the REST PATCH path is guarded by the same hook
    const patch = await call('PATCH', `/api/attachment/${a.body.id as string}`, { body: { linkedEntity: 'partner', linkedId: newId() } });
  ...
  it('OpenAPI documents the upload and download routes', async () => {
    ...
    expect(doc.paths['/api/attachments/upload']?.post).toMatchObject({ tags: ['attachment'], summary: expect.stringContaining('multipart') });
    expect(doc.paths['/api/attachments/{id}/download']?.get).toMatchObject({ tags: ['attachment'] });
    for (const name of ['attachment.search', 'attachment.supersede', 'attachment.link', 'attachment.for_record']) expect(doc.paths[`/actions/${name}`]?.post).toBeDefined();
  });
});
