...
let phase = 'prepare';
const step = (name: string) => { phase = name; process.stdout.write(`setup acceptance: ${name}\n`); };
const replaceDatabase = (url: string, database: string) => { const value = new URL(url); value.pathname = `/${database}`; return value.toString(); };
async function useDb<T>(url: string, fn: (db: Database) => Promise<T>): Promise<T> { const db = connect(url, { max: 1 }); try { return await fn(db); } finally { await db.close(); } }

async function sourceFolders(root: string) {
  const journal = readJournal(), legacy = join(root, 'legacy-0008'), synthetic = join(root, 'synthetic');
  ...
  for (const entry of journal.entries) {
    ...
    await copyFile(join(MIGRATIONS_DIR, 'meta', `${String(entry.idx).padStart(4, '0')}_snapshot.json`), join(synthetic, 'meta', `${String(entry.idx).padStart(4, '0')}_snapshot.json`));
    if (entry.idx <= 8) await copyFile(join(MIGRATIONS_DIR, `${entry.tag}.sql`), join(legacy, `${entry.tag}.sql`));
  }
  await writeFile(join(legacy, 'meta', '_journal.json'), JSON.stringify({ ...journal, entries: journal.entries.filter((entry) => entry.idx <= 8) }));
  const last = journal.entries.at(-1); assert(last && last.idx >= 9, '0009 release migration is required');
  const next = { ...last, idx: last.idx + 1, when: last.when + 1, tag: `${String(last.idx + 1).padStart(4, '0')}_setup_acceptance` };
  await writeFile(join(synthetic, 'meta', '_journal.json'), JSON.stringify({ ...journal, entries: [...journal.entries, next] }));
  await copyFile(join(synthetic, 'meta', `${String(last.idx).padStart(4, '0')}_snapshot.json`), join(synthetic, 'meta', `${String(next.idx).padStart(4, '0')}_snapshot.json`));
...
async function populateLegacy(url: string, folder: string) {
  const tenant = newId(), company = newId(), user = newId(), password = 'Original-LegacyCredential9!', hash = hashPassword(password);
...
async function interruptInstallation(options: SetupOptions, envFile: string) {
  await assert.rejects(runSetup(options, { checkpoint: async (name) => { if (name === 'migrated') throw new SetupError('INJECTED', 'acceptance failure'); } }), { code: 'INJECTED' });
  const state = await readState(options.stateDir); assert(state?.identity);
  ...
  await assert.rejects(runSetup({ ...options, envFile }, { checkpoint: async (name) => { if (name === 'bootstrap-committed') throw new SetupError('INJECTED', 'acceptance failure'); } }), { code: 'INJECTED' });
}
async function verifyCliPlan(options: SetupOptions) {
  const { stdout, stderr } = await promisify(execFile)('pnpm', ['run', 'setup', 'install', '--env', options.envFile, '--state-dir', options.stateDir]);
  const source = parseEnv(await privateFile(options.envFile));
  for (const value of [source.DATABASE_URL_OWNER, source.DATABASE_URL, source.RESTORE_CHECK_URL, source.JWT_SECRET]) if (value) {
    assert(!(stdout + stderr).includes(value));
    if (value.startsWith('postgres:')) assert(!(stdout + stderr).includes(decodeURIComponent(new URL(value).password)));
  }
...
async function main(): Promise<void> {
  ...
  if (!input) throw new SetupError('TEST_ENV_REQUIRED', 'SETUP_TEST_ENV_FILEで専用保護envを明示してください。接続先のfallbackはありません。');
  const originalEnv = await privateFile(input), env = parseEnv(originalEnv);
  assert(env.DATABASE_URL_OWNER && env.DATABASE_URL && env.RESTORE_CHECK_URL);
  const ownerUrl = env.DATABASE_URL_OWNER, appUrl = env.DATABASE_URL;
  ...
  await useDb(ownerUrl, async (db) => assert.equal((await relationNames(db)).length, 0, 'explicit test DB must be new'));
  const artifact = await mkdtemp(join(dirname(input), 'acceptance-'));
  const base: SetupOptions = { mode: 'install', envFile: input, stateDir: join(artifact, 'install'), execute: false, maintenanceConfirmed: true, allowRemote: false, generateAdminPassword: true, tenantName: 'Setup acceptance', companyCode: 'SAFE', companyName: '安全導入検証', adminEmail: 'setup-admin@example.test', adminName: 'Setup Admin' };
  const changeEnv = async (number: number, source = ownerUrl, app = appUrl, jwt?: string) => {
    const file = join(artifact, `input-${number}.env`);
    await writeExclusive(file, `DATABASE_URL_OWNER=${source}\nDATABASE_URL=${app}\nRESTORE_CHECK_URL=${replaceDatabase(ownerUrl, `daifuku_setup_restore_${number}${suffix}`)}\n${jwt ? `JWT_SECRET=${jwt}\n` : ''}`);
    return file;
  };
  step('AC-1 plan has no writes');
  await verifyCliPlan(base);
  const plan = await runSetup(base); assert.equal(plan.empty, true); assert.equal(await exists(base.stateDir), false);
  ...
  await useDb(ownerUrl, async (busy) => { await busy.sql`select 1`; await assert.rejects(runSetup(first), { code: 'DATABASE_BUSY' }); });
  ...
  const partial = await readState(base.stateDir); assert(partial?.identity);
  const firstTenantId = partial.identity.tenantId;
  const runtimePath = join(base.stateDir, 'runtime.env'), runtime = await privateFile(runtimePath);
  const generatedPassword = (await privateFile(join(base.stateDir, 'initial-admin-password.txt'))).trimEnd();
  const hash = await useDb(ownerUrl, async (db) => (await db.sql`select password_hash from users where tenant_id=${firstTenantId}`)[0]?.password_hash);
  const changedPassword = 'Changed-SetupCredential9!';
  const passwordFile = join(artifact, 'different-password'); await writeExclusive(passwordFile, changedPassword);
  const second = { ...first, envFile: await changeEnv(2), generateAdminPassword: false, adminPasswordFile: passwordFile };
  step('AC-6 resume preserves committed password and runtime config');
  await runSetup(second);
  assert.equal(await privateFile(runtimePath), runtime); assert.equal(await privateFile(input), originalEnv);
  await useDb(ownerUrl, async (db) => {
    assert.equal((await db.sql`select password_hash from users where tenant_id=${firstTenantId}`)[0]?.password_hash, hash);
  ...
  const folders = await sourceFolders(artifact), legacyUrl = replaceDatabase(ownerUrl, `daifuku_setup_legacy${suffix}`), legacyApp = replaceDatabase(appUrl, `daifuku_setup_legacy${suffix}`);
  step('AC-3 real populated 0008 fixture');
  const { tenant, company, user, password: legacyPassword, hash: legacyHash } = await populateLegacy(legacyUrl, folders.legacy);
  const jwt = parseEnv(runtime).JWT_SECRET; assert(jwt);
  const upgrade: SetupOptions = { ...base, mode: 'upgrade', stateDir: join(artifact, 'upgrade'), envFile: await changeEnv(3, legacyUrl, legacyApp, jwt), generateAdminPassword: false, execute: true };
  const updatePlan = await buildPlan(upgrade); upgrade.confirmTarget = updatePlan.target.id; assert.equal(updatePlan.applied, 9); assert(updatePlan.pending.includes('0009_operations_control'));
  await useDb(legacyUrl, async (db) => { await db.sql`alter table companies alter column name drop not null`; });
  await assert.rejects(buildPlan(upgrade), { code: 'SCHEMA_DRIFT' });
  await useDb(legacyUrl, async (db) => { await db.sql`alter table companies alter column name set not null`; });
  ...
  await useDb(legacyUrl, async (db) => {
    ...
    assert.deepEqual((await db.sql`select settings from companies where id=${company}`)[0]?.settings, { custom: 'keep', 'tax.price_includes_tax': true });
  ...
  const occupied = { ...base, stateDir: join(artifact, 'occupied'), mode: 'upgrade' as const, envFile: await changeEnv(2 + 6, ownerUrl, appUrl, jwt), execute: true, confirmTarget: plan.target.id };
  const usedRestoreEnv = parseEnv(await privateFile(second.envFile));
  await writeFile(occupied.envFile, `DATABASE_URL_OWNER=${ownerUrl}\nDATABASE_URL=${appUrl}\nRESTORE_CHECK_URL=${usedRestoreEnv.RESTORE_CHECK_URL}\nJWT_SECRET=${jwt}\n`, { mode: 0o600 });
...
main().catch((error: unknown) => { process.stderr.write(`FAIL at ${phase}: ${safeFailure(error)}\n`); process.exitCode = 1; });
