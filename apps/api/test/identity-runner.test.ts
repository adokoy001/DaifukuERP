...
function environment(cli: string) {
  return {
    ...
    TEST_DATABASE_URL_OWNER: 'postgres://owner:synthetic@127.0.0.1/daifuku_e2e_test_enterprise',
    TEST_DATABASE_URL: 'postgres://daifuku_app:synthetic@127.0.0.1/daifuku_e2e_test_enterprise',
    E2E_IDENTITY_BASE_URL: 'http://localhost:5189',
  };
}
...
describe('identity fixture runner isolation', () => {
  it('rejects an ambient external browser URL before launching any service', async () => {
    const child = spawn(process.execPath, [runner], {
      env: { ...environment('/does-not-run'), E2E_IDENTITY_BASE_URL: 'https://outside.example.test' },
    ...
    expect(error).toContain('fixed local fixture origin');
  ...
  for (const signal of ['SIGINT', 'SIGTERM'] as const)
    it(`stops and awaits its detached service descendants on ${signal}`, async () => {
      ...
      child.stdout.on('data', (chunk: Buffer) => {
        ...
        if (output.includes('fixture-listening:5189:')) signalReady();
      });
      try {
        await Promise.race([
          ready,
        ...
        child.kill(signal);
        expect((await finished)[0]).toBe(signal === 'SIGINT' ? 130 : 143);
        await available(3109);
        await available(5189);
      } finally {
        child.kill('SIGTERM');
        for (const pid of serviceIds)
          try {
