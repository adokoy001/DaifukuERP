...
import { analyticsMeasureValue, analyticsScopeKey, collectCompleteRows, snapshotInput } from '../src/analytics/snapshot.ts';

const context = (params: Partial<ContextParams> = {}) => makeContext({} as never, { tenantId: 'tenant', companyId: 'company', actor: { type: 'user', id: 'user' }, roles: ['admin'], appliedPacks: ['restaurant_chain'], ...params });
...
describe('browser analytics source contracts', () => {
  it('exposes only existing allowlisted, readable source fields and permission-specific catalogs', () => {
    ...
    for (const source of SOURCES) {
      ...
      for (const key of [...source.dimensions, ...source.measures]) expect(key in entity.columns || source.derived?.[key]).toBeTruthy();
    }
    expect(analyticsCatalog(context({ roles: ['nobody'] }))).toEqual([]);
    expect(analyticsCatalog(context({ companyId: null }))).toEqual([]);
    const employee = analyticsCatalog(context({ roles: ['workforce_employee'], accessScope: 'sites', siteIds: ['site'] }));
    ...
    expect(admin.find((item) => item.id === 'workforce_payroll')?.measures.map((item) => item.key)).not.toContain('calculation');
  });
  ...
  it('omits masked measures and rejects a source whose required filter would reveal a hidden field', () => {
    ...
    try {
      Object.assign(permissions, { fieldGroups: { sensitive: { fields: ['total', 'subtotal'], roles: ['accounting'] } } });
  ...
  it('does not infer masked bank direction through a signed derived amount', () => {
    ...
    const previous = permissions.fieldGroups;
    try {
      Object.assign(permissions, { fieldGroups: { direction: { fields: ['direction'], roles: ['bank_direction_reviewer'] } } });
  ...
  it('invalidates scope on role, site, actor and company changes, independently of array order', () => {
    ...
    for (const params of [{ roles: ['sales'] }, { siteIds: ['a'] }, { actor: { type: 'user' as const, id: 'other' } }, { companyId: 'other' }]) expect(analyticsScopeKey(context(params))).not.toBe(key);
  });
  ...
  it('signs bank withdrawals exactly, preserves missing values and bounds minute conversion precision', () => {
    ...
    expect(analyticsMeasureValue(bank, 'signedAmount', { amount: '99999999999999.123456', direction: 'pay' })).toBe('-99999999999999.123456');
...
describe('complete bounded analytics snapshots', () => {
  it('returns all pages including exactly the limit and supports empty sources', async () => {
    ...
    expect(await collectCompleteRows(async (offset) => ({ items: pages.slice(offset, offset + 2), total: 3 }), 3)).toEqual(pages);
  ...
  it('rejects over-limit sources and incomplete or changing page totals instead of returning partial aggregates', async () => {
    await expect(collectCompleteRows(async () => ({ items: [{ id: '1' }], total: 50_001 }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(collectCompleteRows(async () => ({ items: [], total: 2 }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(collectCompleteRows(async (offset) => ({ items: [{ id: String(offset) }], total: offset ? 3 : 2 }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
  });
});
