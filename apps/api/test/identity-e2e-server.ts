...
async function main() {
  ...
  const api = await buildServer({
    owner: db.owner,
    app: db.app,
    jwtSecret: 'synthetic-enterprise-identity-test-secret',
    corsOrigins: [webUrl],
    identity: {
      encryptionKey: key,
      webUrl,
  ...
  const timer = setInterval(() => {
    ...
    delivering = true;
    void deliverIdentityMail(db.owner, key, transport)
      .catch(() => process.stderr.write('Synthetic mail drain failed.\n'))
      .finally(() => {
        delivering = false;
      });
  }, 500);
  ...
  process.stdout.write(
    'Synthetic identity API/OIDC/SMTP ready on API3109 and IdP3110. No external recipients are accepted.\n',
  );
...
void main().catch(() => {
  process.stderr.write(
    'Identity fixture failed. Use new empty dedicated databases, safe roles and unused loopback ports.\n',
  );
  process.exit(1);
});
