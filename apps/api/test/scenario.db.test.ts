...
import { Decimal, DOCSTATUS, configureStorage, getCompany, LocalStorage, repo, runAction, setSetting, systemParams, withContext, type Context, type ContextParams } from '@daifuku/kernel';
...
type DocJson = Row & { id: string; number: string | null; docstatus: number; status: string; journalEntryId: string | null };
type InvoiceJson = DocJson & { partnerId: string; date: string; dueDate: string | null; priceIncludesTax: boolean; subtotal: string; taxTotal: string; total: string; paidAmount: string; balance: string };
type BillJson = InvoiceJson & { deductibleTax: string; nonDeductibleTax: string; creditRatio: string; supplierTaxStatus: string };
...
const run = <T>(fn: (ctx: Context) => Promise<T>, params: Partial<ContextParams> = {}): Promise<T> => db.run({ now: () => FIXED_NOW, ...params }, fn);
const act = <T = Row>(name: string, input: unknown): Promise<T> => run((ctx) => runAction(ctx, name, input)) as Promise<T>;
const sum = (values: readonly string[]): string => Decimal.sum(values.map((v) => Decimal.from(v))).toString();

async function call(method: 'GET' | 'POST', url: string, opts: { body?: unknown; token?: string | null } = {}): Promise<JsonResponse> {
...
async function entryPartners(entryId: string | null): Promise<Record<string, string | null>> {
  ...
  return run(async (ctx) => {
    const lines = await repo(ctx, JournalLine).list({ where: { entryId }, orderBy: [{ field: 'seq', dir: 'asc' }], limit: 100 });
    return Object.fromEntries(lines.items.filter((l) => l.partnerId !== null).map((l) => [codeOf.get(l.accountId) ?? l.accountId, l.partnerId]));
  });
}
/** Σ over lines of one account code within the entry. */
const sumOf = (lines: LineTriple[], code: string, side: 1 | 2): string => sum(lines.filter((l) => l[0] === code).map((l) => l[side]));
...
describe('台本: 個人事業（デザイン事務所）の 2026年10月 (docs/domain/scenario-kojin.md)', () => {
  it('1. 会計年度 2026（accounting.open_fiscal_year）・モジュール登録・全シード（system context, モジュール順）・会社設定（tax.rounding は l10n/jp 既定, sales.issuer）', async () => {
    ...
    expect(modules.map((m) => m.name)).toEqual(['partner', 'product', 'tax', 'accounting', 'attachment', 'sales', 'purchase', 'payment', 'inventory', 'contract', 'workforce', 'workforce_evidence', 'industry_operations', 'pos_integration', 'group_accounting', 'franchise', 'edge', 'trade', 'banking', 'tax_filing', 'l10n_jp']);
    ...
    for (const m of modules) {
      if (!m.seed) continue;
      await withContext(db.owner, systemParams(db.tenantId, db.companyId, { now: () => FIXED_NOW }), async (ctx) => m.seed?.(ctx));
      seeded.push(m.name);
    }
    ...
    for (const code of ['1100', '1300', '1500', '2100', '2200', '3000', '4000', '6200', '6960', '6970', '6980']) expect(acc[code], `account ${code} seeded`).toBeDefined();
    ...
    expect(await act('tax.get_settings', {})).toEqual({ rounding: { mode: 'down', unit: 'invoice' }, priceIncludesTax: false });
    ...
    expect(years.items[0]).toMatchObject({ code: 'FY2026', startDate: '2026-01-01', endDate: '2026-12-31', isClosed: false });
    const periods = await act<ListJson>('fiscal_period.list', { limit: 100, orderBy: [{ field: 'startDate', dir: 'asc' }] });
  ...
  it('2. 取引先 C1..S3（C2: 20日締め翌月10日払い、S2: 免税事業者）と品目 P1..P4（P4: 軽減税率 8%）', async () => {
    const partners: Row[] = [
      ...
      { code: 'C2', name: 'ベータ商店', isCustomer: true, taxStatus: 'registered', closingDay: 20, paymentMonthOffset: 1, paymentDay: 10 },
    ...
    // defaults: 月末締め翌月末払い
    expect(await act('partner.get', { id: partner.C1 })).toMatchObject({ closingDay: 31, paymentMonthOffset: 1, paymentDay: 31, taxStatus: 'registered' });
    expect(await act('partner.get', { id: partner.S2 })).toMatchObject({ taxStatus: 'exempt' });
    // ひっかけ 2: C2 の期日計算 (10-20 → 締め 10-20 → 翌月 10 日)
    expect(await act('partner.compute_due_date', { partnerId: partner.C2, invoiceDate: '2026-10-20' })).toEqual({ partnerId: partner.C2, invoiceDate: '2026-10-20', closingDate: '2026-10-20', dueDate: '2026-11-10' });
  ...
  it('3. 期首 JE0: 元入金 300,000 → Dr 1100 普通預金 / Cr 3000 元入金（journal_entry 2 行、submit）', async () => {
    const je0 = await createSubmit<DocJson & { date: string; totalDebit: string; totalCredit: string }>('journal_entry', {
      date: '2026-10-01',
      description: 'JE0 元入金',
      lines: { journal_line: [{ accountId: acc['1100'], debit: '300000' }, { accountId: acc['3000'], credit: '300000' }] },
    });
    expect(je0).toMatchObject({ docstatus: DOCSTATUS.submitted, number: 'JE-2026-000001', date: '2026-10-01', totalDebit: '300000', totalCredit: '300000' });
  ...
  it('4. 売上請求書 INV1..INV4（税抜入力、10% / INV4 は軽減 8% で税率ごとに 1 回丸め）: 金額・期日・転記 Dr 1300 / Cr 4000 / Cr 2200 — INV1 は REST 経由', async () => {
    ...
    const login = await call('POST', '/auth/login', { body: { email: 'admin@example.com', password: 'password' }, token: null });
    expect(login.status).toBe(200);
    token = login.body.token as string;
    const created = await call('POST', '/api/sales_invoice', {
      body: { partnerId: partner.C1, date: '2026-10-05', lines: { sales_invoice_line: [{ productId: product.P1, quantity: '1' }, { productId: product.P2, quantity: '1' }] } },
    });
    expect(created.status, JSON.stringify(created.body.error)).toBe(200);
    expect(created.body).toMatchObject({ docstatus: DOCSTATUS.draft, status: 'draft', priceIncludesTax: false, subtotal: '150000', taxTotal: '15000', total: '165000', dueDate: '2026-11-30' });
    ...
    expect(submitted.body).toMatchObject({ docstatus: DOCSTATUS.submitted, status: 'open', number: 'INV-2026-000001', balance: '165000', journalEntryId: expect.any(String) });
    ...
    inv.INV2 = await createSubmit<InvoiceJson>('sales_invoice', { partnerId: partner.C2, date: '2026-10-20', lines: { sales_invoice_line: [{ productId: product.P3, quantity: '10' }] } });
    inv.INV3 = await createSubmit<InvoiceJson>('sales_invoice', { partnerId: partner.C3, date: '2026-10-28', lines: { sales_invoice_line: [{ productId: product.P2, quantity: '1' }, { productId: product.P3, quantity: '3' }] } });
    inv.INV4 = await createSubmit<InvoiceJson>('sales_invoice', {
      partnerId: partner.C1,
      date: '2026-10-30',
      lines: { sales_invoice_line: ['菓子A', '菓子B', '菓子C'].map((description) => ({ productId: product.P4, quantity: '1', description })) },
    });
    ...
    for (const [no, p, date, subtotal, taxTotal, total, dueDate] of expected) {
      ...
      if (!i) continue;
      expect(i).toMatchObject({ docstatus: DOCSTATUS.submitted, status: 'open', partnerId: partner[p], date, priceIncludesTax: false, paidAmount: '0' });
      ...
      expect(new Set(lines.map((l) => l[0])), `${no} posts to 1300 / 4000 / 2200 only`).toEqual(new Set(['1300', '4000', '2200']));
      expect((await entryPartners(i.journalEntryId))['1300'], `${no} 売掛金 line carries the customer`).toBe(partner[p]);
    }
    ...
    expect(inv.INV4?.taxSummary).toMatchObject([{ category: 'reduced', rate: '0.08', taxable: '3333', tax: '266', gross: '3599', lineCount: 3 }]);
    ...
    const perLineRounded = sum(inv4Lines.map((l) => Decimal.from(String(l.amount)).times(Decimal.from('0.08')).round('down', 0).toString()));
  ...
  it('5. 仕入・経費請求書 BILL1..BILL4（税込入力）: 税抜・税額・控除率（S2 は l10n/jp の 70%）・控除対象外・転記', async () => {
    bill.BILL1 = await createSubmit<BillJson>('purchase_invoice', { partnerId: partner.S1, date: '2026-10-08', lines: { purchase_invoice_line: [{ accountId: acc['6960'], description: '名刺印刷', unitPrice: '22000' }] } });
    bill.BILL2 = await createSubmit<BillJson>('purchase_invoice', { partnerId: partner.S2, date: '2026-10-15', lines: { purchase_invoice_line: [{ accountId: acc['6980'], description: '撮影', unitPrice: '55000' }] } });
    bill.BILL3 = await createSubmit<BillJson>('purchase_invoice', { partnerId: partner.S3, date: '2026-10-25', lines: { purchase_invoice_line: [{ accountId: acc['6200'], description: '10月分家賃', unitPrice: '80000' }] } });
    bill.BILL4 = await createSubmit<BillJson>('purchase_invoice', { partnerId: partner.S1, date: '2026-10-30', lines: { purchase_invoice_line: [{ accountId: acc['6970'], description: '会議用飲料', unitPrice: '5400', taxCategory: 'reduced' }] } });
    ...
    for (const [no, p, date, category, total, subtotal, taxTotal, ratio, deductible, nonDeductible] of expected) {
      ...
      if (!b) continue;
      expect(b).toMatchObject({ docstatus: DOCSTATUS.submitted, status: 'open', partnerId: partner[p], date, priceIncludesTax: true, paidAmount: '0' });
      expect(b.number).toMatch(/^BILL-2026-\d{6}$/);
      const lines = (b.lines as { purchase_invoice_line: Row[] }).purchase_invoice_line;
      expect(lines.map((l) => l.taxCategory), `${no} 税区分（科目既定 or 明示）`).toEqual([category]);
      ...
      expect((await entryPartners(b.journalEntryId))['2100'], `${no} 買掛金 line carries the supplier`).toBe(partner[p]);
    }
    ...
    const bill2Memos = await run(async (ctx) => (await repo(ctx, JournalLine).list({ where: { entryId: bill.BILL2?.journalEntryId ?? '' }, orderBy: [{ field: 'seq', dir: 'asc' }], limit: 10 })).items.map((l) => l.memo));
    ...
    expect(await entryLines(bill.BILL3?.journalEntryId ?? null), 'ひっかけ 4: 非課税の家賃は仮払消費税を生まない').toEqual([
      ['6200', '80000', '0'],
      ['2100', '0', '80000'],
    ]);
  ...
  it('6. 入出金 PAY1..PAY3（2026-10-31、普通預金 1100）: 消込で INV2 / BILL1 / BILL3 が paid', async () => {
    const mk = (direction: 'receive' | 'pay', p: string, amount: string, entity: 'sales_invoice' | 'purchase_invoice', invoiceId: string) =>
      createSubmit<PaymentJson>('payment', { direction, partnerId: partner[p], date: '2026-10-31', amount, method: 'bank_transfer', lines: { payment_allocation: [{ invoiceEntity: entity, invoiceId, amount }] } });
    ...
    for (const [no, amount] of [
      ...
      if (!p) continue;
      expect(p).toMatchObject({ docstatus: DOCSTATUS.submitted, accountId: acc['1100'], amount, allocatedAmount: amount, unallocatedAmount: '0' });
    ...
    // 消込結果
    const status = async (entity: string, id: string | undefined) => act<InvoiceJson>(`${entity}.get`, { id: id ?? '' });
    expect(await status('sales_invoice', inv.INV2?.id)).toMatchObject({ status: 'paid', paidAmount: '33000', balance: '0' });
    expect(await status('purchase_invoice', bill.BILL1?.id)).toMatchObject({ status: 'paid', paidAmount: '22000', balance: '0' });
    expect(await status('purchase_invoice', bill.BILL3?.id)).toMatchObject({ status: 'paid', paidAmount: '80000', balance: '0' });
  ...
  it('7. 期待値（2026-10-31）: 試算表 10/01〜10/31 の全行と合計 863,899、売掛金 233,499・買掛金 60,400・普通預金 231,000、損益 85,833', async () => {
    ...
    for (const r of tb.rows) {
      ...
      expect.soft([r.periodDebit, r.periodCredit, r.closingBalance], `${String(r.code)} ${String(r.name)} has no movement`).toEqual(['0', '0', '0']);
    }
    ...
    const viaHttp = await call('POST', '/actions/accounting.trial_balance', { body: { from: '2026-10-01', to: '2026-10-31' } });
    ...
    const openInvoices = await act<ListJson>('sales_invoice.list', { where: { docstatus: DOCSTATUS.submitted, status: 'open' }, orderBy: [{ field: 'date', dir: 'asc' }] });
    ...
    const openBills = await act<ListJson>('purchase_invoice.list', { where: { docstatus: DOCSTATUS.submitted, status: 'open' }, orderBy: [{ field: 'date', dir: 'asc' }] });
    ...
    const bank = await act<Table>('accounting.general_ledger', { accountId: acc['1100'], from: '2026-10-01', to: '2026-10-31' });
    ...
    const revenue = sum(tb.rows.filter((r) => r.type === 'revenue').map((r) => Decimal.from(String(r.closingBalance)).neg().toString()));
  ...
  it('8. 消費税の集計（10月）: 売上税額 24,166（課税売上 239,000 @10% → 23,900、3,333 @8% → 266）、仕入控除税額 5,900、差引 18,266', async () => {
    ...
    const october = { posted: true, $and: [{ entryDate: { $gte: '2026-10-01' } }, { entryDate: { $lte: '2026-10-31' } }] };
    const byRate = (accountCode: string) =>
      run(async (ctx) => {
        const rows = await repo(ctx, JournalLine).aggregate({ where: { ...october, accountId: acc[accountCode] }, groupBy: ['taxCategory', 'taxRate'], metrics: { debit: { sum: 'debit' }, credit: { sum: 'credit' } } });
        return rows
          .map((r) => ({ category: r.taxCategory as string | null, rate: r.taxRate === null ? null : Decimal.from(String(r.taxRate)).toString(), debit: (r.debit as Decimal).toString(), credit: (r.credit as Decimal).toString() }))
    ...
    expect.soft(Decimal.from(outputTotal).minus(Decimal.from(sum(inputTax.map((g) => g.debit)))).toString(), '差引（納付見込）24,166 − 5,900').toBe('18266');
    ...
    const taxableSales = await run(async (ctx) => {
      const rows = await repo(ctx, JournalLine).aggregate({ where: { ...october, accountId: acc['4000'] }, groupBy: ['taxCategory', 'taxRate'], metrics: { credit: { sum: 'credit' } } });
      return rows.map((r) => [String(r.taxCategory), Decimal.from(String(r.taxRate)).toString(), (r.credit as Decimal).toString()]).sort((a, b) => String(a[1]).localeCompare(String(b[1])));
    });
    expect(taxableSales).toEqual([
      ['reduced', '0.08', '3333'],
      ['standard', '0.1', '239000'],
    ]);
    const nonTaxable = await run(async (ctx) => repo(ctx, JournalLine).aggregate({ where: { ...october, taxCategory: 'non_taxable' }, groupBy: ['accountId'], metrics: { debit: { sum: 'debit' }, credit: { sum: 'credit' } } }));
    expect(nonTaxable.map((r) => [codeOf.get(String(r.accountId)), (r.debit as Decimal).toString()])).toEqual([['6200', '80000']]);
  });
  ...
  it('9. 売掛金年齢表（基準日 2026-12-05）: C1 168,599（INV1 165,000 + INV4 3,599）と C3 64,900 が 1〜30日、計 233,499', async () => {
    ...
    expect.soft(byPartner.get(partner.C1 ?? ''), 'C1 株式会社アルファ').toMatchObject({ partnerName: '株式会社アルファ', notDue: '0', days1to30: '168599', days31to60: '0', days61to90: '0', over90: '0', total: '168599' });
    expect.soft(byPartner.get(partner.C3 ?? ''), 'C3 ガンマ（個人）').toMatchObject({ partnerName: 'ガンマ（個人）', notDue: '0', days1to30: '64900', days31to60: '0', days61to90: '0', over90: '0', total: '64900' });
    expect.soft(aging.totals, '年齢表 合計').toEqual({ notDue: '0', days1to30: '233499', days31to60: '0', days61to90: '0', over90: '0', total: '233499' });
  ...
  it('10. 適格請求書（INV3）: 登録番号 T1234567890123、10% 対象 59,000 / 消費税 5,900 / 税込 64,900、宛名、支払期限 2026-11-30（令和8年11月30日）', async () => {
    ...
    expect(html.startsWith('<!DOCTYPE html>'), 'l10n/jp layout is active (registry override sales.invoice_html)').toBe(true);
    ...
    expect(html).toContain('<th scope="row">10%対象</th><td class="num">¥59,000</td><td class="num">¥5,900</td><td class="num">¥64,900</td>');
    expect(html).toContain('<th scope="row">合計</th><td class="num">¥59,000</td><td class="num">¥5,900</td><td class="num">¥64,900</td>');
    expect(html).toContain('ご請求金額（税込）</span><span class="value">¥64,900</span>');
    // lines: P2×1 50,000 + P3×3 9,000
    expect(html).toContain('<td>ロゴ制作</td><td class="num">1</td><td class="num">¥50,000</td><td class="num">¥50,000</td><td class="center">10%</td>');
    expect(html).toContain('<td>デザイン素材集（書籍）</td><td class="num">3</td><td class="num">¥3,000</td><td class="num">¥9,000</td><td class="center">10%</td>');
    ...
    expect(html, 'the script writes the due date as 2026-11-30; the jp layout prints 2026年11月30日 (same date, 西暦 in Japanese form)').toMatch(/2026-11-30|2026年11月30日/);
    // and over HTTP
  ...
  it('11. 消費税集計表（accounting.tax_period_summary 10/01〜10/31）: 売上 standard 239,000 / 23,900・reduced 3,333 / 266、仕入 3 行、totals 24,166 / 5,900 / 18,266', async () => {
    ...
    // and over HTTP
    const viaHttp = await call('POST', '/actions/accounting.tax_period_summary', { body: { from: '2026-10-01', to: '2026-10-31' } });
    expect(viaHttp.status).toBe(200);
