...
import { appMeta, applyPack, connect, dropAll, registry, repo, runAction, runMigrations, systemParams, withContext } from '@daifuku/kernel';
...
beforeAll(async () => {
  ...
  preserved = await withContext(owner, systemParams(boot.tenantId, boot.companyId), async (ctx) => {
    const partner = await runAction(ctx, 'partner.create', { name: 'Keep this customer', isCustomer: true }) as { id: string };
    return runAction(ctx, 'sales_invoice.create', { partnerId: partner.id, date: '2026-09-12', lines: { sales_invoice_line: [{ description: 'Original document', quantity: '1', unitPrice: '1000', taxCategory: 'exempt' }] } }) as Promise<{ id: string; total: string }>;
  });
});
...
describe('industry-templates integration', () => {
  ...
  it('prepares three separate companies and applies each sample without forcing existing settings', async () => {
    ...
    for (const demo of demos) await withContext(app, systemParams(demo.tenantId, demo.companyId), async (ctx) => {
      ...
      expect(meta.modules.filter((m) => ['appliance_store', 'farm', 'restaurant_chain'].includes(m.name)).map((m) => m.name)).toEqual([demo.pack]);
    });
    const [original] = await owner.sql`select settings->'packs.applied' as applied from companies where id=${boot.companyId}`;
  ...
  it('keeps all demo rows on repeated provisioning and blocks unselected company access', async () => {
    const snapshots = async () => Promise.all(demos.map((demo) => withContext(app, systemParams(demo.tenantId, demo.companyId), async (ctx) => {
      const name = { appliance_store: 'appliance_store_service', farm: 'farm_field', restaurant_chain: 'restaurant_chain_store' }[demo.pack];
      const rows = await repo(ctx, registry.entity(name ?? '')).list({ limit: 500 });
      expect(rows.total).toBeGreaterThan(0);
      return rows.items.map((row) => ({ id: row.id, version: row.version })).sort((a, b) => a.id.localeCompare(b.id));
    })));
