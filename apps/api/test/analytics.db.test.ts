import { bootstrapTenant, companyMemberships, hashPassword, newId, registry, repo, runAction, users, withContext, type ContextParams } from '@daifuku/kernel';
...
let db: TestDb, app: FastifyInstance, admin: string, employee: string, manager: string, managerId: string, siteId: string, otherSiteId: string;
const from = '2026-09-01', to = '2026-09-30';
const headers = (token: string) => ({ authorization: `Bearer ${token}` });
const snapshot = (token: string, dataset = 'workforce_expense', state = 'all', extra: Record<string, string> = {}) => app.inject({ method: 'POST', url: '/analytics/snapshot', headers: { ...headers(token), ...extra }, payload: { dataset, from, to, state } });
...
beforeAll(async () => {
  ...
  for (let index = 0; index < 4; index++) {
    const id = newId(), role = index === 3 ? 'workforce_manager' : 'workforce_employee', site = index === 2 ? otherSiteId : siteId;
    const email = `analytics-${index}@example.com`;
    await db.owner.drizzle.insert(users).values({ id, tenantId: db.tenantId, email, name: `Analytics ${index}`, passwordHash: hashPassword('password'), roles: [role], defaultCompanyId: db.companyId });
    await db.owner.drizzle.insert(companyMemberships).values({ tenantId: db.tenantId, companyId: db.companyId, userId: id, roles: [role], accessScope: 'sites', siteIds: [site] });
    await db.run({}, (ctx) => runAction(ctx, 'workforce.register_employee', { userId: id, siteId: site, code: `AN-${index}`, name: '同姓同名', hiredOn: '2026-01-01' }));
    const params: Partial<ContextParams> = { actor: { type: 'user', id }, roles: [role], accessScope: 'sites', siteIds: [site], now: () => new Date('2026-09-13T00:00:00Z') };
    if (index < 3) await db.run(params, (ctx) => runAction(ctx, 'workforce.save_expense', { expectedVersion: 0, idempotencyKey: newId(), expenseDate: '2026-09-10', category: '交通費', description: '非公開の詳細', amount: String((index + 1) * 1000), evidence: '非公開の証憑参照' }));
    if (index === 0) employee = await login(email);
    if (index === 3) { manager = await login(email); managerId = id; }
  }
});
...
describe('authorized browser analytics HTTP', () => {
  it('requires authentication and excludes arbitrary or unauthorized datasets and query input', async () => {
    ...
    expect((catalog.json() as { datasets: { id: string }[] }).datasets.map((row) => row.id)).not.toContain('bank_statement');
    ...
    expect((await app.inject({ method: 'POST', url: '/analytics/snapshot', headers: headers(admin), payload: { dataset: 'workforce_expense', from, to, state: 'all', where: {} } })).statusCode).toBe(400);
  });

  it('keeps employee self scope, site scope and field projection with distinct reference identities', async () => {
    const own = await snapshot(employee), site = await snapshot(manager), all = await snapshot(admin);
    expect(own.statusCode).toBe(200); expect(site.statusCode).toBe(200); expect(all.statusCode).toBe(200);
    const selfRows = own.json<AnalyticsSnapshot>(), siteRows = site.json<AnalyticsSnapshot>(), allRows = all.json<AnalyticsSnapshot>();
  ...
  it('reloads site assignments on the next request and changes the cache scope key', async () => {
    ...
    await db.owner.sql`update user_company_memberships set site_ids = ${JSON.stringify([otherSiteId])}::jsonb where user_id = ${managerId}`;
    try {
    ...
    } finally { await db.owner.sql`update user_company_memberships set site_ids = ${JSON.stringify([siteId])}::jsonb where user_id = ${managerId}`; }
  });

  it('keeps company and tenant boundaries even for administrators', async () => {
    const company = newId();
    await db.owner.sql`insert into companies (id, tenant_id, code, name) values (${company}, ${db.tenantId}, 'AN-OTHER', 'Other company')`;
    expect((await snapshot(admin, 'workforce_expense', 'all', { 'x-company-id': company })).json<AnalyticsSnapshot>().rows).toEqual([]);
    expect((await snapshot(employee, 'workforce_expense', 'all', { 'x-company-id': company })).statusCode).toBe(403);
    const other = await bootstrapTenant(db.owner, { tenantName: 'Other tenant', companyCode: 'O', companyName: 'Other', adminEmail: 'other-analytics@example.com', adminName: 'Other', adminPassword: 'password' });
  ...
  it('uses a read-only repeatable snapshot across concurrent business writes', async () => {
    const params: ContextParams = { tenantId: db.tenantId, companyId: db.companyId, actor: { type: 'user', id: db.adminUserId }, roles: ['admin'] };
    const target = registry.entity('partner'), where = { name: 'During analytics snapshot' };
    await withContext(db.app, params, async (ctx) => {
    ...
    await expect(withContext(db.app, params, (ctx) => repo(ctx, target).create({ name: 'Read-only must refuse this' }), { readOnlySnapshot: true })).rejects.toThrow();
