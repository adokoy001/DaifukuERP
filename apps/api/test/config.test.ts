...
const LOCAL = {
  DATABASE_URL: 'postgres://app:fixture@localhost/example',
  DATABASE_URL_OWNER: 'postgres://owner:fixture@localhost/example',
...
describe('public-release AC-7 startup configuration', () => {
  ...
  it.each(['', '0', '65536', '-1', '3.5', 'NaN', 'Infinity', '1e3', ' 3000', '3000x'])(
    'rejects malformed PORT %j before listening',
    (PORT) => {
  ...
  it('requires strong secrets for production and every non-loopback bind, including development', () => {
    ...
    for (const JWT_SECRET of [`${STRONG}\n`, `${STRONG}\r`, `${STRONG}\0`])
      expect(() => readApiConfig({ ...LOCAL, JWT_SECRET })).toThrow('JWT_SECRET');
  });
  ...
  it('reports missing names and never emits supplied secrets or connection URLs on startup failure', () => {
    ...
    let error: unknown;
    try {
      readApiConfig({ ...LOCAL, NODE_ENV: 'production' });
    } catch (caught) {
      error = caught;
    }
    ...
    expect(startupErrorMessage(new Error(`Invalid URL ${LOCAL.DATABASE_URL_OWNER}`))).not.toContain(
      LOCAL.DATABASE_URL_OWNER,
    );
  });
});
...
describe('public-release AC-8 browser origin policy', () => {
  it('allows only local development origins by default and no cross-origin requests by default in production', () => {
    expect(readApiConfig(LOCAL).corsOrigins).toEqual([
      'http://localhost:5173',
      'http://127.0.0.1:5173',
      'http://[::1]:5173',
    ]);
  ...
  it('normalizes and deduplicates explicitly allowed origins without adding development defaults', () => {
    expect(
      readApiConfig({
        ...LOCAL,
        CORS_ORIGINS: ' https://erp.example.test/,https://erp.example.test:443,http://127.0.0.1:8080 ',
      }).corsOrigins,
    ).toEqual(['https://erp.example.test', 'http://127.0.0.1:8080']);
  });
  ...
  it.each([
    ...
    'https://example.test?',
    'https://example.test#',
  ])('rejects non-origin CORS_ORIGINS %j without echoing it', (CORS_ORIGINS) => {
    try {
      readApiConfig({ ...LOCAL, CORS_ORIGINS });
    } catch (error) {
      expect(error).toBeInstanceOf(ApiConfigError);
