...
const tables = ['workforce_shift_profile', 'workforce_shift_availability', 'workforce_shift_plan', 'workforce_shift_assignment'];
const tenant = newId(), company = newId(), user = newId(), site = newId(), employee = newId();
type Sql = postgres.TransactionSql;
const scoped = <T>(db: Database, tenantId: string, fn: (tx: Sql) => Promise<T>) => db.sql.begin(async tx => {
  await tx`select set_config('app.tenant_id',${tenantId},true)`; return fn(tx);
});
beforeAll(async () => {
...
it('preserves populated 0010 employee, approved attendance and leave facts; adds empty protected shift tables idempotently', async () => {
  ...
  await scoped(app, tenant, tx => tx`insert into workforce_shift_profile(id,tenant_id,company_id,employee_id,user_id,profile) values(${newId()},${tenant},${company},${employee},${user},'{}'::jsonb)`);
  expect((await scoped(app, newId(), tx => tx`select count(*)::int n from workforce_shift_profile`))[0]?.n).toBe(0);
  await expect(scoped(app, tenant, tx => tx`insert into workforce_shift_profile(id,tenant_id,company_id,employee_id,user_id,profile) values(${newId()},${newId()},${company},${employee},${user},'{}'::jsonb)`)).rejects.toMatchObject({ code: '42501' });
  const otherCompany = newId(); await scoped(owner, tenant, tx => tx`insert into companies(id,tenant_id,code,name) values(${otherCompany},${tenant},'OTHER','Other company')`);
  await expect(scoped(app, tenant, tx => tx`insert into workforce_shift_profile(id,tenant_id,company_id,employee_id,user_id,profile) values(${newId()},${tenant},${otherCompany},${employee},${user},'{}'::jsonb)`)).rejects.toMatchObject({ code: '23503' });
});
