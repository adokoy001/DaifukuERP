...
export function edgeFixture(db: TestDb, app: FastifyInstance, token: string) {
  ...
  const post = (url: string, payload: Record<string, unknown> = {}, auth = token): Promise<LightMyRequestResponse> =>
    app.inject({ method: 'POST', url, payload, headers: { authorization: 'Bearer ' + auth } });
  const session = (secret: string): Promise<LightMyRequestResponse> =>
    app.inject({ method: 'GET', url: '/relay/session', headers: { authorization: 'Bearer ' + secret } });
  ...
  const fixture = async () => {
    ...
    const g = await post('/actions/edge.create_gateway', { siteId: site.id, code: newId(), name: 'Gateway' });
    expect(g.statusCode, g.body).toBe(200);
    const gateway = g.json<{ id: string; version: number }>();
    const d = await post('/actions/edge.register_device', {
  ...
  const issue = async (f: Fixture, auth = token) => {
    const response = await post(
      '/edge/pairings',
      { gatewayId: f.gateway.id, expectedVersion: f.gateway.version, stepUpToken: await step(auth) },
      auth,
    );
    expect(response.statusCode, response.body).toBe(200);
    const pairingToken = response.json<{ pairingToken: string }>().pairingToken;
    secrets.push(pairingToken);
    return pairingToken;
  };
  const pair = async (f: Fixture) => {
    ...
    const response = await post(
      '/relay/pair',
      { pairingToken, credentialSecret, protocolVersion: 1, agentVersion: 'test-1' },
      '',
    );
    expect(response.statusCode, response.body).toBe(200);
  ...
  const enqueue = async (f: Fixture) => {
    const response = await post('/actions/edge.enqueue', {
      deviceId: f.device.id,
      idempotencyKey: newId(),
      request: { kind: 'print.text', payload: { text: 'Receipt', title: 'Test', copies: 1 } },
