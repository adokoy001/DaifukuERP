...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'daifuku-readiness-')); temporary.push(root);
  const migrationsDirectory = join(root, 'migrations'), storageDirectory = join(root, 'evidence');
  await mkdir(join(migrationsDirectory, 'meta'), { recursive: true }); await mkdir(storageDirectory, { mode: 0o700 });
  const sql = 'select 1;';
  await writeFile(join(migrationsDirectory, '0000_test.sql'), sql);
  await writeFile(join(migrationsDirectory, 'meta/_journal.json'), JSON.stringify({ entries: [{ idx: 0, when: 100, tag: '0000_test' }] }));
  ...
  const owner = database(() => Promise.resolve([row])), app = database(() => Promise.resolve([identity]));
...
function options(f: Awaited<ReturnType<typeof fixture>>) { return { ...f, owner: f.owner.db, app: f.app.db }; }
describe('opaque, bounded deployment readiness', () => {
  it('matches the entire release migration history and checks both principals without creating evidence', async () => {
    const f = await fixture(), probe = createReadiness(options(f));
    expect(await probe()).toEqual({ ready: true });
    expect(f.owner.sql).toHaveBeenCalledOnce(); expect(f.app.sql).toHaveBeenCalledOnce();
  ...
  it('does not expose connection errors and requires a private, real evidence directory', async () => {
    const f = await fixture(), failed = database(() => Promise.reject(new Error('postgres://synthetic:secret@host/db')));
    ...
    const link = join(f.root, 'linked'); await symlink(f.storageDirectory, link);
    expect(await createReadiness({ ...options(f), storageDirectory: link })()).toEqual({ ready: false });
    expect(await createReadiness({ ...options(f), storageDirectory: join(f.root, 'missing') })()).toEqual({ ready: false });
  ...
  it('deduplicates concurrent probes, cancels timed out queries, and recovers on a new check', async () => {
    const f = await fixture(), stalled = database(() => new Promise(() => undefined));
    const probe = createReadiness({ ...options(f), owner: stalled.db, timeoutMs: 10 });
    expect(await Promise.all([probe(), probe(), probe()])).toEqual([{ ready: false }, { ready: false }, { ready: false }]);
    expect(stalled.sql).toHaveBeenCalledOnce(); expect(stalled.cancel).toHaveBeenCalledOnce();
  ...
  it('does not report ready with an invalid bundled migration catalog', async () => {
    const f = await fixture();
    await writeFile(join(f.migrationsDirectory, 'meta/_journal.json'), JSON.stringify({ entries: [{ idx: 0, when: 100, tag: '../../secret' }] }));
    expect(await createReadiness(options(f))()).toEqual({ ready: false });
  });
});
