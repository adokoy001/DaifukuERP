...
const post = (path: string, body: NonNullable<InjectOptions['payload']>, auth = '') =>
  app.inject({
    ...
    url: `/auth/${path}`,
    payload: body,
    ...(auth ? { headers: { authorization: `Bearer ${auth}`, 'x-company-id': 'stale-company-value' } } : {}),
  });
const login = () => post('login', { email: 'admin@example.com', password: 'password' });
const security = (auth: string) =>
  app.inject({ method: 'GET', url: '/auth/security', headers: { authorization: `Bearer ${auth}` } });
async function stepup() {
  const result = await post('step-up', { currentPassword: 'password' }, token);
...
async function start(link = false) {
  ...
  expect(result.statusCode, result.body).toBe(200);
  return { browserNonce, authorizationUrl: result.json<{ authorizationUrl: string }>().authorizationUrl };
}
async function oidcComplete(input: Awaited<ReturnType<typeof start>>, claims: JWTPayload = {}) {
  return post('oidc/complete', {
    providerId: 'fixture',
    browserNonce: input.browserNonce,
    ...oidc.code(input.authorizationUrl, claims),
  });
}
beforeAll(async () => {
  db = await freshDb();
  oidc = await oidcFixture();
  app = await buildServer({
    owner: db.owner,
    app: db.app,
    jwtSecret: 'synthetic-identity-jwt-secret',
    identity: {
      encryptionKey: key,
      ...
      providers: [
        {
          ...
          authorizationEndpoint: `${oidc.issuer}/authorize`,
          tokenEndpoint: `${oidc.issuer}/token`,
          jwksUri: `${oidc.issuer}/jwks`,
        },
      ],
    },
  });
  await app.ready();
  token = (await login()).json<{ token: string }>().token;
});
afterAll(async () => {
  await app?.close();
  await oidc?.close();
...
describe('OIDC HTTP boundaries with real signed local ID tokens', () => {
  it('does not turn an email-matched external identity into an administrator or new account', async () => {
    const attempt = await oidcComplete(await start());
  ...
  it('binds link state to this browser, uses PKCE and revokes sessions on explicit account linking', async () => {
    ...
    const wrong = await post('oidc/complete', { providerId: 'fixture', browserNonce: opaqueToken(), ...grant });
    expect(wrong.statusCode).toBe(401);
    const complete = await post('oidc/complete', { providerId: 'fixture', browserNonce: input.browserNonce, ...grant });
    ...
    expect(
      (await post('oidc/complete', { providerId: 'fixture', browserNonce: input.browserNonce, ...grant })).statusCode,
    ).toBe(401);
    const logged = await oidcComplete(await start());
    expect(logged.statusCode, logged.body).toBe(200);
    token = logged.json<{ token: string }>().token;
  });
  it('rejects issuer, audience, expiry, nonce and authorized-party confusion', async () => {
    for (const claims of [
      ...
      const response = await oidcComplete(await start(), claims);
      expect(response.statusCode, JSON.stringify(claims)).toBe(401);
...
describe('MFA, invitation and reset HTTP boundaries', () => {
  it('keeps wrong current-password attempts recoverable and enables MFA only after a valid proof', async () => {
    const wrong = await post('step-up', { currentPassword: 'incorrect' }, token);
    expect(wrong.statusCode).toBe(400);
    expect((await security(token)).statusCode).toBe(200);
    const setup = await post('mfa/setup', await stepup(), token);
    ...
    const confirmed = await post(
      'mfa/confirm',
      { setupToken: body.setupToken, code: totp(body.secret, Math.floor(Date.now() / 30000)) },
      token,
    );
    expect(confirmed.statusCode, confirmed.body).toBe(200);
    codes = confirmed.json<{ recoveryCodes: string[] }>().recoveryCodes;
  ...
  it('issues no normal token before MFA and accepts each recovery code just once', async () => {
    ...
    const verified = await post('mfa/verify', { challengeToken, code: codes[0] });
    expect(verified.statusCode, verified.body).toBe(200);
    token = verified.json<{ token: string }>().token;
    expect((await security(token)).json()).toMatchObject({ mfaEnabled: true, recoveryCodesRemaining: 9 });
    const next = (await login()).json<{ challengeToken: string }>();
    expect((await post('mfa/verify', { challengeToken: next.challengeToken, code: codes[0] })).statusCode).toBe(400);
    const external = await oidcComplete(await start());
    expect(external.json()).toMatchObject({ mfaRequired: true });
  ...
  it('returns identical reset request responses and leaves MFA enabled after a single-use password reset', async () => {
    ...
    const [mail] = await db.owner.drizzle.select().from(identityMail);
    ...
    const link = payload.text.match(/https:\/\/erp\.example\.com\/[^\s]+/)?.[0];
    if (!link) throw new Error('Missing reset link');
    const resetToken = new URLSearchParams(new URL(link).hash.slice(1)).get('token');
    const complete = await post('password-reset/complete', { token: resetToken, newPassword: 'new-password-value' });
    expect(complete.statusCode, complete.body).toBe(200);
    expect(
