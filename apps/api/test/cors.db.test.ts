...
describe('public-release AC-8 CORS with real API authentication', () => {
  it('permits PATCH and DELETE preflights only for the configured local UI origins', async () => {
    for (const origin of ['http://localhost:5173', 'http://127.0.0.1:5173', 'http://[::1]:5173']) {
      for (const method of ['PATCH', 'DELETE']) {
        const res = await local.inject({ method: 'OPTIONS', url: '/api/partner', headers: { origin, 'access-control-request-method': method, 'access-control-request-headers': 'authorization,content-type,x-company-id' } });
  ...
  it('never reflects a foreign, null, deceptive suffix, or wrong-port origin', async () => {
    for (const origin of ['https://untrusted.example.test', 'null', 'http://localhost:5173.untrusted.example.test', 'http://localhost:5174']) {
      const res = await local.inject({ method: 'OPTIONS', url: '/api/partner', headers: { origin, 'access-control-request-method': 'PATCH' } });
  ...
  it('uses the explicit deployment allowlist without inheriting local defaults', async () => {
    for (const origin of ['https://erp.example.test', 'http://localhost:5173']) {
      ...
      expect(res.headers['access-control-allow-origin']).toBe(origin === 'https://erp.example.test' ? origin : undefined);
    }
  });

  it('defaults a directly built server to no cross-origin grants and preserves JWT authentication without Origin', async () => {
    const login = await sameOrigin.inject({ method: 'POST', url: '/auth/login', payload: { email: 'admin@example.com', password: 'password' } });
    expect(login.statusCode).toBe(200);
    expect(login.headers['access-control-allow-origin']).toBeUndefined();
    const token = login.json<{ token: string }>().token;
    const me = await sameOrigin.inject({ method: 'GET', url: '/auth/me', headers: { authorization: `Bearer ${token}` } });
    expect(me.statusCode).toBe(200);
    expect(me.json()).toMatchObject({ user: { id: db.adminUserId }, companyId: db.companyId });
    const foreign = await sameOrigin.inject({ method: 'GET', url: '/health', headers: { origin: 'http://localhost:5173' } });
