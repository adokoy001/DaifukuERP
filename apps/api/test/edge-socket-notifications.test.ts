...
import {
  RELAY_SOCKET_BUFFER_LIMIT,
  relaySocketHasCapacity,
  sendRelayNotification,
} from '../src/edge/socket-notifications.ts';
...
function stalledReceiver() {
  const socket = {
    ...
    send: vi.fn((data: unknown) => {
      socket.bufferedAmount += Buffer.byteLength(String(data), 'utf8') + 2;
    }),
  };
  return socket;
}
...
describe('bounded WSS notification sending', () => {
  ...
  it('checks again after asynchronous authorization before appending another frame', async () => {
    const socket = stalledReceiver();
    await sendRelayNotification(socket, async () => {
      socket.bufferedAmount = RELAY_SOCKET_BUFFER_LIMIT;
      return true;
    });
