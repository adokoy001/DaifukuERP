...
const request = (method: 'GET' | 'POST' | 'PATCH', url: string, token: string, payload?: InjectOptions['payload']) =>
  app.inject({
    method,
    url,
...
async function person(name: string, role: string) {
  ...
  const row = await command('register_employee', admin, {
    userId: id,
    siteId,
    code: name,
    name,
...
beforeAll(async () => {
  db = await freshDb();
  app = await buildServer({ owner: db.owner, app: db.app, jwtSecret: 'synthetic-shift-adapter-secret' });
  await app.ready();
  admin = await login('admin@example.com', 'password');
  for (const code of ['A', 'B']) {
    const response = await request('POST', '/api/workforce_site', admin, { code, name: code });
    expect(response.statusCode, response.body).toBe(200);
    if (code === 'A') siteId = response.json<{ id: string }>().id;
    else otherSiteId = response.json<{ id: string }>().id;
  }
  await command('initialize_policy', admin, {});
  employee = await person('shift-employee', 'workforce_employee');
  manager = await person('shift-manager', 'workforce_manager');
});
afterAll(async () => {
  await app?.close();
  await db?.close();
...
describe('employee-shift-planner generated HTTP boundaries', () => {
  it('exposes self-service only to the employee and rejects forged ownership and manager operations', async () => {
    const meta = await request('GET', '/meta', employee.token);
    expect(meta.statusCode).toBe(200);
    const actions = meta.json<{ actions: { name: string }[] }>().actions.map((row) => row.name);
    expect(actions).toContain('workforce.my_shifts');
    expect(actions).toContain('workforce.save_shift_availability');
    expect(actions).not.toContain('workforce.shift_board');
    const forged = await request('POST', '/actions/workforce.save_shift_availability', employee.token, {
      weekStart,
      expectedVersion: 0,
      days,
    ...
    expect(
      (await request('POST', '/actions/workforce.shift_board', employee.token, { siteId, weekStart })).statusCode,
    ).toBe(403);
    expect(
      (
        await request('POST', '/actions/workforce.save_shift_profile', employee.token, {
    ...
    const data = own.json<MyShifts>();
    expect(data.employee?.id).toBe(employee.employeeId);
    expect(data.availability?.days).toHaveLength(7);
    expect(own.body).not.toContain(manager.employeeId);
    expect(data.assignments).toEqual([]);
  });
  it('limits planning snapshots to the assigned site without payroll, expense or leave reasons', async () => {
    await command('save_shift_profile', manager.token, {
      employeeId: employee.employeeId,
      expectedVersion: 0,
      profile,
    });
    ...
    expect(response.statusCode, response.body).toBe(200);
    const board = response.json<ShiftBoard>();
    expect(board.problem.employees.find((row) => row.id === employee.employeeId)?.profile).toMatchObject(profile);
    for (const field of ['payrolls', 'expenses', 'passwordHash', 'grossPay', 'reviewReason'])
      expect(response.body).not.toContain('"' + field + '"');
    expect(
      (await request('POST', '/actions/workforce.shift_board', manager.token, { siteId: otherSiteId, weekStart }))
        .statusCode,
    ).toBe(404);
    const generic = await request('POST', '/api/workforce_shift_plan', manager.token, {
      siteId,
      weekStart,
      status: 'published',
      slots: [],
  ...
  it('rechecks current membership for an already signed-in manager', async () => {
    expect(
      (await request('POST', '/actions/workforce.shift_board', manager.token, { siteId, weekStart })).statusCode,
    ).toBe(200);
    ...
    expect(denied.body).not.toContain(employee.employeeId);
  });
});
