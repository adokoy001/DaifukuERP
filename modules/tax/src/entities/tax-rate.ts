...
export const TaxRate = defineEntity({
  ...
  fields: {
    ...
    category: f.enum(TAX_CATEGORIES, {
      label: label('税区分', 'Category'),
      required: true,
      labels: TAX_CATEGORY_LABELS,
    }),
    rate: f.decimal({
      label: label('税率', 'Rate'),
      description: label('0.10 = 10%', '0.10 = 10%'),
      required: true,
      min: '0',
      scale: 4,
    }),
    ...
    validTo: f.date({
      label: label('適用終了日', 'Valid to'),
      description: label('空欄 = 無期限', 'Empty = open-ended'),
    }),
