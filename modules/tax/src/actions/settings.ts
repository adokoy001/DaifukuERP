...
import { z } from 'zod';
import { TAX_PRICE_INCLUDES_TAX_KEY, TAX_ROUNDING_KEY, taxPriceIncludesTaxSchema, taxRoundingSchema, taxSettingsSchema } from '../services/settings.ts';
import { loadTaxSettings } from '../summary.ts';

export const getSettingsAction = defineAction({
  name: 'tax.get_settings',
  description: label('会社の消費税設定（端数処理・税込入力）を返します。未設定なら既定値。', 'Return the company tax settings (rounding, price-includes-tax); defaults when unset.'),
  input: z.object({}),
...
export const setSettingsAction = defineAction({
  ...
  handler: async (ctx, input) => {
    ...
    if (input.priceIncludesTax !== undefined) await setSetting(ctx, TAX_PRICE_INCLUDES_TAX_KEY, taxPriceIncludesTaxSchema, input.priceIncludesTax);
