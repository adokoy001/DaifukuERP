...
describe('tax module (docs/specs/tax.md)', () => {
  it('AC-8 seed creates the 6 Japanese rate rows and is idempotent', async () => {
    ...
    await seed();
    expect(await count()).toBe(6);
    const std10 = await db.run(
      {},
      async (ctx) => (await repo(ctx, TaxRate).list({ where: { code: 'STD10' } })).items[0],
    );
    expect(std10).toMatchObject({
      category: 'standard',
  ...
  it('AC-1 entity: code is upper-cased, unique and immutable; rate >= 0; zero-rate categories must be 0', async () => {
    const std5 = await db.run({}, (ctx) =>
      repo(ctx, TaxRate).create({
        code: 'std5',
        category: 'standard',
        rate: '0.05',
    ...
    await expect(
      db.run({}, (ctx) =>
        repo(ctx, TaxRate).create({
          ...
          category: 'standard',
          rate: '0.05',
          validFrom: '1989-04-01',
          validTo: '1997-03-31',
          label: 'dup',
        }),
      ),
    ).rejects.toBeInstanceOf(Conflict);
    await expect(db.run({}, (ctx) => repo(ctx, TaxRate).update(std5.id, { code: 'STD5X' }))).rejects.toMatchObject({
      code: 'VALIDATION',
    ...
    await expect(
      db.run({}, (ctx) =>
        repo(ctx, TaxRate).create({
          code: 'BADCAT',
          category: 'vat' as 'standard',
          rate: '0.1',
          validFrom: '1900-01-01',
          label: 'x',
        }),
      ),
    ).rejects.toMatchObject({ details: { issues: [{ path: 'category' }] } });
    await expect(
      db.run({}, (ctx) =>
        repo(ctx, TaxRate).create({
          code: 'bad code',
    ...
    ).rejects.toMatchObject({ details: { issues: [{ path: 'code' }] } });
  });
  ...
  it('AC-1 overlap: create/update whose period overlaps another row of the same category is rejected, naming the conflicting code', async () => {
    const attempt = (input: Record<string, unknown>) =>
      caught(db.run({}, (ctx) => runAction(ctx, 'tax_rate.create', input)));
    ...
    ).toMatchObject({ message: expect.stringContaining('STD8') });
    expect(
      await attempt({
        code: 'X2',
        category: 'reduced',
    ...
    ).toMatchObject({ details: { issues: [{ path: 'validTo' }] } });
    ...
    const x2id = x2?.id ?? '';
    await expect(db.run({}, (ctx) => repo(ctx, TaxRate).update(x2id, { validTo: '2019-10-01' }))).rejects.toMatchObject(
      { code: 'VALIDATION', message: expect.stringContaining('RED8') },
    );
    await expect(db.run({}, (ctx) => repo(ctx, TaxRate).update(x2id, { validTo: null }))).rejects.toMatchObject({
      code: 'VALIDATION',
      message: expect.stringContaining('RED8'),
    });
    ...
    const other = await withContext(db.app, systemParams(db.tenantId, otherCompany), (ctx) =>
      repo(ctx, TaxRate).create({
        code: 'STD10',
        category: 'standard',
        rate: '0.1',
  ...
  it('AC-1 permissions: accounting/settings create+update, sales/purchasing/viewer read only, others denied', async () => {
    const created = await db.run(asRole(['accounting']), (ctx) =>
      repo(ctx, TaxRate).create({
        code: 'ACC1',
        category: 'reduced',
        rate: '0.03',
    ...
    const updated = await db.run(asRole(['settings']), (ctx) =>
      repo(ctx, TaxRate).update(created.id, { label: 'acc2' }),
    );
    ...
    for (const role of ['sales', 'purchasing', 'viewer']) {
      ...
      await expect(
        db.run(asRole([role]), (ctx) => repo(ctx, TaxRate).update(created.id, { label: 'no' })),
      ).rejects.toBeInstanceOf(PermissionDenied);
    }
    await expect(db.run(asRole(['accounting']), (ctx) => repo(ctx, TaxRate).delete(created.id))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
    await expect(db.run(asRole(['nobody']), (ctx) => repo(ctx, TaxRate).list())).rejects.toBeInstanceOf(
      PermissionDenied,
    );
  ...
  it('AC-5 settings: defaults when unset, set by admin/settings role only, validated, audited, declared in the registry', async () => {
    ...
    expect(await db.run(asRole(['viewer']), (ctx) => runAction(ctx, 'tax.get_settings', {}))).toEqual(
      TAX_SETTINGS_DEFAULT,
    );
    await expect(
      db.run(asRole(['accounting']), (ctx) => runAction(ctx, 'tax.set_settings', { priceIncludesTax: true })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      db.run({}, (ctx) => runAction(ctx, 'tax.set_settings', { rounding: { mode: 'banker' } })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'rounding.mode' }] } });
    const set = await db.run(asRole(['settings']), (ctx) =>
      runAction(ctx, 'tax.set_settings', { rounding: { mode: 'half_up' } }),
    );
    ...
    await db.run({}, (ctx) =>
      runAction(ctx, 'tax.set_settings', { rounding: { mode: 'down', unit: 'invoice' }, priceIncludesTax: false }),
    );
  });

  it('AC-6 tax.resolve uses the company rows; zero-rate categories resolve to 0; missing period -> hint', async () => {
    expect(
      await db.run(asRole(['viewer']), (ctx) =>
        runAction(ctx, 'tax.resolve', { category: 'standard', date: '2026-09-10' }),
      ),
    ).toEqual({ category: 'standard', code: 'STD10', rate: '0.1', label: '標準10%' });
    expect(
      await db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'standard', date: '2019-09-30' })),
    ).toMatchObject({ code: 'STD8', rate: '0.08' });
    expect(
      await db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'standard', date: '2000-01-01' })),
    ).toMatchObject({ code: 'STD5', rate: '0.05' });
    expect(
      await db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'exempt', date: '2026-09-10' })),
    ).toEqual({ category: 'exempt', code: 'EXEMPT', rate: '0', label: '免税' });
    expect(
      await db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'out_of_scope', date: '1990-01-01' })),
    ).toMatchObject({ code: 'OOS', rate: '0' });
    await expect(
      db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'reduced', date: '2019-09-30' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', hint: 'add a tax_rate row for reduced covering 2019-09-30' });
    await expect(
      db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'standard', date: '2026-02-30' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'date' }] } });
    await expect(
      db.run(asRole(['nobody']), (ctx) => runAction(ctx, 'tax.resolve', { category: 'standard', date: '2026-09-10' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
  });
  ...
  it('AC-7 tax.summarize: company rates + rounding setting; priceIncludesTax from setting or input', async () => {
    ...
    const down = (await db.run(asRole(['sales']), (ctx) =>
      runAction(ctx, 'tax.summarize', { date: '2026-09-10', lines }),
    ...
    const halfUp = (await db.run({}, (ctx) => runAction(ctx, 'tax.summarize', { date: '2026-09-10', lines }))) as {
      groups: { tax: string }[];
      totals: { tax: string };
    };
    ...
    const old = (await db.run({}, (ctx) =>
      runAction(ctx, 'tax.summarize', { date: '2019-09-30', lines: [{ amount: '1000', category: 'standard' }] }),
    )) as { groups: { code: string; tax: string }[] };
    ...
    const incl = (await db.run({}, (ctx) =>
      runAction(ctx, 'tax.summarize', {
        date: '2026-09-10',
        priceIncludesTax: true,
        lines: [
          { amount: '1100', category: 'standard' },
          { amount: '1540', category: 'reduced' },
        ],
      }),
    )) as { priceIncludesTax: boolean; groups: { taxable: string; tax: string; gross: string }[] };
    expect(incl.priceIncludesTax).toBe(true);
    expect(incl.groups).toEqual([
      expect.objectContaining({ taxable: '1000', tax: '100', gross: '1100' }),
      expect.objectContaining({ taxable: '1426', tax: '114', gross: '1540' }),
    ]);
    await db.run({}, (ctx) =>
      runAction(ctx, 'tax.set_settings', { rounding: { mode: 'down' }, priceIncludesTax: true }),
    );
    const inclBySetting = (await db.run({}, (ctx) =>
      runAction(ctx, 'tax.summarize', { date: '2026-09-10', lines: [{ amount: '1100', category: 'standard' }] }),
    )) as { priceIncludesTax: boolean; totals: { tax: string } };
    ...
    await expect(
      db.run({}, (ctx) =>
        runAction(ctx, 'tax.summarize', { date: '2026-09-10', lines: [{ amount: '12.3.4', category: 'standard' }] }),
      ),
    ).rejects.toMatchObject({ details: { issues: [{ path: 'lines.0.amount' }] } });
    await expect(
      db.run({}, (ctx) =>
        runAction(ctx, 'tax.summarize', { date: '2026-09-10', lines: [{ amount: '1', category: 'vat' }] }),
      ),
    ).rejects.toMatchObject({ details: { issues: [{ path: 'lines.0.category' }] } });
    await expect(
      db.run({}, (ctx) =>
        runAction(ctx, 'tax.summarize', { date: '2019-09-30', lines: [{ amount: '1', category: 'reduced' }] }),
      ),
    ).rejects.toMatchObject({ hint: 'add a tax_rate row for reduced covering 2019-09-30' });
    await expect(
      db.run(asRole(['nobody']), (ctx) => runAction(ctx, 'tax.summarize', { date: '2026-09-10', lines })),
    ).rejects.toBeInstanceOf(PermissionDenied);
  });

  it('taxSummaryFor (in-process API for sales/purchase) returns Decimals with code/label per group', async () => {
    const s = await db.run(asRole(['purchasing']), (ctx) =>
      taxSummaryFor(ctx, {
        date: '2026-09-10',
        lines: [
          { amount: Decimal.from('1235'), category: 'standard' },
          { amount: '-333', category: 'reduced' },
        ],
      }),
    );
  ...
  it('AC-9 RED1 (reduced 1% from 2027-04-01) is added as data: close RED8, create RED1, resolve switches on the date', async () => {
    ...
    ).rejects.toMatchObject({ message: expect.stringContaining('RED8') });
    ...
    expect(
      await db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'reduced', date: '2027-03-31' })),
    ).toMatchObject({ code: 'RED8', rate: '0.08' });
    expect(
      await db.run({}, (ctx) => runAction(ctx, 'tax.resolve', { category: 'reduced', date: '2027-04-01' })),
    ).toMatchObject({ code: 'RED1', rate: '0.01' });
    const s = (await db.run({}, (ctx) =>
      runAction(ctx, 'tax.summarize', { date: '2027-04-01', lines: [{ amount: '1000', category: 'reduced' }] }),
    )) as { groups: { code: string; tax: string }[] };
