...
export async function fillLineUom(
  ctx: Context,
  { row, previous }: { row: Record<string, unknown>; previous?: Record<string, unknown> | undefined },
): Promise<void> {
  ...
  if (product?.kind === 'goods' && id !== product.uomId)
    throw new ValidationError('Goods must use their stock unit', [
      { path: 'uomId', message: 'unit conversion is not implemented; use the product stock unit' },
    ]);
  if (typeof id !== 'string') {
    row.uomId = null;
    row.uomCode = 'unit';
