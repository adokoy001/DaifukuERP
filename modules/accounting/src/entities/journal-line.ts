...
export const JournalLine = defineEntity({
  ...
  fields: {
    accountType: f.enum(ACCOUNT_TYPES, {
      label: label('転記時勘定区分', 'Account type at posting'),
      serverOwned: true,
      hidden: true,
    }),
    accountTaxRole: f.enum(TAX_ROLES, {
      label: label('転記時税分類', 'Account tax role at posting'),
      serverOwned: true,
      hidden: true,
    }),
    ...
    taxRate: f.decimal({
      ...
      description: label('例: 0.10', 'e.g. 0.10'),
      scale: 4,
      min: '0',
    }),
    ...
    entryDate: f.date({
      serverOwned: true,
      label: label('仕訳日付', 'Entry date'),
      description: label('submit 時にヘッダから複写', 'copied from the header at submit'),
      hidden: true,
    }),
    posted: f.bool({
      serverOwned: true,
      label: label('転記済み', 'Posted'),
      description: label('submit 時に true', 'set at submit'),
      required: true,
      default: false,
      hidden: true,
    }),
