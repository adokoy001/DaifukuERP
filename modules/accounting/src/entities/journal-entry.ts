...
export const JournalEntry = defineDocument({
  ...
  fields: {
    ...
    sourceEntity: f.text({ serverOwned: true, label: label('起票元エンティティ', 'Source entity'), description: label('例: sales_invoice', 'e.g. sales_invoice'), maxLength: 100 }),
    ...
    totalDebit: f.money({ serverOwned: true, label: label('借方合計', 'Total debit'), description: label('submit 時に明細から算出', 'computed from the lines at submit'), required: true, default: '0' }),
    totalCredit: f.money({ serverOwned: true, label: label('貸方合計', 'Total credit'), description: label('submit 時に明細から算出', 'computed from the lines at submit'), required: true, default: '0' }),
  },
