...
import { registry, repo, StateError, withLock, ValidationError, type Context, type Infer, type LocalDate } from '@daifuku/kernel';
...
export async function loadLines(ctx: Context, entryId: string): Promise<LineRow[]> {
  const res = await repo(ctx, JournalLine).list({ where: { entryId }, orderBy: [{ field: 'seq', dir: 'asc' }], limit: 500 });
  if (res.total > res.items.length) throw new ValidationError('Journal lines exceed the supported 500-line aggregate limit', [{ path: 'lines', message: 'at most 500 lines; no partial posting is permitted' }]);
  return res.items;
}
...
export async function assertOpenPeriod(ctx: Context, date: LocalDate): Promise<Infer<typeof FiscalPeriod>> {
  ...
  const periods = await repo(ctx, FiscalPeriod).list({ where: { $and: [{ startDate: { $lte: date } }, { endDate: { $gte: date } }] }, limit: 1 });
  const period = periods.items[0];
  if (!period) {
    throw new StateError(`no fiscal period contains ${date}`, `Create the fiscal year with accounting.open_fiscal_year, then ${PERIOD_HINT}.`, { date });
  }
  if (period.isClosed) throw new StateError(`fiscal period ${period.code} is closed`, `Reopen it with accounting.reopen_period (${PERIOD_HINT}).`, { date, periodId: period.id, code: period.code });
  const year = await repo(ctx, FiscalYear).get(period.fiscalYearId);
  if (year.isClosed) throw new StateError(`fiscal year ${year.code} is closed`, `The year is closed; ${PERIOD_HINT}.`, { date, fiscalYearId: year.id, code: year.code });
  return period;
}
...
export function registerValidateEntryHook(): void {
  registry.registerHook(JournalEntry.name, 'before_submit', async (ctx, { row }) => {
    ...
    if (result.issues.length > 0) {
      throw new ValidationError(`journal_entry ${entryId} cannot be submitted`, result.issues, 'Fix the lines (each line debit XOR credit > 0, totals equal, partner where required) and submit again.');
    }
  ...
  registry.registerHook(JournalEntry.name, 'after_submit', async (ctx, { row }) => {
    ...
    await withStamp(ctx, async (internal) => {
      for (const l of await loadLines(ctx, entryId)) {
        ...
        await repo(internal, JournalLine).update(l.id, { entryDate: date, posted: true, accountType: account.type, accountTaxRole: account.taxRole });
      }
    });
  });
}
