...
async function setClosed(ctx: Context, periodId: string, closed: boolean): Promise<Record<string, unknown>> {
  ...
  if (period.isClosed === closed) {
    throw new StateError(
      `fiscal period ${period.code} is already ${closed ? 'closed' : 'open'}`,
      closed
        ? 'Nothing to do; use accounting.reopen_period to reopen it.'
        : 'Nothing to do; use accounting.close_period to close it.',
      { periodId, code: period.code },
    );
  }
...
export const closePeriodAction = defineAction({
  ...
  description: label(
    '会計期間を締めます。以後、その期間の日付の仕訳は転記できません（監査ログに記録）。',
    'Close a fiscal period: submits dated in it are refused from now on (audited).',
  ),
