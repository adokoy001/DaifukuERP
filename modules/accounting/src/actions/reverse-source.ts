...
export async function reverseSourceEntry(
  ctx: Context,
  input: ReverseEntryInput & { sourceEntity: string; sourceId: string },
  ...
  if (original.sourceEntity !== input.sourceEntity || original.sourceId !== input.sourceId) {
    throw new StateError(
      'journal entry does not belong to the cancelling source',
      'Repair the source linkage before retrying the business cancellation.',
    );
  }
