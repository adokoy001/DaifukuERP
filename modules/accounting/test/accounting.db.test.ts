...
const line = (accountId: string, debit: string, credit: string, extra: Row = {}): Row => ({
  accountId,
  debit,
  credit,
  ...extra,
...
async function createEntry(ctx: Context, date: string, lines: Row[], head: Row = {}): Promise<EntryWithKeyedLines> {
  return (await runAction(ctx, 'journal_entry.create', {
    date,
    ...head,
    lines: { journal_line: lines },
  })) as EntryWithKeyedLines;
}
...
): Promise<EntryWithKeyedLines> {
  const draft = await run(params, (ctx) => createEntry(ctx, date, lines, head));
  await run(params, (ctx) => submit(ctx, draft.id));
  return run(
    params,
    async (ctx) => (await runAction(ctx, 'journal_entry.get', { id: draft.id })) as EntryWithKeyedLines,
  );
}
...
describe('accounting module (docs/specs/accounting.md)', () => {
  it('AC-11 seed opens a Jan–Dec fiscal year for the current year only when none exists (idempotent, no CoA)', async () => {
    const seed = () =>
      withContext(db.app, systemParams(db.tenantId, db.companyId, { now: () => FIXED_NOW }), async (ctx) =>
        AccountingModule.seed?.(ctx),
      );
    expect(await run({}, (ctx) => repo(ctx, FiscalYear).count())).toBe(0);
    await seed();
    await seed();
    const years = await run({}, (ctx) => repo(ctx, FiscalYear).list());
    expect(years.items.map((y) => [y.code, y.startDate, y.endDate, y.isClosed, y.version])).toEqual([
      ['FY2026', '2026-01-01', '2026-12-31', false, 1],
    ]);
  ...
  it('AC-1 account: defaults, unique immutable code, enums, kana; accounting read/create/update, viewer read, delete admin only', async () => {
    ...
    await expect(
      run({}, (ctx) => repo(ctx, Account).create({ code: '1000', name: 'dup', type: 'asset' })),
    ).rejects.toBeInstanceOf(Conflict);
    await expect(run({}, (ctx) => repo(ctx, Account).update(acc.cash, { code: '1001' }))).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'code' }] },
    });
    await expect(
      run({}, (ctx) => repo(ctx, Account).create({ code: 'X', name: 'x', type: 'bogus' as never })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'type' }] } });
    await expect(
      run({}, (ctx) =>
        repo(ctx, Account).create({ code: 'X', name: 'x', type: 'asset', taxCategoryDefault: 'vat' as never }),
      ),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'taxCategoryDefault' }] } });
    await expect(
      run({}, (ctx) => repo(ctx, Account).create({ name: 'no code', type: 'asset' } as never)),
    ).rejects.toBeInstanceOf(ValidationError);
    const accounting = asRole(['accounting']);
    const created = await run(accounting, (ctx) =>
    ...
    await expect(run(accounting, (ctx) => repo(ctx, Account).delete(created.id))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
    expect(await run(accounting, async (ctx) => appMeta(ctx).entities.find((e) => e.name === 'account')?.ops)).toEqual([
      'read',
      'create',
      'update',
    ]);
    ...
    await expect(
      run(viewer, (ctx) => repo(ctx, Account).create({ code: 'V', name: 'v', type: 'asset' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run(viewer, (ctx) => repo(ctx, Account).update(created.id, { name: 'v' }))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
  ...
  it('AC-2 open_fiscal_year: 12 monthly periods from any start month; overlap → CONFLICT; non-1st / bad ranges → VALIDATION', async () => {
    const r = (await run({}, (ctx) => runAction(ctx, 'accounting.open_fiscal_year', { startDate: '2027-04-01' }))) as {
      fiscalYear: Row;
      periods: Row[];
    };
    expect(r.fiscalYear).toMatchObject({
      code: 'FY2027',
    ...
    expect(r.periods.map((p) => p.code)).toEqual([
      '2027-04',
      '2027-05',
      '2027-06',
      '2027-07',
    ...
    expect(r.periods[10]).toMatchObject({
      startDate: '2028-02-01',
      endDate: '2028-02-29',
      isClosed: false,
      fiscalYearId: r.fiscalYear.id,
    });
    ...
    await expect(
      run({}, (ctx) => runAction(ctx, 'accounting.open_fiscal_year', { startDate: '2026-04-15' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'startDate' }] } });
    await expect(
      run({}, (ctx) => runAction(ctx, 'accounting.open_fiscal_year', { startDate: '2026-13-01' })),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
    // the generic path is guarded by the same hooks
    await expect(
      run({}, (ctx) => repo(ctx, FiscalYear).create({ code: 'BAD', startDate: '2030-12-31', endDate: '2030-01-01' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'endDate' }] } });
    await expect(
      run({}, (ctx) => repo(ctx, FiscalYear).create({ code: 'OVL', startDate: '2026-12-01', endDate: '2027-11-30' })),
    ).rejects.toBeInstanceOf(Conflict);
    const fy26 = (await run({}, (ctx) => repo(ctx, FiscalYear).list({ where: { code: 'FY2026' } }))).items[0];
    await expect(
      run({}, (ctx) => repo(ctx, FiscalYear).update(fy26?.id ?? '', { endDate: '2027-04-30' })),
    ).rejects.toBeInstanceOf(ValidationError);
    const p = r.periods[0] as { id: string };
    await expect(
      run({}, (ctx) => repo(ctx, FiscalPeriod).update(p.id, { endDate: '2027-03-31' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'endDate' }] } });
    // an overlap-free year after FY2027 is fine; open_fiscal_year needs fiscal_year.create (accounting has it, sales does not)
    await expect(
      run(asRole(['sales']), (ctx) => runAction(ctx, 'accounting.open_fiscal_year', { startDate: '2028-04-01' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    const r2 = (await run(asRole(['accounting']), (ctx) =>
      runAction(ctx, 'accounting.open_fiscal_year', { startDate: '2028-04-01' }),
    )) as { fiscalYear: Row };
  ...
  it('AC-3 journal_entry with journal_line lines: seq from order, money defaults, JE-<year>-<n> numbering, get returns lines', async () => {
    const e = await run({}, (ctx) =>
      createEntry(
        ctx,
        '2026-04-10',
        [
          line(acc.ar, '1100', '0', { partnerId, memo: '請求' }),
          { accountId: acc.sales, credit: '1100', taxCategory: 'standard', taxRate: '0.1' },
        ],
        { description: '売上計上' },
      ),
    );
    ...
    expect(e.lines.journal_line[1]).toMatchObject({
    ...
    await expect(
      run({}, (ctx) => createEntry(ctx, '2026-04-10', [], { sourceId: 'not-a-uuid' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run({}, (ctx) => createEntry(ctx, '2026-04-10', [line(acc.cash, '-1', '0'), line(acc.sales, '0', '1')])),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'debit' }] } });
    await expect(
      run({}, (ctx) => createEntry(ctx, '2026-04-10', [{ accountId: newId(), debit: '1' }, line(acc.sales, '0', '1')])),
    ).rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it('AC-4 submit validates lines (≥2, XOR, exact balance, partner where required) and the period; sets totals; failure leaves the draft', async () => {
    const expectIssue = async (lines: Row[], path: string, message: RegExp) => {
      ...
      expect(
        issues.some((i) => i.path === path && message.test(i.message)),
    ...
    await expectIssue(
      [line(acc.ar, '100', '0'), line(acc.sales, '0', '100')],
      'lines[0].partnerId',
      /requires a partner/,
    );
    // date outside every fiscal period, and a draft whose period is missing: INVALID_STATE with the spec hint
    const noPeriod = await run({}, (ctx) =>
      createEntry(ctx, '2031-01-01', [line(acc.cash, '1', '0'), line(acc.sales, '0', '1')]),
    );
    ...
    const ok = await postEntry('2026-04-11', [
      line(acc.cash, '0.1', '0'),
      line(acc.cash, '0.2', '0'),
      line(acc.sales, '0', '0.3'),
    ]);
    ...
    const bySales = await postEntry(
      '2026-04-11',
      [line(acc.cash, '5', '0'), line(acc.sales, '0', '5')],
      {},
      asRole(['sales']),
    );
    ...
    await expect(
      run(asRole(['viewer']), (ctx) =>
        createEntry(ctx, '2026-04-11', [line(acc.cash, '5', '0'), line(acc.sales, '0', '5')]),
      ),
    ).rejects.toBeInstanceOf(PermissionDenied);
  });
  ...
  it('AC-5 submitted entries are immutable: only description changes; lines frozen on every path; posted/entryDate are system-owned', async () => {
    ...
    const u = (await run({}, (ctx) =>
      runAction(ctx, 'journal_entry.update', { id: e.id, patch: { description: '摘要のみ変更可' } }),
    )) as EntryJson;
    expect(u).toMatchObject({ description: '摘要のみ変更可', version: e.version + 1 });
    await expect(
      run({}, (ctx) => runAction(ctx, 'journal_entry.update', { id: e.id, patch: { date: '2026-04-13' } })),
    ).rejects.toMatchObject({ code: 'INVALID_STATE', details: { blocked: ['date'] } });
    await expect(
      run({}, (ctx) => runAction(ctx, 'journal_entry.update', { id: e.id, patch: { totalDebit: '1' } })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run({}, (ctx) => runAction(ctx, 'journal_entry.update', { id: e.id, patch: { lines: { journal_line: [] } } })),
    ).rejects.toBeInstanceOf(StateError);
    await expect(run({}, (ctx) => repo(ctx, JournalLine).update(lineId, { memo: 'x' }))).rejects.toBeInstanceOf(
      StateError,
    );
    await expect(
      run({}, (ctx) => repo(ctx, JournalLine).update(lineId, { debit: '700.000001' })),
    ).rejects.toBeInstanceOf(StateError);
    await expect(run({}, (ctx) => repo(ctx, JournalLine).update(lineId, { ext: { note: 1 } }))).rejects.toBeInstanceOf(
      StateError,
    );
    await expect(
      run({}, (ctx) => repo(ctx, JournalLine).update(lineId, { posted: true, entryDate: '2026-04-12', memo: 'x' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run({}, (ctx) => repo(ctx, JournalLine).update(lineId, { posted: false }))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
    await expect(
      run({}, (ctx) => repo(ctx, JournalLine).update(lineId, { entryDate: '2026-04-13' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    // Even echoing a stamp requires the owning module's private capability; public writes never impersonate after_submit.
    await expect(
      run({}, (ctx) => repo(ctx, JournalLine).update(lineId, { posted: true, entryDate: '2026-04-12' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    expect(await run({}, (ctx) => repo(ctx, JournalLine).get(lineId))).toMatchObject({
      posted: true,
      entryDate: '2026-04-12',
      memo: 'm',
    });
    await expect(
      run({}, (ctx) => repo(ctx, JournalLine).create({ entryId: e.id, accountId: acc.cash, debit: '1' })),
    ).rejects.toBeInstanceOf(StateError);
    ...
    const d = await run({}, (ctx) =>
      createEntry(ctx, '2026-04-12', [line(acc.cash, '1', '0'), line(acc.sales, '0', '1')]),
    );
    const draftLine = d.lines.journal_line[0]?.id ?? '';
    expect((await run({}, (ctx) => repo(ctx, JournalLine).update(draftLine, { memo: 'editable' }))).memo).toBe(
      'editable',
    );
    await expect(run({}, (ctx) => repo(ctx, JournalLine).update(draftLine, { posted: true }))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
    await expect(
      run({}, (ctx) => repo(ctx, JournalLine).update(draftLine, { entryDate: '2026-04-12' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run({}, (ctx) => repo(ctx, JournalLine).create({ entryId: d.id, accountId: acc.cash, debit: '1', posted: true })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'journal_entry.create', {
          date: '2026-04-12',
          lines: { journal_line: [line(acc.cash, '1', '0', { entryDate: '2026-04-12' })] },
        }),
      ),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run({}, (ctx) => repo(ctx, JournalLine).update(draftLine, { entryId: e.id }))).rejects.toBeInstanceOf(
      ValidationError,
    );
  ...
  it('AC-6 reverse_entry posts the mirror (reversalOf, 逆仕訳: <number>, other line fields kept) and emits journal_entry.reversed; cancel is refused', async () => {
    const e = await postEntry(
      '2026-04-13',
      [
        line(acc.ar, '2200', '0', { partnerId, memo: 'A' }),
        line(acc.sales, '0', '2000', { taxCategory: 'standard', taxRate: '0.1' }),
        line(acc.expense, '0', '200', { memo: 'C' }),
      ],
      { description: '元仕訳' },
    );
    const rev = (await run({}, (ctx) =>
      runAction(ctx, 'accounting.reverse_entry', { id: e.id }),
    )) as EntryWithFlatLines;
    expect(rev).toMatchObject({
      docstatus: 1,
      reversalOf: e.id,
      date: '2026-04-13',
      description: `逆仕訳: ${e.number}`,
      totalDebit: '2200',
      totalCredit: '2200',
      sourceEntity: null,
    });
    ...
    expect(
      rev.lines.map((l) => [l.seq, l.accountId, l.debit, l.credit, l.partnerId, l.memo, l.posted, l.entryDate]),
    ).toEqual([
    ...
    const events = await db.owner.sql<
      { payload: Row }[]
    >`select payload from outbox where topic = ${REVERSED_EVENT} and tenant_id = ${db.tenantId}`;
    expect(events.map((r) => r.payload)).toContainEqual({
      id: e.id,
      number: e.number,
      reversalId: rev.id,
      reversalNumber: rev.number,
    });
    // twice → CONFLICT; a draft → INVALID_STATE; explicit date is honoured (and must be in an open period)
    await expect(run({}, (ctx) => runAction(ctx, 'accounting.reverse_entry', { id: e.id }))).rejects.toBeInstanceOf(
      Conflict,
    );
    const d = await run({}, (ctx) =>
      createEntry(ctx, '2026-04-13', [line(acc.cash, '1', '0'), line(acc.sales, '0', '1')]),
    );
    ...
    await expect(run({}, (ctx) => reverseEntry(ctx, { id: e2.id, date: '2031-01-01' }))).rejects.toMatchObject({
      code: 'INVALID_STATE',
    });
    ...
    await expect(run({}, (ctx) => runAction(ctx, 'journal_entry.cancel', { id: e.id }))).rejects.toBeInstanceOf(
      DependencyError,
    );
    await expect(run({}, (ctx) => runAction(ctx, 'journal_entry.amend', { id: e.id }))).rejects.toBeInstanceOf(
      StateError,
    );
    // permission: submit on journal_entry is required
    await expect(
      run(asRole(['viewer']), (ctx) => runAction(ctx, 'accounting.reverse_entry', { id: e3.id })),
    ).rejects.toBeInstanceOf(PermissionDenied);
  });
  ...
  it('AC-7 post_from_source: create+submit linked to the source in one transaction; a second post is CONFLICT until the entry is reversed', async () => {
    ...
    const input = {
      sourceEntity: 'sales_invoice',
      sourceId,
      date: '2026-04-14',
      description: '請求書 INV-1',
      lines: [line(acc.ar, '1100', '0', { partnerId }), line(acc.sales, '0', '1100')],
    };
    const posted = (await run(asRole(['sales']), (ctx) =>
      runAction(ctx, 'accounting.post_from_source', input),
    )) as EntryWithFlatLines;
    ...
    expect((dup as Conflict).details).toMatchObject({
      sourceEntity: 'sales_invoice',
      sourceId,
      entryId: posted.id,
    ...
    const again = await run({}, (ctx) =>
      postFromSource(ctx, {
        ...input,
        lines: [
          { accountId: acc.ar, debit: Decimal.from('1200'), partnerId },
          { accountId: acc.sales, credit: '1200' },
        ],
      }),
    );
    ...
    expect(
      await run({}, (ctx) => repo(ctx, JournalEntry).count({ sourceEntity: 'sales_invoice', sourceId, docstatus: 1 })),
    ).toBe(2);
    // atomic: an invalid line set leaves no trace of the source
    const bad = newId();
    const err = await caught(
      run({}, (ctx) =>
        runAction(ctx, 'accounting.post_from_source', {
          ...input,
          sourceId: bad,
          lines: [line(acc.cash, '10', '0'), line(acc.sales, '0', '9')],
        }),
      ),
    );
    expect(err).toBeInstanceOf(ValidationError);
    ...
    const closed = await caught(
      run({}, (ctx) => runAction(ctx, 'accounting.post_from_source', { ...input, sourceId: bad, date: '2031-01-01' })),
    );
    expect(closed).toMatchObject({ code: 'INVALID_STATE' });
    expect(await run({}, (ctx) => repo(ctx, JournalEntry).count({ sourceId: bad }))).toBe(0);
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'accounting.post_from_source', { ...input, sourceId: bad, lines: [line(acc.cash, '1', '0')] }),
      ),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'lines' }] } });
    await expect(
      run(asRole(['viewer']), (ctx) => runAction(ctx, 'accounting.post_from_source', { ...input, sourceId: bad })),
    ).rejects.toBeInstanceOf(PermissionDenied);
  });
  ...
  describe('reports (AC-8, AC-9)', () => {
    ...
    beforeAll(async () => {
      tb = {
        a: await createAccount('TB-A', '報告A', 'asset'),
        b: await createAccount('TB-B', '報告B', 'liability'),
        c: await createAccount('TB-C', '報告C', 'revenue'),
      };
      ...
      draftId = (
        await run({}, (ctx) => createEntry(ctx, '2026-04-21', [line(tb.a, '999', '0'), line(tb.c, '0', '999')]))
      ).id;
    });

    it('AC-8 trial_balance: one row per account, opening (before from) / period / closing, totals; posted lines only; TableResult', async () => {
      const t = (await run(asRole(['viewer']), (ctx) =>
        runAction(ctx, 'accounting.trial_balance', { from: '2026-04-01', to: '2026-04-30' }),
      )) as Table;
      ...
      expect(dflt.rows.find((r) => r.code === 'TB-A')).toMatchObject({
        openingDebit: '0',
        periodDebit: '1350',
        closingBalance: '1350',
      });
      await expect(
        run(asRole(['nobody']), (ctx) => runAction(ctx, 'accounting.trial_balance', {})),
      ).rejects.toBeInstanceOf(PermissionDenied);
    });

    it('AC-9 general_ledger: opening row then posted lines in date order with a running balance', async () => {
      const g = (await run(asRole(['viewer']), (ctx) =>
        runAction(ctx, 'accounting.general_ledger', { accountId: tb.a, from: '2026-04-01', to: '2026-04-30' }),
      )) as Table;
      ...
      const c = (await run({}, (ctx) =>
        runAction(ctx, 'accounting.general_ledger', { accountId: tb.c, from: '2026-04-01', to: '2026-05-31' }),
      )) as Table;
      ...
      await expect(
        run({}, (ctx) => runAction(ctx, 'accounting.general_ledger', { accountId: newId() })),
  ...
  it('AC-10 close_period refuses submits dated in it, reopen_period allows them again; both audited; accounting/admin only; a closed year blocks too', async () => {
    ...
    const d = await run({}, (ctx) =>
      createEntry(ctx, '2026-06-15', [line(acc.cash, '1', '0'), line(acc.sales, '0', '1')]),
    );
    const err = await caught(run({}, (ctx) => submit(ctx, d.id)));
    expect(err).toMatchObject({
      code: 'INVALID_STATE',
      hint: expect.stringContaining(PERIOD_HINT),
      details: { periodId, code: '2026-06' },
    });
    await expect(
      run({}, (ctx) =>
        postFromSource(ctx, {
          ...
          sourceId: newId(),
          date: '2026-06-30',
          lines: [line(acc.cash, '1', '0'), line(acc.sales, '0', '1')] as never,
        }),
      ),
    ).rejects.toMatchObject({ code: 'INVALID_STATE' });
    expect((await postEntry('2026-07-01', [line(acc.cash, '1', '0'), line(acc.sales, '0', '1')])).docstatus).toBe(1);
    await expect(
      run(accounting, (ctx) => runAction(ctx, 'accounting.close_period', { periodId })),
    ).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(
      run(asRole(['sales']), (ctx) => runAction(ctx, 'accounting.reopen_period', { periodId })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run(asRole(['viewer']), (ctx) => runAction(ctx, 'accounting.close_period', { periodId: newId() })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    ...
    const updates = trail
      .filter((t) => t.op === 'update')
      .map((t) => [t.actorType, (t.before as Row).isClosed, (t.after as Row).isClosed]);
    ...
    const d2 = await run({}, (ctx) =>
      createEntry(ctx, '2026-08-01', [line(acc.cash, '1', '0'), line(acc.sales, '0', '1')]),
    );
    await expect(run({}, (ctx) => submit(ctx, d2.id))).rejects.toMatchObject({
      code: 'INVALID_STATE',
      details: { code: 'FY2026' },
    });
  ...
  describe('AC-12 properties against the database', () => {
    ...
    const arbBalanced = () =>
      fc
        .record({
          debits: fc.array(fc.tuple(arbAccount(), arbCents), { minLength: 1, maxLength: 4 }),
          credits: fc.array(fc.tuple(arbAccount(), fc.integer({ min: 1, max: 1000 })), { minLength: 1, maxLength: 4 }),
        })
        .map(({ debits, credits: all }) => {
          ...
          return [
            ...debits.map(([a, c]) => line(a, cents(c), '0')),
            ...credits.map(([a], i) => line(a, '0', cents(parts[i] ?? 1))),
          ];
        });
    const arbUnbalanced = () =>
      fc.tuple(arbBalanced(), fc.integer({ min: 1, max: 100_000 }), fc.nat()).map(([lines, delta, pick]) => {
        ...
        return lines.map((l, j) =>
          j === i
            ? {
                ...l,
                [l.debit === '0' ? 'credit' : 'debit']: Decimal.from(String(l.debit === '0' ? l.credit : l.debit))
                  .plus(cents(delta))
                  .toString(),
              }
            : l,
    ...
    it('random unbalanced line sets never submit (VALIDATION on the totals) and leave the draft unposted', async () => {
      await fc.assert(
        fc.asyncProperty(arbUnbalanced(), async (lines) => {
          ...
          expect(err).toBeInstanceOf(ValidationError);
          expect(
            ((err as ValidationError).details as { issues: { path: string; message: string }[] }).issues.some(
              (i) => i.path === 'lines' && /differ by/.test(i.message),
            ),
          ).toBe(true);
          const still = await run({}, (ctx) => repo(ctx, JournalEntry).get(d.id));
    ...
    it('reversing any submitted entry leaves every account net (Σdebit − Σcredit over posted lines) unchanged — aggregate port', async () => {
      await fc.assert(
        fc.asyncProperty(arbBalanced(), fc.constantFrom('2026-02-12', '2026-03-03'), async (lines, reversalDate) => {
          ...
          expect(rev.lines.map((l) => [l.accountId, l.debit.toString(), l.credit.toString()])).toEqual(
            e.lines.journal_line.map((l) => [l.accountId, l.credit, l.debit]),
          );
