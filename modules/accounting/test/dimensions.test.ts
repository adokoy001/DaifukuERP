...
const source = defineEntity({ name: 'dimension_source', label: label('元', 'Source'), fields: { name: f.text() }, permissions: { roles: { viewer: ['read'] } } });
const target = defineEntity({ name: 'dimension_target', label: label('先', 'Target'), fields: { name: f.text() }, permissions: { roles: { viewer: ['read'] } } });
...
describe('explicit posting dimensions', () => {
  it('copies only keys registered on both sides and preserves null values', () => {
    expect(postingDimensions(source.name, target.name, { department: 'Operations', sourceNote: 'private', unknown: 'x' })).toEqual({ department: 'Operations' });
