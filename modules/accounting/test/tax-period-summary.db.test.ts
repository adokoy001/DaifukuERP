...
const dr = (code: string, amount: string, extra: Partial<LineInput> = {}): LineInput => ({
  accountId: id(code),
  debit: amount,
  ...extra,
});
const cr = (code: string, amount: string, extra: Partial<LineInput> = {}): LineInput => ({
  accountId: id(code),
  credit: amount,
  ...extra,
});
...
async function post(date: string, lines: LineInput[], description: string): Promise<string> {
  ...
  const entry = await run({}, (ctx) =>
    postFromSource(ctx, { sourceEntity: 'test_source', sourceId, date, description, lines }),
  );
  sources.set(entry.id, sourceId);
  return entry.id;
}
const summary = (from: string, to: string, params: Partial<ContextParams> = {}) =>
  run(params, async (ctx) => (await runAction(ctx, 'accounting.tax_period_summary', { from, to })) as Table);
/** Rows as [side, taxCategory, taxRate, taxableAmount, taxAmount, count]. */
const tuples = (t: Table) =>
  t.rows.map((r) => [r.side, r.taxCategory, r.taxRate, r.taxableAmount, r.taxAmount, r.count]);
...
beforeAll(async () => {
  ...
  for (const [code, name, type, extra] of chart)
    acc[code] = (await run({}, (ctx) => repo(ctx, Account).create({ code, name, type, ...extra }))).id;
});
afterAll(async () => {
  await db.close();
});
...
describe('accounting.tax_period_summary (docs/specs/tax-period-summary.md)', () => {
  it('AC-1 account.taxRole: enum none|output_tax|input_tax, default none, label 消費税の役割; accounting can set it', async () => {
    ...
    await expect(
      run({}, (ctx) => repo(ctx, Account).create({ code: 'X1', name: 'x', type: 'asset', taxRole: 'vat' as never })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'taxRole' }] } });
    const created = await run(asRole(['accounting']), (ctx) =>
      repo(ctx, Account).create({ code: 'X2', name: '仮受消費税（旧）', type: 'liability' }),
    );
    expect(created.taxRole).toBe('none');
    expect(
      (await run(asRole(['accounting']), (ctx) => repo(ctx, Account).update(created.id, { taxRole: 'output_tax' })))
        .taxRole,
    ).toBe('output_tax');
    await run({}, (ctx) => repo(ctx, Account).delete(created.id));
    const field = await run({}, async (ctx) =>
      appMeta(ctx)
        .entities.find((e) => e.name === 'account')
        ?.fields.find((f) => f.name === 'taxRole'),
    );
    expect(field).toMatchObject({
      ...
      required: true,
      hasDefault: true,
      values: ['none', 'output_tax', 'input_tax'],
    });
  });
  ...
  describe('AC-6 台本 10 月分（scenario-kojin.md §消費税の集計）', () => {
    beforeAll(async () => {
      ...
      await post(
        '2026-10-05',
        [
          dr('1300', '165000', { partnerId: customer }),
          cr('4000', '100000', STD),
          cr('4000', '50000', STD),
          cr('2200', '15000', STD),
        ],
      ...
      await post(
        '2026-10-20',
        [dr('1300', '33000', { partnerId: customer }), cr('4000', '30000', STD), cr('2200', '3000', STD)],
        'INV2',
      );
      await post(
        '2026-10-28',
        [
          dr('1300', '64900', { partnerId: customer }),
          cr('4000', '50000', STD),
          cr('4000', '9000', STD),
          cr('2200', '5900', STD),
        ],
      ...
      await post(
        '2026-10-08',
        [dr('6960', '20000', STD), dr('1500', '2000', STD), cr('2100', '22000', { partnerId: supplier })],
        'BILL1',
      );
      await post(
        '2026-10-15',
        [
          dr('6980', '50000', STD),
          dr('1500', '3500', STD),
          dr('6980', '1500', { memo: '控除対象外消費税' }),
          cr('2100', '55000', { partnerId: supplier }),
        ],
        'BILL2 免税事業者 70%',
      );
      await post(
        '2026-10-25',
        [
          dr('6200', '80000', { taxCategory: 'non_taxable', taxRate: '0' }),
          cr('2100', '80000', { partnerId: supplier }),
        ],
        'BILL3 家賃',
      );
      await post(
        '2026-10-30',
        [dr('6970', '5000', RED), dr('1500', '400', RED), cr('2100', '5400', { partnerId: supplier })],
        'BILL4 軽減 8%',
      );
    ...
    it('AC-2 TableResult: title 消費税集計表, columns, meta; viewer/accounting/admin may read, others PERMISSION_DENIED; from > to VALIDATION', async () => {
      ...
      await expect(summary('2026-10-31', '2026-10-01')).rejects.toMatchObject({
        code: 'VALIDATION',
        details: { issues: [{ path: 'to' }] },
      });
      await expect(
        run({}, (ctx) => runAction(ctx, 'accounting.tax_period_summary', { from: '2026-10-01' })),
      ).rejects.toMatchObject({ code: 'VALIDATION' });
      await expect(
        run({}, (ctx) => runAction(ctx, 'accounting.tax_period_summary', { from: '2026-10-01', to: '2026/10/31' })),
      ).rejects.toMatchObject({ code: 'VALIDATION' });
      // a period with no taxed lines is an empty table with zero totals
      expect(await summary('2026-09-01', '2026-09-30')).toMatchObject({
        rows: [],
        totals: { output_tax_total: '0', input_tax_total: '0', net_tax_due: '0' },
      });
    });
  });
  ...
  it('AC-2 dates inclusive, submitted entries only, reversed pairs net to zero, credit notes subtract', async () => {
    const n1 = await post(
      '2026-11-01',
      [dr('1300', '11000', { partnerId: customer }), cr('4000', '10000', STD), cr('2200', '1000', STD)],
      'N1 (from)',
    );
    await post(
      '2026-11-30',
      [dr('1300', '2200', { partnerId: customer }), cr('4000', '2000', STD), cr('2200', '200', STD)],
      'N2 (to)',
    );
    await post(
      '2026-11-20',
      [dr('2100', '1100', { partnerId: supplier }), cr('6960', '1000', STD), cr('1500', '100', STD)],
      'N3 仕入返品',
    );
    await post(
      '2026-12-01',
      [dr('1300', '5500', { partnerId: customer }), cr('4000', '5000', STD), cr('2200', '500', STD)],
      'N4 (after to)',
    );
    await run({}, (ctx) =>
    ...
    await run({}, (ctx) =>
      runAction(ctx, 'journal_entry.create', {
        ...
        lines: {
          journal_line: [
            dr('1300', '99000', { partnerId: customer }),
            cr('4000', '90000', STD),
            cr('2200', '9000', STD),
          ],
        },
    ...
    expect(tuples(await summary('2026-11-15', '2026-11-15'))).toEqual([
      ['売上', 'standard', '0.10', '-10000', '-1000', 2],
    ]);
  });
  ...
  it('AC-4 仮受/仮払 lines without taxCategory/taxRate are counted under (side, unclassified, "") — nothing silently dropped', async () => {
    await post(
      '2026-12-05',
      [dr('1300', '1100', { partnerId: customer }), cr('4000', '1000', STD), cr('2200', '100')],
      'D1 税区分なしの仮受',
    );
    await post(
      '2026-12-06',
      [
        dr('6960', '500', STD),
        dr('1500', '50', { taxCategory: 'standard' }),
        cr('2100', '550', { partnerId: supplier }),
      ],
      'D2 税率なしの仮払',
    );
    await post(
      '2026-12-07',
      [dr('1300', '3000', { partnerId: customer }), cr('4000', '3000', { taxCategory: 'exempt', taxRate: '0' })],
      'D3 輸出免税',
    );
