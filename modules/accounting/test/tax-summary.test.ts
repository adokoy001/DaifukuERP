...
import { classifyGroup, formatRate, taxSummaryRows, type TaxAccountInfo, type TaxLineGroup } from '../src/services/tax-summary.ts';
...
const g = (accountId: string, debit: string, credit: string, taxCategory: string | null = null, taxRate: string | null = null, lines = 1): TaxLineGroup => ({
  accountId,
...
describe('tax period summary (pure)', () => {
  it('uses the recorded classification and refuses unknown legacy classification', () => {
    ...
    expect(classifyGroup(posted, { type: 'expense', taxRole: 'input_tax' })).toMatchObject({ side: '売上', kind: 'base', amount: D('1000') });
    const legacy = { ...posted };
    delete legacy.accountAtPosting;
    expect(() => classifyGroup(legacy, { type: 'revenue', taxRole: 'none' })).toThrow('Historical account classification is unavailable');
  });
  it('formatRate keeps at least two decimals and never drops digits', () => {
    expect([formatRate(D('0.1')), formatRate(D('0.08')), formatRate(D('0.100000')), formatRate(D('0')), formatRate(D('0.075')), formatRate(D('1'))]).toEqual(['0.10', '0.08', '0.10', '0.00', '0.075', '1.00']);
  });
  ...
  it('AC-2 classification: revenue → 売上 (credit − debit); expense/asset → 仕入 (debit − credit); taxRole lines are tax; no category → skipped', () => {
    ...
    expect(classifyGroup(g('sales', '100', '1000', 'standard', '0.1'), acc('sales'))).toMatchObject({ side: '売上', category: 'standard', kind: 'base', amount: D('900') });
    expect(classifyGroup(g('rent', '80000', '0', 'non_taxable', '0'), acc('rent'))).toMatchObject({ side: '仕入', category: 'non_taxable', kind: 'base', amount: D('80000') });
    expect(classifyGroup(g('equipment', '300000', '0', 'standard', '0.1'), acc('equipment'))).toMatchObject({ side: '仕入', kind: 'base', amount: D('300000') });
    expect(classifyGroup(g('fees', '0', '1000', 'standard', '0.1'), acc('fees'))).toMatchObject({ side: '仕入', kind: 'base', amount: D('-1000') });
    expect(classifyGroup(g('outputTax', '10', '100', 'standard', '0.1'), acc('outputTax'))).toMatchObject({ side: '売上', kind: 'tax', amount: D('90') });
    expect(classifyGroup(g('inputTax', '100', '10', 'reduced', '0.08'), acc('inputTax'))).toMatchObject({ side: '仕入', category: 'reduced', kind: 'tax', amount: D('90') });
    ...
    expect(() => classifyGroup(g('ghost', '1', '0', 'standard', '0.1'), undefined)).toThrow('Historical account classification is unavailable');
  });
  ...
  it('AC-3/AC-4 rows per (side, category, rate) with totals; tax lines without category or rate go to (side, unclassified, "")', () => {
    ...
    expect(rows).toEqual([
      { side: '売上', taxCategory: 'standard', taxCategoryLabel: '標準税率', taxRate: '0.10', taxableAmount: '239000', taxAmount: '23900', count: 8 },
      { side: '売上', taxCategory: 'unclassified', taxCategoryLabel: '未分類', taxRate: '', taxableAmount: '0', taxAmount: '100', count: 1 },
      { side: '仕入', taxCategory: 'standard', taxCategoryLabel: '標準税率', taxRate: '0.10', taxableAmount: '70000', taxAmount: '5500', count: 4 },
      { side: '仕入', taxCategory: 'reduced', taxCategoryLabel: '軽減税率', taxRate: '0.08', taxableAmount: '5000', taxAmount: '400', count: 2 },
      { side: '仕入', taxCategory: 'non_taxable', taxCategoryLabel: '非課税', taxRate: '0.00', taxableAmount: '80000', taxAmount: '0', count: 1 },
      { side: '仕入', taxCategory: 'unclassified', taxCategoryLabel: '未分類', taxRate: '', taxableAmount: '0', taxAmount: '50', count: 1 },
    ]);
  ...
  it('several rates of one category sort high to low; base lines without a rate sort last', () => {
    const { rows } = taxSummaryRows([g('sales', '0', '1', 'standard', null), g('sales', '0', '10', 'standard', '0.08'), g('sales', '0', '100', 'standard', '0.1')], ACCOUNTS);
  ...
  describe('properties', () => {
    ...
    const toGroup = (x: { accountId: string; side: boolean; amount: string; taxCategory: string | null; taxRate: string | null }): TaxLineGroup =>
      g(x.accountId, x.side ? x.amount : '0', x.side ? '0' : x.amount, x.taxCategory, x.taxRate);
    ...
    it('nothing on a taxRole account is dropped: Σ taxAmount per side equals the totals, net = output − input', () => {
      fc.assert(
        fc.property(fc.array(arbGroup, { maxLength: 20 }), (xs) => {
          ...
          const expectedOutput = Decimal.sum(groups.filter((x) => x.accountId === 'outputTax').map((x) => x.credit.minus(x.debit)));
          const expectedInput = Decimal.sum(groups.filter((x) => x.accountId === 'inputTax').map((x) => x.debit.minus(x.credit)));
