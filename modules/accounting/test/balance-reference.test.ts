...
const line = (debit: number, credit: number) => ({ debit: Decimal.from(debit), credit: Decimal.from(credit), seq: 1, partnerId: null, partnerRequired: false });
...
describe('AC-6 independent accounting references', () => {
  it('reports actionable errors even when invalid lines have equal debit and credit totals', () => {
    ...
    for (const amounts of [[0, 0], [1, 1], [-1, -1]] as const) {
  ...
  it('retains the keys and exact nonzero net of independent integer postings', () => {
    const entry = fc.record({ key: fc.constantFrom('cash', 'sales', 'cost'), debit: fc.integer({ min: 0, max: 1000000 }), credit: fc.integer({ min: 0, max: 1000000 }) });
    fc.assert(fc.property(fc.array(entry, { maxLength: 25 }), (generated) => {
      // The prefix guarantees positive, negative and zero-net keys even if the tail shrinks to empty.
      const entries = [{ key: 'positive', debit: 17, credit: 3 }, { key: 'negative', debit: 1, credit: 9 }, { key: 'zero', debit: 4, credit: 4 }, ...generated];
      const expected = new Map<string, bigint>();
      for (const entry of entries) expected.set(entry.key, (expected.get(entry.key) ?? 0n) + BigInt(entry.debit) - BigInt(entry.credit));
      const actual = netByKey(entries.map((entry) => ({ ...line(entry.debit, entry.credit), key: entry.key })), (entry) => entry.key);
