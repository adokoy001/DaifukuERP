...
async function prepare(ctx: Context, input: z.infer<typeof bankImportInput>) {
  ...
  const checked = rows.map((row) => {
    ...
    if (previous && previous !== sourceHash(row)) throw new Conflict('既存の明細と異なる内容が指定されました。', '同じ externalId の元の取引を確認してください。', { externalId: row.externalId });
    return { ...row, duplicate: previous !== undefined };
  });
  return { bankAccountId: account.id, previewHash: contentHash(JSON.stringify([account.id, account.version, rows])), rowCount: rows.length, newCount: checked.filter((row) => !row.duplicate).length, duplicateCount: checked.filter((row) => row.duplicate).length, ...statementTotals(rows), rows: checked };
}
export function previewImport(ctx: Context, input: z.infer<typeof bankImportInput>) { return bankLock(ctx, () => prepare(ctx, input)); }
export function importCsv(ctx: Context, input: z.infer<typeof bankImportCommitInput>) {
  return bankLock(ctx, async () => {
    const preview = await prepare(ctx, input); checkRequest(preview.previewHash, input.previewHash);
    ...
    if (prior) return { importId: prior.id, imported: prior.imported, duplicates: prior.duplicates, previewHash: prior.contentHash };
    const imported = await bankWrite(ctx, BankImport, (inner) => repo(inner, BankImport).create({ bankAccountId: input.bankAccountId, importKey, contentHash: preview.previewHash, rowCount: preview.rowCount, imported: preview.newCount, duplicates: preview.duplicateCount }));
    await bankWrite(ctx, BankStatement, async (inner) => {
      for (const { duplicate, ...row } of preview.rows) if (!duplicate) await repo(inner, BankStatement).create({ ...row, bankAccountId: input.bankAccountId, importId: imported.id, identityKey: identityKey(input.bankAccountId, row.externalId), contentHash: sourceHash(row) });
    });
    return { importId: imported.id, imported: imported.imported, duplicates: imported.duplicates, previewHash: preview.previewHash };
  });
}
