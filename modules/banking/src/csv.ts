...
function invalid(message: string, row = 0): never {
  throw new ValidationError('銀行 CSV を確認してください。', [{ path: `csv.${row}`, message }]);
}
...
export function parseStatementCsv(raw: string): BankSource[] {
  if (new TextEncoder().encode(raw).length > 500_000 || raw.includes('\0') || raw.includes('\ufffd'))
    invalid('UTF-8、500 KB 以内のファイルが必要です。');
  const rows = cells(raw.replace(/^\uFEFF/u, ''));
  if (JSON.stringify(rows.shift()) !== JSON.stringify(HEADER))
    invalid(`ヘッダーは ${HEADER.join(',')} にしてください。`);
  ...
  return rows.map((row, index) => {
    ...
    if (!result.success)
      invalid(result.error.issues.map((issue) => `${issue.path.join('.')}: ${issue.message}`).join('; '), index + 2);
    if (
      /^[\s]|[\s]$/u.test(result.data.externalId) ||
      [...result.data.externalId].some((char) => char.charCodeAt(0) < 32 || char.charCodeAt(0) === 127)
    )
      invalid('externalId の空白・制御文字は使用できません。', index + 2);
...
export function statementTotals(rows: readonly BankSource[]) {
  const sum = (direction: BankSource['direction']) =>
    rows
      .filter((row) => row.direction === direction)
      .reduce((total, row) => total.plus(row.amount), Decimal.zero())
      .toString();
