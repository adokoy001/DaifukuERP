...
import {
  bankAccountView,
  bankBoardOutput,
  bankCandidatesOutput,
  bankImportPreview,
  bankImportResult,
  bankPayeeView,
  bankReconciliationView,
  bankTransferDetail,
  bankTransferExportOutput,
  bankTransferView,
...
describe('banking immutable import and finance boundary', () => {
  it('previews without mutation and imports once under concurrent retries; stable IDs conflict on changed content', async () => {
    ...
    const preview = await f.action(
      'preview_import',
      { bankAccountId: f.account.id, csv },
      bankImportPreview,
      f.accounting,
    );
    ...
    const results = await Promise.all([
      f.action('import_statement_csv', command, bankImportResult, f.accounting),
      f.action('import_statement_csv', command, bankImportResult, f.accounting),
    ]);
    expect(results[0]).toEqual(results[1]);
    expect(await f.run((ctx) => repo(ctx, BankImport).count())).toBe(1);
    expect(
      (await f.action('preview_import', { bankAccountId: f.account.id, csv }, bankImportPreview)).duplicateCount,
    ).toBe(2);
    await expect(
      f.action('preview_import', { bankAccountId: f.account.id, csv: csv.replace('1000', '1001') }, bankImportPreview),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
    await expect(
      f.action('import_statement_csv', { ...command, previewHash: '0'.repeat(64) }, bankImportResult),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
  ...
  it('denies general roles, site/store contexts, relay and cross company/tenant; hides full account from generic output', async () => {
    for (const params of [
      { roles: ['viewer'] },
      { roles: ['sales'] },
      { roles: ['accounting'], accessScope: 'sites' as const, siteIds: [newId()] },
      { roles: ['admin'], accessScope: 'stores' as const, storeIds: [newId()] },
      { roles: ['accounting'], actor: { type: 'relay' as const, id: newId() } },
    ]) {
      await expect(f.action('board', {}, bankBoardOutput, params)).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
      await expect(f.run((ctx) => repo(ctx, BankStatement).list(), params)).rejects.toMatchObject({
        code: 'PERMISSION_DENIED',
      });
    }
    const otherCompanyId = newId();
    await f.db.owner.drizzle
      .insert(companies)
      .values({ id: otherCompanyId, tenantId: f.db.tenantId, code: 'OTHER', name: '別会社' });
    const other = await bootstrapTenant(f.db.owner, {
      tenantName: 'Other bank tenant',
      companyCode: 'OT',
      companyName: '別テナント',
      adminEmail: 'other-bank@example.com',
      adminName: 'Other',
    ...
    for (const params of [{ companyId: otherCompanyId }, { tenantId: other.tenantId, companyId: other.companyId }]) {
      await expect(
        f.action(
          'preview_import',
          { bankAccountId: f.account.id, csv: `${CSV_HEADER}\nx,2026-09-01,pay,1,x` },
          bankImportPreview,
          params,
        ),
    ...
    expect(output).not.toHaveProperty('accountNumber');
    expect(output).not.toHaveProperty('requesterCode');
    const board = await f.action('board', {}, bankBoardOutput, f.accounting);
    expect(board.accounts[0]?.accountNumberMasked).toBe('****543');
    ...
    expect(JSON.stringify(audit)).not.toContain('9876543');
    expect(JSON.stringify(audit)).not.toContain('0123456789');
  });
  it('refuses generic writes/deletes and changes to used account identity', async () => {
    ...
    await expect(
      f.run((ctx) => runAction(ctx, 'bank_statement.update', { id: statement.id, patch: { description: '変更' } })),
    ).rejects.toThrow();
    await expect(f.run((ctx) => repo(ctx, BankStatement).delete(statement.id))).rejects.toThrow();
    await expect(
      f.action(
        'save_account',
        { ...f.accountInput, id: f.account.id, expectedVersion: f.account.version, accountNumber: '1234567' },
        bankAccountView,
      ),
    ).rejects.toThrow('使用済み');
    expect((await f.run((ctx) => repo(ctx, BankAccount).get(f.account.id))).accountNumber).toBe('9876543');
    await expect(f.action('save_account', { ...f.accountInput, code: 'ALIAS' }, bankAccountView)).rejects.toThrow(
      '登録済み',
    );
  });
});
describe('failure and concurrent request recovery', () => {
  it('rolls back draft, allocations, invoice balance and link when posting fails after settlement has begun', async () => {
    ...
    const input = {
      requestId: newId(),
      statementId: required(imported.statements[0]).id,
      targetKind: 'invoice',
      targetId: invoice.id,
      targetVersion: invoice.version,
      expectedBalance: '321',
      reason: '確認済み',
    };
    const before = await f.run(async (ctx) => ({
      payments: await repo(ctx, Payment).count(),
      journal: await repo(ctx, JournalEntry).count(),
    }));
    const settings = {
      cash: '1000',
      bank: '1100',
    ...
    await f.run((ctx) =>
      setSetting(ctx, PAYMENT_ACCOUNTS_KEY, paymentAccountsSchema, { ...settings, advanceReceived: 'missing' }),
    );
    try {
      await expect(f.action('reconcile', input, bankReconciliationView)).rejects.toMatchObject({
        code: 'INVALID_STATE',
      });
    } finally {
      await f.run((ctx) => setSetting(ctx, PAYMENT_ACCOUNTS_KEY, paymentAccountsSchema, settings));
    }
  ...
  it('serializes different confirmation IDs; replaying an old undo never undoes a later confirmed match', async () => {
    ...
    const results = await Promise.allSettled([
      f.action('reconcile', input, bankReconciliationView),
      f.action('reconcile', { ...input, requestId: newId() }, bankReconciliationView),
    ]);
    expect(results.filter((result) => result.status === 'fulfilled')).toHaveLength(1);
    const success = results.find((result) => result.status === 'fulfilled');
    if (success?.status !== 'fulfilled') throw new Error('Expected successful reconciliation');
    ...
    const undo = {
      reconciliationId: success.value.id,
      expectedVersion: success.value.version,
    ...
    await expect(
      f.action('reconcile', { ...input, requestId: newId(), targetVersion: restored.version }, bankReconciliationView),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
    const newRequestId = newId();
    const newer = await f.action(
      'reconcile',
      { ...input, requestId: newRequestId, targetVersion: restored.version, paymentDate: '2026-09-06' },
      bankReconciliationView,
    );
    await f.action('undo_reconciliation', undo, bankReconciliationView);
    ...
    await expect(
      f.action('reconcile', { ...input, requestId: newRequestId, reason: 'changed' }, bankReconciliationView),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
  });
  it('keeps sensitive bank numbers and whole CSV out of physically stored audit records', async () => {
    const rows = await f.db.owner.drizzle
    ...
    expect(serialized).not.toContain(CSV_HEADER);
    expect(serialized).not.toContain('exportBytes');
  });
  it('serializes confirmation against generic payment cancellation without active links to cancelled payments', async () => {
    const payment = await f.run(async (ctx) => {
      ...
      return repo(ctx, Payment).get(draft.id);
    });
    ...
    const input = {
      ...
      targetKind: 'payment',
      targetId: payment.id,
      targetVersion: payment.version,
      expectedBalance: '777',
      reason: '確認',
    };
    const results = await Promise.allSettled([
      f.action('reconcile', input, bankReconciliationView),
      f.run((ctx) => runAction(ctx, 'payment.cancel', { id: payment.id })),
    ]);
    expect(results.filter((result) => result.status === 'fulfilled')).toHaveLength(1);
    const current = await f.run((ctx) => repo(ctx, Payment).get(payment.id));
    const active = await f.run((ctx) =>
      repo(ctx, BankReconciliation).count({ paymentId: payment.id, state: 'active' }),
    );
...
describe('explicit reconciliation and reversal', () => {
  it('creates exactly one submitted payment and journal on concurrent confirmation; reverses source-owned payment atomically', async () => {
    ...
    expect(candidates.candidates.find((row) => row.targetId === invoice.id)).toMatchObject({
      targetKind: 'invoice',
      balance: '5000',
    });
    const input = {
    ...
    const [one, two] = await Promise.all([
      f.action('reconcile', input, bankReconciliationView, f.accounting),
      f.action('reconcile', input, bankReconciliationView, f.accounting),
    ]);
    expect(one).toEqual(two);
    expect(one.createdPayment).toBe(true);
    expect((await f.run((ctx) => repo(ctx, SalesInvoice).get(invoice.id))).balance.toString()).toBe('3000');
    expect(
      await f.run((ctx) => repo(ctx, JournalEntry).count({ sourceEntity: 'payment', sourceId: one.paymentId })),
    ).toBe(1);
    await expect(f.run((ctx) => runAction(ctx, 'payment.cancel', { id: one.paymentId }))).rejects.toThrow('銀行照合');
    await expect(f.action('reconcile', { ...input, requestId: newId() }, bankReconciliationView)).rejects.toMatchObject(
      { code: 'CONFLICT' },
    );
    const undo = {
      reconciliationId: one.id,
    ...
    expect(await f.action('undo_reconciliation', undo, bankReconciliationView)).toEqual(reversed);
  ...
  it('links existing payment with no new journal; unlink leaves its original ledger intact', async () => {
    ...
    expect(matched.createdPayment).toBe(false);
    expect(await f.run((ctx) => repo(ctx, JournalEntry).count())).toBe(before);
    await f.action(
      'undo_reconciliation',
      {
        reconciliationId: matched.id,
        expectedVersion: matched.version,
        correctionDate: '2026-09-03',
        reason: '関連誤り',
      },
      bankReconciliationView,
    );
    expect((await f.run((ctx) => repo(ctx, Payment).get(payment.id))).docstatus).toBe(1);
    expect(await f.run((ctx) => repo(ctx, JournalEntry).count())).toBe(before);
  });
  it('rejects stale target/balance and overpayment without leaving drafts, ledger or links', async () => {
    const invoice = await f.invoice('receive', '100');
    const imported = await f.importRows('oversize,2026-09-03,receive,101,過大');
    const input = {
      requestId: newId(),
      statementId: required(imported.statements[0]).id,
      targetKind: 'invoice',
    ...
    await expect(
      f.action('reconcile', { ...input, targetVersion: invoice.version + 1 }, bankReconciliationView),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
    await expect(
      f.action('reconcile', { ...input, expectedBalance: '99' }, bankReconciliationView),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
    await expect(f.action('reconcile', input, bankReconciliationView)).rejects.toMatchObject({ code: 'INVALID_STATE' });
    expect(await f.run((ctx) => repo(ctx, Payment).count())).toBe(before);
    expect(await f.run((ctx) => repo(ctx, BankReconciliation).count({ statementId: input.statementId }))).toBe(0);
  });
});
describe('transfer preparation is never bank submission', () => {
  it('freezes reviewed account/invoice snapshot and rejects stale export; cancellation frees reservations', async () => {
    ...
    const input = {
      requestId: newId(),
      bankAccountId: f.account.id,
      accountVersion: f.account.version,
    ...
    await expect(
      f.action('prepare_transfer', { ...input, requestId: newId() }, bankTransferDetail),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
    const payee = await f.action(
      'save_payee',
      { ...f.payeeInput, id: f.payee.id, expectedVersion: f.payee.version, accountNumber: '1234567' },
      bankPayeeView,
    );
    await expect(
      f.action(
        'export_transfer',
        { batchId: batch.id, expectedVersion: batch.version, format: 'zengin120', lineEnding: 'none' },
        bankTransferExportOutput,
      ),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
    const cancelled = await f.action(
      'cancel_transfer',
      { batchId: batch.id, expectedVersion: batch.version, reason: '未送信、振込先を修正' },
      bankTransferView,
    );
    expect(cancelled.state).toBe('cancelled');
    f.payee = payee;
    f.payeeInput.accountNumber = '1234567';
    const next = await f.action(
      'prepare_transfer',
      { ...input, requestId: newId(), items: [{ ...required(input.items[0]), payeeVersion: payee.version }] },
      bankTransferDetail,
    );
  ...
  it('exports deterministic bytes and retry result without payments or ledger, preserving full data outside generic outputs', async () => {
    ...
    expect(file.sentToBank).toBe(false);
    expect(file.bytes).toHaveLength(488);
    expect(file.batch.contentHash).toBe(contentHashBytes(Uint8Array.from(file.bytes)));
    ...
    const generic = await f.run((ctx) => runAction(ctx, 'bank_transfer.get', { id: batch.id }));
    expect(generic).not.toHaveProperty('snapshot');
    expect(generic).not.toHaveProperty('exportBytes');
    const audit = await f.run((ctx) => auditTrail(ctx, BankTransfer.name, batch.id));
    expect(JSON.stringify(audit)).not.toContain('exportBytes');
    expect(JSON.stringify(audit)).not.toContain('1234567');
    const storedAudit = await f.db.owner.drizzle
      .select({ entity: auditLog.entity, before: auditLog.before, after: auditLog.after })
      .from(auditLog);
    const exportedAudit = JSON.stringify(storedAudit.filter((row) => row.entity === BankTransfer.name));
    expect(exportedAudit).not.toContain('exportBytes');
    expect(exportedAudit).not.toContain('1234567');
    expect(exportedAudit).not.toContain('9876543');
    await expect(
      f.action('transfer_detail', { batchId: batch.id }, bankTransferDetail, { roles: ['viewer'] }),
    ...
    await expect(
      f.action('export_transfer', { ...input, format: 'canonical_csv' }, bankTransferExportOutput),
    ).rejects.toMatchObject({ code: 'CONFLICT' });
  });
  it('requires explicit format validation and role at action boundaries', async () => {
    await expect(
      f.action('save_account', { ...f.accountInput, code: 'BAD', holderKana: '漢字' }, z.unknown()),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(f.action('board', {}, bankBoardOutput, { companyId: null })).rejects.toMatchObject({
      code: 'PERMISSION_DENIED',
    });
  });
});
