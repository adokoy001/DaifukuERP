...
export async function bankingFixture() {
  ...
  const run = <T>(fn: (ctx: Context) => Promise<T>, params: Partial<ContextParams> = {}) =>
    db.run({ now: () => new Date('2026-09-13T03:00:00Z'), ...params }, fn);
  const action = async <S extends z.ZodType>(
    name: string,
    input: unknown,
    schema: S,
    params: Partial<ContextParams> = {},
  ): Promise<z.infer<S>> => schema.parse(await run((ctx) => runAction(ctx, `banking.${name}`, input), params));
  ...
  await run(async (ctx) => {
    ...
    for (const [code, type] of defs)
      chart.set(
        code,
        (
          await repo(ctx, Account).create({
            code,
            name: `テスト ${code}`,
            type,
            partnerRequired: code === '1300' || code === '2100',
          })
        ).id,
      );
  });
  ...
  const partner = await run((ctx) =>
    repo(ctx, Partner).create({ name: 'カ)テスト', isCustomer: true, isSupplier: true }),
  );
  const accountInput = {
    ...syntheticBank,
    code: 'MAIN',
  ...
  const importRows = async (body: string, bankAccountId = account.id) => {
    ...
    const imported = await action(
      ...
      { bankAccountId, csv, previewHash: preview.previewHash },
      bankImportResult,
    );
    const statements = await run((ctx) =>
      repo(ctx, BankStatement).list({ where: { importId: imported.importId }, limit: 500 }),
    );
    return { csv, preview, imported, statements: statements.items };
  };
  const invoice = async (direction: 'receive' | 'pay', amount: string) =>
    run(async (ctx) => {
      const entity = direction === 'receive' ? SalesInvoice : PurchaseInvoice;
      const created = (await runAction(ctx, `${entity.name}.create`, {
        partnerId: partner.id,
        date: '2026-09-01',
        lines: {
          [`${entity.name}_line`]: [
            {
              ...(direction === 'pay' ? { accountId: chart.get('5000') } : {}),
              description: '合成試験',
              unitPrice: amount,
              taxCategory: 'exempt',
            },
