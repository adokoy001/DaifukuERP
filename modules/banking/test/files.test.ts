...
const snapshot: BankTransferSnapshot = {
  account: { ...identity, name: '振込元', ledgerAccountId: 'ledger', requesterCode: '0123456789' },
  lines: [
    {
      invoiceId: 'invoice',
      invoiceVersion: 1,
      ...
      payeeVersion: 1,
      amount: '1234567890',
      payee: { ...identity, accountNumber: '1' },
    },
  ],
};
describe('canonical bank CSV', () => {
  ...
  it.each(['0', '-1', '+2', '01', '1.5', '1e3', '1,000', '10000000000'])(
    'rejects noncanonical or oversized money %s',
    (amount) => {
  ...
  it('rejects duplicate IDs within a file, malformed quote, extra field and invalid encoding', () => {
    for (const body of [
      'a,2026-09-01,pay,1,x\na,2026-09-02,pay,1,y',
      'a,2026-09-01,pay,1,"x"z',
      'a,2026-09-01,pay,1,"x',
      'a,2026-09-01,pay,1,x,y',
      'a,2026-09-01,pay,1,\ufffd',
    ])
      expect(() => parseStatementCsv(`${header}\n${body}`)).toThrow();
  });
  it('bounds rows and bytes; accepts distinct IDs for otherwise identical real transactions', () => {
    ...
    expect(parseStatementCsv(make(500))).toHaveLength(500);
    expect(() => parseStatementCsv(make(501))).toThrow();
    expect(() => parseStatementCsv(header + '\n' + 'あ'.repeat(170_000))).toThrow();
  });
  it('protects text export cells from spreadsheet formulas', () => {
    for (const value of ['=1+1', '+SUM(A1)', '@x', '-1', '\tformula']) expect(csvCell(value)).toContain("'");
  });
});
describe('limited general transfer 120-byte profile', () => {
  it('matches official field offsets, zero padding, count/total, and single-byte kana', () => {
    ...
    expect(records[0]?.slice(0, 14)).toBe('12100123456789');
    expect(records[0]?.slice(54, 58)).toBe('0930');
    expect(records[1]?.slice(43, 50)).toBe('0000001');
    expect(records[1]?.slice(80, 90)).toBe('1234567890');
  ...
  it('emits exactly requested record endings with no BOM', () => {
    ...
    expect(none.bytes.length).toBe(480);
    expect(crlf.bytes.length).toBe(488);
    expect(none.bytes[0]).toBe(49);
    expect(crlf.bytes.slice(120, 122)).toEqual([13, 10]);
    expect(crlf.bytes.slice(-2)).toEqual([13, 10]);
  });
  it('rejects small kana, full-width, lower-case, unknown symbols and overflow without truncating', () => {
    ...
    expect(() =>
      zenginRecords({ ...snapshot, lines: [{ ...required(snapshot.lines[0]), amount: '10000000000' }] }, '2026-09-30'),
    ).toThrow();
    expect(() =>
      zenginRecords(
        {
          ...snapshot,
          lines: Array.from({ length: 101 }, () => ({ ...required(snapshot.lines[0]), amount: '9999999999' })),
        },
        '2026-09-30',
      ),
    ).toThrow();
    expect(() =>
      zenginRecords(
        {
          ...snapshot,
          lines: [{ ...required(snapshot.lines[0]), payee: { ...identity, holderKana: 'A'.repeat(31) } }],
        },
        '2026-09-30',
      ),
    ).toThrow();
...
it('candidate scoring explains equal amount/date/name and does not treat partial money as exact', () => {
  expect(candidateReasons(true, '2026-09-01', '2026-09-01', 'ｶ)ﾃｽﾄ 入金', 'カ)テスト')).toEqual({
    score: 100,
    reasons: ['金額一致', '日付一致', '摘要に取引先名'],
  });
