...
it.each([
  { format: 'canonical_csv', lineEnding: 'crlf' },
  { format: 'zengin120', lineEnding: 'none' },
  { format: 'zengin120', lineEnding: 'crlf' },
] as const)(
  'AC-3 / BANK-SAVED-01 $format $lineEnding freezes bytes across master edits and rejects cancelled downloads',
  async ({ format, lineEnding }) => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 1, max: 9_999_999 }),
        fc.integer({ min: 0, max: 9_999_999 }),
        async (amount, accountNumber) => {
          ...
          expect(original.bytes.length).toBeGreaterThan(0);
          f.payee = await f.action(
            'save_payee',
            {
              ...f.payeeInput,
              id: f.payee.id,
              expectedVersion: f.payee.version,
          ...
          expect((await f.action('transfer_detail', { batchId: batch.id }, bankTransferDetail)).snapshot).toEqual(
            batch.snapshot,
          );
          ...
          const cancelled = await f.action(
            'cancel_transfer',
            { batchId: batch.id, expectedVersion: original.batch.version, reason: 'Generated unsent cancellation' },
            bankTransferView,
          );
          expect(cancelled.state).toBe('cancelled');
          await expect(
            f.action('export_transfer', { ...request, expectedVersion: cancelled.version }, bankTransferExportOutput),
          ).rejects.toMatchObject({ code: 'INVALID_STATE' });
          expect(await ledger()).toEqual(baseline);
        },
      ),
      {
        seed: Number(process.env['PBT_SEED'] ?? 730305),
