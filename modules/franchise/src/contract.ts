...
export const contractSnapshot = z.object({
  ...
  name: z.string(),
  partnerId: z.uuid(),
  direction: z.enum(['bill', 'pay']),
  startDate: date,
  endDate: date,
  basis: z.enum(['gross', 'net']),
  rate: z.string(),
  fixedAmount: money,
  rounding: z.enum(['down', 'half_up', 'up']),
  taxCategory: z.enum(['standard', 'reduced', 'exempt', 'non_taxable', 'out_of_scope']),
...
export const generateInput = z
  .object({
    ...
    date,
    dueDate: date,
    expectedAgreementVersion: z.number().int().min(1),
  })
  .strict();
...
export const settlementBoard = command.extend({
  agreementId: z.uuid(),
  month: z.string(),
  direction: z.enum(['bill', 'pay']),
  grossSales: money,
...
export type FranchiseContractSnapshot = z.infer<typeof contractSnapshot>;
export type FranchiseBoard = z.infer<typeof settlementBoard>;
