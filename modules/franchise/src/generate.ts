...
export function result(row: Infer<typeof FranchiseSettlement>) {
  return {
    id: row.id,
    version: row.version,
    status: row.status,
...
function snapshot(row: Infer<typeof FranchiseAgreement>) {
  return contractSnapshot.parse({ ...row, rate: row.rate.toString(), fixedAmount: row.fixedAmount.toString() });
}
export async function generateFranchise(ctx: Context, input: z.infer<typeof generateInput>) {
  return withLock(ctx, `franchise:${input.agreementId}:${input.month}`, async () => {
    ...
    if (agreement.version !== input.expectedAgreementVersion)
      throw new Conflict('FC契約が変更されました。', '契約を読み込み直してください。');
    const existing = (
      await repo(ctx, FranchiseSettlement).list({
        where: { agreementId: agreement.id, month: input.month, status: { $ne: 'cancelled' } },
        limit: 1,
      })
    ).items[0];
    if (existing) {
      if (
        existing.grossSales.toString() !== input.grossSales ||
        existing.netSales.toString() !== input.netSales ||
        existing.sourceReference !== input.sourceReference ||
        existing.date !== input.date ||
        existing.dueDate !== input.dueDate
      )
        throw new Conflict(
          '同じ契約・月に異なる精算資料が存在します。',
          '現在の精算を確認し、訂正は理由を記録して取消後に再作成してください。',
        );
    ...
    if (fee.lte(0) || input.date < end || input.date > today || input.dueDate < input.date)
      throw new StateError(
        '精算額または請求日・期日が不正です。',
        '月末以降かつ本日以前の請求日、請求日以降の期日と正の精算額を指定してください。ゼロ円月は請求を生成しません。',
      );
    const invoice = await createInvoice(ctx, agreement, input, fee);
    return writeFranchise(ctx, async (inner) =>
      result(
        await repo(inner, FranchiseSettlement).create({
          ...
          ...(agreement.direction === 'bill' ? { salesInvoiceId: invoice.id } : { purchaseInvoiceId: invoice.id }),
        }),
      ),
    );
  });
...
async function createInvoice(
  ctx: Context,
  agreement: Infer<typeof FranchiseAgreement>,
  ...
  if (agreement.direction === 'bill') {
    ...
    return submitDocument(ctx, SalesInvoice, invoice.id);
  }
  const invoice = await repo(ctx, PurchaseInvoice).create(header);
  await saveLines(ctx, PurchaseInvoice, invoice.id, {
    [PurchaseInvoiceLine.name]: [{ ...line, accountId: agreement.expenseAccountId }],
  });
  return submitDocument(ctx, PurchaseInvoice, invoice.id);
}
