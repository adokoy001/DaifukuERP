...
const writer = defineWriteCapability({ name: 'purchase.settlement', entity: 'purchase_settlement', fields: ['invoiceId', 'date', 'amount'], operations: ['create'] });
const balanceWriter = defineWriteCapability({ name: 'purchase.balance', entity: 'purchase_invoice', fields: ['paidAmount', 'balance', 'status', 'settlementHistory'], operations: ['update'] });
...
async function settlementDays(ctx: Context, invoiceId: string): Promise<Map<string, Decimal>> {
  ...
  for (;;) {
    const groups = await repo(ctx, PurchaseSettlement).aggregate({ where: { invoiceId, ...(after ? { date: { $gt: after } } : {}) }, groupBy: ['date'], metrics: { amount: { sum: 'amount' } }, orderBy: [{ field: 'date', dir: 'asc' }], limit: 500 });
...
export async function appendSettlement(ctx: Context, invoiceId: string, date: LocalDate, amount: Decimal, total: Decimal): Promise<void> {
  ...
  await withWriteCapability(ctx, writer, (internal) => repo(internal, PurchaseSettlement).create({ invoiceId, date, amount }));
}
export function registerSettlementHooks(): void {
  for (const op of ['before_update', 'before_delete'] as const) registry.registerHook('purchase_settlement', op, () => { throw new StateError('Settlement history is append-only', 'Cancel the payment to append a reversing fact.'); });
}
/** Historical balances, without a dependency from invoices back to the payment module. */
export async function invoiceBalancesAsOf(ctx: Context, invoices: readonly Infer<typeof PurchaseInvoice>[], asOf: LocalDate): Promise<Map<string, Decimal>> {
  ...
  for (let offset = 0; offset < invoices.length; offset += 200) {
    ...
    const groups = await repo(ctx, PurchaseSettlement).aggregate({ where: { invoiceId: { $in: chunk.map((i) => i.id) }, date: { $lte: asOf } }, groupBy: ['invoiceId'], metrics: { amount: { sum: 'amount' } }, limit: 200 });
