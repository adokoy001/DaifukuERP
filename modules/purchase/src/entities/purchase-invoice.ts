...
export const PurchaseInvoice = defineDocument({
  ...
  fields: {
    ...
    dueDate: f.date({ label: label('支払期日', 'Due date'), description: label('空欄なら仕入先の締め日・支払サイトから計算', 'Computed from the supplier terms when empty'), index: true }),
    priceIncludesTax: f.bool({ label: label('税込入力', 'Prices include tax'), description: label('受領請求書は税込が多いので既定 true', 'Received bills are usually tax-inclusive'), required: true, default: true }),
    ...
    deductibleTax: f.money({ serverOwned: true, label: label('控除対象仕入税額', 'Deductible tax'), required: true, default: '0' }),
    nonDeductibleTax: f.money({ serverOwned: true, label: label('控除対象外消費税', 'Non-deductible tax'), required: true, default: '0' }),
    ...
    status: f.enum(INVOICE_STATUSES, { serverOwned: true,
      ...
      default: 'draft',
      index: true,
      labels: { draft: label('下書き', 'Draft'), open: label('未払', 'Open'), paid: label('支払済', 'Paid'), cancelled: label('取消', 'Cancelled') },
    }),
    supplierTaxStatus: f.enum(SUPPLIER_TAX_STATUSES, { serverOwned: true,
    ...
    creditRatio: f.decimal({ serverOwned: true, label: label('控除率', 'Credit ratio'), description: label('課税事業者 1、免税事業者は経過措置の率', '1 for registered suppliers; the transitional ratio for exempt ones'), required: true, default: '1', scale: 4, min: '0' }),
