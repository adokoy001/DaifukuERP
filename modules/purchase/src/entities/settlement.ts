...
export const PurchaseSettlement = defineEntity({
  name: 'purchase_settlement', label: label('消込履歴', 'Settlement history'),
  fields: {
    invoiceId: f.ref('purchase_invoice', { label: label('請求書', 'Invoice'), required: true, immutable: true, serverOwned: true }),
