...
export const PurchaseInvoiceLine = defineEntity({
  ...
  fields: {
    ...
    accountId: f.ref('account', {
      ...
      description: label(
        '経費行に指定。品目行は設定 purchase.accounts の仕入科目',
        'For expense lines; product lines use the purchases account setting',
      ),
    }),
    ...
    amount: f.money({
      ...
      description: label('数量 × 単価（自動計算）', 'quantity × unit price, computed'),
      required: true,
      default: '0',
    }),
  },
