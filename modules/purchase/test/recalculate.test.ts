...
const RATE: Record<TaxCategory, string> = {
  standard: '0.1',
  reduced: '0.08',
  exempt: '0',
  non_taxable: '0',
...
const LABEL: Record<TaxCategory, string> = {
  standard: '標準10%',
  reduced: '軽減8%',
  exempt: '免税',
  non_taxable: '非課税',
...
function summaryOf(
  lines: readonly { amount: string; category: TaxCategory }[],
  ...
  return {
    priceIncludesTax,
    rounding: { mode, scale },
    groups: s.groups.map((g) => ({ ...g, code: g.category.toUpperCase(), label: LABEL[g.category] })),
  };
}

const dec = (s: string) => Decimal.from(s);
const str = (d: Decimal | string | undefined | null) =>
  d === undefined || d === null ? '0' : Decimal.from(d).toString();
...
describe('credit ratio (AC-2)', () => {
  it('default: registered → 1, exempt → 0, anything else → VALIDATION; the point name is fixed', () => {
    ...
    expect(() => defaultCreditRatio({ supplierTaxStatus: 'other' as 'exempt', date: '2026-10-15' })).toThrow(
      ValidationError,
    );
  });
  it('assertCreditRatio rejects non-Decimal, negative and > 1 results from an override', () => {
    ...
    for (const bad of [0.7, '0.7', dec('-0.1'), dec('1.5'), null, undefined])
      expect(() => assertCreditRatio(bad, input), String(bad)).toThrow(StateError);
  });
});
...
describe('recalculate (AC-2)', () => {
  ...
  it('splitTax rounds the deductible part once with the given mode; the remainder is non-deductible', () => {
    expect(splitTax(dec('1001'), dec('0.7'), 'down', 0)).toMatchObject({
      deductibleTax: dec('700'),
      nonDeductibleTax: dec('301'),
    });
    expect(splitTax(dec('1001'), dec('0.7'), 'half_up', 0)).toMatchObject({
      deductibleTax: dec('701'),
      nonDeductibleTax: dec('300'),
    });
    expect(splitTax(dec('1001'), dec('0.7'), 'up', 0)).toMatchObject({
      deductibleTax: dec('701'),
      nonDeductibleTax: dec('300'),
    });
    expect(splitTax(dec('123.45'), dec('0.8'), 'down', 2)).toMatchObject({
      deductibleTax: dec('98.76'),
      nonDeductibleTax: dec('24.69'),
    });
  ...
  it('applyCreditRatio splits per rate group and totals the header; JSON carries decimal strings', () => {
    const s = summaryOf(
      [
        { amount: '11000', category: 'standard' },
        { amount: '5400', category: 'reduced' },
        { amount: '1000', category: 'non_taxable' },
      ],
      true,
    );
    const c = applyCreditRatio(s, dec('0.7'));
    expect(
      c.groups.map((g) => [g.category, str(g.taxable), str(g.tax), str(g.deductibleTax), str(g.nonDeductibleTax)]),
    ).toEqual([
    ...
    expect({
      subtotal: str(c.totals.subtotal),
      taxTotal: str(c.totals.taxTotal),
      deductibleTax: str(c.totals.deductibleTax),
      nonDeductibleTax: str(c.totals.nonDeductibleTax),
      total: str(c.totals.total),
    }).toEqual({ subtotal: '16000', taxTotal: '1400', deductibleTax: '980', nonDeductibleTax: '420', total: '17400' });
    const j = calculationToJson(c);
    expect(j.creditRatio).toBe('0.7');
    expect(j.totals).toEqual({
      subtotal: '16000',
      taxTotal: '1400',
      deductibleTax: '980',
      nonDeductibleTax: '420',
      total: '17400',
    });
    expect(j.groups[0]).toMatchObject({
      category: 'standard',
      code: 'STANDARD',
      label: '標準10%',
      rate: '0.1',
...
describe('posting (AC-3)', () => {
  ...
  const flat = (l: {
    accountId: string;
    debit?: Decimal | string | undefined;
    credit?: Decimal | string | undefined;
    memo?: string | null | undefined;
  }) => [CODE_OF[l.accountId] ?? l.accountId, str(l.debit), str(l.credit), l.memo ?? null];
  ...
  it('product lines go to the purchases account, expense lines to their account; partner on the payable line', () => {
    ...
    const { lines } = post(
      [
        { productId: 'prod', accountId: null, amount: dec('10000'), taxCategory: 'standard' },
        { productId: null, accountId: 'acc-6400', amount: dec('2000'), taxCategory: 'standard' },
      ],
      false,
      '1',
    );
  ...
  const arbLine = fc.record({
    account: fc.constantFrom('acc-5000', 'acc-6400', 'acc-7000', null),
    amount: fc.integer({ min: -100000, max: 1000000 }),
  ...
  it('property: the entry always balances, Σ debit on the payable side equals total, and every line is debit XOR credit ≥ 0', () => {
    fc.assert(
      fc.property(
        ...
        (rows, inclusive, ratio, mode) => {
          const lines: PostingLine[] = rows.map((r) => ({
            productId: r.account ? null : 'prod',
            accountId: r.account,
            amount: dec(String(r.amount)),
            taxCategory: r.category,
          }));
          ...
          const inputTax = Decimal.sum(
            jl
              .filter((l) => l.accountId === ACCOUNTS.taxReceivable)
              .map((l) => Decimal.from(l.debit ?? '0').minus(l.credit ?? '0')),
          );
  ...
  it('property: per group deductible + nonDeductible = tax and |deductible| ≤ |tax|', () => {
    fc.assert(
      fc.property(
        fc.array(arbLine, { minLength: 1, maxLength: 6 }),
        fc.boolean(),
        arbRatio,
        (rows, inclusive, ratio) => {
          const s = summaryOf(
...
describe('aging and payments (AC-5)', () => {
  ...
  it('agingRows: one row per partner sorted by code, buckets, totals; missing due date uses the bill date; unknown partner shown by id', () => {
    ...
    expect(agingRows([], partners, '2026-10-15')).toEqual({
      rows: [],
      totals: { notDue: '0', days1to30: '0', days31to60: '0', days61to90: '0', over90: '0', balance: '0' },
    });
  });
  it('applyPaymentAmounts: partial → open, full → paid, negative un-applies; zero / overshoot / below zero are issues', () => {
    const t = dec('11000');
    expect(applyPaymentAmounts(t, dec('0'), dec('4000'))).toMatchObject({
      paidAmount: dec('4000'),
      balance: dec('7000'),
      status: 'open',
    });
    expect(applyPaymentAmounts(t, dec('4000'), dec('7000'))).toMatchObject({
      paidAmount: dec('11000'),
      balance: dec('0'),
      status: 'paid',
    });
    expect(applyPaymentAmounts(t, dec('11000'), dec('-11000'))).toMatchObject({
      paidAmount: dec('0'),
      balance: dec('11000'),
      status: 'open',
    });
...
describe('golden test/golden/exempt-supplier.json (AC-7)', () => {
  for (const c of golden.cases) {
    it(c.name, () => {
      ...
      const s = summaryOf(
        golden.bill.lines,
        golden.bill.priceIncludesTax,
        golden.bill.rounding.mode,
        golden.bill.rounding.scale,
      );
      const calc = applyCreditRatio(s, dec(c.creditRatio));
      const j = calculationToJson(calc);
      expect(j.totals).toEqual({
        subtotal: c.expected.subtotal,
        taxTotal: c.expected.taxTotal,
        deductibleTax: c.expected.deductibleTax,
        nonDeductibleTax: c.expected.nonDeductibleTax,
      ...
      expect(
        lines.map((l) => ({
          account: CODE_OF[l.accountId],
          debit: str(l.debit),
          credit: str(l.credit),
          memo: l.memo ?? null,
        })),
      ).toEqual(c.expected.journal);
      const byAccount: Record<string, string> = {};
      for (const l of lines) {
        const code = CODE_OF[l.accountId] ?? l.accountId;
        byAccount[code] = Decimal.from(byAccount[code] ?? '0')
          .plus(l.debit ?? '0')
          .minus(l.credit ?? '0')
          .toString();
      }
