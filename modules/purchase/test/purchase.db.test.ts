...
import { Decimal, PermissionDenied, StateError, ValidationError, appMeta, newId, registerCrudActions, registry, repo, runAction, setSetting, type Context, type ContextParams } from '@daifuku/kernel';
...
type LineJson = Row & { id: string; seq: number; productId: string | null; accountId: string | null; description: string; quantity: string; unitPrice: string; taxCategory: string; amount: string };
type BillJson = Row & {
  id: string;
  number: string | null;
  docstatus: number;
...
const totalsOf = (b: BillJson) => ({ subtotal: b.subtotal, taxTotal: b.taxTotal, deductibleTax: b.deductibleTax, nonDeductibleTax: b.nonDeductibleTax, total: b.total });
...
async function createBill(ctx: Context, head: Row, lines: Row[]): Promise<BillWithLines> {
  const created = (await runAction(ctx, 'purchase_invoice.create', { ...head, lines: { purchase_invoice_line: lines } })) as BillWithLines;
...
async function entryLines(ctx: Context, entryId: string): Promise<(string | null)[][]> {
  ...
  return lines.items.map((l) => [codeOf.get(l.accountId) ?? l.accountId, l.debit.toString(), l.credit.toString(), l.memo, l.partnerId]);
}
...
beforeAll(async () => {
  ...
  const account = (code: string, name: string, type: 'asset' | 'liability' | 'expense', extra: Row = {}) => run({}, async (ctx) => (await repo(ctx, Account).create({ code, name, type, ...extra })).id);
  acc = {
  ...
  for (const [k, code] of [['purchases', '5000'], ['inputTax', '1500'], ['payable', '2100'], ['supplies', '6400'], ['misc', '6500']] as const) codeOf.set(acc[k], code);
  supplier = await run({}, async (ctx) => ({
    registered: (await repo(ctx, Partner).create({ code: 'S-REG', name: '課税商事', isSupplier: true, taxStatus: 'registered', invoiceRegistrationNo: 'T1234567890123' })).id,
    exempt: (await repo(ctx, Partner).create({ code: 'S-EX', name: '免税工房', isSupplier: true, taxStatus: 'exempt', closingDay: 20, paymentMonthOffset: 1, paymentDay: 10 })).id,
  }));
  product = await run({}, async (ctx) => ({
    goods: (await repo(ctx, Product).create({ code: 'G-1', name: '商品A', purchasePrice: '1000', taxCategory: 'standard' })).id,
    food: (await repo(ctx, Product).create({ code: 'F-1', name: '食品B', purchasePrice: '540', taxCategory: 'reduced' })).id,
  }));
});
afterAll(async () => {
  await db.close();
...
describe('purchase module (docs/specs/purchase.md)', () => {
  it('AC-1 purchase_invoice + lines: defaults (date=today JST, 税込, due date from terms, supplier status copied), line defaults from product/account, VALIDATION rules', async () => {
    const b = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ productId: product.goods }, { accountId: acc.supplies, description: 'コピー用紙', quantity: '2', unitPrice: '550' }]));
    expect(b).toMatchObject({ docstatus: 0, number: null, date: '2026-09-10', dueDate: '2026-10-31', priceIncludesTax: true, status: 'draft', supplierTaxStatus: 'registered', creditRatio: '1', paidAmount: '0', journalEntryId: null, supplierInvoiceNo: null });
    expect(b.lines.purchase_invoice_line.map((l) => [l.seq, l.productId, l.accountId, l.description, l.quantity, l.unitPrice, l.taxCategory, l.amount])).toEqual([
    ...
    expect(totalsOf(b)).toEqual({ subtotal: '1910', taxTotal: '190', deductibleTax: '190', nonDeductibleTax: '0', total: '2100' });
    expect(b.balance).toBe('2100');
    expect(b.taxSummary?.creditRatio).toBe('1');
    expect(b.taxSummary?.groups[0]).toMatchObject({ category: 'standard', code: 'STD10', rate: '0.1', taxable: '1910', tax: '190', gross: '2100', lineCount: 2, deductibleTax: '190', nonDeductibleTax: '0' });
    // exempt supplier with its own terms: closing 20th → 2026-09-20, +1 month, day 10 → 2026-10-10; explicit due date kept
    const e = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.exempt, supplierInvoiceNo: 'K-77' }, [{ productId: product.goods }]));
    expect(e).toMatchObject({ dueDate: '2026-10-10', supplierTaxStatus: 'exempt', supplierInvoiceNo: 'K-77' });
    const d = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.exempt, date: '2026-09-25', dueDate: '2026-12-31' }, [{ productId: product.goods }]));
    ...
    await expect(run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ description: 'x', unitPrice: '1' }]))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'accountId' }] } });
    await expect(run(purchasing, (ctx) => createBill(ctx, { partnerId: newId() }, [{ productId: product.goods }]))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'partnerId' }] } });
    await expect(run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ accountId: acc.misc, description: 'x', unitPrice: '1' }]))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'taxCategory' }] } });
    const misc = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ accountId: acc.misc, description: '雑', unitPrice: '300', taxCategory: 'out_of_scope' }]));
    expect(totalsOf(misc)).toEqual({ subtotal: '300', taxTotal: '0', deductibleTax: '0', nonDeductibleTax: '0', total: '300' });
    await expect(run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ productId: product.goods, taxCategory: 'vat' }]))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'taxCategory' }] } });
    await expect(run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered, date: '2026-13-01' }, [{ productId: product.goods }]))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'date' }] } });
    // system-owned fields while draft: status/paidAmount/balance/journalEntryId are forced
    await expect(run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered, status: 'paid', paidAmount: '999', balance: '1' }, [{ productId: product.goods }]))).rejects.toBeInstanceOf(PermissionDenied);
    // AC-6: ops per role
    const ops = async (roles: string[]) => run(asRole(roles), async (ctx) => appMeta(ctx).entities.find((x) => x.name === 'purchase_invoice')?.ops);
    ...
    await expect(run(asRole(['viewer']), (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ productId: product.goods }]))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run(asRole(['accounting']), (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ productId: product.goods }]))).rejects.toBeInstanceOf(PermissionDenied);
  ...
  it('AC-2 recalculation on every draft save: generic update (header before lines), direct line writes, header-only changes; exempt supplier defaults to ratio 0', async () => {
    const b = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ productId: product.goods, unitPrice: '11000' }, { productId: product.food, quantity: '10', unitPrice: '540' }]));
    expect(totalsOf(b)).toEqual({ subtotal: '15000', taxTotal: '1400', deductibleTax: '1400', nonDeductibleTax: '0', total: '16400' });
    ...
    const u = (await run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.update', { id: b.id, patch: { note: 'x', lines: { purchase_invoice_line: [{ id: line1, productId: product.goods, unitPrice: '2200' }] } } }))) as BillWithLines;
    ...
    expect(totalsOf(u)).toEqual({ subtotal: '2000', taxTotal: '200', deductibleTax: '200', nonDeductibleTax: '0', total: '2200' });
    ...
    const h = (await run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.update', { id: b.id, patch: { priceIncludesTax: false } }))) as BillJson;
    expect(totalsOf(h)).toEqual({ subtotal: '2200', taxTotal: '220', deductibleTax: '220', nonDeductibleTax: '0', total: '2420' });
    ...
    expect(totalsOf(await run({}, (ctx) => getBill(ctx, b.id)))).toMatchObject({ subtotal: '6600', taxTotal: '660', total: '7260' });
    const extra = await run(purchasing, (ctx) => repo(ctx, PurchaseInvoiceLine).create({ invoiceId: b.id, accountId: acc.supplies, description: '追加', unitPrice: '100' } as never));
    ...
    const x = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.exempt, date: '2026-10-15' }, [{ productId: product.goods, unitPrice: '11000' }]));
    expect(x).toMatchObject({ supplierTaxStatus: 'exempt', creditRatio: '0' });
    expect(totalsOf(x)).toEqual({ subtotal: '10000', taxTotal: '1000', deductibleTax: '0', nonDeductibleTax: '1000', total: '11000' });
    // switching the partner re-copies the status/ratio and recomputes the due date
    const sw = (await run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.update', { id: x.id, patch: { partnerId: supplier.registered } }))) as BillJson;
    expect(sw).toMatchObject({ supplierTaxStatus: 'registered', creditRatio: '1', deductibleTax: '1000', nonDeductibleTax: '0', dueDate: '2026-11-30' });
    // a date with no standard rate → VALIDATION from the tax module (rates are data)
    await expect(run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered, date: '2010-01-01' }, [{ productId: product.goods }]))).rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it('AC-7 golden: exempt supplier with a country-pack-style override (0.8 until 2026-09-30, 0.7 after) — totals, journal and re-dating; AC-3 posting', async () => {
    const override: CreditRatioFn = ({ supplierTaxStatus, date }) => (supplierTaxStatus === 'registered' ? Decimal.from(1) : Decimal.from(date >= '2026-10-01' ? '0.7' : '0.8'));
    ...
    try {
      ...
      const b = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.exempt, date: oct.date, supplierInvoiceNo: 'INV-EX-1' }, [{ productId: product.goods, unitPrice: '11000' }]));
      expect(b).toMatchObject({ creditRatio: oct.creditRatio, supplierTaxStatus: 'exempt' });
      expect(totalsOf(b)).toEqual({ subtotal: oct.expected.subtotal, taxTotal: oct.expected.taxTotal, deductibleTax: oct.expected.deductibleTax, nonDeductibleTax: oct.expected.nonDeductibleTax, total: oct.expected.total });
      const s = await run(purchasing, (ctx) => submit(ctx, b.id));
      expect(s).toMatchObject({ docstatus: 1, status: 'open', balance: '11000', paidAmount: '0', deductibleTax: '700', nonDeductibleTax: '300' });
      ...
      expect(entry).toMatchObject({ docstatus: 1, date: oct.date, sourceEntity: 'purchase_invoice', sourceId: b.id, description: '仕入 免税工房 INV-EX-1' });
      ...
      expect(await run({}, (ctx) => entryLines(ctx, entry.id))).toEqual(oct.expected.journal.map((l) => [l.account, l.debit, l.credit, l.memo, l.account === '2100' ? supplier.exempt : null]));
      // the same bill dated 2026-09-15 → 80%: re-dating a draft recomputes the ratio
      const b2 = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.exempt, date: oct.date }, [{ productId: product.goods, unitPrice: '11000' }]));
      const re = (await run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.update', { id: b2.id, patch: { date: sep.date } }))) as BillJson;
      expect(re).toMatchObject({ date: sep.date, creditRatio: sep.creditRatio, dueDate: '2026-10-10' });
      expect(totalsOf(re)).toEqual({ subtotal: sep.expected.subtotal, taxTotal: sep.expected.taxTotal, deductibleTax: sep.expected.deductibleTax, nonDeductibleTax: sep.expected.nonDeductibleTax, total: sep.expected.total });
      const s2 = await run(purchasing, (ctx) => submit(ctx, b2.id));
      expect(await run({}, (ctx) => entryLines(ctx, s2.journalEntryId ?? ''))).toEqual(sep.expected.journal.map((l) => [l.account, l.debit, l.credit, l.memo, l.account === '2100' ? supplier.exempt : null]));
    ...
    const after = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.exempt, date: '2026-10-15' }, [{ productId: product.goods }]));
  ...
  it('AC-3/AC-8 submit an expense bill (経費, account lines + product line): posting per account, 控除対象外 on the first line account, frozen after submit, failure leaves a draft', async () => {
    ...
    expect(totalsOf(b)).toEqual({ subtotal: '4700', taxTotal: '400', deductibleTax: '0', nonDeductibleTax: '400', total: '5100' });
    ...
    await expect(run(purchasing, (ctx) => submit(ctx, empty.id))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'lines' }] } });
    ...
    const outside = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered, date: '2031-01-01' }, [{ productId: product.goods }]));
    await expect(run(purchasing, (ctx) => submit(ctx, outside.id))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    expect(await run({}, (ctx) => getBill(ctx, outside.id))).toMatchObject({ docstatus: 0, status: 'draft', journalEntryId: null });
    ...
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.update', { id: b.id, patch: { date: '2026-10-21' } }))).rejects.toMatchObject({ code: 'INVALID_STATE', details: { blocked: ['date'] } });
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.update', { id: b.id, patch: { lines: { purchase_invoice_line: [] } } }))).rejects.toBeInstanceOf(StateError);
    const lineId = b.lines.purchase_invoice_line[0]?.id ?? '';
    await expect(run(purchasing, (ctx) => repo(ctx, PurchaseInvoiceLine).update(lineId, { unitPrice: '1' }))).rejects.toBeInstanceOf(StateError);
    await expect(run(purchasing, (ctx) => repo(ctx, PurchaseInvoiceLine).delete(lineId))).rejects.toBeInstanceOf(StateError);
    await expect(run(purchasing, (ctx) => repo(ctx, PurchaseInvoiceLine).create({ invoiceId: b.id, productId: product.goods } as never))).rejects.toBeInstanceOf(StateError);
    await expect(run({}, (ctx) => runAction(ctx, 'purchase_invoice.delete', { id: b.id }))).rejects.toBeInstanceOf(StateError);
  ...
  it('AC-4 cancel: refused while paidAmount > 0; otherwise reverses the entry and sets cancelled; amend gives a fresh draft', async () => {
    const b = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered, date: '2026-10-05' }, [{ productId: product.goods, unitPrice: '5500' }]));
    ...
    await run(purchasing, (ctx) => applyPayment(ctx, { invoiceId: b.id, amount: '-500', date: '2026-10-06' }));
    await expect(run(asRole(['accounting']), (ctx) => runAction(ctx, 'purchase_invoice.cancel', { id: b.id }))).rejects.toBeInstanceOf(PermissionDenied);
    ...
    const reversal = await run({}, (ctx) => repo(ctx, JournalEntry).list({ where: { reversalOf: s.journalEntryId ?? '' } }));
    ...
    await expect(run(purchasing, (ctx) => applyPayment(ctx, { invoiceId: b.id, amount: '1', date: '2026-10-07' }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.cancel', { id: b.id }))).rejects.toBeInstanceOf(StateError);
    ...
    expect(a).toMatchObject({ docstatus: 0, status: 'draft', amendedFrom: b.id, number: null, journalEntryId: null, paidAmount: '0', balance: '5500', total: '5500', partnerId: supplier.registered, date: '2026-10-05' });
    const a2 = (await run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.update', { id: a.id, patch: { lines: { purchase_invoice_line: [{ productId: product.goods, unitPrice: '5500' }] } } }))) as BillJson;
  ...
  it('AC-5 record_payment / applyPayment: partial, full (paid), un-apply, overshoot VALIDATION, drafts INVALID_STATE, event, roles', async () => {
    ...
    const b = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered, date: '2026-10-08' }, [{ productId: product.goods, unitPrice: '3300' }]));
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase.record_payment', { invoiceId: b.id, amount: '100', date: '2026-10-09' }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await run(purchasing, (ctx) => submit(ctx, b.id));
    const p1 = (await run(asRole(['accounting']), (ctx) => runAction(ctx, 'purchase.record_payment', { invoiceId: b.id, amount: '1000', date: '2026-10-09' }))) as BillJson;
    expect(p1).toMatchObject({ paidAmount: '1000', balance: '2300', status: 'open' });
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase.record_payment', { invoiceId: b.id, amount: '2301', date: '2026-10-09' }))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'amount' }] } });
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase.record_payment', { invoiceId: b.id, amount: '0', date: '2026-10-09' }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase.record_payment', { invoiceId: b.id, amount: '1', date: '2026-10-9' }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(run(asRole(['viewer']), (ctx) => runAction(ctx, 'purchase.record_payment', { invoiceId: b.id, amount: '1', date: '2026-10-09' }))).rejects.toBeInstanceOf(PermissionDenied);
    const p2 = await run(purchasing, (ctx) => applyPayment(ctx, { invoiceId: b.id, amount: Decimal.from('2300'), date: '2026-10-10' }));
    expect(p2.status).toBe('paid');
    expect(p2.balance.toString()).toBe('0');
    expect(p2.paidAmount.toString()).toBe('3300');
    const p3 = await run(purchasing, (ctx) => applyPayment(ctx, { invoiceId: b.id, amount: '-300', date: '2026-10-11' }));
    expect(p3).toMatchObject({ status: 'open' });
    expect(p3.balance.toString()).toBe('300');
    const events = await db.owner.sql<{ payload: Row }[]>`select payload from outbox where topic = ${PAYMENT_APPLIED_EVENT} and tenant_id = ${db.tenantId} order by created_at`;
    expect(events.map((e) => e.payload)).toContainEqual({ invoiceId: b.id, number: p1.number, amount: '2300', date: '2026-10-10', paidAmount: '3300', balance: '0', status: 'paid' });
  ...
  it('AC-5 ap_aging: open submitted bills per supplier bucketed by days past due as of a date; TableResult; viewer may read', async () => {
    const mk = async (partnerId: string, date: string, dueDate: string, unitPrice: string) => {
      const b = await run(purchasing, (ctx) => createBill(ctx, { partnerId, date, dueDate }, [{ productId: product.goods, unitPrice }]));
      await run(purchasing, (ctx) => submit(ctx, b.id));
      return b.id;
    };
    const asOf = '2026-12-15';
    const aging = (params: Partial<ContextParams>, input: Row) => run(params, async (ctx) => (await runAction(ctx, 'purchase.ap_aging', input)) as Table);
    const bucketsOf = (t: Table, partnerId: string) => {
      const r = t.rows.find((x) => x.partnerId === partnerId);
      return Object.fromEntries(['notDue', 'days1to30', 'days31to60', 'days61to90', 'over90', 'balance'].map((k) => [k, Decimal.from(String(r?.[k] ?? '0'))]));
    };
    const delta = (a: Record<string, Decimal>, b: Record<string, Decimal>) => Object.fromEntries(Object.keys(a).map((k) => [k, (a[k] ?? Decimal.zero()).minus(b[k] ?? Decimal.zero()).toString()]));
    ...
    expect(t.columns.map((c) => c.key)).toEqual(['partnerCode', 'partnerName', 'billCount', 'notDue', 'days1to30', 'days31to60', 'days61to90', 'over90', 'balance', 'partnerId']);
    expect(t.rows.find((r) => r.partnerId === supplier.exempt)).toMatchObject({ partnerCode: 'S-EX', partnerName: '免税工房' });
    expect(t.rows.find((r) => r.partnerId === supplier.registered)).toMatchObject({ partnerCode: 'S-REG', partnerName: '課税商事' });
    expect(delta(bucketsOf(t, supplier.registered), bucketsOf(base, supplier.registered))).toEqual({ notDue: '1000', days1to30: '2000', days31to60: '2500', days61to90: '0', over90: '0', balance: '5500' });
    expect(delta(bucketsOf(t, supplier.exempt), bucketsOf(base, supplier.exempt))).toEqual({ notDue: '0', days1to30: '0', days31to60: '0', days61to90: '0', over90: '4000', balance: '4000' });
    ...
    expect(Decimal.sum(['notDue', 'days1to30', 'days31to60', 'days61to90', 'over90'].map((k) => Decimal.from(t.totals?.[k] ?? '0'))).toString()).toBe(t.totals?.balance);
    ...
    await expect(run(asRole(['nobody']), (ctx) => runAction(ctx, 'purchase.ap_aging', {}))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run({}, (ctx) => runAction(ctx, 'purchase.ap_aging', { asOf: '2026-2-1' }))).rejects.toMatchObject({ code: 'VALIDATION' });
  });
  ...
  it('AC-1/AC-6 accounting may update a draft header but not create lines; purchasing may delete drafts only through admin (spec lists no delete)', async () => {
    const b = await run(purchasing, (ctx) => createBill(ctx, { partnerId: supplier.registered }, [{ productId: product.goods }]));
    const u = (await run(asRole(['accounting']), (ctx) => runAction(ctx, 'purchase_invoice.update', { id: b.id, patch: { note: '経理メモ' } }))) as BillJson;
    expect(u.note).toBe('経理メモ');
    await expect(run(asRole(['accounting']), (ctx) => repo(ctx, PurchaseInvoiceLine).create({ invoiceId: b.id, productId: product.goods } as never))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run(purchasing, (ctx) => runAction(ctx, 'purchase_invoice.delete', { id: b.id }))).rejects.toBeInstanceOf(PermissionDenied);
