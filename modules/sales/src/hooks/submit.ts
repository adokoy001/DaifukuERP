...
import { Decimal, getSetting, registry, defineWriteCapability, withWriteCapability, repo, StateError, ValidationError, type Context, type HookArgs, type LocalDate } from '@daifuku/kernel';
...
const snapshotWrite = defineWriteCapability({ name: 'sales.issue-snapshot', entity: 'sales_invoice', fields: ['issuedSnapshot'], operations: ['update'] });
...
async function beforeSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  if (typeof partnerId !== 'string') throw new ValidationError(`sales_invoice ${id} has no partner`, [{ path: 'partnerId', message: 'required' }], 'Set partnerId, then submit.');
  const { totals, lines } = await recalculateInvoice(ctx, { id, date: row.date as LocalDate, priceIncludesTax: row.priceIncludesTax === true });
  if (lines.length === 0) throw new ValidationError(`sales_invoice ${id} has no lines`, [{ path: 'lines', message: 'at least 1 line is required' }], 'Add at least one line, then submit.');
  if (!totals.total.gt(0)) {
    throw new ValidationError(`sales_invoice ${id} total ${totals.total.toString()} must be greater than 0`, [{ path: 'total', message: 'must be > 0' }], 'An invoice must bill a positive amount; credit notes are out of scope (cancel + amend instead).');
  }
  ...
  const journalLines = journalLinesFor(
    {
      ...
      lines: lines.map((l) => ({ seq: l.seq, description: l.description, amount: l.amount, taxCategory: l.taxCategory, ext: postingDimensions('sales_invoice_line', 'journal_line', l.ext) })),
      taxSummary: totals.taxSummary,
      total: totals.total,
    },
    accounts,
  );
  const entry = await postFromSource(ctx, { sourceEntity: SalesInvoice.name, sourceId: id, date: row.date as LocalDate, description: description(partner.name, null), ext: row.ext as Record<string, unknown>, lines: journalLines.map((l) => ({ ...l, ext: { ...postingDimensions(SalesInvoice.name, 'journal_line', row.ext as Record<string, unknown>), ...l.ext } })) });
  Object.assign(row, {
...
async function afterSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  await withWriteCapability(ctx, snapshotWrite, (internal) => repo(internal, SalesInvoice).update(invoice.id, { issuedSnapshot }));
