...
export const salesIssuerSchema = z.object({
  ...
  invoiceRegistrationNo: z.string().regex(/^T\d{13}$/, 'T + 13 digits').optional(),
...
export const SALES_SETTING_DEFS: readonly SettingDef[] = [
  {
    ...
    description: label('科目コード: receivable=売掛金, revenue=売上高, taxPayable=仮受消費税', 'Account codes: receivable, revenue, taxPayable (output tax)'),
    schema: salesAccountsSchema,
  },
  {
    key: SALES_ISSUER_KEY,
    label: label('請求書の発行者', 'Invoice issuer'),
    description: label('適格請求書に印字する名称・登録番号（T+13桁）・住所・連絡先・振込先', 'Name, registration number (T + 13 digits), address, contact and bank details printed on invoices'),
