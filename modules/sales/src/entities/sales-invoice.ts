...
export const SalesInvoice = defineDocument({
  ...
  fields: {
    ...
    issuedSnapshot: f.json<InvoiceRenderData>({
      label: label('発行時データ', 'Issued invoice snapshot'),
      serverOwned: true,
      hidden: true,
    }),
    ...
    dueDate: f.date({
      ...
      description: label(
        '空欄なら取引先の締め日・支払月・支払日から計算',
        "Computed from the partner's closing day / payment terms when empty",
      ),
      index: true,
    }),
    priceIncludesTax: f.bool({
      ...
      description: label(
        '省略時は会社設定 tax.price_includes_tax',
        'Defaults to the company setting tax.price_includes_tax',
      ),
      required: true,
      default: false,
    }),
    ...
    balance: f.money({
      serverOwned: true,
      label: label('残高', 'Balance'),
      description: label('税込合計 − 入金済額', 'total − paidAmount'),
      required: true,
      default: '0',
    }),
