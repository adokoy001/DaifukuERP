...
import { defineAction, DOCSTATUS, isLocalDate, label, repo, todayLocal, type Context, StateError, type LocalDate } from '@daifuku/kernel';
...
export async function arAging(ctx: Context, asOf: LocalDate): Promise<TableResult> {
  const invoices = [];
  for (let offset = 0; ; ) {
    const page = await repo(ctx, SalesInvoice).list({ where: { date: { $lte: asOf }, $or: [{ docstatus: DOCSTATUS.submitted }, { docstatus: DOCSTATUS.cancelled, cancelledDate: { $gt: asOf } }] }, limit: 500, offset });
    invoices.push(...page.items);
    if (invoices.length > MAX_REPORT_ROWS) throw new StateError('Historical aging exceeds the supported report size', 'Narrow the report scope before retrying.');
    offset += page.items.length;
    if (!page.items.length || offset >= page.total) break;
  }
  const balances = await invoiceBalancesAsOf(ctx, invoices, asOf);
  const inputs: AgingInput[] = invoices.map((inv) => ({ partnerId: inv.partnerId, dueDate: inv.dueDate, balance: balances.get(inv.id) ?? inv.total })).filter((i) => !i.balance.isZero());
  ...
  return {
    ...
    rows,
    totals: Object.fromEntries([...AGING_BUCKETS, 'total'].map((k) => [k, totals[k as keyof typeof totals].toString()])),
