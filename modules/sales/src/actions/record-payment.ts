...
function parseAmount(v: Decimal | string): Decimal {
  ...
  if (!d)
    throw new ValidationError(`invalid amount "${String(v)}"`, [
      { path: 'amount', message: 'must be a decimal string' },
    ]);
...
export async function applyPayment(ctx: Context, input: ApplyPaymentInput): Promise<SalesInvoiceRow> {
  ...
  if (inv.docstatus !== DOCSTATUS.submitted || inv.status === 'cancelled') {
    throw new StateError(
      `sales_invoice ${ref} is not open (docstatus ${inv.docstatus}, status ${inv.status})`,
      'Submit the invoice first; cancelled invoices do not take payments.',
      { id: inv.id, docstatus: inv.docstatus, status: inv.status },
    );
  }
  const paidAmount = inv.paidAmount.plus(amount);
  if (paidAmount.gt(inv.total)) {
    throw new ValidationError(
      `payment ${amount.toString()} exceeds the open balance ${inv.balance.toString()} of sales_invoice ${ref}`,
      [{ path: 'amount', message: `must be <= ${inv.balance.toString()}` }],
      'Apply at most the open balance; book the excess as an advance (前受金) in the payment module.',
    );
  }
  if (paidAmount.isNegative()) {
    throw new ValidationError(
      `un-applying ${amount.abs().toString()} exceeds the paid amount ${inv.paidAmount.toString()} of sales_invoice ${ref}`,
      [{ path: 'amount', message: `must be >= -${inv.paidAmount.toString()}` }],
      'Un-apply at most what was applied.',
    );
  }
  ...
  const updated = await withBalanceWrite(ctx, (internal) =>
    repo(internal, SalesInvoice).update(
      inv.id,
      { paidAmount, balance, status, settlementHistory: true },
