...
const goldenLines = (): Row[] => [
  { productId: product.a },
  { description: '商品B', unitPrice: '567', taxCategory: 'standard' },
  { productId: product.c },
];
...
async function createInvoice(ctx: Context, head: Row, lines: Row[] = goldenLines()): Promise<InvoiceWithLines> {
  const created = (await runAction(ctx, 'sales_invoice.create', {
    partnerId,
    date: '2026-04-10',
    ...head,
    lines: { sales_invoice_line: lines },
  })) as InvoiceWithLines;
...
beforeAll(async () => {
  ...
  product = {
    a: (
      await run({}, (ctx) =>
        repo(ctx, Product).create({ code: 'A', name: '商品A', salePrice: '1234', taxCategory: 'standard' }),
      )
    ).id,
    c: (
      await run({}, (ctx) =>
        repo(ctx, Product).create({ code: 'C', name: '食品C', salePrice: '1333', taxCategory: 'reduced' }),
      )
    ).id,
    noPrice: (await run({}, (ctx) => repo(ctx, Product).create({ code: 'Q', name: '見積品', taxCategory: 'standard' })))
      .id,
  };
});
afterAll(async () => {
...
describe('sales module (docs/specs/sales.md)', () => {
  it('AC-1 sales_invoice with lines: defaults (today JST, tax setting, due date from partner terms), computed fields, get returns lines', async () => {
    ...
    expect(
      inv.lines.sales_invoice_line.map((l) => [
        l.seq,
        l.productId,
        l.description,
        l.quantity,
    ...
    // validation
    await expect(
      run({}, (ctx) => runAction(ctx, 'sales_invoice.create', { date: '2026-04-10' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'partnerId' }] } });
    await expect(run({}, (ctx) => createInvoice(ctx, { date: '2026-13-01' }))).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'date' }] },
    });
    await expect(
      run({}, (ctx) => createInvoice(ctx, {}, [{ description: 'x', unitPrice: '1', taxCategory: 'vat' }])),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'taxCategory' }] } });
    await expect(
      run({}, (ctx) => createInvoice(ctx, {}, [{ description: 'x', taxCategory: 'standard' }])),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'unitPrice' }] } });
  });
  ...
  it('AC-2 recalculation on every path: product defaults, line save via update, header date/priceIncludesTax change, direct line write, tampering refused', async () => {
    ...
    expect(
      (await run({}, (ctx) => createInvoice(ctx, {}, [{ productId: product.noPrice, unitPrice: '10' }]))).total,
    ).toBe('11');
    ...
    const u = (await run({}, (ctx) =>
      runAction(ctx, 'sales_invoice.update', {
        id: inv.id,
        patch: {
          lines: {
            sales_invoice_line: [
    ...
    const incl = (await run({}, (ctx) =>
      runAction(ctx, 'sales_invoice.update', { id: inv.id, patch: { priceIncludesTax: true } }),
    )) as InvoiceJson;
    ...
    const noRate = await caught(
      run({}, (ctx) => runAction(ctx, 'sales_invoice.update', { id: inv.id, patch: { date: '2019-09-01' } })),
    );
    expect(noRate).toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ message: expect.stringContaining('reduced') }] },
    });
    ...
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'sales_invoice.update', {
          id: inv.id,
          patch: { subtotal: '1', total: '2', paidAmount: '3', balance: '4', status: 'paid', journalEntryId: null },
        }),
      ),
    ).rejects.toBeInstanceOf(PermissionDenied);
    const t = (await run({}, (ctx) =>
      runAction(ctx, 'sales_invoice.update', {
        id: inv.id,
        patch: { priceIncludesTax: false, dueDate: null, date: '2026-05-05' },
      }),
    )) as InvoiceJson;
    ...
    await expect(
      run({}, (ctx) => repo(ctx, SalesInvoiceLine).update(lineId, { amount: '999' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    ...
    const direct = await run({}, (ctx) =>
      repo(ctx, SalesInvoiceLine).create({ invoiceId: inv.id, productId: product.c } as never),
    );
    expect([direct.description, direct.unitPrice.toString(), direct.taxCategory, direct.amount.toString()]).toEqual([
      '食品C',
      '1333',
      'reduced',
      '1333',
    ...
    const refilled = await run({}, (ctx) =>
      repo(ctx, SalesInvoiceLine).update(direct.id, { description: null as never }),
    );
  ...
  it('AC-3 / AC-9 submit: ≥1 line, totals fixed, INV-<year>-<n>, status open, balance = total, journal entry with the golden lines; lines frozen afterwards', async () => {
    ...
    expect(je).toMatchObject({
      ...
      sourceId: inv.id,
      date: '2026-04-10',
      description: `売上請求書 ${inv.number} 得意先A`,
      reversalOf: null,
    });
    expect([je.totalDebit.toString(), je.totalCredit.toString()]).toEqual(['3420', '3420']);
    const lines = await run({}, (ctx) =>
      repo(ctx, JournalLine).list({ where: { entryId: je.id }, orderBy: [{ field: 'seq', dir: 'asc' }] }),
    );
    expect(
      lines.items.map((l) => [
        l.seq,
    ...
    expect(
      Decimal.from(after.get(acc.ar) ?? '0')
        .minus(before.get(acc.ar) ?? '0')
        .toString(),
    ).toBe('3420');
    expect(
      Decimal.from(after.get(acc.sales) ?? '0')
        .minus(before.get(acc.sales) ?? '0')
        .toString(),
    ).toBe('-3134');
    expect(
      Decimal.from(after.get(acc.tax) ?? '0')
        .minus(before.get(acc.tax) ?? '0')
        .toString(),
    ).toBe('-286');
    ...
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'sales_invoice.update', { id: inv.id, patch: { lines: { sales_invoice_line: [] } } }),
      ),
    ...
    await expect(
      run({}, (ctx) => repo(ctx, SalesInvoiceLine).create({ invoiceId: inv.id, productId: product.a } as never)),
    ).rejects.toBeInstanceOf(StateError);
    await expect(
      run({}, (ctx) => runAction(ctx, 'sales_invoice.update', { id: inv.id, patch: { date: '2026-04-11' } })),
    ...
    const zero = await run({}, (ctx) =>
      createInvoice(ctx, {}, [{ description: '無償', unitPrice: '0', taxCategory: 'standard' }]),
    );
  ...
  it('AC-3 missing account code -> INVALID_STATE with the seed/settings hint; the draft is not submitted', async () => {
    ...
    expect(err).toMatchObject({
      code: 'INVALID_STATE',
      hint: ACCOUNTS_HINT,
      details: { setting: SALES_ACCOUNTS_KEY, missing: { taxPayable: '2299' } },
    });
  ...
  it('AC-4 / AC-10 cancel reverses the entry (trial balance unchanged), refuses after a payment (hint), amend gives a clean draft', async () => {
    ...
    const reversals = await run({}, (ctx) =>
      repo(ctx, JournalEntry).list({ where: { reversalOf: inv.journalEntryId } }),
    );
    ...
    const closing = (t: Table) =>
      ['1300', '4000', '2200'].map((code) => [code, t.rows.find((r) => r.code === code)?.closingBalance]);
    ...
    await expect(run({}, (ctx) => runAction(ctx, 'sales_invoice.cancel', { id: inv.id }))).rejects.toBeInstanceOf(
      StateError,
    );
    ...
    const resubmitted = await run({}, async (ctx) => {
      await runAction(ctx, 'sales_invoice.update', {
        id: amended.id,
        patch: { lines: { sales_invoice_line: goldenLines() } },
      });
  ...
  it('AC-5 render_invoice_html: 適格請求書 fields from settings/partner/lines; layout replaceable via sales.invoice_html', async () => {
    ...
    const r1 = (await run(asRole(['viewer']), (ctx) =>
      runAction(ctx, 'sales.render_invoice_html', { id: inv.id }),
    )) as { html: string };
    ...
    for (const s of [
      ...
      expect(r1.html).toContain(s);
    await run({}, (ctx) =>
      setSetting(ctx, SALES_ISSUER_KEY, salesIssuerSchema, {
        name: '大福商事株式会社',
    ...
    const r2 = (await run({}, (ctx) => runAction(ctx, 'sales.render_invoice_html', { id: inv.id }))) as {
      html: string;
    };
    expect(r2.html).toBe(r1.html); // issued facts survive subsequent company setting changes
    const newIssued = await postInvoice({});
    const newHtml = (await run({}, (ctx) => runAction(ctx, 'sales.render_invoice_html', { id: newIssued.id }))) as {
      html: string;
    };
    ...
    await expect(
      run({}, (ctx) =>
        setSetting(ctx, SALES_ISSUER_KEY, salesIssuerSchema, { name: 'x', invoiceRegistrationNo: '1234567890123' }),
      ),
    ...
    try {
      const r3 = (await run({}, (ctx) => runAction(ctx, 'sales.render_invoice_html', { id: inv.id }))) as {
        html: string;
      };
    ...
    expect(
      data?.lines.map((l) => [l.seq, l.description, l.quantity, l.unitPrice, l.amount, l.taxCategory, l.rate]),
    ).toEqual([
    ...
    expect(
      ((await run({}, (ctx) => runAction(ctx, 'sales.render_invoice_html', { id: draft.id }))) as { html: string })
        .html,
    ).toContain('（下書き）');
    await expect(run({}, (ctx) => runAction(ctx, 'sales.render_invoice_html', { id: newId() }))).rejects.toMatchObject({
      code: 'NOT_FOUND',
    });
    await expect(
      run(asRole(['nobody']), (ctx) => runAction(ctx, 'sales.render_invoice_html', { id: inv.id })),
    ).rejects.toBeInstanceOf(PermissionDenied);
  });
  ...
  it('AC-7 record_payment / applyPayment: partial, final -> paid, over-payment VALIDATION, un-apply, drafts refused, no posting', async () => {
    ...
    const p1 = (await run(asRole(['sales']), (ctx) =>
      runAction(ctx, 'sales.record_payment', { invoiceId: inv.id, amount: '1000', date: '2026-05-01' }),
    )) as InvoiceJson;
    expect(p1).toMatchObject({ paidAmount: '1000', balance: '2420', status: 'open', docstatus: 1 });
    const over = await caught(
      run({}, (ctx) => applyPayment(ctx, { invoiceId: inv.id, amount: '2421', date: '2026-05-02' })),
    );
    expect(over).toBeInstanceOf(ValidationError);
    expect(issues(over)).toEqual([{ path: 'amount', message: 'must be <= 2420' }]);
    const p2 = await run({}, (ctx) =>
      applyPayment(ctx, { invoiceId: inv.id, amount: Decimal.from('2420'), date: '2026-05-02' }),
    );
    ...
    await expect(
      run({}, (ctx) => applyPayment(ctx, { invoiceId: inv.id, amount: '-1001', date: '2026-05-03' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'amount' }] } });
    await expect(
      run({}, (ctx) => applyPayment(ctx, { invoiceId: inv.id, amount: '0', date: '2026-05-03' })),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(
      run({}, (ctx) => applyPayment(ctx, { invoiceId: inv.id, amount: '1', date: '2026/05/03' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'date' }] } });
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'sales.record_payment', { invoiceId: inv.id, amount: 'abc', date: '2026-05-03' }),
      ),
    ).rejects.toBeInstanceOf(ValidationError);
    const events = await db.owner.sql<
      { payload: Row }[]
    >`select payload from outbox where topic = ${PAYMENT_APPLIED_EVENT} and tenant_id = ${db.tenantId}`;
    ...
    const draft = await run({}, (ctx) => createInvoice(ctx, {}));
    await expect(
      run({}, (ctx) => applyPayment(ctx, { invoiceId: draft.id, amount: '1', date: '2026-05-03' })),
    ).rejects.toMatchObject({ code: 'INVALID_STATE' });
    const cancelled = await postInvoice({});
    await run({}, (ctx) => runAction(ctx, 'sales_invoice.cancel', { id: cancelled.id }));
    await expect(
      run({}, (ctx) => applyPayment(ctx, { invoiceId: cancelled.id, amount: '1', date: '2026-05-03' })),
    ).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(
      run({}, (ctx) => applyPayment(ctx, { invoiceId: newId(), amount: '1', date: '2026-05-03' })),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
    // the audit trail shows the payment fields only
    const trail = await db.owner.sql<
      { after: Row }[]
    >`select after from audit_log where entity = 'sales_invoice' and record_id = ${inv.id} and op = 'update' order by at`;
    const paymentAudit = trail
      .map((t) => t.after)
      .filter((a) => Object.keys(a).sort().join(',') === 'balance,paidAmount');
    expect(
      paymentAudit.map((a) => [
        Decimal.from(String(a.paidAmount)).toString(),
        Decimal.from(String(a.balance)).toString(),
      ]),
    ).toEqual([['1000', '2420']]);
  });
  ...
  it('AC-6 ar_aging: open balances per partner bucketed by days overdue as of asOf; TableResult with totals', async () => {
    ...
    const mk = (date: string, dueDate: string, partner = c, lines = one) =>
      postInvoice({ partnerId: partner, date, dueDate }, lines);
    ...
    const t = (await run(asRole(['viewer']), (ctx) =>
      runAction(ctx, 'sales.ar_aging', { asOf: '2026-09-10' }),
    )) as Table;
    ...
    expect(earlier.rows.find((r) => r.partnerId === c)).toMatchObject({
      notDue: '1000',
      days1to30: '1000',
      over90: '0',
      total: '2000',
    ...
    await expect(run({}, (ctx) => runAction(ctx, 'sales.ar_aging', { asOf: '2026-9-1' }))).rejects.toBeInstanceOf(
      ValidationError,
    );
    await expect(run(asRole(['nobody']), (ctx) => runAction(ctx, 'sales.ar_aging', {}))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
  });
  ...
  it('AC-8 permissions: sales full cycle on invoice + lines, accounting/viewer read only, admin all', async () => {
    ...
    for (const role of ['accounting', 'viewer']) {
      ...
      await expect(
        run(asRole([role]), (ctx) => runAction(ctx, 'sales_invoice.update', { id: inv.id, patch: { note: 'x' } })),
      ).rejects.toBeInstanceOf(role === 'accounting' ? StateError : PermissionDenied);
      if (role === 'viewer')
        await expect(
          run(asRole([role]), (ctx) =>
            runAction(ctx, 'sales.record_payment', { invoiceId: inv.id, amount: '1', date: '2026-05-01' }),
          ),
        ).rejects.toBeInstanceOf(PermissionDenied);
      await expect(
        run(asRole([role]), (ctx) => runAction(ctx, 'sales_invoice.cancel', { id: inv.id })),
      ).rejects.toBeInstanceOf(PermissionDenied);
    }
    await expect(run(asRole(['sales']), (ctx) => repo(ctx, SalesInvoice).delete(inv.id))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
