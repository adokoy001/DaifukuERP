...
import { imbalance, journalLinesFor, ratePercent, type JournalLineSpec, type PostingInput } from '../src/services/posting.ts';
import { balanceOf, lineAmount, rateOfCategory, shapeSummary, totalsFrom, tryDecimal } from '../src/services/recalculate.ts';
import { defaultInvoiceHtml, escapeHtml, formatMoney, formatRate, REDUCED_MARK, type InvoiceRenderData } from '../src/services/render-html.ts';
...
const goldenSchema = z.object({
  input: z.object({
    ...
    lines: z.array(z.object({ seq: z.number().int(), description: z.string(), quantity: z.string(), unitPrice: z.string(), taxCategory: category })),
  }),
  expected: z.object({
    down: z.object({
      subtotal: z.string(),
      taxTotal: z.string(),
      total: z.string(),
      taxSummary: z.array(z.object({ category, code: z.string(), label: z.string(), rate: z.string(), taxable: z.string(), tax: z.string(), gross: z.string(), lineCount: z.number().int() })),
      journalLines: z.array(z.object({ accountId: z.string(), debit: z.string().nullable(), credit: z.string().nullable(), partnerId: z.string().nullable(), taxCategory: category.nullable(), taxRate: z.string().nullable(), memo: z.string().nullable() })),
    }),
...
const golden = goldenSchema.parse(JSON.parse(readFileSync(new URL('./golden/invoice-posting.json', import.meta.url), 'utf8')));
...
function pipeline(input: typeof golden.input, mode: RoundingMode) {
  ...
  const summary = summarizeTax(
    lines.map((l) => ({ amount: l.amount, category: l.taxCategory, rate: rateOf(input.rates, l.taxCategory), priceIncludesTax: input.priceIncludesTax })),
    { roundingMode: mode, scale: input.scale, priceIncludesTax: input.priceIncludesTax },
  );
  const totals = totalsFrom(summary);
  const journalLines = journalLinesFor({ partnerId: input.partnerId, priceIncludesTax: input.priceIncludesTax, lines, taxSummary: totals.taxSummary, total: totals.total }, input.accounts);
...
describe('AC-9 golden: invoice-posting.json', () => {
  ...
  it.each(['half_up', 'up'] as const)('rounding %s changes only the tax lines', (mode) => {
    ...
    expect([totals.subtotal.toString(), totals.taxTotal.toString(), totals.total.toString()]).toEqual([exp.subtotal, exp.taxTotal, exp.total]);
    expect(totals.taxSummary.map((g) => g.tax)).toEqual(exp.taxes);
    expect(journalLines.filter((l) => l.accountId === golden.input.accounts.taxPayable).map((l) => l.credit?.toString())).toEqual(exp.taxes);
...
describe('recalculate services', () => {
  ...
  it('AC-2 shapeSummary/totalsFrom keep group order and code/label when present; balance = total − paid', () => {
    const summary = summarizeTax([{ amount: '1000', category: 'reduced', rate: '0.08' }, { amount: '100', category: 'exempt', rate: '0' }], { roundingMode: 'down', scale: 0 });
    const rows = shapeSummary({ ...summary, groups: summary.groups.map((g) => ({ ...g, code: g.category === 'reduced' ? 'RED8' : 'EXEMPT', label: g.category === 'reduced' ? '軽減8%' : '免税' })) });
    expect(rows).toEqual([
      { category: 'reduced', code: 'RED8', label: '軽減8%', rate: '0.08', taxable: '1000', tax: '80', gross: '1080', lineCount: 1 },
      { category: 'exempt', code: 'EXEMPT', label: '免税', rate: '0', taxable: '100', tax: '0', gross: '100', lineCount: 1 },
    ]);
...
const RATES: Record<string, string> = { standard: '0.10', reduced: '0.08', exempt: '0', non_taxable: '0', out_of_scope: '0' };
const ACC = { receivable: 'ar', revenue: 'rev', taxPayable: 'tax' };

function postingFor(lines: { amount: string; taxCategory: TaxCategory; description?: string }[], priceIncludesTax: boolean, mode: RoundingMode = 'down') {
  const summary = summarizeTax(
  ...
  const input: PostingInput = {
    partnerId: 'p1',
    priceIncludesTax,
    lines: lines.map((l, i) => ({ seq: i + 1, description: l.description ?? `L${i + 1}`, amount: l.amount, taxCategory: l.taxCategory })),
...
describe('posting service (AC-3)', () => {
  it('税込 invoices post revenue per rate group (税抜), not per line', () => {
    ...
    const { totals, journalLines } = postingFor([{ amount: '1100', taxCategory: 'standard' }, { amount: '1650', taxCategory: 'standard' }, { amount: '540', taxCategory: 'reduced' }], true);
    expect([totals.subtotal.toString(), totals.taxTotal.toString(), totals.total.toString()]).toEqual(['3000', '290', '3290']);
    expect(journalLines.map(jsonLine)).toEqual([
      { accountId: 'ar', debit: '3290', credit: null, partnerId: 'p1', taxCategory: null, taxRate: null, memo: '売掛金' },
      { accountId: 'rev', debit: null, credit: '2500', partnerId: null, taxCategory: 'standard', taxRate: '0.1', memo: 'standard 税抜' },
      { accountId: 'rev', debit: null, credit: '500', partnerId: null, taxCategory: 'reduced', taxRate: '0.08', memo: 'reduced 税抜' },
      { accountId: 'tax', debit: null, credit: '250', partnerId: null, taxCategory: 'standard', taxRate: '0.1', memo: '仮受消費税 10%' },
      { accountId: 'tax', debit: null, credit: '40', partnerId: null, taxCategory: 'reduced', taxRate: '0.08', memo: '仮受消費税 8%' },
    ]);
  });

  it('zero-rate lines get a revenue line with rate 0 and no tax line; zero amounts and zero tax are skipped', () => {
    const { journalLines } = postingFor([{ amount: '500', taxCategory: 'exempt' }, { amount: '0', taxCategory: 'standard' }, { amount: '1', taxCategory: 'standard' }], false);
  ...
  it('a negative (discount) line becomes a debit on revenue so every side stays >= 0', () => {
    const { journalLines } = postingFor([{ amount: '1000', taxCategory: 'standard' }, { amount: '-100', taxCategory: 'standard', description: '値引' }], false);
    expect(journalLines.map(jsonLine)).toContainEqual({ accountId: 'rev', debit: '100', credit: null, partnerId: null, taxCategory: 'standard', taxRate: '0.1', memo: '値引' });
  ...
  it('property: journal lines always balance and debit the receivable with the invoice total (税抜 and 税込, every rounding)', () => {
    ...
    fc.assert(
      fc.property(fc.array(arbLine, { minLength: 1, maxLength: 8 }), fc.boolean(), fc.constantFrom<RoundingMode>('down', 'half_up', 'up'), (lines, inclusive, mode) => {
        ...
        const revenue = Decimal.sum(journalLines.filter((l) => l.accountId === 'rev').map((l) => l.credit ?? l.debit?.neg() ?? Decimal.zero()));
...
describe('aging service (AC-6)', () => {
  ...
  it('bucket boundaries: 0 -> not due, 1..30, 31..60, 61..90, 91+', () => {
    expect([-5, 0, 1, 30, 31, 60, 61, 90, 91, 400].map(bucketFor)).toEqual(['notDue', 'notDue', 'days1to30', 'days1to30', 'days31to60', 'days31to60', 'days61to90', 'days61to90', 'over90', 'over90']);
  });
  ...
  it('agingRows groups per partner (first-appearance order), null due date is not due, totals add up', () => {
    ...
    const b = rows.get('b');
    expect([b?.notDue, b?.days1to30, b?.days31to60, b?.days61to90, b?.over90, b?.total].map((d) => d?.toString())).toEqual(['7', '100', '0', '0', '1000', '1107']);
  ...
  it('property: Σbuckets = total per partner and Σtotals = Σinputs', () => {
    const arbItem = fc.record({
      ...
      dueDate: fc.option(fc.integer({ min: 0, max: 400 }).map((d) => `2026-${String(1 + (d % 12)).padStart(2, '0')}-${String(1 + (d % 28)).padStart(2, '0')}`), { nil: null }),
    ...
    fc.assert(
      fc.property(fc.array(arbItem, { maxLength: 30 }), (items) => {
        ...
        for (const r of rows.values()) expect(Decimal.sum([r.notDue, r.days1to30, r.days31to60, r.days61to90, r.over90]).eq(r.total)).toBe(true);
...
const renderData: InvoiceRenderData = {
  issuer: { name: 'テスト株式会社', invoiceRegistrationNo: 'T1234567890123', postalCode: '100-0001', address: '東京都千代田区1-1', phone: '03-0000-0000', email: 'billing@example.com', bankInfo: 'テスト銀行 本店 普通 1234567' },
  invoice: { number: 'INV-2026-000001', date: '2026-04-10', dueDate: '2026-05-31', note: '<b>注</b> & co', priceIncludesTax: false },
  recipient: { name: '得意先A', postalCode: '150-0001', address: '東京都渋谷区2-2' },
  lines: [
    { seq: 1, description: '商品A', quantity: '1', unitPrice: '1234', amount: '1234', taxCategory: 'standard', rate: '0.1' },
    { seq: 2, description: '食品C', quantity: '2.5', unitPrice: '533.2', amount: '1333', taxCategory: 'reduced', rate: '0.08' },
  ],
  taxSummary: [
...
describe('default invoice HTML (AC-5)', () => {
  ...
  it('税込 invoices label the amount column 税込 and show gross per rate; drafts show （下書き）; en locale', () => {
    const html = defaultInvoiceHtml({ ...renderData, invoice: { ...renderData.invoice, number: '', dueDate: null, note: null, priceIncludesTax: true } });
