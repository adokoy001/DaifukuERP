...
export async function assertWorkforcePeriodOpen(ctx: Context, employeeId: string, date: string): Promise<void> {
  const closed = await repo(ctx, WorkforcePeriodLock).count({
    employeeId,
    active: true,
  ...
  if (closed)
    throw new StateError(
      'Payroll period is finalized',
      'A different payroll reviewer must cancel the payslip with a reason before correcting this period.',
    );
}
