...
export async function portalData(ctx: Context, period: string, employeeId?: string) {
  ...
  const readable = <T extends EntityDef>(entity: T, where: Domain) =>
    can(ctx, entity, 'read') ? allRows(ctx, entity, where) : Promise.resolve([]);
  const employees = await readable(E.WorkforceEmployee, employeeId ? { id: employeeId } : {});
  const name = (row: { employeeId: string }) =>
    employees.find((employee) => employee.id === row.employeeId)?.name ?? '異動前の従業員';
  ...
  return {
    employees: employees.map(({ id, userId, siteId, code, name, active, hiredOn, terminatedOn, version }) => ({
      id,
      userId,
      siteId,
      code,
    ...
    attendances: attendance.map((row) => ({
      ...row,
      employeeName: name(row),
      clockIn: row.clockIn.toISOString(),
      clockOut: row.clockOut?.toISOString() ?? null,
      breakStartedAt: row.breakStartedAt?.toISOString() ?? null,
      workedMinutes: minutesDisplay(row.workedMs),
      nightMinutes: minutesDisplay(row.nightMs),
    })),
    ...
    payrolls: payrolls.map((row) => ({
      ...
      premiumPay: row.premiumPay.toString(),
      grossPay: row.grossPay.toString(),
      deductionTotal: row.deductionTotal.toString(),
      netPay: row.netPay.toString(),
      paidLeaveDays: row.paidLeaveDays.toString(),
      workedMinutes: minutesDisplay(row.workedMs),
    })) as ManagementPortal['payrolls'],
  };
}
