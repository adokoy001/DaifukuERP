...
export interface ShiftAvailability extends ShiftAvailabilityDay {
  employeeId: string;
}
...
export interface ShiftLeave {
  employeeId: string;
  date: string;
  portion: 'full' | 'morning' | 'afternoon';
...
export interface ShiftDayRule {
  ...
  breakMinutes: number;
  longBreakAfterMinutes: number;
  longBreakMinutes: number;
}
export interface ShiftWorkRule {
  employeeId: string;
  date: string;
  mode: 'ordinary' | 'monthly_variable' | 'flex';
  dailyLimitMinutes: number;
  weeklyLimitMinutes: number;
  startMinute: number;
  endMinute: number;
...
export interface ShiftPeriodBudget {
  employeeId: string;
  startsOn: string;
  endsOn: string;
...
export interface ShiftProblem {
  ...
  leave: ShiftLeave[];
  existing: ShiftExisting[];
  rules: ShiftDayRule[];
  workRules?: ShiftWorkRule[] | undefined;
  periodBudgets?: ShiftPeriodBudget[] | undefined;
}
export type ShiftIssueCode =
  | 'invalid_input'
  | 'unknown_employee'
  | 'unknown_slot'
  | 'duplicate'
...
export interface ShiftCoverage {
  ...
  assigned: number;
  shortage: number;
  exclusions: Partial<Record<ShiftIssueCode, number>>;
}
export interface ShiftEmployeeMetric {
  employeeId: string;
  minutes: number;
  targetMinutes: number;
  days: number;
...
export interface ShiftOptimizeOptions {
  seed: number;
  iterations?: number;
  assignments?: ShiftAssignment[];
}
