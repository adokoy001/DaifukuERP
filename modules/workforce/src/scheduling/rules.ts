...
const uniqueIssues = (issues: ShiftIssue[]): ShiftIssue[] => [
...
export function assignmentRows(ctx: Prepared, assignments: ShiftAssignment[]): Map<string, ShiftAssignment[]> {
  ...
  for (const assignment of assignments) {
    const rows = result.get(assignment.employeeId) ?? [];
    rows.push(assignment);
    result.set(assignment.employeeId, rows);
  }
  return result;
}
function timeline(ctx: Prepared, employeeId: string, assignments: readonly ShiftAssignment[]): Timeline[] {
  ...
  return result.sort(
    (a, b) => a.date.localeCompare(b.date) || a.startMinute - b.startMinute || a.endMinute - b.endMinute,
  );
}
    ...
    if (previous && (row.slotId !== undefined || previous.slotId !== undefined)) {
      ...
      if (code)
        result.push({
          ...
          ...(row.slotId ? { slotId: row.slotId } : previous.slotId ? { slotId: previous.slotId } : {}),
        });
    }
    if (end > previousEnd) {
      previous = row;
...
function calendarIssues(ctx: Prepared, rows: Timeline[], employee: ShiftEmployee): ShiftIssue[] {
  ...
  for (const row of rows) {
    ...
    current.minutes += workMinutes(row);
    current.breaks += row.breakMinutes;
    ...
    if (
      ...
      value.minutes >
        Math.min(
          ...
          rule.dailyLimitMinutes < 480
            ? Math.min(rule.dailyLimitMinutes, workRule?.dailyLimitMinutes ?? rule.dailyLimitMinutes)
            : (workRule?.dailyLimitMinutes ?? rule.dailyLimitMinutes),
        )
    )
      add('daily_limit');
    if (rule && value.breaks < requiredBreak(rule, value.minutes)) add('break');
  }
  const declared = ctx.problem.rules.flatMap((rule) => {
    const work = ctx.workRules.get(keyOf(employee.id, rule.date));
    return work ? [work.weeklyLimitMinutes] : [];
  });
  ...
  for (const budget of ctx.budgets.get(employee.id) ?? []) {
    const planned = rows
      .filter(
        (row) =>
          row.date >= ctx.problem.weekStart &&
          dayIndex(row.date) < ctx.start + 7 &&
          row.date >= budget.startsOn &&
          row.date <= budget.endsOn,
      )
      .reduce((sum, row) => sum + workMinutes(row), 0);
...
export function canAddToEmployee(
  ctx: Prepared,
  assignment: ShiftAssignment,
  employeeRows: readonly ShiftAssignment[],
  slotCount: number,
): boolean {
  const slot = ctx.slots.get(assignment.slotId);
  if (
    !slot ||
    !ctx.employees.has(assignment.employeeId) ||
    (ctx.staticIssues.get(keyOf(assignment.employeeId, assignment.slotId))?.length ?? 1)
  )
    return false;
...
export function canAdd(ctx: Prepared, assignment: ShiftAssignment, plan: ShiftAssignment[]): boolean {
  return canAddToEmployee(
    ctx,
    assignment,
    plan.filter((a) => a.employeeId === assignment.employeeId),
    plan.filter((a) => a.slotId === assignment.slotId).length,
  );
}
export function hardIssues(ctx: Prepared, assignments: ShiftAssignment[]): ShiftIssue[] {
  ...
  for (const assignment of assignments) {
    ...
    if (!ctx.employees.has(assignment.employeeId))
      issues.push({ code: 'unknown_employee', employeeId: assignment.employeeId, slotId: assignment.slotId });
    if (!ctx.slots.has(assignment.slotId))
      issues.push({ code: 'unknown_slot', employeeId: assignment.employeeId, slotId: assignment.slotId });
  ...
  for (const [slotId, count] of counts)
    if (count > (ctx.slots.get(slotId)?.required ?? Infinity)) issues.push({ code: 'over_capacity', slotId });
  for (const [employeeId, rows] of assignmentRows(ctx, assignments))
    issues.push(...employeeIssues(ctx, employeeId, rows));
