...
import { activeEmployee, assertPayrollOpen, command, employeeLock, expectVersion, identity, policyOn, replay, requireOther, requireSelf, reviewed } from '../common.ts';
...
async function request(ctx: Context, input: z.infer<typeof correctionInput>) {
  ...
  return employeeLock(ctx, initial.employeeId, async () => {
    ...
    const row = await repo(ctx, WorkforceAttendance).get(initial.id); expectVersion(row.version, input.expectedVersion);
    const employee = await activeEmployee(ctx, row.employeeId, row.workDate); await assertPayrollOpen(ctx, row.employeeId, row.workDate);
    if (await repo(ctx, WorkforceAttendanceCorrection).count({ attendanceId: row.id, status: 'pending' })) throw new StateError('A correction is already pending', 'Wait for its review.');
    const start = new Date(input.clockIn), end = new Date(input.clockOut);
    if (jstDate(start) !== row.workDate || end > ctx.now()) throw new ValidationError('Correction must keep the work date and use past times', [{ path: 'clockIn', message: 'Use the original work date and actual completed times.' }]);
    ...
    return command(await internalWrite(ctx, WorkforceAttendanceCorrection, (write) => repo(write, WorkforceAttendanceCorrection).create({ ...identity(employee), attendanceId: row.id, workDate: row.workDate, sourceVersion: row.version, clockIn: start, clockOut: end, breaks: input.breaks, reason: input.reason, idempotencyKey: input.idempotencyKey, requestSnapshot: input })));
  });
}
async function review(ctx: Context, input: z.infer<typeof correctionReviewInput>) {
  ...
  return employeeLock(ctx, initial.employeeId, async () => {
    const correction = await repo(ctx, WorkforceAttendanceCorrection).lock(initial.id); requireOther(ctx, correction); expectVersion(correction.version, input.expectedVersion);
    await activeEmployee(ctx, correction.employeeId, correction.workDate); await assertPayrollOpen(ctx, correction.employeeId, correction.workDate);
    if (correction.status !== 'pending') throw new StateError('Correction was already reviewed', 'Reload its current state.');
...
export const requestCorrectionAction = workflowAction('request_correction', '理由付きの勤怠訂正を申請', correctionInput, [E, M, H, P], request);
export const reviewCorrectionAction = workflowAction('review_correction', '勤怠訂正を承認・却下', correctionReviewInput, [M, H], review);
