...
export const shiftBoardAction = defineAction({
  name: 'workforce.shift_board',
  description: label('拠点の週間シフト計画', 'Site weekly shift planner'),
  input: shiftBoardInput,
  output: shiftBoardOutput,
  ...
  async handler(ctx, input) {
    ...
    return shiftBoardOutput.parse({
      ...
      problem: { ...source.problem, slots: draft?.slots ?? published?.slots ?? [] },
      employeeVersions: source.employees.map(({ id, version }) => ({ employeeId: id, version })),
      draft,
      published,
      publishedEvaluation: published
        ? evaluateShift({ ...source.problem, slots: published.slots }, published.assignments)
        : null,
    });
  },
});
export const myShiftsAction = defineAction({
  name: 'workforce.my_shifts',
  description: label('自分の勤務希望と公開シフト', 'My availability and published shifts'),
  input: myShiftsInput,
  output: myShiftsOutput,
  ...
  async handler(ctx, input) {
    ...
    if (!employee)
      return { weekStart: input.weekStart, employee: null, profile: null, availability: null, assignments: [] };
    const profile =
      (await repo(ctx, WorkforceShiftProfile).list({ where: { employeeId: employee.id }, limit: 1 })).items[0] ?? null;
    const availability =
      (
    ...
    const assignments = await allRows(
      ctx,
      WorkforceShiftAssignment,
      { employeeId: employee.id, active: true, date: { $gte: input.weekStart, $lte: addDays(input.weekStart, 6) } },
      [{ field: 'date' }, { field: 'startMinute' }],
    );
