...
async function profile(ctx: Context, input: z.infer<typeof saveShiftProfileInput>) {
  return employeeLock(ctx, input.employeeId, async () => {
    ...
    if (!(await repo(ctx, WorkforceSite).get(employee.siteId)).active)
      throw new StateError('Work site is inactive', 'Choose an active site.');
    const current = (await repo(ctx, WorkforceShiftProfile).list({ where: { employeeId: employee.id }, limit: 1 }))
      .items[0];
    expectVersion(current?.version ?? 0, input.expectedVersion);
    if (new Set(input.profile.skills).size !== input.profile.skills.length)
      throw new ValidationError('Skills must be unique', [
        { path: 'profile.skills', message: 'Remove duplicate skills.' },
      ]);
    return command(
      await internalWrite(ctx, WorkforceShiftProfile, (write) =>
        current
          ? repo(write, WorkforceShiftProfile).update(
              current.id,
              { profile: input.profile },
              { expectedVersion: current.version },
            )
          : repo(write, WorkforceShiftProfile).create({
...
async function availability(ctx: Context, input: z.infer<typeof saveShiftAvailabilityInput>) {
  ...
  return employeeLock(ctx, initial.id, async () => {
    ...
    if (date > addDays(input.weekStart, 6))
      throw new StateError('勤務希望の週は雇用期間外です', '在籍する週を選択してください。');
    const employee = await activeEmployee(ctx, initial.id, date);
    requireSelf(ctx, employee);
    const days = [...input.days].sort((a, b) => a.date.localeCompare(b.date));
    if (
      days.some(
        (day, index) =>
          day.date !== addDays(input.weekStart, index) ||
          day.endMinute < day.startMinute ||
          (day.preference !== 'unavailable' && day.endMinute <= day.startMinute),
      )
    )
      throw new ValidationError('Enter exactly seven valid availability days', [
        {
          path: 'days',
          message: 'Use each date of the selected week once and a positive daytime interval for available days.',
        },
      ]);
    const current = (
      await repo(ctx, WorkforceShiftAvailability).list({
        where: { employeeId: employee.id, weekStart: input.weekStart },
        limit: 1,
      })
    ).items[0];
    expectVersion(current?.version ?? 0, input.expectedVersion);
    return command(
      await internalWrite(ctx, WorkforceShiftAvailability, (write) =>
        current
...
export const saveShiftProfileAction = workflowAction(
  'save_shift_profile',
  '社員の勤務条件とスキルを保存',
  saveShiftProfileInput,
  [M, H],
  profile,
);
export const saveShiftAvailabilityAction = workflowAction(
  'save_shift_availability',
  '自分の週間勤務希望を保存',
