...
import {
  ...
  workSystemBoardAction,
} from './actions/work-system.ts';
import {
  initializePayrollRulesAction,
  savePayrollConditionAction,
  supersedePayrollConditionAction,
  calculateStatutoryPayrollAction,
  recordPayrollTaxEvidenceAction,
} from './actions/fiscal-payroll.ts';
import { fiscalBoardAction, myFiscalAction } from './actions/fiscal-portals.ts';
import {
  submitYearEndDeclarationAction,
  reviewYearEndDeclarationAction,
  calculateYearEndAction,
  confirmYearEndAction,
  cancelYearEndAction,
  settleYearEndAction,
} from './actions/year-end.ts';
...
export const WorkforceModule = defineModule({
  ...
  actions: [
    ...
    shiftBoardAction,
    myShiftsAction,
    saveShiftProfileAction,
    saveShiftAvailabilityAction,
    saveShiftPlanAction,
    publishShiftPlanAction,
    cancelShiftPlanAction,
    ...
    cancelPayrollAction,
    myPortalAction,
    managementPortalAction,
    initializePolicyAction,
  ],
  hooks: () => {
    registerWorkflowGuards();
    registerMasterGuards();
  },
  seed: seedWorkforce,
  menus: [
  ...
  roles: {
    workforce_employee: label('従業員本人', 'Employee'),
    workforce_manager: label('拠点管理者', 'Site manager'),
    workforce_hr: label('人事本部', 'HR headquarters'),
    workforce_payroll: label('給与・精算本部', 'Payroll headquarters'),
  },
});
