...
export function registerWorkflowGuards(): void {
  ...
  for (const entity of [entities.WorkforcePunch, entities.WorkforceLeaveGrant, entities.WorkforceLeaveUsage]) registry.registerHook(entity.name, 'before_update', () => { throw new StateError('Workforce ledger entries are append-only', 'Create a correcting transaction with its reason.'); });
