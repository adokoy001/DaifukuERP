...
export function payrollInsuranceRound(amount: Decimal): Decimal {
  const floor = amount.roundDown(0);
  return amount.minus(floor).gt('0.5') ? floor.plus(1) : floor;
}
...
export function validateCondition(input: PayrollConditionInput): void {
  ...
  for (const [membership, standard, grades, path] of [
    [input.healthMembership, input.healthStandardMonthly, HEALTH_GRADES, 'healthStandardMonthly'],
    [input.pensionMembership, input.pensionStandardMonthly, PENSION_GRADES, 'pensionStandardMonthly'],
  ] as const) {
...
export function socialInsurance(
  ...
  const healthTable = FISCAL_DATA.health.find((row) => row.from <= insurancePeriod && row.to >= insurancePeriod),
    nursingTable = FISCAL_DATA.nursing.find((row) => row.from <= insurancePeriod && row.to >= insurancePeriod);
  ...
  if (!healthTable || !nursingTable || !employmentTable)
    fail(
      'insurancePeriod',
      'This rules version covers insurance December 2025–December 2026 and regular payments in 2026.',
    );
  ...
  if (condition.healthMembership === 'enrolled' && birthdayAge >= 75)
    fail(
      'healthMembership',
      'Later-stage elderly insurance is outside this calculation; verify the actual membership.',
    );
  if (condition.pensionMembership === 'enrolled' && age >= 70)
    fail('pensionMembership', 'Ordinary pension enrollment at age 70 or over is unsupported.');
  ...
  const combinedHealth = payrollInsuranceRound(
    healthBase.times(D(healthPercent).plus(nursing ? nursingTable.percent : 0)).div(200),
  );
  const childSupport =
    insurancePeriod >= FISCAL_DATA.childSupportFrom
      ? payrollInsuranceRound(healthBase.times(FISCAL_DATA.childSupportPercent).div(200))
      : D(0);
  const pension =
    condition.pensionMembership === 'enrolled'
      ? payrollInsuranceRound(
          D(condition.pensionStandardMonthly ?? '0')
            .times(FISCAL_DATA.pensionPercent)
            .div(200),
        )
      : D(0);
  const employment =
    condition.employmentMembership === 'enrolled'
      ? payrollInsuranceRound(wage.times(employmentTable[condition.employmentCategory]))
      : D(0);
  return {
    health,
    ...
    childSupportPercent: insurancePeriod >= FISCAL_DATA.childSupportFrom ? FISCAL_DATA.childSupportPercent : '0',
    pensionPercent: FISCAL_DATA.pensionPercent,
    employmentRate: employmentTable[condition.employmentCategory],
    rounding: FISCAL_DATA.insuranceRounding,
  };
}
