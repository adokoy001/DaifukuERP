...
function termsOn(terms: readonly PayTerms[], date: string): PayTerms {
  ...
  if (found.length !== 1)
    throw new StateError('Missing or overlapping pay terms', `Exactly one confirmed pay condition must cover ${date}.`);
  return found[0] as PayTerms;
}
function hourlyRate(term: PayTerms): Decimal {
  return term.payType === 'hourly' ? D(term.hourlyRate) : D(term.monthlySalary).times(60).div(term.monthlyBaseMinutes);
}
...
function validateDays(input: PayInput): void {
  ...
  for (const day of input.days) {
    if (
      seen.has(day.date) ||
      !Number.isSafeInteger(day.workedMs) ||
      !Number.isSafeInteger(day.nightMs) ||
      day.workedMs < 0 ||
      day.nightMs < 0 ||
      day.nightMs > day.workedMs
    )
      throw new ValidationError('Invalid or duplicate payroll attendance', [
        { path: 'days', message: 'Use unique days with exact nonnegative milliseconds.' },
      ]);
    seen.add(day.date);
    weeks.add(day.policy.weekStartsOn);
  }
  if (weeks.size > 1)
    throw new StateError(
      'Workweek basis changes within payroll input',
      'Use one consistent workweek origin across the payroll month and its boundary week.',
    );
  for (const leave of input.leaves)
    if (D(leave.days).lte(0) || D(leave.days).gt(1))
      throw new ValidationError('Invalid paid leave quantity', [
        { path: 'leave', message: 'Use approved positive full or half days.' },
      ]);
}
function attendanceCosts(input: PayInput) {
  ...
  for (const day of [...input.days].sort((a, b) => a.date.localeCompare(b.date))) {
    ...
    const baseMs =
      term.payType === 'hourly' ? day.workedMs : extraMs + holidayMs + (classification?.regularSupplementMs ?? 0);
    const baseAmount = rate.times(baseMs).div(3600000);
    const premiumAmount = rate
      .times(
        D(extraMs - highMs)
          .times(policy.overtimePremiumRate)
          .plus(D(highMs).times(policy.highOvertimePremiumRate))
          .plus(D(holidayMs).times(policy.holidayPremiumRate))
          .plus(D(day.nightMs).times(policy.nightPremiumRate)),
      )
      .div(3600000);
    base = base.plus(baseAmount);
    premium = premium.plus(premiumAmount);
    details.push({
      date: day.date,
      termsId: term.id,
      policyId: policy.id,
      workedMs: day.workedMs,
      regularMs: Math.max(0, day.workedMs - extraMs - holidayMs),
      regularSupplementMs: classification?.regularSupplementMs ?? 0,
      workSystem: classification?.workSystem ?? 'ordinary',
      overtimeMs: extraMs,
      highOvertimeMs: highMs,
      holidayMs,
      nightMs: day.nightMs,
      hourlyBasis: rate.toString(),
...
export function calculatePay(input: PayInput) {
  ...
  for (const leave of input.leaves) {
    if (leave.date < input.employmentStart || leave.date > input.employmentEnd)
      throw new StateError(
        'Leave is outside the payroll employment interval',
        'Check the employment and paid leave dates.',
      );
    const term = termsOn(input.terms, leave.date);
    paidLeaveDays = paidLeaveDays.plus(leave.days);
    if (term.payType === 'hourly')
      leavePay = leavePay.plus(
        hourlyRate(term)
          .times(leaveMinutes(input, leave.date, term.paidLeaveDayMinutes))
          .times(leave.days)
          .div(60),
      );
  }
  ...
  return {
    basePay,
    premiumPay,
    grossPay: basePay.plus(premiumPay),
    workedMs: work.workedMs,
    paidLeaveDays,
    details: work.details,
    monthlyBase: monthly.toString(),
    paidLeavePay: leavePay.toString(),
    overtimeMs: work.overtimeMs,
    rounding: 'Exact milliseconds; base and premium totals rounded up to whole JPY.',
    monthlyProration:
