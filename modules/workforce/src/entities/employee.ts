...
export const WorkforceEmployee = defineEntity({
  ...
  fields: {
    userId: f.uuid({ ...owned, required: true, immutable: true, unique: true, label: label('会社所属の利用者', 'Company user') }),
  ...
  permissions: { roles: { [E]: read, [M]: read, [H]: edit, [P]: read }, rowRules: [{ roles: [E], where: { userId: '$ctx.userId' } }] },
