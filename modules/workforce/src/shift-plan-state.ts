...
export async function shiftPlanLock<T>(
  ctx: Context,
  siteId: string,
  weekStart: string,
  work: () => Promise<T>,
  ...
  return withLock(ctx, 'workforce:policies', () =>
    withLock(ctx, `workforce:shift:${siteId}:${weekStart}`, async () => {
...
export async function checkedSource(ctx: Context, siteId: string, weekStart: string, revision: string) {
  ...
  if (source.sourceRevision !== revision)
    throw new Conflict(
      'シフトの基礎情報が更新されています',
      '最新の社員情報・勤務希望・休暇・周辺の公開シフトを読み直して再計算し、保存してください。',
    );
  return source;
}
export function assertDraft(status: string): void {
  if (status !== 'draft')
    throw new StateError('公開済みの計画は直接変更できません', '新しい下書きを作成して改訂してください。');
}
  ...
  if (evaluation.issues.length)
    throw new ValidationError(
      ...
      evaluation.issues.map((issue) => ({
        path: 'assignments',
        message: `${issue.code}${issue.employeeId ? ` (${issue.employeeId})` : ''}${issue.slotId ? ` [${issue.slotId}]` : ''}`,
      })),
    );
  return evaluation;
}
export async function deactivateAssignments(ctx: Context, planId: string): Promise<void> {
  ...
  for (const row of rows)
    await internalWrite(ctx, WorkforceShiftAssignment, (write) =>
      repo(write, WorkforceShiftAssignment).update(row.id, { active: false }, { expectedVersion: row.version }),
    );
}
export async function currentPlan(ctx: Context, id: string, expectedVersion: number) {
  ...
  if (row.version !== expectedVersion)
    throw new Conflict('シフト計画が更新されています', '最新の計画を読み直してください。');
  return row;
}
