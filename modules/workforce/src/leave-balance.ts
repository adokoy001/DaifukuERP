...
export async function availableGrants(ctx: Context, employeeId: string, date: string) {
  const grants = await allRows(ctx, WorkforceLeaveGrant, { employeeId, validFrom: { $lte: date }, expiresOn: { $gte: date } }, [{ field: 'expiresOn' }, { field: 'id' }]);
  const usage = await allRows(ctx, WorkforceLeaveUsage, { employeeId });
  return grants.map((grant) => ({ grant, available: grant.days.minus(Decimal.sum(usage.filter((row) => row.grantId === grant.id).map((row) => row.days))) }));
}
...
export function leaveDays(portion: string): Decimal { return D(portion === 'full' ? '1' : '0.5'); }
