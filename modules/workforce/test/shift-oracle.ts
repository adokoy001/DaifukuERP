...
const minutes = (row: { startMinute: number; endMinute: number; breakMinutes: number }) =>
  row.endMinute - row.startMinute - row.breakMinutes;
function assertScope(problem: ShiftProblem): void {
  if (
    problem.employees.length > 3 ||
    problem.slots.length > 4 ||
    problem.workRules?.length ||
    problem.periodBudgets?.length
  )
...
function employeeFeasible(problem: ShiftProblem, employee: ShiftEmployee, own: ShiftAssignment[]): boolean {
  ...
  for (const slot of planned) {
    if (slot.date < employee.hiredOn || (employee.terminatedOn !== null && slot.date > employee.terminatedOn))
      return false;
  ...
  const rows = [
    ...problem.existing.filter((row) => row.employeeId === employee.id).map((row) => ({ ...row, planned: false })),
    ...planned,
  ];
  ...
  const weekRows = rows.filter(
    (row) => row.date >= problem.weekStart && dayNumber(row.date) < dayNumber(problem.weekStart) + 7,
  );
  if (
    weekRows.reduce((sum, row) => sum + minutes(row), 0) >
    Math.min(profile.maxWeeklyMinutes, ...problem.rules.map((row) => row.weeklyLimitMinutes))
  )
    return false;
...
function consecutiveFeasible(rows: (ShiftExisting & { planned: boolean })[], maximum: number): boolean {
  ...
  for (const start of dates) {
    ...
    if (run.every((day) => dates.includes(day)) && rows.some((row) => row.planned && run.includes(dayNumber(row.date))))
      return false;
  }
  return true;
}
export function oracleFeasible(problem: ShiftProblem, assignments: ShiftAssignment[]): boolean {
  ...
  if (
    assignments.some(
      (row) =>
        !problem.employees.some((employee) => employee.id === row.employeeId) ||
        !problem.slots.some((slot) => slot.id === row.slotId),
    )
  )
    return false;
  if (problem.slots.some((slot) => assignments.filter((row) => row.slotId === slot.id).length > slot.required))
    return false;
  return problem.employees.every((employee) =>
    employeeFeasible(
      problem,
      employee,
...
export function oracleShortage(problem: ShiftProblem, assignments: ShiftAssignment[]): number {
  return problem.slots.reduce(
    (sum, slot) => sum + Math.max(0, slot.required - assignments.filter((row) => row.slotId === slot.id).length),
    0,
  );
}
export function* enumerateAssignments(
  problem: ShiftProblem,
  fixed: ShiftAssignment[] = [],
): Generator<ShiftAssignment[]> {
  assertScope(problem);
  const decisions = problem.employees.flatMap((employee) =>
    problem.slots.map((slot) => ({
      employeeId: employee.id,
      slotId: slot.id,
      locked: fixed.some((row) => row.employeeId === employee.id && row.slotId === slot.id),
    })),
  );
  for (let mask = 0; mask < 2 ** decisions.length; mask++) {
    ...
    if (
      fixed.every((row) =>
        rows.some((candidate) => candidate.employeeId === row.employeeId && candidate.slotId === row.slotId),
      )
    )
      yield rows;
  }
