...
async function punch(kind: string, version: number, time: string, key = newId()) {
  return call(f.db, f.alice.params, 'punch', { kind, expectedVersion: version, idempotencyKey: key }, time);
}
describe('workforce attendance invariants', () => {
  it('serializes double clock-in, preserves seconds, and rejects generic ownership bypass', async () => {
    ...
    const results = await Promise.all([
      call(f.db, f.alice.params, 'punch', input, '2026-08-03T09:00:00.123+09:00'),
      call(f.db, f.alice.params, 'punch', input, '2026-08-03T09:00:00.123+09:00'),
    ]);
    ...
    await expect(
      f.db.run(f.alice.params, (ctx) => repo(ctx, WorkforceAttendance).update(row.id, { workedMs: 0 })),
    ...
    await expect(call(f.db, f.alice.params, 'punch', { ...input, expectedVersion: 1 })).rejects.toMatchObject({
      code: 'CONFLICT',
    });
  });
  it('applies employee and site filters to reads, counts, aggregates and portals', async () => {
    for (const person of [f.bob, f.remote]) {
      ...
      const portal = (await f.db.run(person.params, (ctx) =>
        runAction(ctx, person === f.bob ? 'workforce.my_portal' : 'workforce.management_portal', { period: '2026-08' }),
      )) as { attendances: unknown[]; users?: unknown[] };
  ...
  it('records clock-out despite insufficient breaks, then requires reviewed correction', async () => {
    ...
    const correction = await call(f.db, f.alice.params, 'request_correction', {
      attendanceId: row.id,
      expectedVersion: submitted.version,
      idempotencyKey: newId(),
      clockIn: '2026-08-04T09:00:00+09:00',
      clockOut: '2026-08-04T18:00:00+09:00',
      breaks: [{ start: '2026-08-04T12:00:00+09:00', end: '2026-08-04T13:00:00+09:00' }],
      reason: '休憩打刻漏れ',
    });
    await expect(
      call(f.db, { ...f.alice.params, roles: ['workforce_manager'] }, 'review_correction', {
  ...
  it('recovers a forgotten clock-out after 24 hours without deleting the original punch', async () => {
    ...
    const correction = await call(f.db, f.alice.params, 'request_correction', {
      ...
      idempotencyKey: newId(),
      clockIn: '2026-08-05T09:00:00+09:00',
      clockOut: '2026-08-05T10:00:00+09:00',
      breaks: [],
      reason: '退勤忘れ',
    });
    await call(f.db, f.manager.params, 'review_correction', {
  ...
  it('rejects correction overlap with the next shift and protects employee transfer while unfinished', async () => {
    ...
    const correction = await call(f.db, f.alice.params, 'request_correction', {
      ...
      expectedVersion: row.version,
      idempotencyKey: newId(),
      clockIn: '2026-08-07T22:00:00+09:00',
      clockOut: '2026-08-08T04:30:00+09:00',
      breaks: [{ start: '2026-08-08T01:00:00+09:00', end: '2026-08-08T02:00:00+09:00' }],
      reason: '訂正',
    });
    await expect(
    ...
    expect((await f.db.run({}, (ctx) => repo(ctx, WorkforceAttendanceCorrection).get(correction.id))).status).toBe(
      'pending',
    );
    await expect(
      f.db.run(f.hr.params, (ctx) => repo(ctx, WorkforceEmployee).update(f.employee.id, { siteId: f.otherSiteId })),
    ).rejects.toBeInstanceOf(StateError);
  });
});
