...
async function save(
  ...
  return call(f.db, f.manager.params, 'save_shift_plan', {
    siteId: f.siteId,
    weekStart,
    expectedVersion: 0,
    sourceRevision,
...
describe('shift planner preserves live constraints and employment history', () => {
  it('flags published work after pending leave without disclosing the reason, and revalidates revisions', async () => {
    ...
    const request = await call(f.db, f.alice.params, 'request_leave', {
      leaveDate: weekStart,
      portion: 'morning',
      reason: '公開しない個人的な事情',
    ...
    expect(source.problem.leave).toEqual([
      { employeeId: f.employee.id, date: weekStart, portion: 'morning', status: 'pending' },
    ]);
    expect(JSON.stringify(source)).not.toContain('公開しない個人的な事情');
    expect(source.publishedEvaluation?.issues).toContainEqual(
      expect.objectContaining({ code: 'leave', employeeId: f.employee.id }),
    );
    ...
    await call(f.db, f.manager.params, 'review_leave', {
      requestId: request.id,
      expectedVersion: request.version,
      decision: 'approve',
      halfDayAgreement: true,
      reason: '半日取得に合意',
    });
    const approved = await board(f, weekStart);
    expect(approved.problem.leave[0]?.status).toBe('approved');
  ...
  it('detects conditions changed after draft save and applies the stricter individual limit', async () => {
    ...
    const updated = await call(f.db, f.manager.params, 'save_shift_profile', {
      ...
      profile: { ...profile, maxDailyMinutes: 120 },
    });
  ...
  it('includes adjacent published days and rejects a rest conflict across the week boundary', async () => {
    ...
    expect(next.problem.existing).toContainEqual({
      employeeId: f.employee.id,
      date: late.date,
      startMinute: 1200,
      endMinute: 1440,
    ...
    const current = await board(f, weekStart);
    expect(current.problem.existing.every((row) => row.date < weekStart || row.date > addDays(weekStart, 6))).toBe(
      true,
    );
  });
  it('serializes concurrent publish and refuses stale draft edits without changing public history', async () => {
    ...
    expect(
      await f.db.run(f.manager.params, (ctx) => repo(ctx, WorkforceShiftAssignment).count({ planId: row.id })),
    ).toBe(1);
    const current = await board(f, weekStart);
    await expect(
      call(f.db, f.manager.params, 'cancel_shift_plan', {
        planId: row.id,
        expectedVersion: row.version,
        reason: '古い画面の取消',
      }),
  ...
  it('protects published commitments and site history during employee lifecycle changes', async () => {
    ...
    for (const change of [{ active: false }, { terminatedOn: '2026-11-08' }, { siteId: f.otherSiteId }]) {
      await expect(
        f.db.run({ ...f.hr.params, ...at(NOW) }, (ctx) =>
          repo(ctx, WorkforceEmployee).update(before.id, change, { expectedVersion: before.version }),
        ),
      ).rejects.toBeDefined();
    ...
    const publicRows = await f.db.run(f.manager.params, (ctx) =>
      repo(ctx, WorkforceShiftAssignment).list({ where: { employeeId: before.id, active: true } }),
    );
  ...
  it('serializes site deactivation against publication and preserves a consistent active site', async () => {
    ...
    const outcomes = await Promise.allSettled([
      publish(f, row),
      f.db.run({ ...f.hr.params, ...at(NOW) }, (ctx) =>
        repo(ctx, WorkforceSite).update(site.id, { active: false }, { expectedVersion: site.version }),
      ),
    ]);
    // Other future published rows already prohibit deactivation; the raced plan still publishes safely.
    expect(outcomes[0]?.status).toBe('fulfilled');
    expect(outcomes[1]).toMatchObject({ status: 'rejected', reason: { code: 'INVALID_STATE' } });
  ...
  it('denies cross-company and cross-tenant planning with no snapshot disclosure', async () => {
    ...
    const other = await bootstrapTenant(f.db.owner, {
      ...
      companyCode: 'OT',
      companyName: '別テナント会社',
      adminEmail: 'other-shift@example.com',
      adminName: 'Other',
      adminPassword: 'test-password',
    });
    for (const scope of [{ companyId: otherCompanyId }, { tenantId: other.tenantId, companyId: other.companyId }]) {
      await expect(
        f.db.run({ ...f.manager.params, ...scope }, (ctx) =>
          runAction(ctx, 'workforce.shift_board', { siteId: f.siteId, weekStart: '2026-09-14' }),
        ),
  ...
  it('refuses an unsupported company workweek without silently changing weekly limits', async () => {
    ...
    const modified = await f.db.run(f.hr.params, (ctx) =>
      repo(ctx, WorkforcePayPolicy).update(policy.id, { weekStartsOn: 0 }, { expectedVersion: policy.version }),
    );
    await expect(board(f, '2026-09-14')).rejects.toMatchObject({
      code: 'INVALID_STATE',
      message: 'シフト推薦は月曜始まりの会社制度に対応しています',
    });
    await f.db.run(f.hr.params, (ctx) =>
      repo(ctx, WorkforcePayPolicy).update(policy.id, { weekStartsOn: 1 }, { expectedVersion: modified.version }),
    );
  });
  it('permits a no-history transfer with parent-scoped conditions and prepares future hires', async () => {
    ...
    const saved = await call(f.db, f.manager.params, 'save_shift_profile', {
      employeeId: row.id,
      expectedVersion: 0,
      profile,
    });
    ...
    expect((await f.db.run(f.remote.params, (ctx) => repo(ctx, WorkforceShiftProfile).get(saved.id))).employeeId).toBe(
      row.id,
    );
    await expect(
      f.db.run(f.manager.params, (ctx) => repo(ctx, WorkforceShiftProfile).get(saved.id)),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
    const weekStart = '2026-10-12';
    await call(f.db, { ...person.params, siteIds: [f.otherSiteId] }, 'save_shift_availability', {
      weekStart,
      expectedVersion: 0,
      days: availability(weekStart),
    });
    const own = await f.db.run({ ...person.params, siteIds: [f.otherSiteId] }, (ctx) =>
      runAction(ctx, 'workforce.my_shifts', { weekStart }),
    );
