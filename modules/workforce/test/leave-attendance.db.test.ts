...
const punch = (w: Worker, kind: string, version: number, time: string) =>
  call(f.db, w.params, 'punch', { kind, expectedVersion: version, idempotencyKey: newId() }, time);
...
const request = (w: Worker, portion = 'full') =>
  call(f.db, w.params, 'request_leave', { leaveDate: '2026-09-11', portion, idempotencyKey: newId(), reason: '私用' });
const approveLeave = (row: Command) =>
...
const approveAttendance = (row: Command) =>
  call(f.db, f.manager.params, 'review_attendance', {
    attendanceId: row.id,
    expectedVersion: row.version,
    decision: 'approve',
    dayKind: 'workday',
...
const usageCount = (w: Worker) =>
  f.db.run(w.params, (ctx) => repo(ctx, WorkforceLeaveUsage).count({ employeeId: w.id }));

describe('full-day leave and actual work on every JST calendar date', () => {
  it.each(['full', 'two halves'])(
    'rejects %s approval after a shift crosses into the leave date without consuming balance',
    async (kind) => {
  ...
  it('rejects an approved-shift correction extending into full-day leave and preserves both records', async () => {
    ...
    const correction = await call(f.db, w.params, 'request_correction', {
      ...
      clockIn: '2026-09-10T22:00:00+09:00',
      clockOut: '2026-09-11T02:00:00+09:00',
      breaks: [],
      reason: '退勤時刻の訂正',
    });
    await expect(
      call(f.db, f.manager.params, 'review_correction', {
    ...
    expect(
      (await f.db.run(w.params, (ctx) => repo(ctx, WorkforceAttendanceCorrection).get(correction.id))).status,
    ).toBe('pending');
    ...
    expect(unchanged.clockOut?.toISOString()).toBe('2026-09-10T14:00:00.000Z');
    expect(await usageCount(w)).toBe(1);
  });
  it.each(['leave first', 'attendance first'])('allows a shift ending exactly at midnight: %s', async (order) => {
    ...
    expect((await approveAttendance(await submit(w, await shift(w, '2026-09-11T00:00:00+09:00')))).status).toBe(
      'approved',
    );
    if (order === 'attendance first') await approveLeave(await request(w));
    expect(await usageCount(w)).toBe(1);
  });
  it.each(['leave first', 'attendance first'])(
    'excludes a recorded break covering the next-date segment: %s',
    async (order) => {
      const w = await worker();
      if (order === 'leave first') await approveLeave(await request(w));
      const started = await punch(w, 'clock_in', 0, '2026-09-10T22:00:00+09:00');
      const correction = await call(f.db, w.params, 'request_correction', {
        attendanceId: started.id,
        expectedVersion: started.version,
        idempotencyKey: newId(),
        clockIn: '2026-09-10T22:00:00+09:00',
        clockOut: '2026-09-11T02:00:00+09:00',
        breaks: [{ start: '2026-09-11T00:00:00+09:00', end: '2026-09-11T02:00:00+09:00' }],
        reason: '実際の勤務・休憩確認',
      });
      await call(f.db, f.manager.params, 'review_correction', {
        correctionId: correction.id,
        expectedVersion: correction.version,
        decision: 'approve',
        reason: '確認',
  ...
  it('blocks leave approval for actual elapsed work in an unclosed previous-date shift', async () => {
    const w = await worker();
    await punch(w, 'clock_in', 0, '2026-09-10T22:00:00+09:00');
