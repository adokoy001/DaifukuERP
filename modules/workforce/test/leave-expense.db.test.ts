...
async function grant(w: Worker, days: string, validFrom = '2026-09-01', expiresOn = '2026-09-30') {
  return call(f.db, f.hr.params, 'grant_leave', {
    employeeId: w.employeeId,
    validFrom,
    expiresOn,
    days,
    eligibilityConfirmed: true,
    basis: '雇用・出勤条件確認済みの演習付与',
  });
}
const balance = (w: Worker, date = '2026-09-12') =>
  f.db.run(w.params, async (ctx) => (await leaveBalance(ctx, w.employeeId, date)).toString());
const requestLeave = (w: Worker, leaveDate: string, portion = 'full', idempotencyKey = newId()) =>
  call(f.db, w.params, 'request_leave', { leaveDate, portion, idempotencyKey, reason: '私用のため' });
const reviewLeave = (row: Command, halfDayAgreement = false) =>
  call(f.db, f.manager.params, 'review_leave', {
    requestId: row.id,
    expectedVersion: row.version,
    decision: 'approve',
    halfDayAgreement,
    reason: '業務調整と本人の希望を確認',
  });
const withdraw = (w: Worker, row: Command) =>
  call(f.db, w.params, 'cancel_leave', {
    requestId: row.id,
    expectedVersion: row.version,
    reason: '本人都合により取消',
  });
async function expense(w: Worker, overrides: Record<string, unknown> = {}) {
  const input = {
    ...
    category: '交通費',
    description: '訪問交通費（演習）',
    amount: '1000',
    evidence: '演習の領収書参照-001',
    ...overrides,
  };
...
async function approveExpense(w: Worker, row: Command) {
  ...
  return call(f.db, f.manager.params, 'review_expense', {
    expenseId: row.id,
...
describe('paid leave: eligibility, expiry and serialized ledger usage', () => {
  it('requires explicit eligibility, positive full/half days and valid employment/expiry dates', async () => {
    ...
    for (const patch of [{ eligibilityConfirmed: false }, { days: '0' }, { days: '0.25' }, { expiresOn: '2026-08-31' }])
      await expect(call(f.db, f.hr.params, 'grant_leave', { ...input, ...patch })).rejects.toMatchObject({
        code: 'VALIDATION',
      });
  ...
  it('uses earliest expiry first, requires half-day agreement, and restores only the original grant on cancellation', async () => {
    ...
    const usages = await f.db.run(w.params, (ctx) =>
      repo(ctx, WorkforceLeaveUsage).list({ where: { requestId: request.id } }),
    );
    expect(usages.items).toHaveLength(1);
    expect(usages.items[0]?.grantId).toBe(first.id);
    expect(await balance(w)).toBe('1.5');
    expect(await balance(w, '2026-09-13')).toBe('1');
    const cancelled = await withdraw(w, approved);
    expect(await balance(w)).toBe('2');
    expect(await balance(w, '2026-09-13')).toBe('1');
    await expect(withdraw(w, cancelled)).rejects.toMatchObject({ code: 'INVALID_STATE' });
    const ledger = await f.db.run(w.params, (ctx) =>
      repo(ctx, WorkforceLeaveUsage).list({ where: { requestId: request.id } }),
    );
    expect(ledger.total).toBe(2);
    expect(Decimal.sum(ledger.items.map((row) => row.days)).toString()).toBe('0');
  });
  it('rejects grants not yet effective or expired on the requested use date', async () => {
    const w = await worker();
    await grant(w, '1', '2026-08-01', '2026-08-31');
    await grant(w, '1', '2026-09-10', '2026-09-30');
    await expect(requestLeave(w, '2026-09-01')).rejects.toMatchObject({ code: 'INVALID_STATE' });
    expect(await balance(w, '2026-09-09')).toBe('0');
    expect(await balance(w, '2026-09-10')).toBe('1');
  });
  ...
  it('replays one request, rejects changed input / overlapping portions, and cancellation does not resurrect a replayed request', async () => {
    ...
    const pair = await Promise.all([
      requestLeave(w, '2026-09-22', 'morning', key),
      requestLeave(w, '2026-09-22', 'morning', key),
    ]);
    ...
    expect(await requestLeave(w, '2026-09-22', 'morning', key)).toMatchObject({
      id: first.id,
      status: 'cancelled',
    ...
    expect(await balance(w)).toBe('2');
    expect(
      (await f.db.run(w.params, (ctx) => repo(ctx, WorkforceLeaveUsage).list({ where: { requestId: first.id } })))
        .total,
    ).toBe(0);
  });
  it('rejects out-of-site and self approval, and a rejected request consumes no leave', async () => {
    ...
    const w = { employeeId: employee.id, params: f.manager.params };
    await grant(w, '1');
    ...
    await expect(
      call(f.db, f.remote.params, 'review_leave', {
        requestId: request.id,
        expectedVersion: request.version,
        decision: 'approve',
        reason: '別拠点',
        halfDayAgreement: false,
      }),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
    await call(f.db, f.hr.params, 'review_leave', {
      requestId: request.id,
      expectedVersion: request.version,
      decision: 'reject',
      reason: '申請を再確認',
...
describe('employee expenses: idempotency, freeze and settlement', () => {
  it('replays initial save exactly and rejects changed duplicate input, fractional yen and future expense dates', async () => {
    ...
    await expect(call(f.db, w.params, 'save_expense', { ...saved.input, amount: '2000' })).rejects.toMatchObject({
      code: 'CONFLICT',
    });
  ...
  it('freezes submitted amounts, permits returned corrections, and settles once under concurrency', async () => {
    ...
    await expect(
      call(f.db, w.params, 'save_expense', {
        ...saved.input,
        expenseId: saved.row.id,
        expectedVersion: submitted.version,
        amount: '1200',
    ...
    const returned = await call(f.db, f.manager.params, 'review_expense', {
      expenseId: saved.row.id,
      expectedVersion: submitted.version,
      decision: 'return',
      reason: '経路と金額を再確認',
    });
    const corrected = await call(f.db, w.params, 'save_expense', {
      ...saved.input,
      expenseId: saved.row.id,
    ...
    await expect(
      call(f.db, w.params, 'cancel_expense', {
    ...
    const result = await Promise.allSettled([
      call(f.db, f.payroll.params, 'settle_expense', input),
      call(f.db, f.payroll.params, 'settle_expense', input),
    ]);
    ...
    expect(final).toMatchObject({
      status: 'settled',
      paidOn: '2026-09-12',
      paymentReference: '経理精算-001',
      settledBy: f.payroll.id,
    });
    expect(final.amount.toString()).toBe('1200');
    await expect(
      call(f.db, f.payroll.params, 'settle_expense', { ...input, expectedVersion: final.version }),
    ).rejects.toMatchObject({ code: 'INVALID_STATE' });
  });
  it('requires headquarters and a real settlement date; cancelled submissions cannot be approved', async () => {
    ...
    await expect(call(f.db, f.manager.params, 'settle_expense', input)).rejects.toMatchObject({
      code: 'PERMISSION_DENIED',
    });
    for (const paidOn of ['2026-09-09', '2026-09-13'])
      await expect(call(f.db, f.payroll.params, 'settle_expense', { ...input, paidOn })).rejects.toMatchObject({
        code: 'VALIDATION',
      });
    ...
    await expect(
      call(f.db, f.manager.params, 'review_expense', {
