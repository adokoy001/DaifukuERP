...
export function problem(employees = [employee()], slots = [slot()]): ShiftProblem {
  return {
    ...
    employees,
    slots,
    availability: employees.flatMap((e) =>
    ...
    leave: [],
    existing: [],
    rules: Array.from({ length: 7 }, (_, day) => ({
      date: dateAt(day),
...
export const assignment = (slotId = 'morning', employeeId = 'one', locked = false): ShiftAssignment => ({
  slotId,
  employeeId,
  locked,
});
export function benchmarkProblem(): ShiftProblem {
  ...
  const slots = Array.from({ length: 42 }, (_, index) => ({
    ...slot(`slot-${index}`, Math.floor(index / 6)),
    endMinute: 1080,
    breakMinutes: 60,
    required: 20,
  }));
