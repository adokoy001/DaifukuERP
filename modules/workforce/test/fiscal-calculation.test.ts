...
describe('2026 official regular payroll calculation', () => {
  ...
  it('keeps monthly old deduction distinct from the December annual amendment', () => {
    ...
    expect(annual.deductions.basic.toString()).toBe('1040000');
    expect(annual.annualTax.toString()).toBe('40800');
    expect(annual.refund.toString()).toBe('49200');
  });
  it.each([
    ['66.5', '66'],
    ['66.500001', '67'],
  ...
  it('uses insurance months and the wage cutoff independently of the eventual payment month', () => {
    ...
    expect(march.employment.toString()).toBe('1650');
    expect(april.childSupport.toString()).toBe('345');
    expect(april.pension.toString()).toBe('27450');
    expect(april.employment.toString()).toBe('1500');
    expect(april.total.toString()).toBe('46500');
  });
  ...
  it('requires explicit exemption and keeps the agriculture employment worker rate', () => {
    const amounts = socialInsurance(
      condition({
        ...
        pensionMembership: 'exempt',
        pensionStandardMonthly: null,
        employmentCategory: 'agriculture_sake',
      }),
      '2026-08',
      '2026-08-31',
      D(300000),
    );
    expect(amounts.health.toString()).toBe('0');
    expect(amounts.nursing.toString()).toBe('0');
    expect(amounts.pension.toString()).toBe('0');
    expect(amounts.employment.toString()).toBe('1800');
  });
  ...
  it('calculates student, insurance, child and housing deductions from verified declaration facts', () => {
    ...
    expect(result.annualTax.toString()).toBe('0');
    expect(result.refund.toString()).toBe('90000');
  });
});
