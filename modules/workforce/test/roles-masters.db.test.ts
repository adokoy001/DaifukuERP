...
describe('workforce role composition and mutable masters', () => {
  it('permits manager, HR and payroll self-service but forbids their self-approval', async () => {
    for (const [index, person] of [f.manager, f.hr, f.payroll].entries()) {
      ...
      const closed = await call(
        ...
        person.params,
        'punch',
        { kind: 'clock_out', expectedVersion: punch.version, idempotencyKey: newId() },
        '2026-08-10T10:00:00+09:00',
      );
      const submitted = await call(f.db, person.params, 'submit_attendance', {
        attendanceId: punch.id,
        expectedVersion: closed.version,
      });
      await expect(
        call(f.db, person.params, 'review_attendance', {
          attendanceId: punch.id,
      ...
      await call(f.db, {}, 'review_attendance', {
        attendanceId: punch.id,
        expectedVersion: submitted.version,
        decision: 'approve',
        dayKind: 'workday',
      ...
      const payroll = await call(f.db, f.payroll.params, 'calculate_payroll', {
        employeeId: employee.id,
        period: '2026-08',
      ...
      const input = {
        ...
        deductions: deductionKinds.map((kind) => ({ kind, amount: '0', basis: '外部確認済みのゼロ', confirmed: true })),
        allowances: [],
      ...
      )) as { payrolls: { employeeId: string }[] };
      expect(portal.payrolls.map((row) => row.employeeId)).toEqual([employee.id]);
      const expense = await call(f.db, person.params, 'save_expense', {
    ...
    expect(
      await f.db.run({ ...f.manager.params, roles: ['workforce_manager', 'workforce_employee'] }, (ctx) =>
        repo(ctx, WorkforcePayroll).count(),
      ),
    ).toBe(1);
  ...
  it('rejects weakened policies and serializes overlapping terms', async () => {
    ...
    if (!policy) throw new Error('Missing policy');
    await expect(
      f.db.run(f.hr.params, (ctx) => repo(ctx, WorkforcePayPolicy).update(policy.id, { overtimePremiumRate: '0' })),
    ...
    const result = await Promise.allSettled([
      f.db.run(f.payroll.params, (ctx) => repo(ctx, WorkforcePayTerms).create(data)),
      f.db.run(f.payroll.params, (ctx) => repo(ctx, WorkforcePayTerms).create(data)),
    ]);
  ...
  it('does not permit a caller to manufacture payroll evidence through generic records', async () => {
    await expect(
      f.db.run({}, (ctx) =>
        repo(ctx, WorkforceAttendance).create({
          employeeId: f.employee.id,
          userId: f.alice.id,
          siteId: f.siteId,
          workDate: '2026-08-20',
          clockIn: new Date('2026-08-20T09:00:00+09:00'),
        }),
      ),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      call(f.db, f.payroll.params, 'calculate_payroll', {
        employeeId: f.employee.id,
        period: '2026-08',
        attendanceCompleteConfirmed: false,
      }),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(f.db.run(f.manager.params, (ctx) => repo(ctx, WorkforcePayTerms).list())).rejects.toBeInstanceOf(
      PermissionDenied,
    );
  });
});
