...
const day = (date: string, hours: number): PayDay => ({
  date,
  workedMs: hours * 3600000,
  nightMs: 0,
  dayKind: 'workday',
  policy,
});
...
describe('verified ordinary, monthly variable and flex settlement', () => {
  it('rejects non-calendar periods, missing daily plans, excessive planned totals and unconfirmed flex freedom/filing', () => {
    const input = workSystem(employeeId);
    expect(() => validateWorkSystem(input)).not.toThrow();
    expect(() => validateWorkSystem({ ...input, startsOn: '2026-10-02' })).toThrow();
    expect(() => validateWorkSystem({ ...input, days: input.days.slice(1) })).toThrow();
    ...
    expect(() => validateWorkSystem({ ...flex, employeeChoiceConfirmed: false })).toThrow();
    expect(() => validateWorkSystem({ ...flex, filingConfirmed: false })).toThrow();
  });
  it('honors prior ten-hour days without discarding ordinary daily protection', () => {
    const variable = workSystem(employeeId);
    variable.days = variable.days.map((row) =>
      row.date === '2026-10-01'
    ...
    const input = {
      ...
      employmentEnd: '2026-10-31',
      terms,
      days: [day('2026-10-01', 10), day('2026-10-31', 0)],
      leaves: [],
    };
    expect(calculatePay(input).overtimeMs).toBe(2 * 3600000);
    expect(calculatePay({ ...input, workSystems: [variable] }).overtimeMs).toBe(0);
    expect(
      calculatePay({ ...input, days: [day('2026-10-01', 11), day('2026-10-31', 0)], workSystems: [variable] })
        .overtimeMs,
    ).toBe(3600000);
  });
  it('adds only the remaining monthly-variable period excess and ordinary wage difference', () => {
    ...
    expect(result.workedMs).toBe(184 * 3600000);
    expect(result.overtimeMs).toBe(184 * 3600000 - legalPeriodMs(2400, 31));
    expect(result.basePay.toString()).toBe('315000');
    expect(result.details.find((row) => row['date'] === '2026-10-31')?.['regularSupplementMs']).toBe(
      legalPeriodMs(2400, 31) - 176 * 3600000,
    );
  });
  it('settles three-month flex with monthly 50-hour excess removed from final 40-hour excess', () => {
    ...
    let remaining = 480 * 60;
    system.days = system.days.map((row) => {
      const scheduledMinutes = Math.min(row.scheduledMinutes, remaining);
      remaining -= scheduledMinutes;
      return { ...row, scheduledMinutes };
    });
  ...
  it('keeps holiday overtime separate and paid leave out of statutory actual work', () => {
    ...
    const result = calculatePay({
      ...
      employmentEnd: '2026-02-28',
      terms,
      days,
      leaves: [{ date: dates[0] ?? '', days: '1' }],
      workSystems: [system],
    });
    expect(result.overtimeMs).toBe(0);
    expect(result.paidLeaveDays.toString()).toBe('1');
    expect(result.basePay.toString()).toBe('315000');
    expect(result.premiumPay.toString()).toBe('5250');
  });
});
