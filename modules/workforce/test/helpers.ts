...
export async function fixture() {
  ...
  const sites = await db.run({}, async (ctx) => {
    ...
    return [
      await repo(ctx, WorkforceSite).create({ code: 'A', name: '東拠点' }),
      await repo(ctx, WorkforceSite).create({ code: 'B', name: '西拠点' }),
    ];
  });
  ...
  async function person(name: string, role: string, siteIds: string[] = [siteId]) {
    ...
    await db.owner.drizzle.insert(companyMemberships).values({
      ...
      userId: id,
      roles: [role],
      accessScope: scoped ? 'sites' : 'all',
      siteIds: scoped ? siteIds : [],
    });
  ...
  const otherEmployee = await call(db, hr.params, 'register_employee', {
    userId: bob.id,
    siteId,
    code: 'B',
    name: 'Bob',
