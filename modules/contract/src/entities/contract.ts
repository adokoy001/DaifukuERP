...
export const Contract = defineDocument({
  ...
  fields: {
    ...
    endDate: f.date({
      ...
      description: label(
        '空欄は期間の定めなし。submit 後は contract.end で設定',
        'Empty = open-ended. After submit, set it with contract.end',
      ),
    }),
    billingDay: f.int({
      label: label('請求日', 'Billing day'),
      description: label(
        '請求書の日付（1〜31、31 = 月末。月の日数を超える日は月末）',
        'Invoice day of month (1–31; 31 = end of month, clamped to the month length)',
      ),
      required: true,
      default: 1,
      min: 1,
    ...
    billingTiming: f.enum(BILLING_TIMINGS, {
      ...
      default: 'advance',
      labels: {
        advance: label('前払い（当月分を当月請求）', 'In advance (month billed in the month)'),
        arrears: label('後払い（当月分を翌月請求）', 'In arrears (month billed in the next month)'),
      },
    }),
    ...
    prorationRule: f.enum(PRORATION_RULES, {
      ...
      description: label(
        '省略時は会社設定 contract.default_proration',
        'Defaults to the company setting contract.default_proration',
      ),
      required: true,
      default: 'daily',
      labels: {
        daily: label('日割り（当月の実日数）', 'Daily (actual days of the month)'),
        none: label('日割りなし（開始月・終了月も満額）', 'None (full price in the first and last month)'),
      },
    }),
    ...
    status: f.enum(CONTRACT_STATUSES, {
      serverOwned: true,
      ...
      labels: {
        ...
        active: label('契約中', 'Active'),
        ended: label('終了', 'Ended'),
    ...
    nextPeriod: f.text({
      ...
      description: label(
        '請求書をまだ作っていない最も早い対象月（YYYY-MM、自動計算）',
        'Earliest billing period without an invoice (YYYY-MM, computed)',
      ),
