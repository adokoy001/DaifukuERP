...
export const ContractLine = defineEntity({
  ...
  fields: {
    ...
    unitPrice: f.money({ label: label('月額単価', 'Monthly unit price'), description: label('税抜。省略時は品目の販売単価', "Tax-exclusive; defaults to the product's sale price"), required: true }),
    ...
    amount: f.money({ label: label('月額', 'Monthly amount'), description: label('数量 × 単価（丸めなし）', 'quantity × unit price, unrounded'), required: true, default: '0' }),
  },
