...
type Generated = {
  created: { contractId: string; invoiceId: string; number: string | null; total: string }[];
  skipped: { contractId: string; reason: string; invoiceId?: string; number?: string | null }[];
};
type Table = {
...
const rentLine = (unitPrice = '100000', over: Row = {}): Row => ({
  description: '賃料',
  unitPrice,
...
): Promise<ContractWithLines> {
  return run(
    params,
    async (ctx) =>
      (await runAction(ctx, 'contract.create', {
        partnerId: partnerA,
        title: '契約',
        startDate: '2026-05-22',
        ...head,
        lines: { contract_line: lines },
      })) as ContractWithLines,
  );
}
...
const lineView = (inv: InvoiceJson) =>
  inv.lines.sales_invoice_line.map((l) => [l.seq, l.description, l.quantity, l.unitPrice, l.taxCategory, l.amount]);
...
beforeAll(async () => {
  ...
  await run({}, async (ctx) => {
    ...
    await runAction(ctx, 'account.create', {
      code: '1300',
      name: '売掛金',
      type: 'asset',
      subtype: '売掛金',
  ...
  rentProduct = (
    await run({}, (ctx) =>
      repo(ctx, Product).create({ code: 'RENT', name: '事務所賃料', salePrice: '100000', taxCategory: 'standard' }),
    )
  ).id;
});
afterAll(async () => {
...
describe('contract module (docs/specs/contract.md)', () => {
  it('AC-1 / AC-2 draft defaults, lines from the product, validation, submit -> active + nextPeriod, frozen after submit', async () => {
    await expect(createContract({ status: 'active', nextPeriod: '2020-01' }, [rentLine()])).rejects.toBeInstanceOf(
      PermissionDenied,
    );
    const c = await createContract({ title: '渋谷オフィス' }, [
      { productId: rentProduct },
      { description: '住宅部分', unitPrice: '50000', taxCategory: 'non_taxable', quantity: '1' },
    ]);
    ...
    expect(range).toMatchObject({
      code: 'VALIDATION',
      hint: DATE_RANGE_HINT,
      details: { issues: [{ path: 'endDate' }] },
    });
    const patchRange = await caught(
      run({}, (ctx) => runAction(ctx, 'contract.update', { id: c.id, patch: { endDate: '2026-01-01' } })),
    );
    expect(issues(patchRange)[0]?.path).toBe('endDate');
    await expect(createContract({ billingDay: 32 }, [rentLine()])).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'billingDay' }] },
    });
    await expect(createContract({ intervalMonths: 0 }, [rentLine()])).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'intervalMonths' }] },
    });
    await expect(createContract({ title: undefined }, [rentLine()])).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'title' }] },
    });
    await expect(createContract({ billingTiming: 'monthly' }, [rentLine()])).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'billingTiming' }] },
    });
    await expect(createContract({}, [{ description: 'x', unitPrice: '1' }])).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'taxCategory' }] },
    });
    ...
    expect(await caught(run({}, (ctx) => repo(ctx, ContractLine).update(lineId, { quantity: '2' })))).toMatchObject({
      code: 'INVALID_STATE',
    ...
    await expect(
      run({}, (ctx) =>
        repo(ctx, ContractLine).create({ contractId: c.id, description: 'x', unitPrice: '1', taxCategory: 'standard' }),
      ),
    ).rejects.toBeInstanceOf(StateError);
    await expect(
      run({}, (ctx) => runAction(ctx, 'contract.update', { id: c.id, patch: { lines: { contract_line: [] } } })),
    ).rejects.toBeInstanceOf(StateError);
    await expect(
      run({}, (ctx) => runAction(ctx, 'contract.update', { id: c.id, patch: { title: '変更' } })),
    ).rejects.toMatchObject({ code: 'INVALID_STATE', details: { blocked: ['title'] } });
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'contract.update', { id: c.id, patch: { status: 'cancelled', nextPeriod: '2030-01' } }),
      ),
    ...
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'contract.update', { id: empty.id, patch: { status: 'active', nextPeriod: '2026-01' } }),
      ),
    ).rejects.toBeInstanceOf(PermissionDenied);
  ...
  it('AC-3 / AC-4 start month prorated, full month, idempotent second call, nextPeriod; totals and tax through sales', async () => {
    const a = await activeContract({ title: '渋谷オフィス', billingDay: 27 }, [
      { productId: rentProduct },
      { description: '住宅部分', unitPrice: '50000', taxCategory: 'non_taxable' },
    ]);
    ...
    expect(mayInv).toMatchObject({
      docstatus: 0,
      partnerId: partnerA,
      date: '2026-05-27',
      dueDate: '2026-06-30',
      priceIncludesTax: false,
      note: `契約 ${a.number} 2026-05 分`,
      subtotal: '48387',
      taxTotal: '3225',
      total: '51612',
    });
    ...
    expect(june.created).toEqual([
      {
        contractId: a.id,
        invoiceId: expect.any(String),
        number: expect.stringMatching(/^INV-2026-\d{6}$/),
        total: '160000',
      },
    ]);
    const juneInv = await createdInvoice(june, a.id);
    expect(juneInv).toMatchObject({
      docstatus: 1,
      status: 'open',
      date: '2026-06-27',
      subtotal: '150000',
    ...
    expect(again).toEqual({
      created: [],
      skipped: [{ contractId: a.id, reason: 'already_generated', invoiceId: juneInv.id, number: juneInv.number }],
    });
    const mayAgain = await generate({ period: '2026-05', contractId: a.id });
    expect(mayAgain.skipped).toEqual([
      { contractId: a.id, reason: 'already_generated', invoiceId: mayInv.id, number: null },
    ]);
    expect(await run({}, (ctx) => repo(ctx, SalesInvoice).count({ partnerId: partnerA }))).toBe(invoicesBefore);
    const ledger = await run({}, (ctx) =>
      repo(ctx, ContractBilling).list({ where: { contractId: a.id }, orderBy: [{ field: 'period', dir: 'asc' }] }),
    );
    ...
    expect(await generate({ period: '2026-04', contractId: a.id })).toEqual({
      created: [],
      skipped: [{ contractId: a.id, reason: 'not_started' }],
    });
    // one event per generated invoice
    const events = await db.owner.sql<
      { payload: Row }[]
    >`select payload from outbox where topic = ${INVOICE_GENERATED_EVENT} and tenant_id = ${db.tenantId}`;
    expect(events.map((e) => e.payload)).toContainEqual({
      contractId: a.id,
      invoiceId: juneInv.id,
      number: juneInv.number,
      total: '160000',
    ...
    const direct = await caught(
      run({}, (ctx) =>
        repo(ctx, ContractBilling).create({ contractId: a.id, period: '2026-07', invoiceId: mayInv.id }),
      ),
    );
    expect(direct).toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(
      run(asRole(['sales']), (ctx) =>
        runAction(ctx, 'contract_billing.create', { contractId: a.id, period: '2026-08', invoiceId: mayInv.id }),
      ),
    ).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(run({}, (ctx) => repo(ctx, ContractBilling).delete(ledger.items[0]?.id ?? ''))).rejects.toMatchObject({
      code: 'INVALID_STATE',
    });
    await expect(
      run(asRole(['sales']), (ctx) => repo(ctx, ContractBilling).delete(ledger.items[0]?.id ?? '')),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(generate({ period: '2026-6', contractId: a.id })).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'period' }] },
    });
    ...
    const settled = await Promise.allSettled([
      generate({ period: '2026-06', contractId: race.id }),
      generate({ period: '2026-06', contractId: race.id }),
    ]);
    ...
    for (const s of settled) {
      ...
      else if (s.value.created.length === 0)
        expect(s.value.skipped).toEqual([
          { contractId: race.id, reason: 'already_generated', invoiceId: created[0]?.invoiceId, number: null },
        ]);
    }
  ...
  it('AC-2 / AC-3 end month prorated to endDate, contract.end status rule, a skipped month stays next', async () => {
    ...
    expect(await generate({ period: '2026-09', contractId: e.id })).toEqual({
      created: [],
      skipped: [{ contractId: e.id, reason: 'ended' }],
    });
    ...
    expect((await generate({ period: '2026-10', contractId: e.id })).skipped).toEqual([
      { contractId: e.id, reason: 'ended' },
    ]);
    // refusals
    ...
    expect(await caught(endContract(draft.id, '2026-12-31'))).toMatchObject({
      code: 'INVALID_STATE',
      hint: END_STATE_HINT,
    });
    await expect(endContract(newId(), '2026-12-31')).rejects.toMatchObject({ code: 'NOT_FOUND' });
    await expect(
      run({}, (ctx) => runAction(ctx, 'contract.end', { id: e.id, endDate: '2026/09/30' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'endDate' }] } });
    expect(await generate({ period: '2026-09', contractId: draft.id })).toEqual({
      created: [],
      skipped: [{ contractId: draft.id, reason: 'not_active' }],
    });
  });

  it('AC-3 arrears timing bills the next month (billingDay 31 = 月末), submit defaults to contract.auto_submit', async () => {
    const b = await activeContract(
      {
        partnerId: partnerB,
        title: '倉庫（後払い）',
    ...
    expect(await createdInvoice(jan, b.id)).toMatchObject({
      docstatus: 0,
      date: '2026-02-28',
      subtotal: '200000',
      taxTotal: '20000',
      total: '220000',
      note: `契約 ${b.number} 2026-01 分`,
    });
    ...
    try {
      ...
      expect(await createdInvoice(mar, b.id)).toMatchObject({
        priceIncludesTax: false,
        date: '2026-04-30',
        subtotal: '200000',
        taxTotal: '20000',
  ...
  it('AC-3 intervalMonths bills k monthly prices per invoice (aligned periods only); prorationRule none bills full months', async () => {
    ...
    const q = await activeContract(
      { title: '保守（四半期）', startDate: '2026-04-16', intervalMonths: 3, billingDay: 25 },
      [rentLine('10000', { description: '保守料' })],
    );
    const apr = await generate({ period: '2026-04', contractId: q.id });
    expect(await createdInvoice(apr, q.id)).toMatchObject({
      date: '2026-04-25',
      subtotal: '25000',
      taxTotal: '2500',
      total: '27500',
    });
    expect(await generate({ period: '2026-05', contractId: q.id })).toEqual({
      created: [],
      skipped: [{ contractId: q.id, reason: 'not_aligned' }],
    });
    ...
    const n = await activeContract({ title: '看板（満額）', prorationRule: 'none', billingDay: 5 }, [
      rentLine('30000'),
    ]);
    ...
    expect((await createdInvoice(await generate({ period: '2026-05', contractId: up.id }), up.id)).subtotal).toBe(
      '32259',
    );
  });

  it('AC-5 schedule: due / generated / not_due per contract with billing date and prorated expected amount (税抜)', async () => {
    const s1 = await activeContract({
      partnerId: partnerC,
      title: 'S1 10月11日入居',
      startDate: '2026-10-11',
      billingDay: 5,
    });
    const s2 = await activeContract({
      partnerId: partnerC,
      title: 'S2 生成済み',
    ...
    expect(byId.get(s1.id)).toMatchObject({
      partnerName: 'C不動産',
      title: 'S1 10月11日入居',
      billingDate: '2026-10-05',
      expectedAmount: '67741',
      factor: '21/31',
      status: 'due',
      reason: null,
      invoiceId: null,
      invoiceNumber: null,
      contractNumber: s1.number,
      partnerId: partnerC,
    });
    expect(byId.get(s2.id)).toMatchObject({
      ...
      invoiceId: s2Invoice?.invoiceId,
      invoiceNumber: s2Invoice?.number,
    });
    ...
    expect(table.meta).toMatchObject({
      period: '2026-10',
      truncated: false,
      counts: {
        due: table.rows.filter((r) => r.status === 'due').length,
        generated: table.rows.filter((r) => r.status === 'generated').length,
      },
    });
    ...
    expect(run10.skipped).toEqual(
      expect.arrayContaining([
        { contractId: s2.id, reason: 'already_generated', invoiceId: s2Invoice?.invoiceId, number: s2Invoice?.number },
        { contractId: s3.id, reason: 'not_started' },
        { contractId: s4.id, reason: 'not_aligned' },
      ]),
    );
  ...
  it('AC-2 cancel is refused while generated invoices are live; deleting a draft invoice makes its period due again; amend starts clean', async () => {
    ...
    expect(refused).toMatchObject({
      code: 'INVALID_STATE',
      hint: LIVE_INVOICES_HINT,
      details: { invoices: [{ period: '2026-07', invoiceId: draftId, number: null }] },
    });
    ...
    await expect(run({}, (ctx) => runAction(ctx, 'contract.cancel', { id: x.id }))).rejects.toMatchObject({
      code: 'INVALID_STATE',
      hint: LIVE_INVOICES_HINT,
    });
    ...
    expect(await generate({ period: '2026-08', contractId: x.id })).toEqual({
      created: [],
      skipped: [{ contractId: x.id, reason: 'not_active' }],
    });
    expect((await generate({ period: '2026-07', contractId: x.id })).skipped).toEqual([
      { contractId: x.id, reason: 'not_active', invoiceId, number: second.created[0]?.number },
    ]);
  ...
  it('AC-6 permissions, settings and menus', async () => {
    expect(allowedOps({ roles: ['sales'] }, Contract).sort()).toEqual([
      'amend',
      'cancel',
      'create',
      'delete',
    ...
    const c = await activeContract(
      { title: '営業担当の契約', startDate: '2026-08-01', billingDay: 10 },
      [rentLine('80000')],
      sales,
    );
    ...
    expect((await schedule('2026-08', sales)).rows.some((r) => r.contractId === c.id && r.status === 'generated')).toBe(
      true,
    );
    // accounting / viewer read only
    for (const role of ['accounting', 'viewer']) {
      await expect(generate({ period: '2026-09', contractId: c.id }, asRole([role]))).rejects.toBeInstanceOf(
        PermissionDenied,
      );
      ...
      expect(
        (await run(asRole([role]), (ctx) => runAction(ctx, 'contract.get', { id: c.id }))) as ContractJson,
    ...
    for (const name of ['contract.generate_invoices', 'contract.end', 'contract.schedule'])
      expect(registry.actions().map((a) => a.name)).toContain(name);
    expect(registry.action('contract.schedule')).toMatchObject({ tx: 'none', mutates: false });
    expect(registry.action('contract.generate_invoices')).toMatchObject({
      tx: 'required',
      mutates: true,
      permission: { entity: 'sales_invoice', op: 'create' },
    });
    ...
    await expect(
      run({}, (ctx) => setSetting(ctx, CONTRACT_DEFAULT_PRORATION_KEY, contractDefaultProrationSchema, 'weekly')),
