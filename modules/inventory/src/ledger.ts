...
import { can, repo, StateError, type Context, type Decimal, type Infer, type ListQuery, type LocalDate } from '@daifuku/kernel';
...
import { emptyState, inbound, outbound, type InboundCost, type Movement, type OutboundCost, type StockState } from './services/moving-average.ts';
...
export type MovementSpec = { kind: 'in'; qty: Decimal; cost: InboundCost } | { kind: 'out'; qty: Decimal; cost: OutboundCost };

export const NEGATIVE_STOCK_HINT = 'Receive the goods first (or reduce the quantity), or set inventory.allow_negative_stock to true.';

export async function loadBalance(ctx: Context, key: StockKey): Promise<BalanceRow | null> {
  const res = await repo(ctx, StockBalance).list({ where: { productId: key.productId, warehouseId: key.warehouseId }, limit: 1 });
...
export async function listAll<T>(fetch: (q: ListQuery) => Promise<{ items: T[]; total: number }>, query: ListQuery, max: number): Promise<{ items: T[]; truncated: boolean }> {
  const items: T[] = [];
  for (let offset = 0; ; ) {
...
export async function keyLabel(ctx: Context, key: StockKey): Promise<{ product: string; warehouse: string }> {
  ...
  return { product: product ? (product.code ?? product.name) : key.productId, warehouse: warehouse ? warehouse.code : key.warehouseId };
}

async function negativeStockError(ctx: Context, key: StockKey, before: StockState, qty: Decimal, meta: PostingMeta): Promise<StateError> {
  ...
  return new StateError(
    ...
    { ...key, available: before.qty.toString(), requested: qty.toString(), sourceEntity: meta.sourceEntity, sourceId: meta.sourceId, reversal: meta.reversal },
  );
}

/** Appends one ledger row and moves the balance (AC-3). Outbound movements below zero are refused unless allowed. */
export async function postMovement(ctx: Context, key: StockKey, spec: MovementSpec, meta: PostingMeta): Promise<{ row: LedgerRow; movement: Movement }> {
  await assertInventoryDate(ctx, meta.date);
  const latest = (await repo(ctx, StockLedger).list({ where: { ...key }, orderBy: [{ field: 'seq', dir: 'desc' }], limit: 1 })).items[0];
  if (latest && meta.date < latest.date) throw new StateError(`Inventory already contains a movement dated ${latest.date}`, 'Backdated moving-average reposting is not supported; use a correction date on or after the latest movement.');
  ...
  if (spec.kind === 'out' && !meta.allowNegative && m.after.qty.lt(0)) throw await negativeStockError(ctx, key, before, spec.qty, meta);
...
export async function sourceRows(ctx: Context, sourceEntity: string, sourceId: string): Promise<LedgerRow[]> {
  const { items, truncated } = await listAll((q) => repo(ctx, StockLedger).list(q), { where: { sourceEntity, sourceId, reversal: false }, orderBy: [{ field: 'seq', dir: 'desc' }] }, 5000);
  if (truncated) throw new StateError('Source inventory movements exceed the supported reversal limit', 'No partial reversal is permitted.');
  return items;
}
 ...
 */
export async function reverseSource(ctx: Context, sourceEntity: string, sourceId: string, allowNegative: boolean, correctionDate?: LocalDate): Promise<LedgerRow[]> {
  const out: LedgerRow[] = [];
  for (const row of await sourceRows(ctx, sourceEntity, sourceId)) {
    ...
    const posted = await postMovement(ctx, { productId: row.productId, warehouseId: row.warehouseId }, spec, { date: correctionDate ?? row.date, sourceEntity, sourceId, reversal: true, allowNegative });
    out.push(posted.row);
  }
  return out;
}
