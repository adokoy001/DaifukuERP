...
export const LEDGER_WRITE_HINT =
  'Stock moves only through documents: submit a stock_entry (or a purchase/sales invoice, a stock_count); cancel it to append the reverse rows.';

function refuse(entity: string, op: string, id: unknown): StateError {
  return new StateError(
    `${entity} rows cannot be ${op} directly (${entity === StockLedger.name ? 'append-only ledger' : 'derived from the ledger'})`,
    LEDGER_WRITE_HINT,
    { entity, op, id },
  );
}
...
export function registerGuardHooks(): void {
  ...
  registry.registerHook(StockLedger.name, 'before_validate', (ctx, args) =>
    onlyModule(StockLedger.name, args.previous ? 'updated' : 'created')(ctx, args),
  );
  ...
  registry.registerHook(StockBalance.name, 'before_validate', (ctx, args) =>
    onlyModule(StockBalance.name, args.previous ? 'updated' : 'created')(ctx, args),
  );
