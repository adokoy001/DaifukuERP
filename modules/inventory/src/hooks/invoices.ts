...
import { cancelDocument, DOCSTATUS, registry, repo, submitDocument, type Context, type HookArgs, type Infer, type LocalDate } from '@daifuku/kernel';
 ...
 */
export async function createAndSubmitEntry(ctx: Context, head: LinkedEntryHead, lines: readonly (EntryLineDraft & { sign?: 'in' | 'out' })[]): Promise<StockEntryRow> {
  return asModule(ctx, async (ctx) => {
    const created = await repo(ctx, StockEntry).create({ ...head });
    for (const [i, line] of lines.entries()) await repo(ctx, StockEntryLine).create({ ...line, entryId: created.id, seq: i + 1 });
...
export async function cancelLinkedEntries(ctx: Context, sourceEntity: string, sourceId: string, correctionDate?: LocalDate): Promise<string[]> {
  const linked = await repo(ctx, StockEntry).list({ where: { sourceEntity, sourceId, docstatus: DOCSTATUS.submitted }, limit: 500 });
  for (const e of linked.items) await asModule(ctx, (ctx) => cancelDocument(ctx, StockEntry, e.id, correctionDate ? { correctionDate } : {}));
...
async function onPurchaseSubmitted(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  const lines = receiptLinesFromPurchase(items, await kindsFor(ctx, items), { priceIncludesTax: row.priceIncludesTax === true, rates: ratesFromTaxSummary(row.taxSummary) });
  if (lines.length === 0) return;
  const warehouse = await resolveDefaultWarehouse(ctx);
  const head = { type: 'receipt' as const, date: row.date as LocalDate, warehouseId: warehouse.id, partnerId: row.partnerId as string, note: `仕入請求書 ${String(row.number)}` };
...
async function onSalesSubmitted(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  if (lines.length === 0) return;
  const warehouse = await resolveDefaultWarehouse(ctx);
  const head = { type: 'issue' as const, date: row.date as LocalDate, warehouseId: warehouse.id, partnerId: row.partnerId as string, note: `売上請求書 ${String(row.number)}` };
...
export function registerInvoiceHooks(): void {
  ...
  registry.registerHook(PurchaseInvoice.name, 'after_cancel', (ctx, { row, correctionDate }) => cancelLinkedEntries(ctx, PurchaseInvoice.name, row.id as string, correctionDate).then(() => undefined));
  registry.registerHook(SalesInvoice.name, 'after_cancel', (ctx, { row, correctionDate }) => cancelLinkedEntries(ctx, SalesInvoice.name, row.id as string, correctionDate).then(() => undefined));
}
