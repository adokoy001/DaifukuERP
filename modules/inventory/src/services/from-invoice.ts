...
export function stockLines<L extends InvoiceLineLike>(lines: readonly L[], kinds: ReadonlyMap<string, ProductKind>): (L & { productId: string })[] {
  return lines.filter((l): l is L & { productId: string } => l.productId !== null && kinds.get(l.productId) === 'goods' && l.quantity.gt(0) && l.unitPrice.gte(0));
}
...
export function ratesFromTaxSummary(summary: unknown): Map<string, Decimal> {
  ...
  for (const g of groups) {
    ...
    const rate = rec?.rate;
    if (typeof category === 'string' && typeof rate === 'string' && Decimal.isDecimalString(rate)) out.set(category, Decimal.from(rate));
  }
  return out;
}

/** Receipt lines for a submitted purchase invoice (AC-5): goods lines at the 税抜 unit price. */
export function receiptLinesFromPurchase(lines: readonly InvoiceLineLike[], kinds: ReadonlyMap<string, ProductKind>, tax: { priceIncludesTax: boolean; rates: ReadonlyMap<string, Decimal> }): EntryLineDraft[] {
  return stockLines(lines, kinds).map((l) => {
    const rate = tax.rates.get(l.taxCategory);
    if (rate === undefined) {
      throw new StateError(`purchase invoice line ${l.seq}: tax category ${l.taxCategory} is not in the invoice tax summary`, 'Recalculate the purchase invoice (save it again) so its tax summary covers every line, then submit.', { seq: l.seq, taxCategory: l.taxCategory });
    }
    return { productId: l.productId, quantity: l.quantity.toString(), unitCost: netUnitPrice(l.unitPrice, rate, tax.priceIncludesTax).toString() };
  });
}

/** Issue lines for a submitted sales invoice (AC-5): goods lines; the cost comes from the moving average at submit. */
export function issueLinesFromSales(lines: readonly InvoiceLineLike[], kinds: ReadonlyMap<string, ProductKind>): EntryLineDraft[] {
...
export function adjustmentLinesFromCount(lines: readonly CountLineLike[], avgCostOf: (productId: string) => Decimal): (EntryLineDraft & { sign: 'in' | 'out' })[] {
  return lines
    .filter((l) => !l.varianceQty.isZero())
    .map((l) =>
      l.varianceQty.gt(0)
        ? { productId: l.productId, quantity: l.varianceQty.toString(), sign: 'in' as const, unitCost: avgCostOf(l.productId).toString() }
