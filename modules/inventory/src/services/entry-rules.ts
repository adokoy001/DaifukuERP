...
export const ROLE_HINT =
  'inventory operates every stock entry; purchasing only receipts; sales only through sales invoices (auto issue).';
...
export function headIssues(head: EntryHead): Issue[] {
  ...
  if (head.type === 'transfer') {
    ...
    else if (head.toWarehouseId === head.warehouseId)
      issues.push({ path: 'toWarehouseId', message: 'must differ from warehouseId' });
...
export function lineIssues(
  ...
  const issues: Issue[] = [];
  if (kind === undefined)
    issues.push({ path: at('productId'), message: `product ${line.productId} does not exist or is not visible` });
  else if (kind !== 'goods')
    issues.push({ path: at('productId'), message: 'a service product has no stock; use a goods product' });
  ...
  if (type === 'adjustment' && line.sign === null)
    issues.push({ path: at('sign'), message: 'required for an adjustment (in or out)' });
  if (type !== 'adjustment' && line.sign !== null)
    issues.push({ path: at('sign'), message: 'only adjustment lines have a sign' });
