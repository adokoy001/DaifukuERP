...
import { getSetting, label, registry, repo, StateError, type Context, type Infer, type SettingDef } from '@daifuku/kernel';
import { z } from 'zod';
...
export const INVENTORY_SETTING_DEFS: readonly SettingDef[] = [
  {
    ...
    description: label('false（既定）なら在庫が足りない出庫・取消を拒否', 'false (default): issues and cancellations that would go below zero are refused'),
  ...
  },
  {
    ...
    description: label('仕入請求書の submit で物品明細を既定の倉庫に入庫（既定 true）', 'Submitting a purchase invoice receives its goods lines into the default warehouse (default true)'),
    schema: boolSettingSchema,
    default: true,
  },
  {
    ...
    description: label('売上請求書の submit で物品明細を既定の倉庫から出庫（既定 true）', 'Submitting a sales invoice issues its goods lines from the default warehouse (default true)'),
    schema: boolSettingSchema,
    default: true,
  },
  {
    ...
    description: label('自動入出庫・倉庫省略時に使う倉庫のコード（既定 MAIN）', 'Warehouse code used by auto receipts/issues and when a document omits the warehouse (default MAIN)'),
...
export function allowNegativeStock(ctx: Context): Promise<boolean> {
  return getSetting(ctx, ALLOW_NEGATIVE_STOCK_KEY, boolSettingSchema, INVENTORY_SETTING_DEFAULTS[ALLOW_NEGATIVE_STOCK_KEY]);
}
export function autoReceiptOnPurchase(ctx: Context): Promise<boolean> {
  return getSetting(ctx, AUTO_RECEIPT_ON_PURCHASE_KEY, boolSettingSchema, INVENTORY_SETTING_DEFAULTS[AUTO_RECEIPT_ON_PURCHASE_KEY]);
}
export function autoIssueOnSales(ctx: Context): Promise<boolean> {
  return getSetting(ctx, AUTO_ISSUE_ON_SALES_KEY, boolSettingSchema, INVENTORY_SETTING_DEFAULTS[AUTO_ISSUE_ON_SALES_KEY]);
}
...
export async function resolveDefaultWarehouse(ctx: Context): Promise<Infer<typeof Warehouse>> {
  const code = await getSetting(ctx, DEFAULT_WAREHOUSE_KEY, warehouseCodeSchema, INVENTORY_SETTING_DEFAULTS[DEFAULT_WAREHOUSE_KEY]);
  ...
  if (!found) throw new StateError(`no default warehouse: no warehouse has code ${code} and none is marked isDefault`, DEFAULT_WAREHOUSE_HINT, { setting: DEFAULT_WAREHOUSE_KEY, code });
  return found;
}
