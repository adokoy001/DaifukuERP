...
export const StockEntry = defineDocument({
  ...
  fields: {
    type: f.enum(STOCK_ENTRY_TYPES, {
      ...
      index: true,
      labels: {
        receipt: label('入庫', 'Receipt'),
        issue: label('出庫', 'Issue'),
        transfer: label('移動', 'Transfer'),
        adjustment: label('調整', 'Adjustment'),
      },
    }),
    date: f.date({ label: label('日付', 'Date'), required: true, default: 'today', index: true }),
    warehouseId: f.ref('warehouse', {
      label: label('倉庫（移動元）', 'Warehouse (from)'),
      description: label('省略時は既定の倉庫', 'defaults to the default warehouse'),
      required: true,
    }),
    toWarehouseId: f.ref('warehouse', {
      label: label('移動先倉庫', 'To warehouse'),
      description: label('移動のときだけ指定', 'transfers only'),
    }),
    partnerId: f.ref('partner', { label: label('取引先', 'Partner') }),
    note: f.text({ label: label('備考', 'Note'), multiline: true, maxLength: 2000 }),
    sourceEntity: f.text({
      label: label('発生元エンティティ', 'Source entity'),
      description: label('請求書・棚卸から自動作成したときに設定', 'set when created from an invoice or a stock count'),
      maxLength: 100,
    }),
