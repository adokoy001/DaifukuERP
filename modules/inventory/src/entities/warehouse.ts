...
export const Warehouse = defineEntity({
  ...
  fields: {
    ...
    isDefault: f.bool({
      label: label('既定の倉庫', 'Default'),
      description: label(
        '設定 inventory.default_warehouse のコードが無いときに使う',
        'Used when the code in inventory.default_warehouse does not exist',
      ),
      required: true,
      default: false,
    }),
