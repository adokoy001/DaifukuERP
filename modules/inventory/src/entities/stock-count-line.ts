...
export const StockCountLine = defineEntity({
  ...
  fields: {
    ...
    productId: f.ref('product', {
      ...
      description: label('物品のみ', 'goods only'),
      required: true,
    }),
    countedQty: f.quantity({ label: label('実棚数量', 'Counted quantity'), required: true, min: '0' }),
    systemQty: f.quantity({
      label: label('帳簿数量', 'System quantity'),
      description: label('在庫残高から自動', 'from the stock balance'),
      required: true,
      default: '0',
    }),
    varianceQty: f.quantity({
      label: label('差異数量', 'Variance'),
      description: label('実棚 − 帳簿', 'counted − system'),
      required: true,
      default: '0',
    }),
  },
