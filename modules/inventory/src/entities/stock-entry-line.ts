...
export const StockEntryLine = defineEntity({
  ...
  fields: {
    ...
    productId: f.ref('product', { label: label('品目', 'Product'), description: label('物品のみ（サービスは不可）', 'goods only (not services)'), required: true }),
    quantity: f.quantity({ label: label('数量', 'Quantity'), description: label('0 より大きい（小数 6 桁まで）', '> 0, at most 6 decimals'), required: true }),
    sign: f.enum(LINE_SIGNS, { label: label('増減', 'Direction'), description: label('調整のときだけ: in=増, out=減', 'adjustments only'), labels: { in: label('増', 'In'), out: label('減', 'Out') } }),
    unitCost: f.money({ label: label('単価', 'Unit cost'), description: label('入庫・調整増は必須。出庫・移動・調整減は submit 時に移動平均単価', 'required for receipts / adjustment in; outbound lines get the moving average at submit'), min: '0', scale: 6 }),
    amount: f.money({ label: label('金額', 'Amount'), description: label('数量 × 単価（submit 後は計上額）', 'quantity × unit cost (posted cost after submit)'), required: true, default: '0', scale: 6 }),
  },
  indexes: [['entryId', 'seq']],
  permissions: {
    roles: {
