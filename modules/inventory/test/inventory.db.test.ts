...
import { Decimal, DependencyError, DOCSTATUS, PermissionDenied, registry, repo, setSetting, StateError, ValidationError, type Context, type ContextParams, type Infer } from '@daifuku/kernel';
...
import { asRole, caught, issuesOf, setupFixture, type EntryJson, type EntryWithLines, type Fixture, type Row, type Table } from './fixture.ts';
...
const createEntry = async (params: Partial<ContextParams>, head: Row, lines: Row[]): Promise<EntryWithLines> => {
  const created = await fx.act<EntryJson>(params, 'stock_entry.create', { ...head, lines: { stock_entry_line: lines } });
...
const submitEntry = (params: Partial<ContextParams>, id: string) => fx.act<EntryJson>(params, 'stock_entry.submit', { id });
const cancelEntry = (params: Partial<ContextParams>, id: string, correctionDate?: string) => fx.act<EntryJson>(params, 'stock_entry.cancel', { id, ...(correctionDate ? { correctionDate } : {}) });
...
type LedgerView = { seq: number; qtyDelta: string; unitCost: string; costDelta: string; balanceQty: string; balanceCost: string; reversal: boolean; sourceId: string; date: string };
const ledgerOf = (productId: string, warehouseId: string): Promise<LedgerView[]> =>
  fx.run(admin, async (ctx) => {
    const rows = await repo(ctx, StockLedger).list({ where: { productId, warehouseId }, orderBy: [{ field: 'seq', dir: 'asc' }], limit: 500 });
    return rows.items.map((r) => ({ seq: r.seq, qtyDelta: r.qtyDelta.toString(), unitCost: r.unitCost.toString(), costDelta: r.costDelta.toString(), balanceQty: r.balanceQty.toString(), balanceCost: r.balanceCost.toString(), reversal: r.reversal, sourceId: r.sourceId, date: r.date }));
  });
const newProduct = (code: string, extra: Row = {}) => fx.run(admin, async (ctx) => (await repo(ctx, Product).create({ code, name: `品目${code}`, taxCategory: 'standard', ...extra })).id);
...
describe('inventory module (docs/specs/inventory.md)', () => {
  it('AC-1 / AC-8 entities, MAIN seed (idempotent), one default warehouse, settings with defaults', async () => {
    expect(InventoryModule.entities.map((e) => e.name)).toEqual(['inventory_period_close', 'warehouse', 'stock_ledger', 'stock_balance', 'stock_entry', 'stock_entry_line', 'stock_count', 'stock_count_line']);
    expect(StockEntry.kind).toBe('document');
    const list = () => fx.act<{ items: Row[] }>(admin, 'warehouse.list', { orderBy: [{ field: 'code', dir: 'asc' }] });
    expect((await list()).items.map((w) => [w.code, w.name, w.isDefault])).toEqual([['MAIN', '本店（主倉庫）', true], ['SUB', '第二倉庫', false]]);
    await fx.act(admin, 'warehouse.update', { id: fx.wh.sub, patch: { isDefault: true } });
    expect((await list()).items.map((w) => [w.code, w.isDefault])).toEqual([['MAIN', false], ['SUB', true]]);
    await fx.act(clerk, 'warehouse.update', { id: fx.wh.main, patch: { isDefault: true } });
    expect((await list()).items.map((w) => [w.code, w.isDefault])).toEqual([['MAIN', true], ['SUB', false]]);
    expect(await caught(fx.act(admin, 'warehouse.create', { code: 'main', name: 'dup' }))).toMatchObject({ code: 'CONFLICT' });
    for (const key of [ALLOW_NEGATIVE_STOCK_KEY, AUTO_RECEIPT_ON_PURCHASE_KEY, AUTO_ISSUE_ON_SALES_KEY, DEFAULT_WAREHOUSE_KEY]) expect(registry.hasSetting(key)).toBe(true);
    expect(INVENTORY_SETTING_DEFAULTS).toEqual({ [ALLOW_NEGATIVE_STOCK_KEY]: false, [AUTO_RECEIPT_ON_PURCHASE_KEY]: true, [AUTO_ISSUE_ON_SALES_KEY]: true, [DEFAULT_WAREHOUSE_KEY]: 'MAIN' });
  ...
  it('AC-2 stock_entry + lines: defaults, system-owned source, computed amount, line and header rules (service → VALIDATION with hint)', async () => {
    const e = await createEntry(clerk, { type: 'receipt', sourceEntity: 'purchase_invoice', sourceId: fx.wh.main }, [{ productId: fx.product.a, quantity: '10', unitCost: '100.5' }]);
    expect(e).toMatchObject({ docstatus: 0, number: null, type: 'receipt', date: '2026-09-10', warehouseId: fx.wh.main, toWarehouseId: null, sourceEntity: null, sourceId: null });
    expect(e.lines.stock_entry_line.map((l) => [l.seq, l.productId, l.quantity, l.sign, l.unitCost, l.amount])).toEqual([[1, fx.product.a, '10', null, '100.5', '1005']]);
    const service = await caught(createEntry(clerk, { type: 'receipt' }, [{ productId: fx.product.svc, quantity: '1', unitCost: '1' }]));
    ...
    expect(issuesOf(await caught(createEntry(clerk, { type: 'receipt' }, [{ productId: fx.product.a, quantity: '0', unitCost: '1' }])))).toEqual(['quantity']);
    expect(issuesOf(await caught(createEntry(clerk, { type: 'receipt' }, [{ productId: fx.product.a, quantity: '1.0000001', unitCost: '1' }])))).toEqual(['quantity']);
    expect(issuesOf(await caught(createEntry(clerk, { type: 'receipt' }, [{ productId: fx.product.a, quantity: '1' }])))).toEqual(['unitCost']);
    expect(issuesOf(await caught(createEntry(clerk, { type: 'receipt' }, [{ productId: fx.product.a, quantity: '1', unitCost: '1', sign: 'in' }])))).toEqual(['sign']);
    expect(issuesOf(await caught(createEntry(clerk, { type: 'adjustment' }, [{ productId: fx.product.a, quantity: '1' }])))).toEqual(['sign']);
    expect(issuesOf(await caught(createEntry(clerk, { type: 'adjustment' }, [{ productId: fx.product.a, quantity: '1', sign: 'in' }])))).toEqual(['unitCost']);
    // outbound lines drop a sent unit cost: it is computed at submit
    const issue = await createEntry(clerk, { type: 'issue' }, [{ productId: fx.product.a, quantity: '2', unitCost: '999' }]);
    expect(issue.lines.stock_entry_line.map((l) => [l.unitCost, l.amount])).toEqual([[null, '0']]);
    const out = await createEntry(clerk, { type: 'adjustment' }, [{ productId: fx.product.a, quantity: '1', sign: 'out', unitCost: '5' }]);
    expect(out.lines.stock_entry_line.map((l) => [l.sign, l.unitCost, l.amount])).toEqual([['out', null, '0']]);
    // header
    expect(issuesOf(await caught(createEntry(clerk, { type: 'receipt', toWarehouseId: fx.wh.sub }, [])))).toEqual(['toWarehouseId']);
    expect(issuesOf(await caught(createEntry(clerk, { type: 'transfer', warehouseId: fx.wh.sub, toWarehouseId: fx.wh.sub }, [])))).toEqual(['toWarehouseId']);
    ...
    await fx.act(clerk, 'stock_entry.update', { id: e.id, patch: { sourceEntity: 'sales_invoice', sourceId: e.id, type: 'issue' } });
  ...
  it('AC-3 receipt / issue / transfer / adjustment post the moving average to stock_ledger and stock_balance', async () => {
    ...
    const r = await post({ type: 'receipt', date: '2026-09-01' }, [{ productId: p, quantity: '10', unitCost: '100' }, { productId: p, quantity: '10', unitCost: '120' }]);
    ...
    expect(rows.map((l) => [l.seq, l.qtyDelta, l.unitCost, l.costDelta, l.balanceQty, l.balanceCost, l.reversal, l.date])).toEqual([
    ...
    const raw = await fx.run(admin, async (ctx) => (await repo(ctx, StockLedger).list({ where: { productId: p }, limit: 1 })).items[0]);
    expect(raw?.sourceEntity).toBe('stock_entry');
    expect(await balanceOf(p, fx.wh.main)).toEqual(['20', '110', '2200', 2]);
    expect(r.lines.stock_entry_line.map((l) => [l.unitCost, l.amount])).toEqual([['100', '1000'], ['120', '1200']]);
    ...
    const t = await post({ type: 'transfer', date: '2026-09-03', toWarehouseId: fx.wh.sub }, [{ productId: p, quantity: '3' }]);
    ...
    expect((await ledgerOf(p, fx.wh.sub)).map((l) => [l.seq, l.qtyDelta, l.unitCost, l.costDelta, l.sourceId])).toEqual([[1, '3', '110', '330', t.id]]);

    const a = await post({ type: 'adjustment', date: '2026-09-04' }, [{ productId: p, quantity: '2', sign: 'in', unitCost: '95' }, { productId: p, quantity: '1', sign: 'out' }]);
    // in: (1320 + 190) / 14 = 107.857142857… → 107.857143; out: 1 × 107.857143
    expect(a.lines.stock_entry_line.map((l) => [l.sign, l.unitCost, l.amount])).toEqual([['in', '95', '190'], ['out', '107.857143', '107.857143']]);
    ...
    const values = [(await balanceOf(p, fx.wh.main))?.[2], (await balanceOf(p, fx.wh.sub))?.[2]].map((v) => Decimal.from(String(v)));
    expect(Decimal.sum(values).toString()).toBe(Decimal.from('2200').minus('550').plus('190').minus('107.857143').toString());
  });
  ...
  it('AC-3 negative stock is refused with the available quantity (nothing posted, entry stays a draft) unless inventory.allow_negative_stock', async () => {
    ...
    expect((err as StateError).details).toMatchObject({ productId: p, warehouseId: fx.wh.main, available: '0', requested: '4', reversal: false });
    ...
    const two = await createEntry(clerk, { type: 'issue' }, [{ productId: p, quantity: '5' }, { productId: p, quantity: '5' }]);
    ...
    for (const scenario of golden.scenarios) {
      ...
      for (const [k, step] of scenario.steps.entries()) {
        ...
        if (step.op === 'receipt' || step.op === 'issue') {
          const lines = [{ productId: p, quantity: step.qty, ...(step.op === 'receipt' ? { unitCost: step.unitCost } : {}) }];
        ...
        const target = [...posted].reverse().find((x) => !x.cancelled && x.step.op === origOp && x.step.qty === step.qty && Decimal.from(x.step.expect.costDelta).abs().eq(step.totalCost));
        if (!target) throw new TypeError(`golden step ${k} has no entry to cancel`);
        await cancelEntry(clerk, target.id, date);
        target.cancelled = true;
      }
      const rows = await ledgerOf(p, fx.wh.main);
      expect(rows.map(({ qtyDelta, costDelta, unitCost, balanceQty, balanceCost }) => ({ qtyDelta, costDelta, unitCost, balanceQty, balanceCost })), scenario.name).toEqual(scenario.steps.map((s) => s.expect));
      const last = scenario.steps.at(-1)?.expect;
      expect((await balanceOf(p, fx.wh.main))?.slice(0, 3)).toEqual([last?.balanceQty, last?.unitCost, last?.balanceCost]);
    }
    await setBool(ALLOW_NEGATIVE_STOCK_KEY, false);
  });
  ...
  it('AC-4 cancel appends reverse rows (never deletes), restores balances, refuses to go negative; cancelled entries are read-only; amend', async () => {
    ...
    expect(after[2]).toMatchObject({ seq: 3, qtyDelta: '4', costDelta: '400', unitCost: '100', balanceQty: '10', balanceCost: '1000', reversal: true, sourceId: e2.id, date: '2026-09-06' });
    ...
    expect(await caught(fx.act(clerk, 'stock_entry.update', { id: e1.id, patch: { note: 'x' } }))).toBeInstanceOf(StateError);
    ...
    expect(await caught(fx.act(clerk, 'stock_entry_line.create', { entryId: e1.id, productId: p, quantity: '1', unitCost: '1' }))).toBeInstanceOf(StateError);
    ...
    expect((await getEntry(amended.id)).lines.stock_entry_line.map((l) => [l.quantity, l.unitCost])).toEqual([['10', '100']]);
  ...
  it('AC-6 stock count: systemQty on save, adjustment entry for the variances at submit, count_variance, cancel', async () => {
    ...
    await post({ type: 'receipt', date: '2026-09-08' }, [{ productId: pa, quantity: '10', unitCost: '100' }, { productId: pb, quantity: '5', unitCost: '20' }]);
    const created = await fx.act<Row & { id: string }>(clerk, 'stock_count.create', { lines: { stock_count_line: [{ productId: pa, countedQty: '8' }, { productId: pb, countedQty: '5' }, { productId: pc, countedQty: '3' }] } });
    const draft = await fx.act<Row & { lines: { stock_count_line: Row[] } }>(clerk, 'stock_count.get', { id: created.id });
    expect(draft).toMatchObject({ docstatus: 0, warehouseId: fx.wh.main, date: '2026-09-10', adjustmentEntryId: null });
    expect(draft.lines.stock_count_line.map((l) => [l.systemQty, l.countedQty, l.varianceQty])).toEqual([['10', '8', '-2'], ['5', '5', '0'], ['0', '3', '3']]);
    expect(issuesOf(await caught(fx.act(clerk, 'stock_count_line.create', { countId: created.id, productId: fx.product.svc, countedQty: '1' })))).toEqual(['productId']);
    expect(await caught(fx.act(clerk, 'stock_count_line.create', { countId: created.id, productId: pa, countedQty: '-1' }))).toBeInstanceOf(ValidationError);

    const live = await fx.act<Table>(clerk, 'inventory.count_variance', { countId: created.id });
    expect(live.rows.map((r) => [r.seq, r.productCode, r.systemQty, r.countedQty, r.varianceQty, r.unitCost, r.varianceValue])).toEqual([
      [1, 'CA', '10', '8', '-2', '100', '-200'],
      [2, 'CB', '5', '5', '0', '20', '0'],
      [3, 'CC', '0', '3', '3', '0', '0'],
    ]);
    ...
    const submitted = await fx.act<Row & { number: string; adjustmentEntryId: string }>(clerk, 'stock_count.submit', { id: created.id });
    expect(submitted.number).toMatch(/^CNT-2026-\d{6}$/);
    const count = await fx.act<Row & { adjustmentEntryId: string; lines: { stock_count_line: Row[] } }>(clerk, 'stock_count.get', { id: created.id });
    expect(count.lines.stock_count_line.map((l) => [l.systemQty, l.varianceQty])).toEqual([['9', '-1'], ['5', '0'], ['0', '3']]);
    const adj = await getEntry(count.adjustmentEntryId);
    expect(adj).toMatchObject({ docstatus: 1, type: 'adjustment', date: '2026-09-10', warehouseId: fx.wh.main, sourceEntity: 'stock_count', sourceId: created.id });
    expect(adj.lines.stock_entry_line.map((l) => [l.productId, l.sign, l.quantity, l.unitCost, l.amount])).toEqual([[pa, 'out', '1', '100', '100'], [pc, 'in', '3', '0', '0']]);
    ...
    expect(fixed.rows.map((r) => [r.systemQty, r.varianceQty, r.unitCost, r.varianceValue])).toEqual([['9', '-1', '100', '-100'], ['5', '0', '0', '0'], ['0', '3', '0', '0']]);
    ...
    expect(await caught(fx.act(clerk, 'stock_count_line.update', { id: String(count.lines.stock_count_line[0]?.id), patch: { countedQty: '1' } }))).toBeInstanceOf(StateError);
    ...
    const noVariance = await fx.act<Row & { id: string }>(clerk, 'stock_count.create', { lines: { stock_count_line: [{ productId: pb, countedQty: '5' }] } });
    expect(await fx.act<Row>(clerk, 'stock_count.submit', { id: noVariance.id })).toMatchObject({ docstatus: 1, adjustmentEntryId: null });
    const dup = await fx.act<Row & { id: string }>(clerk, 'stock_count.create', { lines: { stock_count_line: [{ productId: pb, countedQty: '1' }, { productId: pb, countedQty: '2' }] } });
    expect(issuesOf(await caught(fx.act(clerk, 'stock_count.submit', { id: dup.id })))).toEqual(['lines.2.productId']);
    expect(issuesOf(await caught(fx.act(clerk, 'stock_count.submit', { id: (await fx.act<Row & { id: string }>(clerk, 'stock_count.create', {})).id })))).toEqual(['lines']);
  });
  ...
  it('AC-7 reports: stock_on_hand (current / warehouse / asOf), ledger with opening and running balance, valuation', async () => {
    ...
    const t = await post({ type: 'transfer', date: '2026-07-15', toWarehouseId: fx.wh.sub }, [{ productId: p, quantity: '4' }]);
    await post({ type: 'issue', date: '2026-08-01' }, [{ productId: p, quantity: '3' }]);
    const onHand = (params: Partial<ContextParams>, input: Row) => fx.act<Table>(params, 'inventory.stock_on_hand', input);
    const rowsFor = (tbl: Table) => tbl.rows.filter((r) => r.productId === p).map((r) => [r.productCode, r.productName, r.warehouseCode, r.qty, r.avgCost, r.value]);

    const current = await onHand(clerk, {});
    expect(current.columns.map((c) => c.key)).toEqual(['productCode', 'productName', 'warehouseCode', 'qty', 'avgCost', 'value', 'productId', 'warehouseId']);
    expect(rowsFor(current)).toEqual([['R', '品目R', 'MAIN', '13', '115', '1495'], ['R', '品目R', 'SUB', '4', '115', '460']]);
    const allValues = await fx.run(admin, async (ctx) => Decimal.sum((await repo(ctx, StockBalance).list({ limit: 500 })).items.map((b) => b.value)).toString());
    ...
    expect(rowsFor(await onHand(asRole(['sales']), { warehouseId: fx.wh.sub }))).toEqual([['R', '品目R', 'SUB', '4', '115', '460']]);
    ...
    expect(rowsFor(asOf15)).toEqual([['R', '品目R', 'MAIN', '16', '115', '1840'], ['R', '品目R', 'SUB', '4', '115', '460']]);
    expect(asOf15.meta).toMatchObject({ source: 'ledger', asOf: '2026-07-15' });

    const ledger = await fx.act<Table>(asRole(['viewer']), 'inventory.ledger', { productId: p, warehouseId: fx.wh.main, from: '2026-07-05', to: '2026-07-31' });
    expect(ledger.rows.map((r) => [r.date, r.type, r.qtyIn, r.qtyOut, r.unitCost, r.costDelta, r.balanceQty, r.balanceValue, r.entryId])).toEqual([
    ...
    expect(ledger.meta).toMatchObject({ productCode: 'R', opening: { qty: '10', value: '1000' }, closing: { qty: '16', value: '1840' }, truncated: false });
    const all = await fx.act<Table>(clerk, 'inventory.ledger', { productId: p, from: '2026-01-01', to: '2026-12-31' });
    expect(all.rows.map((r) => [r.warehouseCode, r.balanceQty, r.balanceValue])).toEqual([[null, '0', '0'], ['MAIN', '10', '1000'], ['MAIN', '20', '2300'], ['MAIN', '16', '1840'], ['SUB', '20', '2300'], ['MAIN', '17', '1955']]);
    expect(all.rows[1]?.entryId).toBe(r1.id);
    expect(await caught(fx.act(clerk, 'inventory.ledger', { productId: p, from: '2026-02-01', to: '2026-01-01' }))).toBeInstanceOf(ValidationError);

    const valuation = await fx.act<Table>(clerk, 'inventory.valuation', { asOf: '2026-12-31' });
    expect(valuation.rows.filter((r) => r.productId === p).map((r) => [r.productCode, r.qty, r.avgCost, r.value])).toEqual([['R', '17', '115', '1955']]);
    expect(valuation.totals).toEqual({ value: allValues });
    expect(valuation.meta).toMatchObject({ asOf: '2026-12-31', method: 'moving_average' });
    expect((await fx.act<Table>(clerk, 'inventory.valuation', { asOf: '2026-06-30' })).rows.filter((r) => r.productId === p)).toEqual([]);
    ...
    expect(acct.rows.filter((r) => r.productId === p).map((r) => [r.productCode, r.productName, r.value])).toEqual([[null, p, '1955']]);
    expect(await caught(fx.act(asRole(['sales']), 'inventory.count_variance', { countId: p }))).toBeInstanceOf(PermissionDenied);
  ...
  it('AC-9 roles: purchasing receipts only, sales only through invoices, accounting / viewer read; inventory alone cannot read products', async () => {
    ...
    const receipt = await createEntry(purchasing, { type: 'receipt' }, [{ productId: fx.product.b, quantity: '1', unitCost: '10' }]);
    ...
    expect(((await caught(fx.act(purchasing, 'stock_entry.update', { id: draft.id, patch: { type: 'transfer' } }))) as PermissionDenied).details).toMatchObject({ op: 'update:transfer' });
    const clerkIssue = await createEntry(clerk, { type: 'issue' }, [{ productId: fx.product.b, quantity: '1' }]);
    expect(((await caught(fx.act(purchasing, 'stock_entry_line.create', { entryId: clerkIssue.id, productId: fx.product.b, quantity: '1' }))) as PermissionDenied).details).toMatchObject({ op: 'create:issue' });
    expect(((await caught(submitEntry(purchasing, clerkIssue.id))) as PermissionDenied).details).toMatchObject({ op: 'submit:issue' });
    expect(await caught(fx.act(purchasing, 'stock_count.create', {}))).toBeInstanceOf(PermissionDenied);

    const sales = asRole(['sales']);
    expect(((await caught(createEntry(sales, { type: 'issue' }, []))) as PermissionDenied).details).toMatchObject({ op: 'create:issue' });
    expect(((await caught(submitEntry(sales, clerkIssue.id))) as PermissionDenied).details).toMatchObject({ op: 'submit:issue' });
    expect((await fx.act<{ total: number }>(sales, 'stock_entry.list', {})).total).toBeGreaterThan(0);
    expect(await caught(fx.act(sales, 'warehouse.update', { id: fx.wh.sub, patch: { name: 'x' } }))).toBeInstanceOf(PermissionDenied);

    for (const roles of [['accounting'], ['viewer']]) {
      expect(await caught(fx.act(asRole(roles), 'stock_entry.create', { type: 'receipt' }))).toBeInstanceOf(PermissionDenied);
    ...
    const alone = await caught(createEntry(asRole(['inventory']), { type: 'receipt' }, [{ productId: fx.product.b, quantity: '1', unitCost: '1' }]));
  ...
  it('AC-1 stock_ledger is append-only and stock_balance derived: every direct write is refused, admin included; balances equal the ledger replay', async () => {
    ...
    const id = anyRow?.id ?? '';
    const values = { date: '2026-09-10', warehouseId: fx.wh.main, productId: fx.product.a, qtyDelta: '1', unitCost: '1', costDelta: '1', balanceQty: '1', balanceCost: '1', sourceEntity: 'x', sourceId: id, seq: 999 };
    ...
    const replay = await fx.run(admin, async (ctx: Context) => {
      const sums = await repo(ctx, StockLedger).aggregate({ groupBy: ['productId', 'warehouseId'], metrics: { qty: { sum: 'qtyDelta' }, value: { sum: 'costDelta' }, rows: { count: true }, lastSeq: { max: 'seq' } } });
    ...
    for (const b of replay.balances) {
      ...
      expect([(s?.qty as Decimal).toString(), (s?.value as Decimal).toString(), Number(s?.rows), Number(s?.lastSeq)]).toEqual([b.qty.toString(), b.value.toString(), b.lastSeq, b.lastSeq]);
    }
