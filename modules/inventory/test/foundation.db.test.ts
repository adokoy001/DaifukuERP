...
async function receipt(date: string) {
  const draft = await act<{ id: string }>('stock_entry.create', {
    type: 'receipt',
    date,
    warehouseId: fx.wh.main,
    lines: { stock_entry_line: [{ productId: fx.product.a, quantity: '10', unitCost: '100' }] },
  });
...
describe('inventory foundation invariants', () => {
  it('cannot move a submitted stock-count line to a draft parent', async () => {
    ...
    const head = {
      date: '2026-09-02',
      warehouseId: fx.wh.main,
      lines: { stock_count_line: [{ productId: fx.product.a, countedQty: '10' }] },
    };
    ...
    const line = await fx.run(
      {},
      async (ctx) => (await repo(ctx, StockCountLine).list({ where: { countId: posted.id } })).items[0],
    );
    await expect(
      act('stock_count_line.update', { id: line?.id, patch: { countId: other.id, countedQty: '0' } }),
  ...
  it('refuses backdated stock posting and zero-variance counts without changing the ledger', async () => {
    ...
    const old = await act<{ id: string }>('stock_count.create', {
      date: '2026-09-04',
      warehouseId: fx.wh.main,
      lines: { stock_count_line: [{ productId: fx.product.a, countedQty: '20' }] },
    });
    await expect(act('stock_count.submit', { id: old.id })).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(act('stock_entry.cancel', { id: later.id, correctionDate: '2026-09-03' })).rejects.toMatchObject({
      code: 'INVALID_STATE',
    });
