...
import { newId, registerCrudActions, registry, repo, runAction, type Context, type ContextParams, type EntityDef } from '@daifuku/kernel';
...
export type Table = { title: { ja: string; en: string }; columns: { key: string; kind: string; ref?: string }[]; rows: Row[]; totals?: Record<string, string>; meta?: Row };
export type EntryLineJson = Row & { id: string; seq: number; productId: string; quantity: string; sign: string | null; unitCost: string | null; amount: string };
export type EntryJson = Row & { id: string; number: string | null; docstatus: number; type: string; date: string; warehouseId: string; toWarehouseId: string | null; sourceEntity: string | null; sourceId: string | null; amendedFrom: string | null };
export type EntryWithLines = EntryJson & { lines: { stock_entry_line: EntryLineJson[] } };

export const asRole = (roles: string[]): Partial<ContextParams> => ({ roles, actor: { type: 'user' as const, id: newId() } });
export const caught = (p: Promise<unknown>): Promise<unknown> => p.then(() => null, (e: unknown) => e);
export const issuesOf = (e: unknown) => ((e as { details?: { issues?: { path: string; message: string }[] } }).details?.issues ?? []).map((i) => i.path);
...
export async function setupFixture(): Promise<Fixture> {
  ...
  await run({}, async (ctx) => {
    await repo(ctx, taxRate).create({ code: 'STD10', category: 'standard', rate: '0.10', validFrom: '2019-10-01', label: '標準10%' } as never);
    await repo(ctx, taxRate).create({ code: 'RED8', category: 'reduced', rate: '0.08', validFrom: '2019-10-01', label: '軽減8%' } as never);
    await seedUoms(ctx);
    await seedWarehouses(ctx);
    await seedWarehouses(ctx); // idempotent
  });
  const productOf = (code: string, name: string, extra: Row = {}) => run({}, async (ctx) => (await repo(ctx, Product).create({ code, name, taxCategory: 'standard', salePrice: '200', purchasePrice: '100', ...extra })).id);
  const product = {
  ...
  const main = await run({}, async (ctx) => (await repo(ctx, Warehouse).list({ where: { code: 'MAIN' }, limit: 1 })).items[0]?.id ?? '');
  const sub = (await run({}, (ctx) => repo(ctx, Warehouse).create({ code: 'sub', name: '第二倉庫' }))).id;
  if (InventoryModule.name !== 'inventory' || !main) throw new TypeError('inventory fixture: module or MAIN warehouse missing');
