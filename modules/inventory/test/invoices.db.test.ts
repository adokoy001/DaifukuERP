...
import { AUTO_ISSUE_ON_SALES_KEY, AUTO_RECEIPT_ON_PURCHASE_KEY, boolSettingSchema, DEFAULT_WAREHOUSE_HINT, DEFAULT_WAREHOUSE_KEY, loadBalance, warehouseCodeSchema } from '../src/index.ts';
import { asRole, caught, setupFixture, type EntryWithLines, type Fixture, type Row } from './fixture.ts';

type InvoiceJson = Row & { id: string; number: string | null; docstatus: number; status: string; journalEntryId: string | null; total: string };
...
const createSubmit = async (params: Partial<ContextParams>, entity: 'purchase_invoice' | 'sales_invoice', head: Row, lines: Row[]): Promise<InvoiceJson> => {
...
const linkedEntries = async (sourceEntity: string, sourceId: string): Promise<EntryWithLines[]> => {
  const list = await fx.act<{ items: { id: string }[] }>(admin, 'stock_entry.list', { where: { sourceEntity, sourceId } });
...
const bill = (lines: Row[], head: Row = {}) => createSubmit(purchasing, 'purchase_invoice', { partnerId: fx.partner.supplier, date: '2026-09-01', priceIncludesTax: false, ...head }, lines);
const invoice = (lines: Row[], head: Row = {}) => createSubmit(sales, 'sales_invoice', { partnerId: fx.partner.customer, date: '2026-09-05', ...head }, lines);
...
describe('inventory hooks on invoices (docs/specs/inventory.md AC-5)', () => {
  ...
  it('AC-5 a purchase invoice with 1 goods + 1 service line (+ 1 expense line) → 1 submitted receipt with 1 line, purchasing role', async () => {
    ...
    expect(entries[0]).toMatchObject({ type: 'receipt', docstatus: DOCSTATUS.submitted, warehouseId: fx.wh.main, partnerId: fx.partner.supplier, date: '2026-09-01', note: `仕入請求書 ${bill1.number ?? ''}`, sourceEntity: 'purchase_invoice', sourceId: bill1.id });
    expect(entries[0]?.number).toMatch(/^STK-2026-\d{6}$/);
    expect(entries[0]?.lines.stock_entry_line.map((l) => [l.productId, l.quantity, l.sign, l.unitCost, l.amount])).toEqual([[fx.product.a, '10', null, '100', '1000']]);
  ...
  it('AC-5 tax-inclusive bill: unitCost = 税抜 unit price = unitPrice ÷ (1 + the line rate), 6 decimals', async () => {
    const b = await bill([{ productId: fx.product.b, quantity: '3', unitPrice: '1100' }, { productId: fx.product.food, quantity: '2', unitPrice: '1000' }], { priceIncludesTax: true });
  ...
  it('AC-5 no goods line → no stock entry; inventory.auto_receipt_on_purchase = false → no stock entry', async () => {
    const onlyService = await bill([{ productId: fx.product.svc, quantity: '1', unitPrice: '5000' }, { accountId: fx.acc['6400'], description: '消耗品', unitPrice: '100', taxCategory: 'standard' }]);
  ...
  it('AC-5 a sales invoice issues its goods lines from the default warehouse at the moving average, sales role', async () => {
    sale1 = await invoice([{ productId: fx.product.a, quantity: '4' }, { productId: fx.product.svc, quantity: '1' }]);
    ...
    expect(entries).toHaveLength(1);
    expect(entries[0]).toMatchObject({ type: 'issue', docstatus: DOCSTATUS.submitted, warehouseId: fx.wh.main, partnerId: fx.partner.customer, date: '2026-09-05', note: `売上請求書 ${sale1.number ?? ''}` });
    expect(entries[0]?.lines.stock_entry_line.map((l) => [l.productId, l.quantity, l.unitCost, l.amount])).toEqual([[fx.product.a, '4', '100', '400']]);
  ...
  it('AC-5 insufficient stock fails the sales invoice submit as a whole: invoice stays a draft, no journal entry, no movement', async () => {
    ...
    expect(await fx.act<InvoiceJson>(sales, 'sales_invoice.get', { id })).toMatchObject({ docstatus: DOCSTATUS.draft, status: 'draft', journalEntryId: null });
  ...
  it('AC-5 cancel: the sales invoice cancel cancels its issue; a bill whose goods were sold cannot be cancelled until the sale is', async () => {
    const refused = await caught(fx.act(purchasing, 'purchase_invoice.cancel', { id: bill1.id, correctionDate: '2026-09-10' }));
    ...
    expect(await fx.act<InvoiceJson>(purchasing, 'purchase_invoice.get', { id: bill1.id })).toMatchObject({ docstatus: DOCSTATUS.submitted, status: 'open' });
    ...
    expect(await fx.act<InvoiceJson>(purchasing, 'purchase_invoice.cancel', { id: bill1.id, correctionDate: '2026-09-10' })).toMatchObject({ docstatus: DOCSTATUS.cancelled, status: 'cancelled' });
    ...
    expect((await linkedEntries('purchase_invoice', amended.id)).map((e) => [e.docstatus, e.lines.stock_entry_line.length])).toEqual([[DOCSTATUS.submitted, 1]]);
    expect(await balanceOf(fx.product.a, fx.wh.main)).toEqual(['10', '100', '1000']);
  });
  ...
  it('AC-5 default warehouse: inventory.default_warehouse code, else the isDefault warehouse, else INVALID_STATE (the invoice stays a draft)', async () => {
    const setCode = (code: string) => fx.run(admin, (ctx) => setSetting(ctx, DEFAULT_WAREHOUSE_KEY, warehouseCodeSchema, code));
    await setCode('SUB');
    ...
    expect((await fx.act<InvoiceJson>(purchasing, 'purchase_invoice.get', { id: (err as { invoiceId?: string }).invoiceId ?? '' })).docstatus).toBe(DOCSTATUS.draft);
