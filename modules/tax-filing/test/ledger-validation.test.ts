...
const D = Decimal.from, year = { startDate: '2026-01-01', endDate: '2026-12-31' };
const entries = [{ id: '1', docstatus: 1, totalDebit: D(100), totalCredit: D(100) }], lines = [{ entryId: '1', debit: D(100), credit: D(0) }, { entryId: '1', debit: D(0), credit: D(100) }];
describe('independent ledger evidence checks', () => {
 it('requires complete, non-overlapping annual period coverage', () => {
  ...
  for (const periods of [[], [{ ...year, startDate: '2026-02-01' }], [{ ...year, endDate: '2026-11-30' }], [year, year]]) expect(ledgerIntegrityIssues(year, periods, entries, lines).map((i) => i.code)).toContain('period_coverage');
 });
