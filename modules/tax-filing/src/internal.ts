...
export function filingWrite<T>(ctx: Context, entity: EntityDef, work: (write: Context) => Promise<T>): Promise<T> {
  const cap = capabilities.get(entity.name);
  if (!cap) throw new Error('Unknown filing entity');
  return withWriteCapability(ctx, cap, work);
}
export function registerFilingGuards() {
  for (const e of [Account, JournalEntry])
    for (const phase of ['before_create', 'before_update', 'before_delete'] as const)
      registry.registerHook(e.name, phase, (ctx) => withLock(ctx, 'accounting-periods', async () => undefined));
  registry.registerHook(WorkforceEmployee.name, 'before_create', (ctx) =>
    withLock(ctx, 'workforce:policies', async () => undefined),
  );
  registry.registerHook(WorkforceEmployee.name, 'before_delete', (ctx, { row }) =>
    withLock(ctx, 'workforce:employee:' + String(row.id), async () => undefined),
  );
