...
export async function payrollSource(ctx: Context, taxYear: number): Promise<FilingSource> {
  ...
  const rows: FilingPayrollRow[] = selected.map((employee) => {
    ...
    const previousPay = previousKnown
      ? previous.reduce<Decimal>((sum, row) => sum.plus(String(object(row).taxablePay)), D(0))
      : null;
    if (!previousKnown)
      issues.push(
        issue(
          'prior_employer_amount_missing',
          '確定年調の前職支払額に読取不能な情報があります。原票を再確認してください。',
          employee.id,
          'warning',
        ),
      );
    const missingEvidence = pay.some(
      (p) =>
        p.docstatus === 1 && p.period.startsWith(String(taxYear)) && !allEvidence.some((e) => e.payrollId === p.id),
    );
    if (!facts?.address || !facts.nameKana || !facts.birthDate || !facts.municipalityCode)
      issues.push(
        issue(
          'recipient_facts_missing',
          '提出に必要な住所・氏名カナ・生年月日・自治体を補完してください。',
          employee.id,
          'warning',
        ),
      );
    if (!facts || facts.unpaidSalaryAmount === null || facts.uncollectedTaxAmount === null)
      issues.push(issue('unpaid_unconfirmed', '未払給与・未徴収税額の有無が未確認です。', employee.id, 'warning'));
    if (missingEvidence || !paid.length)
      issues.push(
        issue(
          'payroll_evidence_missing',
          '年内支払の証跡が不足しています。翌年支払を含む支払年の判定を確認してください。',
          employee.id,
          'warning',
        ),
      );
    if (pay.some((p) => p.docstatus === 0 && p.period.startsWith(String(taxYear))))
      issues.push(issue('payroll_draft', '年度内の給与下書きが残っています。', employee.id, 'warning'));
    if (!adjustment)
      issues.push(
        issue(
          'adjustment_missing',
          '一意の確定年末調整がありません。税額を未算定として扱います。',
          employee.id,
          'warning',
        ),
      );
    if (declared.spouse || (Array.isArray(declared.relatives) && declared.relatives.length))
      issues.push(
        issue('family_form_details', '親族氏名等の正式帳票記載情報は原資料で補完が必要です。', employee.id, 'warning'),
      );
    if (previous.length)
      issues.push(
        issue('prior_employer_details', '前職の住所・退職日等を原票から補完してください。', employee.id, 'warning'),
      );
    if (D(money(calculation.housingTaxCredit) ?? '0').gt(0))
      issues.push(
        issue(
          'housing_form_details',
          '住宅控除の居住開始日・区分・残高等は原資料を参照してください。',
          employee.id,
          'warning',
        ),
      );
    const total = (key: 'taxablePay' | 'socialPremium' | 'incomeTax') =>
      paid.length && !missingEvidence ? paid.reduce((sum, r) => sum.plus(r[key]), D(0)).toString() : null;
    const taxablePay = total('taxablePay');
    if (
      adjustment &&
      taxablePay !== null &&
      previousPay !== null &&
      !D(taxablePay).plus(previousPay).eq(adjustment.taxablePay)
    )
      issues.push(
        issue(
          'year_end_source_mismatch',
          '社内給与と前職額の合計が確定年調と一致しません。再確認してください。',
          employee.id,
          'warning',
        ),
      );
    return {
      ...
      facts,
      payrollCount: paid.length,
      taxablePay,
      socialPremium: total('socialPremium'),
      withheldTax: total('incomeTax'),
      adjustmentId: adjustment?.id ?? null,
      annualTax: adjustment?.annualTax.toString() ?? null,
      salaryIncome: money(calculation.salaryIncome),
      deductionTotal: money(deduction.total),
      previousEmployerPay: adjustment && previousPay ? previousPay.toString() : null,
      refund: adjustment?.refund.toString() ?? null,
      additionalTax: adjustment?.additionalTax.toString() ?? null,
      issues,
    };
  });
  const issues = rows.flatMap((r) => r.issues);
  if (!profile.payerAddress || !profile.payerPhone)
    issues.push(issue('payer_facts_missing', '支払者の所在地・連絡先を原資料で補完してください。', null, 'warning'));
  ...
  return {
    ...
    totals: {
      taxablePay:
        rows.length && rows.every((r) => r.taxablePay !== null)
          ? rows.reduce((sum, r) => sum.plus(r.taxablePay ?? '0'), D(0)).toString()
          : null,
      annualTax:
        rows.length && rows.every((r) => r.annualTax !== null)
          ? rows.reduce((sum, r) => sum.plus(r.annualTax ?? '0'), D(0)).toString()
          : null,
    },
    versions: [
      ...rowVersions(FilingPayrollProfile, [saved]),
      ...rowVersions(WorkforceEmployee, employees),
      ...rowVersions(WorkforcePayroll, payrolls),
      ...rowVersions(WorkforcePayrollTaxEvidence, allEvidence),
      ...rowVersions(WorkforceYearEndAdjustment, adjustments),
      ...rowVersions(WorkforceYearEndDeclaration, declarations),
      ...rowVersions(WorkforcePayrollCondition, conditions),
    ],
    records: {
