...
export function summary(kind: FilingKind, row: FilingPack) {
  assertEvidence(row);
  return filingSummary.parse({
    ...row,
    kind,
    createdAt: row.createdAt.toISOString(),
    confirmedAt: row.confirmedAt?.toISOString() ?? null,
    issues: row.prepared.issues,
    totals: row.prepared.totals,
  });
}
export async function filingBoard(ctx: Context, kind: FilingKind) {
  ...
  const closed = (year: { id: string; startDate: string; endDate: string }) => {
    ...
    return (
      relevant.length > 0 &&
      relevant.every((p) => p.isClosed) &&
      ledgerIntegrityIssues(year, relevant, [], []).length === 0
    );
  };
  return filingBoardOutput.parse({
    kind,
    profiles: filingProfiles()
      .filter((p) => p.kind === kind)
      .map((p) => p.option),
    accountingProfile:
      kind === 'accounting' && saved ? { ...accountingProfileData.parse(saved.data), version: saved.version } : null,
    payrollProfile:
      kind === 'payroll' && saved ? { ...payrollProfileData.parse(saved.data), version: saved.version } : null,
    years: years.map((y) => ({ id: y.id, code: y.code, from: y.startDate, to: y.endDate, closed: closed(y) })),
    accounts,
    employees,
    packs: packs.items.map((r) => summary(kind, r)),
    truncated: packs.total > packs.items.length,
  });
}
export async function filingDetail(ctx: Context, kind: FilingKind, id: string) {
  return sourceLocks(ctx, kind, async () => {
    ...
    return filingDetailOutput.parse({
      ...summary(kind, row),
      stale: sourceHash(latest.source) !== row.sourceHash,
      statements: row.prepared.statements,
      payrollRows: row.prepared.payrollRows,
      sourceCount: row.source.versions.length,
      sourceVersions: row.source.versions,
