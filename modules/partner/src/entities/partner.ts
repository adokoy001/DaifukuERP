...
export const Partner = defineEntity({
  ...
  fields: {
    ...
    closingDay: f.int({
      label: label('締め日', 'Closing day'),
      description: label('31 は月末', '31 means end of month'),
      required: true,
      default: END_OF_MONTH,
      min: 1,
      max: 31,
    }),
    paymentMonthOffset: f.int({
      label: label('支払月', 'Payment month offset'),
      description: label('締め月からの月数', 'Months after the closing month'),
      required: true,
      default: 1,
      min: 0,
      max: 3,
    ...
    bankAccountType: f.enum(['ordinary', 'current', 'savings'], {
      ...
      labels: {
        ordinary: label('普通', 'Ordinary'),
        current: label('当座', 'Current'),
        savings: label('貯蓄', 'Savings'),
      },
    }),
    bankAccountNo: f.text({ label: label('口座番号', 'Account no.'), pattern: /^\d{1,7}$/ }),
    bankAccountHolderKana: f.text({
      label: label('口座名義カナ', 'Account holder (kana)'),
      normalize: 'halfwidth-kana',
      maxLength: 30,
    }),
