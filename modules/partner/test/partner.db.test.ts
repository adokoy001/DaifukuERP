import { Conflict, NotFound, PermissionDenied, ValidationError, appMeta, newId, registerCrudActions, repo, runAction, systemParams, withContext } from '@daifuku/kernel';
...
describe('partner entity (docs/specs/partner.md)', () => {
  ...
  it('AC-2 invoiceRegistrationNo accepts T+13 digits only; the error names the field', async () => {
    const ok = await db.run({}, (ctx) => repo(ctx, Partner).create({ name: 'OK', invoiceRegistrationNo: 'T1234567890123' }));
    ...
    for (const bad of ['1234567890123', 'T123456789012', 'T12345678901234', 't1234567890123', 'T-1234567890123']) {
      const err = await db.run({}, (ctx) => repo(ctx, Partner).create({ name: 'NG', invoiceRegistrationNo: bad })).catch((e: unknown) => e);
  ...
  it('AC-3 nameKana / bankAccountHolderKana are stored as upper-cased half-width katakana', async () => {
    const p = await db.run({}, (ctx) =>
      repo(ctx, Partner).create({ name: 'カナ', nameKana: 'かぶしきがいしゃ　だいふく', bankAccountHolderKana: 'ｶ)ﾀﾞｲﾌｸ ｱｲｳ abc' }),
    );
  ...
  it('AC-4 code is unique per company (Conflict) and immutable after create', async () => {
    ...
    await expect(db.run({}, (ctx) => repo(ctx, Partner).create({ name: 'B', code: 'DUP-1' }))).rejects.toBeInstanceOf(Conflict);
    await expect(db.run({}, (ctx) => repo(ctx, Partner).update(a.id, { code: 'DUP-2' }))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'code' }] } });
    ...
    const b = await withContext(db.app, systemParams(db.tenantId, otherCompany), (ctx) => repo(ctx, Partner).create({ name: 'B', code: 'DUP-1' }));
  ...
  it('AC-5 closingDay/paymentDay accept 1..31, paymentMonthOffset 0..3; everything else is rejected', async () => {
    const ok = await db.run({}, (ctx) => repo(ctx, Partner).create({ name: 'Terms', closingDay: 1, paymentMonthOffset: 0, paymentDay: 31 }));
    expect(ok).toMatchObject({ closingDay: 1, paymentMonthOffset: 0, paymentDay: 31 });
    const ok3 = await db.run({}, (ctx) => repo(ctx, Partner).create({ name: 'Terms3', closingDay: 31, paymentMonthOffset: 3, paymentDay: 1 }));
    expect(ok3.paymentMonthOffset).toBe(3);
    for (const bad of [{ closingDay: 0 }, { closingDay: 32 }, { paymentDay: 0 }, { paymentDay: 32 }, { paymentMonthOffset: -1 }, { paymentMonthOffset: 4 }, { closingDay: 15.5 }]) {
  ...
  it('AC-6 viewer may read but not create/update/delete', async () => {
    ...
    await expect(db.run(viewer, (ctx) => repo(ctx, Partner).create({ name: 'x' }))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(db.run(viewer, (ctx) => repo(ctx, Partner).update(target.id, { name: 'y' }))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(db.run(viewer, (ctx) => repo(ctx, Partner).delete(target.id))).rejects.toBeInstanceOf(PermissionDenied);
  });
  ...
  it('AC-7 sales/purchasing: read+create+update; accounting: read+update+export; admin: everything', async () => {
    for (const role of ['sales', 'purchasing']) {
      ...
      const updated = await db.run(asRole([role]), (ctx) => repo(ctx, Partner).update(created.id, { name: `by ${role} 2` }));
      expect(updated.version).toBe(2);
      await expect(db.run(asRole([role]), (ctx) => repo(ctx, Partner).delete(created.id))).rejects.toBeInstanceOf(PermissionDenied);
      const ops = await db.run(asRole([role]), async (ctx) => appMeta(ctx).entities.find((e) => e.name === 'partner')?.ops);
      expect(ops).toEqual(['read', 'create', 'update']);
    }
    const acc = asRole(['accounting']);
    await expect(db.run(acc, (ctx) => repo(ctx, Partner).create({ name: 'x' }))).rejects.toBeInstanceOf(PermissionDenied);
    ...
    await expect(db.run(asRole(['nobody']), (ctx) => repo(ctx, Partner).list())).rejects.toBeInstanceOf(PermissionDenied);
  });
  ...
  it('AC-9 seed creates 3 partners and is idempotent', async () => {
    const seed = () => withContext(db.app, systemParams(db.tenantId, db.companyId), async (ctx) => PartnerModule.seed?.(ctx));
  ...
  it('AC-10 partner.compute_due_date uses the partner terms; 月末 clamps; after closing day -> next period', async () => {
    const p = await db.run({}, (ctx) => repo(ctx, Partner).create({ name: 'Terms', closingDay: 20, paymentMonthOffset: 1, paymentDay: 31 }));
    const inPeriod = await db.run({}, (ctx) => runAction(ctx, 'partner.compute_due_date', { partnerId: p.id, invoiceDate: '2026-01-20' }));
    expect(inPeriod).toEqual({ partnerId: p.id, invoiceDate: '2026-01-20', closingDate: '2026-01-20', dueDate: '2026-02-28' });
    const nextPeriod = await db.run({}, (ctx) => runAction(ctx, 'partner.compute_due_date', { partnerId: p.id, invoiceDate: '2026-01-21' }));
    expect(nextPeriod).toMatchObject({ closingDate: '2026-02-20', dueDate: '2026-03-31' });
    await expect(db.run({}, (ctx) => runAction(ctx, 'partner.compute_due_date', { partnerId: p.id, invoiceDate: '2026-13-01' }))).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'invoiceDate' }] },
    });
    await expect(db.run({}, (ctx) => runAction(ctx, 'partner.compute_due_date', { partnerId: newId(), invoiceDate: '2026-01-01' }))).rejects.toBeInstanceOf(NotFound);
    // permission: read on partner is enough; a role without read is denied
    const viewerResult = await db.run(asRole(['viewer']), (ctx) => runAction(ctx, 'partner.compute_due_date', { partnerId: p.id, invoiceDate: '2026-01-01' }));
    expect(viewerResult).toMatchObject({ dueDate: '2026-02-28' });
    await expect(db.run(asRole(['nobody']), (ctx) => runAction(ctx, 'partner.compute_due_date', { partnerId: p.id, invoiceDate: '2026-01-01' }))).rejects.toBeInstanceOf(PermissionDenied);
  });
});
