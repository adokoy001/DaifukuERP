...
export const inboxResult = (row: Infer<typeof PosInbox>) => ({
  id: row.id,
  version: row.version,
  status: row.status,
...
export async function processInbox(ctx: Context, id: string, expectedVersion?: number) {
  ...
  return withLock(ctx, 'pos-payment:' + paymentKey(e.merchantId, e.paymentId ?? e.externalId), async () => {
    ...
    try {
      ...
      status = failure instanceof StateError && failure.message === 'POS_PAYMENT_PENDING' ? 'deferred' : 'blocked';
      error =
        failure instanceof DaifukuError
          ? `${failure.code}: ${failure.message} — ${failure.hint}`.slice(0, 500)
          : 'INTERNAL: POS processing failed';
    }
    const updated = await internalWrite(ctx, PosInbox, (inner) =>
      repo(inner, PosInbox).update(id, {
        status,
        error,
        transactionId,
...
export async function receivePosEvent(ctx: Context, locationId: string, raw: NormalizedPosEvent) {
  ...
  if (
    ...
    location.merchantId !== event.merchantId ||
    (event.locationId !== null && location.externalLocationId !== event.locationId)
  )
    throw new StateError(
      'POS mapping does not match this event',
      'Use the configured company, merchant and location mapping.',
    );
  const result = await withLock(ctx, 'pos-event:' + event.eventId, async () => {
    ...
    if (previous) {
      if (previous.locationId !== location.id || normalizedPosEvent.parse(previous.event).rawHash !== event.rawHash)
        throw new Conflict('POS event ID has conflicting content', 'Do not reuse or alter an accepted external event.');
      return processInbox(ctx, previous.id);
    }
    const row = await internalWrite(ctx, PosInbox, (inner) =>
      repo(inner, PosInbox).create({
        locationId,
  ...
  if (event.kind === 'payment' && result.status === 'posted') {
    const pending = await repo(ctx, PosInbox).list({
      where: { locationId, paymentId: event.externalId, status: 'deferred' },
      limit: 500,
    });
