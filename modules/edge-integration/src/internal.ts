...
export function edgeWrite<T>(ctx: Context, e: EntityDef, work: (inner: Context) => Promise<T>) {
  const cap = caps.get(e.name);
  if (!cap) throw new Error('Unknown edge authority');
...
export function registerEdgeGuards() {
      ...
      registry.registerHook(e.name, phase, (ctx) => {
        if (phase === 'before_delete' || !hasWriteCapability(ctx, e.name, 'workflow'))
          throw new StateError(
            'Use the edge workflow',
            'Device configuration and execution history cannot be changed through generic CRUD.',
          );
      });
  registry.registerHook(WorkforceSite.name, 'before_update', async (ctx, { row, previous }) => {
    ...
    await withRelayLock(ctx, 'site:' + String(row.id), async () => {
      if (await repo(ctx, EdgeGateway).count({ siteId: String(row.id), active: true }))
        throw new StateError('The site has active LAN gateways', 'Disable its gateways before disabling this site.');
    });
  });
}
