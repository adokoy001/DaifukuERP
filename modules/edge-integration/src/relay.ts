...
const leaseUntil = (ctx: Context, expiry: Date) =>
  new Date(Math.min(expiry.getTime(), ctx.now().getTime() + EDGE_LEASE_SECONDS * 1000));
export async function claimJob(ctx: Context) {
  return relayWork(ctx, async () => {
    await touchRelayPresence(ctx);
    await sweepGateway(ctx, ctx.actor.id);
    const jobs = (
      await repo(ctx, EdgeJob).list({
        where: { gatewayId: ctx.actor.id, state: { $in: pendingStates } },
        limit: 500,
        orderBy: [
    ...
    const devices = new Map(
      (await repo(ctx, EdgeDevice).list({ where: { gatewayId: ctx.actor.id, active: true }, limit: 100 })).items.map(
        (d) => [d.id, d],
      ),
    );
    ...
    for (const candidate of jobs) {
      ...
      if (
        await repo(ctx, EdgeJob).count({
          deviceId: candidate.deviceId,
          state: { $in: ['claimed', 'executing', 'uncertain'] },
        })
      ) {
        busy.add(candidate.deviceId);
        continue;
      }
      selected = candidate;
      break;
...
async function leasedJob(ctx: Context, input: z.infer<typeof edgeLease>) {
  ...
  if (job.attempt !== input.attempt || job.leaseHash !== relayHash(input.leaseToken))
    throw new StateError('The claim has been superseded', 'Do not execute or retry this physical operation.');
  return job;
}
export async function startJob(ctx: Context, input: z.infer<typeof edgeLease>) {
  return relayWork(ctx, async () => {
    await sweepGateway(ctx, ctx.actor.id);
    let job = await leasedJob(ctx, input);
    ...
    if (job.state === 'claimed' && job.leaseUntil && job.leaseUntil > ctx.now() && job.expiresAt > ctx.now()) {
      ...
      job = await saveJob(ctx, job, {
        state: 'executing',
        startedAt: ctx.now(),
        leaseUntil: leaseUntil(ctx, job.expiresAt),
      });
      startGranted = true;
    }
    return {
...
export async function heartbeatJob(ctx: Context, input: z.infer<typeof edgeLease>) {
  return relayWork(ctx, async () => {
    ...
    await sweepGateway(ctx, ctx.actor.id);
    let job = await leasedJob(ctx, input);
    const accepted = job.state === 'executing';
...
export async function completeJob(ctx: Context, input: z.infer<typeof edgeCompleteInput>) {
  return relayWork(ctx, async () => {
    const job = await repo(ctx, EdgeJob).get(input.jobId);
    if (job.attempt !== input.attempt || job.leaseHash !== relayHash(input.leaseToken))
      return { accepted: false, state: job.state, version: job.version, ignored: 'obsolete_attempt' as const };
    if (job.resolvedAt)
      return { accepted: false, state: job.state, version: job.version, ignored: 'manually_resolved' as const };
    if (job.state === 'succeeded' || job.state === 'failed') {
      const accepted =
        job.result?.state === input.result.state &&
        job.result.code === input.result.code &&
        job.result.deviceJobId === input.result.deviceJobId &&
        job.result.summary === input.result.summary;
    ...
    const unknownBeforeStart =
      ['claimed', 'queued', 'expired'].includes(job.state) && input.result.state === 'uncertain';
    if (!unknownBeforeStart && !['executing', 'uncertain'].includes(job.state))
      throw new StateError(
        'The job cannot accept this outcome',
        'Do not retry the physical operation; review its current state.',
      );
    if (input.result.state !== 'uncertain' && !job.startedAt)
      throw new StateError(
        'No execution start was granted',
        'Only a physical operator can resolve this uncertain pre-start outcome.',
      );
    const changed = await saveJob(ctx, job, {
      state: input.result.state,
      result: input.result,
      reason: input.result.summary ?? input.result.code,
    });
...
export async function recordDeviceEvent(ctx: Context, input: z.infer<typeof edgeEventInput>) {
  return relayWork(ctx, async () => {
    ...
    if (device.localDeviceId !== input.localDeviceId)
      throw new StateError('Device binding mismatch', 'Use the paired local device mapping.');
    const observedAt = new Date(input.observedAt);
    if (observedAt.getTime() > ctx.now().getTime() + 300000)
    ...
    if (prior) {
      if (prior.bodyHash !== bodyHash)
        throw new StateError(
          ...
          'Preserve event UUID and content while retrying.',
        );
      return { ok: true as const, duplicate: true };
    }
    await touchRelayPresence(ctx);
    await edgeWrite(ctx, EdgeDeviceEvent, (inner) =>
      repo(inner, EdgeDeviceEvent).create({
        ...input,
        observedAt,
        gatewayId: ctx.actor.id,
        key,
        bodyHash,
        receivedAt: ctx.now(),
      }),
...
export async function notificationPending(ctx: Context) {
  return relayWork(ctx, async () => {
    await touchRelayPresence(ctx);
    return (
      (await repo(ctx, EdgeJob).count({
        gatewayId: ctx.actor.id,
        state: { $in: ['queued', 'claimed', 'executing'] },
      })) > 0
    );
  });
}
