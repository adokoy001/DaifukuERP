...
async function invoice(amount = '1000'): Promise<Row> {
  const draft = await act('sales_invoice.create', { partnerId: customer, date: '2026-08-01', lines: { sales_invoice_line: [{ description: 'Service', unitPrice: amount, taxCategory: 'exempt' }] } });
...
async function payment(invoiceId: string, amount: string, date = '2026-09-02'): Promise<Row> {
  return act('payment.create', { direction: 'receive', partnerId: customer, date, amount, lines: { payment_allocation: [{ invoiceEntity: 'sales_invoice', invoiceId, amount }] } });
}
async function balanceAt(id: string, date: string): Promise<string> {
  return run(async (ctx) => (await invoiceBalancesAsOf(ctx, [await repo(ctx, SalesInvoice).get(id)], date)).get(id)?.toString() ?? 'missing');
}
...
beforeAll(async () => {
  ...
  for (const [code, type] of [['1000', 'asset'], ['1100', 'asset'], ['1300', 'asset'], ['1301', 'asset'], ['2100', 'liability'], ['2200', 'liability'], ['2400', 'liability'], ['1900', 'asset'], ['4000', 'revenue']] as const) {
    accounts[code] = (await run((ctx) => repo(ctx, Account).create({ code, name: code, type, partnerRequired: code === '1300' || code === '1301' }))).id;
  }
...
describe('foundation: source ownership, concurrent settlement and effective history', () => {
  ...
  it('two concurrent payments cannot both consume the same remaining balance', async () => {
    ...
    const entries = await run((ctx) => repo(ctx, JournalEntry).list({ where: { sourceEntity: 'payment', sourceId: { $in: drafts.map((p) => p.id) } } }));
  ...
  it('as-of balances retain later payments and their dated cancellation; settlement uses the original control account', async () => {
    const inv = await invoice();
    await run((ctx) => setSetting(ctx, PAYMENT_ACCOUNTS_KEY, paymentAccountsSchema, { cash: '1000', bank: '1100', receivable: '1301', payable: '2100', advanceReceived: '2400', advancePaid: '1900' }));
    const draft = await payment(inv.id, '400');
    const posted = await act('payment.submit', { id: draft.id });
    const journal = await run((ctx) => repo(ctx, JournalLine).list({ where: { entryId: String(posted.journalEntryId) } }));
    ...
    await expect(act('sales_settlement.create', { invoiceId: inv.id, date: '2026-08-01', amount: '1' })).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
  });
  ...
  it('generic edits cannot sever source linkage or forge balances; independent reversal cannot undo a business cancellation', async () => {
    const inv = await invoice();
    for (const patch of [{ journalEntryId: null }, { paidAmount: '1000', balance: '0' }, { controlAccountId: accounts['1301'] }]) {
      await expect(act('sales_invoice.update', { id: inv.id, patch })).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    }
    await expect(act('accounting.reverse_entry', { id: inv.journalEntryId })).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await act('sales_invoice.cancel', { id: inv.id, correctionDate: '2026-09-07' });
    const reversal = await run((ctx) => repo(ctx, JournalEntry).list({ where: { reversalOf: String(inv.journalEntryId) } }));
    expect(reversal.items).toHaveLength(1);
    await expect(act('accounting.reverse_entry', { id: reversal.items[0]?.id })).rejects.toMatchObject({ code: 'INVALID_STATE' });
  ...
  it('issued snapshots survive partner edits, while unsupported money and oversized documents fail atomically', async () => {
    ...
    await expect(act('sales_invoice.create', { partnerId: customer, date: '2026-08-01', currency: 'USD' })).rejects.toMatchObject({ code: 'VALIDATION' });
    const count = await run((ctx) => repo(ctx, SalesInvoice).count());
    await expect(act('sales_invoice.create', { partnerId: customer, date: '2026-08-01', lines: { sales_invoice_line: Array.from({ length: 501 }, () => ({ description: 'x', unitPrice: '1', taxCategory: 'exempt' })) } })).rejects.toMatchObject({ code: 'VALIDATION' });
