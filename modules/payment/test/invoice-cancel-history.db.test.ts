...
async function cycle(side: 'sales' | 'purchase', reverseDate: string) {
  const partner = await run((ctx) =>
    repo(ctx, Partner).create({ name: `${side}-${reverseDate}`, isCustomer: true, isSupplier: true }),
  );
  const entity = `${side}_invoice`;
  const draft = await act(`${entity}.create`, {
      ...
      [`${entity}_line`]: [
        {
          ...
          unitPrice: '1000',
          taxCategory: 'exempt',
          ...(side === 'purchase' ? { accountId: expense } : {}),
        },
      ],
    },
  });
  await act(`${entity}.submit`, { id: draft.id });
  const p = await act('payment.create', {
    direction: side === 'sales' ? 'receive' : 'pay',
    partnerId: partner.id,
    date: '2026-09-02',
    amount: '400',
    lines: { payment_allocation: [{ invoiceEntity: entity, invoiceId: draft.id, amount: '400' }] },
  });
...
describe('invoice cancellation effective history', () => {
  for (const side of ['sales', 'purchase'] as const) {
    it(`${side}: refuses cancelling across a surviving payment interval and permits the final reversal date`, async () => {
      ...
      await expect(act(`${c.entity}.cancel`, { id: c.invoice.id })).rejects.toMatchObject({
        code: 'VALIDATION',
        details: { issues: [{ path: 'correctionDate' }] },
      });
      await expect(act(`${c.entity}.cancel`, { id: c.invoice.id, correctionDate: '2026-09-03' })).rejects.toMatchObject(
        { code: 'VALIDATION' },
      );
      ...
      const before = await act<Table>(side === 'sales' ? 'sales.ar_aging' : 'purchase.ap_aging', {
        asOf: '2026-09-03',
      });
      expect(before.rows.find((row) => row.partnerId === c.partnerId)).toMatchObject({
        [side === 'sales' ? 'total' : 'balance']: '600',
      });
    ...
    it(`${side}: preserves original-date invoice cancellation when payment and its reversal net to zero on one day`, async () => {
      ...
      expect(await act(`${c.entity}.cancel`, { id: c.invoice.id })).toMatchObject({
        docstatus: 2,
        cancelledDate: '2026-08-01',
      });
    });
  }
});
