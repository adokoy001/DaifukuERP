...
async function createPayment(ctx: Context, head: Row, lines?: Row[]): Promise<PaymentWithLines> {
  const created = (await runAction(ctx, 'payment.create', {
    ...head,
    ...(lines ? { lines: { payment_allocation: lines } } : {}),
  })) as PaymentWithLines;
...
async function postSalesInvoice(partnerId: string, amount: string, date = '2026-08-01'): Promise<InvoiceJson> {
  return run({}, async (ctx) => {
    const inv = (await runAction(ctx, 'sales_invoice.create', {
      partnerId,
      date,
      lines: { sales_invoice_line: [{ description: '商品', unitPrice: amount, taxCategory: 'exempt' }] },
    })) as InvoiceJson;
...
async function postBill(partnerId: string, amount: string, date = '2026-08-05'): Promise<InvoiceJson> {
  return run({}, async (ctx) => {
    const bill = (await runAction(ctx, 'purchase_invoice.create', {
      partnerId,
      date,
      priceIncludesTax: false,
      lines: {
        purchase_invoice_line: [
          { accountId: acc['5000'], description: '仕入', unitPrice: amount, taxCategory: 'exempt' },
        ],
      },
    })) as InvoiceJson;
...
const delta = (after: Record<string, string>, before: Record<string, string>, code: string) =>
  Decimal.from(after[code] ?? '0')
    .minus(before[code] ?? '0')
    .toString();
const trialBalance = () =>
  run({}, (ctx) =>
    runAction(ctx, 'accounting.trial_balance', { from: '2026-01-01', to: '2026-12-31' }),
  ) as Promise<Table>;
...
describe('payment module (docs/specs/payment.md)', () => {
  it('AC-1 payment + allocations: defaults (today JST, bank_transfer, accountId from payment.accounts by method), computed fields, get returns lines, validation', async () => {
    ...
    const p = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1000' }),
    );
    ...
    const cash = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'pay', partnerId: supplierS, amount: '500', method: 'cash', date: '2026-08-20' }),
    );
    expect(cash).toMatchObject({ accountId: acc['1000'], date: '2026-08-20', method: 'cash' });
    const explicit = await run({}, (ctx) =>
      createPayment(ctx, {
        direction: 'receive',
        partnerId: customerA,
        amount: '10',
    ...
    const withLines = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1000' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '600' },
      ]),
    );
    expect([withLines.allocatedAmount, withLines.unallocatedAmount]).toEqual(['600', '400']);
    expect(withLines.lines.payment_allocation.map((l) => [l.seq, l.invoiceEntity, l.invoiceId, l.amount])).toEqual([
      [1, 'sales_invoice', inv.id, '600'],
    ]);
    ...
    await expect(
      run({}, (ctx) =>
        runAction(ctx, 'payment.update', { id: withLines.id, patch: { allocatedAmount: '1', unallocatedAmount: '2' } }),
      ),
    ).rejects.toBeInstanceOf(PermissionDenied);
    const u = (await run({}, (ctx) =>
      runAction(ctx, 'payment.update', { id: withLines.id, patch: { amount: '800', method: 'cash', accountId: null } }),
    )) as PaymentJson;
    ...
    // validation
    await expect(run({}, (ctx) => createPayment(ctx, { partnerId: customerA, amount: '1' }))).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'direction' }] },
    });
    await expect(run({}, (ctx) => createPayment(ctx, { direction: 'receive', amount: '1' }))).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'partnerId' }] },
    });
    await expect(
      run({}, (ctx) => createPayment(ctx, { direction: 'receive', partnerId: customerA })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'amount' }] } });
    await expect(
      run({}, (ctx) => createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '0' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'amount', message: 'must be > 0' }] } });
    await expect(
      run({}, (ctx) => createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '-5' })),
    ).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'amount' }] } });
    await expect(
      run({}, (ctx) => runAction(ctx, 'payment.update', { id: p.id, patch: { amount: '0' } })),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
    const badMethod = await caught(
      run({}, (ctx) =>
        createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1', method: 'cheque' }),
      ),
    );
    ...
    await expect(
      run({}, (ctx) =>
        createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1', date: '2026-13-01' }),
      ),
    ...
    const noAcc = await caught(
      run({}, (ctx) => createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1' })),
    );
    ...
    expect(
      (
        await run({}, (ctx) =>
          createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1', accountId: acc['1100'] }),
        )
      ).accountId,
    ).toBe(acc['1100']);
    await run({}, (ctx) =>
  ...
  it('AC-2 allocation rules on save (entity/direction, existence, open, partner, balance, amount) and at submit (Σ <= amount, one line per invoice)', async () => {
    ...
    const draftInv = (await run({}, (ctx) =>
      runAction(ctx, 'sales_invoice.create', {
        partnerId: customerA,
        date: '2026-08-01',
        lines: { sales_invoice_line: [{ description: 'x', unitPrice: '10', taxCategory: 'exempt' }] },
      }),
    )) as InvoiceJson;
    const mk = (lines: Row[], head: Row = {}) =>
      run({}, (ctx) =>
        createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '2500', ...head }, lines),
      );
    ...
    await expectIssue(
      mk([line({ invoiceEntity: 'purchase_invoice', invoiceId: billS.id })]),
      'invoiceEntity',
      'must be sales_invoice for direction receive',
    );
    await expectIssue(mk([line({ invoiceId: newId() })]), 'invoiceId', 'does not exist or is not visible');
    await expectIssue(
      mk([line({ invoiceId: draftInv.id, amount: '10' })]),
      'invoiceId',
      'is not open (docstatus 0, status draft)',
    );
    await expectIssue(mk([line({ invoiceId: invB.id, amount: '700' })]), 'invoiceId', 'belongs to another partner');
    await expectIssue(
      mk([line({ amount: '3001' })], { amount: '5000' }),
      'amount',
      'must be <= the invoice balance 3000',
    );
    ...
    await expectIssue(
      mk([line({ invoiceEntity: 'sales_invoice', invoiceId: billS.id })], { direction: 'pay', partnerId: supplierS }),
      'invoiceEntity',
      'must be purchase_invoice for direction pay',
    );
    await expect(mk([line({ invoiceEntity: 'bill' })])).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'invoiceEntity' }] },
    });
    await expect(mk([line({ invoiceId: 'not-a-uuid' })])).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'invoiceId' }] },
    });
    ...
    const u = (await run({}, (ctx) =>
      runAction(ctx, 'payment.update', {
        id: p.id,
        patch: {
          amount: '1000',
          lines: { payment_allocation: [{ invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '1000' }] },
        },
      }),
    )) as PaymentWithLines;
    ...
    const direct = await run({}, (ctx) =>
      repo(ctx, PaymentAllocation).update(u.lines.payment_allocation[0]?.id ?? '', { amount: '250' }),
    );
    expect(direct.amount.toString()).toBe('250');
    expect((await run({}, (ctx) => repo(ctx, Payment).get(p.id))).unallocatedAmount.toString()).toBe('750');
    const added = await run({}, (ctx) =>
      repo(ctx, PaymentAllocation).create({
        paymentId: p.id,
    ...
    const over = (await run({}, (ctx) =>
      runAction(ctx, 'payment.update', { id: p.id, patch: { amount: '100' } }),
    )) as PaymentJson;
    ...
    expect(issues(e2)).toEqual([
      { path: 'lines.2.invoiceId', message: `sales_invoice ${inv.number} is allocated twice; merge the lines` },
    ]);
  ...
  it('AC-3 / AC-7 receive cycle: partial (sales role) then final with an advance (accounting) -> invoice paid; PAY-<year>-<n>; entries; frozen after submit', async () => {
    ...
    const draft1 = await run(sales, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1000', date: '2026-08-10' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '1000' },
      ]),
    );
    ...
    expect([
      (await salesInvoice(inv.id)).paidAmount.toString(),
      (await salesInvoice(inv.id)).balance.toString(),
    ]).toEqual(['1000', '2000']);
    const je1 = await run({}, (ctx) => repo(ctx, JournalEntry).get(p1.journalEntryId ?? ''));
    expect(je1).toMatchObject({
      docstatus: 1,
      sourceEntity: 'payment',
      sourceId: p1.id,
      date: '2026-08-10',
      description: `入金 ${p1.number} 得意先A`,
      reversalOf: null,
    });
    ...
    const draft2 = await run(accounting, (ctx) =>
      createPayment(
        ctx,
        { direction: 'receive', partnerId: customerA, amount: '2500', date: '2026-08-20', method: 'other' },
        [{ invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '2000' }],
      ),
    );
    ...
    expect([delta(after, before, '1100'), delta(after, before, '1300'), delta(after, before, '2400')]).toEqual([
      '3500',
      '-3000',
      '-500',
    ]);
    ...
    await expect(
      run({}, (ctx) => runAction(ctx, 'payment.update', { id: p1.id, patch: { lines: { payment_allocation: [] } } })),
    ...
    expect(frozen).toMatchObject({
      code: 'INVALID_STATE',
      hint: 'Use the owning module operation, or cancel and amend.',
    });
    ...
    await expect(
      run({}, (ctx) => runAction(ctx, 'payment.update', { id: p1.id, patch: { amount: '1' } })),
    ).rejects.toMatchObject({ code: 'INVALID_STATE', details: { blocked: ['amount'] } });
    await expect(run({}, (ctx) => repo(ctx, Payment).delete(p1.id))).rejects.toBeInstanceOf(StateError);
    await expect(
      run({}, (ctx) =>
        createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1' }, [
          { invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '1' },
        ]),
      ),
      ...
      details: {
        issues: [
          { path: 'invoiceId', message: expect.stringContaining('status paid') },
          { path: 'amount', message: 'must be <= the invoice balance 0' },
        ],
      },
    });
    ...
    const d = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '100' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: inv2.id, amount: '100' },
      ]),
    );
    await run({}, (ctx) =>
      setSetting(ctx, PAYMENT_ACCOUNTS_KEY, paymentAccountsSchema, {
    ...
    expect(err).toMatchObject({
      code: 'INVALID_STATE',
      details: { setting: PAYMENT_ACCOUNTS_KEY, missing: { advanceReceived: '2499' } },
    });
    await run({}, (ctx) =>
      setSetting(ctx, PAYMENT_ACCOUNTS_KEY, paymentAccountsSchema, {
        cash: '1000',
  ...
  it('AC-3 / AC-7 pay cycle (purchasing role): Dr 買掛金 / Dr 前払金 / Cr 現金 -> bill paid', async () => {
    ...
    const before = await netByCode();
    const draft = await run(purchasing, (ctx) =>
      createPayment(
        ctx,
        { direction: 'pay', partnerId: supplierS, amount: '6000', method: 'cash', date: '2026-08-25' },
        [{ invoiceEntity: 'purchase_invoice', invoiceId: b.id, amount: '5000' }],
      ),
    );
    ...
    const after = await netByCode();
    expect([delta(after, before, '2100'), delta(after, before, '1900'), delta(after, before, '1000')]).toEqual([
      '5000',
      '1000',
      '-6000',
    ]);
    // an advance with no allocation at all: Dr 前払金 / Cr 普通預金
    const adv = await run(purchasing, (ctx) =>
      createPayment(ctx, { direction: 'pay', partnerId: supplierS, amount: '300', date: '2026-08-26' }),
    );
  ...
  it('AC-4 cancel un-applies the allocations and reverses the entry (balances and trial balance restored); amend gives a clean draft', async () => {
    ...
    const d1 = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1000', date: '2026-08-11' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '1000' },
      ]),
    );
    ...
    const d2 = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '3500', date: '2026-08-12' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '3000' },
      ]),
    );
    ...
    expect(['1100', '1300', '2400'].map((c) => closing(tbPaid, c))).not.toEqual(
      ['1100', '1300', '2400'].map((c) => closing(tb1, c)),
    );
    // sales may not cancel (spec AC-6); accounting may
    await expect(run(sales, (ctx) => runAction(ctx, 'payment.cancel', { id: p2.id }))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
    ...
    expect(cancelled).toMatchObject({
      ...
      journalEntryId: p2.journalEntryId,
    });
    const reopened = await salesInvoice(inv.id);
    expect([reopened.paidAmount.toString(), reopened.balance.toString(), reopened.status]).toEqual([
      '1000',
      '3000',
      'open',
    ]);
    const reversals = await run({}, (ctx) =>
      repo(ctx, JournalEntry).list({ where: { reversalOf: p2.journalEntryId } }),
    );
    expect(reversals.items.map((e) => [e.docstatus, e.date, e.totalDebit.toString()])).toEqual([
      [1, '2026-08-12', '3500'],
    ]);
    ...
    const someDraft = await run({}, (c) =>
      createPayment(c, { direction: 'receive', partnerId: customerA, amount: '1' }),
    );
    await expect(run({}, (ctx) => runAction(ctx, 'payment.cancel', { id: someDraft.id }))).rejects.toBeInstanceOf(
      StateError,
    );
    ...
    expect(
      (await run({}, (ctx) => getPayment(ctx, amended.id))).lines.payment_allocation.map((l) => [
        l.invoiceId,
        l.amount,
      ]),
    ).toEqual([[inv.id, '3000']]);
    ...
    expect(
      ((await run({}, (ctx) => runAction(ctx, 'sales_invoice.cancel', { id: inv.id }))) as InvoiceJson).status,
    ).toBe('cancelled');
    // a cancelled payment is read-only; its allocations cannot be touched
    await expect(
      run({}, (ctx) => runAction(ctx, 'payment.update', { id: p1.id, patch: { note: 'x' } })),
    ).rejects.toBeInstanceOf(StateError);
    await expect(
      run({}, (ctx) => repo(ctx, PaymentAllocation).delete(d1.lines.payment_allocation[0]?.id ?? '')),
    ).rejects.toBeInstanceOf(StateError);
  });

  it('AC-5 payment.outstanding: open invoices with balances per direction (and partner) as a TableResult; role gates', async () => {
    const c = (await run({}, (ctx) => repo(ctx, Partner).create({ name: 'C工業', isCustomer: true, isSupplier: true })))
      .id;
    ...
    const half = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: c, amount: '1500', date: '2026-07-10' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: i2.id, amount: '1500' },
      ]),
    );
    await run({}, (ctx) => submitPayment(ctx, half.id));
    const full = await run({}, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: c, amount: '500', date: '2026-07-11' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: i3.id, amount: '500' },
      ]),
    );
    ...
    await run({}, (ctx) =>
      runAction(ctx, 'sales_invoice.create', {
        partnerId: c,
        date: '2026-07-05',
        lines: { sales_invoice_line: [{ description: 'draft', unitPrice: '9', taxCategory: 'exempt' }] },
      }),
    );
    const t = (await run(viewer, (ctx) =>
      runAction(ctx, 'payment.outstanding', { direction: 'receive', partnerId: c }),
    )) as Table;
    ...
    const all = (await run(accounting, (ctx) =>
      runAction(ctx, 'payment.outstanding', { direction: 'receive' }),
    )) as Table;
    ...
    const bills = (await run(purchasing, (ctx) =>
      runAction(ctx, 'payment.outstanding', { direction: 'pay', partnerId: c }),
    )) as Table;
    ...
    await expect(
      run(sales, (ctx) => runAction(ctx, 'payment.outstanding', { direction: 'pay' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run(purchasing, (ctx) => runAction(ctx, 'payment.outstanding', { direction: 'receive' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run(asRole(['nobody']), (ctx) => runAction(ctx, 'payment.outstanding', { direction: 'receive' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run({}, (ctx) => runAction(ctx, 'payment.outstanding', {}))).rejects.toBeInstanceOf(ValidationError);
    await expect(run({}, (ctx) => runAction(ctx, 'payment.outstanding', { direction: 'in' }))).rejects.toBeInstanceOf(
      ValidationError,
    );
  });

  it('AC-6 permissions: accounting all; sales receive only; purchasing pay only (hook + row rules); viewer read', async () => {
    const ops = (roles: string[], entity: string) =>
      run(asRole(roles), async (ctx) => appMeta(ctx).entities.find((e) => e.name === entity)?.ops);
    expect(await ops(['accounting'], 'payment')).toEqual([
      'read',
      'create',
      'update',
      'delete',
    ...
    const salesPay = await caught(
      run(sales, (ctx) => createPayment(ctx, { direction: 'pay', partnerId: supplierS, amount: '1' })),
    );
    expect(salesPay).toBeInstanceOf(PermissionDenied);
    expect(salesPay).toMatchObject({
      code: 'PERMISSION_DENIED',
      details: { entity: 'payment', op: 'create:pay', roles: ['sales'] },
    });
    await expect(
      run(purchasing, (ctx) => createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1' })),
    ).rejects.toMatchObject({ code: 'PERMISSION_DENIED', details: { op: 'create:receive' } });
    const receive = await run(sales, (ctx) =>
      createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '100' }, [
        { invoiceEntity: 'sales_invoice', invoiceId: inv.id, amount: '100' },
      ]),
    );
    await expect(
      run(sales, (ctx) => runAction(ctx, 'payment.update', { id: receive.id, patch: { direction: 'pay' } })),
    ).rejects.toMatchObject({ code: 'PERMISSION_DENIED', details: { op: 'update:pay' } });
    ...
    const pay = await run(accounting, (ctx) =>
      createPayment(ctx, { direction: 'pay', partnerId: supplierS, amount: '100' }, [
        { invoiceEntity: 'purchase_invoice', invoiceId: b.id, amount: '100' },
      ]),
    );
    await expect(run(sales, (ctx) => getPayment(ctx, pay.id))).rejects.toMatchObject({ code: 'NOT_FOUND' });
    await expect(
      run(sales, (ctx) => runAction(ctx, 'payment.update', { id: pay.id, patch: { note: 'x' } })),
    ...
    const listed = async (params: Partial<ContextParams>) =>
      (await run(params, (ctx) => repo(ctx, Payment).list({ where: { id: { $in: [receive.id, pay.id] } } }))).items
        .map((p) => p.direction)
        .sort();
    ...
    const lines = async (params: Partial<ContextParams>) =>
      (
        await run(params, (ctx) =>
          repo(ctx, PaymentAllocation).list({ where: { paymentId: { $in: [receive.id, pay.id] } } }),
        )
      ).items
        .map((l) => l.invoiceEntity)
        .sort();
    ...
    await expect(
      run(viewer, (ctx) => createPayment(ctx, { direction: 'receive', partnerId: customerA, amount: '1' })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(
      run(viewer, (ctx) => runAction(ctx, 'payment.update', { id: receive.id, patch: { note: 'x' } })),
    ).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run(viewer, (ctx) => submitPayment(ctx, receive.id))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(run(asRole(['nobody']), (ctx) => getPayment(ctx, receive.id))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
    ...
    for (const role of [sales, purchasing]) {
      await expect(run(role, (ctx) => runAction(ctx, 'payment.cancel', { id: p.id }))).rejects.toBeInstanceOf(
        PermissionDenied,
      );
      await expect(run(role, (ctx) => runAction(ctx, 'payment.amend', { id: p.id }))).rejects.toBeInstanceOf(
        PermissionDenied,
      );
    ...
    const draft = await run(purchasing, (ctx) =>
      createPayment(ctx, { direction: 'pay', partnerId: supplierS, amount: '5' }, [
        { invoiceEntity: 'purchase_invoice', invoiceId: b.id, amount: '5' },
      ]),
    );
    await expect(run(purchasing, (ctx) => repo(ctx, Payment).delete(draft.id))).rejects.toBeInstanceOf(
      PermissionDenied,
    );
  ...
  it('AC-7 trial balance consistency: the ledger balances and the AR/AP/advance accounts equal the open documents', async () => {
    ...
    const sumBalance = async (entity: typeof SalesInvoice | typeof PurchaseInvoice) =>
      run({}, async (ctx) => {
        const rows = await repo(ctx, entity).aggregate({
          where: { docstatus: DOCSTATUS.submitted, status: { $in: ['open', 'paid'] } },
          metrics: { balance: { sum: 'balance' } },
        });
        return (rows[0]?.balance as Decimal).toString();
      });
    const sumUnallocated = async (direction: 'receive' | 'pay') =>
      run({}, async (ctx) => {
        const rows = await repo(ctx, Payment).aggregate({
          where: { docstatus: DOCSTATUS.submitted, direction },
          metrics: { unallocated: { sum: 'unallocatedAmount' } },
        });
        return (rows[0]?.unallocated as Decimal).toString();
      });
    expect(closing(t, '1300')).toBe(await sumBalance(SalesInvoice));
    expect(closing(t, '2100')).toBe(
      Decimal.from(await sumBalance(PurchaseInvoice))
        .neg()
        .toString(),
    );
    expect(closing(t, '2400')).toBe(
      Decimal.from(await sumUnallocated('receive'))
        .neg()
        .toString(),
    );
