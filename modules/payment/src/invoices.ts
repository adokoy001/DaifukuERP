...
export async function loadInvoice(ctx: Context, entity: InvoiceEntity, id: string): Promise<InvoiceSnapshot | null> {
  const row =
    entity === 'sales_invoice' ? await repo(ctx, SalesInvoice).find(id) : await repo(ctx, PurchaseInvoice).find(id);
...
export async function loadInvoices(
  ...
  if (lock) {
    for (const id of unique) {
      ...
      if (!visible) continue;
      const row =
        entity === 'sales_invoice' ? await repo(ctx, SalesInvoice).lock(id) : await repo(ctx, PurchaseInvoice).lock(id);
  ...
  for (let i = 0; i < unique.length; i += 200) {
    ...
    const rows: (SalesRow | PurchaseRow)[] =
      entity === 'sales_invoice'
        ? (await repo(ctx, SalesInvoice).list({ where, limit: 200 })).items
        : (await repo(ctx, PurchaseInvoice).list({ where, limit: 200 })).items;
...
export async function listOpenInvoices(
  ...
  for (;;) {
    const page =
      entity === 'sales_invoice'
        ? await repo(ctx, SalesInvoice).list({ where, orderBy, limit: 500, offset })
        : await repo(ctx, PurchaseInvoice).list({ where, orderBy, limit: 500, offset });
    for (const r of page.items as (SalesRow | PurchaseRow)[])
      items.push({ ...snapshotOf(entity, r), date: r.date, dueDate: r.dueDate });
...
export async function applyInvoicePayment(ctx: Context, input: ApplyInput): Promise<InvoiceSnapshot> {
  if (input.entity === 'sales_invoice')
    return snapshotOf(
      input.entity,
      await applySalesPayment(ctx, { invoiceId: input.invoiceId, amount: input.amount, date: input.date }),
    );
  return snapshotOf(
    input.entity,
    await applyPurchasePayment(ctx, { invoiceId: input.invoiceId, amount: input.amount, date: input.date }),
  );
}
