...
async function beforeCancel(ctx: Context, { row, correctionDate }: HookArgs): Promise<void> {
  ...
  if (date < String(row.date))
    throw new ValidationError('Cancellation cannot precede the payment', [
      { path: 'correctionDate', message: 'must be on or after the original date' },
    ]);
  row.cancelledDate = date;
  for (const l of (await loadAllocationLines(ctx, id)).sort((a, b) => a.invoiceId.localeCompare(b.invoiceId)))
    await applyInvoicePayment(ctx, { entity: l.invoiceEntity, invoiceId: l.invoiceId, amount: l.amount.neg(), date });
