...
export const IndustryOperationsModule = defineModule({
  name: 'industry_operations', label: label('業界の履行案件基盤', 'Industry fulfillment engine'),
  depends: [PartnerModule.name, ProductModule.name, TaxModule.name, SalesModule.name, AccountingModule.name, PaymentModule.name, InventoryModule.name],
  entities: [], actions: [],
});
