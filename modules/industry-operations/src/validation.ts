...
export function amount(value: unknown, path: string): Decimal {
  ...
  if (typeof value !== 'string' || !Decimal.isDecimalString(value)) invalid(path, '有効な小数文字列を入力してください。');
  return Decimal.from(value);
}
export function requiredText(row: Record<string, unknown>, field: string): void {
  if (typeof row[field] !== 'string' || !String(row[field]).trim()) invalid(field, `${field}: 証跡・内容を入力してください。`);
}
...
export function businessDate(value: unknown, field: string): LocalDate {
  if (typeof value !== 'string' || !isLocalDate(value)) invalid(field, `${field}: YYYY-MM-DD形式の有効日を入力してください。`);
  return value;
}
