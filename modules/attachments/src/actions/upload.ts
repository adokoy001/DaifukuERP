...
export const uploadFieldsSchema = z.object(Object.fromEntries(UPLOAD_FIELDS.map((k) => [k, optional(k)])) as Record<(typeof UPLOAD_FIELDS)[number], z.ZodType>).strict();
...
function parseFields(fields: UploadInput['fields']): z.output<typeof uploadFieldsSchema> {
  ...
  const issues = r.error.issues.flatMap((i) =>
    i.code === 'unrecognized_keys' ? i.keys.map((k) => ({ path: k, message: 'unknown field' })) : [{ path: i.path.map(String).join('.'), message: i.message }],
  );
  throw new ValidationError('invalid upload fields', issues, `Allowed fields: ${UPLOAD_FIELDS.join(', ')} (unknown fields are rejected); dates are YYYY-MM-DD, amount a decimal string.`);
}
...
export async function uploadAttachment(ctx: Context, input: UploadInput): Promise<AttachmentRow> {
  ...
  if (stored.sha256 !== sha256) {
    throw new ValidationError('stored content hash does not match the uploaded bytes', [{ path: 'file', message: 'hash mismatch' }], 'Retry the upload; if it persists this is a storage bug.');
  }
  ...
  const insert = { storageKey: stored.key, filename: meta.filename, contentType: meta.contentType, size: stored.size, sha256, ...fields } as InsertInput<typeof Attachment>;
