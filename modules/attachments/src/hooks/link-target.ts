...
export async function assertLinkTarget(ctx: Context, entity: string, recordId: string): Promise<void> {
  if (!registry.hasEntity(entity)) {
    throw new ValidationError(`entity "${entity}" does not exist`, [{ path: 'entity', message: 'unknown entity' }], 'List entities with GET /meta and pass one of their names as `entity`.');
  }
...
export function assertLinkPair(entity: unknown, recordId: unknown): void {
  ...
  if (hasEntity !== hasId) {
    throw new ValidationError('linkedEntity and linkedId must be given together', [{ path: hasEntity ? 'linkedId' : 'linkedEntity', message: 'missing' }], 'Pass both linkedEntity and linkedId, or neither.');
  }
}
