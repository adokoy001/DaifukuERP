...
function canonicalAmount(path: string, s: string | undefined): string | undefined {
  ...
  try {
    ...
    throw new ValidationError(`${path} is not a decimal`, [{ path, message: 'expected a decimal string like "1234.50"' }]);
  }
}
...
export function buildSearchDomain(q: SearchCriteria): Domain | undefined {
  if (q.txnDateFrom !== undefined && q.txnDateTo !== undefined && q.txnDateFrom > q.txnDateTo) {
    throw new ValidationError('txnDateFrom is after txnDateTo', [{ path: 'txnDateFrom', message: 'must be <= txnDateTo' }]);
  }
  ...
  if (amountFrom !== undefined && amountTo !== undefined && Decimal.from(amountFrom).gt(amountTo)) {
    throw new ValidationError('amountFrom is greater than amountTo', [{ path: 'amountFrom', message: 'must be <= amountTo' }]);
  }
