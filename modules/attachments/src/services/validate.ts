...
export const ALLOWED_CONTENT_TYPES: ReadonlySet<string> = new Set(['application/pdf', 'image/png', 'image/jpeg', 'text/csv', 'application/xml', 'text/xml', 'text/plain']);
...
export function validateUpload(input: UploadMeta): UploadMeta {
  ...
  if (filename.length > MAX_FILENAME_LENGTH) issues.push({ path: 'file.filename', message: `filename must be at most ${MAX_FILENAME_LENGTH} characters` });
  if (!ALLOWED_CONTENT_TYPES.has(contentType)) issues.push({ path: 'file.contentType', message: `content type "${contentType}" is not allowed` });
  if (!Number.isInteger(input.size) || input.size <= 0) issues.push({ path: 'file', message: 'file is empty' });
  if (input.size > MAX_UPLOAD_BYTES) issues.push({ path: 'file', message: `file exceeds ${MAX_UPLOAD_BYTES} bytes (20 MB)` });
  if (issues.length > 0) {
    throw new ValidationError('invalid upload', issues, `Allowed content types: ${[...ALLOWED_CONTENT_TYPES].join(', ')}. Max size 20 MB. Fix the file and retry.`);
  }
...
export function assertSupersedable(old: SupersedeRow, replacement: SupersedeRow): void {
  if (old.id === replacement.id) {
    throw new ValidationError('an attachment cannot supersede itself', [{ path: 'newAttachmentId', message: 'must differ from id' }], 'Upload the corrected file first, then pass its id as newAttachmentId.');
  }
  if (old.supersededById !== null) {
    throw new StateError(`attachment ${old.id} is already superseded by ${old.supersededById}`, 'Supersede history is append-only. Supersede the current version (details.supersededById) instead.', { id: old.id, supersededById: old.supersededById });
  }
  if (replacement.supersededById !== null) {
    throw new StateError(`attachment ${replacement.id} is itself superseded by ${replacement.supersededById}`, 'Use the current version of the replacement as newAttachmentId.', { newAttachmentId: replacement.id, supersededById: replacement.supersededById });
  }
}
