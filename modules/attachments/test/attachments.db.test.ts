import { auditTrail, Conflict, configureStorage, LocalStorage, NotFound, PermissionDenied, StateError, ValidationError, appMeta, newId, registerCrudActions, repo, runAction, systemParams, withContext, type Context } from '@daifuku/kernel';
...
function upload(ctx: Context, fields: Record<string, string | undefined> = {}, opts: { content?: string; filename?: string; contentType?: string } = {}): Promise<AttachmentRow> {
  seq += 1;
...
describe('attachment entity (docs/specs/attachments.md)', () => {
  it('AC-1 fields, defaults, immutability and the module manifest', async () => {
    ...
    expect(AttachmentsModule.actions?.map((a) => a.name)).toEqual(['attachment.search', 'attachment.supersede', 'attachment.link', 'attachment.for_record']);
    expect(Attachment.fieldNames).toEqual(['storageKey', 'filename', 'contentType', 'size', 'sha256', 'kind', 'txnDate', 'amount', 'partnerId', 'linkedEntity', 'linkedId', 'note', 'supersededById']);
    const a = await db.run({}, (ctx) => upload(ctx));
    expect(a).toMatchObject({ kind: 'other', txnDate: null, amount: null, partnerId: null, linkedEntity: null, linkedId: null, note: null, supersededById: null, version: 1 });
    ...
    await expect(db.run({}, (ctx) => repo(ctx, Attachment).create({ storageKey: 'k', filename: 'f', contentType: 'text/plain', size: 1, sha256: 'not-hex' }))).rejects.toMatchObject({
  ...
  it('AC-1 permissions: accounting/sales/purchasing read+create+update, viewer read, nobody denied; delete is denied to every role and refused for admin', async () => {
    ...
    for (const role of ['accounting', 'sales', 'purchasing']) {
      ...
      const updated = await db.run(asRole([role]), (ctx) => repo(ctx, Attachment).update(own.id, { note: `by ${role}` }));
      expect(updated.version).toBe(2);
      await expect(db.run(asRole([role]), (ctx) => repo(ctx, Attachment).delete(own.id))).rejects.toBeInstanceOf(PermissionDenied);
      const ops = await db.run(asRole([role]), async (ctx) => appMeta(ctx).entities.find((e) => e.name === 'attachment')?.ops);
    ...
    await expect(db.run(viewer, (ctx) => repo(ctx, Attachment).update(target.id, { note: 'x' }))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(db.run(asRole(['nobody']), (ctx) => repo(ctx, Attachment).list())).rejects.toBeInstanceOf(PermissionDenied);
    // admin implicitly holds every op, so the before_delete hook is what keeps evidence undeletable (電帳法)
    const err = await db.run({}, (ctx) => runAction(ctx, 'attachment.delete', { id: target.id })).catch((e: unknown) => e);
  ...
  it('AC-2 uploadAttachment stores the bytes through ctx.storage, records size/sha256/normalised type and the metadata fields', async () => {
    const a = await db.run({}, (ctx) =>
      upload(ctx, { kind: 'receipt', txnDate: '2026-04-01', amount: '1234.50', partnerId: partnerC, note: ' タクシー代 ', linkedEntity: '', linkedId: '' }, { content: '領収書', filename: 'up/領収書.PDF', contentType: 'Application/PDF; charset=binary' }),
    );
    expect(a).toMatchObject({ filename: '領収書.PDF', contentType: 'application/pdf', size: bytes('領収書').byteLength, kind: 'receipt', txnDate: '2026-04-01', partnerId: partnerC, note: 'タクシー代', linkedEntity: null });
    ...
    const stored = await db.run({}, (ctx) => ctx.storage.put(ctx.tenantId, bytes('領収書'), { filename: 'x', contentType: 'x' }));
  ...
  it('AC-2 rejects bad content types, empty files, invalid fields and invisible refs before anything is stored', async () => {
    ...
    const cases: { fields?: Record<string, string>; opts?: { content?: string; contentType?: string }; error: new (...args: never[]) => unknown; path?: string }[] = [
  ...
  it('AC-7 the same bytes twice in one company is a Conflict naming the existing id; another company may store them', async () => {
    ...
    const err = await db.run({}, (ctx) => upload(ctx, {}, { content: 'dup-bytes', filename: 'other-name.pdf' })).catch((e: unknown) => e);
    ...
    const viaCreate = await db.run({}, (ctx) => runAction(ctx, 'attachment.create', { storageKey: 'k/k', filename: 'f', contentType: 'text/plain', size: 9, sha256: first.sha256 })).catch((e: unknown) => e);
    ...
    const second = await withContext(db.app, systemParams(db.tenantId, otherCompany), (ctx) => upload(ctx, {}, { content: 'dup-bytes' }));
...
describe('attachment.search (AC-4)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const [key, fields] of rows) ids[key] = (await db.run({}, (ctx) => upload(ctx, fields, { content: `search ${key}` }))).id;
  });

  const search = async (input: Record<string, unknown>, roles?: string[]) => {
    const res = (await db.run(roles ? asRole(roles) : {}, (ctx) => runAction(ctx, 'attachment.search', input))) as { items: { id: string; amount: string | null }[]; total: number };
  ...
  it('filters by date range, amount range, partner and kind, and by their combination', async () => {
    expect((await search({ txnDateFrom: '2026-02-01', txnDateTo: '2026-02-28' })).keys.sort()).toEqual(['feb-a-2500', 'feb-b-2500']);
    ...
    expect((await search({ amountFrom: '2500', amountTo: '2500.00' })).keys.sort()).toEqual(['feb-a-2500', 'feb-b-2500']);
    ...
    expect((await search({ txnDateFrom: '2026-01-01', txnDateTo: '2026-12-31', amountFrom: '1000', amountTo: '3000', partnerId: partnerA, kind: 'receipt' })).keys).toEqual(['feb-a-2500']);
  ...
  it('orders by txnDate desc and paginates; validates input; requires attachment read', async () => {
    ...
    await expect(search({ txnDateFrom: '2026-02-01', txnDateTo: '2026-01-01' })).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'txnDateFrom' }] } });
    await expect(search({ amountFrom: 'x' })).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'amountFrom' }] } });
    await expect(search({ kind: 'nope' })).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'kind' }] } });
...
describe('attachment.supersede (AC-5, AC-8)', () => {
  it('records the replacement and the reason in the audit trail; the old file stays retrievable; history is write-once and acyclic', async () => {
    ...
    const old = (await db.run(acc, (ctx) => runAction(ctx, 'attachment.supersede', { id: v1.id, newAttachmentId: v2.id, reason: '金額誤り' }))) as { supersededById: string; version: number };
    ...
    expect(trail.find((e) => e.op === 'supersede')).toMatchObject({ action: 'attachment.supersede', actorType: 'user', actorId: acc.actor.id, before: { supersededById: null }, after: { supersededById: v2.id, reason: '金額誤り' } });
    expect(trail.find((e) => e.op === 'update')).toMatchObject({ before: { supersededById: null }, after: { supersededById: v2.id } });
    ...
    await expect(db.run(acc, (ctx) => runAction(ctx, 'attachment.supersede', { id: v1.id, newAttachmentId: v3.id, reason: 'again' }))).rejects.toBeInstanceOf(StateError);
    await expect(db.run(acc, (ctx) => runAction(ctx, 'attachment.supersede', { id: v3.id, newAttachmentId: v3.id, reason: 'self' }))).rejects.toBeInstanceOf(ValidationError);
    await expect(db.run(acc, (ctx) => runAction(ctx, 'attachment.supersede', { id: v3.id, newAttachmentId: v1.id, reason: 'cycle' }))).rejects.toBeInstanceOf(StateError);
    await expect(db.run(acc, (ctx) => runAction(ctx, 'attachment.supersede', { id: v2.id, newAttachmentId: newId(), reason: 'ghost' }))).rejects.toBeInstanceOf(NotFound);
    await expect(db.run(acc, (ctx) => runAction(ctx, 'attachment.supersede', { id: v2.id, newAttachmentId: v3.id, reason: '  ' }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(db.run({}, (ctx) => repo(ctx, Attachment).update(v1.id, { supersededById: null }))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(db.run({}, (ctx) => repo(ctx, Attachment).update(v1.id, { supersededById: v3.id }))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(db.run({}, (ctx) => repo(ctx, Attachment).create({ storageKey: 'k/s', filename: 'f', contentType: 'text/plain', size: 1, sha256: 'b'.repeat(64), supersededById: v3.id }))).rejects.toBeInstanceOf(PermissionDenied);
    // chain: v2 -> v3 is fine (v3 is current); viewer may not supersede
    expect(((await db.run(acc, (ctx) => runAction(ctx, 'attachment.supersede', { id: v2.id, newAttachmentId: v3.id, reason: 'v3' }))) as { supersededById: string }).supersededById).toBe(v3.id);
    await expect(db.run(asRole(['viewer']), (ctx) => runAction(ctx, 'attachment.supersede', { id: v3.id, newAttachmentId: v2.id, reason: 'x' }))).rejects.toBeInstanceOf(PermissionDenied);
  });
});
...
describe('attachment.link / for_record (AC-6)', () => {
  it('links to a visible record of a registered entity and lists attachments of that record', async () => {
    ...
    const linked = (await db.run(asRole(['sales']), (ctx) => runAction(ctx, 'attachment.link', { id: a.id, entity: 'partner', recordId: partnerA }))) as { linkedEntity: string; linkedId: string };
    ...
    const list = (await db.run(asRole(['viewer']), (ctx) => runAction(ctx, 'attachment.for_record', { entity: 'partner', recordId: partnerA }))) as { items: { id: string }[]; total: number };
    expect(list.total).toBe(2);
    expect(list.items.map((i) => i.id)).toEqual([b.id, a.id]); // newest first
    expect(((await db.run({}, (ctx) => runAction(ctx, 'attachment.for_record', { entity: 'partner', recordId: partnerB }))) as { total: number }).total).toBe(0);
    // an attachment can be re-linked, and linked to an attachment (any registered entity)
    const relinked = (await db.run({}, (ctx) => runAction(ctx, 'attachment.link', { id: a.id, entity: 'attachment', recordId: b.id }))) as { linkedEntity: string };
  ...
  it('rejects unknown entities, invisible records and callers without update/read', async () => {
    ...
    for (const name of ['attachment.link', 'attachment.for_record']) {
      const input = name === 'attachment.link' ? { id: a.id, entity: 'nope', recordId: partnerA } : { entity: 'nope', recordId: partnerA };
      await expect(db.run({}, (ctx) => runAction(ctx, name, input))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'entity' }] } });
    }
    await expect(db.run({}, (ctx) => runAction(ctx, 'attachment.link', { id: a.id, entity: 'partner', recordId: newId() }))).rejects.toBeInstanceOf(NotFound);
    await expect(db.run({}, (ctx) => runAction(ctx, 'attachment.for_record', { entity: 'partner', recordId: newId() }))).rejects.toBeInstanceOf(NotFound);
    await expect(db.run({}, (ctx) => runAction(ctx, 'attachment.link', { id: newId(), entity: 'partner', recordId: partnerA }))).rejects.toBeInstanceOf(NotFound);
    await expect(db.run(asRole(['viewer']), (ctx) => runAction(ctx, 'attachment.link', { id: a.id, entity: 'partner', recordId: partnerA }))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(db.run(asRole(['nobody']), (ctx) => runAction(ctx, 'attachment.for_record', { entity: 'partner', recordId: partnerA }))).rejects.toBeInstanceOf(PermissionDenied);
    // the generic update path is guarded by the same hook
    await expect(db.run({}, (ctx) => repo(ctx, Attachment).update(a.id, { linkedEntity: 'partner' }))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: 'linkedId' }] } });
    await expect(db.run({}, (ctx) => repo(ctx, Attachment).update(a.id, { linkedEntity: 'partner', linkedId: newId() }))).rejects.toBeInstanceOf(NotFound);
  });
});
