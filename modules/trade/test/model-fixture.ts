...
export const quantity = (quarters: number): string =>
  quarters < 0 ? '-' + quantity(-quarters) : `${Math.floor(quarters / 4)}${['', '.25', '.5', '.75'][quarters % 4]}`;
export async function tradeModelFixture() {
  ...
  const warehouse = await run(async (ctx) => {
    ...
    for (const [code, type] of [
      ...
      accounts.set(
        code,
        (await repo(ctx, Account).create({ code, name: code, type, partnerRequired: ['1300', '2100'].includes(code) }))
          .id,
      );
    }
  ...
  async function order(direction: 'sales' | 'purchase', quarters: number) {
    const values = await run(async (ctx) => ({
      partner: await repo(ctx, Partner).create({
        name: 'Generated trade ' + newId(),
        isCustomer: true,
        isSupplier: true,
      }),
      product: await repo(ctx, Product).create({
        code: newId().slice(-24),
        name: 'Generated quarter units',
        salePrice: '1200',
        purchasePrice: '800',
      }),
    }));
    if (direction === 'sales') {
      const stock = await action('stock_entry.create', {
        type: 'receipt',
        date: DAY,
        warehouseId: warehouse,
        lines: { stock_entry_line: [{ productId: values.product.id, quantity: '100', unitCost: '800' }] },
      });
      await action('stock_entry.submit', { id: stock.id });
    }
    const created = await action('trade_order.create', {
      direction,
        ...
        trade_order_line: [
          {
            ...
            unitPrice: direction === 'sales' ? '1200' : '800',
            taxCategory: 'exempt',
          },
        ],
      },
  ...
  async function projection(order: Awaited<ReturnType<typeof order>>) {
    return run(async (ctx) => {
      const stock = await repo(ctx, StockBalance).list({
        where: { productId: order.product.id, warehouseId: warehouse },
      });
      const ledger = await repo(ctx, StockLedger).list({
      ...
      const lines = await repo(ctx, JournalLine).list({
        where: {
          ...
          accountId: required(accounts.get(order.direction === 'sales' ? '1300' : '2100')),
          posted: true,
        },
        limit: 500,
      });
      const net = lines.items.reduce(
        (sum, line) => sum + BigInt(line.debit.toString()) - BigInt(line.credit.toString()),
        0n,
      );
      return {
        quantity: stock.items[0]?.qty.toString() ?? '0',
        cost: stock.items[0]?.value.toString() ?? '0',
        movements: ledger.items.map((line) => ({
          qty: line.qtyDelta.toString(),
          cost: line.costDelta.toString(),
          reverse: line.reversal,
        })),
        journalLineCount: lines.total,
        net: String(order.direction === 'sales' ? net : -net),
      };
    });
  }
...
export function required<T>(value: T | undefined | null): T {
  if (value === null || value === undefined) throw new Error('Missing generated fixture value');
  return value;
}
