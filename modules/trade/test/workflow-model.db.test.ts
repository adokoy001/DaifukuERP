...
type Model = {
  side: number;
  movements: { quarters: number; reverse: boolean }[];
  ordered: number;
  closed: boolean;
...
const ops = fc.array(
  fc.record({
    ...
    quarters: fc.integer({ min: 1, max: 48 }),
    pick: fc.nat(8),
  }),
  { minLength: 3, maxLength: 10 },
);
const op = (kind: Kind, quarters = 4, pick = 0): Op => ({ kind, quarters, pick });
const totals = (m: Model) => ({
  fulfilled: m.fulfillments.filter((v) => v.active).reduce((n, v) => n + v.quantity, 0),
  billed: m.fulfillments
    .flatMap((v) => v.bills)
    .filter((v) => v.active)
...
async function verify(m: Model, order: Awaited<ReturnType<TradeFixture['order']>>) {
  ...
  expect(d.lines[0]).toMatchObject({
    fulfilledQuantity: quantity(fulfilled),
    billedQuantity: quantity(billed),
    remainingQuantity: quantity(order.quarters - fulfilled),
    unbilledQuantity: quantity(fulfilled - billed),
  });
  ...
  expect(p.quantity).toBe(quantity(stockQuarters));
  expect(p.cost).toBe(String(BigInt(stockQuarters) * 200n));
  ...
  expect(p.movements).toEqual(
    m.movements.map((v) => ({
      qty: quantity(v.quarters),
      cost: String(BigInt(v.quarters) * 200n),
      reverse: v.reverse,
    })),
  );
  ...
  for (const fm of m.fulfillments) {
    ...
    expect(actual.docstatus).toBe(fm.active ? 1 : 2);
    expect(actual.billings).toHaveLength(fm.bills.length);
...
async function attempt(
  ...
  if (valid) {
    const result = await f.action(name, input);
    update(result.id);
    m.success++;
  } else {
    await expect(f.action(name, input)).rejects.toMatchObject({ code });
    m.rejected++;
  }
}
async function fulfill(m: Model, step: Op, d: TradeOrderDetail) {
  const input = {
    ...
    requestId: newId(),
    lines: [{ orderLineId: required(d.lines[0]).id, quantity: quantity(step.quarters) }],
  };
  const valid = !m.closed && totals(m).fulfilled + step.quarters <= m.ordered;
  await attempt(m, valid, 'trade.fulfill_order', input, (id) => {
    m.fulfillments.push({ id, quantity: step.quarters, active: true, input, bills: [] });
    m.movements.push({ quarters: m.side * step.quarters, reverse: false });
  });
}
async function bill(m: Model, step: Op, d: TradeOrderDetail) {
  ...
  const input = {
    ...
    date: DAY,
    requestId: newId(),
    lines: [{ fulfillmentLineId: actual?.lines[0]?.id ?? newId(), quantity: quantity(step.quarters) }],
  };
  const used = target?.bills.filter((v) => v.active).reduce((n, v) => n + v.quantity, 0) ?? 0;
  await attempt(m, !!target?.active && used + step.quarters <= target.quantity, 'trade.bill_fulfillment', input, (id) =>
    required(target).bills.push({ id, quantity: step.quarters, active: true, input }),
  );
}
async function cancel(m: Model, step: Op, d: TradeOrderDetail) {
  if (step.kind === 'cancelBill') {
    const target = pick(
      m.fulfillments.flatMap((v) => v.bills),
      step.pick,
    );
    ...
    await attempt(
      m,
      !!target?.active,
      'trade.cancel_billing',
      {
        billingId: target?.id ?? newId(),
        expectedVersion: actual?.version ?? 1,
        correctionDate: DAY,
        reason: 'Generated correction',
      },
      () => {
        required(target).active = false;
      },
    );
  } else {
    const target = pick(m.fulfillments, step.pick);
    ...
    await attempt(
      ...
      () => {
        required(target).active = false;
        m.movements.push({ quarters: -m.side * required(target).quantity, reverse: true });
      },
      target?.active && target.bills.some((v) => v.active) ? 'HAS_DEPENDENTS' : 'INVALID_STATE',
    );
  }
}
async function retry(m: Model, step: Op) {
  const target =
    step.kind === 'retryFulfill'
      ? pick(m.fulfillments, step.pick)
      : pick(
          m.fulfillments.flatMap((v) => v.bills),
          step.pick,
        );
  ...
  await attempt(
    ...
    step.kind === 'retryFulfill' ? 'trade.fulfill_order' : 'trade.bill_fulfillment',
    required(target).input,
    (id) => {
      expect(id).toBe(required(target).id);
      m.retries++;
    },
  );
}
async function execute(m: Model, step: Op, order: Awaited<ReturnType<TradeFixture['order']>>) {
  ...
  switch (step.kind) {
    case 'fulfill':
      ...
      break;
    case 'bill':
      await bill(m, step, before.detail);
      break;
    case 'cancelBill':
    case 'cancelFulfillment':
      await cancel(m, step, before.detail);
      break;
    case 'retryBill':
    case 'retryFulfill':
      await retry(m, step);
    ...
    case 'close':
      await attempt(
        ...
        !m.closed,
        'trade.close_order',
        { orderId: order.id, expectedVersion: before.detail.order.version, reason: 'Generated close' },
        () => {
          m.closed = true;
        },
      );
  }
  if (m.rejected > rejected)
    expect({ detail: await f.detail(order.id), effects: await f.projection(order) }).toEqual(before);
  await verify(m, order);
}
describe('AC-2 / TRADE-QTY-01 generated operation traces', () => {
  it.each(['sales', 'purchase'] as const)(
    '%s compares every transition, rejection and replay with an independent quantity/history model',
    async (direction) => {
      await fc.assert(
        fc.asyncProperty(fc.integer({ min: 12, max: 48 }), ops, async (quarters, tail) => {
          ...
          expect(model.success).toBeGreaterThanOrEqual(9);
          expect(model.rejected).toBeGreaterThanOrEqual(3);
          expect(model.retries).toBeGreaterThanOrEqual(2);
        }),
        {
          seed: Number(process.env['PBT_SEED'] ?? 730202),
          numRuns: Number(process.env['PBT_RUNS'] ?? 6),
