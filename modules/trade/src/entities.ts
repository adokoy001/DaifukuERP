...
export const TradeQuotation = defineDocument({
  ...
  permissions: {
    roles: { sales: ['read', 'create', 'update', 'delete', 'submit', 'cancel', 'amend'], viewer: ['read'] },
  },
...
export const TradeQuotationLine = defineEntity({
  ...
  fields: {
    ...lineFields(),
    quotationId: f.ref('trade_quotation', { label: label('見積', 'Quotation'), required: true, onDelete: 'cascade' }),
  },
...
export const TradeOrder = defineDocument({
  ...
  views: {
    list: ['direction', 'partnerId', 'date', 'requiredDate', 'total', 'closed'],
    form: [['direction', 'partnerId', 'date', 'requiredDate'], ['note']],
  },
});
export const TradeOrderLine = defineEntity({
  ...
  fields: {
    ...lineFields(),
    orderId: f.ref('trade_order', { label: label('受発注', 'Order'), required: true, onDelete: 'cascade' }),
  },
...
export const TradeFulfillmentLine = defineEntity({
  ...
  fields: {
    ...lineFields(),
    fulfillmentId: f.ref('trade_fulfillment', {
      label: label('出荷・入荷', 'Fulfillment'),
      required: true,
      onDelete: 'cascade',
    }),
    orderLineId: f.ref('trade_order_line', {
      label: label('受発注明細', 'Order line'),
      required: true,
      serverOwned: true,
    }),
  },
...
export const TradeBillingLine = defineEntity({
  ...
  fields: {
    ...lineFields(),
    billingId: f.ref('trade_billing', { label: label('商流請求', 'Billing'), required: true, onDelete: 'cascade' }),
    fulfillmentLineId: f.ref('trade_fulfillment_line', {
      label: label('出荷・入荷明細', 'Fulfillment line'),
      required: true,
      serverOwned: true,
    }),
