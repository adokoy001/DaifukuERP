...
export async function fulfillmentQuantities(ctx: Context, fulfillmentId: string) {
    const lines = await all(ctx, TradeFulfillmentLine, { where: { fulfillmentId }, orderBy: [{ field: 'seq', dir: 'asc' }] });
    const billings = await all(ctx, TradeBilling, { where: { fulfillmentId }, orderBy: [{ field: 'createdAt', dir: 'asc' }] });
...
export async function orderQuantities(ctx: Context, orderId: string) {
    ...
    const fulfillments = await all(ctx, TradeFulfillment, { where: { orderId }, orderBy: [{ field: 'createdAt', dir: 'asc' }] });
    const fulfilled = new Map<string, Decimal>(), billed = new Map<string, Decimal>();
    for (const fulfillment of fulfillments) {
        if (fulfillment.docstatus !== DOCSTATUS.submitted)
            continue;
        ...
        for (const line of q.lines) {
            ...
            billed.set(line.orderLineId, (billed.get(line.orderLineId) ?? Decimal.zero()).plus(q.billed.get(line.id) ?? Decimal.zero()));
        }
    }
    return {
        lines,
