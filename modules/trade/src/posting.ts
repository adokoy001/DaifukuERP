import { cancelDocument, DOCSTATUS, registry, repo, StateError, submitDocument, type Context, type HookArgs } from '@daifuku/kernel';
import { cancelSourceStockEntries, createSourceStockEntry, registerInvoiceStockOwner, registerStockDocumentSource, StockEntry } from '@daifuku/mod-inventory';
...
async function postFulfillment(ctx: Context, { row }: HookArgs) {
    assertInternal(ctx);
    const lines = await all(ctx, TradeFulfillmentLine, { where: { fulfillmentId: String(row.id) }, orderBy: [{ field: 'seq', dir: 'asc' }] });
    const stock = await createSourceStockEntry(ctx, {
    ...
    }, lines.map((l) => ({ productId: l.productId, quantity: l.quantity.toString(), unitCost: row.direction === 'purchase' ? l.unitPrice.toString() : '0' })));
...
async function postBilling(ctx: Context, { row }: HookArgs) {
    assertInternal(ctx);
    const sales = row.direction === 'sales', entity = sales ? SalesInvoice : PurchaseInvoice, lineEntity = sales ? SalesInvoiceLine : PurchaseInvoiceLine;
    ...
    await repo(ctx, TradeBilling).update(String(row.id), sales ? { salesInvoiceId: invoice.id } : { purchaseInvoiceId: invoice.id });
    const lines = await all(ctx, TradeBillingLine, { where: { billingId: String(row.id) }, orderBy: [{ field: 'seq', dir: 'asc' }] });
...
async function ownedInvoice(ctx: Context, entity: string, id: string) {
    return (await repo(ctx, TradeBilling).list({ where: { [entity === SalesInvoice.name ? 'salesInvoiceId' : 'purchaseInvoiceId']: id }, limit: 1 })).items[0];
}
export function registerPostingHooks() {
    ...
    registerInvoiceStockOwner('trade', async (ctx, entity, row) => {
        ...
        if (owner.docstatus !== DOCSTATUS.submitted || fulfillment.docstatus !== DOCSTATUS.submitted || !fulfillment.stockEntryId || owner.partnerId !== row.partnerId || (owner.direction === 'sales') !== (entity === SalesInvoice.name))
            throw new StateError('商流請求と履行が一致しません。', '元資料の関連を確認してください。');
        const stock = await repo(ctx, StockEntry).get(fulfillment.stockEntryId);
        if (stock.docstatus !== DOCSTATUS.submitted || stock.sourceEntity !== TradeFulfillment.name || stock.sourceId !== fulfillment.id)
    ...
    registry.registerHook(TradeFulfillment.name, 'after_cancel', (ctx, { row, correctionDate }) => writeTrade(ctx, (inner) => cancelSourceStockEntries(inner, TradeFulfillment.name, String(row.id), correctionDate)).then(() => undefined));
    registry.registerHook(TradeBilling.name, 'after_cancel', (ctx, { row, correctionDate }) => writeTrade(ctx, async (inner) => {
        const entity = row.direction === 'sales' ? SalesInvoice : PurchaseInvoice, id = row.direction === 'sales' ? row.salesInvoiceId : row.purchaseInvoiceId;
        if (!id)
            throw new StateError('生成請求が不明です。', '商流請求の関連を確認してください。');
    ...
    for (const entity of [SalesInvoice, PurchaseInvoice]) {
        registry.registerHook(entity.name, 'before_cancel', async (ctx, { row }) => {
            if (await ownedInvoice(ctx, entity.name, String(row.id)))
                assertInternal(ctx);
        });
    ...
    registry.registerHook(StockEntry.name, 'before_submit', async (ctx, { row }) => {
        if (row.amendedFrom && (await repo(ctx, StockEntry).get(String(row.amendedFrom))).sourceEntity === TradeFulfillment.name)
            throw new StateError('商流の生成入出庫は単独改訂できません。', '元の受発注から再度履行してください。');
    });
    registry.registerHook(StockEntry.name, 'before_cancel', (ctx, { row }) => {
        if (row.sourceEntity === TradeFulfillment.name)
            assertInternal(ctx);
    });
}
