...
const quantity = z
  ...
  .refine((v) => /[1-9]/.test(v), '数量は0より大きくしてください');
...
export const fulfillInput = z
  .object({
    ...
    warehouseId: z.uuid(),
    lines: z
      .array(z.object({ orderLineId: z.uuid(), quantity }).strict())
      .min(1)
      .max(500),
    requestId: z.uuid(),
  })
...
export const billInput = z
  .object({
    ...
    lines: z
      .array(z.object({ fulfillmentLineId: z.uuid(), quantity }).strict())
      .min(1)
      .max(500),
    requestId: z.uuid(),
  })
  .strict();
export const cancelBillingInput = z
  .object({
    billingId: z.uuid(),
    expectedVersion: version,
    correctionDate: date,
  ...
  .strict();
export const cancelFulfillmentInput = z
  .object({
    fulfillmentId: z.uuid(),
    expectedVersion: version,
    correctionDate: date,
...
export const closeInput = z
  .object({ orderId: z.uuid(), expectedVersion: version, reason: z.string().trim().min(1).max(1000) })
  .strict();
...
export const fulfillmentLine = line.extend({
  orderLineId: z.uuid(),
  billedQuantity: decimal,
  unbilledQuantity: decimal,
});
...
export const detailOutput = z.object({
  ...
  fulfillments: z.array(fulfillmentRow),
});
