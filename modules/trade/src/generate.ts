import { contentHash, DOCSTATUS, repo, saveLines, StateError, submitDocument, todayLocal, withLock, type Context, type Infer } from '@daifuku/kernel';
import type { z } from 'zod';
import { Warehouse } from '@daifuku/mod-inventory';
import type { billInput, convertInput, fulfillInput } from './contract.ts';
import { TradeBilling, TradeBillingLine, TradeFulfillment, TradeFulfillmentLine, TradeOrder, TradeOrderLine, TradeQuotation, TradeQuotationLine } from './entities.ts';
...
function assertAfterCancellations(date: string, rows: {
...
export async function convertQuotation(ctx: Context, input: z.infer<typeof convertInput>) {
    ...
    if (quote.docstatus !== DOCSTATUS.submitted || (quote.validUntil && input.date > quote.validUntil) || input.date < quote.date)
        throw new StateError('有効な確定見積のみ受注へ変換できます。', '見積の状態と有効期限を確認してください。');
    const existing = await repo(ctx, TradeOrder).list({ where: { quotationId: quote.id, docstatus: { $ne: DOCSTATUS.cancelled } }, limit: 1 });
    if (existing.items.length)
        throw new StateError('この見積には受注が存在します。', '既存の受注を開いてください。');
    return writeTrade(ctx, async (inner) => {
        ...
        const lines = await all(inner, TradeQuotationLine, { where: { quotationId: quote.id }, orderBy: [{ field: 'seq', dir: 'asc' }] });
        await saveLines(inner, TradeOrder, order.id, { [TradeOrderLine.name]: lines.map((l) => ({
...
export async function fulfillOrder(ctx: Context, input: z.infer<typeof fulfillInput>) {
    return withLock(ctx, 'trade.fulfill.' + input.requestId, async () => {
        ...
        const { expectedVersion: _version, ...payload } = input, hash = contentHash(JSON.stringify(payload));
        const existing = (await repo(ctx, TradeFulfillment).list({ where: { requestId: input.requestId }, limit: 1 })).items[0];
        if (existing) {
            if (existing.requestHash !== hash || existing.docstatus !== DOCSTATUS.submitted)
                throw new StateError('再送識別子は別内容または取消済みです。', '新しい操作には新しい識別子を使用してください。');
