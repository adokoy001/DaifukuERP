...
async function orderRow(ctx: Context, row: Infer<typeof TradeOrder>, q: Awaited<ReturnType<typeof orderQuantities>>) {
    ...
    const unbilledLineCount = q.lines.filter((l) => (q.fulfilled.get(l.id) ?? zero).gt(q.billed.get(l.id) ?? zero)).length;
    const status = row.docstatus === DOCSTATUS.cancelled ? 'cancelled' : row.docstatus === DOCSTATUS.draft ? 'draft' : row.closed ? 'closed' : openLineCount ? 'open' : 'fulfilled';
    return {
...
async function billingRow(ctx: Context, row: Infer<typeof TradeBilling>) {
    const entity = row.direction === 'sales' ? SalesInvoice : PurchaseInvoice, invoiceId = row.salesInvoiceId ?? row.purchaseInvoiceId, invoice = invoiceId ? await repo(ctx, entity).get(invoiceId) : null;
    return {
...
export async function orderDetail(ctx: Context, { orderId }: {
    orderId: string;
}): Promise<TradeOrderDetail> {
    const row = await repo(ctx, TradeOrder).get(orderId), q = await orderQuantities(ctx, orderId);
    ...
    return { order: await orderRow(ctx, row, q), lines: q.lines.map((l) => ({
            ...lineView(l),
...
export async function tradeBoard(ctx: Context, input: z.infer<typeof boardInput>) {
    const page = await repo(ctx, TradeOrder).list({
        where: { direction: input.direction, ...(input.partnerId ? { partnerId: input.partnerId } : {}), ...(input.status === 'open' ? { docstatus: DOCSTATUS.submitted } : {}) },
        orderBy: [{ field: 'date', dir: 'desc' }, { field: 'id', dir: 'asc' }],
        limit: input.limit,
        offset: input.offset
    });
    const rows = [];
    for (const order of page.items)
        rows.push(await orderRow(ctx, order, await orderQuantities(ctx, order.id)));
    return {
        rows,
        total: page.total,
        limit: page.limit,
