...
async function cancelDate(ctx: Context, args: HookArgs, children: EntityDef, parentField: string) {
  ...
  if (
    descendants.some((c) => c.docstatus !== DOCSTATUS.cancelled || (c.cancelledDate && String(c.cancelledDate) > date))
  )
    throw new StateError(
      '後続文書が残る、または後続取消日より前です。',
      '後続文書を取消し、その取消日以降を指定してください。',
    );
  row.cancelledDate = date;
}
export function registerLifecycleHooks() {
  for (const entity of [TradeFulfillment, TradeFulfillmentLine, TradeBilling, TradeBillingLine]) generated(entity);
  ...
  registry.registerHook(TradeOrder.name, 'before_cancel', (ctx, args) =>
    cancelDate(ctx, args, TradeFulfillment, 'orderId'),
  );
}
export async function cancelBilling(ctx: Context, input: z.infer<typeof cancelBillingInput>) {
  ...
  return writeTrade(ctx, async (inner) => {
    ...
    return result(
      await cancelDocument(inner, TradeBilling, row.id, { correctionDate: input.correctionDate }),
      TradeBilling,
    );
  });
}
export async function cancelFulfillment(ctx: Context, input: z.infer<typeof cancelFulfillmentInput>) {
  ...
  return writeTrade(ctx, async (inner) => {
    ...
    return result(
      await cancelDocument(inner, TradeFulfillment, row.id, { correctionDate: input.correctionDate }),
      TradeFulfillment,
    );
  });
}
export async function closeOrder(ctx: Context, input: z.infer<typeof closeInput>) {
  ...
  return writeTrade(ctx, async (inner) =>
    result(await repo(inner, TradeOrder).update(row.id, { closed: true, closedReason: input.reason }), TradeOrder),
  );
}
