import { Decimal, DOCSTATUS, getCompany, isSavingLines, registry, repo, StateError, type Context, type EntityDef, type LocalDate } from '@daifuku/kernel';
...
export async function fillPlanningLine(ctx: Context, row: Raw, previous: Raw | undefined, parent: EntityDef, field: string) {
    const value = { ...previous, ...row }, head = await repo(ctx, parent).get(String(value[field]));
    row.direction = head.direction;
    const product = await repo(ctx, Product).get(String(value.productId));
    if (product.kind !== 'goods' || !product.isActive || (head.direction === 'sales' ? !product.isSold : !product.isPurchased))
    ...
    if (!unit)
        throw new StateError('在庫単位が未設定です。', '品目に固定在庫単位を設定してください。');
    ...
    const qty = positiveQuantity(value.quantity ?? '1'), price = Decimal.from(String(row.unitPrice));
...
export function registerPlanningLines(parent: EntityDef, line: EntityDef, field: string) {
    registry.registerHook(line.name, 'before_validate', (ctx, { row, previous }) => fillPlanningLine(ctx, row, previous, parent, field));
    ...
    for (const phase of ['after_create', 'after_update', 'after_delete'] as const)
        registry.registerHook(line.name, phase, async (ctx, { row, previous }) => {
            for (const id of new Set([row[field], previous?.[field]]))
                if (typeof id === 'string' && !isSavingLines(ctx, parent.name, id))
                    await repo(ctx, parent).update(id, {});
        });
}
async function estimate(ctx: Context, row: Raw, line: EntityDef, parentField: string, requireLines: boolean) {
    const lines = await all(ctx, line, { where: { [parentField]: String(row.id) }, orderBy: [{ field: 'seq', dir: 'asc' }] });
    if (lines.length > 500 || (requireLines && !lines.length))
        throw new StateError('明細は1～500行です。', '物品明細を入力してください。');
    const summary = await taxSummaryFor(ctx, { date: row.date as LocalDate, priceIncludesTax: false, lines: lines.map((l) => ({ amount: Decimal.from(String(l.quantity)).times(String(l.unitPrice)), category: l.taxCategory as TaxCategory })) });
    Object.assign(row, {
...
function registerHead(entity: EntityDef, line: EntityDef, field: string) {
    registry.registerHook(entity.name, 'before_validate', async (ctx, { row, previous }) => {
        if (entity === TradeQuotation && !previous)
            row.direction = 'sales';
        if (!previous)
            Object.assign(row, {
                subtotal: '0',
    ...
    registry.registerHook(entity.name, 'before_update', async (ctx, { row }) => {
        if (row.docstatus === DOCSTATUS.draft)
            await estimate(ctx, row, line, field, false);
    });
    registry.registerHook(entity.name, 'after_lines_saved', (ctx, { row }) => repo(ctx, entity).update(String(row.id), {}).then(() => undefined));
    registry.registerHook(entity.name, 'before_submit', async (ctx, { row }) => {
        ...
        if (row.quotationId) {
            ...
            if (await repo(ctx, TradeOrder).count({ quotationId: quote.id, id: { $ne: String(row.id) }, docstatus: { $ne: DOCSTATUS.cancelled } })) throw new StateError('同じ見積を使用する受注が存在します。', '既存受注を確認し、重複する下書きを整理してください。');
...
export function registerPlanningHooks() {
    registry.registerHook(TradeQuotation.name, 'before_cancel', async (ctx, { row }) => {
        if ((await repo(ctx, TradeOrder).count({ quotationId: String(row.id), docstatus: { $ne: DOCSTATUS.cancelled } })) > 0)
            throw new StateError('元見積を使用する受注が残っています。', '下書きは削除、確定受注は取消してから見積を取り消してください。');
    });
