...
const grants = tradeEntities.map((e) =>
  defineWriteCapability({
    name: 'trade.' + e.name,
    entity: e.name,
    fields: e.fieldNames,
...
export async function all<E extends EntityDef>(ctx: Context, entity: E, query: ListQuery): Promise<Infer<E>[]> {
  ...
  for (let offset = 0; ; offset += 500) {
    ...
    if (page.items.length === 0 || offset + page.items.length >= page.total) return rows;
    if (rows.length >= 10000) throw new StateError('関連資料が1万件を超えました。', '商流を複数注文へ分けてください。');
  }
}
