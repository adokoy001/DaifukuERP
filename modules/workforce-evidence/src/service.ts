...
const write = defineWriteCapability({
  name: 'workforce_evidence.upload',
  entity: 'workforce_receipt',
  fields: Object.keys(WorkforceReceipt.config.fields),
  operations: ['create'],
});
const actor = (ctx: Context) =>
  ctx.actor.type === 'agent' ? ctx.actor.onBehalfOf : ctx.actor.type === 'user' ? ctx.actor.id : undefined;

async function editableExpense(ctx: Context, expenseId: string) {
  const expense = await repo(ctx, WorkforceExpense).get(expenseId);
  if (expense.userId !== actor(ctx))
    throw new PermissionDenied('workforce_receipt', 'upload-for-another-employee', ctx.roles);
  if (!['draft', 'returned'].includes(expense.status))
    throw new StateError(
      'Receipts are frozen after submission.',
      'Ask for the expense to be returned before adding evidence.',
    );
  return expense;
}
...
export async function uploadReceipt(ctx: Context, expenseId: string, input: ReceiptUpload) {
  ...
  return withLock(ctx, `workforce:employee:${first.employeeId}`, async () => {
    ...
    if (expense.version !== input.expectedVersion)
      throw new Conflict('The expense changed.', 'Reload it before adding the receipt.', { version: expense.version });
    const digest = await sha256Hex(input.data);
    if (await repo(ctx, WorkforceReceipt).count({ expenseId, sha256: digest }))
      throw new Conflict('This expense already has the same receipt.', 'Use the existing receipt.');
    if ((await repo(ctx, WorkforceReceipt).count({ expenseId })) >= 10)
      throw new StateError(
        'An expense may have at most ten receipts.',
        'Keep additional evidence in your controlled document store and record the reference.',
      );
    const stored = await ctx.storage.put(ctx.tenantId, input.data, {
      filename: meta.filename,
      contentType: meta.contentType,
    });
    if (stored.sha256 !== digest || stored.size !== meta.size || !stored.key.startsWith(`${ctx.tenantId}/`))
      throw new StateError('Stored receipt integrity check failed.', 'Verify the storage adapter.');
    const row = await withWriteCapability(ctx, write, (internal) =>
      repo(internal, WorkforceReceipt).create({
        expenseId,
...
export async function listReceipts(ctx: Context, expenseId: string) {
  ...
  const list = await repo(ctx, WorkforceReceipt).list({
    where: { expenseId },
...
export async function downloadReceipt(ctx: Context, receiptId: string) {
  ...
  if (!row.storageKey.startsWith(`${ctx.tenantId}/`))
    throw new PermissionDenied('workforce_receipt', 'storage-scope', ctx.roles);
  const bytes = await ctx.storage.get(row.storageKey);
  if (bytes.byteLength !== row.size || (await sha256Hex(bytes)) !== row.sha256)
    throw new StateError(
      'Receipt bytes do not match the recorded digest.',
...
export function receiptHooks(): void {
  registry.registerHook('workforce_receipt', 'before_create', async (ctx, { row }) => {
    ...
    if (row.userId !== parent.userId || row.siteId !== parent.siteId || row.employeeId !== parent.employeeId)
      throw new PermissionDenied('workforce_receipt', 'receipt-identity', ctx.roles);
  });
  const frozen = () => {
    throw new StateError(
      'Receipt evidence is immutable.',
      'Return the expense and add corrected evidence with an explanation.',
    );
  };
