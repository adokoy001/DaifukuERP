...
| 認証負荷・検索索引・APIビルド | [追加レビュー対応](docs/specs/review-hardening.md)、[API成果物](docs/architecture/compiled-api.md)、[Cookie移行方針](docs/adr/0025-browser-session-migration.md) | [パスワード計算](kernel/src/password-hash.ts)、[コンパイラ](deploy/compile-api.mjs) |
| 宣言・エラー・booleanの一貫性 | [一貫性仕様](docs/specs/source-consistency.md)、[エラー規約](docs/conventions/errors.md)、[責務ヘッダ](docs/conventions/source-headers.md)、[基盤の移行](docs/operations/setup.md#基盤のboolean移行) | [lint](eslint.config.js)、[基盤表](kernel/src/db/system-tables.ts)、[文書定義](kernel/src/dsl/entity.ts) |
| 配信時のブラウザ保護・転記の照会数 | [仕上げ仕様](docs/specs/release-finishing.md)、[配備手順](docs/operations/deployment.md)、[引継ぎの記録](docs/architecture/extension-guide.md#引継ぎを確認する記録) | [Caddy生成](deploy/profile.mjs)、[転記検証](modules/accounting/src/hooks/validate-entry.ts) |
