...
export function parseLedger(source) {
  ...
  for (const line of source.split(/\r?\n/)) {
    ...
    if (heading) { entry = { id: heading[1], fields: {} }; entries.push(entry); }
...
function localTarget(root, document, link) {
  ...
  if (!path || isAbsolute(path) || /^[a-z][a-z0-9+.-]*:/i.test(path)) throw new Error('reference must be a relative file');
  const target = realpathSync(resolve(dirname(document), decodeURIComponent(path)));
  const scoped = relative(realpathSync(root), target);
  if (scoped === '..' || scoped.startsWith('../') || scoped.startsWith('..\\') || isAbsolute(scoped) || !statSync(target).isFile()) throw new Error('reference escapes the repository or is not a file');
  if (anchor !== undefined) {
    const line = /^L([1-9][0-9]*)$/.exec(anchor);
    if (!line || Number(line[1]) > readFileSync(target, 'utf8').split(/\r?\n/).length) throw new Error('reference requires an existing #L<number> source line');
  }
  return target;
}

export function validateLedger(source, { root, document, minimum = 8 }) {
  const entries = parseLedger(source), ids = new Set(), failures = [];
    ...
    for (const field of ['仕様', '実装', '試験']) {
      ...
      for (const link of references) {
        try {
          ...
          if (field === '試験' && !/\.(test|spec)\.(ts|mts|js|mjs)$/.test(target)) throw new Error('test reference must name an executable test');
        } catch (error) { failures.push(`${entry.id}: invalid ${field} reference ${link}: ${error.message}`); }
      }
    }
  }
...
export function affectedEntries(entries, changed, { root, document }) {
  ...
  return entries.filter(entry => ['仕様', '実装', '試験'].some(field => links(entry.fields[field]).some(link => {
    const target = localTarget(root, document, link);
