...
for (let i = 0; i < args.length; i += 2) {
  ...
  if (!['--jar', '--java', '--only'].includes(name) || !args[i + 1])
    throw new Error('Use --jar PATH, --java PATH, or --only base|mutations|witnesses|traces');
  options[name.slice(2)] = args[i + 1];
}
if (options.only && !['base', 'mutations', 'witnesses', 'traces'].includes(options.only))
  throw new Error('Unknown check selection');
...
async function verifiedJar() {
  ...
  try {
    ...
    for await (const chunk of response.body) {
      size += chunk.length;
      if (size > tool.bytes) throw new Error('TLC asset size mismatch');
      chunks.push(chunk);
    }
...
async function check(name, config, expected, module = 'Edge', content) {
  ...
  try {
    ({ stdout, stderr } = await promisify(execFile)(
      ...
      [
        ...
        '0',
        '-metadir',
        join(directory, 'states'),
        '-config',
        module + '.cfg',
        module + '.tla',
      ],
      { cwd: directory, timeout: 240000, maxBuffer: 16 * 1024 * 1024 },
    ));
  } catch (error) {
    stdout = error.stdout ?? '';
    stderr = error.stderr ?? '';
  ...
  const violation = stdout.match(/Invariant (\w+) is violated/);
  const invariant = violation?.[1];
  ...
  const passed = expected
    ...
    : code === 0 &&
      /Model checking completed\. No error has been found\./.test(stdout) &&
      /0 states left on queue\./.test(stdout);
  const counts = stdout.match(/(\d+) states generated, (\d+) distinct states found, (\d+) states left on queue/);
  const result = {
    name,
    passed,
    expected: expected ?? 'complete finite exploration',
    invariant: invariant ?? null,
    exitCode: code,
    elapsedMs: Math.round(performance.now() - start),
    generated: counts ? Number(counts[1]) : null,
    distinct: counts ? Number(counts[2]) : null,
    witnessStates,
  };
  results.push(result);
  ...
  await writeFile(
    ...
    JSON.stringify(
      { tool, limits: { jobs: 2, workers: 2, attempts: 2, claimPerWorker: 1, crashesPerWorker: 1 }, results },
      null,
      2,
    ) + '\n',
  );
...
const invariantConfig = (invariant, mutation = 'none') =>
  baseConfig
    .replace('Mutation = "none"', `Mutation = "${mutation}"`)
    .replace(/INVARIANTS[\s\S]*/, `INVARIANTS TypeOK ${invariant}\n`);
if (selected('base')) await check('safety', baseConfig);
if (selected('mutations')) {
  for (const [mutation, invariant] of [
    ['start', 'StartUnique'],
    ['requeue', 'NoStartedRequeue'],
    ['fence', 'FencingSafe'],
    ['resolve', 'ResolvedSafe'],
    ['claim', 'ClaimSafe'],
  ])
    await check('mutation-' + mutation, invariantConfig(invariant, mutation), invariant);
}
if (selected('witnesses')) {
  for (const invariant of ['NotLostStart', 'NotLostComplete', 'NotObsolete', 'NotResolved', 'NotPhysicalSend'])
    await check('witness-' + invariant, invariantConfig(invariant), invariant);
}
...
if (selected('traces')) {
  ...
  for (const trace of traces) {
    const inputs = {
      TraceEvents: trace.steps.map((s) => s.event),
      TraceStates: trace.steps.map((s) => s.states),
      TraceAttempts: trace.steps.map((s) => s.attempts),
      TraceGrants: trace.steps.map((s) => s.grants),
      TraceJournals: trace.steps.map((s) => s.journals),
      TraceSends: trace.steps.map((s) => s.sends),
    };
    const extra = Object.keys(inputs)
      .map((key) => `  ${key} <- Input${key}`)
      .join('\n');
    const module =
      '---- MODULE TraceCase ----\nEXTENDS EdgeTrace\n' +
      Object.entries(inputs)
        .map(([key, value]) => `Input${key} == ${tla(value)}`)
        .join('\n') +
      '\n====\n';
    const config = baseConfig
      .replace('CONSTANTS', 'CONSTANTS\n' + extra)
      .replace('KeepEvent = FALSE', 'KeepEvent = TRUE')
      .replace('SPECIFICATION Spec', 'SPECIFICATION TraceSpec');
    await check(
      'trace-' + trace.id,
      config + ' ProjectionMatches TraceUnfinished\n',
      'TraceUnfinished',
      'TraceCase',
      module,
    );
