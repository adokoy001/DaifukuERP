...
try {
  ...
  const sources = Object.fromEntries(
    await Promise.all(
      Object.keys(EXPECTED_MUTANTS).map(async (path) => [path, await readFile(resolve(root, path), 'utf8')]),
    ),
  );
  const count = validateMutationReport(report, sources);
  console.log(
    `Mutation evidence: ${count} killed, 0 survivors, 0 timeouts; current full source and inventory verified.`,
  );
} catch (error) {
  console.error(`Mutation evidence rejected: ${error.message}`);
  process.exitCode = 1;
}
