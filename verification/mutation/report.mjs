...
function completedFailingTests(reason) {
  ...
  const complete = (summary, total) =>
    summary &&
    Number(summary[1]) > 0 &&
    Number(summary[1]) + Number(summary[2] ?? 0) === total &&
    Number(summary[3]) === total;
  const namedFailure =
    /^\s*FAIL\s+(?:kernel\/test\/decimal(?:-reference)?|modules\/accounting\/test\/balance(?:-reference)?)\.test\.ts\s*>/m.test(
      output,
    );
  ...
  return (
    ...
    complete(files, 4) &&
    namedFailure &&
    !/Unhandled Errors|Unhandled Rejection|\bErrors\s+\d+ errors?|(?:Test|Hook) timed out|Worker exited unexpectedly/.test(
      output,
    )
  );
}
...
export function validateMutationReport(report, sources, inventory = EXPECTED_MUTANTS) {
  ...
  if (
    ...
    report.framework?.version !== '10.0.0'
  )
    throw new Error('Unreviewed mutation report format or engine');
  if (!report.files || !sameKeys(report.files, inventory) || !sameKeys(sources, inventory))
    throw new Error('Mutation scope is missing or unexpected');
  if (
    report.config?.testRunner !== 'command' ||
    report.config.commandRunner?.command !== TEST_COMMAND ||
    JSON.stringify(report.config.mutate) !== JSON.stringify(Object.keys(inventory))
  )
    throw new Error('Mutation scope or runner differs from the reviewed configuration');
  let total = 0;
  const ids = new Set();
  for (const [path, expected] of Object.entries(inventory)) {
    ...
    if (!Array.isArray(file.mutants) || file.mutants.length !== expected)
      throw new Error(`Review changed mutation inventory: ${path}`);
    for (const mutant of file.mutants) {
      ...
      if (!completedFailingTests(mutant.statusReason))
        throw new Error(`Missing completed failing-test evidence: ${path} ${mutant.id}`);
      total += 1;
    }
  }
  return total;
