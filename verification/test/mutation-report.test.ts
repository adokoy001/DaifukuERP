...
const fixture = () => ({
  ...
  config: { testRunner: 'command', commandRunner: { command: TEST_COMMAND }, mutate: ['sample.ts'] },
  files: {
    'sample.ts': {
      source: sources['sample.ts'],
      mutants: ['0', '1'].map((id) => ({ id, status: 'Killed', statusReason: failureEvidence })),
    },
  },
});
...
describe('AC-6 mutation evidence must fail closed', () => {
  ...
  it('rejects survivors, timeouts, errors and unexecuted mutations', () => {
    for (const status of ['Survived', 'Timeout', 'RuntimeError', 'CompileError', 'NoCoverage', 'Ignored', 'Pending']) {
      ...
      report.files['sample.ts'].mutants = report.files['sample.ts'].mutants.map((mutant) => ({ ...mutant, status }));
  ...
  it('rejects stale source, partial inventory, duplicate ids and alternate runners', () => {
    ...
    expect(() => validateMutationReport({ ...fixture(), framework: { name: 'other' } }, sources, inventory)).toThrow(
      'engine',
    );
  });
  it('rejects Killed caused by startup failures, no tests, partial execution or unhandled errors', () => {
    for (const statusReason of [
      ...
      failureEvidence.replace('20 passed (21)', '19 passed (20)'),
      failureEvidence.replace('FAIL kernel/test/decimal.test.ts > comparison', 'FAIL unrelated.test.ts > error'),
      failureEvidence + 'Unhandled Errors',
      failureEvidence + 'Test timed out in 5000ms',
      failureEvidence + 'Hook timed out in 10000ms',
    ]) {
      const report = fixture();
      report.files['sample.ts'].mutants = report.files['sample.ts'].mutants.map((mutant) => ({
        ...mutant,
        statusReason,
      }));
