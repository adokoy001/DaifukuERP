...
export const ApplianceServiceLine = defineEntity({
  name: 'appliance_store_service_line', label: label('作業・部品明細', 'Labour / parts line'),
  fields: {
    serviceId: f.ref('appliance_store_service', { label: label('受付', 'Service job'), required: true, onDelete: 'cascade' }),
    ...
    taxCategory: f.enum(TAX_CATEGORIES, { label: label('税区分', 'Tax category'), required: true, default: 'standard', labels: TAX_CATEGORY_LABELS }),
    amount: f.money({ label: label('税抜金額', 'Amount excluding tax'), required: true, default: '0', serverOwned: true }),
  },
