import { cancelDocument, Decimal, DOCSTATUS, getLines, isLocalDate, registry, repo, StateError, ValidationError, type Context, type HookArgs, type LocalDate } from '@daifuku/kernel';
...
function validDate(value: unknown): value is LocalDate { return typeof value === 'string' && isLocalDate(value); }
...
async function validateDevice(ctx: Context, { row, previous }: HookArgs): Promise<void> {
  ...
  if (typeof value.contractId === 'string') {
    ...
    if (contract.partnerId !== value.partnerId) invalid('contractId', 'The maintenance contract must belong to the same customer.');
  }
  if (validDate(value.purchaseDate) && validDate(value.warrantyUntil) && value.warrantyUntil < value.purchaseDate) invalid('warrantyUntil', 'Warranty expiry cannot precede purchase.');
}
...
async function validateService(ctx: Context, { row, previous }: HookArgs): Promise<void> {
  ...
  if (validDate(value.date)) {
    for (const key of ['scheduledDate', 'completedDate']) { const date = value[key]; if (validDate(date) && date < value.date) invalid(key, 'The date cannot precede reception.'); }
  }
}
...
async function beforeSubmit(ctx: Context, args: HookArgs): Promise<void> {
  ...
  const { row } = args;
  if (row.status !== 'in_progress') throw new StateError('Start work before completing this job.', 'Use appliance_store.start_service.');
  ...
  if (row.billing === 'billable' && (lines.length === 0 || !Decimal.sum(lines.map((line) => Decimal.from(line.amount as string | Decimal))).gt(0))) invalid('lines', 'A billable job requires a positive service or parts total.');
  if (row.billing === 'no_charge' && lines.length) invalid('lines', 'No-charge jobs cannot contain billing lines; record warranty parts consumption with an inventory entry.');
...
async function afterCancel(ctx: Context, { row, correctionDate }: HookArgs): Promise<void> {
  ...
  if (invoice.docstatus === DOCSTATUS.submitted) await cancelDocument(ctx, SalesInvoice, invoice.id, { correctionDate });
}
...
export function registerApplianceHooks(): void {
  ...
  registry.registerHook(ApplianceService.name, 'before_cancel', (_ctx, { row }) => { row.status = 'cancelled'; });
  registry.registerHook(ApplianceService.name, 'after_cancel', afterCancel);
  registry.registerHook(ApplianceService.name, 'before_delete', (_ctx, { row }) => { if (row.status !== 'queued') throw new StateError('A started job cannot be deleted.', 'Complete it, then cancel and amend if a correction is needed.'); });
}
