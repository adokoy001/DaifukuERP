...
export const ApplianceStorePack = definePack({
  name: 'appliance_store',
  label: label('電器店', 'Appliance store'),
  version: '0.1.0',
  depends: [
    PartnerModule.name,
    ProductModule.name,
  ...
  ext: {
    product: {
      applianceManufacturer: f.text({
        label: label('家電メーカー', 'Appliance manufacturer'),
        maxLength: 100,
        searchable: true,
      }),
      applianceModel: f.text({ label: label('家電型番', 'Appliance model'), maxLength: 100, searchable: true }),
      applianceWarrantyMonths: f.int({
        label: label('標準保証月数（記録）', 'Recorded warranty months'),
        min: 0,
        max: 120,
      }),
    },
  ...
  settings: {
    'inventory.auto_issue_on_sales': true,
    'inventory.auto_receipt_on_purchase': true,
  ...
  menus: [
    ...
    {
      label: label('作業の請求書を発行', 'Invoice service work'),
      route: '/a/appliance_store.invoice_service',
      order: 105,
    },
  ...
  seed: seedApplianceStore,
  sample: sampleApplianceStore,
});
