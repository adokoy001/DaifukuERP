...
export const RetailClosing = defineDocument({
  ...
  fields: {
    ...
    warehouseId: f.ref(Warehouse.name, {
      ...
      description: label('省略時は既定の倉庫（在庫の自動出庫は既定の倉庫から）', 'defaults to the default warehouse (auto issues come from it)'),
      required: true,
    }),
    cashAmount: f.money({ label: label('現金売上', 'Cash sales'), description: label('税込', 'tax included'), required: true, default: '0', min: '0' }),
    cardAmount: f.money({ label: label('カード売上', 'Card sales'), description: label('税込。売掛金として残り、カード会社の入金で消し込む', 'tax included; stays receivable until the card company pays'), required: true, default: '0', min: '0' }),
    subtotal: f.money({ label: label('税抜合計', 'Subtotal'), description: label('明細から自動計算', 'computed from the lines'), required: true, default: '0' }),
    taxTotal: f.money({ label: label('消費税額', 'Tax'), description: label('税率ごとに1回丸め', 'rounded once per rate'), required: true, default: '0' }),
    total: f.money({ label: label('税込合計', 'Total'), description: label('現金 + カードと一致すること', 'must equal cash + card'), required: true, default: '0' }),
    // No DSL default for json (drizzle-kit rejects bound json defaults; see modules/sales). hooks/recalc.ts always sets it.
    taxSummary: f.json<InvoiceTaxSummaryRow[]>({ label: label('税率別内訳', 'Tax summary'), description: label('税率ごとの対価の額・消費税額', 'per-rate taxable / tax / gross') }),
    salesInvoiceId: f.ref(SalesInvoice.name, { label: label('売上請求書', 'Sales invoice'), description: label('submit 時に作成', 'created at submit') }),
    paymentId: f.ref(Payment.name, { label: label('現金入金', 'Cash receipt'), description: label('submit 時に作成（現金 > 0 のとき）', 'created at submit when cash > 0') }),
