...
export const RetailClosingLine = defineEntity({
  ...
  fields: {
    closingId: f.ref(RetailClosing.name, { label: label('レジ締め', 'Register closing'), required: true, onDelete: 'cascade' }),
    ...
    quantity: f.quantity({ label: label('数量', 'Quantity'), description: label('0 以外。返品はマイナス', 'non-zero; returns are negative'), required: true }),
    unitPrice: f.money({ label: label('税込単価', 'Unit price (tax incl.)'), description: label('省略時は品目の販売単価', "defaults to the product's sale price"), required: true }),
    ...
    amount: f.money({ label: label('金額', 'Amount'), description: label('数量 × 税込単価（丸めなし）', 'quantity × unit price, unrounded'), required: true, default: '0' }),
  },
