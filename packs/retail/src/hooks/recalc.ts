...
export async function loadClosingLines(ctx: Context, closingId: string) {
  return (
    await repo(ctx, RetailClosingLine).list({
      where: { closingId },
...
export async function computeClosing(
  ctx: Context,
  closingId: string,
  date: LocalDate,
): Promise<{ totals: InvoiceTotals; lineCount: number }> {
...
export function assertTender(
  row: { number?: unknown; cashAmount: unknown; cardAmount: unknown; total: unknown },
  id: string,
): void {
  ...
  throw new ValidationError(
    ...
    [
      {
        path: 'cardAmount',
        message: `cash + card must equal total ${total.toString()} (difference ${check.difference.toString()})`,
      },
    ],
