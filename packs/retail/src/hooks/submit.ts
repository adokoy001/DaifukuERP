...
export const WAREHOUSE_HINT =
  'Inventory issues sales from the default warehouse (inventory.default_warehouse): close the register on that warehouse, or change the setting. Multiple stores are out of scope.';
...
export async function findWalkIn(ctx: Context): Promise<string> {
  ...
  if (!found)
    throw new StateError(`walk-in partner ${WALK_IN_CODE} does not exist`, WALK_IN_HINT, { code: WALK_IN_CODE });
  return found.id;
}

async function assertWarehouse(ctx: Context, warehouseId: unknown): Promise<void> {
  const autoIssue = await getSetting(
    ctx,
    AUTO_ISSUE_ON_SALES_KEY,
    boolSettingSchema,
  ...
  if (warehouseId !== main.id)
    throw new StateError(
      `retail_closing warehouse ${String(warehouseId)} is not the default warehouse ${main.code}`,
      WAREHOUSE_HINT,
      { warehouseId, defaultWarehouseId: main.id },
    );
}
...
async function beforeSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  if (lineCount === 0)
    throw new ValidationError(
      `retail_closing ${id} has no lines`,
      [{ path: 'lines', message: 'at least 1 line is required' }],
      'Add the products sold, then submit.',
    );
  Object.assign(row, {
  ...
  if (!totals.total.gt(0)) {
    throw new ValidationError(
      `retail_closing ${id} total ${totals.total.toString()} must be greater than 0`,
      [{ path: 'total', message: 'must be > 0' }],
      'A closing with only returns cannot become a sales invoice (credit notes are out of scope).',
    );
  }
...
async function createInvoice(ctx: Context, head: ClosingHead): Promise<string> {
  ...
  const draft = await repo(ctx, SalesInvoice).create({
    partnerId: head.partnerId,
    date: head.date,
    priceIncludesTax: true,
    note: `レジ締め ${head.number}`,
  });
  await saveLines(ctx, SalesInvoice, draft.id, {
    [SalesInvoiceLine.name]: lines.map((l) => ({
...
async function createCashReceipt(ctx: Context, head: ClosingHead, invoiceId: string): Promise<string> {
  ...
  if (!account.id)
    throw new StateError(
      `cash account ${account.code} does not exist`,
      'Seed the chart of accounts (l10n/jp) or change payment.accounts.cash.',
      { code: account.code },
    );
  const draft = await repo(ctx, Payment).create({
    direction: 'receive',
    partnerId: head.partnerId,
    date: head.date,
    amount: head.cash,
    ...
    note: `レジ締め ${head.number} 現金売上`,
  });
  await saveLines(ctx, Payment, draft.id, {
    [PaymentAllocation.name]: [{ invoiceEntity: SalesInvoice.name, invoiceId, amount: head.cash }],
  });
...
async function afterSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  const head: ClosingHead = {
    id: row.id as string,
    number: String(row.number),
    date: row.date as LocalDate,
    partnerId: await findWalkIn(ctx),
    cash: decimalOf(row.cashAmount),
  };
