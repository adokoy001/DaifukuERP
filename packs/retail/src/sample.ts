...
export const SAMPLE_PRODUCTS = [
  { code: 'P1', name: '弁当', taxCategory: 'reduced', salePrice: '540', purchasePrice: '300', ext: { jan: '2000000000015', supplierCode: 'S1-BENTO', shelf: 'A-01' } },
  { code: 'P2', name: 'お茶', taxCategory: 'reduced', salePrice: '162', purchasePrice: '80', ext: { jan: '2000000000022', supplierCode: 'S1-TEA', shelf: 'A-02' } },
  { code: 'P3', name: '雑貨', taxCategory: 'standard', salePrice: '1100', purchasePrice: '600', ext: { jan: '2000000000039', supplierCode: 'S1-GOODS', shelf: 'B-01' } },
  { code: 'P4', name: '文具', taxCategory: 'standard', salePrice: '330', purchasePrice: '150', ext: { jan: '2000000000046', supplierCode: 'S1-PEN', shelf: 'B-02' } },
] as const;

export async function sampleRetail(ctx: Context): Promise<void> {
  const products = repo(ctx, Product);
  const present = new Set((await products.list({ where: { code: { $in: SAMPLE_PRODUCTS.map((p) => p.code) } }, limit: SAMPLE_PRODUCTS.length })).items.map((p) => p.code));
  ...
  await partners.create({ code: SAMPLE_SUPPLIER_CODE, name: '卸売商事', isSupplier: true, taxStatus: 'registered', ext: { retailKind: 'wholesaler' satisfies RetailKind } });
}
