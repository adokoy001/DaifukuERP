...
describe('retail month close serialization and inventory cutoff', () => {
  it('concurrent calls for one month store one close, posting and cutoff', async () => {
    const results = await Promise.all([
      s.act<Row>('retail.close_month', { period: '2026-11' }),
      s.act<Row>('retail.close_month', { period: '2026-11' }),
    ]);
  ...
  it('rejects inventory changes through the closed date, and carries the stored closing into the next month', async () => {
    ...
    await expect(
      createSubmit(s, 'stock_entry', {
        type: 'receipt',
        date: '2026-11-30',
        warehouseId,
        lines: { stock_entry_line: [{ productId, quantity: '1', unitCost: '999' }] },
      }),
    ...
    await createSubmit(s, 'stock_entry', {
      ...
      date: '2026-12-01',
      warehouseId,
      lines: { stock_entry_line: [{ productId, quantity: '1', unitCost: '200' }] },
    });
