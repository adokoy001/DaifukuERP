...
describe('retail cancellation after accounting and inventory close', () => {
  it('propagates the chosen correction date through cash, invoice and stock reversals', async () => {
    ...
    await createSubmit(s, 'stock_entry', {
      type: 'receipt',
      date: '2026-11-01',
      warehouseId,
      lines: { stock_entry_line: [{ productId, quantity: '10', unitCost: '100' }] },
    });
    const closing = await createSubmit<DocJson>(s, 'retail_closing', {
      date: '2026-11-02',
      warehouseId,
      cashAmount: '540',
      cardAmount: '0',
      lines: { retail_closing_line: [{ productId, quantity: '1' }] },
    });
    ...
    expect(await s.act('retail_closing.cancel', { id: closing.id, correctionDate: '2026-12-02' })).toMatchObject({
      docstatus: 2,
    });
    ...
    const reversals = await s.act<ListJson>('journal_entry.list', {
      where: { reversalOf: { $in: [payment.journalEntryId, invoice.journalEntryId] } },
    });
