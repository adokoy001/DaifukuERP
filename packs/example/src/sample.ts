...
export async function sampleExample(ctx: Context): Promise<void> {
  ...
  const existing = await r.list({
    ...
    limit: SAMPLE_PARTNER_CODES.length,
  });
  ...
  const rows = [
    {
      code: SAMPLE_PARTNER_CODES[0],
      name: 'サンプル得意先A',
      isCustomer: true,
      ext: { customerRank: 'A', note2: '年間契約あり' },
    },
    {
      code: SAMPLE_PARTNER_CODES[1],
      name: 'サンプル仕入先B',
      isSupplier: true,
      ext: { customerRank: defaultRank, note2: 'スポット取引' },
    },
  ];
