...
import { DEFAULT_RANK_KEY, defaultRankSchema, ExamplePack, ExampleTag, SAMPLE_PARTNER_CODES, SEED_TAGS } from '../src/index.ts';
...
beforeAll(async () => {
  ...
  db = await freshDb();
  secondCompany = newId();
  await db.owner.sql`insert into companies (id, tenant_id, code, name) values (${secondCompany}, ${db.tenantId}, 'T2', 'Second Co')`;
  unappliedCompany = newId();
  await db.owner.sql`insert into companies (id, tenant_id, code, name) values (${unappliedCompany}, ${db.tenantId}, 'T3', 'Unapplied Co')`;
});
afterAll(async () => {
  await db.close();
});
...
describe('pack example: apply (AC-3, AC-6)', () => {
  it('AC-3 first apply writes the setting default, seeds 3 tags, records packs.applied; no sample without the flag', async () => {
    ...
    expect(res).toMatchObject({ name: 'example', version: '0.1.0', alreadyApplied: false, seeded: true, sampled: false, settings: { written: [DEFAULT_RANK_KEY], kept: [] } });
    ...
    expect((await companySettings(db.companyId))[PACKS_APPLIED_KEY]).toEqual({ example: { at: T1.toISOString(), version: '0.1.0' } });
  ...
  it('AC-3 second apply is a no-op: nothing written (tag versions, settings, audit rows unchanged)', async () => {
    ...
    expect(res).toMatchObject({ alreadyApplied: true, seeded: false, sampled: false, settings: { written: [] }, record: { at: T1.toISOString(), version: '0.1.0' } });
    ...
    expect(await db.run({}, (ctx) => auditTrail(ctx, 'company_settings', db.companyId))).toHaveLength(auditBefore.length);
  });
  ...
  it("AC-3 an admin's explicit setting is kept on first apply; force overwrites it and re-runs seed (idempotent)", async () => {
    ...
    expect(forced).toMatchObject({ alreadyApplied: true, seeded: true, settings: { written: [DEFAULT_RANK_KEY], kept: [] }, record: { at: T2.toISOString() } });
  ...
  it('AC-3 sample: runs on an applied pack once (2 ranked partners, default rank from the setting), then not again', async () => {
    ...
    expect(res).toMatchObject({ alreadyApplied: true, seeded: false, sampled: true, record: { at: T1.toISOString(), sampledAt: T2.toISOString() } });
    const partners = await db.run({}, (ctx) => repo(ctx, Partner).list({ where: { code: { $in: [...SAMPLE_PARTNER_CODES] } }, orderBy: [{ field: 'code', dir: 'asc' }] }));
  ...
  it('AC-4 pack.apply is admin-only (PERMISSION_DENIED for sales); pack.list shows applied status per company', async () => {
    ...
    await expect(db.run(sales, (ctx) => runAction(ctx, 'pack.apply', { name: 'example' }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run({}, (ctx) => runAction(ctx, 'pack.apply', { name: 'nope' }))).rejects.toMatchObject({ code: 'NOT_FOUND', hint: expect.stringContaining('example') });
    const listed = (await db.run(sales, (ctx) => runAction(ctx, 'pack.list', {}))) as { items: Record<string, unknown>[] };
    expect(listed.items).toEqual([
      { name: 'example', label: ExamplePack.label, version: '0.1.0', depends: ['partner'], hasSample: true, applied: true, appliedAt: T1.toISOString(), appliedVersion: '0.1.0', sampledAt: T2.toISOString() },
    ]);
  });
});
...
describe('pack example: ext on partner and example.hello (AC-6)', () => {
  it('AC-6 partner create with a bad rank -> ValidationError on ext.customerRank; nothing written', async () => {
    ...
    const err = await db.run({}, (ctx) => repo(ctx, Partner).create({ name: 'Bad rank', ext: { customerRank: 'Z' } })).catch((e: unknown) => e);
...
describe('pack example: /meta (AC-5, AC-6)', () => {
  it('AC-5 appMeta: partner label override, extFields with source pack:example, packs[].applied, pack menu', async () => {
    const meta = await db.run({}, async (ctx) => appMeta(ctx, { appliedPacks: Object.keys(await readAppliedPacks(ctx)) }));
    ...
    expect(meta.modules.find((m) => m.name === 'example')?.menus).toEqual([{ label: { ja: 'タグ', en: 'Tags' }, entity: 'example_tag', order: 90 }]);
    expect(meta.entities.find((e) => e.name === 'example_tag')?.module).toBe('example');
    const unapplied = await db.run({ companyId: unappliedCompany, roles: ['viewer'], actor: { type: 'user' as const, id: newId() } }, async (ctx) => appMeta(ctx));
