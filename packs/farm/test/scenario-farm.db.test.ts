...
describe('farm: northern field tomato scenario (AC-1..9)', () => {
  it('seeds only drafts, preserves edits and does not duplicate samples on forced apply', async () => {
    ...
    for (const entity of ['farm_field', 'farm_crop', 'farm_season', 'farm_work', 'farm_harvest', 'farm_material_line']) expect((await s.act<List>(`${entity}.list`, {})).total).toBe(1);
    expect(await s.act('farm_work.get', { id: ids.workId })).toMatchObject({ note: '利用者が確認した作業メモ', docstatus: 0 });
    expect(await s.act('farm_harvest.get', { id: ids.harvestId })).toMatchObject({ quantity: '100', uomCode: 'KGM', valuationAmount: '20000', docstatus: 0 });
  });
  it('runs purchase -> work/material issue -> harvest -> sale -> receipt and season summary', async () => {
    ...
    await s.submit('purchase_invoice', { partnerId: ids.supplierId, date: '2026-05-01', lines: { purchase_invoice_line: [{ productId: ids.materialId, quantity: '50', unitPrice: '100' }] } });
    ...
    const materialBalance = await s.run((ctx) => repo(ctx, StockBalance).list({ where: { productId: ids.materialId, warehouseId: ids.warehouseId } }));
    ...
    const sale = await s.submit('sales_invoice', { partnerId: ids.customerId, date: '2026-07-02', lines: { sales_invoice_line: [{ productId: ids.tomatoId, quantity: '30', unitPrice: '400' }] } });
    expect(sale.total).toBe('12960');
    await s.submit('payment', { direction: 'receive', partnerId: ids.customerId, date: '2026-07-03', amount: sale.total, method: 'cash', lines: { payment_allocation: [{ invoiceEntity: 'sales_invoice', invoiceId: sale.id, amount: sale.total }] } });
    ...
    const tomatoBalance = await s.run((ctx) => repo(ctx, StockBalance).list({ where: { productId: ids.tomatoId, warehouseId: ids.warehouseId } }));
    expect(tomatoBalance.items[0]?.qty.toString()).toBe('70');
    const summary = await s.act<Table>('farm.season_summary', { from: '2026-04-01', to: '2026-10-31', asOf: '2026-10-31' });
    expect(summary.rows).toEqual([expect.objectContaining({ field: '北畑（デモ）', season: '2026年トマト（デモ）', harvestQuantity: '100', unit: 'KGM', harvestValuation: '20000', laborHours: '3', workCount: 1, harvestCount: 1 })]);
    expect(summary.totals).toEqual({ harvestValuation: '20000', laborHours: '3' });
    await expect(s.act('farm_harvest.cancel', { id: ids.harvestId, correctionDate: '2026-07-04' })).rejects.toMatchObject({ code: 'INVALID_STATE' });
    ...
    expect(await s.act('farm.close_season', { seasonId: ids.seasonId, closedDate: '2026-10-31' })).toMatchObject({ closedDate: '2026-10-31' });
    await expect(s.act('farm_work.create', { seasonId: ids.seasonId, date: '2026-07-04', activity: 'other', laborHours: '1' })).rejects.toMatchObject({ code: 'INVALID_STATE' });
  });
});
