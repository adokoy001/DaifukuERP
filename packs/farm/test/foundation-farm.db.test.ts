...
async function harvest(seasonId: string, extra: Record<string, unknown> = {}): Promise<Doc> {
  return s.act('farm_harvest.create', {
    seasonId,
    date: '2026-07-01',
...
describe('farm integrity and effective history', () => {
  it('rejects service crops, invalid dates, nonpositive quantities, and forged owned fields', async () => {
    ...
    await expect(s.act('farm_crop.create', { code: 'BAD', name: 'bad', productId: serviceId })).rejects.toMatchObject({
      code: 'VALIDATION',
    });
    ...
    await expect(
      s.act('farm_harvest.submit', { id: future.id }, { now: () => new Date('2026-06-30T03:00:00Z') }),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
    expect(await s.act('farm_harvest.get', { id: future.id })).toMatchObject({ docstatus: 0, stockEntryId: null });
    await expect(harvest(active.id, { productId: ids.materialId })).rejects.toMatchObject({
      code: 'PERMISSION_DENIED',
    });
    await expect(harvest(active.id, { stockEntryId: newId() })).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(
      s.act('farm_season.update', { id: active.id, patch: { closedDate: '2026-10-31' } }),
  ...
  it('rolls back parent submit on material shortage and freezes all submitted material edits', async () => {
    ...
    const work = await s.act('farm_work.create', {
      seasonId: active.id,
      date: '2026-05-02',
      activity: 'fertilizing',
      laborHours: '1',
      warehouseId: ids.warehouseId,
      lines: { farm_material_line: [{ productId: ids.materialId, quantity: '10' }] },
    });
    ...
    await s.submit('stock_entry', {
      type: 'receipt',
      date: '2026-05-01',
      warehouseId: ids.warehouseId,
      lines: { stock_entry_line: [{ productId: ids.materialId, quantity: '20', unitCost: '100' }] },
    });
    ...
    await expect(s.act('farm_material_line.update', { id: line.id, patch: { quantity: '9' } })).rejects.toMatchObject({
      code: 'INVALID_STATE',
    });
    await expect(s.act('farm_material_line.delete', { id: line.id })).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(s.act('stock_entry.cancel', { id: posted.stockEntryId })).rejects.toMatchObject({
      code: 'HAS_DEPENDENTS',
    });
    await s.act('farm_work.cancel', { id: work.id, correctionDate: '2026-05-03' });
    const balance = await s.run((ctx) =>
  ...
  it('serializes duplicate harvest submission, blocks child cancellation, and drops owned links on amend', async () => {
    ...
    const results = await Promise.allSettled([
      s.act('farm_harvest.submit', { id: draft.id }),
      s.act('farm_harvest.submit', { id: draft.id }),
    ]);
    ...
    await expect(s.act('farm_harvest.update', { id: draft.id, patch: { quantity: '1' } })).rejects.toMatchObject({
      code: 'INVALID_STATE',
    });
    await expect(s.act('farm_harvest.update', { id: draft.id, patch: { stockEntryId: null } })).rejects.toMatchObject({
      code: 'PERMISSION_DENIED',
    });
  ...
  it('refuses closing drafts and serializes creation against closing without orphan records', async () => {
    ...
    await expect(
      s.act('farm.close_season', { seasonId: ids.seasonId, closedDate: '2026-10-31' }),
    ...
    await expect(s.act('farm.close_season', { seasonId: late.id, closedDate: '2026-09-29' })).rejects.toMatchObject({
      code: 'VALIDATION',
    });
    ...
    await expect(s.act('farm.close_season', { seasonId: active.id, closedDate: '2027-01-01' })).rejects.toMatchObject({
      code: expect.stringMatching(/VALIDATION|INVALID_STATE/),
    });
  });
  it('keeps company and tenant references isolated and enables only applied pack companies', async () => {
    const companyId = newId();
    await s.db.owner
      .sql`insert into companies (id, tenant_id, code, name) values (${companyId}, ${s.db.tenantId}, 'FARM-B', 'Farm B')`;
    ...
    await expect(s.act('farm_harvest.get', { id: ids.harvestId }, { companyId })).rejects.toMatchObject({
      code: 'NOT_FOUND',
    });
    await expect(
      s.act(
    ...
    await s.db.owner
      .sql`insert into companies (id, tenant_id, code, name) values (${tenantCompany}, ${tenantId}, 'OTHER', 'Other')`;
    await s.act('pack.apply', { name: 'farm' }, { tenantId, companyId: tenantCompany });
    await expect(
      s.act('farm_harvest.get', { id: ids.harvestId }, { tenantId, companyId: tenantCompany }),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
  });
  it('aggregates beyond the repository 500-row page without losing labor hours', async () => {
    ...
    await s.db.owner
      .sql`insert into farm_work (id, tenant_id, company_id, season_id, date, activity, labor_hours, docstatus, number)
      select gen_random_uuid(), ${s.db.tenantId}, ${s.db.companyId}, ${active.id}, '2026-09-01', 'other', 1, 1, 'PAGE-' || n from generate_series(1,501) n`;
    const report = await s.act<Table>('farm.season_summary', {
      from: '2026-09-01',
      to: '2026-09-01',
      asOf: '2026-09-01',
    });
  ...
  it('propagates correction dates after inventory close and keeps earlier reports stable', async () => {
    ...
    expect(await s.act('farm_harvest.get', { id: draft.id })).toMatchObject({
      docstatus: 2,
      cancelledDate: '2026-08-02',
    });
