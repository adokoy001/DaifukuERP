...
export const FarmField = defineEntity({
  name: 'farm_field',
  label: label('圃場', 'Farm field'),
  fields: {
    code: f.text({
      label: label('圃場コード', 'Field code'),
      required: true,
      unique: true,
      immutable: true,
      maxLength: 40,
  ...
  permissions,
  views: {
    list: ['code', 'name', 'areaM2', 'location', 'isActive'],
    search: ['code', 'name'],
    form: [
      ['code', 'name'],
      ['areaM2', 'location'],
...
export const FarmCrop = defineEntity({
  ...
  permissions,
  views: {
    list: ['code', 'name', 'productId', 'variety'],
    search: ['code', 'name'],
    form: [['code', 'name'], ['productId', 'variety'], ['note']],
  },
});
