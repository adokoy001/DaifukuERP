...
export const FarmWork = defineDocument({
  name: 'farm_work',
  label: label('農作業・資材投入', 'Farm work and material use'),
  naming: { type: 'sequence', prefix: 'FWK-', period: 'year' },
  fields: {
    sampleKey: f.text({
      label: label('サンプル識別キー', 'Sample key'),
      unique: true,
      immutable: true,
      serverOwned: true,
      maxLength: 80,
    ...
    activity: f.enum(FARM_ACTIVITIES, {
      label: label('作業区分', 'Activity'),
      required: true,
      labels: {
        sowing: label('播種', 'Sowing'),
        planting: label('定植', 'Planting'),
        fertilizing: label('施肥', 'Fertilizing'),
        weeding: label('除草', 'Weeding'),
        irrigation: label('灌水', 'Irrigation'),
        other: label('その他', 'Other'),
      },
    }),
    laborHours: f.quantity({
      label: label('延べ作業時間（人時）', 'Labor (person-hours)'),
      required: true,
      default: '0',
      min: '0',
    }),
  ...
  lines: [{ entity: 'farm_material_line', parentField: 'workId' }],
  permissions: {
    roles: {
  ...
  views: {
    list: ['date', 'seasonId', 'activity', 'laborHours', 'warehouseId', 'stockEntryId'],
    search: ['note'],
    form: [
      ['seasonId', 'date'],
      ['activity', 'laborHours', 'warehouseId'],
      ['note'],
      ['stockEntryId', 'cancelledDate'],
    ],
  },
});

export const FarmMaterialLine = defineEntity({
  name: 'farm_material_line',
  label: label('資材投入明細', 'Material input'),
  fields: {
    workId: f.ref(FarmWork.name, {
      label: label('農作業', 'Farm work'),
      required: true,
      immutable: true,
      onDelete: 'cascade',
    }),
