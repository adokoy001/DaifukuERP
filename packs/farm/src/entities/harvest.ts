...
export const FarmHarvest = defineDocument({
  ...
  fields: {
    ...
    valuationUnitCost: f.money({
      label: label('評価単価（円/在庫単位）', 'Valuation unit cost (JPY/stock unit)'),
      description: label(
        '利用者が明示入力。生産原価の自動計算ではありません',
        'Explicit input; not an automatic production cost calculation',
      ),
      required: true,
      min: '0',
      scale: 6,
    }),
    valuationAmount: f.money({
      label: label('収穫評価額（円）', 'Harvest valuation (JPY)'),
      required: true,
      default: '0',
      scale: 6,
      serverOwned: true,
  ...
  views: {
    list: ['date', 'seasonId', 'quantity', 'uomCode', 'valuationAmount', 'warehouseId'],
    search: ['note'],
    form: [
      ['seasonId', 'date', 'warehouseId'],
      ['quantity', 'valuationUnitCost'],
      ['productId', 'uomCode', 'valuationAmount'],
