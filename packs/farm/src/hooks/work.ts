...
async function beforeSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  if (!decimal(row.laborHours).gt(0) && lines.total === 0) invalid('laborHours', 'Enter labor hours or material input.');
...
async function afterSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  const lines = (await repo(ctx, FarmMaterialLine).list({ where: { workId: String(row.id) }, orderBy: [{ field: 'seq', dir: 'asc' }], limit: 500 })).items;
  if (lines.length > 0) await createStock(ctx, FarmWork, row, 'issue', lines.map((line) => ({ productId: line.productId, quantity: line.quantity })));
}
async function beforeCancel(ctx: Context, args: HookArgs): Promise<void> {
  await cancellationDate(ctx, args);
  if (!args.row.stockEntryId && await repo(ctx, FarmMaterialLine).count({ workId: String(args.row.id) })) throw new StateError('Submitted material input has no stock issue', 'Restore the missing linkage before cancelling.');
}
