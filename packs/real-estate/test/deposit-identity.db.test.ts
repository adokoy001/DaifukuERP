...
describe('deposit identity and ledger ownership (A3)', () => {
  it('rejects mismatched partner/unit/null on create and derives omitted references', async () => {
    for (const wrong of [{ partnerId: otherPartnerId }, { unitId: otherUnitId }, { unitId: null }]) {
      const error = await caught(
        s.act(D, 'real_estate_deposit.create', { contractId, partnerId, unitId, amount: '100', ...wrong }),
      );
      expect(error).toMatchObject({
        code: 'VALIDATION',
        details: { issues: expect.arrayContaining([expect.objectContaining({ path: Object.keys(wrong)[0] })]) },
      });
    }
  ...
  it('rejects retargeting and amount changes after receipt; return keeps the original counterparty', async () => {
    ...
    for (const patch of [{ partnerId: otherPartnerId }, { unitId: otherUnitId }, { unitId: null }, { amount: '1' }]) {
      expect(await caught(s.act(D, 'real_estate_deposit.update', { id, patch }, { roles: ['sales'] }))).toMatchObject({
        code: 'INVALID_STATE',
      });
    }
    expect(await s.act(D, 'real_estate_deposit.get', { id })).toMatchObject({
      partnerId,
      unitId,
      amount: '100',
      journalEntryId: received.journalEntryId,
    });
    const returned = await s.act<Row>('2026-11-02', 'real_estate.return_deposit', {
      depositId: id,
      date: '2026-11-02',
      amount: '100',
    });
    const lines = await s.run(D, (ctx) =>
      repo(ctx, JournalLine).list({
        where: { entryId: { $in: [String(received.journalEntryId), String(returned.returnJournalEntryId)] } },
        limit: 20,
      }),
    );
  ...
  it('rejects public ledger field writes while private receipt/return remain usable', async () => {
    ...
    for (const patch of [{ receivedDate: D }, { returnedAmount: '0' }, { returnJournalEntryId: null }]) {
      expect(await caught(s.act(D, 'real_estate_deposit.update', { id, patch }))).toMatchObject({
        code: 'PERMISSION_DENIED',
      });
    }
    expect(
      await caught(
        s.act(D, 'real_estate_deposit.create', { contractId, partnerId, unitId, amount: '100', receivedDate: D }),
      ),
  ...
  it('serializes concurrent receipt/return and never posts a second return', async () => {
    ...
    const returns = await Promise.allSettled([
      s.act('2026-11-03', 'real_estate.return_deposit', { depositId: id, date: '2026-11-03', amount: '100' }),
      s.act('2026-11-03', 'real_estate.return_deposit', { depositId: id, date: '2026-11-03', amount: '100' }),
    ]);
    ...
    const entries = await s.act<ListJson>(D, 'journal_entry.list', {
      where: { sourceEntity: 'real_estate_deposit_return', sourceId: id },
    });
