...
export type InvoiceLineJson = Row & {
  seq: number;
  description: string;
  quantity: string;
  unitPrice: string;
...
export type InvoiceJson = DocJson & {
  partnerId: string;
  date: string;
  dueDate: string | null;
  subtotal: string;
...
export function scenario(getDb: () => TestDb) {
  ...
  const run = <T>(date: string, fn: (ctx: Context) => Promise<T>, params: Partial<ContextParams> = {}): Promise<T> =>
    getDb().run({ now: () => at(date), ...params }, fn);
  const act = <T = Row>(date: string, name: string, input: unknown, params: Partial<ContextParams> = {}): Promise<T> =>
    run(date, (ctx) => runAction(ctx, name, input), params) as Promise<T>;
  ...
  const seedModules = async (date: string): Promise<string[]> => {
    ...
    for (const m of registry.allModules()) {
      if (!m.seed) continue;
      await withContext(db.owner, systemParams(db.tenantId, db.companyId, { now: () => at(date) }), async (ctx) =>
        m.seed?.(ctx),
      );
      seeded.push(m.name);
    }
  ...
  const entryLines = async (entryId: string | null): Promise<LineTriple[]> => {
    if (!entryId) return [];
    const lines = await run('2026-12-31', (ctx) =>
      repo(ctx, JournalLine).list({ where: { entryId }, orderBy: [{ field: 'seq', dir: 'asc' }], limit: 100 }),
    );
