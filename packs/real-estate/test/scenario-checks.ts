...
type LeaseJson = DocJson & {
  title: string;
  startDate: string;
  ext: Row;
  partnerId: string;
  lines: { contract_line: Row[] };
};
...
export async function checkApply(s: S, st: ScriptState): Promise<void> {
  ...
  expect(res).toMatchObject({
    ...
    settings: {
      written: [
        'sales.accounts',
        'contract.default_proration',
        'contract.auto_submit',
        'tax.price_includes_tax',
        'real_estate.accounts',
      ],
      kept: [],
    },
  });
  ...
  for (const code of ['1100', '1300', '2200', '2500', '3000', '4100', '4200', '4210'])
    expect(st.acc[code], `account ${code}`).toBeDefined();
  const accounts = await s.act<ListJson>(D, 'account.list', {
    where: { code: { $in: ['4200', '4210'] } },
  ...
  const products = await s.act<ListJson>(D, 'product.list', {
    where: { code: { $in: ['RENT', 'RENT_TAXABLE', 'KEY_MONEY'] } },
    orderBy: [{ field: 'code', dir: 'asc' }],
  });
  ...
  expect(
    (await s.act<ListJson>(D, 'real_estate_unit.list', { orderBy: [{ field: 'code', dir: 'asc' }] })).items.map(
      (u) => u.version,
    ),
  ).toEqual(units.items.map((u) => u.version));
...
async function checkSample(s: S, st: ScriptState): Promise<void> {
  ...
  const tenants = await s.act<ListJson>(D, 'partner.list', {
    where: { code: { $in: ['T1', 'T2', 'T3', 'T4'] } },
    orderBy: [{ field: 'code', dir: 'asc' }],
  });
  ...
  const leases = await s.act<ListJson>(D, 'contract.list', {
    where: { 'ext.unitId': { $in: Object.values(st.unit) } },
    orderBy: [{ field: 'startDate', dir: 'asc' }],
  });
  const full = await Promise.all(leases.items.map((l) => s.act<LeaseJson>(D, 'contract.get', { id: l.id })));
  const shape = full.map((l) => [
    byTenant[l.partnerId],
    l.startDate,
    l.docstatus,
    l.billingDay,
    l.billingTiming,
    l.ext,
    l.lines.contract_line.map((x) => [x.description, x.unitPrice, x.taxCategory]),
  ]);
  expect(shape.sort((a, b) => String(a[0]).localeCompare(String(b[0])))).toEqual([
    [
      ...
      1,
      'advance',
      { unitId: st.unit['101'], keyMoney: '0', depositMonths: '0', renewalFee: '0' },
      [['家賃', '60000', 'non_taxable']],
    ],
    [
      'T2',
      '2026-11-11',
      ...
      1,
      'advance',
      { unitId: st.unit['102'], keyMoney: '65000', depositMonths: '1', renewalFee: '0' },
      [['家賃', '65000', 'non_taxable']],
    ],
    [
      'T3',
      '2026-11-01',
      ...
      1,
      'advance',
      { unitId: st.unit['201'], keyMoney: '100000', depositMonths: '2', renewalFee: '0' },
      [['家賃', '100000', 'standard']],
    ],
    [
      'T4',
      ...
      1,
      'advance',
      { unitId: st.unit.P1, keyMoney: '0', depositMonths: '0', renewalFee: '0' },
      [['駐車場代', '8000', 'standard']],
    ],
  ]);
...
export async function checkExtAndDefaults(s: S, st: ScriptState): Promise<void> {
  ...
  expect(badKind).toMatchObject({
    code: 'VALIDATION',
    details: { issues: [expect.objectContaining({ path: 'ext.tenantKind' })] },
  });
  const noUnit = await caught(
    s.act(D, 'contract.create', {
      partnerId: st.tenant.T1,
      title: 'x',
      startDate: '2026-12-01',
      ext: { unitId: newId() },
      lines: { contract_line: [{ description: 'x', unitPrice: '1' }] },
    }),
  );
  expect(noUnit).toMatchObject({
    code: 'VALIDATION',
    details: { issues: [{ path: 'ext.unitId', message: 'unit not found' }] },
  });
  expect(
    (await s.act<ListJson>(D, 'contract.list', { where: { 'ext.unitId': st.unit['201'] } })).items.map(
      (c) => c.partnerId,
    ),
  ).toEqual([st.tenant.T3]);
  expect(
    (await s.act<ListJson>(D, 'partner.list', { where: { 'ext.tenantKind': 'corporate' } })).items.map((p) => p.code),
  ).toEqual(['T3']);
  ...
  const draft = await s.act<LeaseJson>(D, 'contract.create', {
    ...
    lines: {
      contract_line: [
        { description: '家賃', unitPrice: '1000' },
        { description: '明示', unitPrice: '1000', taxCategory: 'standard' },
        { productId: rentTaxable, unitPrice: '1000' },
      ],
    },
  });
  ...
  const monthly = await s.act<LeaseJson>(D, 'contract.create', {
    partnerId: st.tenant.T1,
    title: '短期',
    startDate: '2026-12-01',
    endDate: '2026-12-20',
    ext: { unitId: st.unit['101'] },
    lines: { contract_line: [{ description: '短期滞在', unitPrice: '1000' }] },
  });
  expect(monthly.lines.contract_line.map((l) => l.taxCategory)).toEqual(['standard']);
  const plain = await s.act<LeaseJson>(D, 'contract.create', {
    partnerId: st.tenant.T1,
    title: '保守契約（部屋なし）',
    startDate: '2026-12-01',
    lines: { contract_line: [{ description: '保守', unitPrice: '1000', taxCategory: 'standard' }] },
  });
  ...
  expect(ext('contract')).toEqual(
    ['unitId', 'keyMoney', 'depositMonths', 'renewalFee'].map((k) => [`ext.${k}`, 'pack:real_estate']),
  );
  expect(ext('partner')).toEqual(['tenantKind', 'emergencyContact'].map((k) => [`ext.${k}`, 'pack:real_estate']));
  expect(meta.packs).toContainEqual({
    name: 'real_estate',
    label: { ja: '賃貸管理（不動産）', en: 'Rental management (real estate)' },
    applied: true,
  });
  expect(meta.modules.find((m) => m.name === 'real_estate')?.menus.map((m) => m.label.ja)).toEqual([
    '物件',
    '部屋・区画',
    '賃貸借契約',
    '敷金台帳',
  ...
  expect(await caught(s.act(D, 'real_estate.move_in', { contractId: st.lease.T1 }, viewer))).toMatchObject({
...
export async function checkAfterScript(s: S, st: ScriptState): Promise<void> {
  ...
  expect(await s.act('2026-12-25', 'real_estate.move_out', { contractId: T2, endDate: '2026-12-31' })).toEqual({
    contractId: T2,
    endDate: '2026-12-31',
  ...
  const ret = await s.act<DocJson & Row & { returnJournalEntryId: string }>(
    '2026-12-31',
    'real_estate.return_deposit',
    { depositId: st.deposit.T2, date: '2026-12-31', amount: '65000', deductionAmount: '22000' },
  );
  expect(ret).toMatchObject({
    amount: '65000',
    receivedDate: '2026-11-10',
    returnedDate: '2026-12-31',
    returnedAmount: '43000',
  ...
  expect(await s.act('2027-01-04', 'real_estate.move_out', { contractId: T2, endDate: '2026-12-31' })).toMatchObject({
  ...
  expect(
    await code(
      s.act('2026-12-31', 'real_estate.return_deposit', {
        depositId: st.deposit.T2,
        date: '2026-12-31',
        amount: '65000',
      }),
    ),
  ...
  expect(
    await code(s.act('2026-12-31', 'real_estate.receive_deposit', { depositId: st.deposit.T3, date: '2026-12-31' })),
  ).toBe('INVALID_STATE');
  expect(await code(s.act('2026-12-31', 'real_estate.move_in', { contractId: st.lease.T3 }))).toBe('INVALID_STATE');
  expect(
    await caught(
      s.act('2026-12-31', 'real_estate.return_deposit', {
        depositId: st.deposit.T3,
        date: '2026-12-31',
        amount: '100000',
      }),
    ),
  ).toMatchObject({ code: 'VALIDATION', details: { issues: [expect.objectContaining({ path: 'amount' })] } });
  expect(
    await code(
      s.act('2026-12-31', 'real_estate_deposit.update', { id: st.deposit.T3, patch: { receivedDate: '2026-12-01' } }),
    ),
  ).toBe('PERMISSION_DENIED');
  expect(
    await code(s.act('2026-12-31', 'real_estate_deposit.update', { id: st.deposit.T3, patch: { amount: '1' } })),
  ).toBe('INVALID_STATE');
  ...
  expect(
    await s.act('2026-12-31', 'real_estate_unit.update', {
      id: st.unit['101'],
      patch: { status: 'vacant', note: '改ざんの確認' },
    }),
  ).toMatchObject({ status: 'occupied', note: '改ざんの確認' });
}
