...
export const SEED_PRODUCTS: readonly {
...
}[] = [
  {
    ...
    description: '住宅の家賃（非課税）。事務所・店舗・駐車場には RENT_TAXABLE',
  },
  {
    code: 'RENT_TAXABLE',
    name: '家賃（課税）',
  ...
  },
  {
    ...
    kind: 'service',
    taxCategory: 'non_taxable',
    description: '住宅の礼金（非課税）。real_estate.move_in は部屋の用途から税区分を決める',
  },
];

async function seedMissing<T extends { code: string }>(
  codes: readonly T[],
  existing: () => Promise<{ code: string | null }[]>,
...
export async function seedRealEstate(ctx: Context): Promise<void> {
  ...
  await seedMissing(
    SEED_ACCOUNTS,
    async () =>
      (await accounts.list({ where: { code: { $in: SEED_ACCOUNTS.map((a) => a.code) } }, limit: SEED_ACCOUNTS.length }))
        .items,
    (a) => accounts.create(a),
  );
  ...
  await seedMissing(
    SEED_PRODUCTS,
    async () =>
      (await products.list({ where: { code: { $in: SEED_PRODUCTS.map((p) => p.code) } }, limit: SEED_PRODUCTS.length }))
        .items,
    (p) => products.create(p),
  );
}
