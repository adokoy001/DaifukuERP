...
import { Decimal, defineWriteCapability, isDecimal, isUuid, registry, repo, StateError, ValidationError, withWriteCapability, type Context, type HookArgs } from '@daifuku/kernel';
...
export const DEPOSIT_LEDGER_HINT = 'Receipt and return are recorded by real_estate.receive_deposit / real_estate.return_deposit, which also post the journal entries.';

const writers = new WeakMap<Context, number>();
const depositWrite = defineWriteCapability({ name: 'real-estate-deposit-ledger', entity: RealEstateDeposit.name, fields: DEPOSIT_SYSTEM_FIELDS, operations: ['update'] });
...
export async function assertDepositLease(ctx: Context, row: Record<string, unknown>): Promise<void> {
  ...
  if (issues.length) throw new ValidationError('The deposit identity does not match its lease', issues, 'Choose the lease partner and unit. Existing unreceived deposits can be deleted and recreated before receipt.');
}

async function beforeValidate(ctx: Context, { row, previous }: HookArgs): Promise<void> {
  if (!previous) {
    if (!isDepositWrite(ctx)) Object.assign(row, { receivedDate: null, returnedDate: null, returnedAmount: '0', deductionAmount: '0', journalEntryId: null, returnJournalEntryId: null });
    await assertDepositLease(ctx, row);
    return;
  }
  if (isDepositWrite(ctx)) return;
  const touched: string[] = [...DEPOSIT_SYSTEM_FIELDS, 'contractId', 'partnerId', 'unitId'].filter((k) => row[k] !== undefined && !sameValue(row[k], previous[k]));
  if (previous.journalEntryId !== null && row.amount !== undefined && !sameValue(row.amount, previous.amount)) touched.push('amount');
  if (touched.length > 0) {
    throw new StateError(`real_estate_deposit ${String(previous.id)}: ${touched.join(', ')} cannot be changed directly`, DEPOSIT_LEDGER_HINT, { fields: touched });
  }
}
...
async function beforeDelete(_ctx: Context, { row }: HookArgs): Promise<void> {
  if (row.journalEntryId !== null && row.journalEntryId !== undefined) {
    throw new StateError(`real_estate_deposit ${String(row.id)} has been received and cannot be deleted`, 'Return the deposit with real_estate.return_deposit instead.', { journalEntryId: row.journalEntryId });
  }
}
