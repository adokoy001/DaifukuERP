...
export const RECEIVED_HINT =
  'A deposit can be received only once. Check the linked receipt and deposit ledger; a receipt correction workflow is not supported yet.';
...
export async function receiveDeposit(ctx: Context, input: ReceiveDepositInput): Promise<DepositRow> {
  ...
  if (deposit.journalEntryId !== null || deposit.receivedDate !== null) {
    throw new StateError(
      `real_estate_deposit ${deposit.id} was already received on ${String(deposit.receivedDate)}`,
      RECEIVED_HINT,
      { depositId: deposit.id, journalEntryId: deposit.journalEntryId },
    );
  }
  if (!deposit.amount.gt(0))
    throw new ValidationError(
      `real_estate_deposit ${deposit.id} amount must be greater than 0`,
      [{ path: 'amount', message: 'must be > 0' }],
      'Set the deposit amount, then receive it.',
    );
  ...
  const entry = await postFromSource(ctx, {
    ...
    lines: [
      { accountId: bankId, debit: deposit.amount, partnerId: deposit.partnerId, memo: '敷金受領' },
      {
        ...
        taxCategory: DEPOSIT_TAX_CATEGORY,
        memo: '預り金（敷金）',
      },
    ],
  });
  return withDepositWrite(ctx, (owned) =>
    repo(owned, RealEstateDeposit).update(deposit.id, { receivedDate: input.date, journalEntryId: entry.id }),
  );
}
