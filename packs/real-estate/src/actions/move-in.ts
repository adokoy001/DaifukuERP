...
export const moveInInput = z.object({
  contractId: z.uuid(),
  depositReceivedDate: localDate.optional(),
  accountId: z.uuid().optional(),
});
export type MoveInInput = z.output<typeof moveInInput>;
export const moveInOutput = z.object({
  invoiceId: z.uuid(),
...
export function assertLease(contract: ContractRow): string {
  ...
  if (contract.docstatus !== DOCSTATUS.submitted || contract.status !== 'active' || unitId === null) {
    throw new StateError(
      `contract ${contract.number ?? contract.id} is not an active lease (docstatus ${contract.docstatus}, status ${contract.status}, unit ${unitId ?? 'none'})`,
      LEASE_HINT,
      { contractId: contract.id, docstatus: contract.docstatus, status: contract.status, unitId },
    );
  }
  return unitId;
}

async function addKeyMoneyLine(
  ctx: Context,
  invoiceId: string,
  keyMoney: Decimal,
  taxCategory: RentTaxCategory,
): Promise<void> {
  ...
  await repo(ctx, SalesInvoiceLine).create({
    invoiceId,
    seq,
    productId: product?.id ?? null,
    description: KEY_MONEY_DESCRIPTION,
...
async function leaseDeposit(ctx: Context, contract: ContractRow, unitId: string): Promise<string | null> {
  ...
  const created = await r.create({
    ...
    unitId,
    amount: months.times(monthly).round(contract.roundingMode, scale),
  });
  return created.id;
}
...
export async function moveIn(ctx: Context, input: MoveInInput): Promise<MoveInResult> {
  ...
  const billed = (await repo(ctx, ContractBilling).list({ where: { contractId: contract.id, period }, limit: 1 }))
    .items[0];
  if (billed)
    throw new StateError(
      `contract ${contract.number ?? contract.id} has already moved in (${period} is billed)`,
      'Bill later months with contract.generate_invoices.',
      { contractId: contract.id, period, invoiceId: billed.invoiceId },
    );
  await refreshUnitStatus(ctx, unitId);
  const generated = generateInvoicesOutput.parse(
    await runAction(ctx, 'contract.generate_invoices', { period, contractId: contract.id, submit: false }),
  );
  const created = generated.created[0];
  if (!created)
    throw new StateError(
      `contract ${contract.number ?? contract.id}: no invoice for ${period} (${generated.skipped[0]?.reason ?? 'unknown'})`,
      LEASE_HINT,
  ...
  if (input.depositReceivedDate !== undefined) {
    if (depositId === null)
      throw new ValidationError(
        `contract ${contract.number ?? contract.id} takes no deposit`,
        [{ path: 'depositReceivedDate', message: 'the lease has depositMonths 0' }],
        'Omit depositReceivedDate, or set ext.depositMonths on the lease before submitting it.',
      );
    await receiveDeposit(ctx, {
      depositId,
      date: input.depositReceivedDate,
