...
export async function setup(): Promise<Fixture> {
  ...
  for (const m of [
    ...
    if (m.seed)
      await withContext(db.owner, systemParams(db.tenantId, db.companyId, { now }), async (ctx) => m.seed?.(ctx));
  }
...
export async function stock(s: Fixture, code: string, rice = '10', chicken = '5'): Promise<Row> {
  ...
  const entry = await s.act('stock_entry.create', {
    type: 'receipt',
...
export async function closing(s: Fixture, code = 'RC-A', patch: Record<string, unknown> = {}): Promise<Row> {
  return s.act('restaurant_chain_closing.create', {
    storeId: s.ids[code],
    date: '2026-09-12',
    cashAmount: '10000',
    cardAmount: '5000',
...
export const submit = (s: Fixture, row: Row) =>
  s.act('restaurant_chain_closing.submit', { id: row.id, expectedVersion: row.version });
