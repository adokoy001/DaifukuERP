...
describe('飲食店二店舗の手計算台本', () => {
  ...
  it('青葉店: 店内10食・持帰5食、現金と売掛、材料消費と廃棄が一致する', async () => {
    ...
    expect(a).toMatchObject({ docstatus: 1, subtotal: '15000', taxTotal: '1400', total: '16400', quantity: '15', consumptionCost: '3000', wasteCost: '50' });
    ...
    expect(balances.items.find((row) => row.productId === s.ids['RC-RICE'])).toMatchObject({ qty: '6.9', value: '3450' });
    expect(balances.items.find((row) => row.productId === s.ids['RC-CHICKEN'])).toMatchObject({ qty: '3.5', value: '3500' });
  ...
  it('港店: 同日4食は別店舗で確定でき、全店と店舗指定の集計が一致する', async () => {
    ...
    b = await submit(s, await closing(s, 'RC-B', { cashAmount: '4400', cardAmount: '0', qrAmount: '0', lines: { restaurant_chain_closing_line: [{ recipeId: s.ids['RC-CURRY-V1'], quantity: '4' }] } }));
    expect(b).toMatchObject({ total: '4400', consumptionCost: '800', wasteCost: '0', wasteEntryId: null });
    const report = await s.act<{ rows: Row[]; totals: Row }>('restaurant_chain.daily_summary', { from: '2026-09-12', to: '2026-09-12' });
    expect(report.rows).toHaveLength(2);
    expect(report.totals).toMatchObject({ subtotal: '19000', taxTotal: '1800', total: '20800', cashAmount: '14400', cardAmount: '5000', qrAmount: '1400', quantity: '19', consumptionCost: '3800', wasteCost: '50' });
    const single = await s.act<{ rows: Row[]; totals: Row }>('restaurant_chain.daily_summary', { from: '2026-09-12', to: '2026-09-12', storeId: s.ids['RC-B'] });
    expect(single.rows).toHaveLength(1); expect(single.totals.total).toBe('4400');
  });
  it('カード・QRの後日銀行入金は既存入出金で消込み、追加入金がある締めの取消は全体を保全する', async () => {
    const invoice = await s.act('sales_invoice.get', { id: a.salesInvoiceId });
    const receipt = await s.act('payment.create', { direction: 'receive', partnerId: invoice.partnerId, date: '2026-09-20', amount: '6400', method: 'bank_transfer', accountId: s.ids['1100'], lines: { payment_allocation: [{ invoiceEntity: 'sales_invoice', invoiceId: invoice.id, amount: '6400' }] } });
    ...
    expect(await s.act('sales_invoice.get', { id: invoice.id })).toMatchObject({ paidAmount: '10000', balance: '6400' });
  });
  it('生成伝票の単独取消は禁止し、閉期後は訂正日へ売上・現金・材料を一括取消する', async () => {
    ...
    await expect(s.act('restaurant_chain_closing.cancel', { id: a.id })).rejects.toThrow();
    a = await s.act('restaurant_chain_closing.cancel', { id: a.id, correctionDate: '2026-10-01', expectedVersion: a.version });
    expect(a).toMatchObject({ docstatus: 2, cancelledDate: '2026-10-01' });
    for (const [entity, id] of [['sales_invoice', a.salesInvoiceId], ['payment', a.paymentId], ['stock_entry', a.consumptionEntryId], ['stock_entry', a.wasteEntryId]]) {
      expect(await s.act(`${entity}.get`, { id })).toMatchObject({ docstatus: 2 });
    }
    const september = await s.act<{ totals: Row }>('restaurant_chain.daily_summary', { from: '2026-09-01', to: '2026-09-30' });
    const october = await s.act<{ totals: Row }>('restaurant_chain.daily_summary', { from: '2026-10-01', to: '2026-10-31' });
    expect(september.totals.total).toBe('20800');
    expect(october.totals).toMatchObject({ total: '-16400', taxTotal: '-1400', consumptionCost: '-3000', wasteCost: '-50' });
    const invoice = await s.act('sales_invoice.get', { id: a.salesInvoiceId });
    const reversals = await s.act<Page>('journal_entry.list', { where: { reversalOf: invoice.journalEntryId } });
    expect(reversals.items).toHaveLength(1); expect(reversals.items[0]?.date).toBe('2026-10-01');
    const stockReversals = await s.act<Page>('stock_ledger.list', { where: { sourceId: a.consumptionEntryId, reversal: true } });
    expect(stockReversals.items).toHaveLength(2); expect(stockReversals.items.every((row) => row.date === '2026-10-01')).toBe(true);
    const store = await s.act('restaurant_chain_store.get', { id: s.ids['RC-A'] });
    const balances = await s.act<Page>('stock_balance.list', { where: { warehouseId: store.warehouseId } });
    expect(balances.items.find((row) => row.productId === s.ids['RC-RICE'])).toMatchObject({ qty: '10', value: '5000' });
  });
});
