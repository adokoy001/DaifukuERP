...
export async function operationsSnapshot(ctx: Context, input: OperationsInput): Promise<OperationsSnapshot> {
  ...
  return {
    range,
    overview,
    series: seriesTable(facts, board, range),
    stores: storesTable(data.stores, facts, previous, board),
    submissions: submissionsTable(board),
    sourceTable: sourceTable(facts),
  };
}
export const operationsSnapshotAction = defineAction({
  name: 'restaurant_chain.operations_snapshot',
  description: label('チェーン運営ボード', 'Chain operations snapshot'),
  input: operationsInput,
  output: operationsResult,
  permission: { entity: RestaurantClosing.name, op: 'read' },
  storeAccess: true,
  exportEntities: [RestaurantClosing.name, RestaurantStore.name, RestaurantDayPlan.name],
  tx: 'none',
  mutates: false,
  handler: operationsSnapshot,
});
export const operationsSourcesAction = defineAction({
  name: 'restaurant_chain.operations_sources',
  description: label('運営集計の元伝票・CSV', 'Operations source evidence and CSV'),
  input: operationsInput,
  output: tableResult,
  permission: { entity: RestaurantClosing.name, op: 'read' },
  storeAccess: true,
