...
export async function settlementEvidence(ctx: Context, input: OperationsInput): Promise<TableResult> {
  ...
  for (const id of invoiceIds.sort()) {
    ...
    if (invoice.docstatus === 0 || (invoice.docstatus === 2 && !invoice.cancelledDate))
      throw new StateError(
        '請求書の確定・取消履歴が不明です',
        'Restore its posting and cancellation dates before reporting.',
      );
    if (
      invoice.date <= range.asOf &&
      (invoice.docstatus === 1 || (invoice.cancelledDate && invoice.cancelledDate > range.asOf))
    )
  ...
  for (const closing of closings) {
    ...
    if (!invoice) continue;
    if (!stores.has(closing.storeId))
      stores.set(closing.storeId, (await repo(ctx, RestaurantStore).get(closing.storeId)).name);
    const balance = balances.get(invoice.id);
    if (!balance)
      throw new StateError('入金履歴の残高が取得できません', 'Reload the source invoice and settlement history.');
    const settled = invoice.total.minus(balance);
    totals.sales = totals.sales.plus(invoice.total);
    totals.settled = totals.settled.plus(settled);
    totals.balance = totals.balance.plus(balance);
    rows.push({
      storeId: closing.storeId,
      store: stores.get(closing.storeId),
      date: closing.date,
      closingId: closing.id,
      closingNumber: closing.number,
      invoiceId: invoice.id,
      invoiceNumber: invoice.number,
      sales: invoice.total.toString(),
      settled: settled.toString(),
      balance: balance.toString(),
    });
  }
  return {
    title: label('本部・決済債権の根拠', 'Headquarters settlement evidence'),
    columns: [
      ...
      column('invoiceId', label('請求書', 'Invoice'), 'ref', { ref: SalesInvoice.name }),
      column('invoiceNumber', label('請求番号', 'Invoice number'), 'text'),
      column('sales', label('税込売上', 'Gross sales'), 'decimal'),
      column('settled', label('基準日までの入金', 'Settled as of'), 'decimal'),
      column('balance', label('基準日の未入金', 'Balance as of'), 'decimal'),
    ],
    rows,
    totals: Object.fromEntries(Object.entries(totals).map(([key, value]) => [key, value.toString()])),
    meta: {
      asOf: range.asOf,
      balanceBasis: 'effective_dated_settlements',
      receiptScope: 'invoices_from_selected_business_dates',
...
export const settlementEvidenceAction = defineAction({
  ...
  description: label('本部・基準日決済債権', 'Headquarters settlement evidence as of date'),
  input: operationsInput,
  output: tableResult,
  permission: { entity: SalesInvoice.name, op: 'read' },
  exportEntities: [RestaurantClosing.name, RestaurantStore.name, SalesInvoice.name, SalesSettlement.name],
  tx: 'none',
  mutates: false,
  handler: settlementEvidence,
});
