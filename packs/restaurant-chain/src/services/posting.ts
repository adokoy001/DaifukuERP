...
export async function postSales(
  ...
  const payment = await repo(ctx, Payment).create({
    ...
    amount: head.cash,
    method: 'cash',
    accountId: head.cashAccountId,
    note: `飲食店日次締め ${head.number} 現金`,
  });
  await saveLines(ctx, Payment, payment.id, {
    [PaymentAllocation.name]: [{ invoiceEntity: SalesInvoice.name, invoiceId: invoice.id, amount: head.cash }],
  });
...
async function consumedIngredients(ctx: Context, closingId: string): Promise<Map<string, Decimal>> {
  ...
  for (const line of await closingLines(ctx, closingId)) {
    for (const ingredient of await ingredients(ctx, line.recipeId))
      addIngredient(consumed, ingredient.productId, ingredient.quantity.times(line.quantity));
  }
  return consumed;
}
async function postIssue(
  ctx: Context,
  head: PostingHead,
  items: Map<string, Decimal>,
  label: string,
): Promise<{ id: string | null; cost: Decimal }> {
  if (!items.size) return { id: null, cost: Decimal.zero() };
  if (items.size > 500)
    throw new ValidationError('材料種類数が上限を超えています', [
      { path: 'lines', message: 'at most 500 ingredients' },
    ]);
  const entry = await repo(ctx, StockEntry).create({
    type: 'issue',
    warehouseId: head.warehouseId,
    date: head.date,
    note: `飲食店日次締め ${head.number} ${label}`,
  });
  await saveLines(ctx, StockEntry, entry.id, {
    [StockEntryLine.name]: [...items]
...
export async function postIngredients(ctx: Context, head: PostingHead) {
  ...
  return {
    consumptionEntryId: consumed.id,
    wasteEntryId: discarded.id,
    consumptionCost: consumed.cost,
    wasteCost: discarded.cost,
  };
}
