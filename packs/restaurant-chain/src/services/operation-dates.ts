...
export function dates(from: string, to: string): string[] {
  return Array.from({ length: dayCount(from, to) }, (_, i) => shiftDay(from, i));
}
export function assertPeriod(from: string, to: string): void {
  if (from > to || dayCount(from, to) > 366)
    throw new ValidationError('期間は開始日から366日以内で指定してください', [
      { path: 'to', message: 'from <= to, maximum 366 inclusive days' },
    ]);
}
export function resolveRange(ctx: Context, input: OperationsInput): OperationsRange {
  ...
  if (input.to > asOf || asOf > today(ctx))
    throw new ValidationError('終了日・基準日は今日以前にしてください', [
      { path: 'asOf', message: 'to <= asOf <= today in Asia/Tokyo' },
    ]);
  ...
  return {
    ...
    to: input.to,
    asOf,
    previousFrom: shiftDay(input.from, -count),
    previousTo: shiftDay(input.from, -1),
    timeZone: 'Asia/Tokyo',
