...
function bounded<T>(page: { items: T[]; total: number }): T[] {
  if (page.total > 500)
    throw new ValidationError('日次締めの明細数が上限を超えています', [
      { path: 'lines', message: 'at most 500 lines per group' },
    ]);
  return page.items;
}
export async function closingLines(ctx: Context, closingId: string) {
  return bounded(
    await repo(ctx, RestaurantClosingLine).list({
      where: { closingId },
...
export async function ingredients(ctx: Context, recipeId: string) {
  return bounded(
    await repo(ctx, RecipeIngredient).list({
      where: { recipeId },
      orderBy: [{ field: 'seq', dir: 'asc' }],
      limit: 500,
    }),
...
export async function wasteLines(ctx: Context, closingId: string) {
  return bounded(
    await repo(ctx, RestaurantWasteLine).list({
      where: { closingId },
      orderBy: [{ field: 'seq', dir: 'asc' }],
      limit: 500,
    }),
