...
async function beforeSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  ...
  if (account.type !== 'asset' || !partner.isCustomer) throw new ValidationError('店舗の現金勘定・店頭客を確認してください', [{ path: 'storeId', message: 'cash asset account and customer required' }]);
  if (row.reviewStatus !== 'approved') Object.assign(row, { reviewStatus: 'approved', reviewedAt: ctx.now(), reviewedBy: ctx.actor.id });
}
async function afterSubmit(ctx: Context, { row }: HookArgs): Promise<void> {
  const store = await repo(ctx, RestaurantStore).get(String(row.storeId));
  const head: PostingHead = { id: String(row.id), number: String(row.number), date: row.date as LocalDate, warehouseId: store.warehouseId, partnerId: store.partnerId, cashAccountId: store.cashAccountId, cash: decimal(row.cashAmount) };
