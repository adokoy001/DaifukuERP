...
async function mutableDay(ctx: Context, row: Record<string, unknown>): Promise<void> {
  if (String(row.date) < today(ctx))
    throw new StateError(
      '過去日の営業予定・目標は変更できません',
      'Plan today or a future day; historical targets are fixed.',
    );
  if (
    await repo(ctx, RestaurantClosing).count({
    ...
    throw new StateError(
      '確定実績がある日の予定・目標は変更できません',
      'Historical targets cannot be rewritten after posting.',
    );
}
async function submit(ctx: Context, { row }: HookArgs): Promise<void> {
  await withLock(ctx, dayLock(String(row.storeId), String(row.date)), async () => {
    ...
    if (
      await repo(ctx, RestaurantDayPlan).count({ storeId: String(row.storeId), date: String(row.date), docstatus: 1 })
    )
      throw new StateError('この日の営業予定は確定済みです', 'Cancel and amend the existing future plan.');
  });
  ...
  if (!row.expectedOpen && !target.isZero())
    throw new ValidationError('休業予定の売上目標はゼロにしてください', [
      { path: 'grossSalesTarget', message: 'closed days require zero target' },
    ]);
}
export function registerDayPlanHooks(): void {
  registry.registerHook(RestaurantDayPlan.name, 'before_submit', submit);
  registry.registerHook(RestaurantDayPlan.name, 'before_cancel', async (ctx, { row }) =>
    withLock(ctx, dayLock(String(row.storeId), String(row.date)), () => mutableDay(ctx, row)),
  );
}
