...
export const RestaurantDayPlan = defineDocument({
  name: 'restaurant_chain_day_plan', label: label('店舗営業予定・目標', 'Store daily plans'),
  ...
  fields: {
    ...
    grossSalesTarget: f.money({ label: label('税込売上目標', 'Gross sales target'), required: true, default: '0', min: '0' }),
    note: f.text({ label: label('備考', 'Notes'), maxLength: 1000 }),
  },
  indexes: [['storeId', 'date']], storeAccess: { kind: 'store', field: 'storeId' },
  permissions: { roles: {
    inventory: ['read', 'export', 'create', 'update', 'delete', 'submit', 'cancel', 'amend'],
    sales: ['read', 'export', 'create', 'update', 'delete', 'submit', 'cancel', 'amend'],
    accounting: ['read', 'export'], viewer: ['read', 'export'], chain_staff: ['read', 'export'], chain_manager: ['read', 'export'],
  } },
  views: { list: ['storeId', 'date', 'expectedOpen', 'grossSalesTarget'], form: [['storeId', 'date'], ['expectedOpen', 'grossSalesTarget'], ['note']] },
});
