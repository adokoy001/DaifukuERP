...
export const RestaurantStore = defineEntity({
  ...
  fields: {
    ...
    warehouseId: f.ref('warehouse', {
      label: label('厨房倉庫', 'Kitchen warehouse'),
      required: true,
      immutable: true,
      unique: true,
    }),
  ...
  permissions: {
    roles: {
      ...
      inventory: ['read', 'export', 'create', 'update'],
      sales: ['read', 'export'],
  ...
  views: {
    list: ['code', 'name', 'warehouseId', 'isActive'],
    search: ['code', 'name'],
    form: [['code', 'name'], ['warehouseId', 'partnerId', 'cashAccountId'], ['isActive']],
  },
});
