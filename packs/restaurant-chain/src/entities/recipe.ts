...
export const RestaurantRecipe = defineDocument({
  ...
  fields: {
    code: f.text({ label: label('レシピコード・版', 'Recipe code / revision'), required: true, index: true, immutable: true, maxLength: 40 }),
  ...
  permissions: { roles: { chain_staff: ['read', 'export'], chain_manager: ['read', 'export'], inventory: ['read', 'export', 'create', 'update', 'delete', 'submit', 'cancel', 'amend'], sales: ['read', 'export'], accounting: ['read', 'export'], viewer: ['read', 'export'] } },
  views: { list: ['code', 'name', 'unitPrice', 'alcohol'], search: ['code', 'name'], form: [['code', 'name'], ['productId', 'unitPrice', 'alcohol'], ['note']] },
});
