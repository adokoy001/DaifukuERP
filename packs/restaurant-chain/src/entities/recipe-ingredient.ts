...
export const RecipeIngredient = defineEntity({
  ...
  fields: {
    recipeId: f.ref('restaurant_chain_recipe', { label: label('レシピ', 'Recipe'), required: true, onDelete: 'cascade' }),
    ...
    quantity: f.quantity({ label: label('1食分の基準単位数量', 'Base units per serving'), required: true, min: '0.000001' }),
  },
  storeAccess: { kind: 'sharedRead' },
  permissions: { roles: { chain_staff: ['read', 'export'], chain_manager: ['read', 'export'], inventory: ['read', 'export', 'create', 'update', 'delete'], sales: ['read', 'export'], accounting: ['read', 'export'], viewer: ['read', 'export'] } },
