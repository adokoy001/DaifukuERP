...
async function sampleProducts(ctx: Context): Promise<Map<string, string>> {
  ...
  for (const p of SAMPLE_PRODUCTS) {
    ...
    const product =
      existing ?? (await repo(ctx, Product).create({ ...p, ...(p.kind === 'goods' ? { uomId: kg.id } : {}) }));
...
async function sampleStores(ctx: Context): Promise<void> {
  ...
  if (!account || !partner)
    throw new StateError(
      '先に日本の勘定科目と飲食店テンプレートを初期化してください',
      'Apply the Japan module and restaurant seed first.',
    );
  for (const [code, name] of [
    ...
    const warehouse =
      (await repo(ctx, Warehouse).list({ where: { code }, limit: 1 })).items[0] ??
      (await repo(ctx, Warehouse).create({ code, name: `${name} 厨房` }));
    await repo(ctx, RestaurantStore).create({
      code,
      name,
...
export async function sampleRestaurantChain(ctx: Context): Promise<void> {
  ...
  const recipe = await repo(ctx, RestaurantRecipe).create({
    ...
    productId: curry,
    unitPrice: '1100',
    note: '1食につき米0.2 kg・鶏肉0.1 kg。持帰り税込単価1080円の例は明細で変更します。',
  });
  await saveLines(ctx, RestaurantRecipe, recipe.id, {
