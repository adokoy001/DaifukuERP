...
async function receiptDraft(ctx: Context, productId: string, date: string): Promise<void> {
  ...
  const stock = await repo(ctx, StockEntry).create({
    type: 'receipt',
    date,
    warehouseId: warehouse.id,
    note: WHOLESALE_RECEIPT_NOTE,
  });
  await saveLines(ctx, StockEntry, stock.id, {
    [StockEntryLine.name]: [{ productId, quantity: '100', unitCost: '500' }],
  });
}
export async function sampleIndustry(ctx: Context, profile: IndustryProfile, Job: JobDef): Promise<void> {
  ...
  await repo(ctx, Job).create({
    reference: sampleReference(profile.job.name),
    title: `${profile.productName}の履行（サンプル）`,
    partnerId: partner.id,
    productId: product.id,
    date,
    plannedDate: addDays(date, 2),
