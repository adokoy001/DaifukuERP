...
export function loadIndustryPack(name: string): PackDef {
  ...
  const pack = definePack({ name, label: profile.job.label, version: '0.1.0', depends: [IndustryOperationsModule.name, JapanModule.name], entities: [engine.Job], actions: engine.actions, hooks: engine.registerHooks,
    ...
    ], seed: (ctx) => seedIndustry(ctx, profile), sample: (ctx) => sampleIndustry(ctx, profile, engine.Job),
  });
  loaded.set(name, pack); return pack;
}
