...
export async function seedIndustry(ctx: Context, profile: IndustryProfile): Promise<void> {
  ...
  if (!(await products.count({ code: productCode(profile) })))
    await products.create({
      code: productCode(profile),
      name: profile.productName,
      kind: profile.job.productKind,
      uomId: unit.id,
      salePrice: profile.unitPrice,
      taxCategory: 'standard',
      isPurchased: profile.job.productKind === 'goods',
      description: '架空の練習品目。実取引の料金・税区分は契約に応じて確認してください。',
    });
}
