...
まず試せる中心領域は、**複数店舗を持つ事業の本部・店舗・従業員の業務**です。[日次報告と店舗比較](docs/manual/appendix-e-operations-control.md)、[勤怠・申請・給与明細](docs/manual/appendix-f-workforce.md)、[シフト推薦](docs/operations/shift-planning.md)を共通の会社・拠点権限でつなぎます。多業種へ拡張できる土台を保ち、業種ごとの対応範囲と未検証項目は [STATUS](docs/STATUS.md) に分けて記録します。
...
| 給与・勤務制度 | 期間付き制度データの確認・会社別導入、2026年月額甲欄の税/保険自動算定、本人申告と年末調整、通常/1か月変形/1〜3か月フレックス |
...
給与本部では、制度パッケージの出典・適用期間・計算方式を確認して会社ごとに導入できます。旧2026年データと保存済み結果は保持します。収録済みの法定値は2026年の対応範囲に限り、翌年度や未対応期間へ自動流用しません。新年度を追加するときの構造と互換性は [給与制度の設計](docs/architecture/payroll-automation.md) を参照してください。
