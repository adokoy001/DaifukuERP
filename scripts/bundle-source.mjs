...
function restricted(path) {
  return (
    /(^|\/)(\.git|node_modules|\.data|\.runtime|backups)(\/|$)/.test(path) ||
...
function main() {
  ...
  if (git('diff', '--name-only').trim() || git('diff', '--cached', '--name-only').trim())
    throw new Error('Commit or restore tracked changes before exporting. Only the reviewed HEAD is distributed.');
  const paths = git('ls-tree', '-rz', '--name-only', 'HEAD').split('\0').filter(Boolean);
  if (paths.some(restricted))
    throw new Error('Refusing archive: HEAD tracks a restricted runtime, backup, credential or archive path.');
...
try {
  ...
  process.stderr.write(`${error instanceof Error ? error.message : 'Source export failed'}\n`);
  process.exitCode = 1;
}
