...
async function main() {
  ...
  if (
    ...
    throw new Error('Use --directory /new/absolute/path --bin /postgres/bin --port 55443 (unoccupied port)');
  ...
  const credentials = Object.fromEntries(
    ['admin', 'owner', 'app'].map((key) => [key, `D!${randomBytes(32).toString('base64url')}9a`]),
  );
  ...
  run(bin, 'initdb', [
    ...
    `--pwfile=${join(base, 'admin-password')}`,
    '--no-locale',
    '--encoding=UTF8',
  ]);
  ...
  run(bin, 'pg_ctl', [
    ...
    '-o',
    `-h 127.0.0.1 -p ${port} -k ${join(base, 'socket')}`,
    '-w',
    'start',
  ]);
  const address = `127.0.0.1:${port}`;
  const db = postgres(`postgres://postgres:${encodeURIComponent(credentials.admin)}@${address}/postgres`, {
    max: 1,
    onnotice: () => undefined,
  });
  try {
    if ((await db`select current_setting('data_directory') as directory`)[0]?.directory !== join(base, 'data'))
      throw new Error('Unexpected cluster; no fixture roles were created');
    for (const [name, secret, bypass] of [
      ['daifuku_setup_owner', credentials.owner, 'BYPASSRLS'],
      ['daifuku_app', credentials.app, 'NOBYPASSRLS'],
    ]) {
    ...
    const names = [
      'daifuku_setup_test',
      'daifuku_setup_legacy',
      ...Array.from({ length: 12 }, (_, i) => `daifuku_setup_restore_${i + 1}`),
    ];
...
main().catch((error) => {
  process.stderr.write(
    `Setup test fixture failed (${typeof error?.code === 'string' && /^[A-Z0-9_]+$/.test(error.code) ? error.code : 'OPERATION_FAILED'}). Existing databases were not reset; inspect the private fixture directory.\n`,
  );
  process.exitCode = 1;
});
