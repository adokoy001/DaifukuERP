...
try {
  const names = ['release', 'state', 'config', 'output', 'node', 'user', 'hostname', 'profile', 'cert', 'key', 'manifest-sha256'];
  const { values } = parseArgs({ options: { ...Object.fromEntries(names.map((name) => [name, { type: 'string' }])), execute: { type: 'boolean', default: false }, 'allow-candidate': { type: 'boolean', default: false } } });
  const result = await renderProfile({ ...values, manifestHash: values['manifest-sha256'], allowCandidate: values['allow-candidate'] });
  process.stdout.write(JSON.stringify(result, null, 2) + '\n');
} catch (error) { process.stderr.write(`Deployment plan stopped: ${error instanceof Error ? error.message : 'INVALID_CONFIGURATION'}\n`); process.exitCode = 1; }
