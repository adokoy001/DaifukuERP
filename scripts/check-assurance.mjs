...
try {
  ...
  if (process.argv[2] === '--changed') {
    ...
    if (!base || !/^[a-zA-Z0-9][a-zA-Z0-9_./~-]*$/.test(base) || process.argv.length !== 4) throw new Error('Use --changed <existing git ref>');
    ...
    console.log(`Recheck these linked invariant assumptions: ${affectedEntries(entries, changed, { root, document }).join(', ') || '(none)'}`);
    console.log('This reports linked file changes; it does not infer semantic validity or replace review of dependency changes.');
  } else if (process.argv.length !== 2) throw new Error('Use no arguments, or --changed <existing git ref>');
  console.log(`Assurance ledger: ${entries.length} unique conditions, required assumptions and local references verified.`);
} catch (error) { console.error(`Assurance ledger check failed: ${error.message}`); process.exitCode = 1; }
