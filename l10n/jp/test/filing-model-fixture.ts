...
export async function filingModelFixture(capitalAmount: number, saleAmount: number) {
  ...
  const data = await run(async (ctx) => {
    ...
    await createAndSubmitEntry(ctx, { date: '2026-01-01' }, [
      { accountId: cash.id, debit: String(capitalAmount), credit: '0' },
      { accountId: capital.id, debit: '0', credit: String(capitalAmount) },
    ]);
    await createAndSubmitEntry(ctx, { date: '2026-12-20' }, [
      { accountId: cash.id, debit: String(saleAmount), credit: '0' },
      { accountId: sales.id, debit: '0', credit: String(saleAmount), taxCategory: 'out_of_scope' },
    ]);
  ...
  const evidence = (id: string) =>
    run(async (ctx) => {
      const row = await repo(ctx, FilingAccountingPack).get(id);
      return { request: row.request, source: row.source, prepared: row.prepared, sourceHash: row.sourceHash };
    });
