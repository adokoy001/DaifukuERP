...
import { EXEMPT_SUPPLIER_CREDIT_RATIO_OVERRIDE, INVOICE_HTML_OVERRIDE, JP_CHART_OF_ACCOUNTS, JapanModule, type ExemptSupplierCreditRatioFn, type InvoiceHtmlRenderer, type InvoiceRenderData } from '../src/index.ts';
...
const accountByCode = (code: string) => run(async (ctx) => (await repo(ctx, Account).list({ where: { code } })).items[0]);
...
describe('l10n/jp module (docs/specs/l10n-jp.md)', () => {
  it('AC-6 registers module l10n_jp with no entities, depending on accounting and tax, and both overrides', () => {
    ...
    const data: InvoiceRenderData = {
      ...
      lines: [{ seq: 1, description: '食品', quantity: '1', unitPrice: '1333', amount: '1333', taxCategory: 'reduced', rate: '0.08' }],
  ...
  it('AC-1/AC-2/AC-7 seed is idempotent per company and never overwrites what exists', async () => {
    ...
    const cash = await run((ctx) => repo(ctx, Account).create({ code: '1000', name: 'Cash (operator-defined)', type: 'asset', subtype: 'custom' }));
    ...
    expect(await run((ctx) => getSetting(ctx, TAX_ROUNDING_KEY, taxRoundingSchema, { mode: 'half_up', unit: 'delivery_note' }))).toEqual({ mode: 'down', unit: 'invoice' });
    ...
    expect(new Set(accountsAfterSecond.items.map((a) => a.id))).toEqual(new Set(accountsAfterFirst.items.map((a) => a.id)));
    ...
    expect((await run((ctx) => auditTrail(ctx, 'company_settings', db.companyId))).map((e) => e.op)).toEqual(['update', 'update']); // operator's + seed's
  });
  ...
  it('AC-1 chart of accounts: every spec code with its type, Japanese subtype, partnerRequired and default tax category', async () => {
    ...
    const pick = (code: string) => {
      ...
      return a ? { name: a.name, type: a.type, subtype: a.subtype, partnerRequired: a.partnerRequired, taxCategoryDefault: a.taxCategoryDefault, isActive: a.isActive } : undefined;
    };
    expect(pick('1300')).toEqual({ name: '売掛金', type: 'asset', subtype: '売掛金', partnerRequired: true, taxCategoryDefault: null, isActive: true });
    expect(pick('2100')).toEqual({ name: '買掛金', type: 'liability', subtype: '買掛金', partnerRequired: true, taxCategoryDefault: null, isActive: true });
    expect(pick('1500')).toMatchObject({ name: '仮払消費税', type: 'asset', subtype: '仮払消費税', partnerRequired: false });
    ...
    expect(pick('4000')).toEqual({ name: '売上高', type: 'revenue', subtype: '売上', partnerRequired: false, taxCategoryDefault: 'standard', isActive: true });
    expect(pick('4100')).toMatchObject({ name: '雑収入', type: 'revenue', taxCategoryDefault: null });
    expect(pick('5000')).toEqual({ name: '仕入高', type: 'expense', subtype: '仕入', partnerRequired: false, taxCategoryDefault: 'standard', isActive: true });
    expect(pick('6100')).toMatchObject({ name: '給料手当', type: 'expense', subtype: '販売費及び一般管理費', taxCategoryDefault: 'out_of_scope' });
    ...
    expect(pick('6980')).toMatchObject({ name: '外注費', type: 'expense', subtype: '販売費及び一般管理費', taxCategoryDefault: 'standard' });
  ...
  it('AC-2 seeds the tax rates through the tax module (same 6 codes) and a second company is independent', async () => {
    ...
    const other = (fn: (ctx: Context) => Promise<unknown>) => withContext(db.app, systemParams(db.tenantId, otherCompany), fn);
    ...
    expect(await other(async (ctx) => (await getCompany(ctx)).settings)).toEqual({ [TAX_ROUNDING_KEY]: { mode: 'down', unit: 'invoice' }, [TAX_PRICE_INCLUDES_TAX_KEY]: false });
