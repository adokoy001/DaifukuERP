...
確認した追跡対象の画像は `docs/manual/img` と `docs/log` 配下のアプリ操作スクリーンショットです。外部の写真素材・イラスト・アイコン画像ファイル、音声・動画、フォントバイナリは追跡対象にありません。CSS は OS のシステムフォントを参照しており、Web フォントのダウンロード設定はありません。フォント名の指定はフォントソフトウェアの同梱ではありません。
