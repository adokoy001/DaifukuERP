...
definePack({ name: 'dbpack_unknown', label: label('未登録設定', 'Unknown setting'), depends: ['test'], settings: { [GREETING]: 'hi', 'nope.missing': 1 }, seed: seedPartner('Unknown seed') });
definePack({ name: 'dbpack_invalid', label: label('不正値', 'Invalid value'), depends: ['test'], settings: { [LIMIT]: -3 } });
definePack({
  name: 'dbpack_boom',
...
definePack({ name: 'dbpack_ok', label: label('正常', 'OK'), version: '2.0.0', depends: ['test'], settings: { [GREETING]: 'hello', [LIMIT]: 5 }, seed: seedPartner('OK seed') });
...
describe('applyPack (AC-3)', () => {
  it('AC-3 unknown setting key -> ValidationError at apply (settings.<key>); nothing written, seed not run', async () => {
    ...
    expect((e as ValidationError).details).toEqual({ issues: [{ path: 'settings.nope.missing', message: 'not a registered setting' }] });
  ...
  it('AC-3 a default that fails its setting schema -> ValidationError with the schema path', async () => {
    await expect(db.run({}, (ctx) => applyPack(ctx, 'dbpack_invalid'))).rejects.toMatchObject({ code: 'VALIDATION', details: { issues: [{ path: `settings.${LIMIT}` }] } });
    expect(await storedSettings()).toEqual({});
  });
  ...
  it('AC-3 permission: admin only (PermissionDenied for other roles); unknown pack -> NOT_FOUND listing known packs; no company -> NotFound', async () => {
    ...
    await expect(db.run(clerk, (ctx) => runAction(ctx, 'pack.apply', { name: 'dbpack_ok' }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
  ...
  it('AC-3 success: every default written, seed run, packs.applied keeps other entries; sample flag without a sample is ignored', async () => {
    ...
    await db.owner.sql`update companies set settings = ${JSON.stringify({ [PACKS_APPLIED_KEY]: other })}::jsonb where id = ${db.companyId}`;
    const now = new Date('2026-09-11T09:00:00.000Z');
    const res = await db.run({ now: () => now }, (ctx) => runAction(ctx, 'pack.apply', { name: 'dbpack_ok', sample: true }));
    expect(res).toEqual({
      name: 'dbpack_ok',
      version: '2.0.0',
    ...
    expect(await db.run({}, (ctx) => readAppliedPacks(ctx))).toEqual({ ...other, dbpack_ok: { at: now.toISOString(), version: '2.0.0' } });
    const listed = (await db.run({ roles: ['viewer'], actor: { type: 'user' as const, id: newId() } }, (ctx) => runAction(ctx, 'pack.list', {}))) as { items: { name: string; applied: boolean }[] };
