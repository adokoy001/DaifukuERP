...
const count = async (key: string) =>
  (
    await db.owner.drizzle
      .select()
      .from(identityLimits)
      .where(eq(identityLimits.key, tokenHash(key)))
  )[0]?.attempts;
describe('failure reservations outside identity transactions', () => {
  it('does not erase a concurrent failure when a successful request completes', async () => {
    ...
    await expect(withIdentityAttempt(db.owner, budget, async () => 'must not run', now)).rejects.toMatchObject({
      httpStatus: 429,
    });
    expect(await count('concurrent-identity')).toBe(2);
    complete.resolve();
    expect(await successful).toBe('valid');
    expect(await count('concurrent-identity')).toBe(1);
    await expect(
      withIdentityAttempt(
        db.owner,
        budget,
  ...
  it('refunds earlier reservations if another budget rejects before verification runs', async () => {
    ...
    await expect(
      withIdentityAttempt(
        db.owner,
        [
          { key: 'shared-nat', limit: 2 },
          { key: 'blocked-identity', limit: 1, seconds: 60 },
        ],
        async () => {
          called = true;
        },
    ...
    expect(await count('shared-nat')).toBe(0);
    expect(await count('blocked-identity')).toBe(1);
  });
  it('cannot refund or rewind a later window when an earlier successful request finishes', async () => {
    ...
    complete.resolve();
    await pending;
    expect(await count('window-identity')).toBe(1);
    await expect(identityRateLimit(db.owner, 'window-identity', 1, 60, now)).rejects.toMatchObject({ httpStatus: 429 });
    await expect(identityRateLimit(db.owner, 'window-identity', 1, 60, next)).rejects.toMatchObject({
      httpStatus: 429,
    });
    await expect(
      identityRateLimit(db.owner, 'window-identity', 1, 60, new Date(next.getTime() + 60000)),
  ...
  it('limits MFA failures per identity even when the password holder repeatedly obtains fresh challenges', async () => {
    ...
    const enabled = await confirmMfa(
      db.owner,
      original,
      setup.setupToken,
    ...
    const challenge = await db.run(params, (ctx) => beginMfaLogin(ctx, identity));
    await expect(
      completeMfaLogin(db.owner, challenge.challengeToken, enabled.recoveryCodes[0] ?? '', key, now),
    ).rejects.toMatchObject({ httpStatus: 429 });
    const next = new Date(now.getTime() + 301000);
    await expect(
      completeMfaLogin(db.owner, challenge.challengeToken, enabled.recoveryCodes[0] ?? '', key, next),
    ).rejects.toMatchObject({ httpStatus: 401 });
    const fresh = await db.run({ companyId: null, now: () => next }, (ctx) => beginMfaLogin(ctx, identity));
    await expect(
      completeMfaLogin(db.owner, fresh.challengeToken, enabled.recoveryCodes[0] ?? '', key, next),
