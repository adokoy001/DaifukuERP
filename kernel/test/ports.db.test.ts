...
describe('document lines', () => {
  it('AC-1 create with lines via the generic action assigns seq and parent; get returns lines in order', async () => {
    const created = (await db.run({}, (ctx) =>
      runAction(ctx, 'test_memo.create', {
        ...
        date: '2026-09-10',
        amount: '30',
        lines: { test_memo_line: [{ description: 'B', amount: '20' }, { description: 'A', amount: '10', qty: '2' }] },
      }),
    ...
    const fetched = (await db.run({}, (ctx) => runAction(ctx, 'test_memo.get', { id: created.id }))) as Row & { lines: { test_memo_line: Row[] } };
    expect(fetched.lines.test_memo_line).toHaveLength(2);
    expect(entityMeta(await db.run({}, async (ctx) => ctx), TMemo).lines).toEqual([{ entity: 'test_memo_line', parentField: 'memoId' }]);
  });

  it('AC-2 update with lines replaces: keeps rows by id, deletes missing, inserts new', async () => {
    const created = (await db.run({}, (ctx) =>
      runAction(ctx, 'test_memo.create', { partnerId, date: '2026-09-10', amount: '1', lines: { test_memo_line: [{ description: 'keep', amount: '1' }, { description: 'drop', amount: '2' }] } }),
    )) as Row & { lines: { test_memo_line: Row[] } };
    const keepId = created.lines.test_memo_line[0]?.id as string;
    const updated = (await db.run({}, (ctx) =>
      runAction(ctx, 'test_memo.update', { id: created.id, patch: { note: 'n', lines: { test_memo_line: [{ id: keepId, description: 'kept', amount: '5' }, { description: 'new', amount: '3' }] } } }),
    )) as Row & { lines: { test_memo_line: Row[] } };
  ...
  it('AC-3 lines are frozen after submit; unknown line entity is rejected with a hint', async () => {
    ...
    await expect(db.run({}, (ctx) => saveLines(ctx, TMemo, m.id, { test_memo_line: [] }))).rejects.toBeInstanceOf(StateError);
    await expect(db.run({}, (ctx) => saveLines(ctx, TMemo, m.id, { nope: [] }))).rejects.toBeInstanceOf(ValidationError);
...
describe('aggregate (report port)', () => {
  it('AC-4 groups and sums decimals as Decimal, counts as number, honours where/orderBy', async () => {
    const rows = await db.run({}, (ctx) =>
      repo(ctx, TMemoLine).aggregate({ groupBy: ['memoId'], metrics: { total: { sum: 'amount' }, n: { count: true }, maxQty: { max: 'qty' } }, orderBy: [{ field: 'total', dir: 'desc' }] }),
    );
    ...
    expect(top.n).toBe(2);
    const filtered = await db.run({}, (ctx) => repo(ctx, TMemoLine).aggregate({ where: { description: 'kept' }, metrics: { total: { sum: 'amount' } } }));
    expect((filtered[0] as { total: Decimal }).total.toString()).toBe('5');
    await expect(db.run({}, (ctx) => repo(ctx, TMemoLine).aggregate({ groupBy: ['nope'], metrics: { n: { count: true } } }))).rejects.toBeInstanceOf(ValidationError);
    await expect(db.run({ roles: ['viewer'] }, (ctx) => repo(ctx, TMemoLine).aggregate({ metrics: { n: { count: true } } }))).rejects.toBeInstanceOf(PermissionDenied);
  });
});
...
describe('company settings', () => {
  ...
  it('AC-5 falls back when unset, validates on set, audits, and requires admin/settings role', async () => {
    ...
    await expect(db.run({ roles: ['sales'] }, (ctx) => setSetting(ctx, 'tax.rounding', schema, { unit: 'invoice', mode: 'up' }))).rejects.toBeInstanceOf(PermissionDenied);
    await expect(db.run({}, (ctx) => setSetting(ctx, 'tax.rounding', schema, { unit: 'bad' }))).rejects.toBeInstanceOf(ValidationError);
    await db.run({}, (ctx) => setSetting(ctx, 'tax.rounding', schema, { unit: 'delivery_note', mode: 'up' }));
    expect(await db.run({ roles: ['sales'] }, (ctx) => getSetting(ctx, 'tax.rounding', schema, fb))).toEqual({ unit: 'delivery_note', mode: 'up' });
...
describe('storage port', () => {
  it('AC-6 put/get round-trips bytes with sha256 and refuses unsafe keys', async () => {
    ...
    const stored = await db.run({}, (ctx) => ctx.storage.put(ctx.tenantId, data, { filename: 'receipt.pdf', contentType: 'application/pdf' }));
