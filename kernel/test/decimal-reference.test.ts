...
function roundThousandths(value: bigint, scale: number, mode: RoundingMode): string {
  ...
  const divisor = 10n ** BigInt(3 - scale), remainder = magnitude % divisor;
...
describe('AC-6 independent Decimal amount/quantity references', () => {
  it('orders exact micro-units including equality and signs against BigInt', () => {
    ...
    fc.assert(fc.property(units, units, (a, b) => {
      const left = Decimal.from(scaled(a, 6)), right = Decimal.from(scaled(b, 6));
      for (const [value, integer] of [[right, b], [left, a]] as const) {
        expect(left.cmp(value)).toBe(a < integer ? -1 : a > integer ? 1 : 0);
        expect(left.eq(value)).toBe(a === integer); expect(left.lt(value)).toBe(a < integer);
        expect(left.lte(value)).toBe(a <= integer); expect(left.gt(value)).toBe(a > integer); expect(left.gte(value)).toBe(a >= integer);
      }
  ...
  it('rounds positive and negative amounts at every selected boundary using integer division', () => {
    ...
    const check = (value: bigint) => {
      ...
      for (const scale of [0, 1, 2, 3]) for (const mode of ['half_up', 'down', 'up'] as const) {
  ...
  it('divides exact scaled integer amounts and rejects a zero divisor', () => {
    fc.assert(fc.property(fc.bigInt({ min: -10000000n, max: 10000000n }), fc.bigInt({ min: 1n, max: 10000n }), (a, b) => {
      expect(Decimal.from(scaled(a * b, 4)).div(b).eq(scaled(a, 4))).toBe(true);
  ...
  it('preserves wire amounts, type identity and presentation without accepting malformed input', () => {
    ...
    for (const text of ['', '1e3', '+1', '1.2.3', '1x', '--1']) {
      expect(Decimal.isDecimalString(text)).toBe(false); expect(() => Decimal.from(text)).toThrow('not a decimal string');
    }
    expect(value.toFixed(2)).toBe('12.30'); expect(value.toNumberUnsafe()).toBe(12.3);
    expect(value.toJSON()).toBe('12.3'); expect(isDecimal(value)).toBe(true); expect(isDecimal({ value: '12.3' })).toBe(false);
    expect(Decimal.sum([]).toString()).toBe('0'); expect(Decimal.zero().isZero()).toBe(true);
  });
});
