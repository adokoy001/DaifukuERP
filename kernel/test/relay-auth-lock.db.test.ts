...
it('does not hold the user row while waiting for a membership being removed', async () => {
  const userId = newId(), gatewayId = newId(), application = 'relay-lock-' + newId();
  await db.owner.drizzle.insert(users).values({ id: userId, tenantId: db.tenantId, email: userId + '@example.com', name: 'Synthetic relay operator', defaultCompanyId: db.companyId });
  await db.owner.drizzle.insert(companyMemberships).values({ tenantId: db.tenantId, userId, companyId: db.companyId, roles: ['edge_manager'] });
  let worker: Promise<string> | undefined;
  await db.run({}, async (holder) => {
    await holder.db.select().from(companyMemberships).where(and(eq(companyMemberships.userId, userId), eq(companyMemberships.companyId, db.companyId))).for('update');
    worker = db.run({ actor: { type: 'user', id: userId }, roles: ['edge_manager'], sessionVersion: 1 }, async (ctx) => {
    ...
    while (Date.now() < deadline) {
      const rows = await db.app.sql`select pid from pg_stat_activity where application_name = ${application} and wait_event_type = 'Lock'`;
