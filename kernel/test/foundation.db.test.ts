...
const Owned = defineEntity({ name: 'test_owned', label: label('所有権', 'Ownership'), fields: {
  name: f.text({ required: true }), result: f.int({ default: 0, serverOwned: true }),
}, permissions: { roles: { editor: ['read', 'create', 'update'], viewer: ['read'] } } });
const TenantItem = defineEntity({ name: 'test_tenant_item', scope: 'tenant', label: label('共通', 'Tenant item'),
  fields: { name: f.text({ required: true }) }, permissions: { roles: { viewer: ['read'] } } });
const ownedWrite = defineWriteCapability({ name: 'test-owned-compute', entity: Owned.name, fields: ['result'], operations: ['update'] });
registry.registerHook(Owned.name, 'before_validate', (_ctx, { row }) => { if (row.name === 'computed') row.result = 7; });
let db: TestDb;
...
beforeAll(async () => {
  db = await freshDb();
  registerCrudActions();
  registry.registerExt(TPartner.name, { foundationTag: f.text(), relatedPartner: f.ref(TPartner.name), controlled: f.text({ serverOwned: true }) });
  companyB = newId();
  await db.owner.drizzle.insert(companies).values({ id: companyB, tenantId: db.tenantId, code: 'B', name: 'Company B' });
  partnerId = (await db.run({}, (ctx) => repo(ctx, TPartner).create({ name: 'Foundation partner' }))).id;
});
afterAll(async () => { await db.close(); });
const draft = (extra = {}) => db.run({}, (ctx) => repo(ctx, TMemo).create({ partnerId, date: '2026-09-12', amount: '1', ...extra }));
...
describe('Scoped identity and ownership', () => {
  it('rejects another company reference in repository and direct SQL; rejects a foreign company context', async () => {
    ...
    await expect(db.run({}, (ctx) => repo(ctx, TPartner).create({ name: 'Ext cross-company', ext: { relatedPartner: p.id } }))).rejects.toMatchObject({ code: 'VALIDATION' });
    const m = await draft();
    await expect(db.run({}, (ctx) => repo(ctx, TMemo).update(m.id, { partnerId: p.id }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(db.owner.sql`update test_memo set partner_id = ${p.id} where id = ${m.id}`).rejects.toMatchObject({ code: '23503' });
    const other = await bootstrapTenant(db.owner, { tenantName: 'Other', companyCode: 'O', companyName: 'Other', adminEmail: 'other@foundation.test', adminName: 'Other', adminPassword: 'pw' });
    await expect(db.run({ companyId: other.companyId }, (ctx) => repo(ctx, TPartner).count())).rejects.toMatchObject({ code: 'INVALID_STATE' });
  });

  it('server-owned caller input is denied even to admin; hooks and private grants preserve op checks', async () => {
    await expect(db.run({}, (ctx) => repo(ctx, Owned).create({ name: 'forged', result: 99 }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run({}, (ctx) => repo(ctx, TPartner).create({ name: 'Ext owned', ext: { controlled: 'forged' } }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    const row = await db.run({}, (ctx) => repo(ctx, Owned).create({ name: 'computed' }));
    expect(row.result).toBe(7);
    await expect(db.run({}, (ctx) => repo(ctx, Owned).update(row.id, { result: 99 }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    const updated = await db.run({ roles: ['editor'] }, (ctx) => withWriteCapability(ctx, ownedWrite, (inner) => repo(inner, Owned).update(row.id, { result: 8 })));
    expect(updated.result).toBe(8);
    await expect(db.run({}, (ctx) => withWriteCapability(ctx, ownedWrite, (inner) => repo(inner, Owned).create({ name: 'wrong operation', result: 9 })))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run({ roles: ['viewer'] }, (ctx) => withWriteCapability(ctx, ownedWrite, (inner) => repo(inner, Owned).update(row.id, { result: 9 })))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run({}, (ctx) => withWriteCapability(ctx, { ...ownedWrite }, async () => 1))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run({}, (ctx) => repo(ctx, Owned).rawUpdate(row.id, { result: 99 }, 'update', { ...row }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
  });
  ...
  it('tenant entities satisfy generic action output; ambiguous email needs tenant identity; bootstrap replay is tenant-scoped', async () => {
    ...
    const input = { tenantName: 'Same mail', companyCode: 'S', companyName: 'Same mail', adminEmail: 'same@foundation.test', adminName: 'Same', adminPassword: 'pw' };
...
describe('Aggregate serialization and lifecycle', () => {
  it('direct lines bump the parent version and every public mutation freezes after submit', async () => {
    ...
    await expect(db.run({}, (ctx) => runAction(ctx, 'test_memo.update', { id: m.id, expectedVersion: m.version, patch: { lines: { test_memo_line: [] } } }))).rejects.toMatchObject({ code: 'CONFLICT' });
    await db.run({}, (ctx) => submitDocument(ctx, TMemo, m.id));
    await expect(db.run({}, (ctx) => repo(ctx, TMemoLine).update(line.id, { amount: '10' }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(db.run({}, (ctx) => repo(ctx, TMemoLine).delete(line.id))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(db.run({}, (ctx) => repo(ctx, TMemoLine).create({ memoId: m.id, description: 'extra' }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(db.run({}, (ctx) => repo(ctx, TMemo).update(m.id, { ext: { injected: true } }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
  });
  ...
  it('generic submit honors declared role and guard; lifecycle stale versions fail under lock', async () => {
    ...
    await expect(db.run({ roles: ['sales'] }, (ctx) => runAction(ctx, 'test_memo.submit', { id: zero.id }))).rejects.toThrow(/requires roles/);
    ...
    await expect(db.run({}, (ctx) => submitDocument(ctx, TMemo, zero.id, { expectedVersion: zero.version }))).rejects.toMatchObject({ code: 'CONFLICT' });
    const submitted = await db.run({}, (ctx) => submitDocument(ctx, TMemo, zero.id, { expectedVersion: updated.version }));
    await expect(db.run({}, (ctx) => repo(ctx, TMemo).update(zero.id, { ext: {} }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(db.run({}, (ctx) => cancelDocument(ctx, TMemo, zero.id, { expectedVersion: updated.version }))).rejects.toMatchObject({ code: 'CONFLICT' });
  ...
  it('saveLines rejects duplicate/foreign ids and 501; direct row 501 and oversized legacy reads fail', async () => {
    ...
    await expect(db.run({}, (ctx) => saveLines(ctx, TMemo, b.id, { test_memo_line: [{ id: line.id, description: 'stolen' }] }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(db.run({}, (ctx) => saveLines(ctx, TMemo, a.id, { test_memo_line: [{ id: line.id }, { id: line.id }] }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(db.run({}, (ctx) => saveLines(ctx, TMemo, b.id, { test_memo_line: Array.from({ length: 501 }, () => ({ description: 'too many' })) }))).rejects.toMatchObject({ code: 'VALIDATION' });
    // Simulate an imported legacy record at the exact boundary without 500 unrelated transactions.
    await db.owner.sql`insert into test_memo_line (id, tenant_id, company_id, memo_id, seq, description, qty, amount) select gen_random_uuid(), ${db.tenantId}::uuid, ${db.companyId}::uuid, ${b.id}::uuid, n, 'boundary', 1, 0 from generate_series(1,500) n`;
    expect((await db.run({}, (ctx) => getLines(ctx, TMemo, b.id))).test_memo_line).toHaveLength(500);
    await expect(db.run({}, (ctx) => repo(ctx, TMemoLine).create({ memoId: b.id, description: '501' }))).rejects.toMatchObject({ code: 'VALIDATION' });
    await db.owner.sql`insert into test_memo_line (id, tenant_id, company_id, memo_id, seq, description, qty, amount) values (${newId()}, ${db.tenantId}, ${db.companyId}, ${b.id}, 501, 'legacy', 1, 0)`;
    await expect(db.run({}, (ctx) => getLines(ctx, TMemo, b.id))).rejects.toMatchObject({ code: 'INVALID_STATE' });
    await expect(db.run({}, (ctx) => saveLines(ctx, TMemo, b.id, { test_memo_line: [] }))).rejects.toMatchObject({ code: 'INVALID_STATE' });
  });
  ...
  it('delete holds the parent lock through hooks, so a simultaneous submit cannot leave a submitted deletion', async () => {
    ...
    registry.registerHook(TMemo.name, 'before_delete', async (_ctx, { row }) => { if (row.id === m.id) { entered(); await blocked; } });
...
describe('Read authority, precision and outbox atomicity', () => {
  it('masked fields cannot be filtered, sorted, grouped, aggregated or recovered from audit; empty OR matches none', async () => {
    ...
    await expect(db.run(asSales, (ctx) => repo(ctx, TPartner).list({ where: { $or: [{ secretNote: 'classified' }] } }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run(asSales, (ctx) => repo(ctx, TPartner).list({ orderBy: [{ field: 'secretNote' }] }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run(asSales, (ctx) => repo(ctx, TPartner).aggregate({ metrics: { leak: { max: 'secretNote' } } }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(db.run(asSales, (ctx) => repo(ctx, TPartner).aggregate({ groupBy: ['secretNote'], metrics: { n: { count: true } } }))).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    ...
    const withNull = await db.run({}, (ctx) => repo(ctx, TPartner).list({ where: { id: p.id, 'ext.foundationTag': { $in: [null, 'present'] } } }));
  ...
  it('rejects decimals that numeric(20,6) would round or overflow', async () => {
    for (const creditLimit of ['0.0000001', '100000000000000']) {
      await expect(db.run({}, (ctx) => repo(ctx, TPartner).create({ name: 'precision', creditLimit }))).rejects.toMatchObject({ code: 'VALIDATION' });
    }
    expect((await db.run({}, (ctx) => repo(ctx, TPartner).create({ name: 'precision valid', creditLimit: '99999999999999.999999' }))).creditLimit?.toString()).toBe('99999999999999.999999');
  });
  ...
  it('outbox restores event company, rolls back failed subscribers, and concurrent claims run each event once', async () => {
    ...
    registry.subscribe('foundation.atomic', async (ctx) => {
      ...
      if (fail) { fail = false; throw new Error('injected subscriber failure'); }
    });
