...
const outsider = fc.record({
  ...
  cents: fc.integer({ min: 1, max: 999999 }),
  secret: fc.string({ maxLength: 20 }),
});
...
beforeAll(async () => {
  ...
  await db.run({}, async (ctx) => {
    ...
    const row = await repo(ctx, ScopeWork).create({
      siteId: allowedSite,
      name: 'needle alpha',
      amount: '10.25',
      category: 'a',
  ...
  await db.run(observerAt(allowedSite), async (ctx) => {
    ...
    expect(String(updated.amount)).toBe('20.75');
    await repo(ctx, ScopeChild).create({ workId: row.id, name: row.name });
  });
});
afterAll(async () => {
...
async function newCompany() {
  ...
  await db.owner.drizzle
    .insert(companies)
    .values({ id: companyId, tenantId: db.tenantId, code: companyId, name: 'Synthetic outsider' });
  return companyId;
}
async function assertBaseline(scope: Partial<ContextParams> = observerAt(allowedSite)) {
  ...
  expect(before.list.items).toHaveLength(2);
  expect(before.search.total).toBe(2);
  expect(before.child.total).toBe(2);
...
async function assertInvisible(companyId: string, siteId: string, rows: { id: string }[], own: Partial<ContextParams>) {
  ...
  await expect(db.run(own, (ctx) => repo(ctx, ScopeWork).update(first.id, { name: 'forged' }))).rejects.toMatchObject({
    code: 'NOT_FOUND',
  });
  await expect(
    db.run(own, (ctx) => repo(ctx, ScopeWork).create({ siteId, name: 'forged', amount: '1', category: 'a' })),
  ).rejects.toMatchObject({ code: companyId === db.companyId ? 'PERMISSION_DENIED' : 'VALIDATION' });
...
async function changeHidden(companyId: string, siteId: string, changes: HiddenInput[]) {
  ...
  for (const [index, input] of changes.entries()) {
    ...
    ids.push(created);
    const updated = await db.run({ companyId }, (ctx) =>
      repo(ctx, ScopeWork).update(
        created.id,
        { amount: money(input.cents + index + 1), name: 'needle changed' },
        { expectedVersion: created.version },
      ),
    );
    expect(updated.version).toBe(created.version + 1);
    expect(String(updated.amount)).not.toBe(String(created.amount));
  }
  return ids;
}
describe('AC-4 / PV-SCOPE-01 two-state Repository noninterference', () => {
  it.each(['company', 'site'] as const)(
    ...
    async (boundary) => {
        ...
        fc.asyncProperty(cases, async (changes) => {
          ...
          const siteId =
            boundary === 'company'
              ? await db.run(
                  { companyId },
                  async (ctx) => (await repo(ctx, ScopeSite).create({ name: 'Other company site' })).id,
                )
              : forbiddenSite;
          const rows = await changeHidden(companyId, siteId, changes);
          writes += rows.length * 2;
          await assertInvisible(companyId, siteId, rows, scope);
          rejected += 3;
          ...
          for (const row of rows)
            await db.run({ companyId }, async (ctx) => {
              for (const child of (await repo(ctx, ScopeChild).list({ where: { workId: row.id } })).items)
                await repo(ctx, ScopeChild).delete(child.id);
      ...
      expect(writes).toBeGreaterThanOrEqual(16);
  ...
  it('generated pack application in another company changes its metadata/hooks without changing this user or company', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(cases, async (changes) => {
        ...
        await db.run({ companyId }, (ctx) => applyPack(ctx, scopePack.name));
        applied++;
        const metadata = await db.run({ companyId }, async (ctx) => entityMeta(ctx, ScopeWork));
        expect(metadata.label).not.toEqual(before.entityMetadata.label);
        expect(metadata.extFields.map((field) => field.name)).toContain('ext.requiredTag');
        await expect(
          db.run({ companyId }, (ctx) =>
            repo(ctx, ScopeWork).create({ siteId, name: 'missing tag', amount: '1', category: 'a' }),
          ),
        ).rejects.toMatchObject({ code: 'VALIDATION' });
        rejected++;
        for (const input of changes) {
          const row = await db.run({ companyId }, (ctx) =>
            repo(ctx, ScopeWork).create({
              siteId,
              name: input.label,
              category: input.category,
              amount: money(input.cents),
              ext: { requiredTag: input.secret },
            }),
          );
