...
const Event = defineEntity({
  name: 'timestamp_query_event',
  label: label('日時検索検証', 'Timestamp query event'),
  fields: {
    name: f.text({ required: true }),
    position: f.int({ required: true }),
    at: f.timestamp(),
    date: f.date({ required: true }),
    text: f.text({ required: true }),
  },
  permissions: {
    roles: { timestamp_reader: ['read'], timestamp_before: ['read'] },
    rowRules: [{ roles: ['timestamp_before'], where: { at: { $lt: midnight } } }],
  },
});
registerCrudActions();
let db: TestDb;
beforeAll(async () => {
  ...
  for (const [name, at] of [
    ['before', '2026-09-10T14:59:59.999Z'],
    ['exact', '2026-09-10T15:00:00.000Z'],
    ['after', '2026-09-10T15:00:00.001Z'],
    ['unset', null],
  ] as const) {
    await db.run({ now: () => new Date(at ?? '2026-09-10T15:00:00.002Z') }, (ctx) =>
      repo(ctx, Event).create({
        name,
        position: position++,
        at: at ? new Date(at) : null,
        date: name === 'before' ? '2026-09-10' : '2026-09-11',
        text: midnight,
      }),
    );
  }
...
async function names(where: Domain, roles = ['timestamp_reader']): Promise<string[]> {
  ...
  const result = (await db.run({ roles }, (ctx) => runAction(ctx, 'timestamp_query_event.list', input))) as {
    items: { name: string }[];
  };
...
describe('timestamp query operands use exact zoned instants before reaching the driver', () => {
  it.each(['2026-09-10T15:00:00Z', midnight, '2026-09-10T11:00:00-04:00', '2026-09-10T15:00:00.0Z'])(
    'matches equivalent instant %s',
    async (value) => {
  ...
  it('applies every range and not-equal operator with millisecond boundaries', async () => {
    ...
    expect(await names({ $and: [{ at: { $gte: midnight } }, { at: { $lt: '2026-09-10T15:00:00.001Z' } }] })).toEqual([
      'exact',
    ]);
  });
  ...
  it('uses column metadata for system timestamps, row permissions, count and aggregate', async () => {
    ...
    await db.run({ roles: ['timestamp_reader'] }, async (ctx) => {
      ...
      const result = await repo(ctx, Event).aggregate({
        where: { at: { $gte: midnight } },
        metrics: { count: { count: true } },
      });
  ...
  it('rejects malformed datetime operands with structured paths instead of driver errors', async () => {
    const invalid: DomainScalar[] = [
      'not a timestamp',
      '2026-02-30T00:00:00Z',
      '2026-09-11T00:00:00',
      '2026-09-11',
      '2026-09-11T24:00:00Z',
      '2026-09-11T00:00:00+99:00',
      '2026-09-11T00:00:00.0001Z',
      '',
      0,
      true,
    ];
    for (const value of invalid) {
      await expect(names({ at: value })).rejects.toMatchObject({
        code: 'VALIDATION',
        details: { issues: [{ path: 'where.at' }] },
      });
      await expect(names({ at: { $gte: value } })).rejects.toMatchObject({
        code: 'VALIDATION',
        details: { issues: [{ path: 'where.at.$gte' }] },
      });
      await expect(names({ at: { $in: [midnight, value] } })).rejects.toMatchObject({
        code: 'VALIDATION',
        details: { issues: [{ path: 'where.at.$in.1' }] },
      });
    }
    await expect(names({ at: { $like: '2026%' } })).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(names({ at: { $in: 'not-an-array' } } as unknown as Domain)).rejects.toMatchObject({
      code: 'VALIDATION',
      details: { issues: [{ path: 'where.at.$in' }] },
    });
  });
});
