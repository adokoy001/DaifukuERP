...
const NoExt = defineEntity({
  name: 'ext_unit_plain',
  label: label('ext なし', 'No ext'),
  ext: false,
  fields: { name: f.text() },
  permissions: { roles: { clerk: ['read'] } },
});
const FreeExt = defineEntity({
  name: 'ext_unit_free',
  label: label('ext 未登録', 'Unregistered ext'),
  fields: { name: f.text() },
  permissions: { roles: { clerk: ['read'] } },
});
...
describe('registerExt (AC-1, AC-12)', () => {
  it('AC-1 stores fields per entity with their source; extVersion changes on each registration', () => {
    ...
    registry.registerExt(
      ExtItem.name,
      { jan: f.text({ searchable: true, label: label('JAN', 'JAN'), maxLength: 13 }) },
      { source: 'pack_a' },
    );
  ...
  it('AC-1 the same key twice for one entity -> Conflict whose hint names both sources; nothing from that call is added', () => {
    const e = caught(() =>
      registry.registerExt(ExtItem.name, { color: f.text(), jan: f.text() }, { source: 'pack_c' }),
    );
  ...
  it('AC-12 rejects names colliding with system/document/entity fields, bad names, non-JSONB kinds and column-only options (ValidationError)', () => {
    const cases: [string, Record<string, AnyField>, string, string][] = [
      ...
      [
        TPartner.name,
        { items: { kind: 'lines', required: false, hasDefault: false, opts: {} } as unknown as AnyField },
        'ext.items',
    ...
    expect(() =>
      registry.registerExt(TPartner.name, { parentPartnerId: f.ref('test_partner') }, { source: 'pack_ok' }),
    ).not.toThrow();
  ...
  it('searchable is refused on entity fields (it only means something for ext fields)', () => {
    expect(() =>
      defineEntity({
        name: 'ext_unit_bad_search',
        label: label('x', 'x'),
        fields: { name: f.text({ searchable: true }) },
        permissions: { roles: { clerk: ['read'] } },
      }),
    ).toThrow(/views\.search/);
  });
});
...
describe('ext validation schema (AC-2)', () => {
  it('AC-2 registered keys are validated with the field zod; unknown keys are kept; decimals become canonical strings', () => {
    registry.registerExt(
      TMemoLine.name,
      {
        unitCost: f.money({ min: '0' }),
        lot: f.text({ normalize: 'upper' }),
        best: f.date(),
        supplierRef: f.ref('test_partner'),
      },
      { source: 'pack_lines' },
    );
    const ok = validateExt(
      TMemoLine,
      {
        unitCost: '12.50',
        lot: 'ab-1',
        best: '2026-12-31',
        supplierRef: '01890000-0000-7000-8000-000000000000',
        note: { free: true },
      },
      'insert',
    );
    ...
    const bad = caught(() =>
      validateExt(TMemoLine, { unitCost: 12.5, best: '2026-02-30', supplierRef: 'nope', lot: null }, 'insert'),
    );
    ...
    expect(
      issuesOf(bad)
        .map((i) => i.path)
        .sort(),
    ).toEqual(['ext.best', 'ext.supplierRef', 'ext.unitCost']);
  ...
  it('AC-2 required ext fields: insert without ext fails per key; update without ext leaves ext alone; entities without registrations pass through', () => {
    ...
    expect(issuesOf(caught(() => validateExt(ExtItem, { jan: '490' }, 'insert')))).toEqual([
      { path: 'ext.grade', message: expect.any(String) },
    ]);
...
describe('ext in meta and action inputs (AC-3)', () => {
  ...
  it('AC-3 generic create/update inputs (OpenAPI/MCP schemas) document registered ext keys, including ones registered after registerCrudActions', () => {
    const props = (s: z.ZodType) =>
      z.toJSONSchema(s, { io: 'input', unrepresentable: 'any' }) as {
    ...
    const lineItems = ((
      memo.properties?.lines?.properties?.test_memo_line as {
        items?: { properties?: Record<string, { properties?: Record<string, unknown> }> };
      }
    ).items?.properties ?? {}) as Record<string, { properties?: Record<string, unknown> }>;
    ...
    expect(Object.keys(props(registry.action('test_partner.create').input).properties?.ext?.properties ?? {})).toEqual([
      'parentPartnerId',
    ]);
    // ext column but nothing registered: the free-form record as before; no ext column: no ext property
    const free = props(registry.action(`${FreeExt.name}.create`).input).properties?.ext as
      { properties?: unknown; additionalProperties?: unknown } | undefined;
...
describe('currency scale (AC-11)', () => {
  ...
  it('AC-11 money FieldMeta carries scale from the company currency (explicit scale wins); quantities/rates are not money', () => {
    ...
    expect(entityMeta(ctx, TMemo, { currency: 'KRW' }).fields.find((x) => x.name === 'amount')).toMatchObject({
      money: true,
      scale: 0,
    });
  });
