...
const roles = {
  chain_staff: ['read', 'create', 'update', 'delete'] as const,
  chain_manager: ['read', 'create', 'update', 'delete', 'export'] as const,
};
...
const Hidden = defineEntity({
  name: 'access_hidden',
  label: label('金融', 'Finance'),
  fields: { name: f.text({ required: true }) },
  permissions: { roles },
});
const Closing = defineDocument({
  name: 'access_closing',
  label: label('締め', 'Closing'),
  naming: { type: 'sequence', prefix: 'ACL-' },
  fields: { storeId: f.ref(Store.name, { required: true }), amount: f.money({ required: true }) },
  permissions: { roles },
  storeAccess: { kind: 'store', field: 'storeId' },
  lines: [{ entity: 'access_line', parentField: 'closingId' }],
});
const Line = defineEntity({
  name: 'access_line',
  label: label('明細', 'Line'),
  fields: { closingId: f.ref(Closing.name, { required: true }), name: f.text({ required: true }) },
  permissions: { roles },
  storeAccess: { kind: 'parent', field: 'closingId', entity: Closing.name },
});
const report = defineAction({
  name: 'access.report',
  description: label('集計', 'Report'),
  input: z.object({}),
  output: z.object({ count: z.number() }),
  permission: { entity: Closing.name, op: 'read' },
  tx: 'none',
  storeAccess: true,
  exportEntities: [Closing.name],
  handler: async (ctx) => ({ count: await repo(ctx, Closing).count() }),
});
defineAction({
  name: 'access.undeclared',
...
describe('company/store boundary applies below every transport', () => {
  it('filters list, count, aggregate, search, get, child rows and audit, even with admin role', async () => {
    ...
    await expect(
      db.run({ ...limited(), roles: ['admin'] }, (ctx) => auditTrail(ctx, Closing.name, closingB)),
    ).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
  });
  it('rejects cross-store create, record move, child reparent and direct posting', async () => {
    await expect(
      db.run(limited(), (ctx) => repo(ctx, Closing).create({ storeId: storeB, amount: '1' })),
    ).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(
      db.run(limited(), (ctx) => repo(ctx, Closing).update(closingA, { storeId: storeB })),
    ).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await expect(
      db.run(limited(), (ctx) => repo(ctx, Line).update(lineA, { closingId: closingB })),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
    await expect(
      db.run({ ...limited(), roles: ['admin'] }, (ctx) => submitDocument(ctx, Closing, closingA)),
    ).rejects.toMatchObject({ code: 'PERMISSION_DENIED' });
    await db.run({}, async (ctx) => {
      expect((await repo(ctx, Closing).get(closingA)).storeId).toBe(storeA);
    });
  });
  it('fails closed for empty assignments, undeclared entities/actions, and shared writes', async () => {
    await db.run({ ...limited(), storeIds: [] }, async (ctx) => {
      expect(await repo(ctx, Closing).count()).toBe(0);
    });
    ...
    await expect(db.run(limited(), (ctx) => runAction(ctx, 'access.undeclared', {}))).rejects.toMatchObject({
      code: 'PERMISSION_DENIED',
    });
    await db.run(limited(), async (ctx) => {
    ...
    await db.run({ ...limited(), roles: ['chain_staff'] }, async (ctx) => {
      expect(() => checkActionExport(ctx, report)).toThrow();
    });
  });
  it('rejects caller-forged store ids in other companies through normal company scope', async () => {
    const otherCompany = newId();
    await db.owner
      .sql`insert into companies (id, tenant_id, code, name) values (${otherCompany}, ${db.tenantId}, 'O', 'Other')`;
    await db.run({ ...limited(), companyId: otherCompany }, async (ctx) => {
      expect(await repo(ctx, Closing).count()).toBe(0);
    });
  });
});
