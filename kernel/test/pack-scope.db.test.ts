...
let db: TestDb;
let companyB: string;
const PackItem = defineEntity({
  name: 'test_pack_item',
  label: label('専用', 'Pack item'),
  fields: { name: f.text({ required: true }) },
  permissions: { roles: { viewer: ['read'] } },
});
const packAction = defineAction({
  name: 'scope_pack.check',
  description: label('確認', 'Check'),
  input: z.object({}),
...
definePack({
  name: 'scope_pack',
  label: label('会社別', 'Company scoped'),
  depends: ['test'],
  entities: [PackItem],
  actions: [packAction],
  ...
  hooks: () =>
    registry.registerHook(TPartner.name, 'before_validate', (_ctx, { row }) => {
...
it('loaded but unapplied pack cannot change core required/default fields, hooks or metadata in another company', async () => {
  await expect(db.run({}, (ctx) => repo(ctx, PackItem).create({ name: 'unapplied' }))).rejects.toMatchObject({
    code: 'PERMISSION_DENIED',
  });
  await expect(db.run({}, (ctx) => runAction(ctx, 'scope_pack.check', {}))).rejects.toMatchObject({
    code: 'PERMISSION_DENIED',
  });
  ...
  await expect(db.run({}, (ctx) => repo(ctx, TPartner).create({ name: 'missing tag' }))).rejects.toMatchObject({
    code: 'VALIDATION',
  });
  await expect(
    db.run({}, (ctx) => repo(ctx, TPartner).create({ name: 'blocked-by-pack', ext: { requiredTag: 'ok' } })),
  ).rejects.toThrow('pack guard');
  ...
  expect(await db.run({}, async (ctx) => entityMeta(ctx, TPartner).extFields.map((field) => field.name))).toEqual([
    'ext.requiredTag',
    'ext.defaultTag',
  ]);
  expect(await db.run({ companyId: companyB }, async (ctx) => entityMeta(ctx, TPartner).label)).toEqual(
    TPartner.config.label,
  );
});
...
it('an administrator choosing the module default is preserved unless force is explicitly requested', async () => {
  ...
  expect(await db.run({ companyId: companyB }, (ctx) => getSetting(ctx, 'scoped.mode', z.string(), 'missing'))).toBe(
    'module-default',
  );
