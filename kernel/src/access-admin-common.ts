...
export const createUserSchema = z
  .object({
    ...
    tenantAdmin: z.boolean().default(false),
  })
  .strict();
export const updateUserSchema = z
  .object({
    ...
    tenantAdmin: z.boolean().optional(),
...
export const membershipSchema = z
  .object({
    ...
    roles: z
      ...
      .min(1)
      .max(50),
    accessScope: z.enum(['all', 'stores', 'sites']),
    storeIds: z.array(z.uuid()).max(500),
...
export function actorUser(ctx: Context): string {
  return ctx.actor.type === 'agent' ? (ctx.actor.onBehalfOf ?? '') : ctx.actor.id;
}
...
export async function assertAccessAdmin(ctx: Context, lock = false): Promise<void> {
  if (lock)
    await ctx.db.execute(sql`select pg_advisory_xact_lock(hashtextextended(${`access-admin:${ctx.tenantId}`}, 0))`);
  const [actor] = await ctx.db
    .select({ active: users.active, admin: users.tenantAdmin })
    .from(users)
...
export async function managedUser(ctx: Context, id: string) {
  const [user] = await ctx.db
    .select()
    .from(users)
    .where(and(eq(users.tenantId, ctx.tenantId), eq(users.id, id)))
    .limit(1);
...
export function assertAccessVersion(actual: number, expected: number): void {
  if (actual !== expected)
    throw new Conflict(
      'Access settings were changed concurrently',
      'Reload the user and company assignments before saving.',
    );
}
export async function managedCompany(ctx: Context, id: string) {
  const [company] = await ctx.db
    .select()
    .from(companies)
    .where(and(eq(companies.tenantId, ctx.tenantId), eq(companies.id, id)))
    .limit(1);
...
export function availableRoles(): string[] {
  ...
  for (const a of registry.allActions())
    if (a.permission !== 'authenticated' && 'roles' in a.permission)
      for (const role of a.permission.roles) roles.add(role);
  roles.delete('relay');
  return [...roles].sort();
}
export async function directoryStores(ctx: Context, companyId?: string, kind: 'stores' | 'sites' = 'stores') {
  ...
  for (const e of registry.allEntities()) {
    ...
    const rows = await ctx.db
      .select({ id: e.col('id'), companyId: e.col('companyId'), name: e.col(e.displayField ?? 'id') })
      .from(e.table)
      .where(and(eq(e.col('tenantId'), ctx.tenantId), companyId ? eq(e.col('companyId'), companyId) : undefined));
    for (const row of rows)
      items.push({ id: String(row.id), companyId: String(row.companyId), name: String(row.name) });
  }
  return items;
}
