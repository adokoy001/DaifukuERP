...
export interface MailIdentityOptions {
  encryptionKey: string;
  webUrl: string;
  mailConfigured: boolean;
}
function messageLink(config: MailIdentityOptions, path: string, token: string): string {
...
export async function inviteIdentity(
  ...
  input: { email: string; name: string; stepUpToken: string },
  config: MailIdentityOptions,
) {
  await assertAccessAdmin(ctx, true);
  await lockIdentity(ctx, sessionVersion);
  await consumeStepUp(ctx, input.stepUpToken, sessionVersion);
  ...
  if (existing) {
    const memberships = await ctx.db
      ...
      .where(and(eq(companyMemberships.tenantId, ctx.tenantId), eq(companyMemberships.userId, existing.id)))
      .limit(1);
    const invites = await ctx.db
      ...
      .where(
        and(
          ...
          eq(identityChallenges.userId, existing.id),
          eq(identityChallenges.purpose, 'invitation'),
        ),
      )
      .limit(1);
    ...
    await revokeIdentity(ctx, existing);
    user = { ...existing, version: existing.version + 1, sessionVersion: existing.sessionVersion + 1 };
  } else {
  ...
  await queueIdentityMail(
    ctx,
    config.encryptionKey,
    {
      to: email,
      subject: 'Daifuku invitation',
      text: `You have been invited to Daifuku. Set your password within 24 hours: ${messageLink(config, '/accept-invitation', token)}\nIf unexpected, ignore this message.`,
    },
  ...
  return {
    ok: true as const,
    userId: user.id,
    delivery: config.mailConfigured ? ('queued' as const) : ('unconfigured' as const),
  };
}
export async function requestIdentityReset(
  owner: Database,
  email: string,
  ...
  const rows = await owner.drizzle
    ...
    .limit(2);
  const user = rows[0];
  if (rows.length !== 1 || !user) return;
  await withContext(
    owner,
    { tenantId: user.tenantId, companyId: null, actor: { type: 'user', id: user.id }, roles: [], now: () => now },
    async (ctx) => {
      ...
      await queueIdentityMail(
        ...
        {
          to: current.email,
          subject: 'Daifuku password reset',
          text: `Reset your password within 30 minutes: ${messageLink(config, '/reset-password', token)}\nIf you did not request this, ignore it. MFA remains enabled.`,
        },
...
export async function completeIdentityToken(
  owner: Database,
  token: string,
