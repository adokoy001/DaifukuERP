...
export async function linkedIdentity(
  ctx: Context,
  provider: { id: string; issuer: string },
  subject: string,
  ...
  const [link] = await ctx.db
    ...
    .where(
      and(
        eq(identityLinks.tenantId, ctx.tenantId),
        eq(identityLinks.providerId, provider.id),
        eq(identityLinks.issuer, provider.issuer),
        eq(identityLinks.subject, subject),
      ),
    )
    .limit(1);
...
export async function linkIdentity(
  ...
  await ctx.db.insert(identityLinks).values({
    id: newId(),
  ...
  await authAudit(ctx, user.id, 'oidc_linked');
  return { linked: true };
}
export async function unlinkIdentity(ctx: Context, sessionVersion: number, providerId: string, stepUpToken: string) {
  ...
  const deleted = await ctx.db
    ...
    .where(
      and(
        eq(identityLinks.tenantId, ctx.tenantId),
        eq(identityLinks.userId, user.id),
        eq(identityLinks.providerId, providerId),
      ),
    )
    .returning({ id: identityLinks.id });
  if (!deleted.length) throw identityDenied();
  await revokeIdentity(ctx, user);
  await authAudit(ctx, user.id, 'oidc_unlinked');
