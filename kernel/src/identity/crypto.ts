...
export function encryptionKey(encoded: string): Buffer {
  ...
  if (key.length !== 32 || key.toString('base64') !== encoded)
    throw new StateError('Identity encryption key is invalid.', 'Configure a base64 encoded 32-byte key.');
  return key;
}
...
export function unseal(value: string, key: string, purpose: string): string {
  ...
  if (!iv || iv.length !== 12 || !tag || tag.length !== 16 || !data)
...
export function matchingTotpStep(secret: string, code: string, now: Date, lastStep = -1): number | null {
  ...
  for (const step of [current, current - 1, current + 1])
    if (step >= 0 && step > lastStep && timingSafeEqual(Buffer.from(totp(secret, step)), Buffer.from(code)))
      return step;
  return null;
}
