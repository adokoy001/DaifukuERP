...
export interface IdentityAttemptLimit {
  key: string;
  limit: number;
  seconds?: number;
}
...
async function reserve(owner: Database, input: IdentityAttemptLimit, now: Date): Promise<Reservation> {
  ...
  const [row] = await owner.drizzle
    ...
    .onConflictDoUpdate({
      ...
      set: {
        windowStart: start,
        attempts: sql`case when ${identityLimits.windowStart} = ${start.toISOString()} then ${identityLimits.attempts} + 1 else 1 end`,
      },
...
async function release(owner: Database, reservations: Reservation[]): Promise<void> {
  for (const reservation of reservations)
    await owner.drizzle
      .update(identityLimits)
      .set({ attempts: sql`greatest(0, ${identityLimits.attempts} - 1)` })
      .where(and(eq(identityLimits.key, reservation.key), eq(identityLimits.windowStart, reservation.start)));
}
...
export async function withIdentityAttempt<T>(
  owner: Database,
  limits: IdentityAttemptLimit[],
  work: () => Promise<T>,
  now = new Date(),
): Promise<T> {
  const reservations: Reservation[] = [];
  try {
    for (const limit of limits) reservations.push(await reserve(owner, limit, now));
  } catch (error) {
