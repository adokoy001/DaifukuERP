...
export function checkActionPermission(ctx: Context, action: ActionDef): void {
  if (ctx.actor.type === 'relay' && action.relayAccess !== true)
    throw new PermissionDenied(action.name, 'relay-action', ctx.roles);
  if (ctx.accessScope === 'stores' && !action.generic && action.storeAccess !== true)
    throw new PermissionDenied(action.name, 'store-action', ctx.roles);
  if (
    ctx.accessScope === 'sites' &&
    !action.generic &&
    action.siteAccess !== true &&
    !(action.storeAccess === true && ctx.storeIds?.length)
  )
    throw new PermissionDenied(action.name, 'site-action', ctx.roles);
  if (registry.hasPack(action.module) && !(ctx.appliedPacks ?? []).includes(action.module))
    throw new PermissionDenied(action.name, 'pack-not-applied', ctx.roles);
...
export function checkActionExport(ctx: Context, action: ActionDef): void {
  ...
  const entities =
    ...
    (action.generic && action.permission !== 'authenticated' && 'entity' in action.permission
...
export async function runAction(ctx: Context, name: string, rawInput: unknown): Promise<unknown> {
  ...
  if (!action) {
    throw new DaifukuError(
      ...
      `List available actions with GET /meta or the MCP tool list.`,
      { action: name },
      404,
    );
  }
  ...
  if (action.inputMode === 'lenient') {
    if (typeof input !== 'object' || input === null || Array.isArray(input)) {
      throw new ValidationError(
        ...
        'Send a JSON object; see the input schema in the action metadata.',
      );
    }
  } else {
    ...
    if (!parsed.success) {
      ...
      throw new ValidationError(
        `invalid input for ${name}`,
        issues,
        'See details.issues; the input schema is in the action metadata.',
      );
    }
    input = parsed.data;
  }
  ...
  if (!out.success) {
    ...
    throw new DaifukuError(
      'INTERNAL',
      `action ${name} returned an invalid result`,
      'This is a bug in the action; its output does not match its declared schema.',
