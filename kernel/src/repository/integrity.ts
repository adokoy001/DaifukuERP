...
export async function duringLifecycle<T>(
  ctx: Context,
  entity: string,
  id: string,
  op: Op,
...
export async function validateReferences(ctx: Context, entity: EntityDef, row: Raw): Promise<void> {
  ...
  const refs = [
    ...Object.entries(entity.config.fields).map(([field, fd]) => ({ field, fd, value: row[field] })),
    ...registry
      .extFields(entity.name)
      .filter((d) => !d.source.startsWith('pack:') || ctx.appliedPacks?.includes(d.source.slice(5)))
      .map((d) => ({ field: `ext.${d.key}`, fd: d.field, value: ext[d.key] })),
  ];
  for (const { field, fd, value } of refs) {
    ...
    const found = await ctx.db
      .select({ id: target.col('id') })
      .from(target.table)
      .where(and(scopeCondition(ctx, target), eq(target.col('id'), value as string)))
      .limit(1)
      .for('key share');
    if (!found.length)
      throw new ValidationError(`${entity.name}.${field}: reference is outside the current scope or missing`, [
        { path: field, message: 'reference must exist in the same tenant and company' },
      ]);
  }
}
...
async function lockedParent(ctx: Context, entity: EntityDef, id: string, op: Op): Promise<Raw> {
  ...
  const rows = await ctx.db
    .select()
    .from(entity.table)
    .where(and(scopeCondition(ctx, entity), rowFilter(ctx, entity, op), eq(entity.col('id'), id)))
    .limit(1)
    .for('update');
...
export async function lockParents(
  ctx: Context,
  entity: EntityDef,
  row: Raw,
  ...
  const owners = registry
    .allEntities()
    .flatMap((doc) =>
      (doc.doc?.lines ?? []).filter((l) => l.entity === entity.name).map((l) => ({ doc, field: l.parentField })),
    );
  ...
  for (const { doc, field } of owners) {
    ...
    for (const id of ids) {
      if (previous && row[field] !== previous[field])
        throw new ValidationError('A line cannot be moved to another document', [
          { path: field, message: 'create a new line under the other parent' },
        ]);
      ...
      const same =
        operation === 'update' &&
        Object.keys(input).every((key) => JSON.stringify(input[key]) === JSON.stringify(previous?.[key]));
      const internal =
        parent.docstatus === DOCSTATUS.submitted &&
        hasWriteCapability(ctx, entity.name, operation) &&
        Object.keys(input).every((key) => ownsField(ctx, entity.name, key));
      if (parent.docstatus !== DOCSTATUS.draft && !same && !internal)
        throw new StateError(
          `${doc.name} ${id}: submitted/cancelled lines are frozen`,
          'Use the owning module operation, or cancel and amend.',
        );
      if (operation === 'create') {
        const total = await ctx.db
          .select({ n: count() })
          .from(entity.table)
          .where(and(scopeCondition(ctx, entity), eq(entity.col(field), id)));
        if ((total[0]?.n ?? 0) >= MAX_LINES)
          throw new ValidationError(`${doc.name} exceeds ${MAX_LINES} lines`, [
            { path: 'lines', message: `maximum ${MAX_LINES} lines` },
          ]);
      } else if ((await lineCount(ctx, entity, field, id)) > MAX_LINES)
        throw new StateError(
          `${doc.name} exceeds ${MAX_LINES} lines`,
          'Repair the oversized document before changing it.',
        );
...
export async function lineCount(ctx: Context, entity: EntityDef, parentField: string, id: string): Promise<number> {
  const rows = await ctx.db
    .select({ n: count() })
    .from(entity.table)
    .where(and(scopeCondition(ctx, entity), eq(entity.col(parentField), id)));
  return rows[0]?.n ?? 0;
}
...
export async function assertAggregateLimit(ctx: Context, doc: EntityDef, id: string): Promise<void> {
  for (const spec of doc.doc?.lines ?? []) {
    if ((await lineCount(ctx, registry.entity(spec.entity), spec.parentField, id)) > MAX_LINES)
      throw new StateError(
        `${doc.name} exceeds ${MAX_LINES} lines`,
        'Repair the oversized document before changing it.',
      );
...
export async function touchParents(ctx: Context, parents: ParentLock[]): Promise<void> {
  for (const parent of parents) {
    if (
      isReplacing(ctx, parent.entity.name, parent.id) ||
      lifecycles.get(contextRoot(ctx))?.has(aggregateKey(parent.entity.name, parent.id))
    )
      continue;
    ...
    const rows = await ctx.db
      .update(entity.table)
      .set({ version: parent.version + 1, updatedAt: ctx.now(), updatedBy })
      .where(
        and(scopeCondition(ctx, entity), eq(entity.col('id'), parent.id), eq(entity.col('version'), parent.version)),
      )
      .returning();
    if (!rows[0]) throw new Conflict('Aggregate was modified concurrently', 'Reload the parent document.');
    if (entity.audit === 'full')
      await writeAudit(ctx, entity.name, parent.id, 'update', snapshot(latest), snapshot(rows[0] as Raw));
  }
}
