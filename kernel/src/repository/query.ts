...
export const listQuerySchema = z.object({
  ...
  orderBy: z.array(z.object({ field: z.string(), dir: z.enum(['asc', 'desc']).optional() })).max(5).optional(),
...
export function searchCondition(ctx: Context, entity: EntityDef, search: string | undefined): SQL | undefined {
  ...
  const textFields = fields.filter((f) => entity.config.fields[f]?.kind === 'text' && !hidden.has(f) && !entity.config.fields[f]?.opts.outputHidden);
...
export function whereCondition(ctx: Context, entity: EntityDef, where: Domain | undefined): SQL | undefined {
  ...
  for (const key of Object.keys(where)) {
    if (key === '$and' || key === '$or') {
      ...
      if (!Array.isArray(children)) throw new ValidationError('invalid where group', [{ path: key, message: 'array required' }]);
    ...
    if (!(key in entity.config.fields) && !(key in entity.columns)) {
      throw new ValidationError(`unknown field "${key}" in where`, [{ path: `where.${key}`, message: 'unknown field' }], `Valid fields: ${entity.fieldNames.join(', ')}`);
    }
  }
