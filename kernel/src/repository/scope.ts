...
export function scopeCondition(ctx: Context, e: EntityDef): SQL {
  const relay =
    ctx.actor.type === 'relay'
      ? e.config.relayAccess && ctx.relay?.gatewayId === ctx.actor.id
        ? eq(e.col(e.config.relayAccess.field), ctx.actor.id)
        : sql`false`
      : undefined;
  ...
  if (!ctx.companyId)
    throw new StateError(
      `${e.name} is company-scoped but the context has no company`,
      'Select a company (companyId) in the context.',
    );
...
export function assertRelayWrite(ctx: Context, e: EntityDef, row: Record<string, unknown>): void {
  ...
  if (
    !e.config.relayAccess ||
    !ctx.relay ||
    ctx.relay.gatewayId !== ctx.actor.id ||
    row[e.config.relayAccess.field] !== ctx.actor.id
  )
    throw new StateError('Relay write is outside its gateway.', 'Use the bound gateway workflow.');
}
