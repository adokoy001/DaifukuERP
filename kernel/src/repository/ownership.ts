...
export function assertOwnedInput(ctx: Context, entity: EntityDef, input: Record<string, unknown>, operation: 'create' | 'update', previous: Record<string, unknown> = {}): void {
  const blocked = Object.keys(input).filter((key) => entity.config.fields[key]?.opts.serverOwned && !ownsField(ctx, entity.name, key, operation));
  if (input.ext && typeof input.ext === 'object') {
    ...
    for (const { key, field } of registry.extFields(entity.name)) {
      if (field.opts.serverOwned && JSON.stringify(next[key]) !== JSON.stringify(prior[key]) && !ownsField(ctx, entity.name, `ext.${key}`, operation)) blocked.push(`ext.${key}`);
    }
  }
