...
export function can(
  ctx: Pick<Context, 'roles' | 'appliedPacks' | 'accessScope'> & Partial<Pick<Context, 'actor'>>,
  entity: EntityDef,
  op: Op,
): boolean {
  if (
    ctx.actor?.type === 'relay' &&
    (!entity.config.relayAccess?.operations.includes(op) || !grantedRoles(ctx, entity, op).includes('relay'))
  )
    return false;
  if (entity.module && registry.hasPack(entity.module) && !(ctx.appliedPacks ?? []).includes(entity.module))
    return false;
  if (!storeAllows(ctx, entity, op)) return false;
  if (
    ctx.accessScope &&
    ctx.accessScope !== 'all' &&
    effectiveSitePolicy(ctx, entity)?.kind === 'sharedRead' &&
    op === 'read'
  )
    return true;
...
function compileIn(ctx: Context, col: PgColumn, field: string, list: readonly DomainScalar[]): SQL {
  if (!Array.isArray(list))
    throw new ValidationError('Invalid search membership', [{ path: `where.${field}.$in`, message: 'array required' }]);
...
function compileSingleCondition(ctx: Context, entity: EntityDef, field: string, cond: DomainCondition): SQL {
  ...
  const value = (operand: DomainScalar, op?: string) =>
    queryScalar(col, substitute(ctx, operand), `where.${field}${op ? `.${op}` : ''}`);
  ...
  if ('$like' in cond) {
    if (isTimestampColumn(col))
      throw new ValidationError('Pattern searches do not support timestamps', [
        { path: `where.${field}.$like`, message: 'Use equality or a timestamp range.' },
      ]);
...
function compileCondition(ctx: Context, entity: EntityDef, field: string, condition: DomainCondition): SQL {
  ...
  const parts = entries.map(([op, operand]) =>
    compileSingleCondition(ctx, entity, field, { [op]: operand } as DomainCondition),
  );
  ...
  for (const [key, value] of Object.entries(domain)) {
    ...
    if (key === '$or' || key === '$and') {
      if (!Array.isArray(value))
        throw new ValidationError('invalid domain group', [{ path: key, message: 'array required' }]);
      const subs = (value as Domain[]).map((d) => compileDomain(ctx, entity, d) ?? sql`true`);
      if (subs.length === 0) {
        parts.push(key === '$or' ? sql`false` : sql`true`);
        continue;
      }
    ...
    if (Array.isArray(value))
      throw new ValidationError(`field ${key} cannot take an array`, [{ path: key, message: 'use $in' }]);
...
export function assertReadableFields(ctx: Pick<Context, 'roles'>, entity: EntityDef, keys: readonly string[]): void {
  ...
  const blocked = keys.filter(
    (k) => masked.has(k) || hidden.has(k) || (k === 'ext' && hiddenExt) || (k.startsWith('ext.') && masked.has('ext')),
  );
