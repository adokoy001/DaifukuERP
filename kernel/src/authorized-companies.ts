...
function denied(ctx: Context): never {
  throw new PermissionDenied('company', 'cross-company', ctx.roles);
}

async function liveUser(ctx: Context) {
  if (ctx.accessScope !== 'all') denied(ctx);
  const userId =
    ctx.actor.type === 'user' ? ctx.actor.id : ctx.actor.type === 'agent' ? ctx.actor.onBehalfOf : undefined;
...
export async function authorizedCompanies(ctx: Context): Promise<AuthorizedCompany[]> {
  ...
  const memberships = await ctx.db
    ...
    .where(and(eq(companyMemberships.tenantId, ctx.tenantId), eq(companyMemberships.userId, user.id)))
    .for('share');
...
export async function withAuthorizedCompany<T>(
  ctx: Context,
  companyId: string,
  work: (company: Context) => Promise<T>,
): Promise<T> {
  const user = await liveUser(ctx);
  ...
  const [company] = await ctx.db
    .select()
    .from(companies)
    .where(and(eq(companies.tenantId, ctx.tenantId), eq(companies.id, companyId)))
    .limit(1);
  ...
  return work(
    makeContext(ctx.db, {
      ...
      storeIds: [],
      siteIds: [],
      appliedPacks: appliedPackNames(company.settings),
