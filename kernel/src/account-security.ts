...
function expiredSession(): DaifukuError {
  return new DaifukuError(
    ...
    'Log in again before changing account security.',
    undefined,
    401,
  );
}
...
export async function changeOwnPassword(ctx: Context, sessionVersion: number, raw: unknown): Promise<{ ok: true }> {
  ...
  if (input.currentPassword === input.newPassword)
    throw new ValidationError('Choose a different password.', [
      { path: 'newPassword', message: 'The new password must differ from the current password.' },
    ]);
  await ctx.db
    .update(users)
    .set({
  ...
  await writeAccessAudit(ctx, null, 'access_user', user.id, 'password_change', null, {
...
export async function revokeOwnSessions(ctx: Context, sessionVersion: number, raw: unknown): Promise<{ ok: true }> {
  ...
  await ctx.db
    .update(users)
    .set({ version: user.version + 1, sessionVersion: user.sessionVersion + 1 })
    .where(and(eq(users.tenantId, ctx.tenantId), eq(users.id, user.id)));
