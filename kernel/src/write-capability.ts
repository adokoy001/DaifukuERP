...
export function defineWriteCapability(config: {
  name: string;
  entity: string;
...
export async function withWriteCapability<T>(
  ctx: Context,
  ...
  const child = inheritContext(
    ctx,
    Object.create(Object.getPrototypeOf(ctx), Object.getOwnPropertyDescriptors(ctx)) as Context,
  );
...
export function ownsField(ctx: Context, entity: string, field: string, operation?: string): boolean {
  return (grants.get(ctx) ?? []).some(
    (c) =>
      c.entity === entity && c.fields.includes(field) && (operation === undefined || c.operations.includes(operation)),
  );
}
