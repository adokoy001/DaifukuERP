...
export async function loadPrincipal(owner: Database, userId: string): Promise<Principal | null> {
  const rows = await owner.drizzle.select().from(users).where(and(eq(users.id, userId), eq(users.active, 1))).limit(1);
  const user = rows[0];
  if (!user) return null;
  return principalFrom(owner, user);
}
...
export async function bootstrapTenant(owner: Database, input: BootstrapInput): Promise<{ tenantId: string; companyId: string; userId: string }> {
  ...
  return owner.drizzle.transaction(async (tx) => {
    ...
    const existing = await tx.select().from(users).where(and(eq(users.tenantId, tenantId), eq(users.email, input.adminEmail.toLowerCase()))).limit(1);
    const found = existing[0];
    if (found) {
      const company = found.defaultCompanyId ? await tx.select({ id: companies.id }).from(companies).where(and(eq(companies.tenantId, tenantId), eq(companies.id, found.defaultCompanyId))).limit(1) : [];
      if (!company[0]) throw new StateError('Existing tenant administrator has no valid default company', 'Repair the company assignment before provisioning again.');
