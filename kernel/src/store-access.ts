...
export function registerStoreAccess(entity: string, policy: StoreAccessPolicy): void {
  ...
  if (existing && JSON.stringify(existing) !== JSON.stringify(policy)) throw new StateError(`Conflicting store access policy for ${entity}`, 'Declare exactly one store boundary per entity.');
  additions.set(def, policy);
}
export function storePolicy(entity: EntityDef): StoreAccessPolicy | undefined { return entity.config.storeAccess ?? additions.get(entity); }
export function effectiveSitePolicy(ctx: Pick<Context, 'accessScope'>, entity: EntityDef): StoreAccessPolicy | undefined {
...
export function storeCondition(ctx: Context, entity: EntityDef, seen = new Set<string>()): SQL | undefined {
  ...
  if (p.kind === 'store') return scopeIds(ctx, entity).length ? inArray(entity.col(p.field), [...scopeIds(ctx, entity)]) : sql`false`;
  const parent = registry.entity(p.entity);
  const scope = and(eq(parent.col('tenantId'), ctx.tenantId), parent.scope === 'company' ? eq(parent.col('companyId'), ctx.companyId ?? '') : undefined, storeCondition(ctx, parent, next));
...
export async function assertStoreWrite(ctx: Context, entity: EntityDef, row: Record<string, unknown>): Promise<void> {
  ...
  if (p.kind === 'store') {
    if (!scopeIds(ctx, entity).includes(String(row[p.field]))) throw new PermissionDenied(entity.name, 'site-write', ctx.roles);
    return;
  }
  const parent = registry.entity(p.entity);
  const found = await ctx.db.select({ id: parent.col('id') }).from(parent.table).where(and(eq(parent.col('tenantId'), ctx.tenantId), parent.scope === 'company' ? eq(parent.col('companyId'), ctx.companyId ?? '') : undefined, storeCondition(ctx, parent), eq(parent.col('id'), String(row[p.field])))).limit(1);
